...
def discover_service_scopes(
    *,
    ...
    guides_by_service = discover_service_guides(
        session=session, main_landing_url=main_landing_url
    )

    return {
        service_id: ServiceScope(guides=guides)
...
def load_service_manifest(path: Path) -> dict[str, ServiceScope]:
    ...
    for entry in services_raw:
        ...
        scopes[service_id] = ServiceScope(guides=tuple(sorted(guides, key=lambda guide: guide.url)))

    if not scopes:
        raise ValueError(f"No services were loaded from manifest {path}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "workflow. If not provided or missing, the crawler falls back to "
...
def _resolve_cli_scope(args: argparse.Namespace) -> tuple[list[str], list[str]]:
    ...
    if args.manifest:
        try:
            manifest_scopes = load_service_manifest(args.manifest)
        except FileNotFoundError:
            LOGGER.warning(
                "Manifest %s not found; falling back to live discovery.", args.manifest
            )
        except ValueError as exc:
            LOGGER.warning(
                "Failed to load manifest %s (%s); falling back to live discovery.",
                args.manifest,
                exc,
            )
        else:
            start_urls = _collect_scope_values(
                manifest_scopes.values(), "start_urls"
            )
