...
If file watching causes high CPU or I/O overhead—for example, in large monorepos, environments with antivirus software, or projects where multiple functions share a `CodeUri`—you can disable it with the `--no-watch` option:

```
$ sam local start-api --warm-containers eager --no-watch
```

With `--no-watch`, code and template changes are not detected automatically. Stop and rerun the command to pick up changes.
