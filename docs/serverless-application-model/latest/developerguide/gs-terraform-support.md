...
+ [Using AWS SAM CLI commands with Terraform](#gs-terraform-support-using)
+ [Set up for Terraform projects](#gs-terraform-support-projects)
+ [Set up for Terraform Cloud](#gs-terraform-support-cloud)
...
The `sam metadata` resource is a `null_resource` Terraform resource type that provides the AWS SAM CLI with the information it needs to locate your Lambda artifacts. A unique `sam metadata` resource is required for each Lambda function or layer in your project. To learn more about this resource type, see [null\_resource](https://registry.terraform.io/providers/hashicorp/null/latest/docs/resources/resource) in the *Terraform registry*.
...
For a sample application that you can use with these examples, see [api\_gateway\_v2\_tf\_cloud](https://github.com/aws-samples/aws-sam-terraform-examples/tree/main/ga/api_gateway_v2_tf_cloud) in the *aws-samples GitHub repository*.
