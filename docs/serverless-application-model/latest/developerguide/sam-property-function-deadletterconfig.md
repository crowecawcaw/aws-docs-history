...
Lambda function that doesn’t exist, or insufficient permissions to invoke the Lambda function. For more information, see [Event retry policy and using dead-letter queues](../../../eventbridge/latest/userguide/rule-dlq.md "../../../eventbridge/latest/userguide/rule-dlq.md") in the
_Amazon EventBridge User Guide_.

###### Note

The [AWS::Serverless::Function](sam-resource-function.md "sam-resource-function.md") resource type has a similar data type, `DeadLetterQueue` which handles
failures that occur after successful invocation of the target Lambda function. Examples of this type of failure include Lambda throttling, or errors returned by the Lambda target
function. For more information about the function `DeadLetterQueue` property, see [Dead-letter queues](../../../lambda/latest/dg/invocation-async.md#invocation-dlq "../../../lambda/latest/dg/invocation-async.md#invocation-dlq") in the _AWS Lambda Developer Guide_.
