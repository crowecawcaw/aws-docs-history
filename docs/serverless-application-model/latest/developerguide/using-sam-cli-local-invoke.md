...
Use the AWS Serverless Application Model Command Line Interface (AWS SAM CLI) `sam local invoke` subcommand to initiate a
one-time invocation of an AWS Lambda function locally.
...
- For a list of `sam local invoke` command options, see
...
When you run `sam local invoke`, the AWS SAM CLI assumes that your current working directory is your
...
2. If your application contains more than one function, provide the function’s logical ID. The following is an
   example:
...
3. The AWS SAM CLI builds your function in a local container using Docker. It then invokes your
   function and outputs your function’s response.
...
When using `sam local invoke`, the Lambda function runtime output (for example, logs) is output to
`stderr`, and the Lambda function result is output to `stdout`.
...
application that are already deployed in the cloud. The following is an example:
...
load just that AWS SAM template and the resources it points to.

To invoke a function of a nested application or stack, provide the application or stack logical ID along with the
function logical ID. The following is an example:
...
run `sam build` every time you update your function code. Then, run `sam local invoke` to locally
...
We want to locally test our Lambda function while having it interact with our DynamoDB table in the cloud. To do this,
we create our environment variables file and save it in our project's root directory as
`locals.json`. The value provided here for `SAMPLE_TABLE` references our DynamoDB table in
the cloud.
...
Next, we run `sam local invoke` and pass in our environment variables with the `--env-vars`
option.
...
For a list of all `sam local invoke` options, see
[sam local invoke](sam-cli-command-reference-sam-local-invoke.md "sam-cli-command-reference-sam-local-invoke.md").
