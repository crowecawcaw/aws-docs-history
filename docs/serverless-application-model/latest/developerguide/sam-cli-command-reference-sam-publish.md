...
The `sam publish` command publishes an AWS SAM application to the AWS Serverless Application Repository. This command takes a packaged AWS SAM template and publishes the
application to the specified AWS Region.

The `sam publish` command expects the AWS SAM template to include a
...
Resource-Based Policy Examples](../../../serverlessrepo/latest/devguide/security_iam_resource-based-policy-examples.md "../../../serverlessrepo/latest/devguide/security_iam_resource-based-policy-examples.md") in the _AWS Serverless Application Repository Developer Guide_.

###### Note

Currently `sam publish` doesn't support publishing nested applications that are
...
use. The default value is "`samconfig.toml`" in the root of the project
...
(Optional) Use this option to provide a semantic version of your application that
overrides the `SemanticVersion` property in the `Metadata` section of
the template file. For more information about semantic versioning, see the [Semantic Versioning specification](https://semver.org/ "https://semver.org/").

`--template, -t `PATH``

The path of AWS SAM template file `[default:
 template.[yaml|yml]]`.
