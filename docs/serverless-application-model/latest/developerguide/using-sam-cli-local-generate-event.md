...
Use the AWS Serverless Application Model Command Line Interface (AWS SAM CLI) `sam local generate-event` subcommand to generate
event payload samples for supported AWS services. You can then modify and pass these events to local resources for
testing.
...
Events generated by one service are passed to other services as an _event source_. For example, an
item placed in an Amazon Simple Storage Service (Amazon S3) bucket can generate an event. This event can then be used as the event source for an
AWS Lambda function to process the data further.

Events that you generate with `sam local generate-event` are formatted in the same structure as the actual
events created by the AWS service. You can modify the contents of these events and use them to test resources
in your application.
...
Use the AWS SAM CLI `sam local generate-event` subcommand to generate events for supported
AWS services.
...
1. Run `sam local generate-event` and provide the supported service name. This will display a list of
   event types that you can generate. The following is an example:
...
2. To generate the sample event, run `sam local generate-event`, providing the service and event
   type.
...
These sample events contain placeholder values. You can modify these values to reference actual resources in your
application or values to help with local testing.
...
2. Use any of these options at the command prompt to modify your sample event payload. The following is an
   example:
...
The following is an example of an event being saved as an `s3.json` file in the
`events` folder of our project.
...
The following is an example of using the `s3.json` event to invoke our Lambda function
locally:
