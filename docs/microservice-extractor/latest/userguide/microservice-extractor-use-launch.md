...
<a name="microservice-extractor-use-launch"></a>
...
From the **Visualization (nodes and dependencies)** page, you can view, in graph form, the logically grouped functionalities identified by Microservice Extractor to be extracted as isolated services. By default, the visualization displays nodes at the Project level, you can then use filters to display specific project level or namespace level nodes. For selected nodes, you can view .NET core portability. You can apply the default groups, modify them, or create new groups to associate with a functionality that guides refactoring. For more details on graph functionality, see [Work with the application visualization](microservice-extractor-use-visualization.md).