...
| `InServiceInferenceUnits` | The number of inference units that the model is using. Valid statistics: `Average` It is recommended that you use the Average statistic to obtain the 1 minute average of how many instances are used. Unit: Count | The following dimensions are supported for the Lookout for Vision metrics. |
