...
You can delete a version of a model by using the console or by using the `DeleteModel` operation.
You can't delete model version that is running or being trained.

If the model is version running, first use the `StopModel` operation to stop the model version.
For more information, see [Stopping your Amazon Lookout for Vision model](run-stop-model.md "run-stop-model.md"). If a model is training, wait
...
[ListProjects](../APIReference/API_ListProjects.md "../APIReference/API_ListProjects.md") and check if the version of the model (`ModelVersion`) is in the
`Models` array.
...
4. On the **Projects** page, select the project that contains the model that you
   want to delete.
...
8. In the **Delete** dialog box, enter **delete** to confirm
   that you want to delete the model.
...
Java V2
This code is taken from the AWS Documentation SDK examples GitHub repository. See the full example
[here](https://github.com/awsdocs/aws-doc-sdk-examples/blob/main/javav2/example_code/lookoutvision/src/main/java/com/example/lookoutvision/DeleteModel.java "https://github.com/awsdocs/aws-doc-sdk-examples/blob/main/javav2/example_code/lookoutvision/src/main/java/com/example/lookoutvision/DeleteModel.java").
