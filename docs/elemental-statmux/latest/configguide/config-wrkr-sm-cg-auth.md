...
<a name="config-wrkr-sm-cg-auth"></a>
...
+ For the REST API, users must include these additional HTTP headers in commands that they send:
  ...
  For more information about using the API with authentication enabled, see the AWS Elemental Statmux REST API documentation.
...
1. If you didn't use the `--https` flag in the configure command, the following prompt appears.
...
1. For the prompt `Do you wish to enable PAM?`, type **N**. If you're using PAM authentication, type **Y**.
   ...
   For information about the different authentication options, see [User Authentication Reference](config-wrkr-sm-cg-auth-ref.md)
...
1. Create users through the node's web interface. For instructions, see [Add Users](config-wrkr-sm-cg-users.md).