...
| `datetime_diff_for_humans` | datetime\_diff\_for\_humans(dt, since=None): Return a human-readable/approximate difference between datetimes. When only one datetime is provided, the comparison will be based on now. |
| `ds_add`                   | ds\_add(ds, days): Add or subtract days from a YYYY-MM-DD.                                                                                                                              |
| `ds_format`                | ds\_format(ds, input\_format, output\_format): Output datetime string in a given format.                                                                                                |
