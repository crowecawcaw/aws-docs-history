...
- **Pipeline definition**
  - **AWS Data Pipeline:**  AWS Data Pipeline uses JSON-based configuration file that defines the workflow.
  - **Amazon MWAA:** Amazon MWAA uses Python-based [Directed Acyclic Graphs](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/dags.html) (DAGs) that define the workflow.

- **Pipeline execution environment**
  - **AWS Data Pipeline:** Workflows run on Amazon EC2 instances. AWS Data Pipeline provisions and manages these instances on your behalf.
  - **Amazon MWAA:** Amazon MWAA uses Amazon ECS containerized environments to run tasks.

- **Pipeline components**
  - **AWS Data Pipeline:** Activities are processing tasks that run as part of the workflow. / **Amazon MWAA:** [Operators](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/operators.html) ([Tasks](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/tasks.html)) are the fundamental processing units of a workflow.
  - **AWS Data Pipeline:** Preconditions contain conditional statements that must be true before an activity can run. / **Amazon MWAA:** [Sensors](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/sensors.html) ([Tasks](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/tasks.html)) represent conditional statements that can wait for a resource or task to be completed before running.
  - **AWS Data Pipeline:** A resource in AWS Data Pipeline refers to the AWS compute resource that performs the work that a pipeline activity specifies. Amazon EC2 and Amazon EMR are two available resources. / **Amazon MWAA:** Using tasks in a DAG, you can define a variety of compute resources, including Amazon ECS, Amazon EMR, and Amazon EKS. Amazon MWAA executes Python operations on workers that run on Amazon ECS.

- **Pipeline execution**
  - **AWS Data Pipeline:** AWS Data Pipeline supports scheduling runs with regular rate-based, and cron-based patterns. / **Amazon MWAA:** Amazon MWAA supports scheduling with [cron](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/dag-run.html#cron-presets) expressions and presets, as well as custom [timetables](https://airflow.apache.org/docs/apache-airflow/stable/authoring-and-scheduling/timetable.html).
  - **AWS Data Pipeline:** An instances refers to each run of the pipeline. / **Amazon MWAA:** A [DAG run](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/dag-run.html) refers to each run of an Apache Airflow workflow.
  - **AWS Data Pipeline:** An attempt refers to a retry of a failed operation. / **Amazon MWAA:** Amazon MWAA supports retries that you define either at the DAG level, or at the task-level.
...
<a name="migrating-pipeline-workloads-price-comparison"></a>
 ...
 Pricing for AWS Data Pipeline is based on the number of pipelines, as well as how much you use each pipeline. Activities that you run more than once a day (high frequency) cost $1 per month per activity. Activities that you run once a day or less (low frequency) cost $0.60 per month per activity. Inactive Pipelines are priced at $1 per pipeline. For more information, refer to the [AWS Data Pipeline pricing](https://aws.amazon.com/datapipeline/pricing/) page. 
