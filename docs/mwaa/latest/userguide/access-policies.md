...
**Note**  
Amazon MWAA provides IAM integration with the five default Apache Airflow role-based access control (RBAC) roles. For more information about working with custom Apache Airflow roles, see [Tutorial: Restricting an Amazon MWAA user's access to a subset of DAGs](limit-access-to-dags.md).
...
1. Substitute the following values:
   ...
   1. {{{your-kms-id}}} – The unique identifer for a customer managed key, applicable only if you use a customer managed key for encryption at-rest.
...
1. Substitute the following values:
   ...
   1. {{{airflow-role}}} – the `Admin` Apache Airflow default role. For more information, see [Access control](https://airflow.apache.org/docs/apache-airflow-providers-fab/stable/auth-manager/access-control.html) on the Apache Airflow website.
