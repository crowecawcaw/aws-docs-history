...
+ **VPC endpoints** for each AWS service used by your environment, and Apache Airflow VPC endpoints in the same AWS Region and Amazon VPC as your Amazon MWAA environment.
  ...
  + The VPC endpoint policy for each endpoint must be configured to allow access to AWS services used by the environment. For example, [(Recommended) Example VPC endpoint policy to allow all access](vpc-security.md#vpc-external-vpce-policies-all).
  + A VPC endpoint policy for Amazon S3 must be configured to allow bucket access. For example, [(Recommended) Example Amazon S3 gateway endpoint policy to allow bucket access](vpc-security.md#vpc-external-vpce-policies-s3).
...
If internet access in your VPC is allowed by your organization, *and* you would like users to access your Apache Airflow webserver over the internet:

1. Create an Amazon VPC network with internet access.

1. Create an environment with the **Public network** access mode for your Apache Airflow webserver.

1. **What we recommend**: We recommend using the CloudFormation quick-start template that creates the Amazon VPC infrastructure, an Amazon S3 bucket, and an Amazon MWAA environment at the same time. To learn more, refer to [Quick start tutorial for Amazon Managed Workflows for Apache Airflow](quick-start.md).

If internet access in your VPC is allowed by your organization, *and* you would like to limit Apache Airflow webserver access to users within your VPC:

1. Create an Amazon VPC network with internet access.

1. Create a mechanism to access the VPC interface endpoint for your Apache Airflow webserver from your computer.

1. Create an environment with the **Private network** access mode for your Apache Airflow webserver.

1. **What we recommend**:

   1. We recommend using the Amazon MWAA console in [Option one: Creating the VPC network on the Amazon MWAA console](vpc-create.md#vpc-create-mwaa-console), or the CloudFormation template in [Option two: Creating an Amazon VPC network *with* internet access](vpc-create.md#vpc-create-template-private-or-public).
...
If internet access in your VPC is **not allowed** by your organization, *and* you want the Apache Airflow web server to remain accessible over the internet:

1. Create an Amazon VPC network without internet access.

1. Create an environment with the **Both public and private network access** mode for your Apache Airflow webserver. This option is available for Apache Airflow version 3.2.1 and later.

1. **What we recommend**: We recommend using the CloudFormation template to create an Amazon VPC without internet access and the VPC endpoints for each AWS service used by Amazon MWAA in [Option three: Creating an Amazon VPC network *without* internet access](vpc-create.md#vpc-create-template-private-only).

If internet access in your VPC is **not allowed** by your organization, *and* you want to limit Apache Airflow webserver access to users within your VPC:
...
1. **What we recommend**:
   ...
   1. We recommend using the CloudFormation template to create an Amazon VPC without internet access and the VPC endpoints for each AWS service used by Amazon MWAA in [Option three: Creating an Amazon VPC network *without* internet access](vpc-create.md#vpc-create-template-private-only).
