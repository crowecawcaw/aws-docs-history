...
You can use the following code example to write a directed acyclic graph (DAG) that runs a
`PythonOperator` to retrieve OS-level metrics for an Amazon MWAA environment. The DAG
then publishes the data as custom metrics to Amazon CloudWatch.

Custom OS-level metrics provide you with additional visibility about how your environment
workers are utilizing resources such as virtual memory and CPU. You can use this
information to select the [environment class](environment-class.md "environment-class.md") that best
suits your workload.

###### Topics

- [Version](#samples-custom-metrics-version "#samples-custom-metrics-version")
- [Prerequisites](#samples-custom-metrics-prereqs "#samples-custom-metrics-prereqs")
- [Permissions](#samples-custom-metrics-permissions "#samples-custom-metrics-permissions")
- [Dependencies](#samples-custom-metrics-dependencies "#samples-custom-metrics-dependencies")
- [Code example](#samples-custom-metrics-code "#samples-custom-metrics-code")
...
- An [Amazon MWAA
  environment](get-started.md "get-started.md").
...
1. In your command prompt, navigate to the folder where your DAG code is stored. For
   example:
...
2. Copy the contents of the following code example and save it locally as
   `dag-custom-metrics.py`. Replace `MWAA-ENV-NAME` with your environment name.
...
4. If the DAG runs successfully, you get something similar to the following in your Apache Airflow
   logs:
