...
- [Version](#samples-ecs-operator-version "#samples-ecs-operator-version")
- [Prerequisites](#samples-ecs-operator-prereqs "#samples-ecs-operator-prereqs")
- [Permissions](#samples-ecs-operator-permissions "#samples-ecs-operator-permissions")
- [Create an Amazon ECS cluster](#create-cfn-template "#create-cfn-template")
- [Code sample](#samples-ecs-operator-code "#samples-ecs-operator-code")
...
For more information about the Amazon MWAA execution role, and how to attach a policy, refer to [Execution role](mwaa-create-role.md "mwaa-create-role.md").
...
2. In your command prompt, use the following AWS CLI command to create a new stack. You must replace the values `SecurityGroups`
   and `SubnetIds` with values for your Amazon MWAA environment's security groups and subnets.
...
Alternatively, you can use the following shell script. The script retrieves the required values for your environment's
security groups, and subnets using the
...
then creates the stack accordingly. To run the script, do the following.
    ...
    --capabilities CAPABILITY_IAM
    ```
    2. Run the script using the following commands. Replace `environment-name` and `stack-name` with your information.
...
In the example DAG, for `awslogs_group`, you might need to modify the log group with the name for your Amazon ECS task log group. The example assumes a log group named `mwaa-ecs-zero`.
For `awslogs_stream_prefix`, use the Amazon ECS task log stream prefix. The example assumes a log stream prefix, `ecs`. 3. Run the following AWS CLI command to copy the DAG to your environment's bucket, then trigger the DAG using the Apache Airflow UI.

```
`aws s3 cp `your-dag`.py s3://`your-environment-bucket`/dags/`
```

4. If successful, you'll get output similar to the following in the task logs for `ecs_operator_task` in the `ecs_fargate_dag` DAG:
