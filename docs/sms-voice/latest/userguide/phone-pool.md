...
Phone pools can be associated with Notify configurations to use your own
dedicated phone numbers alongside AWS-managed identities for templated messaging. For
more information, see [Using dedicated numbers with Notify](notify-dedicated-numbers.md "notify-dedicated-numbers.md").
...
You can also add an AWS RCS Agent as an origination identity in a pool alongside your phone
numbers and sender IDs. When an AWS RCS Agent is in a pool, the pool enables automatic
RCS-to-SMS fallback — if RCS delivery fails, the service automatically retries the message
via SMS using another origination identity in the same pool. For more information about
configuring RCS fallback with pools, see
