...
This topic identifies AWS End User Messaging SMS actions that you can add to IAM policies for your AWS account. To see examples that demonstrate how you can use actions in policies to manage access to AWS End User Messaging SMS resources, see [Identity-based policy examples for AWS End User Messaging SMS](security_iam_id-based-policy-examples.md).

**Topics**
+ [AWS End User Messaging SMS and Voice v2 API actions](#permissions-actions-sms-voice-apiactions-V2)
...
+ Resource ARN – `arn:aws:sms-voice:{{region}}:{{accountId}}:notify-configuration/{{notifyConfigurationId}}`
