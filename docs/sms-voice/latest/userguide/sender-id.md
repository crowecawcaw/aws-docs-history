...
Sender IDs are supported in many countries and regions around the world. In some places, if you're a business that sends SMS messages to individual customers, you must use a sender ID that's pre-registered with a regulatory agency or industry group. For a complete list of countries and regions that support or require sender IDs, see [SMS and MMS country capabilities and limitations](phone-numbers-sms-support-by-country.md).
...
**Topics**
+ [Sender ID country capabilities and limitations](#sender-id-limitations)
+ [Registered and dynamic sender IDs](#sender-id-types)
...
<a name="sender-id-limitations"></a>

For more information on which countries support sender IDs see the **Supports Sender IDs** column in [Supported countries and regions for SMS messaging with AWS End User Messaging SMS](phone-numbers-sms-by-country.md).

For the rules of which sender ID is displayed when you send SMS messages to countries where sender IDs are supported, compared to those where Sender IDs aren't supported, see [Sender ID display name rules](#channels-sms-countries-sender-id).

## What are registered and dynamic sender IDs in AWS End User Messaging SMS
<a name="sender-id-types"></a>

**Registered sender ID** – A registered sender ID is registered with a regulatory agency or industry group. For a complete list of countries and regions that support or require sender IDs, see [Supported countries and regions for SMS messaging with AWS End User Messaging SMS](phone-numbers-sms-by-country.md).

**Dynamic sender ID** – A dynamic sender ID does not have to be registered with a regulatory agency or industry group. Registration requirements can change quickly and it is recommended that you complete any optional registration for dynamic sender IDs. For a complete list of countries and regions that support or optionally have sender ID registration, see [Supported countries and regions for SMS messaging with AWS End User Messaging SMS](phone-numbers-sms-by-country.md).

## Considerations for a sender ID
<a name="sender-id-considerations"></a>
...
<a name="channels-sms-countries-sender-id"></a>
...
- **In a country or region where Sender ID registration is required**
  - **And your SMS message...:** Specifies a Sender ID that has been registered / **The message is sent from...:** The Sender ID.
  - **And your SMS message...:** Doesn't specify a Sender ID, or specifies an unregistered sender ID / **The message is sent from...:** AWS End User Messaging SMS attempts to deliver the message with the Sender ID *NOTICE*. The message might not be received by the recipient based on the carrier requirements in the destination country or region.
