...
that let users draw with a stylus (pen). With AppStream 2.0, your users can connect
...
Drawing tablets are not supported when using the AppStream 2.0 macOS client
application.
...
- Some applications, such GIMP, must detect drawing tablets on the streaming instance to support
  ...
  must share their drawing tablets with AppStream 2.0 every time they start a new streaming session. For more information, see [Qualify USB Devices for Use with Streaming
...
- This feature is not supported on Chromebooks.
  ...
  USB, share the device with AppStream 2.0 if required for pressure sensitivity detection, and then use the AppStream 2.0 client or a [supported web
  browser](drawing-tablet-support-web-access-admin.md "drawing-tablet-support-web-access-admin.md") to start an AppStream 2.0 streaming session.
