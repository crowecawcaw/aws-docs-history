...
The following diagram is an example of the API operations flow between AppStream 2.0 and
...
1. The user connects to AppStream 2.0. A fleet streaming instance is assigned to the
...
3. The service or agent fetches the application entitlements for the user. In
   ...
   (IDs).
4. Your dynamic app provider calls the AppStream 2.0 agent
   `AddApplications` API operation with the application metadata
   for the applications that the user should have.
5. The AppStream 2.0 agent dynamically updates the application catalog with the
...
7. The application is launched by using the application metadata specified by
   ...
   to AppStream 2.0 and logs in to the fleet instance. After login, the list of applications
