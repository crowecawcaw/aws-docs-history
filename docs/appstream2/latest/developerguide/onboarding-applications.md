...
Before you make an application available to end users through WorkSpaces Applications, validate that
the application works correctly in the WorkSpaces Applications cloud environment, verify that the
application can be streamed without rendering artifacts, and size your fleet appropriately
for the application's resource profile. This page provides a structured onboarding
checklist that you can follow for each application you plan to stream. If you plan to use
multi-session, you need to pay specific attention to the ability to run multiple versions
of the application on the same host. If you plan to use
[native application mode](feature-support-native-application-mode.md "feature-support-native-application-mode.md"),
you need to verify that the application does not present any compatibility problems with
this mode.

These guidelines apply whether you are onboarding a new application or migrating an
existing application from another delivery model.
...
- **Application compatibility validation:**
  Confirm the application behaves correctly in the streaming environment across
  the features your users rely on.
- **Instance sizing and capacity planning:**
  Choose an instance type and fleet scaling policy that match the application's
  CPU, memory, and GPU profile and your expected concurrent user count.

We recommend completing compatibility validation first (on an Image Builder and a
pilot fleet), then using the measurements from that validation to drive instance sizing
decisions.
...
2. The user is logged into the server operating system session on the instance.
3. The application is started.
4. The streaming session begins, at which point environment settings that depend on the client (for example, client display resolution, DPI, client time zone, and client-redirected devices like printers) are applied.

Because client-dependent environment settings are applied _after_ the application starts, an application that reads these values only once during startup will not react to the user's actual client configuration. Validate the following:
...
Multi-session fleets currently do not support webcam, [Dynamic Application Framework](dynamic-app-framework.md "dynamic-app-framework.md"), and [Smart Card Authentication](active-directory-prerequisites-smart-card-authentication.md "active-directory-prerequisites-smart-card-authentication.md"). If you need any of these features, we recommend that you use single-session fleets.
...
- USB device redirection, if used by the application. See [USB device redirection](feature-support-USB-devices-qualified.md "feature-support-USB-devices-qualified.md") for the list of supported devices.
...
Real-time audio-video scenarios (voice, video conferencing, and collaboration tools embedded in the application) require higher frame rates and may need a larger instance type. See [Part 2: Instance sizing and capacity planning](#onboarding-sizing "#onboarding-sizing").
...
Select an instance family based on the resource profile of the application. For hardware specifications and pricing, see [WorkSpaces Applications Instance Families](instance-types.md "instance-types.md") and [WorkSpaces Applications Pricing](https://aws.amazon.com/appstream2/pricing/ "https://aws.amazon.com/appstream2/pricing/").
...
After you know the maximum users per instance, configure fleet scaling based on your expected concurrent user count over time. See [Fleet Auto Scaling for WorkSpaces Applications](autoscaling.md "autoscaling.md") for the full mechanics. This section summarizes the decisions you should make as part of onboarding.
