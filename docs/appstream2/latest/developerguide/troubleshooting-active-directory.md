...
+ The userPrincipalName format for the SAML\_Subject NameID is not formatted properly, or does not resolve to the actual domain for the user. For more information, see step 1 in [Prerequisites](certificate-based-authentication-prereq.md).
+ The (optional) ObjectSid attribute in your SAML assertion does not match the Active Directory security identifier (SID) for the user specified in the SAML\_Subject NameID. Confirm that the attribute mapping is correct in your SAML federation, and that your SAML identity provider is synchronizing the SID attribute for the Active Directory user.
...
+ DOMAIN\_JOIN\_INTERNAL\_SERVICE\_ERROR "The group name could not be found."
