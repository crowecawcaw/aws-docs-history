...
AppStream 2.0 users' devices require outbound access on port 443 (TCP) and port 8433
(UDP) when using the internet endpoints, and if you are using DNS servers for domain
name resolution, port 53 (UDP).

- Port 443 is used for HTTPS communication between AppStream 2.0 users' devices and
...
- Port 8433 is used for UDP HTTPS communication between AppStream 2.0 users'
...
For more information, see [Tutorial: Creating and Streaming from Interface VPC Endpoints](creating-streaming-from-interface-vpc-endpoints.md "creating-streaming-from-interface-vpc-endpoints.md").

- Port 53 is used for communication between AppStream 2.0 users' devices and your
