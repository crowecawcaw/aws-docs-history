...
**Note**  
The device qualification process creates a device compatibility filter. This filter determines which USB devices are recognized for use during streaming sessions. This filter is not a security access control. Do not rely on it as a security boundary. Limit qualification to the devices your users need to avoid unexpected device behavior during streaming sessions.
