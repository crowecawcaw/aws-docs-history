...
Data on instance store volumes is lost if the underlying disk
drive fails, or if the instance stops, hibernates, or terminates. To prevent data loss, we
recommend that you back up your long-term data on instance store volumes to persistent
storage, such as an Amazon EBS volume or a network storage device in your on-premises
network.
...
requirements to support the installation, maintenance, and use of the Outposts equipment. An
Outposts rack
