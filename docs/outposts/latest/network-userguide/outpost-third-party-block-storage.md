...
backed by external storage arrays is not supported. To launch an AMI, EBS or instance storage is
required for Outposts racks.
...
- AWS technicians configure the [local
  gateway](outposts-local-gateways.md "outposts-local-gateways.md") to ensure connectivity between the Outpost subnets and the local
  network.
