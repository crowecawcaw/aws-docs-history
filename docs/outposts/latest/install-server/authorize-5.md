# Verify the NSK LEDs for your Outposts server

After the provisioning process completes, check the NSK LEDs.

AWS Outposts supports two versions of NSK: Atlas 2.0 and Atlas 3.0. Both NSK versions have a RGB
**Status** LED. In addition, the Atlas 3.0 has a green
**Power** LED.

The following image shows the location of the LEDs on the Atlas 2.0 and Atlas 3.0:

![An image of the Atlas 2.0 and 3.0 NSKs with the RGB Status LED on each NSK and the green Power LED on the Atlas 3.0.](images/nsk-led-status.png)

###### To verify the Status and Power LEDs on the NSK

1. Check the color of the RGB Status LED. If the color is green, the NSK is healthy. If
   the color is not green, contact Support.
2. If you have an Atlas 3.0 NSK, check the green Power LED. If the green light is on, the
   NSK is correctly connected to the host and has power. If the green light is not on,
   contact Support.
