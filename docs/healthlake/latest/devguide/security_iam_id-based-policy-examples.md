...
For details about actions and resource types defined by HealthLake, including the format of the ARNs for each of the resource types, see [Actions, resources, and condition keys for AWS HealthLake](../../../service-authorization/latest/reference/list_amazonhealthlake.md "../../../service-authorization/latest/reference/list_amazonhealthlake.md") in the _Service Authorization Reference_.
...
`AmazonHealthLakeFullAccess` and
`AWSLakeFormationDataAdmin`. You also need to attach the HealthLake inline
policy which is a service role.
...
the inline policy which creates the required service role, see [Setting up AWS HealthLake](getting-started-setting-up.md "getting-started-setting-up.md").
You must also use the AWS Lake Formation console or CLI to assign your
HealthLake administrator to be an AWS Lake Formation Data Lake administrator.
For more information, see [Setting up AWS HealthLake](getting-started-setting-up.md "getting-started-setting-up.md").

## Accessing an AWS HealthLake data store in Amazon Athena

If you want to provide users and roles with access to the HealthLake data stores in
Amazon Athena, attach the following IAM policies to the role or user:
`AmazonAthenaFullAccess` and `AmazonS3FullAccess`.
`Select` and `Describe` permissions are also required on tables
managed by AWS Lake Formation. AWS Lake Formation table permissions
