...
Complete reference for all Velocity helper functions in HealthLake Data Transformation Agent. All helpers are
automatically registered in the Velocity context.

## Helper Categories

- [Array Helpers](#velocity-array-helpers "#velocity-array-helpers")
- [String Helpers](#velocity-string-helpers "#velocity-string-helpers")
- [Conditional Helpers](#velocity-conditional-helpers "#velocity-conditional-helpers")
- [Date/Time Helpers](#velocity-datetime-helpers "#velocity-datetime-helpers")
- [Math Helpers](#velocity-math-helpers "#velocity-math-helpers")
- [Encoding Helpers](#velocity-encoding-helpers "#velocity-encoding-helpers")
- [JSON Helpers](#velocity-json-helpers "#velocity-json-helpers")
- [CDA Specific Helpers](#velocity-cda-helpers "#velocity-cda-helpers")
- [Care Plan Helpers](#velocity-careplan-helpers "#velocity-careplan-helpers")
- [Specialized Helpers](#velocity-specialized-helpers "#velocity-specialized-helpers")
...
Use Case: Escaping user-provided strings that may contain special characters
(addresses, names, descriptions).
...
Use Case: Escaping HTML/XML content for embedding in JSON strings (e.g., FHIR narrative div).
...
Converts parsed XML Map structure back to XML string with XHTML-compliant
lowercase attributes. Use Case: Converting CDA narrative text sections to
FHIR narrative div content. Features: Reconstructs XML elements with proper
tags, Converts attribute names to lowercase for XHTML compliance (ID → id),
Handles nested elements and lists, Preserves text content from \_ key.
...
Extracts FHIR CarePlan activity from CDA treatment plan encounter.
Returns activity with: status, performer, location, scheduledPeriod, code, description.
...
Checks if input is already a UUID format; returns as-is if UUID, generates
deterministic UUID if not. Use Case: Ensuring consistent UUID format for IDs
that may already be UUIDs or need conversion.
...
Extracts and maps HTML table data to list of key-value maps. Returns list of maps
...
Returns display name for observation category code. Supported codes:
vital-signs, laboratory, imaging, procedure, survey, exam, therapy, activity.
