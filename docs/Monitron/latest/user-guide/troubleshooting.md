...
**Topics**
+ [Troubleshooting Issues with Amazon Monitron Sensors](#trouble-sensor-issues)
+ [Troubleshooting issues with Amazon Monitron gateways](#gateway-fail2)

## Troubleshooting Issues with Amazon Monitron Sensors
<a name="trouble-sensor-issues"></a>
...
**Topics**
+ [If you can't commission your sensors](#trouble-cannot-commission)
+ [If your sensor is offline](#trouble-sending-measurements)
+ [If your sensor falls off](#trouble-sensor-falling-off)

### If you can't commission your sensors
<a name="trouble-cannot-commission"></a>
...
+ **Does your smartphone have NFC activated?**
  ...
  Some iOS devices require that NFC Tag Reader be manually turned on in Control Center. To see if your device is one of them, check the [iPhone User Guide.](https://support.apple.com/guide/iphone/aside/asd-nfc-reader/14.0/ios/14.0)
...
<a name="trouble-sending-measurements"></a>
...
+ Try [taking a one-time measurement](anom-take-measure.md). If you can do so, then the sensor is working. If you cannot, then the sensor is not working, and may have run out of battery power. Replace it with a new sensor.
...
+ If the measurement action starts (you see a loading bar), but does not complete, try to retake the measurement. If the same thing happens again, try to [delete the sensor](as-delete-sensor.md) and [recommission it](as-add-sensors.md).
...
<a name="trouble-sensor-falling-off"></a>

[Re-mount it](as-how-sensors.md).

## Troubleshooting issues with Amazon Monitron gateways
<a name="gateway-fail2"></a>

**Topics**
+ [If your mobile app can't pair with the gateway](#gateway-detection-fail)
+ [If commissioning the gateway fails](#gateway-commissioning-fail)
+ [If your gateway goes offline](#gateway-stops-working)

### If your mobile app can't pair with the gateway
<a name="gateway-detection-fail"></a>
...
+ **Make sure that the gateway is in commissioning mode.**
  ...
  See [Commissioning a Wi-Fi gateway](adding-gateway-Wi-Fi.md) or [Commissioning an Ethernet gateway](adding-gateway-ethernet.md).
+ **Make sure your smartphone's Bluetooth is working.**
  ...
  + Is there anything that might be interfering electronically with the Bluetooth signal? See [If your sensor is offline](#trouble-sending-measurements).

If none of these actions resolves the issue, try the following: 
+ Log out of the mobile app and restart it.
+ [Reset your Wi-Fi gateway](commissioning-button-Wi-Fi.md) or [reset your Ethernet gateway](commissioning-button-ethernet.md).

### If commissioning the gateway fails
<a name="gateway-commissioning-fail"></a>
...
<a name="gateway-stops-working"></a>
...
+ Confirm that you're not dealing with a newly commissioned gateway or a newly paired sensor. If so, wait an hour. Sensors send data once per hour. If you don't want to wait, you can [take a one-time measurement](anom-take-measure.md).
