...
<a name="cross-region-failover"></a>
...
<a name="cross-region-failover-reqmt"></a>
...
+ The output segments sequence numbers should not go back in the past across two encoding sessions. The segment duration should not increase and the reference Epoch time should not change after the first encoding session.
  ...
  If segment numbers go backward, MediaPackage sends a `409` error response to the encoder. To recover from this state, see [Reset for channels and endpoints](resetting.md).
