...
1. (Optional) For **Program date/time interval**, enter the interval (in seconds) for MediaPackage to insert the `EXT-X-PROGRAM-DATE-TIME` tags in the manifest. Default is 0 (`EXT-X-PROGRAM-DATE-TIME` tags aren't inserted). 
