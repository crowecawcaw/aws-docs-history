...
These options don't apply to video on demand (VOD) outputs or harvested
live-to-VOD assets.
...
- `SegmentTemplate` - Defines properties of the
  representation, such as the timescale and access URLs for media and initialization segments.
  There is one `SegmentTemplate` for each `Representation`.
...
- Separate the manifest into multiple periods, to allow ad breaks. See [DASH manifest options in AWS Elemental MediaPackage](multi-period.md "multi-period.md").
- Reduce the length of the manifest to make processing and playback more efficient. See [Compacted DASH manifests](compacted.md "compacted.md").
- Control what segment information is used in the media URL in the `SegmentTemplate`
  properties. See [DASH manifest segment template format](segtemp-format.md "segtemp-format.md").
