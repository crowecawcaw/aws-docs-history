...
Limit which incoming bitrates are available for playback and sort the streams
...
2. (Optional) For **Min video bitrate**, enter
   the minimum bitrate threshold (in bits per second) that video tracks
...
3. (Optional) For **Max video bitrate**, enter
   ...
   must be at or below to be available for playback from this endpoint. This ensures that
   tracks are _no more than_ a certain bitrate.
