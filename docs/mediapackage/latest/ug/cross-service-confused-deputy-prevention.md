...
policies to limit the permissions that AWS Elemental MediaPackage gives another service to the
...
`aws:SourceAccount` global condition context keys in MediaPackage to prevent
the confused deputy problem when working with harvest jobs.
