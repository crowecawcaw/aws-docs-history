...
1. The customer configures the cross-account role either manually or using CloudFormation. For information on cross-account roles, see [Providing access to AWS accounts owned by third parties](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_common-scenarios_third-party.html) in the *IAM User Guide*.
...
1. The customer deploys the partner product into their AWS account manually using CloudFormation, or another deployment tool.

1. The customer defines the necessary IAM policy for the partner product to use when it sends findings to Security Hub CSPM.
