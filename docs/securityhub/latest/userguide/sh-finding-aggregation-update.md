...
Changes to cross-Region aggregation are not implemented for an opt-in Region until you enable the Region in your AWS account.
Regions that AWS introduced on or after March 20, 2019 are opt-in Regions.

When you stop aggregating data from a linked Region, AWS Security Hub does not remove any existing aggregated data from that Region that is accessible in the home Region.

You cannot use the update procedures in this section to change the home Region. To change the home Region, you must do the following:
...
3. Enable cross-Region aggregation. For instructions, see [Enabling cross-Region aggregation](sh-finding-aggregation-enable.md "sh-finding-aggregation-enable.md").
