...
However, this action removes the delegated administrator from both Security Hub and Security Hub CSPM.
We recommend only performing this action when you have confirmed this operation with your security account.

###### Note

If you are using an account other than the organization management account as the Security Hub CSPM delegated administrator,
removing it through either the Security Hub CSPM console or the AWS Organizations API also removes it from Security Hub.
...
it is also removed from Security Hub CSPM. When the delegated administrator is removed from Security Hub CSPM, Central Configuration automatically opts out.
...
3. In **Delegated administrator**, choose **Remove delegated administrator**.
   In the confirmation dialog box, enter _remove_, and choose **Remove**.
