...
This AWS Security Hub control evaluates the Amazon Detective service and resources. The control might
...
This control checks whether an Amazon Detective behavior graph has tags with the specific keys defined in the parameter
`requiredTagKeys`. The control fails if the behavior graph doesn’t have any tag keys or if it doesn’t have all the keys specified in the
parameter `requiredTagKeys`. If the parameter `requiredTagKeys` isn't provided, the control only checks for the existence
of a tag key and fails if the behavior graph isn't tagged with any key. System tags, which are automatically applied and begin with `aws:`,
are ignored.
...
To add tags to a Detective behavior graph, see [Adding tags to a behavior graph](../../../detective/latest/adminguide/graph-tags.md#graph-tags-add-console "../../../detective/latest/adminguide/graph-tags.md#graph-tags-add-console") in the _Amazon Detective Administration Guide_.
