...
**Topics**
+ [Landing Zone Launch Failed](#setup-failed)
+ [Landing Zone Update Failed with KMS Error](#lz-update-failed)
+ [Unable to Update Landing Zone](#unable-to-update-landing-zone)
+ [Landing zone not up to date error](#landing-zone-not-up-to-date)
+ [New Account Provisioning Failed](#account-provisioning-failed)
+ [Failed to Enroll an Existing Account](#enrollment-failed)
+ [Unable to Update an Account Factory Account](#unable-to-update-account)
+ [Failure Error that Mentions AWS Config](#aws-config-error)
+ [AWS Control Tower Account Factory portfolio error](#no-launch-paths-found)
+ [Received an Insufficient Permissions Error](#insufficient-permissions)
+ [Detective controls are not taking effect on accounts](#control-errors)
+ [Rate exceeded error returned by the AWS Organizations API](#rate-exceeded-error)
+ [Failure to move an Account Factory account directly from one AWS Control Tower landing zone to another AWS Control Tower landing zone](#failure-to-move)
+ [AWS Support](#getting-support)

## Landing Zone Launch Failed
<a name="setup-failed"></a>
...
1. Clean up the failed stack with the name **AWSControlTowerBP-BASELINE-CLOUDTRAIL-MASTER**.

1. Wait for the stack deletion to complete.

1. Return to the AWS Control Tower page.

1. Select **Set up landing zone** again.
...
<a name="landing-zone-not-up-to-date"></a>
...
<a name="account-provisioning-failed"></a>
...
Try again to provision your account, without specifying any of those options. For more information, see [Provision accounts in the Service Catalog console, with Account Factory](provision-as-end-user.md).
...
## AWS Control Tower Account Factory portfolio error
<a name="no-launch-paths-found"></a>

When you're trying to create, enroll, or update an account, you may see one of the following error messages:

`Your AWS IAM identity does not have access to the AWS Control Tower Account Factory portfolio in AWS Service Catalog. To resolve this, an administrator must associate your AWS IAM identity with the portfolio.`

or

`No launch paths found for resource: prod-{{xxxx}}`
...
+ If you are authenticated as an IAM user, you must [add it to the AWS Service Catalog portfolio](https://docs.aws.amazon.com/servicecatalog/latest/adminguide/catalogs_portfolios_users) so that it has the correct permissions.
