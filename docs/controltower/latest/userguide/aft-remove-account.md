...
1.  ** Delete Terraform workspace (For HCP Terraform and Terraform Enterprise customers only) ** 
...
**Example: HCP Terraform or Terraform Enterprise**  
 When you choose HCP Terraform or Terraform Enterprise, you find a folder for each account in the `aft-backend-{{012345678901}}-primary-region` and `aft-backend-{{012345678901}}-secondary-region` Amazon S3 buckets. These folders are related to the *customizations pipeline state*. 