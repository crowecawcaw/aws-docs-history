...
For instructions on getting started with AWS Control Tower, see [Getting started with AWS Control Tower](https://docs.aws.amazon.com/controltower/latest/userguide/getting-started-with-control-tower.html). This is where you create the AWS Control Tower management account and set up your AWS Control Tower landing zone. 
...
+ **Optionally configure the Terraform project name**
  ...
  You can customize the Terraform project name used by AFT by setting the `terraform_project_name` parameter. By default, AFT puts the deployment in the "default" project in HCP Terraform or Terraform Enterprise.
