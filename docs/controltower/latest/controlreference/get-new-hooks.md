...
To update the way that AWS Control Tower handles the AWS CloudFormation hooks for your enabled proactive
controls, follow the steps given in this section.

After you complete this process, you can utilize the full capacity of AWS CloudFormation hooks, without
...
requires, without restricting any other AWS CloudFormation hooks that you may have deployed on AWS.
Only AWS Control Tower can change the AWS Control Tower hook, but principals with the correct permissions can
change other AWS CloudFormation hooks in your environment.
