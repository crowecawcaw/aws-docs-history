...
- The following customer-defined metadata may leave the AWS GovCloud (US)
  Regions only when the customer asks AWS to investigate a reported issue:
  - Bot definitions
  ...
  - Session attributes that customers use for the Get customer input block
    in the Amazon Connect console, such as
    `x-amz-lex:start-silence-threshold-ms` or
    `x-amz-lex:end-silence-threshold-ms`. For all
    session attributes, see [Contact block: Get customer input](../../../connect/latest/adminguide/get-customer-input.md "../../../connect/latest/adminguide/get-customer-input.md") in the Amazon Connect
    Administrator Guide.
