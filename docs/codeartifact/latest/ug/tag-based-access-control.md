...
**Important**  
When using tags on resources to limit actions, the tags must be on the resource in which the action operates on. For example, to deny `DescribeRepository` permissions with tags, the tags must be on each repository and not the domain. See [AWS CodeArtifact permissions reference](auth-and-access-control-permissions-reference.md) for a list of actions in CodeArtifact and which resources they operate on.
