...
CodeArtifact normalizes package and asset names and package versions before storing them, which means the names or versions in CodeArtifact may be different than the ones provided when the
...
**Package version normalization:** CodeArtifact normalizes NuGet package versions using the same pattern as NuGet.
...
a normalized package name of `newtonsoft.json` and version of `12.0.3`.
