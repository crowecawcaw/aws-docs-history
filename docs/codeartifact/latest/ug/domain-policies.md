...
When you call `put-domains-permissions-policy`, CodeArtifact ignores the resource policy on the domain when evaluating permissions. This ensures that the owner of a domain cannot lock themselves out of the domain, which would prevent them from being able to update the resource policy.
