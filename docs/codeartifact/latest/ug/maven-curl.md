...
1. Fetch a CodeArtifact authorization token by following the steps in
   [Pass an auth token using an environment variable](tokens-authentication.md#env-var "tokens-authentication.md#env-var") and return to these steps.
...
putting the value in quotation marks, the `@` must be included inside the quotation marks. 3. Use the following `curl` command to publish the POM to a CodeArtifact repository:
