...
1. Publish the `maven-metadata.xml` file of the last build (that is, the build with the most-recent date-time stamp) with `curl`. This creates a version with "`-SNAPSHOT`" in the version string and with the correct set of assets.
...
+ Replace {{my\_maven\_repo}} with your CodeArtifact repository name.
