...
When importing a Maven package version from a public repository, such as Maven Central, AWS CodeArtifact attempts to import all the assets in that package version. As described in [Requesting a package version with upstream repositories](repo-upstream-behavior.md), importing occurs when:
...
The [package version status](packages-overview.md#package-version-status) of package versions in upstream repositories can prevent CodeArtifact from retaining those versions in downstream repositories.
