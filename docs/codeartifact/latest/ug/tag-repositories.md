...
repositories in CodeArtifact. For information about CodeArtifact resource tagging, use cases, tag key and
value constraints, and supported resource types, see [Tagging resources](tag-resources.md "tag-resources.md").

You can use the CLI to specify tags when you create a repository. You can use the console or
CLI to add or remove tags, and update the values of tags in a repository. You can add up to 50
tags to each repository.

###### Topics

- [Tag repositories (CLI)](#tag-repositories-tag-cli "#tag-repositories-tag-cli")
- [Tag repositories (console)](#tag-repositories-console "#tag-repositories-console")
...
- [Add tags to a repository (CLI)](#add-repositories-tag-cli "#add-repositories-tag-cli")
- [View tags for a repository (CLI)](#list-repositories-tag-cli "#list-repositories-tag-cli")
- [Edit tags for a repository (CLI)](#update-repositories-tag-cli "#update-repositories-tag-cli")
- [Remove tags from a repository (CLI)](#delete-repositories-tag-cli "#delete-repositories-tag-cli")

### Add tags to a repository (CLI)

You can use the console or the AWS CLI to tag repositories.

To add a tag to a repository when you create it, see [Create a repository](create-repo.md "create-repo.md").
...
specifying the Amazon Resource Name (ARN) of the repository where you want to add tags
and the key and value of the tag you want to add.
...
repository. For example, to tag a repository named `my_repo` in
a domain named `my_domain`
...
Follow these steps to use the AWS CLI to view the AWS tags for a repository. If no
...
`my_repo` in a domain named `my_domain` with the
...
Follow these steps to use the AWS CLI to edit a tag for a repository. You can change
the value for an existing key or add another key.

At the terminal or command line, run the **tag-resource** command,
specifying the ARN of the repository where you want to update a tag and specify the
...
If you delete a repository, all tag associations are removed from the deleted
...
command, specifying the ARN of the repository where you want to remove tags and the
tag key of the tag you want to remove.
...
repository named `my_repo` in a domain named `my_domain`
with the tag keys
`key1` and
`key2`:
...
If successful, this command has no output. After removing tags, you can view the
remaining tags on the repository using the `list-tags-for-resource` command.
...
2. On the **Repositories** page, choose the repository that you
   want to add tags to.
3. Expand the **Details** section.
4. Under **Repository tags**, if there are no tags on the repository,
   choose **Add repository tags**. If
   there are tags on the repository, choose
   **View and edit repository tags**.
5. Choose **Add new tag**.
6. In the **Key** and **Value** fields,
   enter the text for each tag you want to add. (The
...
2. On the **Repositories** page, choose the repository where you
   want to view tags.
3. Expand the **Details** section.
4. Under **Repository tags**, choose
   **View and edit repository tags**.
...
If there are no tags added to this repository, the console will read **Add repository tags**. 5. In the **Key** and **Value** fields,
...
If there are no tags added to this repository, the console will read **Add repository tags**. 5. Next to the key and value for each tag you want to delete, choose
**Remove**. 6. Choose **Update repository**.
