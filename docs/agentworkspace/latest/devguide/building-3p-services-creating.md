...
workspace. First, install the app package:
...
2. Complete all initialization within the timeout period (30 seconds)
   ...
   2. initializationTimeout is the time the workspace will wait for a
      service to successfully connect, before onCreate is triggered

3. Handle initialization failures to prevent workspace loading issues
...
- `isService` set to true
  - An app that is configured as a service will be started when the
    ...
    the workspace. An application that runs as a service in the
    workspace must be integrated with the app SDK package. If the
    service fails to start before the InitializationTimeout, then an
    error will be sent to workspace causing the workspace to fail
...
- Set a `InitializationTimeout` in milliseconds up to 10000 (10
  ...
  - The InitializationTimeout parameter controls the maximum time
    ...
    the workspace. This is required to be set for applications
