# Get the availability state of the
...
- `name`: The name of the agent's current availability
  state.
- `startTimestamp`: A `Date` object that indicates
  when the state was set.
- `type`: The agent's current availability state type, as per the
  `AgentStateType` enumeration. The different values are as
  follows:
  - `routable`
  - `not_routable`
  - `after_call_work`
