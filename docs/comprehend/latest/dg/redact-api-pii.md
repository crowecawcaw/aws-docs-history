...
To redact the PII entities in your text, you start an asynchronous batch job. To run the job, upload your
documents to Amazon S3, and submit a [StartPiiEntitiesDetectionJob](../APIReference/API_StartPiiEntitiesDetectionJob.md "../APIReference/API_StartPiiEntitiesDetectionJob.md") request.

###### Topics

- [Before you start](#redact-pii-before "#redact-pii-before")
- [Input parameters](#redact-pii-api-inputs "#redact-pii-api-inputs")
- [Output file format](#redact-pii-api-outputs "#redact-pii-api-outputs")
- [PII redaction using the AWS Command Line Interface](#redact-pii-api-cli "#redact-pii-api-cli")
...
- `InputDataConfig` – Provide an [InputDataConfig](../APIReference/API_InputDataConfig.md "../APIReference/API_InputDataConfig.md")
  definition for your request, which includes the
  input properties for the job. For the `S3Uri` parameter, specify the Amazon S3
  location of your input documents.
- `OutputDataConfig` – Provide an [OutputDataConfig](../APIReference/API_OutputDataConfig.md "../APIReference/API_OutputDataConfig.md")
  definition for your request, which includes the
  output properties for the job. For the `S3Uri` parameter, specify the Amazon S3
...
- `RedactionConfig` – Provide an [RedactionConfig](../APIReference/API_RedactionConfig.md "../APIReference/API_RedactionConfig.md")
  definition for your request, which includes the
  ...
  - To replace each PII entity with its type, set the `MaskMode` parameter
    to `REPLACE_WITH_PII_ENTITY_TYPE`. For example, with this setting, the PII
    entity "Jane Doe" is replaced with "[NAME]".
  - To replace the characters in each PII entity with a character of your choice, set
    the `MaskMode` parameter to `MASK`, and set the
    `MaskCharacter` parameter to the replacement character. Provide only a
    single character. Valid characters are !, #, $, %, &, \*, and @. For example, with
    this setting, the PII entity "Jane Doe" can be replaced with "\*\*\*\* \*\*\*"

- `LanguageCode` – Set this parameter to `en` or `es`. Amazon Comprehend
  supports PII detection in English or Spanish text.

## Output file format

The following example shows the input and output files from an analysis job that redacts PII.
...
You can use the [DescribeEventsDetectionJob](../APIReference/API_DescribeEventsDetectionJob.md "../APIReference/API_DescribeEventsDetectionJob.md")
operation to get the status of an existing job.
