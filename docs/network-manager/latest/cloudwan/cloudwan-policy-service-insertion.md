...
add a tag with the key name _Inspection VPC_ and then a key value of
_InspectionVpcs_. The same tag should be applied to any
...
policy rule that identifies the tag _Inspection VPC_ and the key
value of _InspectionVpcs_. Attachment policy rules can be created
