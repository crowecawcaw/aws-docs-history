...
- `instance_type` (string) – The type of instance (for example,
  c5.xlarge).
- `timestamp` (timestamp)
- `number_of_instances` (integer) – The number of instances of that
  particular instance type that was consumed at the timestamp. This is the `target`
  field for which Amazon Forecast generates a forecast.

The following dimension is optional and can be used to change forecasting
granularity:

- `location` (string) – You can provide an AWS Region, such as us-west-2
  or us-east-1. This should only be used if you're modeling multiple Regions.
