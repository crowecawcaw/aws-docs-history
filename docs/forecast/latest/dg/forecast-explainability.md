...
To enable Forecast Explainability, your predictor must include at least one of the following: related time series, item metadata, or additional datasets like Holidays and the Weather Index. See [Restrictions and best practices](predictor-explainability.md#predictor-explainability-best-practices) for more information.
...
1. In the **S3 location** field, enter the location of the file with your time series.
...
+ **Forecast Explainability is only available for some Forecasts generated from AutoPredictor - **You cannot enable Forecast Explainability for Forecasts generated from legacy predictors (AutoML or manual selection). See [Upgrading to AutoPredictor](howitworks-predictor.md#upgrading-autopredictor).
