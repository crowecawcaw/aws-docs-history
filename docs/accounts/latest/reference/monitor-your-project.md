...
1. In the navigation pane, choose **Event history**. You see a filtered list of events, with the most recent events showing first. Choose an event.

1. In the event record, you'll see the following content:
...
1. Run the following command to sign in to the AWS CLI. You can find your Region using [AWS Regions for your projects](project-regions.md).

   ```
   aws login --region {{project-region}}
   ```

1. Run the following command to get the ID for your identity store. The CloudTrail event shows the identity store ARN, but does not indicate the identity store ID. You always use the AWS Region `us-east-1` when finding the ID for your identity store.
