...
+ Multi-Region key replication is available for symmetric Payment Cryptography keys like Triple Data Encryption Standard (3DES), Advanced Encryption Standard (AES), and Hash-based Message Authentication Code (HMAC), as well as asymmetric Payment Cryptography keys such as RSA and Elliptic Curve Cryptography (ECC).
