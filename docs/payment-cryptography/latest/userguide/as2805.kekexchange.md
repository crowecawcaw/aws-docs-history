...
These terms are relative - one side creates a key (sending side) and the other side receives it. So given KEY1, it is referred to on node1 as KEK(s) and on node2 as KEK(r).
 ...
 KEK for AS2805 are always key type = TR31\_K0\_KEY\_ENCRYPTION\_KEY as they are used to protect cryptograms and not key blocks. This maps to TERMINAL\_MAJOR\_KEY\_VARIANT\_00 as defined in AS2805 6.1 

Steps:

**1.Create a key**  
Create a key using the [CreateKey](create-keys.md) api. You will create a key of type TR31\_K0\_KEY\_ENCRYPTION\_KEY
