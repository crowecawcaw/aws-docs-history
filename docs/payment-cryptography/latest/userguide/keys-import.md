...
If the imported KeyUsage was TR31\_K0\_KEY\_ENCRYPTION\_KEY, you can use this key for subsequent key imports using TR-31. For other key types (such as TR31\_D0\_SYMMETRIC\_DATA\_ENCRYPTION\_KEY), you can use the key directly for cryptographic operations. 
...
You can now import a public key. As TR-34 and ECDH rely on passing the leaf certificate at run-time, this option is only used when encrypting data using a public key from another system. KeyUsage will be set to TR31\_D1\_ASYMMETRIC\_KEY\_FOR\_DATA\_ENCRYPTION.  
**Example**  
