...
headers that exactly match the variables defined in the associated event type plus four mandatory variables: EVENT\_ID, EVENT\_TIMESTAMP,
ENTITY\_ID, and ENTITY\_TYPE. You can also optionally include EVENT\_LABEL and LABEL\_TIMESTAMP (LABEL\_TIMESTAMP is required if EVENT\_LABEL
is included).
...
The jobID can’t contain the same ID of a past job, unless the job exists in CREATE\_FAILED state. The inputPath and outputPath must be valid S3 paths.
