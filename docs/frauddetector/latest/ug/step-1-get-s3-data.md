...
1. Go to [Samples](https://github.com/aws-samples/aws-fraud-detector-samples/tree/master/data).
    ...
    There are two data files that has online account registration data - *registration\_data\_20K\_minimum.csv* and *registration\_data\_20K\_full.csv*. The file `registration_data_20K_minimum` contains only two variables: *ip\_address* and *email\_address*. The file `registration_data_20K_full` contains other variables. These variables are for each event and they include *billing\_address*, *phone\_number*, and *user\_agent*. Both data files also contain two mandatory fields: 
