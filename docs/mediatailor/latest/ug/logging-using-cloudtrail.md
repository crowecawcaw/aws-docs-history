...
## AWS Elemental MediaTailor information in
...
AWS Elemental MediaTailor, create a trail. A _trail_ enables CloudTrail to
...
All AWS Elemental MediaTailor actions are logged by CloudTrail and are documented in the [AWS Elemental MediaTailor API reference](../apireference/Welcome.md "../apireference/Welcome.md"). For example, calls to the
`PutPlaybackConfiguration` and
`ListPlaybackConfigurations` operations generate entries in the CloudTrail
log files.
...
`PutPlaybackConfiguration` action:
...
The following example shows a CloudTrail log entry that demonstrates the
`GetPlaybackConfiguration` action:
