...
A MediaTailor client-side integration is required to use the Google Ad Manager Programmatic Access Libraries (PAL) SDKs and to use Google Ad Manager's open auction transaction type.
...
1. Use the PAL SDK to generate a nonce. For information about setting up a nonce, see your SDK documentation at [Google Ad Manager PAL SDK](https://developers.google.com/ad-manager/pal).
**Note**  
The nonce is an encrypted string that PAL generates for stream requests. Each request must have a unique nonce.
