...
| POST\_ADS\_RESPONSE hook timeout | 2 seconds | The maximum number of seconds that a POST\_ADS\_RESPONSE hook execution can take, including all function steps and HTTP calls. If the hook exceeds its timeout, MediaTailor discards all output and proceeds as if no function was executed. | 
| PRE\_MANIFEST\_INSERTION hook timeout | 2 seconds | The maximum number of seconds that a PRE\_MANIFEST\_INSERTION hook execution can take, including all function steps and HTTP calls. If the hook exceeds its timeout, MediaTailor discards all output and proceeds as if no function was executed. | 
| Combined hook budget per request | 2 seconds | The maximum combined execution time for the PRE\_ADS\_REQUEST, POST\_ADS\_RESPONSE, and PRE\_MANIFEST\_INSERTION hooks within a single request. Time spent by an earlier hook reduces the budget available to later hooks. If the budget is exhausted before a hook starts, MediaTailor skips that hook and continues processing the request. | 
