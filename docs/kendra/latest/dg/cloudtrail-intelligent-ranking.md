...
CloudTrail logs all Amazon Kendra Intelligent Ranking actions, which are documented in the [API Reference](https://docs.aws.amazon.com/kendra/latest/APIReference/API_Reference.html). For example, calls to the `CreateRescoreExecutionPlan` generate entries in the CloudTrail log files.
