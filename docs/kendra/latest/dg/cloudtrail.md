...
CloudTrail logs all Amazon Kendra actions, which are documented in the [API Reference](https://docs.aws.amazon.com/kendra/latest/APIReference/API_Reference.html). For example, calls to the `CreateIndex`, `CreateDataSource`, and `Query` operations generate entries in the CloudTrail log files. 
