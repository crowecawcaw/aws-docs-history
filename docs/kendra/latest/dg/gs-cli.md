...
1. Do the [Prerequisites](gs-prerequisites.md "gs-prerequisites.md").
...
3. Wait for Amazon Kendra to create the index. Check the progress using the
   following command. When the status field is `ACTIVE`, go on to the next
   step.
...
If you connect to your data source using a template schema, configure the template
schema.
...
5. It will take Amazon Kendra a while to create the data source. Enter the following
   command to check the progress. When the status is `ACTIVE`, go on to the next
   step.
...
7. Amazon Kendra will index your data source. The amount of time that it takes
   depends on the number of documents. You can check the status of the sync job using the
   following command. When the status is `ACTIVE`, go on to the next step.
