...
**Topics**
+ [When I try to launch an instance, I don't see the role in the Amazon EC2 console **IAM Role** list](#troubleshoot_iam-ec2_missingrole)
...
+ [Amazon EC2: When I try to launch an instance with a role, I get an `AccessDenied` error](#troubleshoot_iam-ec2_access-denied-launch)
...
Check the following:
+ If you are signed in as an IAM user, verify that you have permission to call `ListInstanceProfiles`. For information about the permissions necessary to work with roles, see [Permissions required for using roles with Amazon EC2](id_roles_use_switch-role-ec2.md#roles-usingrole-ec2instance-permissions). For information about adding permissions to a user, see [Manage IAM policies](access_policies_manage.md).
...
For more information about the permissions necessary to work with roles, see [How do I get started?](id_roles_use_switch-role-ec2.md#roles-usingrole-ec2instance-get-started). For information about adding permissions to a user, see [Manage IAM policies](access_policies_manage.md).
...
+ Call the IAM `GetInstanceProfile` action to ensure that the instance profile has a role. Empty instance profiles will fail with an `AccessDenied` error. For more information about creating a role, see [IAM role creation](id_roles_create.md).
