...
- The statement, `DenyAllAwsResourcesOutsideAccountExceptS3`, uses the
  `NotAction` element with the [Deny](reference_policies_elements_effect.md "reference_policies_elements_effect.md") effect
  which explicitly denies access to every action not listed in the statement that also do not
...
- The statement, `DenyAllS3ResoucesOutsideAccountExceptDataExchange`, uses a
  ...
  service is not AWS Data Exchange. The statement does not deny the actions if either the resource belongs
  to the listed account or the listed service principal,
  `dataexchange.amazonaws.com`, performs the operations.
...
The following example shows how you can configure the policy to allow access to the required
Amazon S3 buckets.
