...
When you interact with AWS, you specify your AWS _security
credentials_ to verify who you are and whether you have permission to access the
...
- **Account owner (root user)** — The user who created
  the AWS account and has full access.
- **AWS IAM Identity Center users** — Users managed in
...
- You can provide access to your AWS resources to users without having to define an
  AWS identity for them. Temporary credentials are the basis for [roles and identity federation](id_roles.md "id_roles.md").
...
- Multi-factor authentication (MFA) provides an extra level of security for users who
  ...
  on the AWS account root user credentials and all IAM users. For more information, see [AWS Multi-factor authentication in IAM](id_credentials_mfa.md "id_credentials_mfa.md").
- AWS requires different types of security credentials, depending on how you access
  ...
  determining your user type and sign-in page, see [What is AWS Sign-In](../../../signin/latest/userguide/what-is-sign-in.md "../../../signin/latest/userguide/what-is-sign-in.md") in the
  _AWS Sign-In User Guide_.
- You can't use IAM policies to deny the root user access to resources explicitly. You
  can only use an AWS Organizations [service control policy (SCP)](../../../organizations/latest/userguide/orgs_manage_policies_type-auth.md "../../../organizations/latest/userguide/orgs_manage_policies_type-auth.md") to limit the permissions of the root user.
...
- Do not use the root user for your everyday tasks. Use it to perform the tasks that only
  ...
  root user, see [Tasks that require root user credentials](id_root-user.md#root-user-tasks "id_root-user.md#root-user-tasks").
...
- [Policies](access_policies.md "access_policies.md") determine what actions a user, role,
  ...
  identity.
- Be sure to save the sign-in credentials for your _Emergency Access_
