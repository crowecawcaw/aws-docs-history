...
programmatically with service-specific credentials, you can use `cqlsh` or
open-source Cassandra drivers. _Service-specific credentials_ include a user
...
If you plan to interact with Amazon Keyspaces only through the console, you don't need to generate
service-specific credentials. For more information, see [Accessing Amazon Keyspaces using
the console](../../../keyspaces/latest/devguide/console_keyspaces.md "../../../keyspaces/latest/devguide/console_keyspaces.md") in the _Amazon Keyspaces (for Apache Cassandra) Developer
Guide_.

For more information about the permissions required to access Amazon Keyspaces, see [Amazon Keyspaces (for Apache Cassandra) Identity-Based Policy Examples](../../../keyspaces/latest/devguide/security_iam_id-based-policy-examples.md#security_iam_id-based-policy-examples-console "../../../keyspaces/latest/devguide/security_iam_id-based-policy-examples.md#security_iam_id-based-policy-examples-console") in the _Amazon Keyspaces (for Apache Cassandra)
Developer Guide_.
...
2. In the navigation pane, choose **Users** and then choose the name of
   the user that requires the credentials.
3. On the **Security Credentials** tab beneath **Credentials for
   Amazon Keyspaces (for Apache Cassandra)**, choose **Generate credentials**.
4. Your service-specific credentials are now available. This is the only time that the
   password can be viewed or downloaded. You cannot recover it later. However, you can reset
   your password at any time. Save the user and password in a secure location, because you'll
   need them later.
