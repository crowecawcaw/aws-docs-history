...
An IAM user group is a collection of
...
called _Admins_ and give that user group typical administrator permissions.
Any user in that user group automatically has _Admins_ group
permissions. If a new user joins your organization and needs administrator privileges you can
assign the appropriate permissions by adding the user to the _Admins_ user group. If a person changes jobs in your organization, instead of
editing that user's permissions you can remove them from the old IAM groups and add them to the
appropriate new IAM groups.

You can attach an identity-based policy to a user group so that all of the users in the user
...
- The number and size of IAM resources in an AWS account, such as the number of
  ...
  information, see [IAM and AWS STS quotas](reference_iam-quotas.md "reference_iam-quotas.md").
