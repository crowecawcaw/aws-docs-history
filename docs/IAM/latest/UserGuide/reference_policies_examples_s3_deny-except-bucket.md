...
except Amazon S3. See the following example. According to this policy, you can only access Amazon S3
actions that you can perform on an S3 bucket or S3 object resource.
...
[NotAction](reference_policies_elements_notaction.md "reference_policies_elements_notaction.md") and
[NotResource](reference_policies_elements_notresource.md "reference_policies_elements_notresource.md") are
advanced policy elements that must be used with care. This policy denies access to every AWS
service except Amazon S3. If you attach this policy to a user, any other policies that grant
permissions to other services are ignored and access is denied.
