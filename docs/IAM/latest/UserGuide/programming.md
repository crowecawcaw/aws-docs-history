...
**Important**  
AWS STS also supports sending requests to regional endpoints in addition to the global endpoint. AWS recommends using regional endpoints instead of global endpoints to reduce latency, build in redundancy, and increase session token validity. Before you can use AWS STS in a Region, you must first activate STS in that Region for your AWS account. For more information about activating additional Regions for AWS STS, see [Manage AWS STS in an AWS Region](id_credentials_temp_enable-regions.md).
