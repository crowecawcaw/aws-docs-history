...
+ [Understanding unique ID prefixes](#identifiers-prefixes)
+ [Getting the unique identifier](#identifiers-get-unique-id)
...
Where:
+ `partition` identifies the partition for the resource. For standard AWS Regions, the partition is `aws`. If you have resources in other partitions, the partition is `aws-{{partitionname}}`. For example, the partition for resources in the China (Beijing) Region is `aws-cn`. You cannot [delegate access](access_policies-cross-account-resource-access.md#access_policies-cross-account-delegating-resource-based-policies) between accounts in different partitions.
...
+ An IAM role:
  ...
  arn:aws:iam::123456789012:role/S3Access
  ```
+ A [service-linked role](id_roles.md#iam-term-service-linked-role):

  ```
  arn:aws:iam::123456789012:role/aws-service-role/access-analyzer.amazonaws.com/AWSServiceRoleForAccessAnalyzer
  ```
+ A [service role](id_roles.md#iam-term-service-role):
...
+ The active session of someone assuming the role of "Accounting-Role", with a role session name of "Mary":
  ...
  ```
+ Represents the caller's own session when used as a resource in an API call, such as the AWS STS [SetContext](https://docs.aws.amazon.com/service-authorization/latest/reference/list_awssecuritytokenservice.html) API, that operates on the calling session:
