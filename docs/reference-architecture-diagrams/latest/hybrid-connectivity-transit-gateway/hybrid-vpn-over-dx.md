...
<a name="hybrid-vpn-over-dx"></a>

Publication date: **August 17, 2022 ([Diagram history](#hc6-diagram-history))**

If you require traffic encryption in your [AWS Direct Connect](https://docs.aws.amazon.com/directconnect/latest/UserGuide/Welcome.html) connection, one of the options to achieve this is to create an AWS Site-to-Site VPN on top of the Direct Connect connection. You have two options: either by using a Public VIF to connect to the VPN public endpoint, or by creating a Private IP VPN on top of a Transit VIF to use private IPs.
...
1. Traffic is sent to the AWS Transit Gateway through the Site-to-Site VPN connection, which is created over the Direct Connect link.

1. As per the **on-premises route table** in the Transit Gateway, traffic is forwarded to the **spoke VPC A** attachment. The TGW ENI of the **spoke VPC A** forwards the traffic to the destination.
...
1. As per the **Transit Gateway spoke VPC route table**, the traffic is forwarded to the AWS Site-to-Site VPN attachment.

1. The traffic is sent to the corporate data center through the Site-to-Site VPN connection on top of the Direct Connect link.

For more information about options to encrypt traffic in AWS Direct Connect, see [Traffic encryption in AWS Direct Connect](https://docs.aws.amazon.com/directconnect/latest/UserGuide/encryption-in-transit.html).

## Further reading
<a name="hc6-further-reading"></a>
...
| [Initial publication](#hc6-diagram-history) | Reference architecture diagram first published. | August 17, 2022 | 
