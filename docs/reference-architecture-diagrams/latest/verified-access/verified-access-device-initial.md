...
<a name="verified-access-device-initial"></a>

Publication date: **February 22, 2023 ([Diagram history](#vdi-diagram-history))**

This flow shows how AWS Verified Access handles an initial request when both [IAM Identity Center](https://docs.aws.amazon.com/singlesignon/latest/userguide/what-is.html) and a device trust provider are configured. The browser extension detects redirects and provides device information cookies for combined identity and device posture validation.
...
1. The browser extension detects the 302 redirect but does not identify the application domain as an AWS Verified Access domain. No device information cookie is added.

1. AWS Verified Access expects device information but does not receive it. It redirects the user to the device validation domain. AWS Verified Access repeats the authentication steps, but IAM Identity Center bypasses sign-in because cookies from the previous sign-in exist.

1. The device validation domain sends a 302 redirect. This tells the browser extension to pass the device information cookie to the application domain.

1. The browser extension extracts the application domain from the redirect and adds it to the cache of trusted AWS Verified Access domains. It sets the device information cookie on the application domain.

1. The browser extension allows the redirect to continue.
...
| [Initial publication](#vdi-diagram-history) | Reference architecture diagram first published. | February 22, 2023 | 
