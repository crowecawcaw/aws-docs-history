...
Publication date: **February 22, 2023 ([Diagram history](#vds-diagram-history "#vds-diagram-history"))**
...
![Architecture diagram showing AWS Verified Access subsequent request flow with device trust provider where the browser extension automatically includes device information.](images/verified-access-4.png)
...
2. The user sends the initial request to the AWS Verified Access endpoint. The identity cookie and device information cookie are included with the request to the application domain.
3. AWS Verified Access receives the request with the user identity cookie and device information cookie. For each request, it validates the user request against the policy using both the user identity and device posture.
4. AWS Verified Access proxies validated requests to application endpoints in the customer Amazon VPC.
