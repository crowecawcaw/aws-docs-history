...
1.  The virtual private gateway is associated with the **Direct Connect** gateway, which uses a private VIF (VLAN) to connect to the corporate data center. To achieve segmentation, use several private VIFs. The on-premises router must be configured to use a different VIF (VLAN) connection per VRF configured. 

1.  The on-premises router forwards the traffic from private VIF A to VRF A and the destination server. 

1.  Traffic from the corporate data center’s VRF B destined for Spoke VPC B is first sent to the customer gateway located in the data center. 

1.  The customer gateway uses private VIF B – over the Direct Connect link – to send the traffic to the virtual private gateway in VPC B. 

1.  Traffic is forwarded to the destination according to the Spoke VPC B route table. 
...
1.  Transit Gateway is associated with several Direct Connect gateways (DXGW), each one of them using a transit VIF (VLAN) to connect to the corporate data center, achieving traffic segmentation. The on-premises router must be configured to use a different VIF (VLAN) connection per VRF configured. 

1.  Traffic is forwarded to DXGW A according to the Segment A **Transit Gateway** route table associated with the VPC A attachment. 

1.  The on-premises router forwards the traffic from the transit VIF A to VRF A and the destination server. 

1.  Traffic from the corporate data center’s VRF B destined for Spoke VPC B is first sent to the customer gateway located in the data center. 

1.  The customer gateway uses transit VIF B – over the **Direct Connect** link – to send the traffic to **Transit Gateway** Segment B. 

1.  Traffic is forwarded to VPC B according to the Segment B **Transit Gateway** route table. 

1.  Traffic is forwarded to the destination according to the VPC B route table. 
...
1.  Traffic is forwarded to **Transit Gateway**. Traffic is then routed to the corporate data center through the **Transit Gateway** Connect attachment A according to the Segment A route table. 

1.  The **Transit Gateway** Connect attachment uses the **Direct Connect** connection as transport and connects **Transit Gateway** to the corporate data center router using Generic Routing Encapsulation (GRE) tunneling and Border Gateway Protocol (BGP). The on-premises router must be configured to use a different connect attachment per VRF configured. 

1.  The on-premises router forwards the traffic from Connect attachment A to VRF A and the destination server. 

1.  Traffic from the corporate data center’s VRF B destined for Spoke VPC B is first sent to the customer gateway located in the data center. 

1.  The customer gateway uses the GRE tunnel of the **Transit Gateway** Connect attachment B – over the **Direct Connect** link – to send the traffic to **Transit Gateway**. 

1.  Traffic is forwarded to the Spoke VPC B attachment according to the **Transit Gateway** Segment B route table. 

1.  The TGW ENI of Spoke VPC B forwards the traffic to the destination. 
...
1.  Traffic is forwarded to **Transit Gateway**. Traffic is then routed to the corporate data center through **Site-to-Site VPN** connection A according to the Segment A route table. 

1.  The **Site-to-Site VPN** connection is created using a public VIF as transport (VLAN). The on-premises router must be configured to use a different VPN connection per VRF configured. 

1.  The on-premises router forwards the traffic from **Site-to-Site VPN** connection A to the VRF A and the destination server. 

1.  Traffic from the corporate data center’s VRF B destined for Spoke VPC B is first sent to the customer gateway located in the data center. 

1.  The customer gateway chooses **Site-to-Site VPN** connection B to send the traffic to the **Transit Gateway** over the **Direct Connect** link. 
...
1.  Traffic is forwarded to **Transit Gateway**. Traffic is then routed to the corporate data center through **Transit Gateway** VPN attachment A according to the Segment A route table. 

1.  The **Transit Gateway** **Site-to-Site VPN** attachment uses the **Direct Connect** connection as transport, and connects **Transit Gateway** to the corporate data center router using one IPSec VPN connection (with two tunnels). The on-premises router must be configured to use a different VPN attachment per VRF configured. 

1.  The on-premises router forwards the traffic from VPN attachment A to VRF A and the destination server. 

1.  Traffic from the corporate data center’s VRF B destined for Spoke VPC B is first sent to the customer gateway located in the data center. 

1.  The customer gateway uses **Site-to-Site VPN** connection B – over the **Direct Connect** link – to send the traffic to **Transit Gateway**. 
...
| --- |--- |--- |
| [Diagram updated](#diagram-history) | Added *Segment Your Traffic over AWS Direct Connect by Using Several Transit VIFs and Direct Connect Gateways*  | November 9, 2023 | 
| [Initial publication](#diagram-history) | Reference architecture diagram first published. | June 30, 2022 | 
