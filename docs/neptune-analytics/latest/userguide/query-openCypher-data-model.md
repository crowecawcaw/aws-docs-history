...
| `MERGE (n)-[knows]->(m)`                                                               | Adds two vertices with label "vertex" and an edge with label "knows" to the graph, if an edge with label knows does not exist. `MATCH (n)` returns both those vertices. | Adds an edge between two new vertices to the graph, if an edge with label "knows" does not exist. The two vertices have no label. `MATCH (n)` returns both those vertices. | ###### Note A workaround to implicit deletion of a vertex when all of its labels, properties, and edges get removed is to assign immutable labels to all vertices. This way, the deletion of all properties, edges, or mutable labels of a vertex will not lead to an implicit deletion. A vertex would not get deleted until explicitly deleted. Likewise a workaround to no-op vertex create queries is to always create a vertex with a label or a property. To combine it with the previous point, always create a vertex with an immutable label. Extending this to bulk or batch loads, include all vertices in some vertex files and assign a property or an immutable label to all vertices. |
