...
- Only explicitly provided labels are associated with the vertices. If the label provided is empty, the vertex is
...
- Unlike Neptune Database, a vertex identifier can appear just in edge files. Neptune Analytics allows loading just the edge data from files in
...
support](vector-index.md#vector-index-transaction-support "vector-index.md#vector-index-transaction-support")), that is they become durable and visible to other queries immediately upon write,
...
For more details of loading embeddings, refer to
...
- Float and double values in scientific notation are currently not supported. However, `Infinity`, `INF`,
  `-Infinity`, `-INF`, and `NaN` (`Not-a-number`) values are supported.
- The maximum length of the strings supported is limited to 1,048,062 bytes. The limit is lower for strings
