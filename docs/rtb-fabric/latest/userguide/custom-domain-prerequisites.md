...
If you need to accept both HTTP and HTTPS traffic, configure the gateway with a listener configuration that includes both protocols. See [Listener configuration](custom-domain-concepts.md#custom-domain-concept-listener-config "custom-domain-concepts.md#custom-domain-concept-listener-config").
...
Replace `CERTIFICATE_ARN` with the ARN of your ACM certificate (for example, `arn:aws:acm:us-east-1:`ACCOUNT_ID`:certificate/abcd1234-5678-90ef-ghij-klmnopqrstuv`).
