...
- _Filter and no-bid tracking_ – Monitor `filter-transaction` to understand traffic filtering by modules, and `no-bid-external` / `no-bid-internal` for no-bid rates.
- _Infrastructure health_ – Monitor `target-ip-count` and `healthy-target-ip-count` to track the availability of target endpoints. If [Health checks for Managed Endpoints](health-checks-for-managed-endpoints.md "health-checks-for-managed-endpoints.md") are enabled, compare `healthy-target-ip-count` against `target-ip-count` to identify instances failing health checks.
