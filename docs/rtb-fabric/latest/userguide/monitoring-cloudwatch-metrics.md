...
| `healthy-target-ip-count` | The number of target IP addresses that passed [Health checks for Managed Endpoints](health-checks-for-managed-endpoints.md "health-checks-for-managed-endpoints.md") and are receiving traffic. This metric is only available when health checks are enabled on Auto Scaling group managed endpoints.<br>Valid Statistics: Sum, Average<br>Units: Count |
