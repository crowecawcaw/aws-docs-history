...
**Topics**
+ [Initiating on-demand key rotation (console)](#rotate-on-demand-console)
+ [Initiating on-demand key rotation (AWS KMS API)](#rotate-on-demand-api)
...
1. Choose the **Key material and rotations** tab.
   ...
   The **Key material and rotations** tab appears only on the detail page of symmetric encryption KMS keys that support automatic or on-demand rotation. This includes KMS keys with key material that AWS KMS generated (**AWS\_KMS** origin) and KMS keys with imported key material (**EXTERNAL** origin) 
