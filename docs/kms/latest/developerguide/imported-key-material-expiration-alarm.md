...
| Field | Value | 
| --- | --- | 
| Select metric | Choose **KMS**, then choose **Per-Key Metrics**.<br />Choose the row with the KMS key and the `SecondsUntilKeyMaterialExpires` metric. Then choose **Select metric**.<br />The **Metrics** list displays the `SecondsUntilKeyMaterialExpires` metric only for KMS keys with imported key material that expires. If you don't have KMS keys with these properties in the account and Region, this list is empty. | 
| Statistic | Minimum | 
| Period | 1 minute | 
