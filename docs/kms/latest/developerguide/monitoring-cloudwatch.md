...
| KeySpec | Value for each type of KMS key. The only supported [key spec](create-keys.md#key-spec) for KMS keys in an AWS CloudHSM key store is SYMMETRIC\_DEFAULT. | 
