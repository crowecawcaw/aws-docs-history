...
+ **Multi-Region keys**
  ...
  + Use the [`ImportKeyMaterial`](https://docs.aws.amazon.com/kms/latest/APIReference/API_ImportKeyMaterial.html) operation with the `ImportType` parameter set to **NEW\_KEY\_MATERIAL** for the primary Region key to accomplish this task. For the replica Region key, use the **EXISTING\_KEY\_MATERIAL** parameter for `ImportType` for the `ImportKeyMaterial` operation.
