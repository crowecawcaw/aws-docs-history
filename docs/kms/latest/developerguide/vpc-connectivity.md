...
+ [Step 1: Create an Amazon VPC and subnets](#xks-create-vpc)
+ [Step 2: Create a target group](#xks-target-group)
+ [Step 3: Create a network load balancer](#xks-nlb)
+ [Step 4: Create a VPC endpoint service](#xks-vpc-svc)
+ [Step 5: Verify your private DNS name domain](#xks-private-dns)
+ [Step 6: Authorize AWS KMS to connect to the VPC endpoint service](#xks-vpc-authorize-kms)
...
| Field | Value | 
| --- | --- | 
| AWS KMS ARN | cks.kms.{{<region>}}.amazonaws.comFor example, `cks.kms.us-east-1.amazonaws.com` | 
...
| --- | --- | 
| AWS KMS ARN | cks.kms.{{<region>}}.amazonaws.comFor example, `cks.kms.us-east-1.amazonaws.com` | 
| AWS account ARN | arn:aws:iam::{{111122223333}}:role/{{role\_name}}For example, `arn:aws:iam::{{123456789012}}:role/{{cks_role}}` | 
