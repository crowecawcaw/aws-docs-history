...
| --- | --- | --- | --- | --- | --- | 
| [Customer managed key](#customer-mgn-key) | Yes | Yes | Yes | Optional. | Monthly fee (pro-rated hourly)<br />Per-use fee | 
| [AWS managed key](#aws-managed-key) | Yes | No | Yes | Required. Every year (approximately 365 days). | No monthly fee<br />Per-use fee (some AWS services pay this fee for you) | 
...
*AWS managed keys* are KMS keys in your account that are created, managed, and used on your behalf by an [AWS service integrated with AWS KMS](https://aws.amazon.com/kms/features/#aws-service-integration).
