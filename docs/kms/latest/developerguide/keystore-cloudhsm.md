...
To decide whether an AWS CloudHSM key store is right for your requirements, see [Choosing the right key store](key-store-overview.md#choosing-key-store "key-store-overview.md#choosing-key-store").

**How do AWS CloudHSM key stores work?**

Each AWS CloudHSM key store is associated with an AWS CloudHSM cluster in your AWS account. When you
connect the AWS CloudHSM key store to its cluster, AWS KMS creates the network infrastructure to
...
You create and manage your AWS CloudHSM key stores in AWS KMS and create and manage your HSM
clusters in AWS CloudHSM. When you create AWS KMS keys in an AWS CloudHSM key store, you view
...
![Managing KMS keys in an AWS CloudHSM key store.](images/kms-hsm-view.png)
You can [create symmetric encryption KMS keys](create-cmk-keystore.md "create-cmk-keystore.md")
with key material generated by AWS KMS in your AWS CloudHSM key store. Then use the same techniques
to view and manage the KMS keys in your AWS CloudHSM key store that you use for KMS keys in
...
view, audit, and manage the key material for your KMS keys. While the AWS CloudHSM key store is
disconnected, AWS KMS cannot access it, and users cannot use the KMS keys in the AWS CloudHSM key
store for cryptographic operations. This added layer of control makes AWS CloudHSM key stores a
