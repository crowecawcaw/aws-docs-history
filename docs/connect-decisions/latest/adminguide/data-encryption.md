...
2. **Create an Amazon Connect Decisions instance** and select that key.
3. **(Optional, recommended)** Update the key policy with instance-level restrictions.

##### Step 1 — Create a key with the base policy

Open the [AWS KMS console](https://console.aws.amazon.com/kms "https://console.aws.amazon.com/kms"), choose **Create key**, and replace the default policy with the following. Replace `<customer-account-id>` with your AWS account ID and `<region>` with your AWS Region.
...
You select the KMS key during instance creation in the Amazon Connect Decisions console. For more information about creating an instance, see [Creating an Amazon Connect Decisions instance](creating-your-instance.md "creating-your-instance.md").
...
For more information, see [Logging AWS KMS API calls with AWS CloudTrail](../../../kms/latest/developerguide/logging-using-cloudtrail.md "../../../kms/latest/developerguide/logging-using-cloudtrail.md").
