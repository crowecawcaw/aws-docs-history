...
To enable Amazon Connect Decisions to perform operations on your behalf in your Enterprise
Resource Planning (ERP) system, you must configure a Connection that specifies the necessary
connection parameters and the credentials Amazon Connect Decisions should use during the operation.
...
1. Using your account, use [Secrets Manager](../../../secretsmanager.md "../../../secretsmanager.md")
   ...
   - If using the console, choose `Other type of secret` for the
     `Secret Type`
   ...
   - If using the `Plaintext` tab to enter the secret in the console,
     enter a JSON object with the two pairs:
...
2. Update the inline policy for your instance role to grant permission on the key
   ...
   - Note: Replace `<YourSecretArn>`,
     ...
     and AWS Supply Chain Instance ID.
...
4. In the `Create Connector` page, click the `Select` button
...
5. In the `Configure SAP S/4HANA API Connector` page enter the necessary
   ...
   - `Secret ARN`: the Amazon Resource Name (ARN) of the secret in
     Secrets Manager where the credentials to be used by Amazon Connect Decisions are stored
