...
**X-axis tick label maximum length** sets the maximum
length of bar chart labels. Labels longer than the maximum length are truncated
(with ellipses).

**Bar labels minimum spacing** sets the minimum
spacing between bar labels.
...
- Percent – Bars will be stacked on top
  ...
  height of the stack (all bar stacks will be the same height, adding up to
  100 percent).
...
**Bar radius** controls the radius of the bars, 0 =
minimum and 0.5 = maximum radius.

**Highlight full area on hover** controls if the
surrounding area of the bar is highlighted when you hover over the bar with a
pointer.
...
color scheme field option. Gradient appearance is influenced by the **Fill
opacity** setting.
...
how the legend appears in your visualization. For more information, see
