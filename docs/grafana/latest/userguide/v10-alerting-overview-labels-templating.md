...
**Note**  
If you are using classic conditions then `$labels` will not contain any labels from the query. Refer to [the $labels variable](#v10-alerting-overview-labels-templating-the-labels-variable) for more information.
...
The `graphLink` function returns the path to the graphical view in [Explore in Grafana version 10](v10-explore.md) for the given expression and data source.
...
The `tableLink` function returns the path to the tabular view in [Explore in Grafana version 10](v10-explore.md) for the given expression and data source.
