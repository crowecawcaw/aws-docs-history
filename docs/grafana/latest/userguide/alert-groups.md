...
Alert groups show grouped alerts from an Alertmanager instance. By default, the alerts are grouped by the label keys for the root policy in [Working with notification policies](alert-notifications.md). Grouping common alerts into a single alert group prevents duplicate alerts from being initiated.
...
1. From the **custom group by** drop-down, select a combination of labels to view a grouping other than the default. You can use this view to debug or verify your grouping of notification policies.
