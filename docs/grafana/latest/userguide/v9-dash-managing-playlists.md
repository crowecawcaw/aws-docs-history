...
| Kiosk mode |  +  The side menu, navigation bar, ro and panel controls are completely hidden/removed from view. <br />+  You can enable it manually using the d v sequence shortcut after the playlist has started. <br />+  You can disable it manually with the same shortcut.   | 
