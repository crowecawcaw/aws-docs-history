...
| Values       | KeyValue  | The values of all expressions, including Classic<br>Conditions                                                                                       |
...
| SilenceURL   | string    | A link to silence the alert (with labels for this<br>alert pre-filled). Only for Grafana managed<br>alerts.                                          |
...
| Status            | `string`   | The status is `firing if at least one alert is<br>firing, otherwise<br>`resolved`.` | `{{ .Status }}`                                           |
