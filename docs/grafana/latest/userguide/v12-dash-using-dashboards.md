...
| **8. Time picker dropdown** | Use to select relative time range options and set custom absolute time ranges.<br />You can change the **Timezone** and **fiscal year** settings from the time range controls by clicking the **Change time settings** button.<br />Time settings are saved on a per-dashboard basis. | 
