...
| External ID              | If you are assuming a role in another account, that has been<br>created with an external ID, specify the external ID here. |

### Authentication

This section covers the different types of authentication that you can use
for X-Ray data source.

#### IAM roles

Currently, all access to X-Ray is done server side by the Grafana
workspace backend using the official AWS SDK. If your Grafana server
is running on AWS, you can use IAM roles and authentication will be
...
and EC2 tags/instances/regions. You can attach these permissions to IAM
roles and use the built-in Grafana support for assuming roles.
