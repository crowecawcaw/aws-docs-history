...
|  tag\_values(server)  |  Return tag values for the specified tag.  | 
|  tag\_values(server, server=\~backend\\\*)  |  Returns filtered tag values that occur for the specified tag in series matching those expressions.  | 
|  tag\_values(server, server=\~backend\\\*, app=\~${apps:regex})  |  Multiple filter expressions and expressions can contain other variables.  | 
