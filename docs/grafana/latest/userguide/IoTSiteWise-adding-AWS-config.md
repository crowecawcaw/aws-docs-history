...
1. Choose the **Data sources** tab. Then select the check box for **AWS IoT SiteWise**, and choose **Actions**, **Enable service-managed policy**.

1. Choose the **Data sources** tab again, and then choose **Configure in Grafana** in the **AWS IoT SiteWise** row.

1. Sign into the Grafana workspace console using IAM Identity Center if necessary.

1. In the left navigation bar in the Grafana workspace console, choose the AWS icon and then choose **AWS services**, **IoT SiteWise**.

1. Select the default Region that you want the AWS IoT SiteWise data source to query from, select the accounts, and then choose **Add data source**.