...
add X-Ray as a data source

To use AWS data source configuration, first you use the Amazon Managed Grafana console to
enable service-mananged IAM roles that grant the workspace the IAM policies
necessary to read the X-Ray resources in your account or in your entire
organizational units. Then you use the Amazon Managed Grafana workspace console to add X-Ray
as a data source.

###### To use AWS data source configuration to add X-Ray as a data
...
5. Choose the **Data sources** tab. Then select the
   check box for **AWS X-Ray**, and choose
...
8. In the left navigation bar in the Grafana workspace console, choose
   ...
   **X-Ray**.
9. Select the default Region that you want the X-Ray data source to
