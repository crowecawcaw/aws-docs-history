...
A label matchers consists of 3 distinct parts, the **label**, the **value** and the
**operator**.
...
| `!~`     | Select labels that do not regex-match the value.   | If you are using multiple label matchers, they are combined using the AND logical operator. This means that all matchers must match in order to link a rule to a policy. **Example scenario** If you define the following set of labels for your alert: `{ foo=bar, baz=qux, id=12 }` then: <br>• A label matcher defined as `foo=bar` matches this alert rule. <br>• A label matcher defined as `foo!=bar` does _not_ match this alert rule. <br>• A label matcher defined as `id=~[0-9]+` matches this alert rule. <br>• A label matcher defined as `baz!~[0-9]+` matches this alert rule. <br>• Two label matchers defined as `foo=bar` and `id=~[0-9]+` match this alert rule. |
