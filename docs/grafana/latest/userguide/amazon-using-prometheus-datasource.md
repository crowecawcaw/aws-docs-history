...
| `Url`                     | The URL of your Amazon Managed Service for Prometheus workspace; for<br>example, `https://aps-workspaces.us-east-1.amazonaws.com/workspaces/ws-example1234/`.                                                      |
...
your labels app, job, job\_name only one might with the value you are looking
for.
...
Min step setting (AKA query\_interval, a setting per PromQL query), if
...
- `$<varname>` Example:
  rate(http\_requests\_total{job=~"$job"}[5m])
- `[[varname]]` Example:
  rate(http\_requests\_total{job=~"[[job]]"}[5m])
