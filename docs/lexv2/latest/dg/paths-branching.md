...
| Set slot values                            | Set slot values using session attributes and transcriptions JSON path | `{slotName} = [<sessionAttribute>] AND {slotName} = "$.transcriptions..."`                                | ###### Note `slotName` refers to the name of a slot in the bot. If the slot is not resolved (null), or if the slot does not exist, then the assignments are ignored at runtime. `sessionAttribute` refers to the name of the session attribute that is set by the customer at build time. |
