...
+ **Built-in Intents** – Leverages Amazon Lex V2 built-in intents for common interactions:
  + [AMAZON.HelpIntent](built-in-intent-help.md) - Provides help and guidance
  + [AMAZON.CancelIntent](built-in-intent-cancel.md) - Handles cancellation requests
  + [AMAZON.StopIntent](built-in-intent-stop.md) - Ends conversations gracefully
...
+ **Built-in Slot Types** – Uses Amazon Lex V2 built-in slots for common data formats:
  + [AMAZON.Date](built-in-slot-date.md) - For delivery dates and callback scheduling
  + [AMAZON.Time](built-in-slot-time.md) - For callback times and delivery windows
  + [AMAZON.EmailAddress](built-in-slot-email.md) - For customer email collection
  + [AMAZON.PhoneNumber](built-in-slot-phone.md) - For customer phone numbers
  + [AMAZON.Number](built-in-slot-number.md) - For order numbers and quantities
...
+ **Required Slots:**
  + `OrderNumber` ([AMAZON.Number](built-in-slot-number.md)) - Customer's order number
  + `CustomerEmail` ([AMAZON.EmailAddress](built-in-slot-email.md)) - Email for verification
+ **Sample Utterances:**
...
+ **Required Slots:**
  ...
  + `Budget` ([AMAZON.Number](built-in-slot-number.md)) - Customer's budget range
+ **Sample Utterances:**
...
+ **Required Slots:**
  + `CustomerFirstName` ([AMAZON.FirstName](built-in-slot-first-name.md)) - Customer's first name for personalization
  + `CustomerLastName` ([AMAZON.LastName](built-in-slot-last-name.md)) - Customer's last name for sales history lookup
  + `CustomerEmail` ([AMAZON.EmailAddress](built-in-slot-email.md)) - Email address
  + `CustomerPhone` ([AMAZON.PhoneNumber](built-in-slot-phone.md)) - Phone number
...
+ **AWS Account:** You need an active AWS account with billing enabled. If you don't have one, sign up at [aws.amazon.com](https://aws.amazon.com).
...
**Important**  
For current pricing information, visit the [Amazon Lex Pricing page](https://aws.amazon.com/lex/pricing/) as rates may change.
...
1. In the **Bot configuration** section:
...
1. In the **Permissions** section, choose **Create a new role with basic Amazon Lex permissions**.

1. In the **Children's Online Privacy Protection Act (COPPA)** section, choose **No**.

1. In the **Session timeout** section, set to **15 minutes** to allow for complex multi-turn conversations.

1. Choose **Next**.

1. Add English (US) as the language and choose **Done**.
...
1. Choose **Add slot type** and create the following slot types:
...
1. Choose **Add intent** and name it **CheckOrderStatus**.

1. Add the following sample utterances:
   + "What's the status of my order {OrderNumber}"
   + "I need to check on order number {OrderNumber}"
   + "Where is my package"
   + "Track my order {OrderNumber}"
...
1. Add the following slots:
   + `OrderNumber` ([AMAZON.Number](built-in-slot-number.md)) - Required

     Prompt: "Can you provide your order number?"
   + `CustomerEmail` ([AMAZON.EmailAddress](built-in-slot-email.md)) - Required

     Prompt: "To verify your identity, please provide the email address associated with this order."

1. Set the confirmation prompt: "Let me check order \#{OrderNumber} for {CustomerEmail}. Is this correct?"

1. Set the fulfillment message: "I found your order\! Order \#{OrderNumber} is {OrderStatus}. {UpsellMessage}"
...
1. Add slots:
   ...
   + `Budget` ([AMAZON.Number](built-in-slot-number.md)) - Optional
...
1. Add slots:
   + `CustomerFirstName` ([AMAZON.FirstName](built-in-slot-first-name.md)) - Required

     Prompt: "What's your first name for personalized service?"
   + `CustomerLastName` ([AMAZON.LastName](built-in-slot-last-name.md)) - Required

     Prompt: "And your last name so I can look up your purchase history?"
   + `CustomerEmail` ([AMAZON.EmailAddress](built-in-slot-email.md)) - Required
   + `CustomerPhone` ([AMAZON.PhoneNumber](built-in-slot-phone.md)) - Optional
...
1. Under **Generative AI**, enable:
   + Assisted slot resolution
   + Descriptive bot building

1. Under **Sentiment Analysis**, choose **Enable**.
...
1. Once the build is complete, choose **Test**.

1. Test the following conversation flows:
...
1. Verify that AI features are working:
...
1. Once testing is complete, choose **Publish** to deploy your bot.
