...
`AMAZON.KendraSearchIntent` intent. When Amazon Lex V2
...
When you use the `AMAZON.KendraSearchIntent` with
the `AMAZON.FallbackIntent` in the same bot, Amazon Lex V2
uses the intents as follows:

1. Amazon Lex V2 calls the
   `AMAZON.KendraSearchIntent`. The intent
   calls the Amazon Kendra `Query` operation.
2. If Amazon Kendra returns a response, Amazon Lex V2 displays the
   result to the user.
3. If there is no response from Amazon Kendra, Amazon Lex V2 re-prompts
   ...
   - If the response from the user does not contain
     ...
     makes another call to the `Query`
     operation.
...
- Let the search intent make the request for you. Amazon Lex V2
...
- Add additional query parameters to the request to
  ...
  add a `kendraQueryFilterString` field that
...
- Create a new query using the Lambda function. You can
...
`kendraQueryRequestPayload`field in your Lambda
...
`AMAZON.KendraSearchIntent` using the Amazon Lex V2
...
the `AMAZON.KendraSearchIntent` for your bot.
...
For more information about request attributes, see [Setting request attributes for your Lex V2 bot](context-mgmt-request-attribs.md "context-mgmt-request-attribs.md"). For more
information about session attributes, see [Setting session attributes for your Lex V2 bot](context-mgmt-session-attribs.md "context-mgmt-session-attribs.md").
...
`AMAZON.KendraSearchIntent` must use
...
the intent's `IntentClosingSetting` statement.
...
The `closingResponse` statement must have one
...
to `delegate`. Amazon Lex V2 sends the query to
