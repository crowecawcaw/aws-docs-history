...
A _resource-based policy_ is attached to a
resource, such as a bot or a bot alias. With a resource-based policy
you can specify who has access to the resource and the actions that
they can perform on it. For example, you can add resource-based
policies that enable a user to modify a specific bot, or to allow a
user to use runtime operations on a specific bot alias.
...
- [Use the console to specify a resource-based policy](#security_iam_resource-based-policy-examples-console "#security_iam_resource-based-policy-examples-console")
- [Use the API to specify a resource-based policy](#security_iam_resource-based-policy-examples-api "#security_iam_resource-based-policy-examples-api")
- [Allow an IAM role to update a bot and list bot aliases](#security_iam_resource-based-policy-examples-allow-lex-models "#security_iam_resource-based-policy-examples-allow-lex-models")
...
the console to view and modify the policy.

When you save a policy with the policy editor, the console checks
the syntax of the policy. If the policy contains errors, such as a
non-existent user or an action that is not supported by the
resource, it returns an error and doesn't save the policy.
...
2. From the **Bots** list, choose the bot
   whose policy you want to edit.
3. In the **Resource-based policy** section,
   choose **Edit**.
...
1. Sign in to the AWS Management Console and open the Amazon Lex console at
   [https://console.aws.amazon.com/lex/](https://console.aws.amazon.com/lex/ "https://console.aws.amazon.com/lex/").
2. From the **Bots** list, choose the bot
   that contains the alias that you want to edit.
3. From the left menu, choose **Aliases**,
   then choose the alias to edit.
...
[CreateResourcePolicy](../APIReference/API_CreateResourcePolicy.md "../APIReference/API_CreateResourcePolicy.md")

Adds a new resource policy with the specified policy
statements to a bot or bot alias.

[CreateResourcePolicyStatement](../APIReference/API_CreateResourcePolicyStatement.md "../APIReference/API_CreateResourcePolicyStatement.md")

Adds a new resource policy statement to a bot or bot
alias.

[DeleteResourcePolicy](../APIReference/API_DeleteResourcePolicy.md "../APIReference/API_DeleteResourcePolicy.md")

Removes a resource policy from a bot or bot
alias.

[DeleteResourcePolicyStatement](../APIReference/API_DeleteResourcePolicyStatement.md "../APIReference/API_DeleteResourcePolicyStatement.md")

Removes a resource policy statement from a bot or bot
alias.
...
[UpdateResourcePolicy](../APIReference/API_UpdateResourcePolicy.md "../APIReference/API_UpdateResourcePolicy.md")

Replaces the existing resource policy for a bot or bot
alias with a new one.

Java
The following example shows how to use the
resource-based policy operations to manage a
...
The following example grants permissions for a specific IAM
role to call Amazon Lex V2 model building API operations to modify an
existing bot. The user can list aliases for a bot and update the
bot, but can't delete the bot or bot aliases.
...
call Amazon Lex V2 runtime API operations on a single alias of a
bot.

The user is specifically denied permission to update or delete
the bot alias.
...
use the global context keys `AWS:SourceAccount` and
`AWS:SourceArn`.
