...
create or update an intent. When the intent is fulfilled, responses
from Amazon Lex V2 contain the context and slot values from the intent as
context parameters. You can use these parameters as default values
in subsequent intents or in you application code or Lambda
functions.

To use session attributes, you set the attributes in your Lambda or
application code. For example, a user of the
`ShoeOrdering` bot starts by ordering shoes. The bot
...
`orderNumber` session attribute, which contains the
