...
1. Multi-party approval creates an approval session

1. Approvers receive email notifications prompting them to respond to the requested operation
...
If these conditions are met, the requester can use Multi-party approval APIs (such as [GetSession](https://docs.aws.amazon.com/mpa/latest/APIReference/API_GetSession.html)) to check the status of the request (`PENDING`, `CANCELLED`, `APPROVED`, `FAILED`, `CREATING`).
...
1. Approvers view details for the request including the following non-exhaustive items:
...
Approvers do not receive email notifications about the session result or the execution status for the protected operation. However, approvers can view the session result in the Multi-party approval portal. For more information, see [View operation history](view-operation-history.md).
...
## Considerations
<a name="how-it-works-considerations"></a>
