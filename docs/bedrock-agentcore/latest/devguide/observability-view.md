...
If you have enabled observability by instrumenting your agent code as described in [Enabling AgentCore observability](observability-configure.md#observability-configure-builtin "observability-configure.md#observability-configure-builtin") , your agent automatically generates OTEL metrics, which are sent to CloudWatch using Enhanced Metric Format (EMF):
...
In addition to agent-emitted metrics, the AgentCore service publishes standard service metrics to CloudWatch. Refer to [AgentCore generated runtime observability data](observability-runtime-metrics.md "observability-runtime-metrics.md") for a list of these metrics.
