...
FGAC for Memory is implemented with [Policy in Amazon Bedrock AgentCore](policy.md). You attach a *policy engine* to the [gateway](memory-gateway-connector.md) that fronts your Memory resource and write policies in Cedar, an open-source policy language documented on the [Cedar Policy website](https://www.cedarpolicy.com/). The gateway evaluates these policies before it forwards a request to Memory. The Cedar policy language, principal types, policy engines, and policy validation are all documented in [Policy in Amazon Bedrock AgentCore](policy.md) — this page describes only what is specific to Memory: the actions and request attributes the Memory connector exposes, and the policy patterns that isolate Memory data by caller.

**Note**  
FGAC for Memory is built on the AgentCore Memory connector. Set up a gateway with a Memory connector target first. For more information, see [Access AgentCore Memory through a gateway](memory-gateway-connector.md).

**Topics**
+ [How fine-grained access control works](#memory-gateway-fgac-how)
+ [Set up fine-grained access control for Memory](#memory-gateway-fgac-setup)
+ [Memory actions and request attributes](#memory-gateway-fgac-reference)
+ [What you can enforce](#memory-gateway-fgac-enforced)
+ [Adopt fine-grained access control on an existing Memory](#memory-gateway-fgac-adopting)
+ [Relationship to other access-control options](#memory-gateway-fgac-relationship)
+ [Policy examples for Memory](memory-gateway-fgac-policy-examples.md)

## How fine-grained access control works
<a name="memory-gateway-fgac-how"></a>
...
1. Create a policy engine and add your Cedar policies. For the steps, see [Create a policy engine](policy-create-engine.md) and [Create a policy](policy-create-policies.md). For the policies to write, see [Policy examples for Memory](memory-gateway-fgac-policy-examples.md).

1. Associate the policy engine with the gateway that fronts your Memory resource, by setting the gateway’s `policyEngineConfiguration`. You can set it when you create the gateway with [CreateGateway](https://docs.aws.amazon.com/bedrock-agentcore-control/latest/APIReference/API_CreateGateway.html), or add it later with [UpdateGateway](https://docs.aws.amazon.com/bedrock-agentcore-control/latest/APIReference/API_UpdateGateway.html).

The policy engine’s `mode` controls whether policies are enforced (`ENFORCE`) or only evaluated and logged without blocking traffic (`LOG_ONLY`). Test your policies in `LOG_ONLY` before switching to `ENFORCE` to avoid unintended denials. For enforcement modes, policy validation, and testing, see [Validate and test policies](policy-validate-policies.md) and [Use policies](policy-use-policies.md).

## Memory actions and request attributes
<a name="memory-gateway-fgac-reference"></a>
...
### Memory action ids
<a name="memory-gateway-fgac-action-ids"></a>
...
<a name="memory-gateway-fgac-context-fields"></a>
...
Scope each policy to the specific actions whose requests carry the fields the policy references, and confirm those fields against each operation in the [Memory action ids](#memory-gateway-fgac-action-ids) table.
...
Test policies in `LOG_ONLY` mode before enforcing them, so you can observe evaluation outcomes without denying live traffic. For more information, see [Set up fine-grained access control for Memory](#memory-gateway-fgac-setup).

## What you can enforce
<a name="memory-gateway-fgac-enforced"></a>
...
For policies that implement these, see [Policy examples for Memory](memory-gateway-fgac-policy-examples.md).

## Adopt fine-grained access control on an existing Memory
<a name="memory-gateway-fgac-adopting"></a>
...
**Tip**  
You can validate any of these approaches without affecting live traffic by attaching the policy in `LOG_ONLY` mode first and reviewing the evaluation logs. See [Set up fine-grained access control for Memory](#memory-gateway-fgac-setup).

## Relationship to other access-control options
<a name="memory-gateway-fgac-relationship"></a>
