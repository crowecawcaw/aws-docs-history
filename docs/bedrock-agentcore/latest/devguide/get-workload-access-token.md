...
**Tradeoff:** The platform treats the userId as an opaque string and cannot verify it against an authenticated end-user identity. The security binding relies on the calling workload passing the correct userId and on IAM policies being scoped appropriately. See [Security controls for GetWorkloadAccessTokenForUserId API](#security-controls-getuserid-api "#security-controls-getuserid-api") for recommended controls.
