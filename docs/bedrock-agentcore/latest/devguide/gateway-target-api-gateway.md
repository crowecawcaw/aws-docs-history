...
An API Gateway REST API target connects your gateway to a [stage](../../../apigateway/latest/developerguide/set-up-stages.md "../../../apigateway/latest/developerguide/set-up-stages.md") of your REST API. The gateway translates incoming MCP requests into HTTP requests to your REST API and handles response formatting. When you add or update an API Gateway target, AgentCore Gateway calls API Gateway’s [GetExport](../../../apigateway/latest/api/API_GetExport.md "../../../apigateway/latest/api/API_GetExport.md") API on your behalf.
...
You can also configure your API Gateway REST API stage as a gateway target from the API Gateway console. To learn more, see [Add a stage to an AgentCore gateway](../../../apigateway/latest/developerguide/mcp-server.md "../../../apigateway/latest/developerguide/mcp-server.md") in the Amazon API Gateway documentation.
...
- Your API must be configured with a public [endpoint type](../../../apigateway/latest/developerguide/api-gateway-api-endpoint-types.htm.md "../../../apigateway/latest/developerguide/api-gateway-api-endpoint-types.htm.md") . Private endpoints are not supported. To create a Gateway Target that can access resources in your VPC, you should use a public endpoint and an [API Gateway private integration](../../../apigateway/latest/developerguide/set-up-private-integration.md "../../../apigateway/latest/developerguide/set-up-private-integration.md").
- If your REST API has a method that uses `AWS_IAM` authorization and requires an [API key](../../../apigateway/latest/developerguide/api-gateway-api-usage-plans.md "../../../apigateway/latest/developerguide/api-gateway-api-usage-plans.md") , AgentCore Gateway won’t support this method. It will be excluded from processing.
- If your API uses a proxy resources, such as `/pets/{proxy+}` , AgentCore Gateway won’t support this method.
- To set up your API Gateway Target, AgentCore Gateway calls API Gateway’s [GetExport](../../../apigateway/latest/api/API_GetExport.md "../../../apigateway/latest/api/API_GetExport.md") API on your behalf to get an OpenAPI 3.0 formatted export of your REST API Definition. For more details on this and how it might affect your Target configuration, see [API Gateway Export.](#gateway-target-api-gateway-export "#gateway-target-api-gateway-export")
...
- The exported API is subject to the same [considerations and limitations](gateway-schema-openapi.md#gateway-schema-openapi-considerations "gateway-schema-openapi.md#gateway-schema-openapi-considerations") as the OpenAPI target type.
...
Export the OpenAPI definition from your deployed API stage by calling [GetExport](../../../apigateway/latest/api/API_GetExport.md "../../../apigateway/latest/api/API_GetExport.md") , updating the operations that are missing `operationId` , and reimporting your API.
...
You can configure your API Gateway [Method](../../../apigateway/latest/api/API_Method.md "../../../apigateway/latest/api/API_Method.md") to add an `operationName` using the [UpdateMethod](../../../apigateway/latest/api/API_UpdateMethod.md "../../../apigateway/latest/api/API_UpdateMethod.md") command. When your API is exported, the `operationName` turns into the `operationId`.
...
- **API key** – call your API with an API key managed by AgentCore Gateway. This is not the same as [API keys](../../../apigateway/latest/developerguide/api-gateway-api-usage-plans.md "../../../apigateway/latest/developerguide/api-gateway-api-usage-plans.md") in API Gateway.
...
API Gateway allows you to secure your REST API with IAM. When IAM authorization is enabled, clients must use [Signature Version 4 (SigV4 or SigV4a)](../../../IAM/latest/UserGuide/reference_sigv.md "../../../IAM/latest/UserGuide/reference_sigv.md") to sign their requests with AWS credentials.
...
1. Create an API Key in API Gateway according to [Set up API keys for REST APIs in API Gateway](../../../apigateway/latest/developerguide/api-gateway-setup-api-keys.md "../../../apigateway/latest/developerguide/api-gateway-setup-api-keys.md").
2. Follow the steps to [Set up outbound authorization with an API key](gateway-outbound-auth.md "gateway-outbound-auth.md") , providing the API key that you created through API Gateway.
