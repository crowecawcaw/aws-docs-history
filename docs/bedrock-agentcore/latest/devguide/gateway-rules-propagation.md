...
4. If the agent invokes a gateway with MCP targets, the gateway reads the bundle from the OTel baggage and applies tool description overrides on `tools/list` responses. For details, see [Behavior on MCP targets](#gateway-rules-propagation-mcp "#gateway-rules-propagation-mcp").
