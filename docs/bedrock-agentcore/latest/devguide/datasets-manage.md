...
**Required parameters:**
`datasetName` (alphanumeric and underscores only, `^[a-zA-Z][a-zA-Z0-9_]{0,47}$`), `schemaType`, and `source` (inline examples or S3 URI).

**Optional parameters:**
`description`, `kmsKeyArn` (customer managed encryption key, immutable after creation — see [Dataset encryption](datasets-encryption.md "datasets-encryption.md")), `tags`.
