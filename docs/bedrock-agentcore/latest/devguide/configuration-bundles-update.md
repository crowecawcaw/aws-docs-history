...
| `parentVersionIds` | List of strings | No       | Parent version IDs for lineage tracking. \*_Required when updating `components`._<br>• Regular commits: single parent. Merge commits: two parents (target branch latest, source branch latest). If the branch already exists, the first parent must be the latest version on that branch. |
