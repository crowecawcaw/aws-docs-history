...
**Modification lock protection:** Updates are not allowed
if the evaluator has been used by any enabled evaluation configuration.
...
Modify your custom evaluator settings using the console's editing interface, which
provides form validation and guided configuration options.
...
5. Choose one of the following methods to update the custom evaluator:
   - Choose the custom evaluator name to view its details, then choose
     **Edit** in the upper right of the details
     page.
   - Select the custom evaluator so that it is highlighted, then choose
     **Edit** at the top of the Custom evaluators
     card.

###### Note

If the evaluator is in use in any online evaluation, it cannot be
updated. Instead, you can duplicate the evaluator and update the cloned
version. 6. Update the fields as needed. 7. Choose **Update evaluator** to save the changes.
