...
Configuration bundle encryption uses envelope encryption with caller credentials. When you create or update a bundle with a `kmsKeyArn`, the service uses your credentials (via [Forward Access Sessions](../../../kms/latest/developerguide/concepts.md#fas "../../../kms/latest/developerguide/concepts.md#fas")) to generate a data encryption key (DEK) from KMS. The service encrypts the component configurations locally using AES-GCM with that DEK, then stores the encrypted DEK alongside the bundle. When you retrieve the bundle, the service decrypts the DEK using your credentials and decrypts the components.
...
To encrypt a configuration bundle, specify the `kmsKeyArn` parameter when calling [CreateConfigurationBundle](../../../bedrock-agentcore-control/latest/APIReference/API_CreateConfigurationBundle.md "../../../bedrock-agentcore-control/latest/APIReference/API_CreateConfigurationBundle.md").
...
You can change the encryption configuration on an existing bundle using [UpdateConfigurationBundle](../../../bedrock-agentcore-control/latest/APIReference/API_UpdateConfigurationBundle.md "../../../bedrock-agentcore-control/latest/APIReference/API_UpdateConfigurationBundle.md"):
