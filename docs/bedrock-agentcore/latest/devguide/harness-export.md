...
```
agentcore export harness --name <name> [options]
```
...
| ---------------------------- | ------------------------------------ | -------------------- |
| `--name <name>`              | Name of the harness to export        | Required             |
...
```
# Basic export - generates MyHarnessAgent using CodeZip
...
![Select the harness to export](images/tui/harness-export-01-select.png) 2. Enter a name for the generated runtime agent, or accept the default `<harness>Agent` .

![Name the generated runtime agent](images/tui/harness-export-02-name.png) 3. Choose the build type: **CodeZip** (package Python source as a zip, the default) or **Container** (build a Docker image via ECR and CodeBuild).

![Select the build type](images/tui/harness-export-03-build.png) 4. Review the export configuration and confirm.

![Review and confirm the export](images/tui/harness-export-04-confirm.png)
