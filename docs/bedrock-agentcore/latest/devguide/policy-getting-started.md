...
| Deploy fails with policy validation error | The default validation mode `FAIL_ON_ANY_FINDINGS` rejects policies with any validation findings. For testing, you can set the validation mode to `IGNORE_ALL_FINDINGS` . For production, fix the Cedar policy to pass validation. |
