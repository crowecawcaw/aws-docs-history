...
By default, Instances relies on the [Amazon EBS encryption](../../../ebs/latest/userguide/ebs-encryption.md "../../../ebs/latest/userguide/ebs-encryption.md") setting of your AWS account. If you enable EBS encryption by default in a Region, the volumes that your capacity providers create in that Region are encrypted with the default key you configured, with no additional configuration on the capacity provider.
...
- **The capacity provider operator role** must be able to use the key to create encrypted EBS volumes. For the KMS permissions that EBS requires to create and attach encrypted volumes, see [Requirements for Amazon EBS encryption](../../../ebs/latest/userguide/ebs-encryption-requirements.md "../../../ebs/latest/userguide/ebs-encryption-requirements.md") in the Amazon EBS User Guide.
...
AWS CloudTrail logs the KMS API calls made against your customer managed key when EBS creates and attaches encrypted volumes for your sessions. For more information, see [Logging AWS KMS API calls with AWS CloudTrail](../../../kms/latest/developerguide/logging-using-cloudtrail.md#searching-kms-ct "../../../kms/latest/developerguide/logging-using-cloudtrail.md#searching-kms-ct") in the AWS Key Management Service Developer Guide.
