...
1.  AWS WAF evaluates the request against the rules in the associated web ACL.

1. If the request is allowed, the gateway routes it to the appropriate target.

1. If the request is blocked, the gateway returns an error to the client without forwarding the request.
