...
The AWS [shared
responsibility model](https://aws.amazon.com/compliance/shared-responsibility-model/ "https://aws.amazon.com/compliance/shared-responsibility-model/") applies to data protection in Amazon Bedrock AgentCore Identity. As described
...
set up individual user accounts with IAM. That way each user is given only the permissions
...
- Use advanced managed security services such as Amazon Macie, which assists in
  ...
  customers' account numbers, into free-form fields such as a **Name** field. This includes when you work with Amazon Bedrock AgentCore Identity or other AWS
  services using the console, API, AWS CLI, or AWS SDKs. Any data that you enter into
  Amazon Bedrock AgentCore Identity or other services might get picked up for inclusion in diagnostic logs. When
  you provide a URL to an external server, don't include credentials information in the URL to
  validate your request to that server.

###### Topics

- [Data encryption](identity-data-encryption.md "identity-data-encryption.md")
- [Set customer managed key policy](kms-key-policy-configuration.md "kms-key-policy-configuration.md")
- [Configure with API operations or an AWS SDK](api-configuration-encryption.md "api-configuration-encryption.md")
