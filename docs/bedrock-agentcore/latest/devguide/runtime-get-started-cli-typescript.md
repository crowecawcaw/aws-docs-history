...
This tutorial shows you how to use the AgentCore CLI ([agentcore-cli](https://github.com/aws/agentcore-cli "https://github.com/aws/agentcore-cli"))
to deploy a TypeScript agent to an Amazon Bedrock AgentCore Runtime.

The AgentCore CLI is a command-line tool that you can use to create, develop,
and deploy AI agents to an Amazon Bedrock AgentCore Runtime. This tutorial shows you how to create
and deploy a TypeScript agent using the [bedrock-agentcore](https://www.npmjs.com/package/bedrock-agentcore "https://www.npmjs.com/package/bedrock-agentcore") SDK.
...
- **Docker** (or [Podman](https://podman.io "https://podman.io") or [Finch](https://runfinch.com "https://runfinch.com")) installed. A container runtime is
  required for building and testing your TypeScript agent locally. A local
  container runtime is not required for `agentcore deploy` because
  AWS CodeBuild builds the image remotely.
...
To deploy a TypeScript agent, use container-based deployment. Create a project without
a default agent. You will add a TypeScript container agent in the next step.
...
interactive wizard. Select "No" when asked to add an agent — you will
add a TypeScript container agent in the next step:
...
This tutorial uses the [Strands
Agents](https://strandsagents.com/latest/ "https://strandsagents.com/latest/") framework, which is currently the supported framework for TypeScript
agents deployed to AgentCore Runtime.

Add a TypeScript agent to your project using the bring-your-own-code (BYO) workflow
...
Interactive
Run `agentcore add agent` to launch the interactive wizard.
Select **Bring my own code** as the agent
type:
...
1. Select **Bring my own code** as
   the agent type:

![Add agent wizard: select BYO type](images/tui/runtime-ts-add-agent-byo.png) 2. Set the code location for your agent:

![Add agent wizard: code location](images/tui/runtime-ts-add-agent-code.png) 3. Review the configuration and confirm:

![Add agent wizard: review and confirm](images/tui/runtime-ts-add-agent-confirm.png)

This command registers a TypeScript agent in
`agentcore/agentcore.json` and creates a code directory for your
agent at `app/TsAgent/`.
...
The `--legacy-peer-deps` flag resolves peer dependency conflicts between
`@strands-agents/sdk` and `@opentelemetry/auto-instrumentations-node`.
This is safe and does not affect runtime behavior.
...
Create a `Dockerfile` in the `app/TsAgent/`
directory. Container-based deployment uses a multi-stage Docker build: the builder stage
compiles TypeScript to JavaScript, and the production stage runs only the compiled
output. The image runs as a non-root user for security, and exposes port 8080 (HTTP),
port 8000 (MCP), and port 9000 (A2A). OpenTelemetry instrumentation is included
...
Create a `.dockerignore` file to exclude unnecessary files from
...
Build locally to verify that your TypeScript code compiles. The container build will
also run this step during deployment.
...
The `agentcore create` and `agentcore add agent` commands
generated the configuration files for your project. Open
`agentcore/agentcore.json` to review the configuration:
...
For container-based deployments, the `runtimeVersion` and
`entrypoint` fields are present in the configuration but are not used at
deployment time. The container image (via its `Dockerfile CMD`) defines
the actual runtime and entrypoint. The CLI sets these to Python defaults regardless
of language.

You can also review your deployment target in
...
Before deploying, you must configure at least one deployment target. Open
`agentcore/aws-targets.json` and add your AWS account and
Region:
...
Replace `123456789012` with your AWS account ID and update
...
If your project has multiple runtimes, use `--runtime` to start
a specific runtime directly:
...
- For container agents, this command builds the Docker image and runs the
  container locally with your source directory mounted for hot reload. Your
  AWS credentials are forwarded automatically to the container.
- The server runs on `http://localhost:8080` by default. Use the
  `--port` flag to specify a different port.

###### Note

If your project has multiple runtimes, `agentcore dev` opens an
interactive selection menu. Use `agentcore dev --runtime TsAgent` to
skip the menu and start a specific runtime directly.
...
After deploying your agent, you can stream logs and view traces using the
AgentCore CLI:
...
You can preview the deployment before confirming by using the `--plan`
flag:
...
the status option to view your deployed resources dashboard:
...
you noted in Step 6. You can also find the ARN by running
`agentcore status`.
...
If you no longer want to host the agent in the AgentCore Runtime, remove all resources from
your project configuration and then deploy to tear down the AWS resources:
...
Common issues and solutions when getting started with the AgentCore CLI
for TypeScript. For more troubleshooting information, see [Troubleshoot Amazon Bedrock AgentCore Runtime](runtime-troubleshooting.md "runtime-troubleshooting.md").

**Container runtime not found**

Install Docker, Podman, or Finch. For Docker, ensure Docker Desktop is
running. Verify your container runtime is available by running
`docker info` (or the equivalent command for your
container runtime).

**Dockerfile not found**

Ensure a `Dockerfile` exists in your agent's code
directory (for example, `app/TsAgent/Dockerfile`). The
container build requires a Dockerfile in the directory specified by
`codeLocation` in `agentcore/agentcore.json`.

**Container image exceeds size limit**

Use multi-stage builds to reduce the final image size, minimize installed
packages, and review your `.dockerignore` file to exclude
