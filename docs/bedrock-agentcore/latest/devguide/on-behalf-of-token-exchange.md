...
To configure on-behalf-of token exchange for custom provider on AWS Console, see [Add OAuth client using custom provider](identity-add-oauth-client-custom.md "identity-add-oauth-client-custom.md"). You can also configure it using CLI
...
| --------------- | ----------------------- | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MicrosoftOauth2 | JWT_AUTHORIZATION_GRANT | `requested_token_use=on_behalf_of` is automatically added to the request unless overridden. | [Microsoft identity platform and OAuth 2.0 On-Behalf-Of flow](https://learn.microsoft.com/en-us/entra/identity-platform/v2-oauth2-on-behalf-of-flow "https://learn.microsoft.com/en-us/entra/identity-platform/v2-oauth2-on-behalf-of-flow") |

## Using on-behalf-of token exchange

To request an on-behalf-of token at runtime, call [GetResourceOauth2Token](../APIReference/API_GetResourceOauth2Token.md "../APIReference/API_GetResourceOauth2Token.md") and set oauth2Flow to the `ON_BEHALF_OF_TOKEN_EXCHANGE`. Pass the workload access token you obtained from [GetWorkloadAccessTokenForJWT](../APIReference/API_GetWorkloadAccessTokenForJWT.md "../APIReference/API_GetWorkloadAccessTokenForJWT.md") — this token carries the inbound token as the subject, which AgentCore Identity uses as the subject token in the exchange. For the full API reference, see the AWS documentation.
...
| `subject_token` | We use the inbound JWT token and map the subject token type to `urn:ietf:params:oauth:token-type:jwt` when constructing the token exchange request.<br>Default and can be overridden, see [customParameters](../APIReference/API_GetResourceOauth2Token.md#API_GetResourceOauth2Token_RequestSyntax "../APIReference/API_GetResourceOauth2Token.md#API_GetResourceOauth2Token_RequestSyntax") |
...
AgentCore Identity first performs [machine-to-machine authentication (OAuth 2.0 client credentials grant)](common-use-cases.md#machine-to-machine-auth "common-use-cases.md#machine-to-machine-auth") against the credential provider to obtain an access token, then sends it as the `actor_token`.

| Parameter             | Value                                                                                                                                                                       |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `actor_token_content` | `M2M`<br>• we map it to `urn:ietf:params:oauth:token-type:access_token` when constructing token exchange request<br>• we obtain access token via `client_credentials` grant |
...
AgentCore Identity calls [`sts:GetWebIdentityToken`](../../../STS/latest/APIReference/API_GetWebIdentityToken.md "../../../STS/latest/APIReference/API_GetWebIdentityToken.md") using the credential provider’s token endpoint as the aud claim, then sends the resulting JWT as the `actor_token`.

| Parameter             | Value                                                                                                                                                                                                                                                                                                  |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `actor_token_content` | `AWS_IAM_ID_TOKEN_JWT`<br>• we map it to `urn:ietf:params:oauth:token-type:jwt` when constructing token exchange request<br>• we obtain JWT via [`sts:GetWebIdentityToken`](../../../STS/latest/APIReference/API_GetWebIdentityToken.md "../../../STS/latest/APIReference/API_GetWebIdentityToken.md") |

This mode requires your account to be enabled for outbound web identity federation. See [`iam:EnableOutboundWebIdentityFederation`](../../../IAM/latest/APIReference/API_EnableOutboundWebIdentityFederation.md "../../../IAM/latest/APIReference/API_EnableOutboundWebIdentityFederation.md").
