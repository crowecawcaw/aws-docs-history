...
**Create a policy in `LOG_ONLY` mode**
Create a policy in `LOG_ONLY` mode by setting `enforcementMode` to `LOG_ONLY` in the `CreatePolicy` request. The following example creates a guardrail in policy that forbids violent content above a confidence threshold, but only observes it. For more information about guardrails in policy, see [guardrails in policies](policy-guardrails-in-policies.md "policy-guardrails-in-policies.md").
...
For more information about viewing metrics for your AgentCore resources, see [Bedrock AgentCore generated observability data](observability.md "observability.md").
