...
+ Depending on the outbound credential mode (see [Outbound credential mode](memory-gateway-connector.md#memory-gateway-connector-outbound)): for `GATEWAY_IAM_ROLE`, a gateway execution role that the gateway assumes to call Memory, with its identity policy scoped to only the Memory actions the gateway needs; for `CALLER_IAM_CREDENTIALS`, the caller’s own IAM identity must be permitted to perform the Memory actions, because the gateway forwards that identity to Memory.
...
1. Create a gateway with [CreateGateway](https://docs.aws.amazon.com/bedrock-agentcore-control/latest/APIReference/API_CreateGateway.html) and choose its inbound authorizer type (`authorizerType`). The authorizer determines how callers authenticate to the gateway and which identity information is available to access-control policies. For more information, see [Inbound and outbound authentication modes](memory-gateway-connector.md#memory-gateway-connector-auth-modes).
   ...
   The response includes the `gatewayId`, `gatewayArn`, and the invocation `gatewayUrl`. Creation is asynchronous; use [GetGateway](https://docs.aws.amazon.com/bedrock-agentcore-control/latest/APIReference/API_GetGateway.html) to wait until the gateway’s `status` is `READY`.
**Note**  
...
1. Add a target to the gateway with [CreateGatewayTarget](https://docs.aws.amazon.com/bedrock-agentcore-control/latest/APIReference/API_CreateGatewayTarget.html), using the `agentcore-memory` connector as the target configuration. For `--gateway-identifier`, use the `gatewayId` returned by `CreateGateway` in the previous step. Supply the connector id and the `memoryId` of the Memory resource to front. The target’s `credentialProviderConfigurations` set the outbound credential mode; with OAuth inbound this is `GATEWAY_IAM_ROLE`.
   ...
   Target creation is asynchronous; use [GetGatewayTarget](https://docs.aws.amazon.com/bedrock-agentcore-control/latest/APIReference/API_GetGatewayTarget.html) to wait until the target’s `status` is `READY`. The target name (`my-memory` here) becomes the prefix of every Cedar action id for this target.

1. Attach a policy engine and add fine-grained access control policies. This step is what enforces per-caller isolation. For the full procedure and Memory-specific policy examples, see [Fine-grained access control for Memory](memory-gateway-fgac.md).
