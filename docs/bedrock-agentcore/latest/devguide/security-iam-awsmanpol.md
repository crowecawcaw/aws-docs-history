...
- `kms` (AWS Key Management Service) –
  ...
  when called via the Bedrock Agent Core service.
- `s3` (Amazon Simple Storage Service) –
  Allows principals to get objects from S3 buckets with names that begin with
  "bedrock-agentcore-gateway-" when called via the Bedrock Agent Core service.
- `lambda` (AWS Lambda) –
  Allows principals to list Lambda functions.
- `logs` (Amazon CloudWatch Logs) – Allows principals
  to access, query, and manage log data in log groups related to Bedrock Agent Core
  and Application Signals, including creating log groups and streams.
