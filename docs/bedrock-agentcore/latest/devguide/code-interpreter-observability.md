...
Code interpreter execution console logs are not available in CloudWatch, but
stdout/stderr details are directly available in each invocation.

Metrics

You can view the following metrics in Amazon CloudWatch:

- Session counts: The number of sessions that Code Interpreter session has been
  requested
...
- Throttles: The number of times requests to the tool have been throttled due to
  exceeding limits, with percentage change compared to the previous week
- User errors/system errors: The number of times requests failed due to either
