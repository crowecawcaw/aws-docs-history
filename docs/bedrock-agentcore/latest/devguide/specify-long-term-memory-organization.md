...
When you [create](memory-create-a-memory-store.md "memory-create-a-memory-store.md") an AgentCore Memory, use a namespace to specify where the [long-term memories](memory-types.md#memory-long-term-memory "memory-types.md#memory-long-term-memory") for a [memory strategy](memory-strategies.md "memory-strategies.md") are logically grouped. Every time a new long-term memory is extracted using the memory strategy, it is saved under the namespace you set. This means that all long-term memories are scoped to their specific namespace, keeping them organized and preventing any mix-ups with other users or sessions. You should use a hierarchical format separated by forward slashes `/` . This helps keep memories organized clearly. As needed, you can choose to use the following pre-defined variables within braces in the namespace based on your application’s organization needs:
...
For even more granular organization, you can add [custom namespace variables](#specify-custom-namespace-variables "#specify-custom-namespace-variables") to represent additional dimensions such as tenant, team, or environment.
...
You define the custom variables when creating or updating an existing memory resource using the `namespaceKeys` parameter, and supply their values at runtime through the `extractionConfig.namespaceVariables` field in the [CreateEvent](../APIReference/API_CreateEvent.md "../APIReference/API_CreateEvent.md") operation.
...
When you create an event using the [CreateEvent](../APIReference/API_CreateEvent.md "../APIReference/API_CreateEvent.md") operation, pass the custom namespace variable values in the `extractionConfig.namespaceVariables` field:
...
When you update namespace keys with the [UpdateMemory](../../../bedrock-agentcore-control/latest/APIReference/API_UpdateMemory.md "../../../bedrock-agentcore-control/latest/APIReference/API_UpdateMemory.md") operation, the `namespaceKeys` value fully replaces the existing set. To safely update, retrieve the current keys with `GetMemory`, apply your changes, and submit the complete list.
