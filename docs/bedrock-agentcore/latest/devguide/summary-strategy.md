...
The `SummaryStrategy` is responsible for generating condensed, real-time
...
These summary chunks can be retrieved using the [ListMemoryRecords](../APIReference/API_ListMemoryRecords.md "../APIReference/API_ListMemoryRecords.md") operation with namespace filter, or you can also perform
...
- The outcome of a planning session, like "The team agreed to move the project
  deadline to Friday."
  By referencing this summary, an agent can quickly recall the context of a long or complex
  conversation without needing to re-process the entire history. This is essential for
...
`sessionId` is a required parameter for summary namespace since summaries
are generated and maintained at session level.
