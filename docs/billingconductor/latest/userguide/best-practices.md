...
To learn more about giving users the ability to create billing groups and pricing plans in the AWS Billing Conductor console, see [Identity and access management for AWS Billing Conductor](security-iam.md).
...
For more information about two-level transfers, see [Billing transfer quotas](https://docs.aws.amazon.com/awsaccountbilling/latest/aboutv2/orgs_transfer_billing-quotas.html).


**Account roles in billing transfer configurations**  

| Account role | One-level transfer | Two-level transfer | 
| --- | --- | --- | 
...
`Conglomerate_A` (bill transfer receiver, `accountID 789`) charges `Subsidiary_1` (bill transfer `accountID 456`) $14 for the usage of Business Unit\_1.1 (Bill source `accountID 123`) and $29 for the usage of the Subsidiary 1 (Bill Transfer `accountID 456`) to reward them in using the recommended AWS services.
