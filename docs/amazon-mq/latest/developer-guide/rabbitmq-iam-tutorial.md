...
The following procedure demonstrates how to enable AWS IAM authentication and authorization for an Amazon MQ for RabbitMQ broker. After enabling IAM, users can authenticate using AWS IAM credentials to access the RabbitMQ Management API and connect via AMQP. For details on how IAM authentication works with Amazon MQ for RabbitMQ, see [IAM authentication and authorization for Amazon MQ for RabbitMQ](iam-for-amq-for-rabbitmq.md "iam-for-amq-for-rabbitmq.md").
...
The output displays a unique issuer identifier URL for your account in the format `https://<id>.tokens.sts.global.api.aws`. 3. **Create the IAM policy document**
...
Wait for the broker status to return to `RUNNING` before proceeding to the next step. 8. **Obtain a JWT token**
...
A successful response confirms that IAM authentication is working correctly. The response contains broker overview information in JSON format. 10. **Connect via AMQP using the JWT token**
