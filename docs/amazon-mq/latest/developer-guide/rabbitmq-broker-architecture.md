...
**Note**  
During a *maintenance window*, all maintenance to a cluster is performed one node at a time, keeping at least two running nodes at all times. Each time a node is brought down, client connections to that node are severed and need to be re-established. You must ensure that your client code is designed to automatically reconnect to your cluster. For more information about connection recovery, see [Step 1: Automatically recover from network failures](best-practices-network-resilience.md#automatically-recover-from-network-failures).  
