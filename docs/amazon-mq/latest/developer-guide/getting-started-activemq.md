...
1. On the **Select deployment and storage** page, in the **Deployment mode and storage type** section, do the following:
   ...
   1. Choose the **Deployment mode** (for example, **Active/standby broker**). For more information, see [Deployment options for Amazon MQ for ActiveMQ brokers](amazon-mq-broker-architecture.md).
      + A **Single-instance broker** is comprised of one broker in one Availability Zone. The broker communicates with your application and with an Amazon EBS or Amazon EFS storage volume. For more information, see [Option 1: Amazon MQ single-instance brokers](amazon-mq-broker-architecture.md#single-broker-deployment).
      + An **Active/standby broker for high availability** is comprised of two brokers in two different Availability Zones, configured in a *redundant pair*. These brokers communicate synchronously with your application, and with Amazon EFS. For more information, see [Option 2: Amazon MQ active/standby brokers for high availability](amazon-mq-broker-architecture.md#active-standby-broker-deployment).

   1. Choose the **Storage type** (for example, **EBS**). For more information, see [Amazon MQ for ActiveMQ storage types](broker-storage.md).
**Note**  
...
**Note**  
 ...
 [Configurations](amazon-mq-broker-configuration-parameters.md) 
 [CloudWatch logs](security-logging-monitoring.md) 
 Private access 
 [Broker maintenance window](maintaining-brokers.md) 

   1. Choose the **Broker instance type** (for example, **mq.m5.large**). For more information, see [Amazon MQ for ActiveMQ broker instance types](broker-instance-types.md).
...
1. Choose **{{MyBroker}}**.
   ...
   On the **{{MyBroker}}** page, in the **Connect** section, note your broker's **[ActiveMQ web console](https://activemq.apache.org/web-console.html)** URL, for example:

   ```
   https://b-1234a5b6-78cd-901e-2fgh-3i45j6k178l9-1.mq.us-east-2.amazonaws.com:8162
   ```

   Also, note your broker's [wire-level protocol **Endpoints**](https://activemq.apache.org/configuring-transports.html). The following is an example of an OpenWire endpoint:
