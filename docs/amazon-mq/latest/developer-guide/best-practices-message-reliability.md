...
**Note**  
 Unacknowledged messages must be cached in memory. You can limit the number of messages that a consumer pre-fetches by configuring [pre-fetch](best-practices-performance.md#configure-prefetching) settings for a client application. 
