...
The following example shows how to create a Redshift client using create_client()
from Redshift connection.
...
Some connections are directly associated with an AWS service, and will default to using that
AWS service's client if no service name is specified. Those connections are listed in
the following table.
