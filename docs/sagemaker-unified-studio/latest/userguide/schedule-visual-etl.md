...
- You can schedule your visual jobs directly in the Visual ETL editor. This way you can
  schedule a single visual job quickly.
...
You can schedule your visual jobs to run from within the Visual ETL editor. To do this,
use a project with the **All capabilities** project profile or another
project profile with scheduling enabled in the Tooling blueprint parameters. If you have
created a project that needs to be updated to enable scheduling, contact your admin.
...
## Reviewing scheduled visual jobs in the

editor

You can review scheduled visual jobs in the Visual ETL interface in Amazon SageMaker Unified Studio. On the
schedules page, you can pause, edit, and delete schedules. You can also view the status and
other information for a schedule and choose the name of a schedule to view runs and
additional data.
...
name of a run to see a log and other details for that run.

## Scheduling visual jobs with
...
![The Amazon SageMaker Unified Studio UI showing the option to clone to Notebook .](images/vis-etl/vis-etl-download-notebook.png) 3. Navigate to Build → Workflows menu, click on the “Create workflow in editor”.

![The Amazon SageMaker Unified Studio UI showing the option to "Create workflow in editor" .](images/vis-etl/vis-etl-workflow-editor.png) 4. You will now see an example DAG template in JupyterLab.

![The Amazon SageMaker Unified Studio JupyterLab UI showing the DAG teamplate .](images/vis-etl/vis-etl-dag-template.png) 5. Modify the lines of python code as below, then save it as “mwaa_test_dag.py”. We
...
![The Amazon SageMaker Unified Studio JupyterLab UI showing the notebook path and workflow schedule variables modified. .](images/vis-etl/vis-etl-change-code.png) 6. Pull the file “dataflows/mwaa-test.vetl” from the project’s source code repository
to JupyterLab.

![The Amazon SageMaker Unified Studio UI showing the "VETL" file in the source code repo for JupyterLab .](images/vis-etl/vis-etl-source-repo.png)

![The Amazon SageMaker Unified Studio UI showing a successful pull from the source repo. .](images/vis-etl/vis-etl-pull-notebook.png) 7. Navigate back to the Workflows console, now we can see the DAG is created. We can
access Airflow UI via the “Actions” dropdown list.

![The Amazon SageMaker Unified Studio UI showing the option to "Open Airflow UI" in the Workflow section .](images/vis-etl/vis-etl-airflow-ui.png) 8. Manually trigger the DAG.

![The Airflow UI showing the option to Trigger DAG.](images/vis-etl/dag-start-blurred.png)
