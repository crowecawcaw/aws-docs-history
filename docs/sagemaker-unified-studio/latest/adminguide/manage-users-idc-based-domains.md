...
Configure single sign-on (SSO) access to your Amazon SageMaker unified domain through IAM
Identity Center or SAML federation. You can also manage root domain unit owners from this
page.
...
support. Once IAM Identity Center multi-Region is set up, follow the same steps in the following procedure to
...
4. On the **Choose user authentication method**, choose the **IAM
   ...
   Center can access the domain's Amazon SageMaker Unified Studio.
...
4. On the **Choose user authentication method** page, choose
   **SAML**. With SAML, users configured through external Identity Providers
   (IdPs) can access the domain's Amazon SageMaker Unified Studio. Choose **Next**.
...
1. Navigate to the Amazon SageMaker management console at [https://console.aws.amazon.com/datazone](https://console.aws.amazon.com/datazone "https://console.aws.amazon.com/datazone") and use the region selector in the top
   navigation bar to choose the appropriate AWS Region.
2. Either create a new or choose an existing Amazon SageMaker unified domain and then
