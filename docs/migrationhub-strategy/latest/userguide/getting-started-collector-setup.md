...
1. Enter **Y** for yes to the **Do you have any csharp repositories that should be analyzed on a windows machine?** question, if you want to analyze C\# code.
**Note**  
...
1. For the **Would you like to reuse existing windows credentials on this machine?** question. Enter **Y** for yes, if the Windows machine for C\# source code analysis uses the same credentials as the credentials previously provided as part of setting up `--remote-server-configurations` or `--vcenter-configurations`.
...
1. Copy the public key generated with the name **id\_rsa\_assessment.pub** from the following folder in the container:
...
1.  Copy the certificate generated in the previous step to the following location in the collector: 
   ...
   **/opt/amazon/application-data-collector/remote-auth/linux/keys**
   ...
   The name of the certificate must be **id\_rsa\_assessment-cert.pub**. 
