...
in Security Incident Response, that activity is recorded in a CloudTrail event along with other AWS service events
...
All Security Incident Response actions are logged by CloudTrail and are documented in the [AWS Security Incident Response API Reference](../APIReference.md "../APIReference.md"). For example,
calls to the
`CreateMembership`, `CreateCase` and `UpdateCase` actions generate
