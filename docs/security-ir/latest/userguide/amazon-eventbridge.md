...
![AWS services send events to the EventBridge default event bus. If the event matches a rule's event pattern, EventBridge sends the event to the targets specified for that rule.](images/Define_Rule.png) 6. Scroll to **AWS service,**. and then select **AWS Security Incident Response** from the drop down menu.

![AWS services send events to the EventBridge default event bus. If the event matches a rule's event pattern, EventBridge sends the event to the targets specified for that rule.](images/Event_Pattern_Security.png) 7. From the **Event Type** dropdown, select the event or API call you want to create a pattern for. 8. You can manually edit the pattern to include more than one event. 9. Choose **Next**.
