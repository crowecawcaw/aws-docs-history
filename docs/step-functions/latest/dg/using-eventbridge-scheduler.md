...
- [Set up the execution role](#using-eventbridge-scheduler-execution-role "#using-eventbridge-scheduler-execution-role")
- [Create a schedule](#using-eventbridge-scheduler-create "#using-eventbridge-scheduler-create")
- [Related resources](#using-eventbridge-scheduler-related-resources "#using-eventbridge-scheduler-related-resources")
...
If you want to create a schedule using one of the EventBridge Scheduler SDKs, the AWS CLI, or AWS CloudFormation, you must have an existing execution role that grants the permissions
...
7.  On the **Select target** page, choose the AWS API operation that EventBridge Scheduler invokes:
    ...
    Currently, you can't run Synchronous Express workflows on a schedule. 3. Enter a JSON payload for the execution. Even if your state machine doesn't require any JSON payload, you must still include input in JSON format as shown in the following example.
...
To confirm that EventBridge Scheduler invoked the state machine, check the [state machine's Amazon CloudWatch logs](cw-logs.md "cw-logs.md").
