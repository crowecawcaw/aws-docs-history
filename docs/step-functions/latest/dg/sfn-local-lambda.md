...
+ [Testing state machines with Step Functions Local (unsupported)](sfn-local.md)
...
For information about installing the Docker version of Step Functions, see [Setting Up Step Functions Local (Downloadable Version) in Docker](sfn-local.md#sfn-local-docker).
...
After Step Functions Local is running, create a state machine that references the `HelloWorldFunction` that you initialized in [Step 1: Set Up AWS SAM](#install-sam).
