...
1. Save the exported ASL definition within your project directory.
   ...
   To pass the exported ASL definition to the [aws\_sfn\_state\_machine](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/sfn_state_machine) Terraform resource, use the [jsonencode()](https://developer.hashicorp.com/terraform/language/functions/jsonencode) function in the `definition` field. This approach properly handles all Terraform types, including lists and maps, making it suitable for definitions that include dynamic values such as resource ARNs or security group IDs. Alternatively, you can use `templatefile` for static definitions that require only simple string substitutions. However, `templatefile` cannot handle complex types like lists.
**Tip**  
