...
- To create a basic state machine, use the [Creating state machine that uses a Lambda function](tutorial-creating-lambda-state-machine.md "tutorial-creating-lambda-state-machine.md") tutorial.
...
1. Navigate to the [Amazon S3 console](https://console.aws.amazon.com/s3/ "https://console.aws.amazon.com/s3/"), and then choose **Create bucket** to create the bucket in
...
this name unique. You need to create all resources in the same AWS Region. 3. Keep all the default selections on the page, and choose **Create bucket**.
...
for the rule. 3. For **Event bus** and **Rule type**, keep the default selections. 4. Choose **Next**. This opens the **Build event pattern** page. 5. Scroll down to the **Event pattern** section, and do the following:
    ...
    5. Choose **Specific bucket(s) by name** and enter the bucket name you created in
     [Step 1](#tutorial-cloudwatch-events-s3-bucket "#tutorial-cloudwatch-events-s3-bucket") (``username`-sfn-tutorial`) to store your
     files.
...
3. In the **State machine** list, select the state machine that you [created earlier](#tutorial-cloudwatch-events-s3-prereqs "#tutorial-cloudwatch-events-s3-prereqs") (for example, `Helloworld`).
...
Navigate to the [Amazon S3 console](https://console.aws.amazon.com/s3/ "https://console.aws.amazon.com/s3/"), choose the bucket you created to store files
...
AWS CloudTrail as the input. 3. Check the execution for your state machine.

Navigate to the [Step Functions console and
select the state machine used in your Amazon EventBridge rule
(`Helloworld`)](https://console.aws.amazon.com/states/ "https://console.aws.amazon.com/states/"). 4. Select the most recent execution of that state machine and expand the
