...
AWS Step Functions is designed to run workflows with a finite duration and number of steps. Standard workflow executions have a maximum duration of one year and 25,000 events (see [Step Functions service quotas](service-quotas.md "service-quotas.md")).
...
Step Functions can start workflow executions by calling its own API as an [integrated service](integrate-services.md "integrate-services.md"). We recommend that
...
[state machine that uses a Lambda function](tutorial-creating-lambda-state-machine.md "tutorial-creating-lambda-state-machine.md").
...
You can also view the ASL definition in the [Code editor](workflow-studio.md#wfs-interface-code-editor "workflow-studio.md#wfs-interface-code-editor") of Workflow Studio. In the code editor, you can also edit the ASL definition of your workflow. 7. Specify a name for your state machine. To do this, choose the edit icon next to the default state machine name of **MyStateMachine**. Then, in **State machine configuration**, specify a name in the **State machine name** box.

For this tutorial, enter the name `ParentStateMachine`. 8. (Optional) In **State machine configuration**, specify other workflow settings, such as state machine type and its execution role.

For this tutorial, keep all the default selections in **State machine settings**.

If you've [previously created an IAM role](procedure-create-iam-role.md "procedure-create-iam-role.md") with the correct permissions for your state machine and want to use it, in **Permissions**, select **Choose an existing role**, and then select a role from the list. Or select **Enter a role ARN** and then provide an ARN for that IAM role. 9. In the **Confirm role creation** dialog box, choose **Confirm** to continue.
...
function](tutorial-creating-lambda-state-machine.md "tutorial-creating-lambda-state-machine.md"), you need to attach an inline policy to your state machine's IAM role. For more information, see
[Embedding Inline Policies](../../../IAM/latest/UserGuide/access_policies_manage-attach-detach.md#embed-inline-policy-console "../../../IAM/latest/UserGuide/access_policies_manage-attach-detach.md#embed-inline-policy-console") in the
_IAM User Guide_.
