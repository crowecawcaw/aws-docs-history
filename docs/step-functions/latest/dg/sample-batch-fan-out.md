...
This sample project demonstrates how to use Step Functions’s [Map workflow state](state-map.md) state to fan out AWS Batch jobs.

In this project, Step Functions uses a state machine to invoke a Lambda function to do simple pre-processing, then invokes multiple AWS Batch jobs in parallel using the [Map workflow state](state-map.md) state.
