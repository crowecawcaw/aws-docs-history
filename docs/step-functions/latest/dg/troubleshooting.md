...
functions](intrinsic-functions.md "intrinsic-functions.md"). For example, you could use something similar to the
following:
...
If [cross-account access](concepts-access-cross-acct-resources.md "concepts-access-cross-acct-resources.md") of AWS resources is available in your Region, use the following method to invoke a Lambda function from another account.
...
For an example of a `Task` state definition that specifies a cross-account resource, see [Task state's Credentials field examples](state-task.md#task-state-example-credentials "state-task.md#task-state-example-credentials").
...
An activity task state doesn't start until you poll a task token by using the [GetActivityTask](../apireference/API_GetActivityTask.md "../apireference/API_GetActivityTask.md") API action. As a best practice, add a task level timeout in
order to avoid a stuck execution. For more information, see [Using timeouts to avoid stuck Step Functions workflow executions](sfn-best-practices.md#sfn-stuck-execution "sfn-best-practices.md#sfn-stuck-execution").

If your state machine is stuck in the [ActivityScheduled](../apireference/API_ActivityScheduledEventDetails.md "../apireference/API_ActivityScheduledEventDetails.md") event, it indicates that your activity worker fleet has issues or is under-scaled. You should monitor the [ActivityScheduleTime](procedure-cw-metrics.md#cloudwatch-step-functions-activity-metrics "procedure-cw-metrics.md#cloudwatch-step-functions-activity-metrics") CloudWatch metric and set an alarm when that time increases. However, to time out any stuck state machine executions in which the `Activity` state doesn't transition to the `ActivityStarted` state, define a timeout at state machine-level. To do this, specify a `TimeoutSeconds` field at the beginning of the state machine definition, outside of the `States` field.

### My activity worker times out while waiting for a task token.

Workers use the [GetActivityTask](../apireference/API_GetActivityTask.md "../apireference/API_GetActivityTask.md") API action to retrieve a task with the specified activity
...
timeout, configure a client side socket [with a timeout of at least 65](../apireference/API_GetActivityTask.md "../apireference/API_GetActivityTask.md")
...
button in the **Executions** tab. For more information, see [Viewing execution details in the Step Functions console](concepts-view-execution-details.md "concepts-view-execution-details.md").
