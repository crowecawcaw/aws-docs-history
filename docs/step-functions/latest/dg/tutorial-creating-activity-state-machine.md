...
- The [SDK for Java](https://aws.amazon.com/sdk-for-java/ "https://aws.amazon.com/sdk-for-java/"). The example activity in this tutorial is a Java application that uses
...
1. In the [Step Functions console](https://console.aws.amazon.com/states/home "https://console.aws.amazon.com/states/home"),
...
This is a description of your state machine using the [Amazon States Language](concepts-amazon-states-language.md "concepts-amazon-states-language.md") (ASL). It defines a
single `Task` state named `getGreeting`. For more
information, see [State Machine
Structure](statemachine-structure.md "statemachine-structure.md"). 7. On the [Graph visualization](workflow-studio.md#wfs-interface-code-graph-viz "workflow-studio.md#wfs-interface-code-graph-viz"), make sure the workflow graph for the ASL definition you added looks similar to the following graph.

![Graph visualization of state machine with RunActivity task state.](images/tutorial-create-state-machine-custom-preview.png) 8. Specify a name for your state machine. To do this, choose the edit icon next to the default state machine name of **MyStateMachine**. Then, in **State machine configuration**, specify a name in the **State machine name** box.

For this tutorial, enter the name `ActivityStateMachine`. 9. (Optional) In **State machine configuration**, specify other workflow settings, such as state machine type and its execution role.

For this tutorial, keep all the default selections in **State machine settings**.

If you've [previously created an IAM role](procedure-create-iam-role.md "procedure-create-iam-role.md") with the correct permissions for your state machine and want to use it, in **Permissions**, select **Choose an existing role**, and then select a role from the list. Or select **Enter a role ARN** and then provide an ARN for that IAM role. 10. In the **Confirm role creation** dialog box, choose **Confirm** to continue.
...
For a more complete example of an activity worker, see [Example: Activity Worker in Ruby](concepts-activities.md#example-ruby-activity-worker "concepts-activities.md#example-ruby-activity-worker"). This example provides an
...
example assumes that the `AWS_ACCESS_KEY_ID` (or
...
factory, see [AWSCredentialsProvider](../../../AWSJavaSDK/latest/javadoc/com/amazonaws/auth/AWSCredentialsProvider.md "../../../AWSJavaSDK/latest/javadoc/com/amazonaws/auth/AWSCredentialsProvider.md") in the
_AWS SDK for Java API Reference_ and [Set Up AWS Credentials and Region for Development](../../../sdk-for-java/v1/developer-guide/setup-credentials.md "../../../sdk-for-java/v1/developer-guide/setup-credentials.md") in the
_AWS SDK for Java Developer Guide_.
...
2. To use the AWS SDK, add the full path of the `lib` and
   ...
   [Downloading and Extracting the SDK](../../../sdk-for-java/v1/developer-guide/setup-install.md#download-and-extract-sdk "../../../sdk-for-java/v1/developer-guide/setup-install.md#download-and-extract-sdk") in the
   _AWS SDK for Java Developer Guide_.
3. Compile the file.
...
5. On the [Step Functions console](https://console.aws.amazon.com/states/home?region=us-east-1#/ "https://console.aws.amazon.com/states/home?region=us-east-1#/"), navigate to the _Execution
   Details_ page.
