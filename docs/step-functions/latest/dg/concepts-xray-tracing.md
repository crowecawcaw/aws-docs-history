...
**Important**  
Step Functions doesn't support X-Ray tracing for the child workflow executions started by a [*Distributed Map state*](state-map-distributed.md) because it's easy to exceed the [Trace document size limit](https://docs.aws.amazon.com/general/latest/gr/xray.html#limits_xray) for such executions.
...
**Note**  
When you enable X-Ray for an existing state machine, you must ensure that you have an IAM policy that grants sufficient permissions for X-Ray to perform traces. You can either add one manually, or generate one. For more information, see the IAM policy section for [IAM policies using AWS X-Ray in Step Functions](#xray-iam). 
...
The following steps illustrate what kind of information you can see in the console after you enable X-Ray and run an execution. X-Ray traces for the [Create a callback pattern example with Amazon SQS, Amazon SNS, and Lambda](callback-task-sample-sqs.md) sample project are shown. 
...
![Illustrative screenshot of X-Ray traces for a state machine.](https://docs.aws.amazon.com/step-functions/latest/dg/images/xray-tracing-overview.png)
...
![Illustrative screenshot focused on the service map in X-Ray traces.](https://docs.aws.amazon.com/step-functions/latest/dg/images/xray-tracing-servicemap.png)
...
![Example details for an X-Ray trace node.](https://docs.aws.amazon.com/step-functions/latest/dg/images/xray-tracing-servicemap-detail.png)
...
![Example graphical representation of the state machine being traced.](https://docs.aws.amazon.com/step-functions/latest/dg/images/sample-callback-example.png)
...
![Example screenshot of segments and subsegments for a state machine.](https://docs.aws.amazon.com/step-functions/latest/dg/images/xray-tracing-segments.png)
...
+ Choose **Encryption** to modify the encryption settings. 
  ...
  You can use the default setting, where X-Ray encrypts traces and data at rest, or, if needed, you can choose a KMS key. Standard [AWS KMS](https://docs.aws.amazon.com/kms/latest/developerguide/) charges apply in the latter case.
