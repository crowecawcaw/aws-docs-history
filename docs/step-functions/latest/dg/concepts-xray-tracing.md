...
You can use [AWS X-Ray](../../../xray/latest/devguide/aws-xray.md "../../../xray/latest/devguide/aws-xray.md") to visualize the components of your state machine, identify performance
...
You can use AWS X-Ray with Step Functions in regions where both are supported. See the [Step Functions](../../../general/latest/gr/step-functions.md "../../../general/latest/gr/step-functions.md") and [X-Ray](../../../general/latest/gr/xray.md "../../../general/latest/gr/xray.md") endpoints and quotas pages for information on region support for X-Ray and
Step Functions.
...
result in a frozen trace. See the service quotas section of the [X-Ray endpoints and quotas](../../../general/latest/gr/xray.md#limits_xray "../../../general/latest/gr/xray.md#limits_xray") page for more
information on other quotas for X-Ray.

###### Important

Step Functions doesn't support X-Ray tracing for the child workflow executions started by a [Distributed Map state](state-map-distributed.md "state-map-distributed.md") because it's easy to exceed the [Trace document size limit](../../../general/latest/gr/xray.md#limits_xray "../../../general/latest/gr/xray.md#limits_xray") for such executions.

###### Topics

- [Setup and configuration](#xray-concept-create "#xray-concept-create")
...
1. Open the [Step Functions console](https://console.aws.amazon.com/states/home "https://console.aws.amazon.com/states/home") and choose **Create state
   machine**.
...
1. In the [Step Functions console](https://console.aws.amazon.com/states/home "https://console.aws.amazon.com/states/home"), select the state machine for which
...
policy section for [IAM policies using AWS X-Ray in Step Functions](#xray-iam "#xray-iam"). 4. (Optional) Auto-generate a new role for your state machine to include X-Ray
permissions. 5. Choose **Save**.
...
of any additional requests. One request per second is the _reservoir_.
This ensures that at least one trace is recorded each second as long as the service is
serving requests. Five percent is the _rate_ at which additional requests
...
1. Go to the [X-Ray console](https://console.aws.amazon.com/xray/home "https://console.aws.amazon.com/xray/home").
2. Choose **Sampling**.
...
rules in the X-Ray console](../../../xray/latest/devguide/xray-console-sampling.md "../../../xray/latest/devguide/xray-console-sampling.md").
...
[`StartExecution`](../apireference/API_StartExecution.md "../apireference/API_StartExecution.md") or
[`StartSyncExecution`](../apireference/API_StartSyncExecution.md "../apireference/API_StartSyncExecution.md") API calls yourself.
...
`Root={TRACE_ID};Sampled={1 or 0}`. If you're using a Lambda function, replace the `TRACE_ID` with the trace ID in your current segment and set the Sampled
...
See [Viewing the X-Ray console](#xray-concept-tracing-details "#xray-concept-tracing-details") for information on how to access the X-Ray
console for your state machine executions.

For detailed information about the X-Ray console, see the [X-Ray console documentation](../../../xray/latest/devguide/xray-console.md "../../../xray/latest/devguide/xray-console.md").
...
console](../../../xray/latest/devguide/xray-console-analytics.md "../../../xray/latest/devguide/xray-console-analytics.md")
...
- [Amazon Simple Notification Service](../../../xray/latest/devguide/xray-services-sns.md "../../../xray/latest/devguide/xray-services-sns.md")
- [Amazon Simple Queue Service](../../../xray/latest/devguide/xray-services-sqs.md "../../../xray/latest/devguide/xray-services-sqs.md")
- [AWS Lambda](../../../xray/latest/devguide/xray-services-lambda.md "../../../xray/latest/devguide/xray-services-lambda.md")
- AWS Step Functions

### Instrumentation required

Service integrations that require [X-Ray
instrumentation](../../../xray/latest/devguide/aws-xray.md "../../../xray/latest/devguide/aws-xray.md"):
...
![Illustrative screenshot of X-Ray traces for a state machine.](images/xray-tracing-overview.png)
...
![Illustrative screenshot focused on the service map in X-Ray traces.](images/xray-tracing-servicemap.png)
...
![Example details for an X-Ray trace node.](images/xray-tracing-servicemap-detail.png)
...
![Example graphical representation of the state machine being traced.](images/sample-callback-example.png)
...
![Example screenshot of segments and subsegments for a state machine.](images/xray-tracing-segments.png)
...
- If you see **Data Not Found** in the X-Ray Traces panel, check your
  [IAM account
  settings](https://console.aws.amazon.com/iam/home?#/account_settings "https://console.aws.amazon.com/iam/home?#/account_settings") and ensure that AWS Security Token Service is enabled for the intended region. For more
  information, see [Activating and deactivating AWS STS in an AWS Region](../../../IAM/latest/UserGuide/id_credentials_temp_enable-regions.md#sts-regions-activate-deactivate "../../../IAM/latest/UserGuide/id_credentials_temp_enable-regions.md#sts-regions-activate-deactivate") in the _IAM User Guide_.
