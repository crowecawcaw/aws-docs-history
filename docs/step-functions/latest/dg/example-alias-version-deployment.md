...
The following example of the Canary deployment technique shows how you can deploy a new state machine version with the AWS Command Line Interface. In this example, the alias you create routes 20 percent of execution traffic to the new version. It then routes the remaining 80 percent the earlier version. To deploy a new state machine [version](concepts-state-machine-version.md) and shift execution traffic with an [alias](concepts-state-machine-alias.md), complete the following steps:

1. 

**Publish a version from the current state machine revision.**  
...
**Create an alias that points to the state machine version.**  
...
**Verify that executions started by the alias use correct published version.**  
Start a new execution of `{{myStateMachine}}` by providing the ARN of the alias **PROD** in the **start-execution** command:
   ...
   If you provide the state machine ARN in the [StartExecution](https://docs.aws.amazon.com/step-functions/latest/apireference/API_StartExecution.html) request, it uses the most recent [revision](concepts-cd-aliasing-versioning.md#statemachinerev) of the state machine instead of the version specified in your alias for starting the execution.

1. 

**Update the state machine definition and publish a new version.**  
...
**Update the alias to point to both the versions and set the alias' [routing configuration](concepts-state-machine-alias.md#alias-routing-config).**  
