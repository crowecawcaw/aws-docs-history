...
`States.Timeout`
[Error Name](concepts-error-handling.md#error-handling-error-representation "concepts-error-handling.md#error-handling-error-representation").
...
The `States` field contains [States](workflow-states.md "workflow-states.md").
...
If the current state is the last state in your workflow, or a terminal state, such as [Succeed workflow state](state-succeed.md "state-succeed.md") or [Fail workflow state](state-fail.md "state-fail.md"), you don't need to specify the `Next` field.

**`End`**

Designates this state as a terminal state (ends the execution) if set to `true`. There can be any number of terminal states per state machine. Only one of `Next` or `End` can be used in a state. Some state types, such as `Choice`, or terminal states, such as [Succeed workflow state](state-succeed.md "state-succeed.md") and [Fail workflow state](state-fail.md "state-fail.md"), don't support or use the `End` field.
...
A [path](concepts-input-output-filtering.md "concepts-input-output-filtering.md") that selects a portion of the state's input to be passed
to the state's task for processing. If omitted, it has the value `$` which designates
the entire input. For more information, see [Input and Output Processing](concepts-input-output-filtering.md "concepts-input-output-filtering.md").

**`OutputPath` (Optional, JSONPath only)**

A [path](concepts-input-output-filtering.md "concepts-input-output-filtering.md") that selects a portion of the state's output to be passed to the next state. If omitted, it has the value
`$` which designates the entire output. For more information, see [Input and Output Processing](concepts-input-output-filtering.md "concepts-input-output-filtering.md").
