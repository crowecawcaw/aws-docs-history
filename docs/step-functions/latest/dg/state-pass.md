...
You can also use a `Pass` state to transform JSON state input using filters, and then pass the transformed data to the next state in your workflows. For information about input transformation, see [Manipulate parameters in Step Functions workflows](input-output-inputpath-params.md "input-output-inputpath-params.md").

In addition to the [common state
fields](statemachine-structure.md#amazon-states-language-common-fields "statemachine-structure.md#amazon-states-language-common-fields"), `Pass` states allow the following fields.

**`Assign` (Optional, JSONata only)**

A collection of key-value pairs to assign data to variables. For
more information, see [Passing data between states with variables](workflow-variables.md "workflow-variables.md").
...
Specifies where to place the _output_ (relative to the input) of the virtual task specified in `Result`. The input is further filtered
...
more information, see [Manipulate parameters in Step Functions workflows](input-output-inputpath-params.md "input-output-inputpath-params.md").
