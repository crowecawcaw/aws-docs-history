...
This tutorial shows you how to create an API that uses one resource and the `POST` method to communicate with the `StartExecution` API action.
...
must call `DescribeExecution` to get the result.
...
The IAM role appears in the list of roles. 7. Choose the name of your role and note the **Role ARN**,
...
1. Open the [Amazon API Gateway
   console](https://console.aws.amazon.com/apigateway/ "https://console.aws.amazon.com/apigateway/"), and then choose **Create API**.
...
All Step Functions API actions use the HTTP `POST` method. 8. For **Action type**, select **Use action
name**. 9. For **Action name**, enter `StartExecution`. 10. For **Execution role**, enter [the role ARN of the
IAM role that you created earlier](#api-gateway-procedure-create-iam-role "#api-gateway-procedure-create-iam-role"), as shown in the following
example.
...
2. On the **/execution - POST - Method Test** tab, copy the
   ...
   section using the ARN of an existing state machine (or [create a new state machine that uses a Lambda function](tutorial-creating-lambda-state-machine.md "tutorial-creating-lambda-state-machine.md")), and then
   choose **Test**.
...
[Request Syntax](../apireference/API_StartExecution.md#API_StartExecution_RequestSyntax "../apireference/API_StartExecution.md#API_StartExecution_RequestSyntax") in the
...
`test`, and `prod`). For more information about specifying stage variables in a mapping template, see [`$stageVariables`](../../../apigateway/latest/developerguide/api-gateway-mapping-template-reference.md#stagevariables-template-reference "../../../apigateway/latest/developerguide/api-gateway-mapping-template-reference.md#stagevariables-template-reference") in the _API Gateway Developer Guide_. 3. The execution starts and the execution ARN and its epoch date are
...
[AWS Step Functions
console](https://console.aws.amazon.com/states/ "https://console.aws.amazon.com/states/").
