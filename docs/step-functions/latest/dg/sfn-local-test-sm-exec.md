...
Local. For information about configuring endpoints for Step Functions Local, see [Setting Configuration Options for Step Functions Local](sfn-local.md#sfn-local-config-options "sfn-local.md#sfn-local-config-options").
...
![Mocked service integration example.](images/msi-graph.png)

To do this, you must create a mock
configuration file containing sections as defined in [Mock configuration file structure](#mock-cfg-struct "#mock-cfg-struct").
...
Mock configuration file exampleThe following is an example of a mock configuration
file which
demonstrates how to mock responses from [invoking a Lambda
function](connect-lambda.md "connect-lambda.md") and [sending a message to Amazon SQS](connect-sqs.md "connect-sqs.md"). In
this example, the [LambdaSQSIntegration](#mock-cfg-sm-sect "#mock-cfg-sm-sect") state machine contains three test cases
...
State machine definitionThe following is an example of a state machine definition called `LambdaSQSIntegration`, which defines two service integration task states named `LambdaState` and `SQSState`. `LambdaState` contains a retry policy based on `States.ALL`.
...
see [Step 3: Run Mocked Service Integration Tests](#run-mocked-serv-integ-tests "#run-mocked-serv-integ-tests"). 2. Make sure
that
the mocked
responses'
...
requirements for mocked responses, see [Configuring mocked service integrations](#mock-resp-struct-req "#mock-resp-struct-req").
...
JAR FileUse one of the following ways to provide the mock configuration file to Step Functions
Local:
...
- In the session running Step Functions Local, set an environment variable named
  `SFN_MOCK_CONFIG`, to the full path of the mock configuration file.
  This
  ...
  In
  the following example, the `SFN_MOCK_CONFIG` variable is set to point at a
...
- If you place the mock configuration file in the same directory as
  ...
  and set
  the environment variable `SFN_MOCK_CONFIG`, Step Functions Local will read the file
...
1. Create a state machine based on the previously mentioned definition in the [mock configuration file](#create-mock-config-file "#create-mock-config-file").
...
To use the mock configuration file, make a `StartExecution` API call on a
...
`#`test_name``,
 ...
 ``test_name`` is a test case, which is configured for the state machine in the
...
`HappyPath` test defined in [Step 1: Specify Mocked Service Integrations in a Mock Configuration File](#create-mock-config-file "#create-mock-config-file"). The `HappyPath` test contains the
...
4. Check the
   execution's
   results by making a `ListExecutions`, `DescribeExecution`, or
   `GetExecutionHistory` API call.
...
`MockedLambdaSuccess` and `MockedSQSSuccess` in the [mock configuration file](#create-mock-config-file "#create-mock-config-file"). In addition, the
...
- [StateMachines](#mock-cfg-sm-sect "#mock-cfg-sm-sect")
- [MockedResponses](#mock-cfg-mckd-resp-sect "#mock-cfg-mckd-resp-sect")
...
The following is an example of a `Return` object that contains a mocked response for calling [`Invoke`](../../../lambda/latest/dg/API_Invoke.md "../../../lambda/latest/dg/API_Invoke.md") on a Lambda function:
...
objects. It specifies the [error output](concepts-error-handling.md "concepts-error-handling.md") of
...
The following is an example of a `Throw` object that contains a mocked response for calling [`Invoke`](../../../lambda/latest/dg/API_Invoke.md "../../../lambda/latest/dg/API_Invoke.md") on a Lambda function:
