...
You use the _Inline Map state_ in your workflows to repeatedly perform an action. For more
information about Inline mode, see [Map
state in Inline mode](state-map-inline.md "state-map-inline.md").

In this tutorial, you use the _Inline Map state_ to repeatedly generate version 4 universally
unique identifiers (v4 UUID). You start by creating a workflow that contains two [Pass workflow state](state-pass.md "state-pass.md") states and an _Inline Map state_ in the
...
You can also view the ASL definition in the [Code editor](workflow-studio.md#wfs-interface-code-editor "workflow-studio.md#wfs-interface-code-editor") of Workflow Studio. In the code editor, you can also edit the ASL definition of your workflow.
...
For this tutorial, enter the name `InlineMapDemo`. 3. (Optional) In **State machine configuration**, specify other workflow settings, such as state machine type and its execution role.

For this tutorial, keep all the default selections in **State machine configuration**. 4. In the **Confirm role creation** dialog box, choose **Confirm** to continue.
