...
- Reduce complexity and make it easier to edit and troubleshoot state machines.
  Step Functions can start these workflow executions by calling its own API as an [integrated service](integrate-services.md "integrate-services.md"). Simply call the
  `StartExecution` API action from your `Task` state and pass the
  necessary parameters. You can call the Step Functions API using any of the [service integration patterns](connect-to-resource.md "connect-to-resource.md").
...
ID from the [Context object](input-output-contextobject.md "input-output-contextobject.md") to the execution
...
`AWS_STEP_FUNCTIONS_STARTED_BY_EXECUTION_ID` when you start an execution. If
