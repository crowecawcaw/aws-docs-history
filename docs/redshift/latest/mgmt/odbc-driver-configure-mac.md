...
Additionally, under `/opt/amazon/redshift/Setup` on macOS, there are sample `odbc.ini` and `odbcinst.ini` files. You can use these files as examples for configuring the Amazon Redshift ODBC driver and the data source name (DSN).
...
```
 ...
 The following example shows the configuration for `odbc.ini` on macOS operating systems.
...
The following example shows the `odbcinst.ini` configuration for the driver installed in the default directory on macOS operating systems.
