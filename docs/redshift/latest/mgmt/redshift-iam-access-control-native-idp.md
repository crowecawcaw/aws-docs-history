...
For information about using session roles that are transferred from identity provider (IdP) groups, see [PG\_GET\_SESSION\_ROLES](https://docs.aws.amazon.com/redshift/latest/dg/PG_GET_SESSION_ROLES.html) in the *Amazon Redshift Database Developer Guide*. 
