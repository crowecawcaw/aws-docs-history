...
the configuration files. The sample files in the installed directory are for
example purposes only. If you reinstall the Amazon Redshift ODBC driver at a later
...
user's home directory, add a period (.) to the beginning of the file name to
make it a hidden file.

For the `odbc.ini` and `odbcinst.ini` files, either use
the configuration files in the user's home directory or create new versions in
another directory. By default, your Linux operating system should have an
`odbc.ini` file and an `odbcinst.ini` file in the
user's home directory (`/home/$USER` or `~/.`). These
default files are hidden files, which is indicated by the dot (.) in front of
each file name. These files display only when you use the `-a` flag
...
environment variables to specify where these configuration files are
located.
...
`amazon.redshiftodbc.ini` file in the driver installation
directory. If you store these configuration files elsewhere, set the environment
variables described following so that the driver manager can locate the
files.
...
- Set `ODBCINI` to the full path and file name of the
  `odbc.ini` file.
- Set `ODBCSYSINI` to the full path of the directory that
  contains the `odbcinst.ini` file.
- Set `AMAZONREDSHIFTODBCINI` to the full path and file name
  of the `amazon.redshiftodbc.ini` file.
...
the 64-bit ODBC driver on Linux operating systems.
...
with the 64-bit ODBC driver on Linux operating systems.
