...
Removes the longest string containing only characters in the trim characters list. Trimming is complete when a trim character does not appear in the input string.
...
The following example removes the trailing `'xyz'` strings
from the string `'xyzaxyzbxyzcxyz'`.
The trailing occurrences of `'xyz'` are removed,
