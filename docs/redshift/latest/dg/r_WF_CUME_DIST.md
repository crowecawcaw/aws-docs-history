...
where _x_ equals the value in the current row of the column
specified in the ORDER BY clause. The following dataset illustrates use of this
formula:
...
to one. If ORDER BY is omitted, the return value is 1 for all rows.
...
The following example calculates the cumulative distribution of the quantity for
each seller:
