...
`+--------------+-----+
| octet_length | len | +--------------+-----+
| 9 | 8 | +--------------+-----+` ``` To return the number of bytes and the number of characters in the string `français` without using the OCTET\_LENGTH function, use the following example. For more information, see the [CAST function](r_CAST_function.md "r_CAST_function.md"). ``` `SELECT LEN(CAST('français' AS VARBYTE)) as bytes, LEN('français');` `+-------+-----+
| bytes | len | +-------+-----+
| 9 | 8 | +-------+-----+` ``` To return the number of characters in the strings `cat` with no trailing spaces, `cat`  with three trailing spaces, `cat`  with three trailing spaces cast as a `CHAR` of length 6, and `cat`  with three trailing spaces cast as a `VARCHAR` of length 6, use the following example. Notice that the function does not count trailing spaces for `CHAR` strings, but it does count trailing spaces for `VARCHAR` strings. ``` `SELECT LEN('cat'), LEN('cat '), LEN(CAST('cat ' AS CHAR(6))) AS len_char, LEN(CAST('cat ' AS VARCHAR(6))) AS len_varchar;` `+-----+-----+----------+-------------+
| len | len | len_char | len_varchar | +-----+-----+----------+-------------+
| 3 | 6 | 3 | 6 | +-----+-----+----------+-------------+` ``` The following example uses data from the VENUE table in the TICKIT sample database. For more information, see [Sample database](c_sampledb.md "c_sampledb.md"). To return the 10 longest venue names in the VENUE table, use the following example. ``` `SELECT venuename, LEN(venuename) FROM venue ORDER BY 2 DESC, 1 LIMIT 10;` `+-----------------------------------------+-----+
| venuename | len | +-----------------------------------------+-----+ | Saratoga Springs Performing Arts Center | 39 |
| Lincoln Center for the Performing Arts | 38 | | Nassau Veterans Memorial Coliseum | 33 |
| Jacksonville Municipal Stadium | 30 | | Rangers BallPark in Arlington | 29 |
| University of Phoenix Stadium | 29 | | Circle in the Square Theatre | 28 |
| Hubert H. Humphrey Metrodome | 28 | | Oriole Park at Camden Yards | 27 |
