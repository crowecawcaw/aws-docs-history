...
The `refcursor` data type is the data type of cursor variables within stored procedures. A `refcursor` value can be returned from within a stored procedure. For more information, see [Returning a result set from a stored procedure](stored-procedure-result-set.md).
