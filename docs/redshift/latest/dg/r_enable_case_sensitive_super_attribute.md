...
For more information on navigating uppercase and mixed-case JSON fields, see [Accessing JSON fields with uppercase and mixed-case field names or attributes](super-configurations.md#upper-mixed-case).
...
+  We recommend that regular users querying tables with row-level security policies attached have the default `enable_case_sensitive_identifier` setting. For more information, see For information on row-level security, see [Row-level security](t_rls.md). 