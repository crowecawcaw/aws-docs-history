...
**Note**  
On RG provisioned clusters, data lake queries run on the cluster's own compute resources rather than on dedicated Redshift Spectrum servers. Similarly, Amazon Redshift Serverless processes data lake queries using its own compute capacity. For information for using integrated data lake queries on RG and Redshift Serverless, see [Querying your data lake](https://docs.aws.amazon.com/redshift/latest/gsg/data-lake.html).
