...
All rows
in SVV\_TRANSACTIONS are visible to all users. For more information about data
...
data generated by other users. For example, STL\_QUERY and STL\_QUERY\_TEXT contain the
...
The query-related system tables and views, such as SVL\_QUERY\_SUMMARY, SVL\_QLOG,
...
troubleshooting. Using SYS views such as [SYS\_QUERY\_HISTORY](SYS_QUERY_HISTORY.md "SYS_QUERY_HISTORY.md") and [SYS\_QUERY\_DETAIL](SYS_QUERY_DETAIL.md "SYS_QUERY_DETAIL.md") that consolidate the information of multiple
provisioned-only views, you can streamline your queries.

## Improving query identifier tracking using the SYS monitoring views

SYS monitoring views such as such as [SYS\_QUERY\_HISTORY](SYS_QUERY_HISTORY.md "SYS_QUERY_HISTORY.md") and [SYS\_QUERY\_DETAIL](SYS_QUERY_DETAIL.md "SYS_QUERY_DETAIL.md") contain the query\_id column, which holds the
identifier for users’ queries. Similarly, provisioned-only views such as [STL\_QUERY](r_STL_QUERY.md "r_STL_QUERY.md") and [SVL\_QLOG](r_SVL_QLOG.md "r_SVL_QLOG.md") contain the query column, which also holds the query
...
The difference between the SYS views’ query\_id column values and the provisioned-only
views’ query column values is as follows:

- In SYS views, the query\_id column records user-submitted queries in their
  ...
  single row in [SYS\_QUERY\_HISTORY](SYS_QUERY_HISTORY.md "SYS_QUERY_HISTORY.md"). If you want to see the individual child
  queries, you can find them in [SYS\_QUERY\_DETAIL](SYS_QUERY_DETAIL.md "SYS_QUERY_DETAIL.md").
- In provisioned-only views, the query column records queries at the child
  ...
  multiple child queries, there will be multiple rows in [STL\_QUERY](r_STL_QUERY.md "r_STL_QUERY.md") with differing query
...
In the provisioned-only system view STL\_QUERY, Amazon Redshift records five rows at the
...
In the SYS monitoring view SYS\_QUERY\_HISTORY, Amazon Redshift records the query as
follows:
...
In SYS\_QUERY\_DETAIL, you can find child query-level details using the query\_id
value from SYS\_QUERY\_HISTORY. The child\_query\_sequence column shows the order the
child queries are executed in. For more information on the columns in
SYS\_QUERY\_DETAIL, see [SYS\_QUERY\_DETAIL](SYS_QUERY_DETAIL.md "SYS_QUERY_DETAIL.md").
