...
| exec_time     | bigint    | Number of microseconds that the query has been executing.                                                                                                                                                      | ## Sample query The following query displays the current state of queries in service classes greater than 4. For a list of service class IDs, see [WLM service class IDs](cm-c-wlm-system-tables-and-views.md#wlm-service-class-ids "cm-c-wlm-system-tables-and-views.md#wlm-service-class-ids"). `select * from stv_wlm_query_task_state where service_class > 4;` This query returns the following sample output: ``` service_class | task | query | start_time | exec_time --------------+------+-------+----------------------------+----------- 5 | 466 | 491 | 2010-10-06 13:29:23.063787 | 357618748 (1 row) ``` |
