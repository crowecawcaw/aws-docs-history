...
STL\_RESTARTED\_SESSIONS records the new PID and the old PID.

For more information, see the examples following in this section.

STL\_RESTARTED\_SESSIONS is visible to all users. Superusers can see all rows; regular users can see only their own data. For more information, see [Visibility of data in system tables and views](cm_chap_system-tables.md#c_visibility-of-data "cm_chap_system-tables.md#c_visibility-of-data").
...
| session\_vars | character(2000) | This information is for internal use only.           |

## Sample queries

The following example joins STL\_RESTARTED\_SESSIONS with STL\_SESSIONS to show user
