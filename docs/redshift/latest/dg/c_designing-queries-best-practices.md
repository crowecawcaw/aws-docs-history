...
- Use a [CASE conditional expression](r_CASE_function.md "r_CASE_function.md") to perform
  complex aggregations instead of selecting from the same table multiple times.
...
- Use subqueries in cases where one table in the query is used only for predicate
  ...
  following example uses a subquery to avoid joining the LISTING table.
...
- In the predicate, use the least expensive operators that you can. [Comparison condition](r_comparison_condition.md "r_comparison_condition.md")
  operators are preferable to [LIKE](r_patternmatching_condition_like.md "r_patternmatching_condition_like.md") operators. LIKE operators are
  still preferable to [SIMILAR TO](pattern-matching-conditions-similar-to.md "pattern-matching-conditions-similar-to.md") or [POSIX operators](pattern-matching-conditions-posix.md "pattern-matching-conditions-posix.md").
- Avoid using functions in query predicates. Using them can drive up the cost of the
  ...
  query.
- If possible, use a WHERE clause to restrict the dataset. The query planner can
  ...
  scan participating columns entirely.
- Add predicates to filter tables that participate in joins, even if the predicates
  ...
  that's used in the join condition.

For example, suppose that you want to join `SALES` and
`LISTING` to find ticket sales for tickets listed after December,
grouped by seller. Both tables are sorted by date. The following query joins the
tables on their common key and filters for `listing.listtime` values
greater than December 1.
...
The WHERE clause doesn't include a predicate for `sales.saletime`, so
the execution engine is forced to scan the entire `SALES` table. If you
know the filter would result in fewer rows participating in the join, then add that
filter as well. The following example cuts execution time significantly.
...
You can confirm the use of one-phase aggregation by running the [EXPLAIN](r_EXPLAIN.md "r_EXPLAIN.md") command and looking for `XN
 GroupAggregate` in the aggregation step of the query.

- If you use both GROUP BY and ORDER BY clauses, make sure that you put the columns
  in the same order in both. That is, use the approach just following.
