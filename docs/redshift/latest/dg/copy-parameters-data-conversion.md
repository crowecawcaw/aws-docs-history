...
- [EXPLICIT\_IDS](#copy-explicit-ids "#copy-explicit-ids")
...
character specified by _replacement\_char_. For example,
...
and it adds an entry to the [STL\_REPLACEMENTS](r_STL_REPLACEMENTS.md "r_STL_REPLACEMENTS.md") system table for each affected row,
...
DATEFORMAT [AS] {'_dateformat\_string_' | 'auto' }
...
Valid values for _file\_encoding_ are as
follows:
...
- ACCEPTINVCHARS '_replacement\_char_'
- DATEFORMAT '_dateformat\_string_'
- TIMEFORMAT '_timeformat\_string_'
- NULL AS '_null\_string_'
...
Use EXPLICIT\_IDS with tables that have IDENTITY columns if you want to
...
EXPLICIT\_IDS values must match the IDENTITY format specified by the CREATE
TABLE definition.

When you run a COPY command against a table with the EXPLICIT\_IDS option, Amazon Redshift
...
copied. Values are generated or updated with values that you supply. The EXPLICIT\_IDS
option isn't required. COPY doesn't update the identity high watermark.

For an example of a COPY command using EXPLICIT\_IDS, see
...
IGNOREHEADER [ AS ] _number\_rows_

Treats the specified _number\_rows_ as a file header
and doesn't load them. Use IGNOREHEADER to skip file headers in all files
in a parallel load.

NULL AS '_null\_string_'

Loads fields that match _null\_string_ as NULL, where
_null\_string_ can be any string. If your data includes
...
not use '\n' (newline) for the _null\_string_ value.
...
TIMEFORMAT [AS] {'_timeformat\_string_' | 'auto' |
...
_timeformat\_string_. To load TIMESTAMPTZ data that is
...
_timeformat\_string_, see [DATEFORMAT and TIMEFORMAT strings](r_DATEFORMAT_and_TIMEFORMAT_strings.md "r_DATEFORMAT_and_TIMEFORMAT_strings.md").
