...
This topic demonstrates how to serialize nested data in JSON format. Nested data is
...
- When collection columns are serialized as VARCHAR(65535), their nested subfields can't be
...
- Primitive map keys are promoted to string (for example, `1` to
  `"1"`).
- Top-level null values are serialized as NULLs.
- If the serialization overflows the maximum VARCHAR size of 65535, the cell is set to
  NULL.
...
When `json_serialization_parse_nested_strings` is set, the
