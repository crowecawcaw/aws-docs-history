...
The following CREATE TABLE statement demonstrates the declaration of
...
The following example attempts to insert the value 33000 into a SMALLINT
column.
...
The range for SMALLINT is -32768 to +32767, so Amazon Redshift returns an
error.
...
The following example inserts the a decimal value into an INT
column.
...
The following example inserts a decimal value that has higher precision
that the column.
...
In this case, the value `35.51` is inserted into the
column.
