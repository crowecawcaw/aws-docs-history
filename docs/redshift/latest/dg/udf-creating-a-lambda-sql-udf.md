...
| --- | --- | --- | 
| request\_id | A universally unique identifier (UUID) that uniquely identifies each invoke request. | A valid UUID. | 
...
| num\_records | The number of arguments in the payload. | A value of 1 - 2^64. | 
...
| error\_msg | The error message if the success value is "false" (if the function fails); otherwise, this field is ignored. | A valid message. | 
| num\_records | The number of records in the payload. | A value of 1 - 2^64. | 
