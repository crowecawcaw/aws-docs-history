...
| relation_kind  | text      | The type of the relation that the masking policy is applied to. Possible values are `TABLE`, `VIEW`, `LATE BINDING VIEW`, and `MATERIALIZED VIEW`. | ## Sample queries The following example shows that the `mask_credit_card_full` masking policy is attached to the `credit_db.public.credit_cards` table. ``` select policy_name, database_name, relation_name, schema_name, relation_kind from sys_applied_masking_policy_log; policy_name | database_name | relation_name | schema_name | relation_kind ----------------------+---------------+---------------+-------------+--------------- mask_credit_card_full | credit_db | credit_cards | public | table (1 row) ``` |
