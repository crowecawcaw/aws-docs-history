...
Use the STV\_RECENTS table to find out information about the currently active and
recently run queries against a database.

STV\_RECENTS is visible to all users. Superusers can see all rows; regular users can see only their own data. For more information, see [Visibility of data in system tables and views](cm_chap_system-tables.md#c_visibility-of-data "cm_chap_system-tables.md#c_visibility-of-data").
...
STV\_RECENTS is particularly helpful for determining if a query or collection of queries is currently
...
You can join STV\_RECENTS
to other system views, such as [STV\_INFLIGHT](r_STV_INFLIGHT.md "r_STV_INFLIGHT.md"), to gather additional metadata about running queries. (There's an example
...
System views that compliment STV\_RECENTS include [STL\_QUERYTEXT](r_STL_QUERYTEXT.md "r_STL_QUERYTEXT.md"), which retrieves the query text for SQL commands, and [SVV\_QUERY\_INFLIGHT](r_SVV_QUERY_INFLIGHT.md "r_SVV_QUERY_INFLIGHT.md"), which joins STV\_INFLIGHT to STL\_QUERYTEXT.
