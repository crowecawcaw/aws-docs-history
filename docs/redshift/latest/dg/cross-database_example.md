...
This topic contains examples for how to use cross-database queries. Cross-database queries are
queries that operate on multiple databases within a single Amazon Redshift cluster.

Use the following examples to help learn how to set up a cross-database query that
references an Amazon Redshift database.

To start, create databases `db1` and `db2` and users
`user1` and `user2` in your Amazon Redshift cluster. For more information,
see [CREATE DATABASE](r_CREATE_DATABASE.md "r_CREATE_DATABASE.md") and [CREATE USER](r_CREATE_USER.md "r_CREATE_USER.md").
...
As `user1` on `db1`, create a table, grant access privileges to
`user2`, and insert values into `table1`. For more information,
see [GRANT](r_GRANT.md "r_GRANT.md") and [INSERT](r_INSERT_30.md "r_INSERT_30.md").
...
As `user2` on `db2`, run a cross-database query in
`db2` using the three-part notation.
...
Now grant write privileges to `user2`,
and insert values into `table1` in
`db1` as `user2` .
...
As `user2` on `db2`,
run a cross-database query in
`db2` using the three-part notation
to insert data into `table1` in `db1`.
...
As `user2` on `db2`, create an external schema and
run a cross-database query in `db2` using the external schema notation.
...
To create different views and grant permissions to those views, as `user1` on
`db1`, do the following.
...
As `user2` on `db2`, run the following cross-database query using
the three-part notation to view the particular view.
...
As `user2` on `db2`, run the following cross-database query using
the external schema notation to query the late-binding view.
...
As `user2` on `db2`, run the following command using connected
