...
- Partition Loop
  The following example shows the query plan for a query that joins an external table with
  a local table. Note the S3 Seq Scan and S3 HashAggregate steps that were run against
  the data on Amazon S3.
...
- The `S3 HashAggregate` node indicates aggregation in the Redshift
  Spectrum layer for the group by clause (`group by
 spectrum.sales.eventid`).
  Following are ways to improve Redshift Spectrum performance:

- Use Apache Parquet formatted data files. Parquet stores data in a columnar format,
  so Redshift Spectrum can eliminate unneeded columns from the scan. When data is in text-file
  format, Redshift Spectrum needs to scan the entire file.
- Use multiple files to optimize for parallel processing. Keep your file sizes
  ...
  [File Format: Configurations](https://parquet.apache.org/docs/file-format/configurations/ "https://parquet.apache.org/docs/file-format/configurations/")
  in the _Apache Parquet Documentation_.
- Use the fewest columns possible in your queries.
- Put your large fact tables in Amazon S3 and keep your frequently used, smaller
  dimension tables in your local Amazon Redshift database.
- Update external table statistics by setting the TABLE PROPERTIES numRows
  parameter. Use [CREATE EXTERNAL TABLE](r_CREATE_EXTERNAL_TABLE.md "r_CREATE_EXTERNAL_TABLE.md") or [ALTER TABLE](r_ALTER_TABLE.md "r_ALTER_TABLE.md") to set the TABLE PROPERTIES numRows parameter to
  ...
  are the larger tables and local tables are the smaller tables.
...
- Write your queries to use filters and aggregations that are eligible to be pushed
  to the Redshift Spectrum layer.

The following are examples of some operations that can be pushed to the Redshift
Spectrum layer:
...
Operations that can't be pushed to the Redshift Spectrum layer include DISTINCT
and ORDER BY.
