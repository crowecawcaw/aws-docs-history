...
When a materialized view requires a full recompute, the owner must also have CREATE privilege on the schema that contains the materialized view. A full recompute can be triggered by operations such as VACUUM or TRUNCATE on base tables.
...
+ Aggregate functions: MEDIAN, PERCENTILE\_CONT, LISTAGG, STDDEV\_SAMP, STDDEV\_POP, APPROXIMATE COUNT, APPROXIMATE PERCENTILE, and bitwise aggregate functions.
**Note**  
