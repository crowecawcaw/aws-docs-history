...
current session. PG\_LAST\_COPY\_COUNT is updated with the last COPY ID, which is the query
...
the COPY ID is not updated and PG\_LAST\_COPY\_COUNT returns the count for the previous
COPY. If no COPY commands were run in the current session, or if the last COPY
failed during loading, PG\_LAST\_COPY\_COUNT returns 0. For more information, see [PG\_LAST\_COPY\_ID](PG_LAST_COPY_ID.md "PG_LAST_COPY_ID.md").
