...
1. Connect to the database as the salesengineer user.
2. To update the eventname for eventid 1, run the following example.
...
3. To view the change made in the previous query, use the following example to select the event with eventid 1 from the events table in the sales schema.
...
5. Connect to the database as the salesengineer user using query editor v2.
6. To read the first 10 events from the events table in the sales schema, use the following example.
...
For the full list of system views, refer to [SVV metadata views](svv_views.md "svv_views.md").

## Use row-level security with RBAC (optional)

To have granular access control over your sensitive data, use row-level security (RLS). For more information about RLS, see [Row-level security](t_rls.md "t_rls.md").
...
- [Role hierarchy](t_role_hierarchy.md "t_role_hierarchy.md")
- [Role assignment](t_role_assignment.md "t_role_assignment.md")
- [Database object permissions](r_roles-database-privileges.md "r_roles-database-privileges.md")
- [ALTER DEFAULT PRIVILEGES for RBAC](r_roles-alter-default-privileges.md "r_roles-alter-default-privileges.md")
