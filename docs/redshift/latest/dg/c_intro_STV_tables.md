Amazon Redshift will no longer support the creation of new Python UDFs starting Patch 198.
Existing Python UDFs will continue to function until June 30, 2026. For more information, see the
[blog post](https://aws.amazon.com/blogs/big-data/amazon-redshift-python-user-defined-functions-will-reach-end-of-support-after-june-30-2026/ "https://aws.amazon.com/blogs/big-data/amazon-redshift-python-user-defined-functions-will-reach-end-of-support-after-june-30-2026/") .

# STV tables for snapshot data

STV tables are virtual system tables that contain snapshots of the current system
data.

###### Topics

- [STV\_ACTIVE\_CURSORS](r_STV_ACTIVE_CURSORS.md "r_STV_ACTIVE_CURSORS.md")
- [STV\_BLOCKLIST](r_STV_BLOCKLIST.md "r_STV_BLOCKLIST.md")
- [STV\_CURSOR\_CONFIGURATION](r_STV_CURSOR_CONFIGURATION.md "r_STV_CURSOR_CONFIGURATION.md")
- [STV\_DB\_ISOLATION\_LEVEL](r_STV_DB_ISOLATION_LEVEL.md "r_STV_DB_ISOLATION_LEVEL.md")
- [STV\_EXEC\_STATE](r_STV_EXEC_STATE.md "r_STV_EXEC_STATE.md")
- [STV\_INFLIGHT](r_STV_INFLIGHT.md "r_STV_INFLIGHT.md")
- [STV\_LOAD\_STATE](r_STV_LOAD_STATE.md "r_STV_LOAD_STATE.md")
- [STV\_LOCKS](r_STV_LOCKS.md "r_STV_LOCKS.md")
- [STV\_ML\_MODEL\_INFO](r_STV_ML_MODEL_INFO.md "r_STV_ML_MODEL_INFO.md")
- [STV\_MV\_DEPS](r_STV_MV_DEPS.md "r_STV_MV_DEPS.md")
- [STV\_MV\_INFO](r_STV_MV_INFO.md "r_STV_MV_INFO.md")
- [STV\_NODE\_STORAGE\_CAPACITY](r_STV_NODE_STORAGE_CAPACITY.md "r_STV_NODE_STORAGE_CAPACITY.md")
- [STV\_PARTITIONS](r_STV_PARTITIONS.md "r_STV_PARTITIONS.md")
- [STV\_QUERY\_METRICS](r_STV_QUERY_METRICS.md "r_STV_QUERY_METRICS.md")
- [STV\_RECENTS](r_STV_RECENTS.md "r_STV_RECENTS.md")
- [STV\_SESSIONS](r_STV_SESSIONS.md "r_STV_SESSIONS.md")
- [STV\_SLICES](r_STV_SLICES.md "r_STV_SLICES.md")
- [STV\_STARTUP\_RECOVERY\_STATE](r_STV_STARTUP_RECOVERY_STATE.md "r_STV_STARTUP_RECOVERY_STATE.md")
- [STV\_TBL\_PERM](r_STV_TBL_PERM.md "r_STV_TBL_PERM.md")
- [STV\_TBL\_TRANS](r_STV_TBL_TRANS.md "r_STV_TBL_TRANS.md")
- [STV\_WLM\_CLASSIFICATION\_CONFIG](r_STV_WLM_CLASSIFICATION_CONFIG.md "r_STV_WLM_CLASSIFICATION_CONFIG.md")
- [STV\_WLM\_QMR\_CONFIG](r_STV_WLM_QMR_CONFIG.md "r_STV_WLM_QMR_CONFIG.md")
- [STV\_WLM\_QUERY\_QUEUE\_STATE](r_STV_WLM_QUERY_QUEUE_STATE.md "r_STV_WLM_QUERY_QUEUE_STATE.md")
- [STV\_WLM\_QUERY\_STATE](r_STV_WLM_QUERY_STATE.md "r_STV_WLM_QUERY_STATE.md")
- [STV\_WLM\_QUERY\_TASK\_STATE](r_STV_WLM_QUERY_TASK_STATE.md "r_STV_WLM_QUERY_TASK_STATE.md")
- [STV\_WLM\_SERVICE\_CLASS\_CONFIG](r_STV_WLM_SERVICE_CLASS_CONFIG.md "r_STV_WLM_SERVICE_CLASS_CONFIG.md")
- [STV\_WLM\_SERVICE\_CLASS\_STATE](r_STV_WLM_SERVICE_CLASS_STATE.md "r_STV_WLM_SERVICE_CLASS_STATE.md")
- [STV\_XRESTORE\_ALTER\_QUEUE\_STATE](r_STV_XRESTORE_ALTER_QUEUE_STATE.md "r_STV_XRESTORE_ALTER_QUEUE_STATE.md")
