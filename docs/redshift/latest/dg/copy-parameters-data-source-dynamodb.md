...
*authorization*  
The COPY command needs authorization to access data in another AWS resource, including in Amazon S3, Amazon EMR, DynamoDB, and Amazon EC2. You provide that authorization by referencing an AWS Identity and Access Management (IAM) role that is attached to your cluster (role-based access control). For more information, see [Authorization parameters](copy-parameters-authorization.md).
