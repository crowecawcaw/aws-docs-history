...
DATE\_CMP\_TIMESTAMP compares a date to a timestamp and returns `0` if the values are identical, `1` if *date* is greater chronologically and `-1` if *timestamp* is greater.
