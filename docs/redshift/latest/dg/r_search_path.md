...
+ Search paths aren't supported with external schemas and external tables. External tables must be explicitly qualified by an external schema. Datashares behave as external data and are therefore not supported as search paths.
