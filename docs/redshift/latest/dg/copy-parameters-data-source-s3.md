...
To load data from files located in one or more S3 buckets, use the FROM clause to
indicate how COPY locates the files in Amazon S3. You can provide the object path to the
data files as part of the FROM clause, or you can provide the location of a manifest
file that contains a list of Amazon S3 object paths. COPY from Amazon S3 uses an HTTPS
connection. Make sure that the S3 IP ranges are added to your allow list. To learn more about
...
If the Amazon S3 buckets that hold the data files don't reside in the same AWS Region
as your cluster, you must use the [REGION](#copy-region "#copy-region") parameter to
...
```
FROM { 's3://*objectpath*' | 's3://*manifest\_file*' }
*authorization*
| MANIFEST
| ENCRYPTED
...
`custdata.txt`,`custdata.txt.1`,
`custdata.txt.2`, `custdata.txt.bak`,and so
on. The key prefix can also reference a number of folders. For
example, `'s3://amzn-s3-demo-bucket/custfolder'` refers to the folders
`custfolder`, `custfolder_1`,
`custfolder_2`, and so on. If a key prefix references
multiple folders, all of the files in the folders are loaded. If a key
...
If the S3 bucket that holds the data files doesn't reside
in the same AWS Region as your cluster, you must use the [REGION](#copy-region "#copy-region") parameter to specify the Region in which
the data is located.
...
The manifest is a text file in JSON format that lists the URL of
each file that is to be loaded from Amazon S3. The URL includes the bucket
name and full object path for the file. The files that are specified
in the manifest can be in different buckets, but all the buckets must
be in the same AWS Region as the Amazon Redshift cluster. If a file is listed
twice, the file is loaded twice. The following example shows the JSON
for a manifest that loads three files.
...
The double quotation mark characters are required, and must be simple
quotation marks (0x22), not slanted or "smart" quotation marks. Each entry in
the manifest can optionally include a `mandatory` flag. If
`mandatory` is set to `true`, COPY terminates
if it doesn't find the file for that entry; otherwise, COPY will
continue.
The default value for `mandatory` is `false`.

When loading from data files in ORC or Parquet format, a
`meta` field is required, as shown in the following
example.
...
The manifest file must not be encrypted or compressed, even if the
ENCRYPTED, GZIP, LZOP, BZIP2, or ZSTD options are specified. COPY returns an
error if the specified manifest file isn't found or the manifest file
isn't properly formed.

If a manifest file is used, the MANIFEST parameter must be
specified with the COPY command. If the MANIFEST parameter isn't
specified, COPY assumes that the file specified with FROM is a data
file.
...
resource, including in Amazon S3, Amazon EMR, Amazon DynamoDB, and
...
Specifies the AWS Region where the source data is located. REGION
is required for COPY from an Amazon S3 bucket or an DynamoDB table when the
...
manifest file or multiple Amazon S3 buckets, must be located in the
specified Region.

###### Note

Transferring data across Regions incurs additional charges
against the Amazon S3 bucket or the DynamoDB table that contains the data.
For more information about pricing, see **Data Transfer OUT
From Amazon S3 To Another AWS Region** on the [Amazon S3 Pricing](https://aws.amazon.com/s3/pricing/ "https://aws.amazon.com/s3/pricing/") page and
**Data Transfer OUT** on the [Amazon DynamoDB
Pricing](https://aws.amazon.com/dynamodb/pricing/ "https://aws.amazon.com/dynamodb/pricing/") page.
