...
Defines the window for the SUM function in terms of one or more
expressions.
...
The following example creates a cumulative (rolling) sum of sales quantities
ordered by date and sales ID:
...
The following example creates a cumulative (rolling) sum of sales quantities by
date, partition the results by seller ID, and order the results by date and sales ID
...
The following example numbers all of the rows sequentially in the result set,
ordered by the SELLERID and SALESID columns:
...
The following example numbers all rows sequentially in the result set, partition
the results by SELLERID, and order the results by SELLERID and SALESID within the
partition:
