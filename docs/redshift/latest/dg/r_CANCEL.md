 ...
 *process\_id*   
To cancel a query running in an Amazon Redshift cluster, use the `pid` (Process ID) from [STV\_RECENTS](r_STV_RECENTS.md) that corresponds to the query that you want to cancel.  
To cancel a query running in an Amazon Redshift Serverless workgroup, use the `session_id` from [SYS\_QUERY\_HISTORY](SYS_QUERY_HISTORY.md) that corresponds to the query that you want to cancel.
