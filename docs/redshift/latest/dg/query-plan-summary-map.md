...
| --- | --- | --- | 
| Aggregate<br />HashAggregate<br />GroupAggregate | AGGR | Evaluates aggregate functions and GROUP BY conditions. | 
| DS\_BCAST\_INNER | BCAST (broadcast) | Broadcasts an entire table or some set of rows (such as a filtered set of rows from a table) to all nodes. | 
| Doesn’t appear in query plan | DELETE | Deletes rows from tables. | 
| DS\_DIST\_NONE<br />DS\_DIST\_ALL\_NONE<br />DS\_DIST\_INNER<br />DS\_DIST\_ALL\_INNER<br />DS\_DIST\_ALL\_BOTH | DIST (distribute) | Distributes rows to nodes for parallel joining purposes or other parallel processing. | 
