...
The MAX window function returns the maximum of the input expression values. The MAX
...
A clause that specifies the window clauses for the aggregation
functions. The OVER clause distinguishes window aggregation functions from
normal set aggregation functions.

PARTITION BY _expr_list_

Defines the window for the MAX function in terms of one or more
expressions.
...
The following example shows the sales ID, quantity, and maximum quantity from the
...
The following example shows the salesid, quantity, and maximum quantity in a
restricted frame:
