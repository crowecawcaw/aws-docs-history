...
To see which permissions users have been granted for a database, use [HAS\_DATABASE\_PRIVILEGE](r_HAS_DATABASE_PRIVILEGE.md). To see which permissions users have been granted for a schema, use [HAS\_SCHEMA\_PRIVILEGE](r_HAS_SCHEMA_PRIVILEGE.md). To see which permissions users have been granted for a table, use [HAS\_TABLE\_PRIVILEGE](r_HAS_TABLE_PRIVILEGE.md). 
...
ON COPY JOB *job\_name*  <a name="on-copy-job"></a>
...
EXPLAIN { RLS \| MASKING } TO ROLE *rolename*  
