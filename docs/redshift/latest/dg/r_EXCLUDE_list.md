...
The EXCLUDE column\_list names the columns that are excluded from the query results. Using the EXCLUDE option is helpful when only a subset of columns need to be excluded from a *wide* table, which is a table that contains many columns. 
 ...
 *column\_list*   
A comma-separated list of one or more column names that exist in the tables referenced by the query. The *column\_list* can optionally be enclosed in parentheses. Only column names are supported in the exclude list of column names, not expressions (such as `upper(col1)`) or asterisk (\*).  
