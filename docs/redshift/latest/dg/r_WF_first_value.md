...
selected. The window frame is unbounded so the same first value is selected for each
...
The following example shows the use of the IGNORE NULLS option and relies on the
addition of a new row to the VENUE table:
