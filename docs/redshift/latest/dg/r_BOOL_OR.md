...
If one or more values in a set is `true`, the BOOL\_OR function returns `true` (`t`). If all values in a set are `false`, the function returns `false` (`f`). NULL can be returned if the value is unknown.
