...
ST\_Transform returns a new geometry with coordinates that are transformed in a spatial reference system
defined by the input spatial reference system identifier (SRID).
...
If the SRID value associated with the input _geom_ does not exist, then an error is returned.

If _srid_ does not exist, then an error is returned.
