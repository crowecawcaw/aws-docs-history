...
The following CREATE TABLE statement demonstrates the use of VARCHAR and
CHAR data types:
...
Because ADDRESS1 is a VARCHAR column, the trailing blanks in the second
inserted address are semantically insignificant. In other words, these two
inserted addresses _match_.
...
Character strings are not truncated to fit the declared width of the
column:
...
A workaround for this problem is to cast the value to the size of the
column:
...
In this case, the first 20 characters of the string (`City of South
 San Fr`) would be loaded into the column.
