...
(service classes). To view metrics for queries that have completed, see the [STL\_QUERY\_METRICS](r_STL_QUERY_METRICS.md "r_STL_QUERY_METRICS.md") system table.
...
STV\_QUERY\_METRICS tracks and aggregates metrics at the query, segment, and step level.
...
- If both `segment` and `step_type` are not
  ...
  STV\_QUERY\_METRICS is visible to all users. Superusers can see all rows; regular users can see only their own data. For more information, see [Visibility of data in system tables and views](cm_chap_system-tables.md#c_visibility-of-data "cm_chap_system-tables.md#c_visibility-of-data").
...
| step\_type             | integer   | Type of step that ran. For a description of<br>step types, see [Step types](#r_STV_QUERY_METRICS-step-type "#r_STV_QUERY_METRICS-step-type").                                                                                                                                                                                              |
...
| max\_blocks\_read      | bigint    | Maximum number of 1 MB blocks read by the segment,<br>aggregated across all slices. At the segment level, the maximum<br>number of 1 MB blocks read for the segment across all slices. At the<br>query level, the maximum number of 1 MB blocks read by any query<br>segment.                                                              |
...
| max\_blocks\_to\_disk  | bigint    | The maximum amount of disk space used to write<br>intermediate results, in 1 MB blocks. At the segment level, the<br>maximum amount of disk space used by the segment across all slices.<br>At the query level, the maximum amount of disk space used by any<br>query segment.                                                             |
| blocks\_to\_disk       | bigint    | The amount of disk space used by a query or<br>segment to write intermediate results, in 1 MB blocks.                                                                                                                                                                                                                                      |
