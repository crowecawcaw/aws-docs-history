...
STV\_QUERY\_METRICS is visible to all users. Superusers can see all rows; regular users can see only their own data. For more information, see [Visibility of data in system tables and views](cm_chap_system-tables.md#c_visibility-of-data).
...
| service\_class  | integer  | ID for the WLM query queue (service class). Query queues are defined in the WLM configuration. Metrics are reported only for user-defined queues. | 
...
| step\_type  | integer  | Type of step that ran. For a description of step types, see [Step types](#r_STV_QUERY_METRICS-step-type).  | 
...
| max\_blocks\_read  | bigint  | Maximum number of 1 MB blocks read by the segment, aggregated across all slices. At the segment level, the maximum number of 1 MB blocks read for the segment across all slices. At the query level, the maximum number of 1 MB blocks read by any query segment.  | 
...
| max\_blocks\_to\_disk  | bigint  | The maximum amount of disk space used to write intermediate results, in 1 MB blocks. At the segment level, the maximum amount of disk space used by the segment across all slices. At the query level, the maximum amount of disk space used by any query segment.  | 
| blocks\_to\_disk  | bigint  | The amount of disk space used by a query or segment to write intermediate results, in 1 MB blocks. | 
