...
+ MAX\_BATCH\_ROWS is required when both input and returned data types are SUPER. 
+ When the input data type is `SUPER` the content type of the endpoint invocation is either `application/json` when MAX\_BATCH\_ROWS is `1` or `application/jsonlines` in all other cases. 
+ When the returned data type is `SUPER` the accept type of the endpoint invocation is either `application/json` when MAX\_BATCH\_ROWS is `1` or `application/jsonlines` in all other cases. 
