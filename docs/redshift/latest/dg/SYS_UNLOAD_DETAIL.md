...
SYS\_UNLOAD\_DETAIL is visible to all users. Superusers can see all rows; regular users can see only their own data. For more information, see [Visibility of data in system tables and views](cm_chap_system-tables.md#c_visibility-of-data).
...
| --- | --- | --- | 
| user\_id | integer | The identifier of the user who generated the entry. | 
| query\_id | integer | The query identifier of the UNLOAD command. | 
| session\_id | integer | The ID of the process associated with the query statement. | 
| transaction\_id | bigint | The ID of the transaction associated with the query statement. | 
| file\_name | character (1280) | The complete Amazon S3 object path for the file. | 
