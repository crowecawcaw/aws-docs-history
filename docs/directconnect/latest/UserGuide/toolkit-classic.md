...
For dedicated connections, you can submit a connection request using the AWS Direct Connect console. For hosted connections, work with an AWS Direct Connect Partner
...
4.  On the **Create Connection** pane, under
    **Connection settings,** do the following:
    1. For **Name**, enter a name for the
       connection.
    2. For **Location**, select the appropriate AWS Direct Connect
       location.
    ...
    5. For **On-premises**, select **Connect through
       an AWS Direct Connect partner** when you use this
...
You must accept the hosted connection in the AWS Direct Connect console before you can
...
Support](https://aws.amazon.com/support/createCase "https://aws.amazon.com/support/createCase"). 5. After you download the LOA-CFA, do one of the following:
    ...
    * If you're working with an AWS Direct Connect Partner or network provider, send them
     ...
     AWS Direct Connect location. If they cannot order the cross-connect for you, you
...
A campus is treated as a single AWS Direct Connect location. To achieve high
availability, configure connections to different AWS Direct Connect
locations.
...
To begin using your AWS Direct Connect connection, you must create a virtual interface. You can
...
supports jumbo frames, select it in the AWS Direct Connect console and find **Jumbo Frame
...
5.  Under **Public virtual interface settings**, do the
    ...
    4. For **BGP ASN**, enter the The Border Gateway
       ...
       the new virtual interface. The valid values are 1 to 4294967294. This includes support for both ASNs (1-2147483647) and long ASNs (1-4294967294). For more information about ASNs and long ASNs see [Long ASN support in AWS Direct Connect](long-asn-support.md "long-asn-support.md").
...
6. Choose **Download**, and then use the appropriate
   configuration for your router to ensure that you can connect to AWS Direct Connect.
...
the AWS Direct Connect device's peer IP address, see [Troubleshoot layer 2 (data link) issues](ts-layer-2.md "ts-layer-2.md"). If you can ping the peer IP address, see [Troubleshoot layer 3/4 (Network/Transport) issues](ts-layer-3.md "ts-layer-3.md"). If the BGP peering session
...
- Run `traceroute` and verify that the AWS Direct Connect identifier is in
  the network trace.
...
- Active/Active (BGP multipath). This is the default configuration, where
  both connections are active. AWS Direct Connect supports multipathing to multiple
...
For more information about AWS Direct Connect resiliency, see [AWS Direct Connect Resiliency Recommendations](https://aws.amazon.com/directconnect/resiliency-recommendation/ "https://aws.amazon.com/directconnect/resiliency-recommendation/").
