...
<a name="using-cli-create-connection"></a>

The first step is to submit a connection request. Ensure that you know the port speed that you require and the Direct Connect location. For more information, see [Dedicated and hosted connections](WorkingWithConnections.md).

**To create a connection request**

1. Describe the Direct Connect locations for your current Region. In the output that's returned, take note of the location code for the location in which you want to establish the connection.
...
When you create a virtual interface using the AWS CLI, the output includes generic router configuration information. To create a router configuration that's specific to your device, use the Direct Connect console. For more information, see [Download the router configuration file](vif-router-config.md).
