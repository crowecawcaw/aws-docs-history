...
# Add Users
<a name="config-cond-cf-cg-users"></a>
...
1. Hover over **Configuration** (cog icon) on the main menu and choose **Users** from the dropdown menu.

1. On the Conductor Configuration screen, choose **Settings** > **Users**.

1. On the Users screen, complete all fields and choose **Create**. Some notes:
   ...
   + **Role**: Select a role: Admin, Manager, User, Viewer. For information about the actions allowed with each role, see the following section *user Roles*. 

1. If your organization uses the REST API, make sure to tell each user to choose **Settings** > **User Profile** to make a note of their personal API key.
...
+ **Administrator**
  + Access to the entire AWS Elemental Conductor File and AWS Elemental Server systems, including all of the access provided by the other roles