...
This section describes the system requirements if you're using a virtual machine (VM).

**Note**  
Other than recommended and minimum hardware requirements, this information pertains only to VMs. It is not intended for kernel-based virtual machines (KVMs).
...
<a name="vm-req-guests"></a>

Each instance of AWS Elemental products is considered a *guest*.

We recommend one AWS Elemental Live or AWS Elemental Server virtual machine per host hardware.

For other AWS Elemental products, make sure the combined loads for all products do not exceed recommended hardware requirements. See the following sections for details.
...
+ Disk space: 500 GB
+ CPU cores: 24
+ Processor speed: 2.3 GHz or more (Comparable to an Intel® Xeon processor E5-2630)
...
You can use host hardware with these minimum resources to run AWS Elemental products for functional testing or for integrating with the AWS Elemental software API. These resource levels are not for performance testing. 
...
<a name="vm-req-qual"></a>

Verify that the host hardware platform is compatible with the VMware platform. Look at the *VMware Compatibility Guide* at vmware.com. AWS Elemental has specifically tested and qualified the following hardware:
+ Cisco® UCS®
