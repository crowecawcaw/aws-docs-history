...
1. On the **Set compute and scaling configuration** page, fill out the parameters accordingly, and then choose **Next**.
   ...
   +  **Capacity type** – Select a capacity type. For more information about choosing a capacity type, see [Managed node group capacity types](managed-node-groups.md#managed-node-group-capacity-types). You can’t mix different capacity types within the same node group. If you want to use both capacity types, create separate node groups, each with their own capacity and instance types. See [Manage compute for AI/ML workloads on Amazon EKS with node groups](ml-node-groups.md) for information on provisioning and scaling GPU-accelerated worker nodes.
