...
- Cilium is already installed on your hybrid nodes. For installation instructions, see [Configure CNI for hybrid nodes](hybrid-nodes-cni.md "hybrid-nodes-cni.md").
...
- Cilium 1.19.2-1 or later
  If you are running an older version, upgrade Cilium before enabling VTEP. For upgrade instructions, see [Configure CNI for hybrid nodes](hybrid-nodes-cni.md "hybrid-nodes-cni.md").

## Enable VTEP and disable L7 proxy

Set the Cilium version and run the `helm upgrade` command to enable VTEP and disable the L7 proxy on your existing Cilium installation. You can find the latest available patch version at [Amazon ECR Public: eks/cilium/cilium](https://gallery.ecr.aws/eks/cilium/cilium "https://gallery.ecr.aws/eks/cilium/cilium").
...
After configuring Cilium and VPC CNI, proceed to install the Hybrid Nodes gateway. See [Get started with EKS Hybrid Nodes gateway](hybrid-nodes-gateway-getting-started.md "hybrid-nodes-gateway-getting-started.md").
