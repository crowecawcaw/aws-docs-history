...
1. It cordons the old node as soon as a new node reaches *Ready* state, preventing the service controller from sending new requests to that node.
...
1. It waits 60 seconds after evicting every Pod. It then sends a termination request to the Auto Scaling Group and removes the node from its list of active nodes.
