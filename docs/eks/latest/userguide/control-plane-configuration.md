...
+  **Pricing** – There is no additional charge for configuring control plane parameters. Using `horizontalPodAutoscalerSyncPeriod` requires Provisioned Control Plane, which is billed at the hourly rate for your scaling tier. For more information, see [Amazon EKS pricing](https://aws.amazon.com/eks/pricing/).

## Next steps
<a name="_next_steps"></a>
+  [Configure advanced Kubernetes control plane parameters](control-plane-configuration-getting-started.md) – Set and view control plane parameters using the AWS CLI and AWS Management Console.
