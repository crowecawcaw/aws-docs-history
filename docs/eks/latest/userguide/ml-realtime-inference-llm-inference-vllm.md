...
For additional guidance and advanced deployment resources, check our [EKS Best Practices Guide for AI/ML workloads](../best-practices/aiml.md "../best-practices/aiml.md") and production-ready [AI on EKS inference charts](https://github.com/awslabs/ai-on-eks/tree/main/blueprints/inference/inference-charts "https://github.com/awslabs/ai-on-eks/tree/main/blueprints/inference/inference-charts").
...
Other storage solutions are also available on EKS for model caching, such as EFS and FSx for Lustre. For more information, check [EKS Best Practices](../best-practices/aiml-storage.md "../best-practices/aiml-storage.md").
...
If S3 Mountpoint CSI is not installed in the cluster, follow the deployment steps in [Best Practices Cluster Setup Guide for Real-Time Inference on Amazon EKS](ml-realtime-inference-cluster.md "ml-realtime-inference-cluster.md").
...
We are using the EKS cluster created in [Best Practices Cluster Setup Guide for Real-Time Inference on Amazon EKS](ml-realtime-inference-cluster.md "ml-realtime-inference-cluster.md"). This cluster includes Karpenter nodepools that can provision GPU enabled nodes with sufficient node storage to download vLLM container image. If using your custom EKS cluster, ensure that it can launch GPU enabled nodes.
...
[Amazon G5 EC2 Instances](https://aws.amazon.com/ec2/instance-types/g5/ "https://aws.amazon.com/ec2/instance-types/g5/") with A10G GPUs and [G6 EC2 instances](https://aws.amazon.com/ec2/instance-types/g6/ "https://aws.amazon.com/ec2/instance-types/g6/") with L4 GPUs both provide 24GB VRAM per GPU, sufficient for loading Llama 3.1 8B Instruct weights. If you are deploying a model with larger weights, consider using a multi-GPU instance type or a multi-node setup.
...
**IMPORTANT**: This pod uses a nodeSelector configuration that aligns with Karpenter node pools in [Best Practices Cluster Setup Guide for Real-Time Inference on Amazon EKS](ml-realtime-inference-cluster.md "ml-realtime-inference-cluster.md"). If you are using different node pools, ensure the pod matches nodeSelector and Tolerations accordingly.
...
[AWS Deep Learning Containers](https://github.com/aws/deep-learning-containers/tree/master "https://github.com/aws/deep-learning-containers/tree/master") (DLCs) provide pre-optimized environments with security updates, AWS infrastructure compatibility, and optimized driver configurations. This reduces deployment complexity and maintenance overhead while ensuring production readiness.
...
For demonstration, this guide runs a sample chatbot application using project [nextjs-vllm-ui](https://github.com/yoziru/nextjs-vllm-ui "https://github.com/yoziru/nextjs-vllm-ui") to showcase user interactions with the model.
...
![Chat UI Interface](images/llm-inference-vllm-chatui.png)
...
![Baseline Benchmark Results](images/llm-inference-vllm-guidellm-baseline.png)
...
![Optimized Benchmark Results](images/llm-inference-vllm-guidellm-optimized.png)
