...
If you don’t provide a key pair here, the AWS CloudFormation stack fails to be created.
   ...
   +  **NodeSecurityGroups**: Select the security group that was created for your Linux node group when you created your [VPC](creating-a-vpc.md). If your Linux nodes have more than one security group attached to them, specify all of them. This is the case, for example, if the Linux node group was created with `eksctl`.
