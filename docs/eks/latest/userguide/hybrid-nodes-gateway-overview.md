...
The Amazon EKS Hybrid Nodes gateway automates networking between your Amazon EKS cluster VPC and Kubernetes pods running on [EKS Hybrid Nodes](hybrid-nodes-overview.md "hybrid-nodes-overview.md"). The gateway eliminates the need to make on-premises pod networks routable from the VPC or coordinate network infrastructure changes. It creates VXLAN tunnels between EC2-based gateway nodes in your VPC and Cilium-managed hybrid nodes in your on-premises environment, and it automatically maintains VPC route table entries so that traffic reaches the correct gateway instance.
...
For all deployment targets, at least two nodes are recommended for high availability. For more information, see [Get started with EKS Hybrid Nodes gateway](hybrid-nodes-gateway-getting-started.md "hybrid-nodes-gateway-getting-started.md").

## Pricing

There is no additional charge for the Amazon EKS Hybrid Nodes gateway, but you will be charged for the infrastructure costs for running the gateway, including the EC2 instances and EKS Auto Mode management fees if applicable. For more information, see [Amazon EKS pricing](https://aws.amazon.com/eks/pricing/ "https://aws.amazon.com/eks/pricing/").

## Region availability

The Amazon EKS Hybrid Nodes gateway is available in all AWS Regions where EKS Hybrid Nodes is available, except China Regions. For the current list of supported Regions, see [Amazon EKS Hybrid Nodes overview](hybrid-nodes-overview.md "hybrid-nodes-overview.md").

## Open source

The Amazon EKS Hybrid Nodes gateway codebase is open source. You can view the source code, report issues, and contribute at the [GitHub repository](https://github.com/aws/eks-hybrid-nodes-gateway "https://github.com/aws/eks-hybrid-nodes-gateway").

## Limitations and considerations

Before deploying the Hybrid Nodes gateway, consider the following:

- **No traffic encryption** — The VXLAN tunnels created by the gateway do not encrypt traffic. If you require encryption in transit between the VPC and your on-premises environment, use an encrypted transport such as AWS Direct Connect with MACsec or a VPN connection. For more information, see [Data protection in Amazon EKS](data-protection.md "data-protection.md").
...
- [Get started with EKS Hybrid Nodes gateway](hybrid-nodes-gateway-getting-started.md "hybrid-nodes-gateway-getting-started.md")
- [Amazon EKS Hybrid Nodes gateway configuration reference](hybrid-nodes-gateway-configuration.md "hybrid-nodes-gateway-configuration.md")
- [Amazon EKS Hybrid Nodes gateway operations](hybrid-nodes-gateway-operations.md "hybrid-nodes-gateway-operations.md")
- [Amazon EKS Hybrid Nodes gateway troubleshooting](hybrid-nodes-gateway-troubleshooting.md "hybrid-nodes-gateway-troubleshooting.md")
