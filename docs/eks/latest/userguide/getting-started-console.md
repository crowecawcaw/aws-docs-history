...
1. On the **Specify networking** page, do the following:
   ...
   1. Choose the subnets created in a previous step from the **Subnets** dropdown list. The subnets will be something like {{\* \| my-eks-vpc-stack-\*}}.

   1. Choose the security group created in a previous step from the **Additional security groups** dropdown list. It is something like {{\* \| my-eks-vpc-stack-ControlPlaneSecurityGroup-\*}}.
...
<a name="option_1_connect_using_the_shared_aws_cli"></a>
...
<a name="_option_2_connect_using_the_eks_console"></a>

Alternatively, you can connect directly from the console without any local setup:

1. On the cluster details page for the cluster you created in [Step 1](#eks-create-cluster), choose **Connect**.
...
For more information, see [Connect kubectl to an EKS cluster by creating a kubeconfig file](create-kubeconfig.md).
