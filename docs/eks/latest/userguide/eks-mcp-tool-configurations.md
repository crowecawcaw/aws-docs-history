 ...
 ** `AWS_PROFILE` ** (optional) AWS credentials profile name to use; can be overridden by the `--profile` command-line argument.
+ Example: `export AWS_PROFILE=production` 

 ** `AWS_REGION` ** (optional) AWS region for SigV4 signing; defaults to `us-west-2` if not set.
 ...
 ** `--service` ** (optional) AWS service name for SigV4 signing; auto-detected from the endpoint hostname if not provided.
+ Example: `--service eks-mcp` 

 ** `--profile` ** (optional) AWS credentials profile to use. Defaults to the `AWS_PROFILE` environment variable if not specified.
+ Example: `--profile production` 

 ** `--region` ** AWS region to use. Uses `AWS_REGION` environment variable if not set, defaults to `us-east-1`.
