...
+ If you create a cluster with the AWS Management Console, the AWS Management Console automatically adds the Amazon EKS `kube-proxy`, Amazon VPC CNI plugin for Kubernetes, and CoreDNS add-ons to your cluster. If you create your cluster using `eksctl` without a `config` file, `eksctl` installs `kube-proxy`, Amazon VPC CNI plugin for Kubernetes, CoreDNS, and metrics-server as Amazon EKS add-ons starting with version 0.184.0. For earlier versions of `eksctl`, or if you use any other tool, the self-managed `kube-proxy`, Amazon VPC CNI plugin for Kubernetes, and CoreDNS add-ons install instead. You can either manage them yourself or add the Amazon EKS add-ons manually after cluster creation. Regardless of the method that you use to create your cluster, the Amazon VPC CNI plugin for Kubernetes add-on doesn’t install on hybrid nodes.
