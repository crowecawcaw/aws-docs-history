...
<a name="security-iam-awsmanpol-AmazonEKSBlockStoragePolicyV2"></a>

You can attach `AmazonEKSBlockStoragePolicyV2` to your IAM entities. You can attach this policy to your [cluster IAM role](cluster-iam-role.md) to expand the resources EKS can manage in your account.
...
To view the latest version of the JSON policy document, see [AmazonEKSBlockStoragePolicyV2](https://docs.aws.amazon.com/aws-managed-policy/latest/reference/AmazonEKSBlockStoragePolicyV2.html#AmazonEKSBlockStoragePolicyV2-json) in the AWS Managed Policy Reference Guide.
...
+  ** `wafv2` ** and ** `shield` **: Allows listing, associating, and disassociating Web ACLs and creating/deleting AWS Shield protections for the Elastic Load Balancers.
...
This policy includes the following permissions that allow Amazon EKS to complete the following tasks.
+  ** `ec2` ** – Create and describe Elastic Network Interfaces, Amazon EC2 instances and their statuses, the cluster security group, and VPC that are required to create a cluster. For more information, see [View Amazon EKS security group requirements for clusters](sec-group-reqs.md). Read information about security groups. Update tags on security groups. Read information about On-Demand Capacity Reservations. Read information about Placement Groups. Read VPC configuration including route tables and network ACLs to detect configuration issues as part of cluster insights.
...
+  ** `iam` ** – List all of the managed policies that are attached to an IAM role. This is required so that Amazon EKS can list and validate all managed policies and permissions required to create a cluster.
...
| --- | --- | --- | 
| Updated permissions to [AWS managed policy: AmazonEKSLoadBalancingPolicy](#security-iam-awsmanpol-AmazonEKSLoadBalancingPolicy). | Added `gateway.eks.amazonaws.com.alb/stack`, `gateway.eks.amazonaws.com.alb/resource`, `gateway.eks.amazonaws.com.nlb/stack`, and `gateway.eks.amazonaws.com.nlb/resource` tag keys to the resource-tagging allowlist in `AmazonEKSLoadBalancingPolicy`. This allows Amazon EKS Auto Mode Controller to manage AWS resources for Kubernetes Gateway API (ALB and NLB) workloads. | September 2, 2026 | 
| Added permission to [AWS managed policy: AmazonEKSLoadBalancingPolicy](#security-iam-awsmanpol-AmazonEKSLoadBalancingPolicy). | Added `wafv2:ListWebACLs` permission in `AmazonEKSLoadBalancingPolicy`. This allows Amazon EKS Auto Mode Controller to list AWS WAF Web ACLs when reconciling Application Load Balancer associations. | September 2, 2026 | 
| Added permission to [AmazonEKSServiceRolePolicy](#security-iam-awsmanpol-amazoneksservicerolepolicy). | Added `AmazonEKSPodIdentityPolicy` to the list of access policies that the service-linked role can associate and dissociate from cluster access entries. With this addition, Amazon EKS can manage pod identity credentials for workloads running on EKS clusters. | June 26, 2026 | 
| Updated permissions to [AWS managed policy: AmazonEKSLoadBalancingPolicy](#security-iam-awsmanpol-AmazonEKSLoadBalancingPolicy). | Added `elasticloadbalancing:SetRulePriorities`, `ec2:DescribeIpamPools` permissions in `AmazonEKSLoadBalancingPolicy`. This allows Amazon EKS Auto Mode Controller to set priorities of the specified listener rules for ALB and describe IPAM pools, respectively. | June 15, 2026 | 
...
| Added permission to [AWS managed policy: AmazonEKSComputePolicy](#security-iam-awsmanpol-AmazonEKSComputePolicy). | Added resource permissions to include the `ec2:DescribeCapacityReservations` action. This allows Amazon EKS Auto Mode to launch instances from Interruptible Capacity Reservations in your account. | May 18, 2026 | 
| Introduced [AWS managed policy: AmazonEKSBlockStoragePolicyV2](#security-iam-awsmanpol-AmazonEKSBlockStoragePolicyV2). |  AWS introduced the `AmazonEKSBlockStoragePolicyV2`. | May 12, 2026 | 
| Added permissions to [AmazonEBSCSIDriverPolicy](#security-iam-awsmanpol-amazonebscsidriverservicerolepolicy). | Added new API action `ec2:DescribeInstanceTypes` to enable the EBS CSI Driver to dynamically retrieve volume limit/card information at runtime. | May 12, 2026 | 
| Added permission to [AWS managed policy: AmazonEKSLoadBalancingPolicy](#security-iam-awsmanpol-AmazonEKSLoadBalancingPolicy)  | Added `elasticloadbalancing:DescribeLoadBalancers` permission in `AmazonEKSLoadBalancingPolicy` to allow Amazon EKS Auto Mode Controller to describe load balancers. | April 27, 2026 | 
| Added permission to [AWS managed policy: AmazonEKSLoadBalancingPolicy](#security-iam-awsmanpol-AmazonEKSLoadBalancingPolicy)  | Added `elasticloadbalancing:DescribeCapacityReservation` permission in `AmazonEKSLoadBalancingPolicy` to allow Amazon EKS Auto Mode Controller to describe LoadBalancer capacity units | April 20, 2026 | 
| Added permission to [AWS managed policy: AmazonEKSServiceRolePolicy](#security-iam-awsmanpol-amazoneksservicerolepolicy)  | Added `ec2:DescribeInstanceStatus` and `iam:ListInstanceProfiles` permissions in `AmazonEKSServiceRolePolicy`. These permissions allow Amazon EKS Auto Mode to discover EC2 health events for managed instances and cleanup managed instance profiles | April 20, 2026 | 
| Added permission to [AWS managed policy: AmazonEKSComputePolicy](#security-iam-awsmanpol-AmazonEKSComputePolicy). | Updated resource permissions for the `ec2:RunInstances` and `ec2:CreateFleet` actions to include placement groups ` arn:aws:ec2:*:*:placement-group/*`. This allows Amazon EKS Auto Mode to launch instances into the EC2 Placement Groups in your account. | April 17, 2026 | 
...
| Added permission to [AmazonEKSServiceRolePolicy](#security-iam-awsmanpol-amazoneksservicerolepolicy). | Added `ec2:DescribePlacementGroups` permission to allow Amazon EKS Auto Mode to launch instances into the EC2 Placement Groups in your account. | April 15, 2026 | 
...
| Introduced [AmazonEKSBlockStoragePolicy](https://docs.aws.amazon.com/aws-managed-policy/latest/reference/AmazonEKSBlockStoragePolicy.html). |  AWS introduced the `AmazonEKSBlockStoragePolicy`. | October 30, 2024 | 
