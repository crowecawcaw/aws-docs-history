...
Kubernetes node taints can be applied to new and existing managed node groups using the AWS Management Console or through the Amazon EKS API. This recipe shows how to apply taints to Amazon EKS managed node groups using the AWS CLI. For information on creating a node group with a taint using the AWS Management Console, see [Create a managed node group for your cluster](create-managed-node-group.md "create-managed-node-group.md").

## Prerequisites

- An [existing Amazon EKS cluster](getting-started.md "getting-started.md").
- [AWS CLI installed and configured](setting-up.md "setting-up.md") with appropriate permissions.
...
For more information and examples, see [taint](https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#taint "https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#taint") in the Kubernetes reference documentation.

### Step 2: Update taints on an existing node group

Use the [aws eks update-nodegroup-config](../../../cli/latest/reference/eks/update-nodegroup-config.md "../../../cli/latest/reference/eks/update-nodegroup-config.md")
