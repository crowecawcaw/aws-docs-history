...
1.  Create an `ENIConfig` custom resource for each subnet that you want to deploy Pods in.
   ...
   1. Create a unique file for each network interface configuration.
      ...
      + Replace $cluster\_security\_group\_id with the ID of an existing [security group](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-security-groups.html) that you want to use for each `ENIConfig`.
