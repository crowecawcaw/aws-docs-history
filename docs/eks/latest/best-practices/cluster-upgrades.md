...
[Kubernetes
minor version, such as 1.24](https://kubernetes.io/releases/version-skew-policy/#supported-versions "https://kubernetes.io/releases/version-skew-policy/#supported-versions"). While AWS manages and upgrades the
...
- **Understand Deprecation Policies** — Gain a deep understanding of how
  the
  [Kubernetes
  deprecation policy](https://kubernetes.io/docs/reference/using-api/deprecation-policy/ "https://kubernetes.io/docs/reference/using-api/deprecation-policy/") works. Be aware of any upcoming changes that may
...
- **Review Kubernetes Change Log** — Thoroughly review the
  [Kubernetes
  change log](https://github.com/kubernetes/kubernetes/tree/master/CHANGELOG "https://github.com/kubernetes/kubernetes/tree/master/CHANGELOG") alongside
  [Amazon
  EKS Kubernetes versions](../userguide/kubernetes-versions.md "../userguide/kubernetes-versions.md") to understand any possible impact to your
...
- **Assess Cluster Add-Ons Compatibility** — Amazon EKS doesn’t
  ...
  [Updating
  an add-on](../userguide/managing-add-ons.md#updating-an-add-on "../userguide/managing-add-ons.md#updating-an-add-on") to understand the compatibility of any existing cluster
  add-ons with the cluster version you intend to upgrade to.
- **Enable Control Plane Logging** — Enable
  [control
  plane logging](../userguide/control-plane-logs.md "../userguide/control-plane-logs.md") to capture logs, errors, or issues that can arise during
...
- **Explore eksctl for Cluster Management** — Consider using
  [eksctl](https://eksctl.io/ "https://eksctl.io/") to manage your EKS cluster. It provides you
  with the ability to [update the
  control plane, manage add-ons, and handle worker node updates](https://eksctl.io/usage/cluster-upgrade/ "https://eksctl.io/usage/cluster-upgrade/")
  out-of-the-box.
- **Opt for Auto Mode or Managed Node Groups** — Streamline and
  automate worker node upgrades by using [Auto Mode](../userguide/automode.md "../userguide/automode.md") or
  [EKS
  managed node groups](../userguide/managed-node-groups.md "../userguide/managed-node-groups.md"). These options simplify the process and reduce manual
  intervention.
...
[Review
the EKS Kubernetes release calendar](../userguide/kubernetes-versions.md#kubernetes-release-calendar "../userguide/kubernetes-versions.md#kubernetes-release-calendar") to learn when new versions are
...
[Kubernetes release information](https://kubernetes.io/releases/ "https://kubernetes.io/releases/").
...
[Learn
how to initiate an upgrade.](../userguide/update-cluster.md "../userguide/update-cluster.md") When you initiate a cluster upgrade, AWS
manages upgrading the cluster control plane. You are responsible for
upgrading the data plane, including Fargate pods and
[addons](#upgrade-addons "#upgrade-addons"). You must validate and plan upgrades for workloads running on
...
more complicated, and has a higher risk of downtime. In this situation, see [Evaluate Blue/Green Clusters as an alternative to in-place cluster upgrades](#bluegreen "#bluegreen").
...
1. [Review
   the Kubernetes and EKS release notes.](#usedocs "#usedocs")
2. [Take a backup of the
   cluster. (optional)](#backup-the-cluster-before-upgrading "#backup-the-cluster-before-upgrading")
3. [Identify
   and remediate deprecated and removed API usage in your workloads.](#identify-and-remediate-removed-api-usage-before-upgrading-the-control-plane "#identify-and-remediate-removed-api-usage-before-upgrading-the-control-plane")
4. [Ensure
   Managed Node Groups, if used, are on the same Kubernetes version
   as the control plane.](#node-compatibility "#node-compatibility") EKS managed node groups and nodes created by EKS Fargate Profiles support 2 minor version skew between the control plane and data plane for Kubernetes version 1.27 and below. Starting 1.28 and above, EKS managed node groups and nodes created by EKS Fargate Profiles support 3 minor version skew betweeen control plane and data plane. For example, if your EKS control plane version is 1.28, you can safely use kubelet versions as old as 1.25. If your EKS version is 1.27, the oldest kubelet version you can use is 1.25.
5. [Upgrade
   the cluster control plane using the AWS console or cli.](../userguide/update-cluster.md "../userguide/update-cluster.md")
6. [Review
   add-on compatibility.](#upgrade-addons "#upgrade-addons") Upgrade your Kubernetes add-ons and custom
   controllers, as required.
7. [Update
   kubectl.](../userguide/install-kubectl.md "../userguide/install-kubectl.md")
8. [Upgrade
   the cluster data plane.](../userguide/update-managed-node-group.md "../userguide/update-managed-node-group.md") Upgrade your nodes to the same Kubernetes minor
...
The EKS Kubernetes
[version
documentation](../userguide/kubernetes-versions.md "../userguide/kubernetes-versions.md") includes a detailed list of changes for each version.
...
- [Review release notes for Kubernetes versions on standard support](../userguide/kubernetes-versions-standard.md "../userguide/kubernetes-versions-standard.md")
- [Review release notes for Kubernetes versions on extended support](../userguide/kubernetes-versions-extended.md "../userguide/kubernetes-versions-extended.md")
...
(e.g. [EBS
CSI](../userguide/ebs-csi.md "../userguide/ebs-csi.md"), [EFS
CSI](../userguide/efs-csi.md "../userguide/efs-csi.md")), ingress controllers, and any other workloads or add-ons that rely
on the Kubernetes API directly.

###### Tip

Critical cluster components are often installed in a
`*-system` namespace
...
For example, see the
[AWS
Load Balancer Controller](https://kubernetes-sigs.github.io/aws-load-balancer-controller/v2.4/deploy/installation/ "https://kubernetes-sigs.github.io/aws-load-balancer-controller/v2.4/deploy/installation/") documentation for version compatibility. Some
...
include [CoreDNS](https://github.com/coredns/coredns "https://github.com/coredns/coredns"),
[kube-proxy](https://kubernetes.io/docs/concepts/overview/components/#kube-proxy "https://kubernetes.io/docs/concepts/overview/components/#kube-proxy"),
[VPC CNI](https://github.com/aws/amazon-vpc-cni-k8s "https://github.com/aws/amazon-vpc-cni-k8s"), and storage drivers.
...
- **Amazon VPC CNI:** For the recommended version of the Amazon VPC CNI
  ...
  [Updating
  the Amazon VPC CNI plugin for Kubernetes self-managed add-on](../userguide/managing-vpc-cni.md "../userguide/managing-vpc-cni.md"). **When
  installed as an Amazon EKS Add-on, it can only be upgraded one minor
  version at a time.**
- **kube-proxy:** See
  [Updating
  the Kubernetes kube-proxy self-managed add-on](../userguide/managing-kube-proxy.md "../userguide/managing-kube-proxy.md").
- **CoreDNS:** See
  [Updating
  the CoreDNS self-managed add-on](../userguide/managing-coredns.md "../userguide/managing-coredns.md").
- **AWS Load Balancer Controller:** The AWS Load Balancer Controller needs
  ...
  [installation
  guide](../userguide/aws-load-balancer-controller.md "../userguide/aws-load-balancer-controller.md") for more information.
- **Amazon Elastic Block Store (Amazon EBS) Container Storage Interface
  (CSI) driver:** For installation and upgrade information, see
  [Managing
  the Amazon EBS CSI driver as an Amazon EKS add-on](../userguide/managing-ebs-csi.md "../userguide/managing-ebs-csi.md").
- **Amazon Elastic File System (Amazon EFS) Container Storage Interface
  (CSI) driver:** For installation and upgrade information, see
  [Amazon EFS
  CSI driver](../userguide/efs-csi.md "../userguide/efs-csi.md").
- **Kubernetes Metrics Server:** For more information, see
  [metrics-server](https://kubernetes-sigs.github.io/metrics-server/ "https://kubernetes-sigs.github.io/metrics-server/") on
  GitHub.
- **Kubernetes Cluster Autoscaler:** To upgrade the version of Kubernetes
  ...
  the [GitHub releases](https://github.com/kubernetes/autoscaler/releases "https://github.com/kubernetes/autoscaler/releases")
...
- **Karpenter:** For installation and upgrade information, see the
  [Karpenter documentation.](https://karpenter.sh/docs/upgrading/ "https://karpenter.sh/docs/upgrading/")
...
1. Available IP addresses: Amazon EKS requires up to five available IP
...
2. EKS IAM role: The control plane IAM role is still present in the
   account with the necessary permissions.
3. If your cluster has secret encryption enabled, then make sure that the
