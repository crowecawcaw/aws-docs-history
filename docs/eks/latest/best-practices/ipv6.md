...
EKS in IPv6 mode solves the IPv4 exhaustion challenge often manifested in large scale EKS clusters. EKS’s support for IPv6 is focused on resolving the IPv4 exhaustion problem, which stems from the limited size of the IPv4 address space. This is a significant concern raised by a number of our customers and is distinct from Kubernetes [IPv4/IPv6 dual-stack](https://kubernetes.io/docs/concepts/services-networking/dual-stack/ "https://kubernetes.io/docs/concepts/services-networking/dual-stack/") feature.
...
![Dual Stack VPC](images/networking/ipv6_eks-ipv6-foundation.png)
In the IPv6 world, every address is internet routable. By default, VPC allocates IPv6 CIDR from the public GUA range. However since [August 2024](https://aws.amazon.com/about-aws/whats-new/2024/08/aws-private-ipv6-addressing-vpcs-subnets/ "https://aws.amazon.com/about-aws/whats-new/2024/08/aws-private-ipv6-addressing-vpcs-subnets/") you can also use private IPv6 addressing for VPCs and subnets with Amazon VPC IP Address Manager (IPAM). Please see the [this AWS Networking blog post](https://aws.amazon.com/blogs/networking-and-content-delivery/understanding-ipv6-addressing-on-aws-and-designing-a-scalable-addressing-plan "https://aws.amazon.com/blogs/networking-and-content-delivery/understanding-ipv6-addressing-on-aws-and-designing-a-scalable-addressing-plan") and [VPC documentation](../../../vpc/latest/userguide/vpc-ip-addressing.md#vpc-ipv6-addresses "../../../vpc/latest/userguide/vpc-ip-addressing.md#vpc-ipv6-addresses") for more information.

The following diagram depict a Pod IPv6 Internet egress flow inside an EKS/IPv6 cluster:

![Dual Stack VPC](images/networking/ipv6_eks-egress-ipv6.png)
Best practices for implementing IPv6 subnets can be found in the [VPC user guide](../../../whitepapers/latest/ipv6-on-aws/IPv6-on-AWS.md "../../../whitepapers/latest/ipv6-on-aws/IPv6-on-AWS.md").
...
![Dual Stack VPC](images/networking/ipv6_eks-cluster-ipv6-foundation.png)

## Overview

EKS/IPv6 is only supported in prefix mode (VPC-CNI Plug-in ENI IP assign mode). Learn more on [Prefix Mode](prefix-mode-linux.md "prefix-mode-linux.md").
...
![illustration of worker subnet](images/networking/ipv6_image-2.png)
...
![EKS/IPv6](images/networking/ipv6_eks-ipv4-snat-cni.png)
...
![EKS/IPv6](images/networking/ipv6_eks-ipv4-snat-cni-internet.png)
...
Kubernetes services will receive only IPv6 addresses (ClusterIP) from Unique [Local IPv6 Unicast Addresses (ULA)](https://datatracker.ietf.org/doc/html/rfc4193 "https://datatracker.ietf.org/doc/html/rfc4193"). The ULA Service CIDR for an IPv6 cluster is automatically assigned during EKS cluster creation stage and cannot be modified. The following diagram depict the Pod to Kubernetes Service flow:

![EKS/IPv6](images/networking/ipv6_Pod-to-service-ipv6.png)
...
![Internet IPv4 user to EKS/IPv6 Ingress service](images/networking/ipv6_ipv4-internet-to-eks-ipv6.png)

###### Note

The above pattern requires to deploy the [most recent version](https://kubernetes-sigs.github.io/aws-load-balancer-controller "https://kubernetes-sigs.github.io/aws-load-balancer-controller") of the AWS load balancer controller
...
![illustration of cluster including X-ENIs](images/networking/ipv6_image-5.png)
...
If [custom networking](custom-networking.md "custom-networking.md") is currently enabled, Amazon EKS recommends re-evaluating your need for it with IPv6. If you chose to use custom networking to address the IPv4 exhaustion issue, it is no longer necessary with IPv6. If you are utilizing custom networking to satisfy a security requirement, such as a separate network for nodes and Pods, you are encouraged to submit an [EKS roadmap request](https://github.com/aws/containers-roadmap/issues "https://github.com/aws/containers-roadmap/issues").
