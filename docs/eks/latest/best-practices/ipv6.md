...
EKS/IPv6 is only supported in prefix mode (VPC-CNI Plug-in ENI IP assign mode). Learn more on [Prefix Mode](prefix-mode-linux.md).
...
**Note**  
The above pattern requires to deploy the [most recent version](https://kubernetes-sigs.github.io/aws-load-balancer-controller) of the AWS load balancer controller
