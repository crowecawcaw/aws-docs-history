...
![flow chart of procedure for assigning IP to pod](images/networking/pm_windows-2.jpg)
...
Use prefix delegation if you are experiencing Pod density issues on the worker nodes. To avoid errors, we recommend examining the subnets for contiguous block of addresses for /28 prefix before migrating to prefix mode. Please refer "
[Use Subnet Reservations to Avoid Subnet Fragmentation (IPv4)](../../../vpc/latest/userguide/subnet-cidr-reservation.md "../../../vpc/latest/userguide/subnet-cidr-reservation.md")
" section for Subnet reservation details.
...
For more details about the bootstrap configuration parameters for Windows nodes, please visit the documentation [here](../userguide/eks-optimized-windows-ami.md#bootstrap-script-configuration-parameters "../userguide/eks-optimized-windows-ami.md#bootstrap-script-configuration-parameters").
...
By fine tuning these configuration parameters, you can achieve an optimal balance of conserving the IP addresses and ensuring decreased Pod latency due to assignment of IP address. For more information about these configuration parameters, visit the documentation [here](https://github.com/aws/amazon-vpc-resource-controller-k8s/blob/master/docs/windows/prefix_delegation_config_options.md "https://github.com/aws/amazon-vpc-resource-controller-k8s/blob/master/docs/windows/prefix_delegation_config_options.md").
...
- Cordon and drain all the existing nodes to safely evict all of your existing Pods. To prevent service disruptions, we suggest implementing [Pod Disruption Budgets](https://kubernetes.io/docs/tasks/run-application/configure-pdb "https://kubernetes.io/docs/tasks/run-application/configure-pdb") on your production clusters for critical workloads.
...
- Enable/Disable the Prefix Delegation feature for Windows
- Update the node group using the steps mentioned [here](../userguide/update-managed-node-group.md "../userguide/update-managed-node-group.md"). This performs similar steps as above but are managed by EKS.
...
You can use our debugging guide [here](https://github.com/aws/amazon-vpc-resource-controller-k8s/blob/master/docs/troubleshooting.md "https://github.com/aws/amazon-vpc-resource-controller-k8s/blob/master/docs/troubleshooting.md") to deep dive into the issue you are facing with prefix delegation on Windows.
