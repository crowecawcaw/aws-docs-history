...
continuously try to
[match
the current state with the desired state](https://kubernetes.io/docs/concepts/architecture/controller/#desired-vs-current "https://kubernetes.io/docs/concepts/architecture/controller/#desired-vs-current").

## Recommendations

### Configure Pod Disruption Budgets

[Pod Disruption Budgets](https://kubernetes.io/docs/tasks/run-application/configure-pdb/ "https://kubernetes.io/docs/tasks/run-application/configure-pdb/") are
...
Deployment
[controller](https://kubernetes.io/docs/concepts/architecture/controller/ "https://kubernetes.io/docs/concepts/architecture/controller/")
...
can use the
[Horizontal
Pod Autoscaler](https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/ "https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/") to scale replicas automatically based on workload
demand.
...
1. The `maxSkew` is
   ...
   `0,0,10` across 3 AZs.
2. The `topologyKey` is a key for one of the
...
3. The `whenUnsatisfiable` property is used to determine how you want
   the scheduler to respond if the desired constraints can’t be satisfied.
4. The `labelSelector` is used to find matching pods so that the
...
[Kubernetes
documentation](https://kubernetes.io/docs/concepts/scheduling-eviction/topology-spread-constraints/ "https://kubernetes.io/docs/concepts/scheduling-eviction/topology-spread-constraints/").

###### Pod topology spread constraints across 3 AZs

![Pod topology spread constraints across 3 AZs](images/reliability/pod-topology-spread-constraints.jpg)
...
[metrics server](https://github.com/kubernetes-sigs/metrics-server "https://github.com/kubernetes-sigs/metrics-server") to
...
and
[VPA](https://github.com/kubernetes/autoscaler/tree/master/vertical-pod-autoscaler "https://github.com/kubernetes/autoscaler/tree/master/vertical-pod-autoscaler")
...
[Metrics API format](https://github.com/kubernetes/metrics "https://github.com/kubernetes/metrics").
...
Follow the
[EKS
documentation](../userguide/metrics-server.md "../userguide/metrics-server.md") to install metrics-server in your EKS cluster.
...
[Custom
Metrics](https://github.com/kubernetes-sigs/custom-metrics-apiserver "https://github.com/kubernetes-sigs/custom-metrics-apiserver") API servers provide the `custom-metrics.k8s.io` API that HPA
can use to autoscale applications.

You can use the
[Prometheus
Adapter for Kubernetes Metrics APIs](https://github.com/directxman12/k8s-prometheus-adapter "https://github.com/directxman12/k8s-prometheus-adapter") to collect metrics from Prometheus
and use with the HPA. In this case, Prometheus adapter will expose
Prometheus metrics in
[Metrics
API format](https://github.com/kubernetes/metrics/blob/master/pkg/apis/metrics/types.go "https://github.com/kubernetes/metrics/blob/master/pkg/apis/metrics/types.go").
...
To autoscale Kubernetes workloads you can use KEDA (Kubernetes Event-driven Autoscaling), an open-source project that can drive container scaling based on a number of custom events. This [AWS blog](https://aws.amazon.com/blogs/mt/autoscaling-kubernetes-workloads-with-keda-using-amazon-managed-service-for-prometheus-metrics/ "https://aws.amazon.com/blogs/mt/autoscaling-kubernetes-workloads-with-keda-using-amazon-managed-service-for-prometheus-metrics/") outlines how to use Amazon Managed Service for Prometheus for Kubernetes workload auto-scaling.
...
[EKS
Documentation](../userguide/vertical-pod-autoscaler.md "../userguide/vertical-pod-autoscaler.md") includes a walkthrough for setting up VPA.

[Fairwinds Goldilocks](https://github.com/FairwindsOps/goldilocks/ "https://github.com/FairwindsOps/goldilocks/")
...
[rolling
update](https://kubernetes.io/docs/tutorials/kubernetes-basics/update/update-intro/ "https://kubernetes.io/docs/tutorials/kubernetes-basics/update/update-intro/"). In other words, Kubernetes will only update a portion of the
...
[`Max Unavailable`](https://kubernetes.io/docs/concepts/workloads/controllers/deployment/#max-unavailable "https://kubernetes.io/docs/concepts/workloads/controllers/deployment/#max-unavailable")
...
Many continuous integration tools such as [Flux](https://fluxcd.io "https://fluxcd.io"),
[Jenkins](https://www.jenkins.io "https://www.jenkins.io"), and [Spinnaker](https://spinnaker.io "https://spinnaker.io") let
...
deployment and A/B testing](https://aws.amazon.com/blogs/containers/using-aws-load-balancer-controller-for-blue-green-deployment-canary-deployment-and-a-b-testing/ "https://aws.amazon.com/blogs/containers/using-aws-load-balancer-controller-for-blue-green-deployment-canary-deployment-and-a-b-testing/")
...
you can use tools such as [Flagger](https://github.com/weaveworks/flagger "https://github.com/weaveworks/flagger")
with
[Istio](https://docs.flagger.app/tutorials/istio-progressive-delivery "https://docs.flagger.app/tutorials/istio-progressive-delivery").
...
[health-checks](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/ "https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/"):
...
Sandor Szücs’s post
[LIVENESS
PROBES ARE DANGEROUS](https://srcco.de/posts/kubernetes-liveness-probes-are-dangerous.html "https://srcco.de/posts/kubernetes-liveness-probes-are-dangerous.html") describes problems that can be caused by
...
[Fantastic
Probes And How To Configure Them](https://medium.com/swlh/fantastic-probes-and-how-to-configure-them-fef7e030bd2f "https://medium.com/swlh/fantastic-probes-and-how-to-configure-them-fef7e030bd2f"), Startup Probes should be used when
...
the
[`ENTRYPOINT`](https://docs.docker.com/engine/reference/builder/#entrypoint "https://docs.docker.com/engine/reference/builder/#entrypoint")
of the container to launch the Python application. Alternatively, you
can use a tool like [dumb-init](https://github.com/Yelp/dumb-init "https://github.com/Yelp/dumb-init") to
ensure that your application can handle signals.

You can also use
[Container
hooks](https://kubernetes.io/docs/concepts/containers/container-lifecycle-hooks/#container-hooks "https://kubernetes.io/docs/concepts/containers/container-lifecycle-hooks/#container-hooks") to execute a script or an HTTP request at container start or
...
[nodes
are drained with a fifteen-minute timeout](../userguide/managed-node-update-behavior.md "../userguide/managed-node-update-behavior.md"). After fifteen minutes, if
...
Handler](https://github.com/aws/aws-node-termination-handler "https://github.com/aws/aws-node-termination-handler"), which ensures that the Kubernetes control plane responds
appropriately to events that can cause your EC2 instance to become
unavailable, such as
[EC2
maintenance](../../../AWSEC2/latest/UserGuide/monitoring-instances-status-check_sched.md "../../../AWSEC2/latest/UserGuide/monitoring-instances-status-check_sched.md") events and
[EC2
Spot interruptions](../../../AWSEC2/latest/UserGuide/spot-interruptions.md "../../../AWSEC2/latest/UserGuide/spot-interruptions.md"). It uses the Kubernetes API to cordon the node to
...
[Kubernetes
is a declarative system](https://medium.com/@dominik.tornow/the-mechanics-of-kubernetes-ac8112eaa302 "https://medium.com/@dominik.tornow/the-mechanics-of-kubernetes-ac8112eaa302") where "
_the user supplies a representation of
...
commands to transition from the current state to the desired state._
"
...
`replica` crashes, the
[Deployment
Contoller](https://kubernetes.io/docs/concepts/architecture/controller/#design "https://kubernetes.io/docs/concepts/architecture/controller/#design") will create a new `replica`. In this way, Kubernetes
controllers automatically fix failures.

Chaos engineering tools like [Gremlin](https://www.gremlin.com "https://www.gremlin.com") help you
...
- [LinkerD](http://linkerd.io "http://linkerd.io")
...
[Prometheus](https://prometheus.io "https://prometheus.io") or
[CloudWatch
Container Insights](../../../AmazonCloudWatch/latest/monitoring/ContainerInsights.md "../../../AmazonCloudWatch/latest/monitoring/ContainerInsights.md") can monitor your cluster and workload and provide
...
Sysdig’s post [Best
practices for alerting on Kubernetes](https://sysdig.com/blog/alerting-kubernetes/ "https://sysdig.com/blog/alerting-kubernetes/") includes a comprehensive list of
...
[Prometheus client
library](https://prometheus.io/docs/instrumenting/clientlibs/ "https://prometheus.io/docs/instrumenting/clientlibs/") to expose application-specific custom metrics to improve the
...
[EKS
provide five types of control plane logs](../userguide/control-plane-logs.md "../userguide/control-plane-logs.md"):
...
like [Fluent Bit](http://fluentbit.io "http://fluentbit.io"), [Fluentd](https://www.fluentd.org "https://www.fluentd.org"),
or
[CloudWatch
Container Insights](../../../AmazonCloudWatch/latest/monitoring/deploy-container-insights-EKS.md "../../../AmazonCloudWatch/latest/monitoring/deploy-container-insights-EKS.md") in your cluster.
...
Service Meshes like [LinkerD](http://linkerd.io "http://linkerd.io") and [Istio](http://istio.io "http://istio.io") can be used to
...
Tracing tools like [AWS X-Ray](https://aws.amazon.com/xray/ "https://aws.amazon.com/xray/"),
[Jaeger](https://www.jaegertracing.io "https://www.jaegertracing.io") support both shared library and
service mesh implementations.

Consider using a tracing tool like [AWS
X-Ray](https://aws.amazon.com/xray/ "https://aws.amazon.com/xray/") or [Jaeger](https://www.jaegertracing.io "https://www.jaegertracing.io") that supports both
