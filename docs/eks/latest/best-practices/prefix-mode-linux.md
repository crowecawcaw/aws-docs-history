...
![flow chart of procedure for assigning IP to pod](images/networking/pm_image-2.jpeg)

## Recommendations

### Use Prefix Mode when

Use prefix mode if you are experiencing Pod density issue on the worker nodes. To avoid VPC CNI errors, we recommend examining the subnets for contiguous block of addresses for /28 prefix before migrate to prefix mode. Please refer "
...
For backward compatibility, the [max-pods](https://github.com/aws/amazon-vpc-cni-k8s/blob/master/misc/eni-max-pods.txt "https://github.com/aws/amazon-vpc-cni-k8s/blob/master/misc/eni-max-pods.txt") limit is set to support secondary IP mode. To increase the pod density, specify the `max-pods` value to Kubelet and `--use-max-pods=false` as the user data for the nodes. For more information, see [How maxPods is determined](../userguide/choosing-instance-type.md#max-pods-precedence "../userguide/choosing-instance-type.md#max-pods-precedence") in the Amazon EKS User Guide. Refer to the EKS [user guide](../userguide/cni-increase-ip-addresses.md "../userguide/cni-increase-ip-addresses.md") for example user data.

```
./max-pods-calculator.sh --instance-type m5.large --cni-version ``1.9``.0 --cni-prefix-delegation-enabled
```

Prefix assignment mode is especially relevant for users of [CNI custom networking](../userguide/cni-custom-network.md "../userguide/cni-custom-network.md") where the primary ENI is not used for pods. With prefix assignment, you can still attach more IPs on nearly every Nitro instance type, even without the primary ENI used for pods.
