...
1.  **Latency and throughput SLOs**: What are your p50/p95 targets and peak request rates?

1.  **Workload type**: Online inference, batch scoring, retrieval, or orchestration?

1.  **Cost and capacity constraints**: FinOps budget, regional availability, reservation strategy?
...
+ SLM inference service — quantized 1-8B model using an inference engine such as [llama.cpp](https://github.com/ggerganov/llama.cpp), [Ollama](https://ollama.com), or [vLLM](https://docs.vllm.ai) on CPU
...
This pattern also fits a fine-tuning lifecycle: collect domain data on CPU nodes, fine-tune on GPU, then deploy the quantized model back to CPU for inference at substantially lower cost than an LLM endpoint. Research from LoRA Land ([arXiv:2405.00732](https://arxiv.org/abs/2405.00732)) shows that fine-tuned 7B models outperform GPT-4 on the majority of domain-specific tasks tested, making the "fine-tune a small model and run it on CPU" path viable for many production use cases.
...
We recommend using [Bottlerocket](https://bottlerocket.dev) as the node OS for inference workloads, combined with the [SOCI snapshotter](https://github.com/awslabs/soci-snapshotter) in parallel pull/unpack mode. SOCI replaces the default sequential layer extraction with parallel chunk-based downloads, significantly reducing image pull times for large images. No changes to your container images are needed.

For detailed configuration guidance, see the [Performance](aiml-performance.md) section of this guide, which covers SOCI configuration, EBS snapshot pre-pulling, and container runtime cache strategies.
...
1.  **Establish Baseline** — Run the dataset against a trusted model (e.g., a large LLM you know produces correct results).

1.  **Test CPU Model** — Run the same dataset on your quantized SLM and compare.

1.  **Evaluate** — Define your quality threshold before testing, for example, "SLM accuracy within 5 percentage points of baseline." The right threshold depends on the task: a classifier reviewed by humans can tolerate more errors than a system making automatic decisions.
...
+  **Fine-tune the model on your domain**: The most expensive option, but the most effective. Research from LoRA Land ([arXiv:2405.00732](https://arxiv.org/abs/2405.00732)) found that fine-tuned 7B models outperform GPT-4 on the majority of domain-specific tasks tested.
...
+  [Small Language Models for Agentic AI (NVIDIA)](https://arxiv.org/abs/2506.02153) 
