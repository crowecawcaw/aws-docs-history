...
2. **Latency and throughput SLOs**: What are your p50/p95 targets and peak request rates?
3. **Workload type**: Online inference, batch scoring, retrieval, or orchestration?
...
- SLM inference service — quantized 1-8B model using an inference engine such as [llama.cpp](https://github.com/ggerganov/llama.cpp "https://github.com/ggerganov/llama.cpp"), [Ollama](https://ollama.com "https://ollama.com"), or [vLLM](https://docs.vllm.ai "https://docs.vllm.ai") on CPU
...
This pattern also fits a fine-tuning lifecycle: collect domain data on CPU nodes, fine-tune on GPU, then deploy the quantized model back to CPU for inference at substantially lower cost than an LLM endpoint. Research from LoRA Land ([arXiv:2405.00732](https://arxiv.org/abs/2405.00732 "https://arxiv.org/abs/2405.00732")) shows that fine-tuned 7B models outperform GPT-4 on the majority of domain-specific tasks tested, making the "fine-tune a small model and run it on CPU" path viable for many production use cases.
...
We recommend using [Bottlerocket](https://bottlerocket.dev "https://bottlerocket.dev") as the node OS for inference workloads, combined with the [SOCI snapshotter](https://github.com/awslabs/soci-snapshotter "https://github.com/awslabs/soci-snapshotter") in parallel pull/unpack mode. SOCI replaces the default sequential layer extraction with parallel chunk-based downloads, significantly reducing image pull times for large images. No changes to your container images are needed.

For detailed configuration guidance, see the [Performance](aiml-performance.md "aiml-performance.md") section of this guide, which covers SOCI configuration, EBS snapshot pre-pulling, and container runtime cache strategies.
...
1. **Build Eval Dataset** — Build a small evaluation dataset (100-300 labeled examples) from your actual workload. Avoid generic benchmarks like MMLU that measure general reasoning rather than your real task.
2. **Establish Baseline** — Run the dataset against a trusted model (e.g., a large LLM you know produces correct results).
3. **Test CPU Model** — Run the same dataset on your quantized SLM and compare.
4. **Evaluate** — Define your quality threshold before testing, for example, "SLM accuracy within 5 percentage points of baseline." The right threshold depends on the task: a classifier reviewed by humans can tolerate more errors than a system making automatic decisions.
...
- **Fine-tune the model on your domain**: The most expensive option, but the most effective. Research from LoRA Land ([arXiv:2405.00732](https://arxiv.org/abs/2405.00732 "https://arxiv.org/abs/2405.00732")) found that fine-tuned 7B models outperform GPT-4 on the majority of domain-specific tasks tested.

## References

- [Amazon EKS Documentation](../../../eks.md "../../../eks.md")
- [AWS Graviton Developer Guide](https://github.com/aws/aws-graviton-getting-started "https://github.com/aws/aws-graviton-getting-started")
...
- [Ray Serve Documentation](https://docs.ray.io/en/latest/serve/ "https://docs.ray.io/en/latest/serve/")
- [Amazon OpenSearch Service](../../../opensearch-service.md "../../../opensearch-service.md")
- [AWS Spot Best Practices](../../../AWSEC2/latest/UserGuide/spot-best-practices.md "../../../AWSEC2/latest/UserGuide/spot-best-practices.md")
- [LoRA Land: Fine-Tuned Open-Source LLMs that Outperform GPT-4](https://arxiv.org/abs/2405.00732 "https://arxiv.org/abs/2405.00732")
- [Model Context Protocol (MCP) Specification](https://modelcontextprotocol.io "https://modelcontextprotocol.io")
- [Small Language Models for Agentic AI (NVIDIA)](https://arxiv.org/abs/2506.02153 "https://arxiv.org/abs/2506.02153")
- [SOCI Snapshotter](https://github.com/awslabs/soci-snapshotter "https://github.com/awslabs/soci-snapshotter")
- [Bottlerocket OS](https://bottlerocket.dev "https://bottlerocket.dev")
- [AI on EKS: Container Startup Optimization](https://awslabs.github.io/ai-on-eks/docs/guidance/container-startup-time/accelerate-pull-process "https://awslabs.github.io/ai-on-eks/docs/guidance/container-startup-time/accelerate-pull-process")
