...
Quotas are used to define limits on workloads hosted in your cluster. With quotas, you can limit the total amount of CPU and memory that can be consumed within a namespace, or limit the number of objects that can be created. **Limit ranges** allow you to declare the minimum, maximum, and default CPU and memory values for individual pods and containers within a namespace.
...
Sandboxing is a technique by which each container is run in its own isolated virtual machine. Technologies that perform pod sandboxing include [Firecracker](https://firecracker-microvm.github.io/).
