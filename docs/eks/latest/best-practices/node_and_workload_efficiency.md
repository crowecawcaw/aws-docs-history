...
Using node sizes that are slightly larger (4-12xlarge) increases the available space that we have for running pods due to the fact it reduces the percentage of the node used for "overhead" such as [DaemonSets](https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/ "https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/") and [Reserves](https://kubernetes.io/docs/tasks/administer-cluster/reserve-compute-resources/ "https://kubernetes.io/docs/tasks/administer-cluster/reserve-compute-resources/") for system components. In the diagram below we see the difference between the usable space on a 2xlarge vs. a 8xlarge system with just a moderate number of DaemonSets.
...
![Node size](images/scalability/node-size.png)
...
![Churn rate](images/scalability/churn-rate.png)
...
![cores 1](images/scalability/cores-1.png)
...
![cores 2](images/scalability/cores-2.png)
...
![cores 3](images/scalability/cores-3.png)
...
![The sweet spot](images/scalability/sweet-spot.png)
...
![scaling ratio](images/scalability/scaling-ratio.png)
...
![bad sweetspot](images/scalability/bad-sweetspot.png)
...
![hpa utilization](images/scalability/hpa-utilization.png)
...
![requests 1](images/scalability/requests-1.png)
...
![requests 2](images/scalability/requests-2.png)
...
![util vs saturation 1](images/scalability/util-vs-saturation-1.png)
...
![util vs saturation 2](images/scalability/util-vs-saturation-2.png)
...
![node saturation](images/scalability/node-saturation.png)
...
![stalled io](images/scalability/stalled-io.png)
...
![thread pool](images/scalability/thread-pool.png)
...
![spiky scaling](images/scalability/spiky-scaling.png)
...
![smooth scaling](images/scalability/smooth-scaling.png)
...
![CPU limits](images/scalability/cpu-limits.png)
...
![cpu 1](images/scalability/cpu-1.png)
...
![cpu 2](images/scalability/cpu-2.png)

### Memory

The memory allocation is another example where it is easy to confuse Kubernetes scheduling behavior for Linux CGroup behavior. This is a more nuanced topic as there have been major changes in the way that CGroup v2 handles memory in Linux and Kubernetes has changed its syntax to reflect this; read this [blog](https://kubernetes.io/blog/2021/11/26/qos-memory-resources/ "https://kubernetes.io/blog/2021/11/26/qos-memory-resources/") for further details.
...
Having the ability to "compress" memory was one of the primary drivers behind CGroup v2. For more history on why CGroup V2 was necessary, please see Chris Down’s [presentation](https://www.youtube.com/watch?v=kPMZYoRxtmg "https://www.youtube.com/watch?v=kPMZYoRxtmg") at LISA21 where he covers why being unable to set the minimum memory correctly was one of the reasons that drove him to create CGroup v2 and pressure stall metrics.
...
The key question becomes, what value to set `memory.min` to? This is where memory pressure stall metrics come into play. We can use these metrics to detect memory "thrashing" at a container level. Then we can use controllers such as [fbtax](https://facebookmicrosites.github.io/cgroup2/docs/fbtax-results.html "https://facebookmicrosites.github.io/cgroup2/docs/fbtax-results.html") to detect the correct values for `memory.min` by looking for this memory thrashing, and dynamically set the `memory.min` value to this setting.
