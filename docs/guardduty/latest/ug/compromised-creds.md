...
When GuardDuty generates [IAM finding types](guardduty_finding-types-iam.md), it indicates that your AWS credentials have been compromised. The potentially compromised **Resource** type is **AccessKey**. 
...
This type of key is a long term customer-managed credential associated with an IAM user or AWS account root user. For information about managing access keys for IAM users, see [Managing access keys for IAM users](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html).  
