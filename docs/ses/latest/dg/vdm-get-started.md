...
+ **Optimized shared delivery** – Automatically chooses the optimal IP to use when sending emails, improving end-point delivery of messages to the target email recipients. 
...
You can also enable global deliverability from the Virtual Deliverability Manager Settings page to extend your visibility with analytics across all providers. See [Getting started with global deliverability](vdm-gd-get-started.md).
