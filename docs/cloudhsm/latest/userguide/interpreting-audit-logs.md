...
**Opcode**  
Identifies the management command that was executed. For a list of `Opcode` values in the AWS CloudHSM audit logs, see [AWS CloudHSM audit log reference](cloudhsm-audit-log-reference.md).
...
The third event in this example log stream (`Sequence No 0x2`) is the creation of the [appliance user (AU)](understanding-users-cmu.md#appliance-user-cmu), which is the AWS CloudHSM service. Events that involve HSM users include extra fields for the user name and user type. 
