 ...
 *if the cluster in that region was originally created from the backup
 of a cluster in another region*. Let's say you copied a cluster to another region
...
2. In the renamed copy, add the Elastic Network Interface (ENI) IP of the destination HSM
 ...
 destination HSM *below* the source HSM. 
...
For more information about how to get the IP address, see [Get an IP address for an HSM](#get-ip-clone-cluster "#get-ip-clone-cluster").
