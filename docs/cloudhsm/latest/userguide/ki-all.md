...
+ [Issue: There was an upper limit of 16 KB on data that can be hashed and signed by AWS CloudHSM using Client SDK 3](#ki-all-3)
+ [Issue: Imported keys could not be specified as non-exportable](#ki-all-4)
...
+ [Issue: You receive the error "Failed to create cert/key from provided pfx file. Error: NotPkcs8"](#ki-all-11)
+ [Issue: ECDSA signing fails with "invalid mechanism" error starting with SDK 5.16](#ki-all-12)
+ [Issue: Signing operations with prehashed data do not properly clear session tokens in interactive mode](#ki-all-13)
