...
The first command uses the `-sess` parameter of [genSymKey](key_mgmt_util-genSymKey.md "key_mgmt_util-genSymKey.md") to create a 192-bit AES key that is valid
...
This command uses [findKey](key_mgmt_util-findKey.md "key_mgmt_util-findKey.md") to find the session
...
Specifies the constant that represents the attribute that you want to change. The
 only valid value is `1`, which represents the token attribute,
 `OBJ_ATTR_TOKEN`.


To get the attributes and their integer values, use [listAttributes](key_mgmt_util-listAttributes.md "key_mgmt_util-listAttributes.md").
