...
Use the **key generate-symmetric generic-secret** command in CloudHSM CLI to generate a
...
```
`aws-cloudhsm >` `key help generate-symmetric generic-secret``Generate a generic secret key
