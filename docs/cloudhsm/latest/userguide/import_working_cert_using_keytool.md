# Import a working certificate into AWS CloudHSM
 key store using keytool
...
The alias should be a key pair with an associated certificate in the key store. If
 ...
 For instructions on importing the certificate metadata, see the code sample in [Registering Pre-existing Keys
 with AWS CloudHSM Key Store](register-pre-existing-keys-with-keystore.md "register-pre-existing-keys-with-keystore.md"). 
