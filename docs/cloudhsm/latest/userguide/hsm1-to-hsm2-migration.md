...
1. AWS CloudHSM repeats steps 2 and 3 for each HSM in the cluster, migrating one HSM at a time.
...
If your cluster has had no SDK or client connections for the past 14 days, your cluster is exempt from the 7-day SDK connection requirement.
...
**Warning**  
Be cautious about rolling back after a migration is complete. If you complete a migration and then use AWS CloudHSM to create new keys or users, rolling back can result in data loss. For example, a key created in hsm2m.medium might not be available in hsm1.medium when the rollback has completed. See [Synchronizing Data After a Rollback](#cluster-rollback-datasync) to learn how to mitigate data loss after a rollback.
