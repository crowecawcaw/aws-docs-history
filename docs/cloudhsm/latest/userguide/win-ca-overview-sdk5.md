...
**Topics**
+ [Step 1: Set up the prerequisites](#win-ca-prerequisites-sdk5)
+ [Step 2: Create a Windows Server CA with AWS CloudHSM](#win-ca-setup-sdk5)
+ [Step 3: Sign a certificate signing request (CSR) with your Windows Server CA with AWS CloudHSM](#win-ca-sign-csr-sdk5)
...
1. In step 5, if you used Windows Credentials Manager to set HSM credentials, download [`psexec.exe`](https://live.sysinternals.com/psexec.exe) from SysInternals to run the following command as *NT Authority\\SYSTEM*:
...
1. For **Cryptography**, do the following:
   ...
   1. For **Select a cryptographic provider**, choose one of the **CloudHSM Key Storage Provider** options from the menu. These are the AWS CloudHSM key storage providers. For example, you can choose **RSA\#CloudHSM Key Storage Provider**.
