...
Use the **key unshare** command in CloudHSM CLI to unshare a key with other CUs
in your AWS CloudHSM cluster.

Only the CU who created the key and consequently owns it can unshare the key. Users with whom a key is shared can use the key in cryptographic operations,
...
```
`aws-cloudhsm >` `help key unshare``Unshare a key in the HSM cluster with another user
