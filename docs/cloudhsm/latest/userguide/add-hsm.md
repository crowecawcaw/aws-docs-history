...
automatically. 2. AWS CloudHSM makes a backup copy of an existing HSM in the cluster. For more information, see
[Backups](backups.md "backups.md"). 3. AWS CloudHSM restores the backup onto the new HSM. This ensures that the HSM is in sync with
the others in the cluster. 4. The existing HSMs in the cluster notify the AWS CloudHSM client that there's a new HSM in the
cluster. 5. The client establishes a connection to the new HSM.
...
- At a command prompt, issue the **[create-hsm](../../../cli/latest/reference/cloudhsmv2/create-hsm.md "../../../cli/latest/reference/cloudhsmv2/create-hsm.md")** command,
  specifying a cluster ID and an Availability Zone for the HSM that you are creating. If you
  don't know the cluster ID of your preferred cluster, issue the **[describe-clusters](../../../cli/latest/reference/cloudhsmv2/describe-clusters.md "../../../cli/latest/reference/cloudhsmv2/describe-clusters.md")**
