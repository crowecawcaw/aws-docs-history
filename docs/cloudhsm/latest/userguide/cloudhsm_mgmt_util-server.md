...
Use the **server** command in the AWS CloudHSM cloudhsm\_mgmt\_util to enter _server mode_ and interact directly with a particular hardware
security module (HSM) instance.

Normally, when you issue a command in cloudhsm\_mgmt\_util, the command effects all HSMs in the
...
will be returned to the cloudhsm\_mgmt\_util command prompt.
...
HSM. Server numbers are listed in the trace output generated by cloudhsm\_mgmt\_util upon initiation.
