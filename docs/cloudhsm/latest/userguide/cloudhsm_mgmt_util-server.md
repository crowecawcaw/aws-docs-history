...
Use the **server** command in the AWS CloudHSM cloudhsm\_mgmt\_util to enter *server mode* and interact directly with a particular hardware
...
Normally, when you issue a command in cloudhsm\_mgmt\_util, the command effects all HSMs in the
 designated cluster (*global mode)*. However, there may be
