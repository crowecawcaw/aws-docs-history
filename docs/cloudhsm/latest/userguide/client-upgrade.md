...
With AWS CloudHSM Client SDK 3.1 and higher, the version of the client daemon and any components
you install must match to upgrade. For all Linux-based systems, you must use a single command to
batch upgrade the client daemon with the same version of the PKCS #11 library, the Java Cryptographic Extension (JCE) provider, or
the OpenSSL Dynamic Engine. This requirement does not apply to Windows-based systems because the binaries for
the CNG and KSP providers are already included in the client daemon package.

- On a Red Hat-based Linux system (including Amazon Linux and CentOS), use the
  following command:
...
- [Prerequisites](#client-upgrade-prerequisites "#client-upgrade-prerequisites")
- [Step 1: Stop the client daemon](#client-stop "#client-stop")
- [Step 2: Upgrade the client SDK](#upgrade-commands "#upgrade-commands")
- [Step 3: Start the client daemon](#client-start "#client-start")
...
You do not have to install all the components. For every component you have installed, you
must upgrade that component to match the version of the client daemon.
...
The following command shows the syntax required to upgrade the client daemon and
components. Before you run the command, remove any components you don't intend to
upgrade.
