...
Use the **key unwrap aes-zero-pad** command in CloudHSM CLI to unwrap a payload
key into the AWS CloudHSM cluster using the AES wrapping key and the `AES-ZERO-PAD`
...
To use the **key unwrap aes-zero-pad** command, you must have the AES wrapping key in your AWS CloudHSM cluster, and its `unwrap` attribute must be set to `true`.
...
```
`aws-cloudhsm >` `help key unwrap aes-zero-pad``Usage: key unwrap aes-zero-pad [OPTIONS] --filter [`<FILTER>`...] --key-type-class `<KEY_TYPE_CLASS>` --label `<LABEL>` <--data-path `<DATA_PATH>`|--data `<DATA>`>
