...
The following limits apply to certificate storage. You can't change these limits. The
maximum number of stored certificates applies to each cluster. The read and write rate
limits apply to each HSM, so the total rate that a cluster supports increases as you add
HSMs.
...
- If you exceed the maximum number of stored certificates, the
  `C_CreateObject` operation returns
  `CKR_FUNCTION_FAILED`, and certificate storage records a
  `MaxObjectsReached` error in your certificate storage audit logs.
  Your existing certificates remain readable. To store additional certificates,
  delete certificates that you no longer need.
- If you exceed the read or write rate limit, certificate storage throttles the
  request and operations return `CKR_FUNCTION_FAILED`, except for
  `C_GetAttributeValue`, which returns
  `CKR_DEVICE_ERROR` (see
  [Issue: C\_GetAttributeValue returns CKR\_DEVICE\_ERROR when throttled](ki-pkcs11-sdk.md#ki-pkcs11-15 "ki-pkcs11-sdk.md#ki-pkcs11-15")). The PKCS #11 library
  does not retry throttled certificate storage operations (see
