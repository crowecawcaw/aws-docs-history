...
```
`aws-cloudhsm >` `help key wrap rsa-oaep``Usage: key wrap rsa-oaep [OPTIONS] --payload-filter [`<PAYLOAD_FILTER>`...] --wrapping-filter [`<WRAPPING_FILTER>`...] --hash-function `<HASH_FUNCTION>` --mgf `<MGF>`
