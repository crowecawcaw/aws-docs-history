...
**[sign](key_mgmt_util-sign.md "key_mgmt_util-sign.md")** operation.
...
signing mechanism to produce the `hardwareCertSigned` signed file. Because
...
produce the `userCertSigned` signed file. Because the given parameters do not
...
The handle of the public key that is thought to be used to sign the
file.
...
An integer that represents the proposed signing mechanism that is used to sign
