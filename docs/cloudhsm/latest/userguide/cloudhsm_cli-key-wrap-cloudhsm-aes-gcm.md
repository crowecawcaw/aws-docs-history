...
```
`aws-cloudhsm >` `help key wrap cloudhsm-aes-gcm``Usage: key wrap cloudhsm-aes-gcm [OPTIONS] --payload-filter [`<PAYLOAD_FILTER>`...] --wrapping-filter [`<WRAPPING_FILTER>`...] --tag-length-bits `<TAG_LENGTH_BITS>`
