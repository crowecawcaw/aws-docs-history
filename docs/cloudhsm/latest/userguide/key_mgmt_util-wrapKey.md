...
This command exports a 192-bit Triple DES (3DES) symmetric key (key handle
 ...
 `3DES-encrypted.key` file.

The output shows that key `7` (the 3DES key) was successfully wrapped and
 written to the specified file. The encrypted key is 307 bytes long.
...
The key handle of the key that you want to export. Enter the key handle of a symmetric or private key that you own. To find key handles, use the [findKey](key_mgmt_util-findKey.md "key_mgmt_util-findKey.md") command.
...
To verify that a key can be exported, use the [getAttribute](key_mgmt_util-getAttribute.md "key_mgmt_util-getAttribute.md") command to get the value of
 ...
 `354`. For help interpreting the key attributes, see the [AWS CloudHSM key attribute reference for KMU](key-attribute-table.md "key-attribute-table.md").


You can export only those keys that you own. To find the owner of a key, use the
 [getKeyInfo](key_mgmt_util-getKeyInfo.md "key_mgmt_util-getKeyInfo.md") command.
...
Specifies the wrapping key. Enter the key handle of an AES key or RSA key on the HSM. This
...
To create a wrapping key, use [genSymKey](key_mgmt_util-genSymKey.md "key_mgmt_util-genSymKey.md")
 to generate an AES key (type 31) or [genRSAKeyPair](key_mgmt_util-genRSAKeyPair.md "key_mgmt_util-genRSAKeyPair.md") to generate an RSA key pair (type 0). 
 If you are using an RSA key pair, be sure to wrap the key with one of the keys, and unwrap it with the other. To verify that a key can be used as a wrapping key, use [getAttribute](key_mgmt_util-getAttribute.md "key_mgmt_util-getAttribute.md") to get the value of the
...
| `AES_GCM` | 10 |
| `CLOUDHSM_AES_GCM` | 11 |
| `RSA_PKCS` (for maximum data size, see the note later in this section). See note [1](#wrapKey-params-note-1 "#wrapKey-params-note-1") below for an upcoming change. | 12 | Required: Yes ###### Note When using the `RSA_OAEP` wrapping mechanism, the maximum key size that you can wrap is determined by the modulus of the RSA key and the length of the specified hash as follows: Maximum key size = (modulusLengthInBytes-2\*hashLengthInBytes-2). When using the RSA\_PKCS wrapping mechanism, the maximum key size that you can wrap is determined by the modulus of the RSA key as follows: Maximum key size = (modulusLengthInBytes -11). **-t** The value representing the hash algorithm. CloudHSM supports the following algorithms:
