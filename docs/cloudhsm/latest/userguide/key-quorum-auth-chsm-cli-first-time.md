...
To use quorum authentication, each crypto-user must complete *all* of
 the following steps: 
...
**Required**: Yes
4. After all crypto-users register their public keys, the output from the **user list** command shows this in the quorum field, stating the enabled quorum strategy in use.
 ...
 *key quorum values*. This is the minimum number of crypto-user approvals that 
