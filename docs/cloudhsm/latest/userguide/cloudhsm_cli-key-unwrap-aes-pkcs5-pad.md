...
```
`aws-cloudhsm >` `help key unwrap aes-pkcs5-pad``Usage: key unwrap aes-pkcs5-pad [OPTIONS] --filter [`<FILTER>`...] --key-type-class `<KEY_TYPE_CLASS>` --label `<LABEL>` <--data-path `<DATA_PATH>`|--data `<DATA>`>
