...
To use quorum authentication, each CO must do *all* of
 the following steps: 
 ...
 *quorum minimum value*, also known as the *m value*.
...
In the preceding example, the first number (3) identifies the *HSM
 service* whose quorum minimum value you are setting.
