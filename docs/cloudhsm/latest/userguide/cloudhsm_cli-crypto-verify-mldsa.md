...
# Verify a signature signed with the ML-DSA mechanism in CloudHSM CLI
<a name="cloudhsm_cli-crypto-verify-mldsa"></a>

Use the **crypto verify ml-dsa** command in CloudHSM CLI to confirm data integrity and authenticity after ML-DSA-based signing. With this command, you can:
+ Confirm that a given public key signed a file in the HSM.
+ Verify that the ML-DSA signing mechanism generated the signature.
+ Compare a signed file against a source file to determine whether they are cryptographically related based on a given ML-DSA public key.

To use the **crypto verify ml-dsa** command, you must first have an ML-DSA public key in your AWS CloudHSM cluster with the `verify` attribute set to `true`.
...
**Note**  
To use the `external-mu` data type, you must first generate a 64-byte mu digest. For steps, see [Steps to generate external mu](cloudhsm_cli-crypto-sign-mldsa.md#cloudhsm_cli-crypto-sign-mldsa-external-mu) in the sign ml-dsa documentation.

## User type
<a name="cloudhsm_cli-crypto-verify-mldsa-userType"></a>

The following types of users can run this command.
+ Crypto users (CUs)

## Requirements
<a name="cloudhsm_cli-crypto-verify-mldsa-requirements"></a>
+ To run this command, you must be logged in as a CU.

## Syntax
<a name="cloudhsm_cli-crypto-verify-mldsa-syntax"></a>

```
aws-cloudhsm > help crypto verify ml-dsa
Verifies a signature with a ML-DSA public key

Usage: crypto verify ml-dsa [OPTIONS] --key-filter {{<KEY_FILTER>}} --data-type {{<DATA_TYPE>}} --signature {{<SIGNATURE>}} --hash-function {{<HASH_FUNCTION>}}
...
Options:
      ...
      --key-filter {{<KEY_FILTER>}}
          The key reference or a list of key attributes to filter for a key to verify with
      ...
      --data-path {{<DATA_PATH>}}
          Path to file containing data that was signed

      --data-type {{<DATA_TYPE>}}
          Specifies if the data is raw or processed using external mu [possible values: raw, external-mu]
...
## Examples
<a name="crypto-verify-mldsa-examples"></a>

These examples show how to use **crypto verify ml-dsa** to verify signatures that the ML-DSA signing mechanism generated. This command uses a public key in the HSM.

**Example: Verify a signature with raw data**  
Use this command to verify a signature for raw data with an ML-DSA public key.  

```
aws-cloudhsm > crypto verify ml-dsa \
    --key-filter attr.label=ml-dsa-public-key-example \
    --data-type raw \
...
**Example: Verify a signature with external mu data**  
Use this command to verify a signature for external mu data with an ML-DSA public key.  
...
**Example: Verify a signature from a file path**  
Use this command to verify a signature using file paths for the data and signature.  
...
## Arguments
<a name="cloudhsm_cli-crypto-verify-mldsa-arguments"></a>
...
Required: Yes (unless provided through signature)
...
## Related topics
<a name="cloudhsm_cli-crypto-verify-mldsa-seealso"></a>
