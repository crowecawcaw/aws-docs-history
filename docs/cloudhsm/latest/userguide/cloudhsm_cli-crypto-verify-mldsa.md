# Verify a signature signed with the ML-DSA mechanism in CloudHSM CLI

Use the **crypto verify ml-dsa** command in CloudHSM CLI to confirm data integrity and authenticity after ML-DSA-based signing. With this command, you can:

- Confirm that a given public key signed a file in the HSM.
- Verify that the ML-DSA signing mechanism generated the signature.
- Compare a signed file against a source file to determine whether they are cryptographically related based on a given ML-DSA public key.
  To use the **crypto verify ml-dsa** command, you must first have an ML-DSA public key in your AWS CloudHSM cluster with the `verify` attribute set to `true`.

###### Note

Starting September 1, 2026, ML-DSA is available in FIPS mode for hsm2m.medium clusters.

###### Note

You can generate a signature in CloudHSM CLI with [The crypto sign category in CloudHSM CLI](cloudhsm_cli-crypto-sign.md "cloudhsm_cli-crypto-sign.md") subcommands.

###### Note

To use the `external-mu` data type, you must first generate a 64-byte mu digest. For steps, see [Steps to generate external mu](cloudhsm_cli-crypto-sign-mldsa.md#cloudhsm_cli-crypto-sign-mldsa-external-mu "cloudhsm_cli-crypto-sign-mldsa.md#cloudhsm_cli-crypto-sign-mldsa-external-mu") in the sign ml-dsa documentation.

## User type

The following types of users can run this command.

- Crypto users (CUs)

## Requirements

- To run this command, you must be logged in as a CU.

## Syntax

```
`aws-cloudhsm >` `help crypto verify ml-dsa``Verifies a signature with a ML-DSA public key

Usage: crypto verify ml-dsa [OPTIONS] --key-filter `<KEY_FILTER>` --data-type `<DATA_TYPE>` --signature `<SIGNATURE>` --hash-function `<HASH_FUNCTION>`

Options:
 --cluster-id `<CLUSTER_ID>`
 Unique Id to choose which of the clusters in the config file to run the operation against. If not provided, will fall back to the value provided when interactive mode was started, or error

 --key-filter `<KEY_FILTER>`
 The key reference or a list of key attributes to filter for a key to verify with

 --hash-function `<HASH_FUNCTION>`
 The hash function to use with ML-DSA. [possible values: shake256]

 --data `<DATA>`
 Data that was signed

 --data-path `<DATA_PATH>`
 Path to file containing data that was signed

 --data-type `<DATA_TYPE>`
 Specifies if the data is raw or processed using external mu [possible values: raw, external-mu]

 --signature-path `<SIGNATURE_PATH>`
 The path to where the signature is located

 --signature `<SIGNATURE>`
 Base64 encoded signature to be verified

 -h, --help
 Print help`
```

## Examples

These examples show how to use **crypto verify ml-dsa** to verify signatures that the ML-DSA signing mechanism generated. This command uses a public key in the HSM.

###### Example: Verify a signature with raw data

Use this command to verify a signature for raw data with an ML-DSA public key.

```
`aws-cloudhsm >` `crypto verify ml-dsa \
 --key-filter attr.label=ml-dsa-public-key-example \
 --data-type raw \
 --hash-function shake256 \
 --data YWJjMTIz \
 --signature ""``{
 "error_code": 0,
 "data": {
 "message": "Signature verified successfully"
 }
}`
```

###### Example: Verify a signature with external mu data

Use this command to verify a signature for external mu data with an ML-DSA public key.

```
`aws-cloudhsm >` `crypto verify ml-dsa \
 --key-filter attr.label=ml-dsa-public-key-example \
 --data-type external-mu \
 --hash-function shake256 \
 --data DgFbkWgb9fbejQUSbJ09/Nmp7phb714RTkWdNAwQqd9u7M7XCWbI8EoaIeGcZCUwIzaG6kDavZDYxCcaEicbTw== \
 --signature ""``{
 "error_code": 0,
 "data": {
 "message": "Signature verified successfully"
 }
}`
```

###### Example: Verify a signature from a file path

Use this command to verify a signature using file paths for the data and signature.

```
`aws-cloudhsm >` `crypto verify ml-dsa \
 --key-filter attr.label=ml-dsa-public-key-example \
 --data-path /tmp/data.bin \
 --data-type raw \
 --hash-function shake256 \
 --signature-path /tmp/signature.bin``{
 "error_code": 0,
 "data": {
 "message": "Signature verified successfully"
 }
}`
```

## Arguments

**`<CLUSTER_ID>`**

The ID of the cluster to run this operation on.

Required: If multiple clusters have been [configured.](cloudhsm_cli-configs-multi-cluster.md "cloudhsm_cli-configs-multi-cluster.md")

**`<DATA>`**

Base64 encoded data to be verified.

Required: Yes (unless provided through data path)

**`<DATA_PATH>`**

Specifies the location of the data to be verified.

Required: Yes (unless provided through data)

**`<HASH_FUNCTION>`**

Specifies the hash function to use with ML-DSA.

Valid values:

- shake256

Required: Yes

**`<KEY_FILTER>`**

Key reference (for example, `key-reference=0xabc`) or space separated list of key attributes in the form of `attr.KEY_ATTRIBUTE_NAME=KEY_ATTRIBUTE_VALUE` to select a matching key.

For a listing of supported CloudHSM CLI key attributes, see Key attributes for CloudHSM CLI.

Required: Yes

**`<SIGNATURE>`**

Base64 encoded signature.

Required: Yes (unless provided through signature path)

**`<SIGNATURE_PATH>`**

Specifies the location of the signature.

Required: Yes (unless provided through signature)

**`<DATA_TYPE>`**

Specifies the data type of the provided data. Use `raw` for unhashed data; use `external-mu` for computed external mu data.

Valid values:

- raw
- external-mu

Required: Yes

## Related topics

- [The crypto verify category in CloudHSM CLI](cloudhsm_cli-crypto-verify.md "cloudhsm_cli-crypto-verify.md")
- [Generate a signature with the ML-DSA mechanism in CloudHSM CLI](cloudhsm_cli-crypto-sign-mldsa.md "cloudhsm_cli-crypto-sign-mldsa.md")
- [Generate an asymmetric ML-DSA key pair with CloudHSM CLI](cloudhsm_cli-key-generate-asymmetric-pair-mldsa.md "cloudhsm_cli-key-generate-asymmetric-pair-mldsa.md")
