...
| curve | Elliptic curve used to generate the EC key pair.<br />Valid Values: `secp224r1`, `secp256r1`, `prime256v1`, `secp384r1`, `secp256k1`, `secp521r1`, and `ed25519`<br />`ed25519` is only supported on hsm2m.medium instances in non-FIPS mode. | No | Settable with EC, not settable with RSA | 
...
| ec-point | For EC keys, DER-encoding of ANSI X9.62 ECPoint value "Q" in a hexadecimal format.<br />For other key types, this attribute does not exist. | No | No | 
| encrypt | Default: `False` | Yes | Yes | 
| extractable | Default: `True` | Can be changed from true to false on hsm2m.medium instances, but not from false to true. Cannot be modified on hsm1.medium instances. | Yes | 
| id | Default: Empty | id can be set on hsm2m.medium instances. It cannot be set on hsm1.medium instances. | Yes | 
| key-length-bytes | Required for generating an AES key.<br />Valid values: `16`, `24`, and `32` bytes. | No | No | 
| key-type | Possible values: `aes`, `rsa`, `ec`, and `ml-dsa` | No | Yes | 
...
| mldsa-algorithm | The ML-DSA algorithm used for ML-DSA key pair generation.<br />Valid values: `ML-DSA-44`, `ML-DSA-65`, and `ML-DSA-87` | No | Yes | 
| modulus | The modulus that was used to generate an RSA key pair. For other key types, this attribute does not exist. | No | No | 
| modulus-size-bits | Required for generating an RSA key pair.<br />Minimum value is `2048`. | No | Settable with RSA, not settable with EC | 
