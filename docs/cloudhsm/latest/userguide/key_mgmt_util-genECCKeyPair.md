# Generate an AWS CloudHSM ECC key pair using
 KMU
...
After generating the key, you can examine its attributes. Use
 [getAttribute](key_mgmt_util-getAttribute.md "key_mgmt_util-getAttribute.md") to write all of the
 attributes (represented by the constant `512`) of the new ECC private key to the
 `attr_262177` file.
...
```
Then use the `cat` command to view the contents of the `attr_262177` attribute file.
