...
Use the **syncKey** command in the AWS CloudHSM cloudhsm\_mgmt\_util to manually synchronize
 ...
 are usually created in different AWS Regions in order to simplify the global scaling and
 disaster recovery processes.

You cannot use **syncKey** to synchronize keys across arbitrary clusters:
 one of the clusters must have been created from a backup of the other. Additionally, both
 clusters must have consistent CO and CU credentials in order for the operation to be
 successful. For more information, see [HSM Users](understanding-users-cmu.md "understanding-users-cmu.md").
...
COs can use **syncKey** on any keys, while CUs can only use this
 command on keys that they own. For more information, see [HSM user types for AWS CloudHSM Management Utility](understanding-users-cmu.md "understanding-users-cmu.md").


## Prerequisites


Before you begin, you must know the `key handle` of the key on the source
 HSM to be synchronized with the destination HSM. To find the `key handle`,
 use the [listUsers](cloudhsm_mgmt_util-listUsers.md "cloudhsm_mgmt_util-listUsers.md") command to list all
 identifiers for named users. Then, use the [findAllKeys](cloudhsm_mgmt_util-findAllKeys.md "cloudhsm_mgmt_util-findAllKeys.md") command to find
...
To run **syncKey**, first enter server mode on the HSM which
 contains the key to be synchronized.
...
Run the **server** command to log into the source HSM and
 enter server mode. For this example, we assume that `server 0` is the source
 HSM.
...
Now run the **syncKey** command. In this example, we
 assume key `261251` is to be synced to `server 1`.
...
Specifies the key handle of the key to sync. You can specify only one key
 in each command. To get the key handle of a key, use [findAllKeys](cloudhsm_mgmt_util-findAllKeys.md "cloudhsm_mgmt_util-findAllKeys.md") while logged
 in to an HSM server.
...
* [findAllKeys](cloudhsm_mgmt_util-findAllKeys.md "cloudhsm_mgmt_util-findAllKeys.md")
* [describe-clusters](https://docs.aws.amazon.com/cli/latest/reference/cloudhsmv2/describe-clusters.html "https://docs.aws.amazon.com/cli/latest/reference/cloudhsmv2/describe-clusters.html") in AWS CLI
* [server](cloudhsm_mgmt_util-server.md "cloudhsm_mgmt_util-server.md")
