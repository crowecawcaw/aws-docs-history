...
1. Confirm that the X server of the Amazon DCV session is listening on a port in the range between 6000-6063.

   ```
   $ netstat -punta | grep 600
   ```

1. Add the remote server to the X server host access list.

   ```
   $ xhost +{{remote_server}}
   ```

1. Retrieve the Amazon DCV session display number.
...
1. From the remote server, export the display environment variable to point to the X server of the Amazon DCV session.
