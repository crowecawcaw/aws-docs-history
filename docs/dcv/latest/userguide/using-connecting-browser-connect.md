...
using different proxy settings, see the documentation for your specific web browser.
...
In the following example, the URL connects to a session that's named
...
2. Enter your sign-in credentials and choose **Login**.

###### Note

By default, the connection is terminated after three unsuccessful login attempts. To try
again, restart the connection. 3. Your web browser might warn you that the server's certificate isn't trusted. If you're
unsure about the authenticity of the certificate, confirm it with your Amazon DCV
administrator. Proceed if it's safe to do so.
