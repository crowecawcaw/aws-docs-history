# Exiting full screen on multiple monitors

After you extend the displays or enter full screen mode, a tab appears at the top-center
edge of the screen. To exit full screen mode, choose the tab and then select **Exit
fullscreen**.

###### Note

By default, DCV will save your display settings. If DCV detects a different monitor configuration, the display settings will be reset.
