...
Before setting up the Amazon DCV Access Console Access Manager, you must first install and configure the Session Manager Agent and Broker. For more information about setting up Amazon DCV Session Manager, see the [Amazon DCV Session Manager Administrator Guide](https://docs.aws.amazon.com/dcv/latest/sm-admin/what-is-sm.html).

## Registering a new client with the Broker
<a name="regsitering-client-broker"></a>
...
1. Run the following command to register a new client:

   ```
   $ sudo -u root dcv-session-manager-broker register-api-client --client-name "access-console"
   ```

1. Take note of the `client-id` and `client-password`. We will need these when we set up the components.

1. The Broker host will also need to have a Public DNS assigned to it. Take note of the address. The Access Console Handler will need this to communicate with the Broker

1. Make sure that the host the Broker is running on is accessible by the host the Access Console Handler will be installed on, via the Broker’s `client-to-broker-connector-https-port` and the Public DNS
**Note**  
