...
<a name="web-client-config-files"></a>
...
| `extra-ca-certs` | No |  | Specifies the path to a well known CA certificates in PEM format. If you followed the documentation to create a [self signed certificate](https://docs.aws.amazon.com/dcv/latest/access-console/generate-certs.html), then the value will be the path to `rootCA.pem`. | 
| `session-screenshot-max-width` | No | 1280 | Specifies the maximum pixel width of session screenshots taken using the [GetSessionScreenshots](https://docs.aws.amazon.com/dcv/latest/sm-dev/GetSessionScreenshots.html) API. This takes precedence over the values in the Session Manager Broker configuration file. If not specified, the default value will be used. If set to 0, the values from the Session Manger Broker configuration will apply. | 
