...
| Status | A session has four states associated with it.+  **Creating** – The Broker is in the process of creating the session. <br />+  **Available** – The session is ready to accept client connections (maps to “READY” in the [DescribeSessions](https://docs.aws.amazon.com/dcv/latest/sm-dev/DescribeSessions.html) API). <br />+  **Closing** – The session is being closed (maps to “DELETING” in the [DescribeSessions](https://docs.aws.amazon.com/dcv/latest/sm-dev/DescribeSessions.html) API). <br />+  **Closed** – The session is closed (maps to “DELETED” in the [DescribeSessions](https://docs.aws.amazon.com/dcv/latest/sm-dev/DescribeSessions.html) API). <br />+  **Unknown** – Unable to determine the session's state. The Broker and the Agent might be unable to communicate. Contact your administrator for help troubleshooting.  | 
...
+ [Connecting to a session](connecting-session.md)
+ [Closing a session](closing-session.md)