...
| `jdbc-connection-url` | Only required if persistence-db is set to `mysql` |  | Specifies the connection URL to the MariaDB/MySQL database; it contains the endpoint and the database name.<br />The url should have this format:jdbc:mysql://{{db\_endpoint}}:{{db\_port}}/{{db\_name}}<br />{{db\_endpoint}} is the MariaDB/MySQL database endpoint, {{db\_port}} is the database port and {{db\_name}} is the database name. | 
...
| `jwt-default-groups-claim-key` | No |  | Specifies the key that contains the default groups. To enable access to session templates on first login, create groups and assign them session templates through the Access Console, then use the group IDs in your external OAuth configuration. Groups are only assigned on first login and subsequent group management should be done through the Access Console. The groups value should be comma-separated group IDs (e.g., "group1,group2"). If a group does not exist, it will be created. | 
| `jwt-role-claim-key` | No |  | Specifies the key to retrieve the role from the JWT claims of the auth server. The role value must match a configured role (e.g., Admin, User, Guest). Invalid roles fall back to the default role defined in `default-role` in the Handler configuration file. | 
