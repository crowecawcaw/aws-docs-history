...
| `pam-helper-path` | Either `authentication-header-name` or `pam-helper-path` is required | `/var/usr/dcv-access-console-auth-server/dcvpamhelper` | Specifies the full path of the `dcvpamhelper` that is installed as part of the Authentication Server. | 
...
| `pam-normalize-userid-enabled` | No | False | Enables or disables the use of `pam-normalize-userid-command` to normalize the different usernames to a userid. | 
...
| `post-logout-redirect-uris` | Yes |  | Specifies the url of the Web Client to redirect to after logout. It should be of the format `https://webclient-host:webclient-port`. | 
| `authorization-server-hostname` | Yes |  | Specifies the url of the Authentication Server. It should be of the format `https://auth-server-host:auth-server-port`. | 
...
| `auth-server-client-id` | No | `dcv-access-console-web-client` | Specifies the client id for the Web Client. It should be the same in the Web Client properties. | 
