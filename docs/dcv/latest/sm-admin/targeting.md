...
1. In the file, add each new tag key and value pair on a new line using the `key=value` format. For example:
...
1. Open the Agent configuration file (`/etc/dcv-session-manager-agent/agent.conf` for Linux and macOS or `C:\Program Files\NICE\DCVSessionManagerAgent\conf\agent.conf` for Windows). For `tags_folder`, and specify the path to the directory in which the tag file is located.
