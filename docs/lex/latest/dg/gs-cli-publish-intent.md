...
1. In the AWS CLI, get the `$LATEST` version of the
   `OrderFlowers` intent and save it to a file:
...
2. In a text editor, open the `OrderFlowers_V4.json`
   file. Delete the `createdDate`, `lastUpdatedDate`, and
   `version` fields. Find the `FlowerTypes` slot type
   and change the version to the version number that you recorded in the
   previous step. The following fragment of the
   `OrderFlowers_V4.json` file shows the location of
   the change:
...
The following fragment of the response shows the checksum of the intent.
Record this for the next step.
...
The following fragment of the response shows the new version of the
intent. Record the version number for the next step.
