...
so that you can send it to your bot with the [PostContent](API_runtime_PostContent.md "API_runtime_PostContent.md") and [PostText](API_runtime_PostText.md "API_runtime_PostText.md") operations.
...
To send session attributes with the `PostContent`
...
You can send binary data to the `PostContent` and
`PostText` operations by first converting the data to
a base64-encoded string, and then sending the string as the value in
