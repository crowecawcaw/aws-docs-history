...
As a managed service, AWS Transform MGN is protected by the AWS global network security
...
You use AWS published API calls to access AWS Transform MGN through the
...
AWS Transform MGN customers should secure their replication servers by
...
2. Using a VPN to connect to the replication servers, such as the AWS site-to-site VPN.
   ...
   AWS Transform MGN creates and uses the "aws-replication" user within the Linux
   Source server. The AWS Transform MGN replication server and AWS Replication Agent
...
AWS Transform MGN only uses these permissions to read from block
devices.
