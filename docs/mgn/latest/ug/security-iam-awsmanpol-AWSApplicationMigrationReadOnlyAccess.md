...
The Read-Only policy allows a user to This policy provides permissions to all read-only public APIs of AWS Application Migration Service (AWS MGN), as well as some read-only APIs of other AWS services that are required in
order to make full read-only use of the AWS MGN console. It does not allow them to perform any actions, such as initialize the
