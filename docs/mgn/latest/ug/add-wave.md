NEW - You can now accelerate your migration and modernization with AWS Transform. Read [Getting Started](../../../transform/latest/userguide/getting-started.md "../../../transform/latest/userguide/getting-started.md") in the _AWS Transform User Guide_.

# Add wave

To add a wave, choose the **Add wave** button. You will then
see the **Add wave** prompt that includes the following
parameters:

- **Wave name** – Select a wave name. The name must be unique
  per account per region. Note that uniqueness verification for wave name in Migration
  Application Service is case-insensitive.
- **Description (optional)** – Add a description of the wave
  according to your preferences.
- **Associate applications (optional)** – You can add up to
  200 applications to a single wave. Checking an application in the dropdown list will associate
  it with the wave.
- **Add tags (optional)** – You can add up to 50 tags according to
  your preferences.
  Choose **Add wave** to create the wave.
