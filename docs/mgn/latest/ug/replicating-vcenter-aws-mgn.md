...
Once you have successfully installed the AWS vCenter client, all of your vCenter VMs are added to MGN in the DISCOVERED state. The DISCOVERED state means that the VM has not been
replicated to AWS.
...
By default, the MGN console only shows active servers. You can tell which servers are being
...
regular MGN operations on the server.
