...
- [Re-initialize the AWS Transform MGN](#Re-initialize-the-service "#Re-initialize-the-service")
...
- [Deleting MGN resources](#Deleting-Resources "#Deleting-Resources")
...
AWS Transform MGN converts the Windows OS licenses to AWS Windows licenses and activates them
against the AWS KMS.
...
then ensure that the names of the replication volumes match those given to them by AWS Transform MGN.
...
However, by renaming an EBS volume used by MGN for replication
...
E) on target machines launched by AWS Transform MGN.
...
You can delete various MGN resources, including source servers, jobs, and the
