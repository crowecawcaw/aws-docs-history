...
If you receive an AccessDeniedException error when attempting to log into AWS Transform MGN for the first
time and set up your replication settings template, it means that you are not the administrator
of your AWS Account and therefore cannot initialize MGN. You must be the Admin user of your AWS
Account to initialize MGN. [Learn more about
...
specify a subnet if that is the case, or AWS Transform MGN instance launch will fail. You may see the
