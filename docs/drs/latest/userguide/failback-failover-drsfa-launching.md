...
After your instance is ready, connect to it and download the DRSFA client
installer:
...
This command might ask for a sudo password if you use the Ubuntu ISO. Enter the
password but **do not** run this command as sudo.
...
The DRSFA client is installed in the `drs_failback_automation_client`
directory. You can optionally delete the installer:
...
Generate a `drs_failback_automation_seed.iso` file that contains the
password for the Failback Client VM. Upload this file to your datastore.

```
bash drs_failback_automation_seed_creator.sh
```

Enter a unique password that follows the [AWS
recommended password policy](../../../IAM/latest/UserGuide/id_credentials_passwords_account-policy.md "../../../IAM/latest/UserGuide/id_credentials_passwords_account-policy.md").

Two files are generated: `drs_failback_automation_seed.iso` and
`drs_failback_automation_seed.iso.sha512`. Upload the seed ISO to the same
datastore where the DRS Failback Client ISO is stored.
