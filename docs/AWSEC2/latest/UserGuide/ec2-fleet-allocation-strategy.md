...
The fleet identifies the pools with the highest capacity availability for the number of instances that are launching. This means that Spot Instances are requested from the pools that are most likely to have the lowest chance of interruption in the near term. The fleet then requests Spot Instances from the lowest priced of these pools.  
...
**Capacity optimized**  
The fleet identifies the pools with the highest capacity availability for the number of instances that are launching. This means that Spot Instances are requested from the pools that are most likely to have the lowest chance of interruption in the near term.  
