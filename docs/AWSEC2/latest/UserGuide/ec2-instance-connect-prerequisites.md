...
+ [Allow outbound traffic to the EC2 Instance Connect proxy](#eic-prereqs-browser-network)
...
When you connect to an instance using the Amazon EC2 console, your browser establishes a WebSocket connection on port 443 to the EC2 Instance Connect proxy service at a Region-specific endpoint.

The endpoint format depends on the [partition](https://docs.aws.amazon.com/glossary/latest/reference/glos-chap.html#partition):
