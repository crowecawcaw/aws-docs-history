...
6. Download the bundle for your instance store-based AMI to
   `/tmp/bundle` using the [ec2-download-bundle](ami-tools-commands.md#ami-download-bundle "ami-tools-commands.md#ami-download-bundle")
   command.
...
(without a device partition trailing digit). 11. Create a mount point for the new EBS volume and mount the volume.
...
In this example, the last line should be removed. 13. Unmount the volume and detach it from the instance.
...
14. Create an AMI from the new EBS volume as follows.
    ...
    `ari` ID, take note of that as well. 4. Register your new AMI with the snapshot ID of your new EBS volume and the values from
