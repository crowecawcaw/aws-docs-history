...
**Note**  
In the `eu-central-1-ist-1` Local Zone, advertisement and withdrawal are asynchronous. The address range enters a `pending-advertising` or `pending-withdrawal` state before the operation completes. For more information, see [Local Zone availability](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-byoip.html#byoip-zone-avail).
