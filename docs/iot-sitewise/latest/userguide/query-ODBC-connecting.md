...
+ **`PWD`** or **`IdPPassword`**   –   The password for the specified IdP account. If both `PWD` and `IdPPassword` are provided in the connection string, the `PWD` value will be used unless it's empty.
  ...
  The default is `NONE`.
+ **`OktaApplicationID`**   –   The unique Okta-provided ID associated with the AWS IoT SiteWise application. A place to find the application ID (AppId) is in the `entityID` field provided in the application metadata. An example is:
