...
+ [Storage data index files](#storage-data-index)
...
An external identifier for the hierarchy, target asset, or source asset is retrieved by calling the [DescribeAsset](https://docs.aws.amazon.com/iot-sitewise/latest/APIReference/API_DescribeAsset.html) API. 
...
| `associationType` | The association type of this asset relationship. <br />The value must be `CHILD`. The target asset is a child asset of the source asset. | 
