...
asset models must be configured to run on the edge. For more information, see [Configure an asset model for data processing on SiteWise Edge](edge-processing.md#process-gateway-data-edge "edge-processing.md#process-gateway-data-edge") for
...
- [Edge-only APIs for use with AWS IoT SiteWise edge devices](edge-local-apis.md "edge-local-apis.md")
- [Enable CORS on AWS IoT SiteWise Edge APIs](enable-cors-edge-apis.md "enable-cors-edge-apis.md")
- [Configure session timeouts for AWS IoT SiteWise Edge](edge-apis-session-timeout.md "edge-apis-session-timeout.md")
- [Tutorial: List asset models on an AWS IoT SiteWise Edge gateway](edge-apis-tutorial.md "edge-apis-tutorial.md")
