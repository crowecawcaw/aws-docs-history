...
For events, such as [Amazon Prime Day](https://aws.amazon.com/blogs/aws/prime-day-2023-powered-by-aws-all-the-numbers/ "https://aws.amazon.com/blogs/aws/prime-day-2023-powered-by-aws-all-the-numbers/"), DynamoDB
powers multiple high-traffic Amazon properties and systems, including [Alexa](https://alexa.com/ "https://alexa.com/"), [Amazon.com](https://www.amazon.com/ "https://www.amazon.com/") sites, and all [Amazon fulfillment
centers](https://www.aboutamazon.com/workplace/facilities "https://www.aboutamazon.com/workplace/facilities"). For such events, DynamoDB APIs have handled trillions of calls from Amazon
...
- [Characteristics of DynamoDB](#ddb-characteristics "#ddb-characteristics")
- [DynamoDB use cases](#ddb-use-cases "#ddb-use-cases")
- [Capabilities of DynamoDB](#ddb-capabilities "#ddb-capabilities")
- [Service integrations](#ddb-service-integrations "#ddb-service-integrations")
- [Security](#ddb-intro-security "#ddb-intro-security")
- [Resilience](#ddb-intro-resilience "#ddb-intro-resilience")
- [Accessing DynamoDB](#ddb-access "#ddb-access")
- [DynamoDB pricing](#ddb-pricing "#ddb-pricing")
- [Getting started with DynamoDB](#ddb-intro-get-started "#ddb-intro-get-started")
...
DynamoDB's [on-demand capacity mode](on-demand-capacity-mode.md "on-demand-capacity-mode.md")
...
- **Financial service applications** –
  ...
  With DynamoDB [global tables](GlobalTables.md "GlobalTables.md"), your applications
...
You can use [DynamoDB transactions](transactions.md "transactions.md") to achieve
atomicity, consistency, isolation, and durability (ACID) across one or more
tables with a single request. [(ACID)
transactions](transaction-apis.md "transaction-apis.md") suit workloads that include processing financial
...
To learn more about how customers from different industries use DynamoDB, see [Amazon DynamoDB Customers](https://aws.amazon.com/dynamodb/customers/ "https://aws.amazon.com/dynamodb/customers/") and [This is My Architecture](https://aws.amazon.com/architecture/this-is-my-architecture/?tma.sort-by=item.additionalFields.airDate&tma.sort-order=desc&awsf.category=*all&awsf.industry=*all&awsf.language=*all&awsf.show=*all&awsf.product=*all&tma.q=DynamoDB&tma.q_operator=AND "https://aws.amazon.com/architecture/this-is-my-architecture/?tma.sort-by=item.additionalFields.airDate&tma.sort-order=desc&awsf.category=*all&awsf.industry=*all&awsf.language=*all&awsf.show=*all&awsf.product=*all&tma.q=DynamoDB&tma.q_operator=AND").
...
DynamoDB is built for mission-critical workloads. It includes [(ACID) transactions](transaction-apis.md "transaction-apis.md") support for applications
...
local secondary indexes](SecondaryIndexes.md "SecondaryIndexes.md"), which let you query the table data using an
...
- [AWS AppSync](../../../appsync/latest/devguide/what-is-appsync.md "../../../appsync/latest/devguide/what-is-appsync.md")
...
[DynamoDB Accelerator (DAX)](DAX.md "DAX.md") is a fully managed, highly available
...
DynamoDB utilizes [IAM](../../../IAM/latest/UserGuide/introduction.md "../../../IAM/latest/UserGuide/introduction.md") to help you securely
...
security posture. With IAM, you can also enable [fine-grained access control](../../../IAM/latest/UserGuide/reference_policies_examples_dynamodb_attributes.md "../../../IAM/latest/UserGuide/reference_policies_examples_dynamodb_attributes.md") to provide authorization at the attribute
level. You can also define [resource-based
policies](access-control-resource-based.md "access-control-resource-based.md") with support for [IAM Access Analyzer](../../../IAM/latest/UserGuide/what-is-access-analyzer.md#what-is-access-analyzer-resource-identification "../../../IAM/latest/UserGuide/what-is-access-analyzer.md#what-is-access-analyzer-resource-identification") and [Block Public Access
(BPA)](rbac-bpa-rbp.md "rbac-bpa-rbp.md") to simplify policy management.

By default, DynamoDB encrypts all customer data at rest. [Encryption at rest](EncryptionAtRest.md "EncryptionAtRest.md") enhances the security of your data by using encryption
keys stored in [AWS Key Management Service](../../../kms/latest/developerguide/overview.md "../../../kms/latest/developerguide/overview.md") (AWS KMS). With encryption at rest, you can build security-sensitive
...
using [AWS-owned KMS keys](../../../kms/latest/developerguide/concepts.md#key-mgmt "../../../kms/latest/developerguide/concepts.md#key-mgmt") is available at no additional charge. For
client-side encryption, you can use the [AWS Database Encryption SDK](https://aws.amazon.com/blogs/security/how-to-use-aws-database-encryption-sdk-for-client-side-encryption-and-perform-searches-on-encrypted-attributes-in-dynamodb-tables/ "https://aws.amazon.com/blogs/security/how-to-use-aws-database-encryption-sdk-for-client-side-encryption-and-perform-searches-on-encrypted-attributes-in-dynamodb-tables/").

DynamoDB also adheres to several [compliance standards](https://aws.amazon.com/compliance/services-in-scope/ "https://aws.amazon.com/compliance/services-in-scope/"), including HIPAA,
...
Zones](https://aws.amazon.com/about-aws/global-infrastructure/regions_az/ "https://aws.amazon.com/about-aws/global-infrastructure/regions_az/") to provide high durability and a 99.99% availability SLA. DynamoDB also
...
availability SLA](https://aws.amazon.com/dynamodb/sla/ "https://aws.amazon.com/dynamodb/sla/") and multi-Region resilience. This helps you build
...
integration](Backup-and-Restore.md "Backup-and-Restore.md"), you can use AWS Backup to schedule, copy, tag, and manage the
...
You can work with DynamoDB using the [AWS Management Console](https://console.aws.amazon.com/dynamodb "https://console.aws.amazon.com/dynamodb"), the [AWS Command Line Interface](https://aws.amazon.com/cli/ "https://aws.amazon.com/cli/"),
[NoSQL Workbench for DynamoDB](workbench.md "workbench.md"), or [DynamoDB
APIs](../APIReference/API_Operations_Amazon_DynamoDB.md "../APIReference/API_Operations_Amazon_DynamoDB.md").

For more information, see [Accessing DynamoDB](AccessingDynamoDB.md "AccessingDynamoDB.md").
...
respective billing options for processing reads and writes on your tables: [on-demand](on-demand-capacity-mode.md "on-demand-capacity-mode.md") and [provisioned](provisioned-capacity-mode.md "provisioned-capacity-mode.md").
...
- [Getting started with DynamoDB](GettingStartedDynamoDB.md "GettingStartedDynamoDB.md") – Walks you
...
- [Create and Query a
  NoSQL Table with Amazon DynamoDB](https://aws.amazon.com/tutorials/create-nosql-table/ "https://aws.amazon.com/tutorials/create-nosql-table/")
- [Build an Application Using a NoSQL Key-Value Data Store](https://aws.amazon.com/tutorials/build-an-application-using-a-no-sql-key-value-data-store/ "https://aws.amazon.com/tutorials/build-an-application-using-a-no-sql-key-value-data-store/")

For information about resources, tools, and strategies to migrate to DynamoDB, see [Migrating to DynamoDB](migration-guide.md#migration-guide.title "migration-guide.md#migration-guide.title"). To read the latest blogs
