...
For more details, see [Partition key design](bp-partition-key-design.md "bp-partition-key-design.md").
...
![DynamoDB's distribution of table items across partitions based on the partition key's hash value.](images/HowItWorksPartitionKey.png)
...
If the table has a composite primary key (partition key and sort key), DynamoDB calculates the hash value of the partition key in the same way as described in [Data distribution: Partition key](#HowItWorks.Partitions.SimpleKey "#HowItWorks.Partitions.SimpleKey"). However, it tends to keep items which have the same value of partition key close together and in sorted order by
...
![DynamoDB stores an item with a composite partition key and sorts the item using the sort key attribute's value.](images/HowItWorksPartitionKeySortKey.png)
