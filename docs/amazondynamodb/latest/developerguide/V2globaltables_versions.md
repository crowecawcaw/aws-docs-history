...
- ****Put or Update****
  - **Replica Region:** **Source** / **Behavior before upgrade:** Timestamp population happens using [UpdateItem](https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_UpdateItem.html). / **Behavior during upgrade:** Timestamp population happens using [PutItem](https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_PutItem.html). / **Behavior after upgrade:** No customer visible timestamp is generated.
  - **Behavior before upgrade:** Two Streams records are generated. The first record contains the customer written attributes. The second record contains the aws:rep:\* attributes. / **Behavior during upgrade:** Two Streams records are generated. The first record contains the customer written attributes. The second record contains the aws:rep:\* attributes. / **Behavior after upgrade:** A single Streams record is generated containing the customer-writen attributes.
  - **Behavior before upgrade:** Two rWCUs are consumed for each customer write. / **Behavior during upgrade:** Two rWCUs are consumed for each customer write. / **Behavior after upgrade:** One rWCU is consumed for each customer write.
  ...
  - **Replica Region:** **Destination** / **Behavior before upgrade:** Replication happens using PutItem. / **Behavior during upgrade:** Replication happens using PutItem. / **Behavior after upgrade:** Replication happens using PutItem.
  ...
  - **Behavior before upgrade:** One rWCU is consumed if the item exists in the destination Region. Two rWCUs are consumed if the item doesn't exist in the destination Region. / **Behavior during upgrade:** One rWCU is consumed if the item exists in the destination Region. Two rWCUs are consumed if the item doesn't exist in the destination Region. / **Behavior after upgrade:** One rWCU is consumed for each customer write.
  - **Behavior before upgrade:** ReplicationLatency and PendingReplicationCount metrics are published in CloudWatch. / **Behavior during upgrade:** ReplicationLatency and PendingReplicationCount metrics are published in CloudWatch. / **Behavior after upgrade:** ReplicationLatency metric is published in CloudWatch.

- ****Delete****
  - **Replica Region:** **Source** / **Behavior before upgrade:** Delete any item with smaller timestamp using [DeleteItem](https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_DeleteItem.html). / **Behavior during upgrade:** Delete any item with smaller timestamp using DeleteItem. / **Behavior after upgrade:** Delete any item with smaller timestamp using DeleteItem.
  - **Behavior before upgrade:** A single Streams record is generated, which contains both the customer-written attributes and the aws:rep:\* attributes. / **Behavior during upgrade:** A single Streams record is generated, which contains both the customer-written attributes and the aws:rep:\* attributes. / **Behavior after upgrade:** A single Streams record is generated, which contains the customer-written attributes.
