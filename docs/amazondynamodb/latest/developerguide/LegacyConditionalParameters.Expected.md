...
[use ConditionExpression instead.](#Expected.instead "#Expected.instead").

The legacy conditional parameter `Expected` is a conditional block for an
`UpdateItem` operation. `Expected` is a map of
attribute/condition pairs. Each element of the map consists of an
attribute name, a comparison operator, and one or more values. DynamoDB compares the
attribute with the value(s) you supplied, using the comparison operator. For each
`Expected` element, the result of the evaluation is either true
or false.

If you specify more than one element in the `Expected` map, then by default
all of the conditions must evaluate to true. In other words, the conditions are combined using
`AND` operator. (You can use the `ConditionalOperator` parameter to OR the
conditions instead. If you do this, then at least one of the conditions must evaluate to
...
The following are descriptions of each comparison operator.
    ...
    + `EQ` : Equal. `EQ` is supported for all data
    ...
    `AttributeValueList` can contain only one `AttributeValue` of type String,
     ...
     `AttributeValue` element of a different
    ...
    `AttributeValueList` can contain only one
     `AttributeValue` element of type String,
    ...
    This operator tests for the nonexistence of an attribute, not its
     ...
     `false`. This is because the attribute
     "`a`" exists; its data type is not relevant to the
    ...
    `AttributeValueList` can contain only one
     `AttributeValue` element of type String, Number, or
     Binary (not a set type). If the target attribute of the comparison is of
     type String, then the operator checks for a substring match. If the
     target attribute of the comparison is of type Binary, then the operator
     looks for a subsequence of the target that matches the input. If the
    ...
    `AttributeValueList` can contain only one `AttributeValue` element of type String,
     ...
     "`NS`", or "`BS`"), then the operator evaluates to true if it
     `does not` find an exact match with any member of the set.


    NOT\_CONTAINS is supported for lists: When evaluating "`a NOT CONTAINS b`", "
     `a`" can be a list; however, "`b`" cannot be a set, a map, or a list.
    ...
    `AttributeValueList` can contain one or more
     ...
     are present in the item attribute, the expression evaluates to
     true.
...
The following parameters can be used instead of `AttributeValueList` and
`ComparisonOperator`:
...
## Use _ConditionExpression_ instead – Example

Suppose you wanted to modify an item in the _Music_ table, but
only if a certain condition was true. You could use an `UpdateItem`
request with an `Expected` parameter, as in this AWS CLI example:
