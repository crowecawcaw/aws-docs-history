...
<a name="abac-implementation-streams"></a>
...
**Topics**
+ [Step 1: Add tags to a DynamoDB stream](#abac-streams-step-1)
+ [Step 2: Create an IAM role with a policy including tag-based conditions](#abac-streams-step-2)
+ [Step 3: Test allowed permissions](#abac-streams-step-3)

## Step 1: Add tags to a DynamoDB stream
<a name="abac-streams-step-1"></a>
...
1. Make sure that your DynamoDB stream contains the `"environment"` tag key with a value of `"staging"`.

1. Perform the `dynamodb:DescribeStream` and `dynamodb:GetRecords` actions on the tagged stream. These actions succeed if the `"environment": "staging"` tag key-value pair is present.
