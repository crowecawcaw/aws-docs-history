...
+ If an update to the table changes only attributes that are neither index key attributes nor projected into the index, the index entry is unchanged, so no write capacity is consumed for that index. The base table is still charged for the write.
