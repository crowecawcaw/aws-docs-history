...
| `KeySchema`             | The primary key (simple or composite) structure for the table. A name-value pair for<br>the `HashKeyElement` is required, and a name-value<br>pair for the `RangeKeyElement` is optional (only<br>required for composite primary keys). The maximum hash key size is 2048<br>bytes. The maximum range key size is 1024 bytes. Both limits are<br>enforced separately (i.e. you can have a combined hash + range 2048 +<br>1024 key). For more information about primary keys, see [Primary key](HowItWorks.CoreComponents.md#HowItWorks.CoreComponents.PrimaryKey "HowItWorks.CoreComponents.md#HowItWorks.CoreComponents.PrimaryKey").Type: Map of<br>`HashKeyElement`, or `HashKeyElement` and<br>`RangeKeyElement` for a composite primary key. |
