...
| `gsiHashKeyType` | The data type of the index partition key—`String`, `Number`, or `Binary`:<br />`S \| N \| B` | Yes | 
...
| `gsiRangeKeyType` | The data type of the index sort key—`String`, `Number`, or `Binary`:<br />`S \| N \| B`<br /> Do not specify this parameter if the index only has a simple primary key (partition key). | No | 
