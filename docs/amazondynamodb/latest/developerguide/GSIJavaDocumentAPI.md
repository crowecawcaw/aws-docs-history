...
- [Example: Global Secondary Indexes using the AWS SDK for Java
  document API](GSIJavaDocumentAPI.md "GSIJavaDocumentAPI.md")
...
global secondary indexes. The following Java code example creates a table to hold
...
The following are the steps to create a table with a global secondary index, using the DynamoDB document
API.
...
key, the key schema for the index, and the attribute projection. 3. Call the `createTable` method by providing the request object as a
parameter.
...
1. Create an instance of the `DynamoDB` class.
2. Create an instance of the `Table` class to represent the index you
   want to work with.
...
2. Create an instance of the `Table` class to represent the index you
   want to work with.
3. Create an instance of the `Index` class for the index you want to
   query.
