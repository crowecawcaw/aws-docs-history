...
+  [Java: Setting the AWS Region and endpoint](CodeSamples.Java.md#CodeSamples.Java.RegionAndEndpoint) (DynamoDB local supports the AWS SDK for Java 1.x and 2.x; for the SDK for Java 2.x, see the [AWS SDK for Java 2.x Developer Guide](https://docs.aws.amazon.com/sdk-for-java/latest/developer-guide/home.html)) 
...
+ Vector indexes are not supported. `CreateTable` accepts the `VectorIndexes` parameter without error and then ignores it, so `DescribeTable` reports no vector index and `SearchVectors` is unavailable. Test vector index behavior against the DynamoDB web service.