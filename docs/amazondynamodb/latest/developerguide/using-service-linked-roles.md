...
A service-linked role makes setting up DAX easier because you don’t have to manually add
...
IAM](../../../IAM/latest/UserGuide/reference_aws-services-that-work-with-iam.md "../../../IAM/latest/UserGuide/reference_aws-services-that-work-with-iam.md") in the _IAM User Guide_. Look for the services that have
**Yes** in the **Service-linked roles** column. Choose a
...
The `AWSServiceRoleForDAX` service-linked role makes it easier for you to set up and
maintain a DAX cluster. However, you must still grant each cluster access to DynamoDB before
you can use it. For more information, see [DAX access control](DAX.access-control.md "DAX.access-control.md").

The `AWSServiceRoleForDAX` service-linked role trusts the following services to assume
the role:

- `dax.amazonaws.com`

The role permissions policy allows DAX to complete the following actions on the
specified resources:
...
**To allow an IAM entity to create AWSServiceRoleForDAX service-linked
roles**
...
You don't need to manually create a service-linked role. When you create a cluster, DAX
creates the service-linked role for you.

###### Important

If you were using the DAX service before February 28, 2018, when it began supporting
service-linked roles, DAX created the `AWSServiceRoleForDAX` role in your account. For
more information, see [A New
Role Appeared in My AWS Account](../../../IAM/latest/UserGuide/troubleshoot_roles.md#troubleshoot_roles_new-role-appeared "../../../IAM/latest/UserGuide/troubleshoot_roles.md#troubleshoot_roles_new-role-appeared") in the
_IAM User Guide_.
...
DAX creates the service-linked role for you again.

## Editing a service-linked role for DAX

DAX does not allow you to edit the `AWSServiceRoleForDAX` service-linked role. After
...
2. In the navigation pane of the IAM console, choose **Roles**. Then
   choose the name (not the check box) of the **AWSServiceRoleForDAX** role.
...
If you are unsure whether DAX is using the `AWSServiceRoleForDAX` role, you can
...
See [delete-cluster](../../../cli/latest/reference/dax/delete-cluster.md "../../../cli/latest/reference/dax/delete-cluster.md") in the
_AWS CLI Command Reference_.

###### To delete a DAX cluster (API)

See [DeleteCluster](../APIReference/API_dax_DeleteCluster.md "../APIReference/API_dax_DeleteCluster.md") in
...
`AWSServiceRoleForDAX` service-linked role. For more information, see [Deleting
