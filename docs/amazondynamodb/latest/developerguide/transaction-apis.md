...
The exception contains the list of [CancellationReasons](https://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_CancellationReason.html), ordered according to the list of items in the `TransactItems` request parameter. The exception's error message also includes a string representation of the cancellation reason codes.
...
This capacity is consumed even when a transaction does not succeed. A `TransactWriteItems` or `TransactGetItems` call that is canceled — for example, because a condition in a `ConditionCheck` or other operation evaluates to false (a `ConditionalCheckFailed` cancellation reason) — still consumes the underlying read or write capacity for the items it attempted, in the same way a successful transaction does.
