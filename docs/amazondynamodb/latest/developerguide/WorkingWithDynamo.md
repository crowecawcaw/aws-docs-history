# Working with tables, items, queries, scans, and

indexes

This section provides details about working with tables, items, queries, and more in
Amazon DynamoDB.

###### Topics

- [Working with tables and data in DynamoDB](WorkingWithTables.md "WorkingWithTables.md")
- [Global tables - multi-active, multi-Region
  replication](GlobalTables.md "GlobalTables.md")
- [Working with items and attributes in DynamoDB](WorkingWithItems.md "WorkingWithItems.md")
- [Improving data access with secondary indexes in
  DynamoDB](SecondaryIndexes.md "SecondaryIndexes.md")
- [Managing complex workflows with DynamoDB transactions](transactions.md "transactions.md")
- [Change data capture with Amazon DynamoDB](streamsmain.md "streamsmain.md")
