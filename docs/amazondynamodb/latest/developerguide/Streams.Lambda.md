...
events with Amazon DynamoDB and AWS Lambda using the AWS CLI
...
- [Step 1: Create a DynamoDB table
  with a stream enabled](#Streams.Lambda.Tutorial.CreateTable "#Streams.Lambda.Tutorial.CreateTable")
- [Step 2: Create a Lambda
  execution role](#Streams.Lambda.Tutorial.CreateRole "#Streams.Lambda.Tutorial.CreateRole")
- [Step 3: Create an Amazon SNS
  topic](#Streams.Lambda.Tutorial.SNSTopic "#Streams.Lambda.Tutorial.SNSTopic")
- [Step 4: Create and test a
  Lambda function](#Streams.Lambda.Tutorial.LambdaFunction "#Streams.Lambda.Tutorial.LambdaFunction")
- [Step 5: Create and test a
  trigger](#Streams.Lambda.Tutorial.CreateTrigger "#Streams.Lambda.Tutorial.CreateTrigger")
...
![Woofer application workflow of a DynamoDB table, stream record, Lambda function, and Amazon SNS topic.](images/StreamsAndTriggers.png)
...
the instructions in the [AWS Command Line Interface User Guide](../../../cli/latest/userguide.md "../../../cli/latest/userguide.md") to install and configure the AWS CLI.

## Step 1: Create a DynamoDB table

with a stream enabled
...
create in [Step 4: Create and test a
Lambda function](#Streams.Lambda.Tutorial.LambdaFunction "#Streams.Lambda.Tutorial.LambdaFunction").
...
3. When you create the Lambda function, you specify the Amazon Resource Name
   (ARN) for `WooferLambdaRole`, which you created in [Step 2: Create a Lambda
   execution role](#Streams.Lambda.Tutorial.CreateRole "#Streams.Lambda.Tutorial.CreateRole"). Enter the
...
In [Step 4: Create and test a
Lambda function](#Streams.Lambda.Tutorial.LambdaFunction "#Streams.Lambda.Tutorial.LambdaFunction"), you tested the Lambda
...
You should receive a new email message within a few minutes. 4. Open the DynamoDB console and add a few more items to `BarkTable`.
