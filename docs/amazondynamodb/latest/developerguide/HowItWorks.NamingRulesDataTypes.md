...
DynamoDB has a list of reserved words and special characters. For a complete list, see [Reserved words in DynamoDB](ReservedWords.md). Also, the following characters have special meaning in DynamoDB: **\#** (hash) and **:** (colon).
...
Storing a timestamp as a number uses less storage than the equivalent ISO 8601 string—a difference that can add up across tables with millions or billions of items. When stored as epoch seconds, a number can also be used with the DynamoDB Time to Live (TTL) feature to automatically delete expired items. Alternatively, you can store date and time values as strings in ISO 8601 format, which are human-readable. For more information, see [Using time to live (TTL) in DynamoDB](TTL.md).
