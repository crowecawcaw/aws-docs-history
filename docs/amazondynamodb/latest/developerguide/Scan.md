...
- [Limiting the number of items in the result set](#Scan.Limit "#Scan.Limit")
- [Paginating the results](#Scan.Pagination "#Scan.Pagination")
- [Counting the items in the results](#Scan.Count "#Scan.Count")
- [Capacity units consumed by scan](#Scan.CapacityUnits "#Scan.CapacityUnits")
- [Read consistency for scan](#Scan.ReadConsistency "#Scan.ReadConsistency")
- [Parallel scan](#Scan.ParallelScan "#Scan.ParallelScan")
...
The following AWS Command Line Interface (AWS CLI) example scans the `Thread` table and
...
DynamoDB _paginates_ the results from `Scan` operations.
...
To determine whether there are more results and to retrieve them one page at a time,
...
1. Examine the low-level `Scan` result:
   ...
   - If there is _not_ a `LastEvaluatedKey`
     in the result, then there are no more items to be retrieved.

2. Construct a new `Scan` request, with the same parameters as the
   previous one. However, this time, take the `LastEvaluatedKey` value
...
is no longer present in the results. Consider the following AWS CLI example that scans the
...
- Using `aws dynamodb scan --table-name Prices --max-items 1`
  returns a `NextToken`
- Using `aws dynamodb scan --table-name Prices --limit 1` returns
  a `LastEvaluatedKey`.
...
have been retrieved. The AWS CLI then issues another `Scan` request to DynamoDB.
...
- `ScannedCount` — The number of items evaluated, before any
  `ScanFilter` is applied. A high `ScannedCount` value
  with few, or no, `Count` results indicates an inefficient
  `Scan` operation. If you did not use a filter in the request,
...
all the results (see [Paginating the results](#Scan.Pagination "#Scan.Pagination")).
