...
When you create a new experiment, you specify a name for it and define the goals on the **Document your hypothesis** page in AWS AppConfig experimentation. Enter a name that identifies the experiment goal, for example _Increase add-to-cart rate by adding a button for each product displayed_. If you're creating multiple versions of the same experiment, specify version numbers in the name, for example _V2-Increase add-to-cart rate by adding a button for each product displayed_.
