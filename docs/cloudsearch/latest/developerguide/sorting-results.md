...
- Use a `text` or `literal` field to sort results
  alphabetically. Note that Amazon CloudSearch sorts by Unicode codepoint, so numbers come before letters and uppercase letters come before lowercase letters.
  Numbers are sorted as strings, not by value; for example, 10 will come before 2.
- Use an `int` or `double` field to sort results
  numerically.
...
- Use a custom expression to sort results.
  ...
  more information about configuring fields, see [configure indexing options](configuring-index-fields.md "configuring-index-fields.md").
...
expressions, see [Configuring Expressions](configuring-expressions.md "configuring-expressions.md").
...
When you use a field for sorting, documents without a value in that field are listed
last. If you specify a comma separated list of fields or expressions, the first field or
expression is used as the primary sort criteria, the second is used as the secondary
sort criteria, and so on.
