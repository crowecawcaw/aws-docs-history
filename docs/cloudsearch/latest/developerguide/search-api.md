...
The distance value must be a positive integer. For example, to
 find all documents where *teenage* occurs within
 10 words of *vampire* in the `plot`
 ...
 *star* in the title field and have a year
...
Use the `prefix` operator to combine a prefix search with other search
 ...
 all documents that contain the prefix *sta* in
...
Use the `range` operator to combine a range search with other search
 ...
 all documents that contain *star* in the title
...
Use the `term` operator to combine a term search with other search criteria
 ...
 that contain *star* in the title field and have a
...
You use the simple query parser to search for individual terms or phrases. By default, all statically configured `text` and `text-array` fields are searched. Dynamic fields and `literal` fields are *not* searched by default. You can use the `q.options` parameter to specify which fields you want to search, change the default operator used to combine individual terms in the search string, or disable any of the simple parser operators (`and`, `escape`, 
 `fuzzy`, `near`, `not`, `or`, 
 `phrase`, `precedence`, `prefix`,
 `whitespace`).
