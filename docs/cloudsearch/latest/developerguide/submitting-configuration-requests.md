...
You submit Amazon CloudSearch configuration requests to the Amazon CloudSearch endpoint for your region using the AWS Query protocol. For the current list of supported regions and endpoints, see [Regions and Endpoints](../../../general/latest/gr/rande.md#cloudsearch_region "../../../general/latest/gr/rande.md#cloudsearch_region").

AWS Query requests are HTTP or HTTPS requests submitted via HTTP GET or POST with a
Query parameter named Action. You must specify the API version in all configuration
requests and that version must match the API version specified when the domain was
created.

You must include authorization parameters and a digital signature in every request. Amazon CloudSearch supports AWS Signature Version 4. For detailed signing instructions, see [Signature V4 Signing Process](../../../general/latest/gr/signature-version-4.md "../../../general/latest/gr/signature-version-4.md") in the AWS General Reference.

###### Note

Amazon CloudSearch throttles excessive requests to the configuration service. Throttling occurs
_by action_, so excessive `DescribeDomains` requests
don't cause Amazon CloudSearch to throttle `DescribeIndexFields` requests. The request
...
- Action—the Amazon CloudSearch configuration action you want to perform. For a complete list of actions,
  see [Actions](API_Operations.md "API_Operations.md").
- Parameters—any request parameters required for the specified action. Each query request must also include some common parameters to handle authentication. For more information, see [Request Authentication](#configuration-request-authentication "#configuration-request-authentication").
...
For example, the following GET request creates a new search domain called _movies_:
...
include authorization parameters and a digital signature in every request. Amazon CloudSearch supports AWS Signature Version 4. For detailed signing instructions, see [Signature V4 Signing Process](../../../general/latest/gr/signature-version-4.md "../../../general/latest/gr/signature-version-4.md") in the AWS General Reference.

###### Note

If you are just getting started signing your own AWS requests, take a look at how the SDKs implement signing. The source for most of the AWS SDKs is available at [https://github.com/aws](https://github.com/aws "https://github.com/aws").
