...
If you are no longer using a search domain, you must delete it to avoid incurring
 ...
 During this process, the domain status is `Being Deleted` and your account is not
 charged. 
...
* [Deleting a Domain Using the Amazon CloudSearch Console](#deleting-domains-console "#deleting-domains-console")
* [Deleting a Domain Using the AWS CLI](#deleting-domains-clt "#deleting-domains-clt")
* [Deleting Amazon CloudSearch Domains Using the AWS SDKs](#deleting-domains-sdk "#deleting-domains-sdk")
...
3. Select the checkbox next to the domain that you want to delete, then choose
 **Delete** and confirm deletion.

## Deleting a Domain Using the AWS CLI


Run the `aws cloudsearch delete-domain` command and specify the name of the domain you want to
...
The AWS SDKs (except the Android and iOS SDKs) support all of the Amazon CloudSearch actions defined in the Amazon CloudSearch Configuration API, including `DeleteDomain`. For more information about installing and using the AWS SDKs, see [AWS Software Development Kits](http://aws.amazon.com/code "http://aws.amazon.com/code").
