...
| `OFFLINE` | Your gateway might be in an `OFFLINE` status for one or more of the following reasons: <br>• The gateway can't reach the Storage Gateway service endpoints. <br>• The gateway had an unexpected shutdown. <br>• The gateway has an associated cache disk that is disconnected, has been modified, or has failed. |
