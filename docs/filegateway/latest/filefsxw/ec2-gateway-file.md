...
FSx File Gateway AMIs use the following naming convention. The version number
...
1. Start setting up a new gateway using the Storage Gateway console. For instructions, see
   [Set up an Amazon FSx File Gateway](create-gateway-file.md#set-up-gateway-fsx-file "create-gateway-file.md#set-up-gateway-fsx-file"). When you reach the **Platform
...
8. In the **Inbound security groups rules** subsection, add firewall
   ...
   information on the ports required for Amazon FSx File Gateway, see
   [Port
...
Amazon FSx File Gateway requires TCP port 80 to be open for inbound traffic and
one-time HTTP access during gateway activation. After activation, you can close
this port.

Additionally, you must open TCP port 445 for SMB access, UDP port 137 for
NetBIOS access, UDP port 138 for NetBIOS access, and TCP port 389 for LDAP
...
resume setting up your Amazon FSx File Gateway.
...
- Query the Systems Manager parameter store. You can use the AWS CLI or Storage Gateway API to
  query the Systems Manager public parameter under the namespace `/aws/service/storagegateway/ami/FILE_FSX_SMB/latest`. For example, using
