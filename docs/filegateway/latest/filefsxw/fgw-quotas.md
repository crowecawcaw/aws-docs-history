...
| Maximum number of backup copy requests in progress to a single<br>destination Region per account. | 5 requests                       |
...
The following table recommends sizes for local disk storage for each AWS Storage Gateway in
your deployment.
...
When adding cache to an existing FSx File Gateway, it is important to create new disks
on your virtual host (hypervisor or Amazon EC2 instance). Do not change the size of
