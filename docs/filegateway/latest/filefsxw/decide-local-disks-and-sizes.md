...
data from the cache. The cache on an FSx File Gateway is shared between all of the file shares on
...
FSx File Gateway), but you can't decrease the cache for a given gateway. You can
...
FSx File Gateway provides flexibility and elasticity related to the amount of
