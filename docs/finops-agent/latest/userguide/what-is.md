...
**Powered by Amazon Bedrock**: AWS FinOps Agent is built on Amazon Bedrock and includes [automated abuse detection](../../../bedrock/latest/userguide/abuse-detection.md "../../../bedrock/latest/userguide/abuse-detection.md") implemented in Amazon Bedrock to enforce safety, security, and the responsible use of AI.
...
- **Context files and memory.** Upload organization-specific context files. The agent applies them to its answers and remembers your preferences across sessions.
  For how to use each feature, see [Chatting with AWS FinOps Agent](chatting-with-finops-agent.md "chatting-with-finops-agent.md"), [Task management](task-management.md "task-management.md"), and [Context files and memory](context-files-and-memory.md "context-files-and-memory.md").

## Related services

AWS FinOps Agent integrates with AWS services that you permit during the [agent creation](creating-an-agent.md "creating-an-agent.md") process. For the full list of IAM actions, see the [AWS FinOps Agent IAM setup guide](setting-up.md "setting-up.md").
...
To get started, see [Creating an agent](creating-an-agent.md "creating-an-agent.md").
