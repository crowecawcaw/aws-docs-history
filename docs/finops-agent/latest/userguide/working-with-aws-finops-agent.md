...
The [use cases](chatting-with-finops-agent.md "chatting-with-finops-agent.md") show what the agent can do from a single conversation: ask about costs, investigate anomalies, surface optimization recommendations, and generate reports. Any of these can run on a schedule or in response to an event. For a complete hands-off example, [event-triggered cost anomaly investigation](automated-anomaly-response.md "automated-anomaly-response.md") connects a AWS Cost Anomaly Detection trigger, an investigation, and a Slack destination into one workflow.
