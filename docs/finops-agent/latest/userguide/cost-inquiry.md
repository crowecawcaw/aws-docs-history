...
The depth of the agent's responses depends on which IAM permissions are enabled. If a data source is not granted, the agent disables the tools that depend on it and notes the limitation in its response. For the full list of IAM actions, see the [AWS FinOps Agent IAM setup guide](setting-up.md "setting-up.md").
