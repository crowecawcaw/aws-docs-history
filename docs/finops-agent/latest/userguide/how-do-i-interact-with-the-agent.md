...
1. Verify with your administrator that your IAM identity has the AWS FinOps Agent operator permissions attached. Without these permissions, the web application cannot authenticate your session. See [End-user web app policy](setting-up.md#setting-up-end-user-policy "setting-up.md#setting-up-end-user-policy") for the required policy.
...
- **Side navigation.** Switches between workspaces and tracks your past conversations. See [Side navigation](#webapp-side-navigation "#webapp-side-navigation").
- **Chat area.** The primary way to interact with the agent. See [Chat](#webapp-chat "#webapp-chat").
- **Workspace panel.** Opens to show the workspace you choose from the side navigation: Tasks, Automations, Artifacts, or Context files. See [Workspaces](#webapp-workspaces "#webapp-workspaces").
...
For an overview of what you can ask, see [Chatting with AWS FinOps Agent](chatting-with-finops-agent.md "chatting-with-finops-agent.md").
...
For details on guardrail behavior, see [Agent guardrail controls](agent-guardrail-control.md "agent-guardrail-control.md").
