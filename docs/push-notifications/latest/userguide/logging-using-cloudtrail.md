...
All AWS End User Messaging Push actions are logged by CloudTrail and are documented in the [AWS End User Messaging Push API Reference](https://docs.aws.amazon.com/pinpoint/latest/apireference/welcome.html). For example, calls to the `GetAdmChannel`, `UpdateApnsChannel` and `GetApnsVoipChannel` actions generate entries in the CloudTrail log files.
