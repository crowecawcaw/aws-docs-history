...
- A [USDC wallet address on Base, Solana, or both to receive payments](https://www.coinbase.com/developer-platform/wallets "https://www.coinbase.com/developer-platform/wallets").
- (Recommended) AWS WAF Bot Control enabled to classify AI bot traffic by identity

## Quick start

1. **Enable Bot Control** – If not already enabled, [add the AWS WAF Bot Control managed rule group to your web ACL](waf-using-managed-rule-group.md "waf-using-managed-rule-group.md").
2. **Create a protection pack (webACL)** – In the AWS WAF console, navigate to Protection packs (webACL) in the Cloudfront (Global) region, and [create a new pack](web-acl-creating.md "web-acl-creating.md") with your pricing, bot and agent policies, and payment configuration. [Associate with a CloudFront distribution](cloudfront-features.md "cloudfront-features.md").
3. **Configure MonetizationConfig** – To use AI traffic monetization, you configure a `MonetizationConfig` on your web ACL. This configuration defines the payment networks you accept and the base price for monetized requests.
...
- You use test funds such as [Testnet Faucet](https://faucet.circle.com/ "https://faucet.circle.com/") (available from public faucets) instead of real USDC
