...
The PutRawMessageContent API requires boto3 1.17.8, or you can add a
layer to your Lambda function. To download the correct boto3 version, see
the [boto page on
GitHub](https://github.com/boto/boto "https://github.com/boto/boto"). For more information about adding layers, see [Configure a function to use layers](../../../lambda/latest/dg/configuration-layers.md#configuration-layers-using "../../../lambda/latest/dg/configuration-layers.md#configuration-layers-using").

Here's an example layer:
`"LayerArn":"arn:aws:lambda:${`AWS`::`Region`}:`489970191081`:layer:WorkMailLambdaLayer:2"`.
In this example, substitute
`${`AWS`::`Region`}`
with an appropriate aws region, such as us-east-1.
...
Application Repository to your account, the system creates a Lambda
function in your account with the necessary resources and permissions.
You can then add business logic to the lambda function, based on your
use cases.
...
- Once you have the original message, change the MIME content. When
  ...
  your Amazon WorkMail operations, and that it uses the same AWS Region as your
  API calls.
- For Amazon WorkMail to process requests, your S3 bucket must have the correct
  policy in order to access the S3 object. For more information, see
  [Example S3 policy](#s3example "#s3example").
- Use the [PutRawMessageContent](../APIReference/API_messageflow_PutRawMessageContent.md "../APIReference/API_messageflow_PutRawMessageContent.md") API to send the updated the
  message content back to Amazon WorkMail.

###### Note

The `PutRawMessageContent` API ensures that the MIME
content of the updated message meets RFC standards, as wells as the
criteria mentioned in the [RawMessageContent](../APIReference/API_messageflow_RawMessageContent.md "../APIReference/API_messageflow_RawMessageContent.md") data type. Emails inbound to your Amazon WorkMail
organization don't always meet those standards, so the
`PutRawMessageContent` API may reject them. In such
cases, you can consult the error message returned for more information
...
The following example shows how a Lambda function uses the Python runtime
to update the subject of an in-transit email message.
...
For more examples of ways to analyze the content of in-transit messages,
see the [amazon-workmail-lambda-templates](https://github.com/aws-samples/amazon-workmail-lambda-templates "https://github.com/aws-samples/amazon-workmail-lambda-templates") repository on GitHub.
