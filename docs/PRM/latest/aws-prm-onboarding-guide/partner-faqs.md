...
Since an AWS resource can only have one tag with the `aws-apn-id` key, only one partner identifier is allowed per resource. For multi-partner scenarios where multiple partners operate on the same AWS resource, consider using the [User Agent String](user-agent-string.md) method instead. If you must use resource tagging, coordinate with the other partner and the customer to determine tag ownership before making changes.
