...
Replace `5ugbbrmu7ud3u5hsipfzug61p` with your product code. See [Product Code Retrieval](product-code-retrieval.md "product-code-retrieval.md"). The `$` is the required end delimiter.
...
The Application ID configuration methods described above are supported by most AWS SDKs and tools. For a complete list, see [Application ID support by AWS SDKs and tools](../../../sdkref/latest/guide/feature-appid.md#appid-sdk-compat "../../../sdkref/latest/guide/feature-appid.md#appid-sdk-compat"). JVM system property settings are only supported by the AWS SDK for Java and the AWS SDK for Kotlin.
...
3. Confirm the format matches exactly: `APN_1.1/pc_<YOUR-PRODUCT-CODE>$`
