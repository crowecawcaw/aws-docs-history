...
Amazon Bedrock uses _application inference profiles_ as the taggable resource for Partner Revenue Measurement. You must create an application inference profile, tag it with the `aws-apn-id` tag, and then use that profile for all model invocations.
...
The tag value must use the format `pc:`product-code`, where `product-code`` is your AWS Marketplace product code. To retrieve your product code, see [Product Code Retrieval](product-code-retrieval.md "product-code-retrieval.md"). Do **NOT** use the Product ID or UUID formatted product ID.
...
Use the ARN from the previous step. Replace `5ugbbrmu7ud3u5hsipfzug61p` with your AWS Marketplace product code. For details on the tag format, see [Resource Tagging](resource-tagging.md "resource-tagging.md"). 3. **Verify the tag.**
