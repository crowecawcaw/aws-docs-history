...
In the case of a service disruption, AWS Sustainability will fail over to region `us-west-2`.
When using the SDK, it is recommended to use the following authentication configuration in order to allow requests to fail over to the secondary region:
...
For more information on SDK Authentication, see [Authentication scheme](../../../sdkref/latest/guide/feature-auth-scheme.md "../../../sdkref/latest/guide/feature-auth-scheme.md").
