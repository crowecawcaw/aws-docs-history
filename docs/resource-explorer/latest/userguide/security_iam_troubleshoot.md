...
encounter when working with Resource Explorer and AWS Identity and Access Management (IAM).
...
provided you with the credentials you used to attempt this operation.

For example, the following error occurs when someone assumes the IAM role
`MyExampleRole` tries to use the console to view details about a view but
does not have `resource-explorer-2:GetView` permission.
...
In this case, the person using the role must ask the administrator to update the
...
- To learn whether Resource Explorer supports these features, see [How Resource Explorer works with IAM](security_iam_service-with-iam.md "security_iam_service-with-iam.md").
