...
You can check the status of the background task by viewing the list of indexes on the **[Settings](https://console.aws.amazon.com/resource-explorer/home#/settings)** page or by using the [GetIndex](https://docs.aws.amazon.com/resource-explorer/latest/apireference/API_GetIndex.html) operation. When the asynchronous tasks complete, the `Status` field from the index changes from `UPDATING` to `ACTIVE`. At that time, only results from the local Region appear in query results.
...
1. In the **Indexes** section, select the check box next to the Region that contains the aggregator index that you want to demote to a local index, and then choose **Change index type**.

1. In the **Change index type for <*Region name*>** dialog, choose **Local index**, and then choose **Save changes**.
