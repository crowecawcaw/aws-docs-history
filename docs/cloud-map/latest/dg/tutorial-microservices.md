...
table in DynamoDB](../../../amazondynamodb/latest/developerguide/getting-started-step-1.md "../../../amazondynamodb/latest/developerguide/getting-started-step-1.md") in the _DynamoDB Developer Guide_ and use the
...
2. From the list of namespaces, select the `cloudmap-tutorial` namespace and
   choose **View details**.
...
4. In the **Services** section, select the `data-service` service
   and choose **View details**.
...
boundary because the role is only used for this tutorial, and you can delete it
afterwards.
...
For information about creating a Lambda function, see [Create a Lambda
function with the console](../../../lambda/latest/dg/getting-started.md#getting-started-create-function "../../../lambda/latest/dg/getting-started.md#getting-started-create-function") in the _AWS Lambda Developer
Guide_ and use the following table to determine what options to specify or
choose.
...
and then deploy the function. Note that you're specifying the `datatable` custom
attribute you associated with the AWS Cloud Map service instance you created for the DynamoDB table. The
...
3. From the list of namespaces, select the `cloudmap-tutorial` namespace and
   choose **View details**.
...
5. In the **Services** section, select the `app-service` service
   and choose **View details**.
...
3. From the list of namespaces, select the `cloudmap-tutorial` namespace and
   choose **View details**.
4. In the **Services** section, select the `app-service` service
   and choose **View details**.
5. In the **Service instances** section, choose **Register service
   instance**.
6. On the **Register service instance** page, do the following.
...
1. Open the AWS CloudShell console at [https://console.aws.amazon.com/cloudshell/](https://console.aws.amazon.com/cloudshell/ "https://console.aws.amazon.com/cloudshell/")
...
3. In the `writeclient.py` file, enter insert mode by pressing the `i`
   button. Then, copy and paste the following code. This code discovers the Lambda function to
   write data by searching for the custom attribute `name=writeservice` in the
   `app-service` service. The name of the Lambda function
...
4. Press the escape key, type `:wq`, and press the enter key to save the file and
   exit.
...
6. To verify the write was successful in the previous step, create a read client.
   ...
   by running `writefunction.py` and the random key generated in the Lambda write function.
...
the AWS Cloud Map resources used in the tutorial.
...
3. On the namespace details page, from the list of services, select the
   `data-service` service and choose **View details**.
4. In the **Service instances** section, select the
   `data-instance` instance and choose **Deregister**.
...
7. Repeat steps 3-6 for the `app-service` service and the
   `write-instance` and `read-instance` service instances.
8. In the left navigation, choose **Namespaces**.
9. Select the `cloudmap-tutorial` namespace and choose
   **Delete**.

The following table lists procedures that you can follow to delete the other resources used in
the tutorial.

| Resource                                           | Steps                                                                                                                                                                                                                                                                   |
| -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| DynamoDB table                                     | [Step 6: (Optional)<br>Delete your DynamoDB table to clean up resources](../../../amazondynamodb/latest/developerguide/getting-started-step-6.md "../../../amazondynamodb/latest/developerguide/getting-started-step-6.md") in the _Amazon DynamoDB Developer<br>Guide_ |
| Lambda functions and associated IAM execution role | [Clean up](../../../lambda/latest/dg/getting-started.md#gettingstarted-cleanup "../../../lambda/latest/dg/getting-started.md#gettingstarted-cleanup") in the<br>_AWS Lambda Developer Guide_                                                                            |
