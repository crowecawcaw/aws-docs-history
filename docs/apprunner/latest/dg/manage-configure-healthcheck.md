...
| --- | --- | --- | 
| Protocol | The IP protocol that App Runner uses to perform health checks for your service.<br />If you set the protocol to `TCP`, App Runner pings the default domain assigned to your service at the port that your application is listening to.<br />If you set the protocol to `HTTP`, App Runner sends health check requests to the configured path. | `TCP` | 
