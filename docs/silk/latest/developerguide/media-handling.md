...
## Images
<a name="image-handling"></a>

In addition to all the expected image formats, Amazon Silk supports scalable vector graphics (SVG) both inline via the <svg> tag and externally using the <img>, <object>, and <embed> tags. For more information, see [SVG Element](html5-elements.md#svg).
...
+ **Consider using an image optimizer:** Image optimizers can reduce the size of image files. Available image optimization tools include [TinyPNG](https://tinypng.com/) and the [Grunt imagemin plugin](https://github.com/gruntjs/grunt-contrib-imagemin).

For more information on image optimization, see [Optimizing web graphics](https://developers.google.com/speed/articles/optimizing-images).

## Audio
<a name="audio-handling"></a>

Amazon Silk fully supports the HTML5 [Audio Element](html5-elements.md#audio-element), with playback occurring in the browser. The default controls are improved from earlier Fire devices, and Silk also supports custom controls; playback of multiple streams; and MP3, OGG, and WAV audio formats. JavaScript support is good, enabling, for example, algorithmic PCM Audio using data URIs.
...
Amazon Silk also supports the RTSP and HLS network protocols. For more information about media formats that the Android platform supports, see [Android Supported Media Formats](http://developer.android.com/guide/appendix/media-formats.html).

### Graceful Degradation with the Video Element
<a name="w2aab9c13c11b6"></a>

It's usually a good idea to use [Learn about feature detection](feature-detection.md) for features and media formats that aren't supported by all browsers. To detect video support, you can also try a simpler solution: graceful degradation with the HTML5 video element, which doesn't require JavaScript. In the example below, the video element provides a built-in fallback structure that lets you easily deliver multiple video container formats. 
