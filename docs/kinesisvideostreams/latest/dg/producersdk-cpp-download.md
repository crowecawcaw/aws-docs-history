...
| `CONSTRAINED_DEVICE`      | Boolean | OFF                  | **Non-Windows only.*<br>• When ON, sets<br>the thread stack size to `0.5 MiB`. Needed for [Alpine Linux](https://wiki.alpinelinux.org/wiki/Main_Page "https://wiki.alpinelinux.org/wiki/Main_Page") builds. Otherwise, the operating system default is<br>used.   |
...
`-D`key`=`value``
pairs following the `cmake ..` command. For example:
