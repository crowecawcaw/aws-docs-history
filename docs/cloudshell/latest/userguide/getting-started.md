...
You will use the service quotas for the default Region, Europe (Ireland)
 ...
 notified in the CloudShell browser window.
2. From the AWS Management Console, you can launch CloudShell by choosing one of the following
 options:
...
If you encounter issues that prevent you from successfully launching or
 ...
 issues in [Troubleshooting AWS CloudShell](troubleshooting.md "troubleshooting.md").
3. To choose a pre-installed shell to work with, enter its program name at the
 command line prompt.
...
2. In the **Download file** dialog box, enter the path for the file
 to be downloaded.
...
When you press **Enter**, the shell returns your
 current working directory (for example, `/home/cloudshell-user`).
2. To upload a file to this directory, go to **Actions** and choose
 **Upload file** from the menu.


The **Upload file** dialog box displays.
3. Choose **Browse**.
4. In your system's **File upload** dialog box, select the text file that you
...
A progress bar tracks the upload. If the upload is successful, a message confirms
 ...
 directory.
6. To create a directory for the file, enter the make directories command: `mkdir
 mysub_dir`.
7. To move the uploaded file from the root of your home directory to the new directory, use the
 `mv` command:
...
9. To view the contents of the current directory, `mysub_dir`,
...
2. To remove multiple files that meet specified criteria, run the `find`
 command. 
...
2. To confirm deletion, enter delete in the text input field, and then choose
 **Delete**.
...
4. To create a CloudShell VPC environment, choose **Create a VPC
 environment**.
...
2. To edit the file in Vim, press the **I** key. Now edit the
...
If you paste the text into the editor and have the [Safe
 ...
 safe, choose **Paste**.
3. After you edited the program, press **Esc** to enter the
...
If you're new to the Vim command mode, you might initially find
 ...
 Vim and other tools that are available in AWS CloudShell, see [Development tools and shell utilities](vm-specs.md#utilities-installed "vm-specs.md#utilities-installed").
4. On the main command line interface, run the following program and specify three
...
If you don't adhere to the [rules for naming buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/BucketRestrictions.html#bucketnamingrules "https://docs.aws.amazon.com/AmazonS3/latest/userguide/BucketRestrictions.html#bucketnamingrules"), the following error is displayed:
 **`An error occurred (InvalidBucketName) when calling the CreateBucket
 operation: The specified bucket is not valid.`**
2. To upload a file and add the file as an object to the bucket that you just created,
