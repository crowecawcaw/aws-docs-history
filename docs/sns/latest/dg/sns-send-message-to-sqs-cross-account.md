...
This document describes how to publish a notification to an Amazon SNS topic with one or more subscriptions to Amazon SQS queues in another account. You set up the topic and queues the same way as if they were in the same account (see [Fanout Amazon SNS notifications to Amazon SQS queues for asynchronous processing](sns-sqs-as-subscriber.md)). The major difference is how you handle subscription confirmation. That depends on how you subscribe the queue to the topic.

We recommend following the steps in the [Queue owner creates subscription](#SendMessageToSQS.cross.account.queueowner) section when possible. Confirmation is automatic when the queue owner creates the subscription.
...
1. Enter the following policy:
   ...
   A user with the credentials for account `111122223333` can subscribe to `MyTopic`. This permission lets the account ID delegate permission to their IAM user/role. Only the root account or administrator users can call `sns:Subscribe`. The IAM user/role must also have `sns:subscribe` permission.
...
1. Choose **Subscribe to Amazon SNS topic**.
...
To confirm the subscription, a user with permission to read messages from the queue must retrieve the subscription confirmation URL. The subscription owner must then confirm the subscription using that URL. Until confirmed, no notifications published to the topic are sent to the queue. To confirm, you can use the Amazon SQS console or the `[ReceiveMessage](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/APIReference/Query_QueryReceiveMessage.html)` action.

**Note**  
Before you subscribe an endpoint to the topic, make sure the queue can receive messages from the topic. Set the `sqs:SendMessage` permission for the queue. For more information, see [Step 2: Give permission to the Amazon SNS topic to send messages to the Amazon SQS queue](subscribe-sqs-queue-to-sns-topic.md#SendMessageToSQS.sqs.permissions).
...
1. Choose **Send and receive messages**, and then choose **Poll for messages**.
...
1. In the **Body** column, do the following:
   ...
   1. In the **Message Details** dialog box, find and note the **SubscribeURL** value. This is your subscription link (example below). For more details on API token validation, see [`ConfirmSubscription`](https://docs.aws.amazon.com/sns/latest/api/API_ConfirmSubscription.html) in the Amazon SNS API Reference.
   ...
   1. Make a note of the subscription confirmation link. The URL must be passed from the queue owner to the subscription owner. The subscription owner enters the URL into the [Amazon SNS console](https://console.aws.amazon.com/sns/v3/home).
