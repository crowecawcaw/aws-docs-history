...
| 4 | If no explicit deny is found, the enforcement code looks for any "allow" instructions that would apply to the request.<br />If it finds even one, the enforcement code returns a decision of "allow" and the process is done (the service continues to process the request).  | 
