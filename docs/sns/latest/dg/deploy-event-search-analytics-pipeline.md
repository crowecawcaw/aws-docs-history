...
4. On the **fork-event-search-analytics-pipeline** page, do the
   ...
   one). 2. (Optional) For **DataTransformationFunctionArn**,
   ...
   disabled. 3. (Optional) Enter one of the following **LogLevel**
   ...
   created with the default configuration. 5. For **TopicArn**, enter the ARN of the Amazon SNS topic to
   which this instance of the fork pipeline is to be subscribed. 6. For **SearchIndexName**, enter the name of the OpenSearch Service
   ...
   **SearchIndexRotationPeriod** settings for the
   ...
   expiration of old data. 8. For **SearchTypeName**, enter the name of the OpenSearch Service
   ...
   values, 300 seconds and 5 MB are used. 10. (Optional) Enter one of the following
   **StreamCompressionFormat** settings for
   ...
   enter a value, no prefix is used. 12. (Optional) For **StreamRetryDurationInSecons**, enter
   the retry duration for cases when Firehose can't index events in the OpenSearch Service
   index. If you don't enter a value, then 300 seconds is used. 13. (Optional) For **SubscriptionFilterPolicy**, enter
   ...
   used (all events are indexed). 14. Choose **I acknowledge that this app creates custom IAM roles,
