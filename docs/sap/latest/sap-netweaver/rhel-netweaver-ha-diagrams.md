

# Architecture diagrams
<a name="rhel-netweaver-ha-diagrams"></a>

This guide covers two architectures for SAP cluster solutions on RHEL for SAP – simple-mount and classic (previous standard). See the following images to learn more.

**Topics**
+ [Pacemaker - simple-mount architecture](#simple-mount-diagram-nw-rhel)
+ [Pacemaker - classic architecture](#classic-diagram-nw-rhel)

## Pacemaker - simple-mount architecture
<a name="simple-mount-diagram-nw-rhel"></a>

See the following image for more details.

![Simple Mount Achitecture](https://docs.aws.amazon.com/sap/latest/sap-netweaver/images/image-pacemaker-nw-rhel-simplemount.png)


## Pacemaker - classic architecture
<a name="classic-diagram-nw-rhel"></a>

See the following image for more details.

![Classic Architecture.](https://docs.aws.amazon.com/sap/latest/sap-netweaver/images/image-pacemaker-nw-rhel-classic.png)
