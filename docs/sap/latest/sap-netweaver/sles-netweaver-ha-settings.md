...
- [Shared VPC – optional](#sles-netweaver-ha-shared-vpc "#sles-netweaver-ha-shared-vpc")
...
- Overlay IP – This value is defined by you. For more information, see [Overlay IP](sles-ha.md#overlay-ip "sles-ha.md#overlay-ip").
...
If you are using a Shared VPC, see [Shared VPC – optional](#sles-netweaver-ha-shared-vpc "#sles-netweaver-ha-shared-vpc").

### Assign IAM role

The two cluster resource IAM policies must be assigned to an IAM role associated with your Amazon EC2 instance. If an IAM role is not associated to your instance, create a new IAM role for cluster operations. To assign the role, go to [https://console.aws.amazon.com/ec2/](https://console.aws.amazon.com/ec2/ "https://console.aws.amazon.com/ec2/"), select your instance(s), and then choose **Actions** > **Security** > **Modify IAM role**.
...
Use the same tag key and the local hostname returned using the command `hostname` across instances. For example, a configuration with the values defined in [Global AWS parameters](sles-setup.md#global-aws-parameters "sles-setup.md#global-aws-parameters") would require the tags shown in the following table.
...
You need to add initial route table entries for overlay IPs. For more information on overlay IP, see [Overlay IP](sles-ha.md#overlay-ip "sles-ha.md#overlay-ip").
...
4. To add a route, choose **Add route**. You must choose **Add route** twice to add two routes, one for ASCS and another for ERS.
   ...
   |             |                      |
   | ----------- | -------------------- |
   | Destination | 172.16.30.6/32       |
   | Target      | i-xxxxinstidforhost2 |
...
For more information, see SAP Documentation – [SAP System Directories on UNIX](https://help.sap.com/docs/SAP_NETWEAVER_750/ff18034f08af4d7bb33894c2047c3b71/2744f17a26a74a8abfd202c4f5dc9a0f.html "https://help.sap.com/docs/SAP_NETWEAVER_750/ff18034f08af4d7bb33894c2047c3b71/2744f17a26a74a8abfd202c4f5dc9a0f.html").

The following options can differ depending on how you architect and operate your systems.

- When using the simple mount architecture, you can share at the `/usr/sap/<SID>` level. There is no requirement to manage these file systems with the cluster. In this document, these file systems are separate. This is to simplify the migration, and to ensure that the recommendation to run app server executables from a local copy is followed in case you are co-hosting the ASCS/ERS with an application server.
...
```
mkdir /sapmnt
mkdir /usr/sap/<SLX>/ASCS
mkdir /usr/sap/<SLX>/ERS
...
```
<nfs.fqdn>:/<SLX_ASCS00>  /usr/sap/<SLX>/ASCS<00>   nfs nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2,noresvport 0 0

<nfs.fqdn>:/<SLX_ERS10>  /usr/sap/<SLX>/ERS<10>  nfs nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2,noresvport 0 0
```
...
```
mount <nfs.fqdn>:/<SLX_ERS10>  /usr/sap/<SLX>/ERS
