...
1. Enable Transit Gateway Flow Logs. For more information, see {https---docs-aws-amazon-com-vpc-latest-tgw-flow-logs-s3-html-flow-logs-s3-create-flow-log}[Create a flow log that publishes to Amazon S3].
...
3. Obtain the Transit Gateway data processing charge per-account.
   ...
   2. Calculate the total network traffic and percentage allocation per account using {https---catalog-workshops-aws-cur-query-library-en-US-queries-networking-and-content-delivery-aws-transit-gateway}[AWS Transit Gateway] query.
