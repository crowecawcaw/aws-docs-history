...
For more information, see [Tunnel options for your AWS Site-to-Site VPN connection](https://docs.aws.amazon.com/vpn/latest/s2svpn/VPNTunnels.html).

**Note**  
Pricing examples are for illustration purposes only and are subject to change. For current pricing, see [AWS Site-to-Site VPN pricing](https://aws.amazon.com/vpn/pricing/).

## Standard bandwidth: Up to 1.25 Gbps per tunnel
<a name="_standard_bandwidth_up_to_1_25_gbps_per_tunnel"></a>
...
<a name="_pricing_example_1_single_1_25_gbps_site_to_site_vpn_connection"></a>

**Note**  
Costs vary between AWS Regions. For more information, see [Amazon EC2 pricing Data Transfer](https://aws.amazon.com/ec2/pricing/on-demand/#Data_Transfer).
...
+ Data transfer out fee: The first 100 GB are free, leaving 924 GB (1 TB = 1,024 GB) chargeable at USD 0.09 per GB — a monthly fee of USD 83.16.

The total estimated monthly cost is approximately USD 119.16.
...
<a name="_large_bandwidth_tunnel_up_to_5_gbps_per_tunnel"></a>

With Large Bandwidth Tunnels, you can configure Site-to-Site VPN tunnels that support up to 5 Gbps bandwidth per tunnel, compared to the standard 1.25 Gbps. For more information, see [Large Bandwidth Tunnels](https://docs.aws.amazon.com/vpn/latest/s2svpn/vpn-limits.html).

Large Bandwidth Tunnels work only with VPN connections attached to a Transit Gateway or AWS Cloud WAN. They do not work with Virtual Private Gateways, so you cannot use them to connect directly from your remote network to the RISE with SAP VPC. To use Large Bandwidth Tunnels, route traffic through your own AWS managed account. For more information on this architecture pattern, see [Connecting to RISE using your single AWS account](rise-connection-accounts.md).
...
<a name="_pricing_example_2_single_5_gbps_site_to_site_vpn"></a>
...
+  AWS Transit Gateway fee: At USD 0.05 per hour per attachment in US East (Ohio), the monthly fee for 1 attachment is USD 36.00.
+  AWS Transit Gateway data processing fee: At USD 0.02 per GB, the monthly fee is USD 20.48.
+ Data transfer out fee: The first 100 GB are free, leaving 924 GB (1 TB = 1,024 GB) chargeable at USD 0.09 per GB — a monthly fee of USD 83.16.

The total estimated monthly cost is approximately USD 571.64.

For more information, see [AWS Site-to-Site VPN pricing](https://aws.amazon.com/vpn/pricing/).
...
To scale beyond the 5 Gbps throughput of a single Large Bandwidth Tunnel, see [Achieve ECMP routing with multiple Site-to-Site VPN tunnels associated with a transit gateway](https://repost.aws/knowledge-center/transit-gateway-ecmp-multiple-tunnels).
...
+  **Large Bandwidth Tunnel (up to 5 Gbps)**: Routes through your own AWS account via Transit Gateway. Costs (approximately USD 572 per month) are split between your account and the RISE subscription based on data traffic direction. You might incur some additional cost beyond the subscription.
