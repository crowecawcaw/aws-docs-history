...
| "ec2:DescribeSubnets"                   |
| "ec2:AcceptTransitGatewayVpcAttachment" | Creating a connection between your Managed kdb environment and<br>your transit gateway  | Administrators who configure the transit gateway environment<br>using the `UpdateKxEnvironmentNetwork` API               |
| "ec2:DescribeSubnets"                   |
| "ram:CreateResourceShare"               | Creating a resource share on the transit gateway                                        | Users who update kdb environment                                                                                         |
| “ram:GetResourceShareInvitiations”      | Accepting resource share on private certificate authority for<br>cluster TLS connection | Users who create kdb environment                                                                                         |
| "ram:AcceptResourceShareInvitation"     |
| "iam:CreateServiceLinkedRole"           | Creating the FinSpace service-linked role (SLR) when creating a kdb<br>environment      | Users who create kdb environment                                                                                         |
| "ec2:DescribeTags"                      | Creating and describing tags on FinSpace managed VPC<br>endpoints                       | Users who create and delete clusters                                                                                     |
| "ec2:CreateTags"                        |
...
| "kms:RetireGrant"                       |
| "ec2:DescribeTransitGateways"           | Checking if the transit gateway exists                                                  | Users who configure the transit gateway environment using the<br>UpdateKxEnvironmentNetwork API                          |
...
policy that you can use to grant access to code location.
...
After you grant the FinSpace service access to the S3 bucket , you must ensure that
the IAM role that you use when you [create a
cluster](create-kdb-clusters.md "create-kdb-clusters.md") or when you [update the code on
a cluster](update-cluster-code.md "update-cluster-code.md") has permission to access the files on the Amazon S3 bucket. The
following is an example of the policy that you can use to grant access to the
role.
...
locations a user can access. You can also set `*Deny*` policies on this role to
...
kdb changesets. The following is an example of such a policy.
...
After you grant FinSpace access to the Amazon S3 bucket, you must ensure that the IAM role
you use when you [create a changeset](using-kdb-db.md#kdb-db-changesets "using-kdb-db.md#kdb-db-changesets") has
permission to access the files on the Amazon S3 bucket. The following is an example of
such a policy.
...
can access. You can also set `*Deny*` policies on this role to prevent access to
resources. For example, you can use the `*Deny*` policy to prevent access to resources in another
account.
