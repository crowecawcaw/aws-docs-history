...
1.  `escaper` – Specifies a character to use for escaping. This option is used only when reading CSV files. The default value is none. If enabled, the character that immediately follows is used as-is, except for a small set of well-known escapes (\\n, \\r, \\t, and \\0).
...
1.  `attributePrefix` – A prefix for attributes to differentiate them from elements. This prefix is used for field names. The default value is "\_".
