...
<a name="tutorial-load-data-analyze-finspace"></a>
...
Before you proceed with this section, we recommend that you begin by reading [Adding and managing data in Amazon FinSpace](finspace-add-data.md).
...
<a name="add-data-create-dataset-and-data-view"></a>
...
1. On the next page, drag and drop the `Industrial production total index.csv` file on the page or choose **Browse Files** to select a new file.
...
1. Choose an appropriate permission group that should be associated to the dataset when it gets created. You can add additional permission groups after the dataset creation is complete.
...
1. Choose **Analyze in Notebook** to access data in the data view in the integrated notebook environment.
**Note**  
Starting up the FinSpace notebook environment for the first time may take 10-15 minutes. This is a one-time delay.

## Analyze the data view in Amazon FinSpace notebook
<a name="analyze-the-data-view-in-the-finspace-notebook"></a>

Before you proceed with this section, we recommend that you begin by reading [Working with Amazon FinSpace notebooks](working-with-amazon-finSpace-notebooks.md).

**To analyze the data view in FinSpace notebook**

1. Sign in to the FinSpace web application. For more information, see [Signing in to the Amazon FinSpace web application](signing-into-amazon-finspace.md).

1. Open data view in a notebook. For more information, see [Opening the notebook environment](opening-the-notebook-environment.md).

1. A default notebook in read-only preview is populated with the details of the view. Choose **Create a copy.** The notebook is created with name **notebook.ipynb.** The notebook contains code for:
