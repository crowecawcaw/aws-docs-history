...
permissions, the console won't function as intended for entities (users or roles)
with that policy. To ensure that those entities can use the Amazon Pinpoint console,
...
projects. It also lets users view information about related resources for other AWS
services that the Amazon Pinpoint console depends on, such as Amazon SES, IAM, and Amazon Kinesis. However,
the policy lets users view additional information about only the project that's
...
Similarly, you can create policies that grant a user in your AWS account with
limited write access to one of your Amazon Pinpoint projects, for example the project that has the
`810c7aab86d42fb2b56c8c966example` project ID. In this case, you want to allow the
...
`{
 ...
 "ArnLike": {
 "aws:SourceArn": "arn:aws:mobiletargeting:`us-east-1`:`111122223333`:*"
 }
 }
 }
 ]
...
You can attach this type of policy to the users in your account. If a user named
...
The following example policy allows read-only access to all the resources in your
Amazon Pinpoint account in a specific AWS Region.
...
of an AWS Region, and replace `accountId` with your AWS
account ID.

### Administrator access

The following example policy allows full access to all Amazon Pinpoint actions and resources
...
`{
 ...
 "ArnLike": {
 "aws:SourceArn": "arn:aws:sms-voice:`us-east-1`:`111122223333`:*"
 }
 }
 }
 ]
...
If you use Amazon Pinpoint in the AWS GovCloud (US-West) Region, use the following
policy example instead:
