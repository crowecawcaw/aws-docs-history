...
1. For **Enter your custom message channel URL**, enter the URL of the webhook.
   ...
   The URL that you specify has to begin with "https://." It can only contain alphanumeric characters, plus the following symbols: hyphen (-), period (.), underscore (\_), tilde (\~), question mark (?), slash or solidus (/), pound or hash sign (\#), and semicolon (;). The URL must comply with [RFC3986](https://datatracker.ietf.org/doc/html/rfc3986).
