...
Amazon Pinpoint custom attributes names support spaces. To have a custom attribute called `"Last Name"`, you must format the attribute as `User.UserAttributes.[Last Name]`. To access the first entry in the attribute index list the format is `User.UserAttributes.[Last Name].[0]`.
