...
1. Under **Model details**, for **Recommender model name**, enter a name for the model in Amazon Pinpoint. The name must begin with a letter or number. It can contain up to 128 characters. The characters can be letters, numbers, underscores (\_), or hyphens (‐).

1. (Optional) For **Recommender model description**, enter a brief description of the model. The description can contain up to 128 characters. The characters can be letters, numbers, spaces, or the following symbols: underscores (\_), semicolons (;), parentheses (), commas (,), and hyphens ( ‐).
