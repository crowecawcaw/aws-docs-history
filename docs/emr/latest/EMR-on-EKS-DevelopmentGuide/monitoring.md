...
**Topics**
+ [Monitor jobs with Amazon CloudWatch Events](#monitoring-cloudwatch-events)
+ [Automate Amazon EMR on EKS with CloudWatch Events](#monitoring-automate-emr-on-eks)
+ [Example: Set up a rule that invokes Lambda](#monitoring-example-rule)
+ [Monitor job’s driver pod with a retry policy using Amazon CloudWatch Events](#monitor-jobs-with-retry-policies)
