...
1. Create a file named *RBAC\_Patch.py* as follows:
...
1. A kubectl diff between the new permissions and the old ones appears. Press y to patch the role.

1. Verify the three roles with additional permissions as follows:
...
1. After running the command, it will show a kubectl diff between the new permissions and the old ones. Press y to patch the role.

1. Verify the three roles with additional permissions:

   ```
   kubectl describe role -n ${NAMESPACE}
   ```

1. Submit the job again.
...
1. Based on the permission your application requires, edit each file and add additional rules such as the following:
...
1. Remove the following attributes with their values. This is necessary to apply the update.
   + creationTimestamp
   + resourceVersion
   + uid

1. Finally, run the patch:
