...
2. Edit the `Dockerfile` and add the following content. This `Dockerfile`
   uses the container image you pulled from `public.ecr.aws/emr-on-eks/flink/emr-7.10.0-flink:latest`.

```
FROM public.ecr.aws/emr-on-eks/flink/emr-7.10.0-flink:latest
USER root
...
```
FROM public.ecr.aws/emr-on-eks/fluentd/emr-7.10.0:latest
USER root
### Add customization commands here ####
USER hadoop:hadoop
```
