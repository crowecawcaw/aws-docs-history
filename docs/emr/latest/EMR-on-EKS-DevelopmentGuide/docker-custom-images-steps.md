...
local workspace. For example, for Amazon EMR 7.13.0 release, the following
`docker pull` command gets you the lastest standard base image. You can replace
`emr-7.13.0:latest` with
`emr-7.13.0-spark-rapids:latest` to retrieve the image that has
Nvidia RAPIDS accelerator. You can also replace `emr-7.13.0:latest`
with `emr-7.13.0-java11:latest` to retrieve the image with Java 11
runtime.
