...
<a name="emr-trusted-identity-smus-tip-connect-existing-cluster"></a>
...
+ [Adding existing Amazon EMR on EC2 clusters](https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/adding-existing-emr-on-ec2-clusters.html)
+ [Configuring TIP for Amazon EMR on EC2](https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/emr-ec2-configuring-tip.html)
+ [Enable Amazon EMR on EC2 blueprint](https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/enable-emr-on-ec2-blueprint.html)

## Step 1: Create an access role for Amazon EMR
<a name="emr-trusted-identity-smus-tip-connect-access-role"></a>
...
<a name="emr-trusted-identity-smus-tip-connect-instance-profile"></a>

Add the following inline policy to the EC2 instance profile role on the Amazon EMR on EC2 cluster (as per [Configuring TIP for Amazon EMR on EC2](https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/emr-ec2-configuring-tip.html)):
...
<a name="emr-trusted-identity-smus-tip-connect-blueprint"></a>
...
<a name="emr-trusted-identity-smus-tip-connect-sec-config-association"></a>
...
<a name="emr-trusted-identity-smus-tip-connect-managed-tag"></a>
...
<a name="emr-trusted-identity-smus-tip-connect-project-role"></a>
...
<a name="emr-trusted-identity-smus-tip-connect-certificate"></a>
...
<a name="emr-trusted-identity-smus-tip-connect-cluster-config"></a>
...
<a name="emr-trusted-identity-smus-tip-connect-networking"></a>
...
1. **Find the Amazon SageMaker Unified Studio project security group.** Each Amazon SageMaker Unified Studio project environment has a `securityGroup` resource. Retrieve it from the project's Tooling environment. First, list the environments for your project to get the environment ID, then read the `securityGroup` provisioned resource:
   ...
   The returned value (for example, `sg-{{0abc123smus}}`) is the Amazon SageMaker Unified Studio domain security group used in the following steps.

1. **Open the Livy port (8998) on the Amazon EMR primary node.** Add an ingress rule to the Amazon EMR primary node security group that allows TCP port `8998` from the Amazon SageMaker Unified Studio project security group:
   ...
   Replace `{{emr-primary-node-sg-id}}` with the Amazon EMR managed primary (main) node security group and `{{smus-project-sg-id}}` with the Amazon SageMaker Unified Studio project security group from the previous step.

1. **Add the Amazon EMR interface VPC endpoint to the Amazon SageMaker Unified Studio VPC.** Create an interface endpoint for `com.amazonaws.{{region}}.elasticmapreduce` in the Amazon SageMaker Unified Studio VPC private subnet, associated with the Amazon SageMaker Unified Studio project security group:
