...
+ `spark.hadoop.fs.s3.useDirectoryHeaderAsFolderObject=true` and `spark.hadoop.fs.s3.folderObject.autoAction.disabled=true`: Configure EMRFS to use content type header application/x-directory instead of $folder$ suffix when creating S3 folders. This is required when reading Lake Formation tables, as Lake Formation credentials do not allow reading table folders with $folder$ suffix.
...
| DATAFRAME(BULK\_INSERT) | DATAFRAME(BULK\_INSERT) | saveAsTable.Overwrite | Supported | 
| MOR | DATAFRAME(BULK\_INSERT) | saveAsTable.Append | Not Supported | 
| MOR | DATAFRAME(BULK\_INSERT) | saveAsTable.Ignore | Supported | 
| MOR | DATAFRAME(BULK\_INSERT) | saveAsTable.ErrorIfExists | Supported | 
| MOR | DATAFRAME(BULK\_INSERT) | saveAsTable - External table (Path) | Not Supported | 
| MOR | DATAFRAME(BULK\_INSERT) | save(path) - DF v1 | Not Supported | 