...
**Important**  
If you set `yarn.resourcemanager.nodemanager-graceful-decommission-timeout-secs` to `-1`, Amazon EMR disables the graceful decommission timeout and decommissions the node immediately without waiting for YARN tasks to complete. This differs from open-source YARN, where `-1` means an indefinite wait for graceful termination. To allow more time for tasks to complete before decommissioning, set a high positive timeout value (for example, `86400` for 24 hours).
