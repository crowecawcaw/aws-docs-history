...
1. Type the following command to open an SSH tunnel on your local machine. Replace {{\~/mykeypair.pem}} with the location and file name of your `.pem` file, replace {{8157}} with an unused, local port number, and replace {{ec2-\#\#\#-\#\#-\#\#-\#\#\#.compute-1.amazonaws.com}} with the primary public DNS name of your cluster. 
...
1. Type the following command to open an SSH tunnel to the primary node using dynamic port forwarding. In the following example, replace {{j-2AL4XXXXXX5T9}} with the cluster ID and replace {{\~/mykeypair.key}} with the location and file name of your `.pem` file (for Linux, Unix, and Mac OS X) or `.ppk` file (for Windows).
