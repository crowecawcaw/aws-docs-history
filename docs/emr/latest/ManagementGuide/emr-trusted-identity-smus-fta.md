...
- Complete the [Connect existing Amazon EMR on EC2 clusters to Amazon SageMaker Unified Studio](emr-trusted-identity-smus-tip-connect-existing-cluster.md "emr-trusted-identity-smus-tip-connect-existing-cluster.md") steps to prepare the connection to an existing cluster.

## Step 1: Create an Amazon EMR on EC2 cluster for FTA

Create an Amazon EMR security configuration with IAM Identity Center enabled but _without_ Lake Formation enabled:
...
Launch the Amazon EMR cluster with this security configuration and the software settings specified in [Step 8: Configure Amazon EMR cluster software settings](emr-trusted-identity-smus-tip-connect-existing-cluster.md#emr-trusted-identity-smus-tip-connect-cluster-config "emr-trusted-identity-smus-tip-connect-existing-cluster.md#emr-trusted-identity-smus-tip-connect-cluster-config").
...
2. Choose **Add compute** > **Connect to existing compute resources** > **Amazon EMR on EC2 cluster**.
3. Enter the cluster ID and access role ARN (`EMRAccessRole` created in the connection prerequisites).
4. Verify the cluster appears under the **Data Processing** tab.
...
3. In the first cell, select **Python** from the language dropdown for the cell, then configure Spark for Full Table Access using the `%%configure` magic command:
