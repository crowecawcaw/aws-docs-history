...
[GetTemporaryDataLocationCredentials](../../../lake-formation/latest/APIReference/API_GetTemporaryDataLocationCredentials.md "../../../lake-formation/latest/APIReference/API_GetTemporaryDataLocationCredentials.md") API to obtain temporary credentials for S3
paths that belong to Lake Formation-registered tables.
...
The existing Full Table Access feature uses the
[GetTemporaryGlueTableCredentials](../../../lake-formation/latest/APIReference/API_GetTemporaryGlueTableCredentials.md "../../../lake-formation/latest/APIReference/API_GetTemporaryGlueTableCredentials.md") API, which vends credentials based on
table names. The Lake Formation Access Grants Plugin uses the
[GetTemporaryDataLocationCredentials](../../../lake-formation/latest/APIReference/API_GetTemporaryDataLocationCredentials.md "../../../lake-formation/latest/APIReference/API_GetTemporaryDataLocationCredentials.md") API, which vends credentials based on
S3 paths.

You can use both features together. When both are enabled, FTA credentials take
precedence for table-name queries, while the plugin handles direct S3 path
queries.
...
- In the Lake Formation console, register the S3 path with the owner account:
  ...
  2.  Select the location and enable **Register S3 path
      with Owner Account**.
...
When fallback is enabled, the plugin attempts to obtain credentials in the following
order:

1. **Lake Formation** – The plugin calls
   [GetTemporaryDataLocationCredentials](../../../lake-formation/latest/APIReference/API_GetTemporaryDataLocationCredentials.md "../../../lake-formation/latest/APIReference/API_GetTemporaryDataLocationCredentials.md"). If Lake Formation grants access, the
   plugin returns those credentials.
2. **S3 Access Grants** – If Lake Formation denies
   access, the plugin checks whether S3 Access Grants can provide credentials for
   the requested path. If an S3 Access Grants instance covers the path and the
   caller has a matching grant, the plugin uses those credentials.
3. **IAM role** – If both Lake Formation and S3
   Access Grants deny access, the plugin falls back to the IAM role credentials
   attached to the job.

## Considerations and limitations

- The plugin supports Apache Hive, Apache Hudi, and Delta Lake table formats.
  Apache Iceberg is not currently supported.
- The plugin is not currently supported with Amazon EMR Spark Fine-Grained Access
  Control (FGAC) mode.
- Path-based credential vending requires a 1:1 mapping between a table and its
  S3 location in the AWS Glue Data Catalog.
