...
Catalog where the job runtime role has full table permissions. This capability allows you to read and write to tables that are
protected by Lake Formation from your EMR Spark batch and interactive jobs.

## Using Lake Formation with full table access

You can access AWS Lake Formation protected Glue Data catalog tables from EMR Spark jobs or interactive sessions where
the job's runtime role has full table access. You do not have to enable AWS Lake Formation on the EMR
cluster. When a Spark job is configured for Full Table Access (FTA), AWS Lake Formation credentials are used to
read/write S3 data for AWS Lake Formation registered tables, while the job's runtime role credentials will be used to read/write tables not
registered with AWS Lake Formation.

###### Note

Lake Formation fine-grained access control isn't required or supported for a job in order to run Full Table Access (FTA).

###### Note

Full table access does not require use of an EMR runtime role, and it works with both the runtime role and service role for cluster EC2 instances. For more information,
see [Service role for cluster EC2 instances
(EC2 instance profile)](emr-iam-role-for-ec2.md "emr-iam-role-for-ec2.md") or [Runtime roles for Amazon EMR steps](emr-steps-runtime-roles.md "emr-steps-runtime-roles.md").

### Step 1: Enable Full Table Access in Lake Formation

To use Full Table Access (FTA) mode, you need to allow third-party query engines to access data without the IAM session tag
...
When accessing cross account tables, full-table access must be enabled in both the producer and consumer accounts. In the same manner, when accessing
...
For more information, see [Granting permissions on Data Catalog resources](../../../lake-formation/latest/dg/granting-catalog-permissions.md "../../../lake-formation/latest/dg/granting-catalog-permissions.md").
...
To enable Data Catalog for EMR on EC2, see [Use AWS Glue Data Catalog with Spark on Amazon EMR](../ReleaseGuide/emr-spark-glue.md "../ReleaseGuide/emr-spark-glue.md").
...
Configure EMR Filesystem (EMRFS) to use AWS Lake Formation S3 credentials for Lake Formation registered tables. If the table is not
...
To access Lake Formation registered tables from interactive Spark sessions in JupyterLab notebooks, you must use compatibility permission
...
- Jobs referencing tables with Lake Formation Fine-Grained Access Control (FGAC) rules or Glue Data Catalog Views will fail.
...
- When writing Spark DataFrame to a Lake Formation table, only APPEND mode is
  supported: `df.write.mode("append").saveAsTable(`table_name`)`
