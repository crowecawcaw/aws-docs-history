...
When you create a cluster, you choose one of two configurations to organize your nodes: *instance groups* or *instance fleets*. With instance groups, each group contains Amazon EC2 instances of the same type, and you can have multiple groups per node type (except primary). With instance fleets, each node type has a single fleet that can contain a mix of instance types, with target capacities for On-Demand and Spot Instances. This choice is permanent and can't be changed after cluster creation. For more information, see [Create an Amazon EMR cluster with instance fleets or uniform instance groups](emr-instance-group-configuration.md).
