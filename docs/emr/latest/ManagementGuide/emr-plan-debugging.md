...
This configuration uploads application logs only to the customer S3 bucket, and sets emr-managed policy for system logs. The unspecified log type (persistent UI logs) follows default (emr-managed) behavior.
