...
2. EMR Studio passes an identity-enhanced token to the EMR cluster in Account B.
3. The EMR cluster's instance profile role acts as a carrier role, exchanging the token for temporary credentials that include the user's identity context.
4. When the Spark job accesses a resource link pointing to Account A's Data Catalog, Lake Formation evaluates permissions based on the IAM Identity Center user or group.
5. Lake Formation in Account A authorizes access to the underlying Amazon S3 data based on the granted permissions.
...
- Trusted identity propagation is configured for single-account access as described in .
...
3. Attach a policy that allows the role to access the Amazon S3 location:
...
2. Choose **Allow external engines to filter data from these AWS accounts**.
3. Add the Account B ID to the list of allowed AWS account IDs.
4. Select **Amazon EMR** as the allowed engine.
5. Choose **Save**.
...
2. Open the AWS RAM console.
3. In the navigation pane, choose **Shared with me** > **Resource shares**.
4. Find the pending invitation from Account A and choose **Accept resource share**.
...
2. Select **Resource link**.
3. For **Resource link name**, enter a local name (for example, `cross_account_db_link`).
4. For **Shared database**, select the database shared from Account A.
5. Choose **Create**.
...
2. Under **Principals**, choose **IAM Identity Center** and select the users or groups.
3. Under catalog resources, select the resource link database (`cross_account_db_link`).
4. Under **Resource link permissions**, select **Describe**.
5. Choose **Grant**.

**Grant permissions on the target resource**

1. In the Lake Formation console in Account B, choose **Grant** again.
2. Under **Principals**, choose **IAM Identity Center** and select the same users or groups.
3. Under catalog resources, select the shared table from Account A.
4. Under **Table permissions**, select **Select** and **Describe**.
5. Choose **Grant**.
...
2. Attach a Workspace to the EMR cluster you launched.
3. Open a Spark notebook and run the following test query:

```
spark.sql("SHOW DATABASES").show()
```

You should see `cross_account_db_link` in the output. 4. Query the cross-account table:
...
5. Verify identity propagation by checking CloudTrail. In Account A's CloudTrail logs, look for `lakeformation:GetDataAccess` events. The `onBehalfOf` field should show the IAM Identity Center user ID who made the request.
