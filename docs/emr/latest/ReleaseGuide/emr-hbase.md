...
**Important**  
For Amazon EMR HBase cluster scaling, we do not recommend using [managed scaling](https://docs.aws.amazon.com/emr/latest/ManagementGuide/emr-managed-scaling.html) or [scaling with custom policies](https://docs.aws.amazon.com/emr/latest/ManagementGuide/emr-automatic-scaling.html) with HBase clusters.
...
For the version of components installed with HBase in this release, see [Release 6.15.0 Component Versions](emr-6150-release.md).
...
**Note**  
Apache HBase HBCK2 is a separate operational tool for repairing HBase regions and system tables. In Amazon EMR version 6.1.0 and later, the hbase-hbck2.jar is provided in `/usr/lib/hbase-operator-tools/` on the primary node. For more information about how to build and use the tool, see [HBase HBCK2](https://github.com/apache/hbase-operator-tools/tree/master/hbase-hbck2). 
...
For the version of components installed with HBase in this release, see [Release 5.36.2 Component Versions](emr-5362-release.md).
...
+ [Write-ahead logs (WAL) for Amazon EMR](emr-hbase-wal.md)
...
+ [Monitoring EMR HBase with Amazon CloudWatch](emr-hbase-cw.md)
