...
| strict-mode-restrictions | Specifies the restrictions to apply when strict mode is turned on. Strict mode supports the following restrictions: MANDATORY\_PARTITION\_PREDICATE, DISALLOW\_CROSS\_JOIN, and LIMITED\_SORT. | MANDATORY\_PARTITION\_PREDICATE,DISALLOW\_CROSS\_JOIN, LIMITED\_SORT | 
...
1. Choose **Create cluster** and select Amazon EMR version 6.8.0, and Presto or Trino. For more information, see [Installing PrestoDB and Trino](https://docs.aws.amazon.com/emr/latest/ReleaseGuide/emr-presto-considerations.html#emr-prestodb-prestosql). 

1. Specify the configuration properties for strict mode directly, or upload a JSON file to Amazon S3. Optionally, select the AWS Glue data catalog for your metastore. Specify your VPC, subnets, bootstrap actions, key pair, and security group. Choose **Create cluster** to create your cluster. 

1. Log in to the primary node of the cluster and run `presto-cli` or `trino-cli`. 

1. Submit your queries. Strict mode validates each query and determines if it is long-running. Depending on your `strict-mode-fail-query` setting, Amazon EMR rejects the query or raises a warning. 

1. When you're finished with your queries, terminate the cluster and delete your resources. 
...
1. Run the following commands with your desired values. 
