...
2. In the **Dashboard**, in the **Metrics** section,
   choose **EMRFS**.
3. In the **Job Flow Metrics** pane, select one or more
...
1. To read messages from an Amazon SQS queue, type the following command.
...
You can also use the `-q` and `-o`
shortcuts instead of `--queue-name` and
`--output-file` respectively. 2. To delete an Amazon SQS queue, type the following command:
...
You can also use the `-q` shortcut instead of
`--queue-name`.
