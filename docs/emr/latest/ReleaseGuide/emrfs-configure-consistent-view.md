...
```
aws emr create-cluster --release-label `emr-7.13.0` --instance-type `m5.xlarge` \
...
```
aws emr create-cluster --release-label `emr-7.13.0` --applications Name=Hive \
...
`emrfs-site.xml` Properties for consistent view| Property | Default value | Description |
| --- | --- | --- |
