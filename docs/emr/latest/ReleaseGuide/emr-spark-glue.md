...
1. Under **Amazon EMR on EC2** in the left navigation pane, choose **Clusters**, and then choose **Create cluster**.

1. Under **Application bundle**, choose **Spark** or **Custom**. If you customize your cluster, make sure that you select Zeppelin or Spark as one of your applications.

1. Under **AWS Glue Data Catalog settings**, select the **Use for Spark table metadata** check box.
...
For more information about specifying a configuration classification using the AWS CLI and Amazon EMR API, see [Configure applications](emr-configure-apps.md).
...
+ If you use the Spark session catalog with Iceberg, which is described in [Configuration differences when you use Iceberg SparkCatalog versus SparkSessionCatalog](emr-iceberg-use-spark-cluster.md#emr-iceberg-spark-catalog), you must configure the AWS Glue Data Catalog as the Apache Hive metastore, in addition to configuring the AWS Glue Data Catalog as the Apache Iceberg REST catalog.
