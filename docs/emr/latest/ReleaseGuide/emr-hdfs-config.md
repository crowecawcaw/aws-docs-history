...
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `dfs.block.size`  | The size of HDFS blocks. When operating on data stored in HDFS,<br>the split size is generally the size of an HDFS block. Larger<br>numbers provide less task granularity, but also put less strain on<br>the cluster `NameNode`.                                              | 134217728 (128 MB)                                                                                                                                                              |
| `dfs.replication` | The number of copies of each block to store for durability. Amazon EMR<br>sets this value based on the number of core nodes the cluster is<br>provisioned with. Adjust the value to meet your needs. To overwrite<br>the default value, use the `hdfs-site`<br>classification. | `1` for clusters that are provisioned with less<br>than four core nodes<br>`2` for clusters that are provisioned with less<br>than ten core nodes<br>`3` for all other clusters |
