...
**Warning**  
Setting `dfs.replication` to 1 on clusters with fewer than four nodes can lead to HDFS data loss if a single node goes down. If your cluster has HDFS storage, we recommend that you configure the cluster with at least four core nodes for production workloads to avoid data loss.
...
When you use managed Scaling, auto-scaling, or choose to manually resize your cluster, we recommend that you set `dfs.replication` to `2` or higher.
