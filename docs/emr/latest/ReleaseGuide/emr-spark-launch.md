...
The following procedure creates a cluster with [Spark](https://aws.amazon.com/big-data/what-is-spark/ "https://aws.amazon.com/big-data/what-is-spark/") installed using
**Quick Options** in the Amazon EMR console.
...
To configure Spark when you are creating the cluster, see [Configure Spark](emr-spark-configure.md "emr-spark-configure.md").
...
```
aws emr create-cluster --name "Spark cluster" --release-label `emr-7.11.0` --applications Name=Spark \
...
Specify Spark as an application with `SupportedProductConfig` used in
`RunJobFlowRequest`.

- The following example shows how to create a cluster with Spark using
  Java.
