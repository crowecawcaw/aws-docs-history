...
The following procedure creates a cluster with [Spark](https://aws.amazon.com/big-data/what-is-spark/) installed using **Quick Options** in the Amazon EMR console.

You can alternatively use **Advanced Options** to further customize your cluster setup, or to submit steps to programmatically install applications and then run custom applications. With either cluster creation option, you can choose to use AWS Glue as your Spark SQL metastore. See [Use AWS Glue Data Catalog with Spark on Amazon EMR](emr-spark-glue.md) for more information.
...
1. Enter a **Cluster name**. Your cluster name can't contain the characters <, >, $, \|, or ` (backtick).
