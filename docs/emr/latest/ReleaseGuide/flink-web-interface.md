...
The Application Master that belongs to the Flink application hosts the Flink web
interface. It is an alternative way to submit a JAR as a job or to view the current
status of other jobs. The Flink web interface is active as long as you have a Flink
session running. If you have a long-running YARN job already active, you can follow the
instructions in the [Connect
to the primary node with SSH](../ManagementGuide/emr-connect-master-node-ssh.md "../ManagementGuide/emr-connect-master-node-ssh.md") topic in the
_Amazon EMR Management Guide_ to connect to the YARN ResourceManager. For
