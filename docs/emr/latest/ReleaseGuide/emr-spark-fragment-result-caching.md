...
1. Create a cache bucket in Amazon S3 and authorize read/write access for EMRFS. For more information, see [Authorizing access to EMRFS data in Amazon S3](emr-plan-credentialsprovider.md).

1. Set Amazon EMR Spark config to enable the feature.
...
1. Enable S3 lifecycle management for the bucket to automatically clean cache files.

1. Optionally, configure the reductionRationThreshold and maxBufferSize properties to further tune the feature.
