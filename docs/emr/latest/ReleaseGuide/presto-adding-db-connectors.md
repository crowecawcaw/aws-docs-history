...
`presto-connector`, for example,
`presto-connector-postgresql`. The available configuration classifications
depend on the Amazon EMR release version. For the configuration classifications available
with the most recent release version, see [5.36.2 configuration classifications](emr-5362-release.md#emr-5362-class "emr-5362-release.md#emr-5362-class") for Amazon EMR 5.36.2. If
...
example, `myConfig.json`—with the following content, and
save it locally.

Replace the connection properties as appropriate for your setup and as shown in
the [PostgreSQL connector](https://prestodb.io/docs/current/connector/postgresql.html "https://prestodb.io/docs/current/connector/postgresql.html") topic in Presto Documentation.
...
`--configurations` option as shown in the following example, where
`myConfig.json` is in the same directory where you run the
command:
