...
- Fixed an issue with the EMRFS CLI where it did not use the
  default metadata table name.
- Fixed an issue encountered when using ORC-backed tables in
  Amazon S3.
- Fixed an issue encountered with a Python version mismatch in
  the Spark configuration.
- Fixed an issue when a YARN node status fails to report because
  of DNS issues for clusters in a VPC.
- Fixed an issue encountered when YARN decommissioned nodes,
  resulting in hung applications or the inability to schedule
  new applications.
- Fixed an issue encountered when clusters terminated with
  status TIMED\_OUT\_STARTING.
- Fixed an issue encountered when including the EMRFS Scala
  dependency in other builds. The Scala dependency has been
  removed.
...
emr-4.2.0 classifications| Classifications | Description |
| --- | --- |
