...
+ DDL and DML operations that modify table data using AWS Lake Formation-vended credentials support only S3A. When you use EMRFS, the job runtime role credentials apply instead.
+ EMR Serverless with Lake Formation does not support reading from or writing to tables that point to a non-S3 location.
