...
1. [Enable IAM Identity Center](../../../singlesignon/latest/userguide/enable-identity-center.md "../../../singlesignon/latest/userguide/enable-identity-center.md") in your AWS account. An IAM Identity Center instance can only exist in a single Region per account.

###### Important

Note which Region your IAM Identity Center is deployed in. All subsequent resources (Lake Formation, EMR Serverless, AWS Glue) must be created in the same Region. For example, if your IAM Identity Center is in us-west-2, all resources must also be in us-west-2. 2. [Provision users and groups](../../../singlesignon/latest/userguide/tutorials.md "../../../singlesignon/latest/userguide/tutorials.md") into IAM Identity Center, either manually or by syncing from an external identity provider (such as Okta or Microsoft Entra ID). 3. Note your IAM Identity Center instance ARN. You will need it in subsequent steps. Find it in the IAM Identity Center console under **Settings**.
...
2. Set up a [data lake administrator](../../../lake-formation/latest/dg/admins-setup.md "../../../lake-formation/latest/dg/admins-setup.md").
...
3. Create the IAM Identity Center integration for Lake Formation using the instance ARN you noted in Step 1:
...
4. Create an IAM role for Lake Formation location registration. Do not use the service-linked role for trusted identity propagation. For more information, see [Setting up AWS Lake Formation with IAM Identity Center](../../../singlesignon/latest/userguide/tip-tutorial-lf.md "../../../singlesignon/latest/userguide/tip-tutorial-lf.md").
...
5. Register your Amazon S3 data location with Lake Formation using the role you created:
...
6. Create a database and table in the AWS Glue Data Catalog pointing to your sample data.
7. Grant IAM Identity Center users or groups permissions on both the database and table.
...
**Using the console:**
    ...
    2. Under **Principals**, choose **IAM Identity Center users and groups** and select your user or group.
    3. Under **Named Data Catalog resources**, select your database.
    4. Under **Database permissions**, select **Describe**.
    5. Choose **Grant**.
    6. Repeat steps 1 through 5, this time selecting the table and granting **Select** and **Describe** under **Table permissions**.
...
For additional downstream service configurations, see [Using Lake Formation with EMR Serverless](lake-formation-section.md "lake-formation-section.md") and [Using Amazon S3 Access Grants with EMR Serverless](access-grants.md "access-grants.md").
...
3. In the project settings, ensure the profile has `enableTrustedIdentityPropagationPermissions` set to `true` in the blueprint parameters.
...
2. Select the EMR Serverless connection as the compute for notebook execution.
3. In a new notebook cell, run the following PySpark queries:
...
4. Verify that the query returns data. This confirms that your IAM Identity Center identity was propagated through to Lake Formation and access was granted based on your identity.
...
| `TABLE_OR_VIEW_NOT_FOUND`                                                | The Spark session is not configured to use the AWS Glue Data Catalog.                        | Ensure the EMR Serverless application includes `spark.sql.catalogImplementation=hive` and the AWS Glue metastore factory class in its `runtimeConfiguration` (see Step 3). |
...
2. Remove the Amazon SageMaker Unified Studio connection and delete the Amazon SageMaker Unified Studio project (and domain, if you created it only for this tutorial).
3. Revoke the Lake Formation permissions you granted, and deregister the Amazon S3 data location.
4. Delete the IAM roles you created in Step 2 and Step 4.
5. Delete any test Amazon S3 buckets and sample data.
...
- For more information about using EMR Serverless with Amazon SageMaker Unified Studio, see [EMR Serverless in Amazon SageMaker Unified Studio](../../../next-generation-sagemaker/latest/userguide/emr-serverless.md "../../../next-generation-sagemaker/latest/userguide/emr-serverless.md").
