...
encounter when working with Amazon EMR Serverless and IAM.
...
The following example error occurs when the `mateojackson` user tries to use the console to access
details about a fictional `my-example-widget` resource but does not have the
fictional `emr-serverless:`GetWidget`` permissions.
...
`emr-serverless:`GetWidget`` action.
...
Amazon EMR Serverless. However, the action requires the service to have permissions that are granted by a service role. Mary does not have permissions to pass the
role to the service.
...
your VPC policy to S3 gateway endpoint for EMR Serverless to store
and serve application logs.
