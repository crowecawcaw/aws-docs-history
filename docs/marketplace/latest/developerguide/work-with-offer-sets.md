...
This section provides detailed information about creating and managing offer sets using AWS Marketplace Catalog API [change types](../APIReference/API_Change.md "../APIReference/API_Change.md").
...
- `Entity` (object) (required) – The entity being updated.
  ...
  - `Identifier` (string) (required) – Your offer set ID. For more information, see [Identifier](catalog-apis.md#identifier "catalog-apis.md#identifier").
  - `Type` (string) (required) – Must be `OfferSet@1.0`.
...
For information about required IAM permissions, see [IAM permissions for offer sets](#iam-permissions-offer-sets "#iam-permissions-offer-sets").
...
Released offer sets generate notifications at two levels: individual offers and the offer set itself. All individual offer notifications include the OfferSetId in existing notification channels ([email notifications](../userguide/email-notifications.md "../userguide/email-notifications.md") and [Amazon EventBridge events](../userguide/notifications-eventbridge.md "../userguide/notifications-eventbridge.md")). Offer set-level notifications are sent for key events such as when the offer set is released.
...
2. For offers that don't require changes, clone existing offers through the AWS Marketplace Management Portal
3. For offers requiring modifications, create new offers with the requested changes, specifying the new `OfferSetId`
4. Associate all offers (cloned and new) with the new offer set using `AssociateOffers`
5. Release the new offer set using `ReleaseOfferSet`
6. Expire the original offer set by setting the availability end date of associated offers
