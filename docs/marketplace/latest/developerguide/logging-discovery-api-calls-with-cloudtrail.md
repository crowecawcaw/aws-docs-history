...
The AWS Marketplace Discovery API is integrated with AWS CloudTrail, a service that provides a record
of actions taken by a user, role, or an AWS service. CloudTrail captures all Discovery API
calls as events, including calls from the AWS SDK and the AWS CLI.

## Discovery API information in CloudTrail

CloudTrail is enabled on your AWS account when you create the account. When activity
occurs in the Discovery API, that activity is recorded in a CloudTrail event along with
other AWS service events in **Event history**.

All Discovery API actions are logged by CloudTrail. The following operations generate
...
The event source for Discovery API events is
