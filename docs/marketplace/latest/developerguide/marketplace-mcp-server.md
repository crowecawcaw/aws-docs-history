...
- [Available tools](#marketplace-mcp-available-tools "#marketplace-mcp-available-tools")
- [Integration guide](#marketplace-mcp-integration-guide "#marketplace-mcp-integration-guide")
- [Tool details](#marketplace-mcp-tool-details "#marketplace-mcp-tool-details")
- [Workflows](#marketplace-mcp-workflows "#marketplace-mcp-workflows")
- [End-to-end example](#marketplace-mcp-end-to-end-example "#marketplace-mcp-end-to-end-example")
- [Error handling](#marketplace-mcp-error-handling "#marketplace-mcp-error-handling")
...
1. Open the [AWS Marketplace connector](https://claude.ai/directory/connectors/2ed43e1e-f547-48a3-85cc-b9baa412d06b "https://claude.ai/directory/connectors/2ed43e1e-f547-48a3-85cc-b9baa412d06b") in the Claude Connectors Directory.
...
5. Ask Claude to find, compare, or evaluate AWS Marketplace products — it automatically uses the MCP tools.

For more details on how connectors work, see [Use connectors to extend Claude's capabilities](https://support.claude.com/en/articles/11176164-use-connectors-to-extend-claude-s-capabilities "https://support.claude.com/en/articles/11176164-use-connectors-to-extend-claude-s-capabilities").
...
2. An HTTP client capable of making JSON-RPC 2.0 requests.
...
| --------------- | ---- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `workflow_type` | enum | Yes      | `"recommendations"` for ranked product discovery, `"comparison"` for side-by-side analysis, `"evaluation"` for single-product assessment. |
...
| `max_results` | number   | No       | Maximum results per query (1-100, default: 10).                                                       |
...
| Field          | Type     | Required | Description                                                                                                                  |
| -------------- | -------- | -------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `solution_ids` | string[] | Yes      | Array of solution IDs (1-10). IDs start with `prodview-`. Also accepts legacy `solution_id` (single string, auto-converted). |
...
| Field          | Type     | Required | Description                                                                                      |
| -------------- | -------- | -------- | ------------------------------------------------------------------------------------------------ |
| `solution_ids` | string[] | Yes      | Array of solution IDs (1-10). Also accepts legacy `solution_id` (single string, auto-converted). |
...
| Field          | Type              | Required | Description                                         |
| -------------- | ----------------- | -------- | --------------------------------------------------- |
| `solution_ids` | string[]          | Yes      | Array of solution IDs in `prodview-*` format (1-5). |
...
| `results[].solution_id`                 | string                 | The researched solution ID.                  |
| `results[].solution_name`               | string                 | Product name resolved from solution ID.      |
...
| `feedback_text` | string   | No       | Free-form text (max 1000 characters).                                             |
...
2. The tool returns step-by-step workflow instructions and an output format template.
3. The AI assistant follows the returned instructions, which typically involve calling catalog API and research tools to gather data.
4. The assistant generates a formatted report following the output template.
5. After delivering the report, the assistant collects user feedback through `submit_aws_marketplace_feedback`.

**Recommendations workflow:**

1. `get_aws_marketplace_report_guidelines({ workflow_type: "recommendations" })`
2. `search_aws_marketplace_solutions({ queries: ["Product A", "Product B"], max_results: 3 })`
3. Write report following the format template.
4. `submit_aws_marketplace_feedback({ sentiment: "positive" })`

**Comparison workflow:**

1. `get_aws_marketplace_report_guidelines({ workflow_type: "comparison" })`
2. `get_aws_marketplace_solution({ solution_ids: ["prodview-abc", "prodview-xyz"] })`
3. `research_aws_marketplace_solution({ solution_ids: [...], sections: ["features", "integrations"] })`
4. Build comparison matrix and write report following the format template.
5. `submit_aws_marketplace_feedback({ sentiment: "positive" })`

**Evaluation workflow:**

1. `get_aws_marketplace_report_guidelines({ workflow_type: "evaluation" })`
2. `get_aws_marketplace_solution({ solution_ids: ["prodview-target"] })`
3. `research_aws_marketplace_solution({ solution_ids: ["prodview-target"], sections: ["features", "reviews", "limitations", "case_studies"] })`
4. Write evaluation report following the format template.
5. `submit_aws_marketplace_feedback({ sentiment: "positive" })`
...
2. Use the returned `solution_id` values to call `get_aws_marketplace_solution` for full details.
3. Optionally call `get_aws_marketplace_related_solutions` to discover related products.
...
2. If positive, call `submit_aws_marketplace_feedback` with `sentiment: "positive"`.
3. If negative, ask which categories apply (`irrelevant`, `incomplete`, `inaccurate`, `other`), optionally collect free-text feedback, and submit.
