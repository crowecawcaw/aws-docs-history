...
# Managing AMI agreement events with Amazon EventBridge
<a name="ami-eventbridge-integration"></a>

You can use Amazon EventBridge to receive notifications about changes to customer agreements for your AMI products on AWS Marketplace. Events are sent for agreement lifecycle changes, so you can track when customers subscribe, amend, or cancel. These events serve two primary roles:
...
## EventBridge events for AMI products
<a name="ami-eventbridge-event-types"></a>
...
**Note**  
License events are only sent for AMI agreements that have an associated license.  
AWS handles metering automatically for AMI products – you do not need to take any action in response to these events to fulfill customer entitlements. Use license events to keep your internal records current with the customer's active license state.
...
## Integrate EventBridge with your AMI product
<a name="ami-eventbridge-using"></a>

Use EventBridge to track agreement lifecycle events for your AMI listing in AWS Marketplace.
