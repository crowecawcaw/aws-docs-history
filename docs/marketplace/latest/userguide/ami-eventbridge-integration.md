...
AMI products on AWS Marketplace. Events are sent for agreement lifecycle changes, so you can track
when customers subscribe, amend, or cancel. These events serve two primary roles:
...
License events are only sent for AMI agreements that have an associated
license.

AWS handles metering automatically for AMI products – you do not need to
take any action in response to these events to fulfill customer entitlements. Use
license events to keep your internal records current with the customer's active
license state.
