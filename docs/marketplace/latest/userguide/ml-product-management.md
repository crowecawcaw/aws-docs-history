# Managing your machine learning products

In AWS Partner Central, choose **Request changes** to modify a product or version in AWS Marketplace.
When you submit your changes, the system processes them. Processing time varies from minutes to days, depending on the type of modification.
You can monitor the status of your changes in AWS Partner Central.

###### You can modify ML products in AWS Marketplace through self-service by:

- [Updating product information](ml-update-product.md "ml-update-product.md")
- [Updating product visibility](ml-update-visibility.md "ml-update-visibility.md")
- [Updating the allowlist](ml-update-allowlist.md "ml-update-allowlist.md")
- [Managing product versions](ml-manage-product-version.md "ml-manage-product-version.md")
- [Updating product pricing](ml-update-public-offer.md "ml-update-public-offer.md")
- [Updating your refund policy](ml-update-refund-policy.md "ml-update-refund-policy.md")
- [Updating your EULA](ml-update-eula.md "ml-update-eula.md")
- [Removing a product](ml-remove-a-product.md "ml-remove-a-product.md")

###### Note

In addition to making changes through AWS Partner Central, you can also
make changes using the [AWS Marketplace Catalog API](../APIReference/welcome.md "../APIReference/welcome.md").
