...
This data feed lists how product IDs and offer IDs map to legacy globally unique identifiers
(GUIDs). The legacy GUIDs were used in older reports, and the new IDs are used in data feeds and
in AWS Marketplace APIs.
...
The legacy mapping data feed is refreshed every 24 hours, so new data is available
daily.
...
| new\_id       | The friendly ID for this product or offer. This ID is used as the primary key and<br>with all current API actions. |
