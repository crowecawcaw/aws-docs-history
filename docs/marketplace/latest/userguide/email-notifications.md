...
| Payment request - Auto approval | Auto approval: Payment creation \+ reminder (email sent 1, 3, 7, 9 days after payment request creation) | Buyer | You received an AWS Marketplace payment request | Auto Approval Date, Offer ID, Agreement ID, Payment request ID, Payment request created date, Payment request amount, Payment request currency code | 
| Payment request - Manual approval | Manual approval: Payment creation \+ reminder (email sent every 7 days until contract date) | Buyer | You received an AWS Marketplace payment request | Offer ID, Agreement ID, Payment request ID, Payment request created date, Payment request amount, Payment request currency code | 
...
The `Resource ARN` field shows the invalid bank account Amazon Resource Number (ARN). You can fix this issue by [adding the bank account](#add-bank-account-details) in AWS Partner Central.

### Adding bank account details
<a name="add-bank-account-details"></a>

Add bank account details in AWS Partner Central to fix invalid bank account ARN errors in disbursements.

The `Resource ARN` field displays the ARN of the invalid bank account. To fix the issue, add the bank account in AWS Partner Central. The following steps explain how.<a name="resolve-invalid-bank-account-details"></a>
