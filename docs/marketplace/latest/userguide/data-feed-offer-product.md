...
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| offer_id            | The friendly identifier of this offer.Can be used to join to the `offer_id` field of the `Offer` data feed.                                                                       |
| offer_revision      | Combines with `offer_id` field to form the foreign key to the offer revision.                                                                                                     |
| product_id          | The friendly identifier of the product, this is the foreign key to the product that this offer exposes. Can be used to join to the `product_id` field of the `Product` data feed. | ## Example of Offer product data feed The following shows an example of the Offer product data feed. |
