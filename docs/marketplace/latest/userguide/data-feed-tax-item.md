...
| taxed\_customer\_account\_id | A foreign key to the account entity who is taxed.Can be used to join to the `Account` data feed on the `account_id` field. | 
...
| tax\_invoice\_date | The date of the tax invoice. This field will be null for unified invoicing scenarios. | 
