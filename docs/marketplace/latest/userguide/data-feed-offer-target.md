...
| polarity | Indicates whether the offer is intended to be made to the `target_type`. Acceptable values are as follows:+  `PositiveTargeting` – The offer applies for this `target_type`. <br />+  `NegativeTargeting` – The offer doesn't apply for this `target_type`.  | 
