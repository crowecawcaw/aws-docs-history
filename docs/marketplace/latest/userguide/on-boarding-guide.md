...
+  For *--destination-s3-bucket-name*, replace *{amzn-s3-demo-bucket}* with the name of the Amazon S3 bucket you created in [Step 2: Create a destination Amazon S3 bucket](#create-a-destination-amazon-s3-bucket). 
+  For *–sns-topic-arn*, replace *{YOUR-SNS-TOPIC-ARN}* with the Amazon SNS topic you created in [Step 3: Configure an Amazon SNS topic for response notifications](#create-an-amazon-sns-topic-for-response-notifications). 
