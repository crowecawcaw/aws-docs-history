...
**Note**  
You can only add dimensions for the pricing model selected for your product (for example, contract, usage, contract with consumption, or Free). For limited products using non-free pricing models, the prices for the newly added dimensions are set to $0.01. For products using the Free pricing model, newly added dimensions must be set to $0.00. For non-free pricing models, you can update the prices when the product is ready for public visibility. If all dimensions are set to $0.00, the product will automatically convert to the Free pricing model.
