...
If you are using HTTP requests to call AWS HealthImaging actions, you must use different endpoints depending on the actions being called. The following menu lists the available service endpoints for HTTP requests and the actions they support.
...
The following *data store*, *import*, and *tagging* actions are accessible via endpoint:
...
The following *image set* actions are accessible via endpoint:
...
HealthImaging offers a representations of DICOMweb Retrieve WADO-RS services. For more information, see [Retrieving DICOM data from HealthImaging](dicomweb-retrieve.md).

The following DICOMweb services are accessible via endpoint:
