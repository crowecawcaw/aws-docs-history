...
a study in an HealthImaging [data store](getting-started-concepts.md#concept-data-store "getting-started-concepts.md#concept-data-store"). Construct a request URL
...
3. Construct a URL for the request, including all applicable search elements.
...
Instance elements for `SearchDICOMStudyInstances`| DICOM element tag | DICOM element name |
| --- | --- |
...
| `(0008,0018)` | `SOP Instance UID` | 4. Prepare and send your request. `SearchDICOMStudyInstances` uses an HTTP GET
...
`datastoreId` with your data store ID. Replace
`StudyInstanceUID` with the study's UID.
