...
Amazon EventBridge Scheduler is integrated with AWS CloudTrail, a service that provides a record of actions
taken by a user, role, or an AWS service in EventBridge Scheduler. CloudTrail captures all API calls for
EventBridge Scheduler as events. The calls captured include calls from the EventBridge Scheduler console and
code calls to the EventBridge Scheduler API operations. If you create a trail, you can enable
continuous delivery of CloudTrail events to an Amazon S3 bucket, including events for EventBridge Scheduler. If
...
determine the request that was made to EventBridge Scheduler, the IP address from which the request
...
in EventBridge Scheduler, that activity is recorded in a CloudTrail event along with other AWS service events
...
For an ongoing record of events in your AWS account, including events for EventBridge Scheduler,
...
All EventBridge Scheduler API actions are logged by CloudTrail and are documented in the [Amazon EventBridge Scheduler API Reference](../APIReference.md "../APIReference.md"). For example,
calls to the
`CreateSchedule`, `UpdateSchedule` and `DeleteSchedule` actions generate
