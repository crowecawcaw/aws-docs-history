...
continue to invoke it's target indefinitely. 6. Choose **Next**. 7. On the **Select target** page, do the following:
...
8. Choose **Next**, then on the **Settings - _optional_** page, do the following: 9. 1. In the **Schedule state** section, for
**Enable schedule**, toggle feature on or off
using the switch. By default, the EventBridge Scheduler enables your schedule. 2. In the **Action after schedule completion** section, configure the action EventBridge Scheduler takes after the schedule completes:
    ...
    5. For **Maximum retries**, enter the maximum number of
    ...
    6. For **Dead-letter queue (DLQ)**, choose from the
     following options:
...
10. Choose **Next**. 11. In the **Review and create schedule** page, review the
details of your schedule. In each section, choose **Edit** to
go back to that step and edit its details. 12. Choose **Create schedule** to finish creating your new
schedule. You can view a list of your new and existing schedules on the
**Schedules** page. Under the **Status**
column, verify that your new schedule is **Enabled**. 13. To verify that your schedule invokes the Amazon SQS target, open the Amazon SQS console
