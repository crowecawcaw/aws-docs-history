...
When a flow is run before an outbound call is made as part of an outbound contact,
this action calls the outbound destination. If this action is not used, the first
participant action implicitly completes the outbound call.
...
This action can be used only when the contact is in the process of making an
outbound call, but
has
not yet called the outbound number.

## Corresponding block in the UI

[Call phone number](../adminguide/call-phone-number.md "../adminguide/call-phone-number.md")
