...
## Action types
<a name="flow-language-concepts-action-types"></a>

Flow actions have the following implicit types associated with them. A type determines when an action is attempted. 
+ [Contact actions in the Connect Customer Flow language](contact-actions.md). These actions are attempted only when the flow is run in context of a contact. They generally result in contact data being manipulated in some way.
