...
1. Use the Amazon SNS console to create a new standard SNS topic and set up the
   messages.
2. Call the [StartChatContact](../APIReference/API_StartChatContact.md "../APIReference/API_StartChatContact.md") API to initiate the chat contact.
3. Call the [StartContactStreaming](../APIReference/API_StartContactStreaming.md "../APIReference/API_StartContactStreaming.md") API to initiate message streaming.
4. Call the [CreateParticipantConnection](../../../connect-participant/latest/APIReference/API_CreateParticipantConnection.md "../../../connect-participant/latest/APIReference/API_CreateParticipantConnection.md") API to create the participant's
   connection.
...
2. [Create a
   SNS topic](../../../sns/latest/dg/sns-create-topic.md "../../../sns/latest/dg/sns-create-topic.md") in your AWS account. In the **Details**
   section, for **Type**, choose **Standard**,
   enter a name for the topic, and then choose **Create
   topic**.

###### Note

Currently, the message streaming APIs only support standard SNS for
...
The default **Access policy** comes with conditions
applied to `sourceOwner` such as:
...
Make sure you remove it and replace with `SourceAccount`, for
example:
...
This prevents a [cross-service confused deputy](cross-service-confused-deputy-prevention.md "cross-service-confused-deputy-prevention.md") issue. 5. If you're using server-side encryption on SNS, verify you have
`connect.amazonaws.com` permission enabled on the KMS key. Following is a sample policy:
...
- Call [CreateParticipantConnection](../../../connect-participant/latest/APIReference/API_CreateParticipantConnection.md "../../../connect-participant/latest/APIReference/API_CreateParticipantConnection.md") with the
  ...
  - Calling [CreateParticipantConnection](../../../connect-participant/latest/APIReference/API_CreateParticipantConnection.md "../../../connect-participant/latest/APIReference/API_CreateParticipantConnection.md") with
    `ConnectParticipant` set to true only works if you
    enabled streaming in [Step 2: Initiate the chat contact](#step2-chat-streaming "#step2-chat-streaming") and caller participant is
    `Customer`.
  - This step (creating the participant connection) is optional if you
    have already successfully connected to the chat contact using
    `WEBSOCKET`.
...
1. To verify it is working, check that messages are published to the SNS topic
   you created. You can do this using Amazon CloudWatch metrics. For instructions, see
...
3. Using [StopContactStreaming](../APIReference/API_StopContactStreaming.md "../APIReference/API_StopContactStreaming.md") is optional and not required if the chats are
   being [disconnected](disconnect-hang-up.md "disconnect-hang-up.md") through a contact
   flow, or if the customer disconnects the chat. However,
   `StopContactStreaming` provides the option to stop the message
   streaming on the SNS topic, even if the chat is active and ongoing.
