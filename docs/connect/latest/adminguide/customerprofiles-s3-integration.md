...
In this topic we show how to create an S3 integration with a sync interval of 15
minutes, the minimum value currently supported.
...
- `ScheduleStartTime`: Set to the current
  `DateTime` + 5 minutes in epoch time.
- `FirstExecutionFrom`: Go to S3, look at the file date, and
  use a date that is before the oldest date.
- `Tasks`: Define `TaskType`. In the
  `Sourcefields` field you have to supply ALL the columns
  you have in your CSV in that array. Then, for each of the items in that
  array, you need to specify the `ConnectorOperator`. This
...
The response from `PutIntegration` returns a flow URI. For
example:
