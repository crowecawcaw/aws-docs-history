...
+ Use the Connect Customer console. The user interface makes data mapping features readily accessible. For example, you can add custom attributes, and define search and unique identifiers for contact models. No coding required\! 
+ Use the Customer Profiles API. For more information, see the [Connect Customer Customer Profiles API Reference](https://docs.aws.amazon.com/customerprofiles/latest/APIReference/Welcome.html).
...
Connect Customer provides a no-code experience for mapping customer data from homegrown and third-party applications with Amazon S3, Salesforce, ServiceNow, Zendesk, and Marketo.
...
1. In the **Data source** section:
   ...
   1. Choose where the data is coming from, such as Salesforce or Zendesk. Based on your selection, Connect Customer automatically selects the available destinations based on the predefined template. 
   ...
   1. In the **Mapping destination** section, choose the data that you want to use to build your unified customer profile. This information can be surfaced to your flows and agents to personalize the interactions with contacts.
      ...
      For more information about supported mapping destinations, see [About mapping destinations in Connect Customer](about-mapping-destinations.md).
...
**Note**  
The names `_profileId`, `_orderId`, `_caseId`, and `_assetId` are reserved for internal use. If you decide to use these names as one of your identifier names, they must be declared as `LOOKUP_ONLY`, which means our system will not save them to match with profiles, standard assets, standard orders, standard cases, or save them for searching your profile, asset, case, or order. If you would like these keys to be usable for searching and matching, you must rename your key. For more information on the `LOOKUP_ONLY` standard identifier, see [Standard identifiers for setting attributes on the key in Customer Profiles](standard-identifiers.md). 
+ **Unique identifier**: You must have a unique identifier for your data to avoid an error when it is ingested. This identifier is also known as the unique key. Customer Profiles uses it to distinguish this data from other data source objects, and to index for search and update data. 

  There can be only one unique identifier.
+ **Customer identifier**: You must have at least one customer identifier for your data to avoid an error when it is ingested. The identifier is also known as the profile key.
...
+ **Case identifier**: You must have at least one case identifier for your data to avoid an error when it is ingested. The identifier is also known as the case key.
...
+ **Order identifier**: You must have at least one order identifier for your data to avoid an error when it is ingested. The identifier is also known as the order key.
