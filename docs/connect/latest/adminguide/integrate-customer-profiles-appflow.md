...
These integrations use Connect Customer featured applications to provide periodic updates to Connect Customer Customer Profiles.
...
The role created in the console is only usable by the domain it was created on. This is because Connect Customer limits the access of the role to only the KMS key used by the domain. 
