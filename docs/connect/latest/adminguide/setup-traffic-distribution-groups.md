...
+ When creating a traffic distribution group, it must be created in the source AWS Region. A *source Region* is the Region where you set up your existing Connect Customer instance.
...
+ When you create a replica Connect Customer instance, a default traffic distribution group is created for it. The default traffic distribution group is the only traffic distribution group where you can change the `SignInConfig` distribution. See the `IsDefault` parameter in the [TrafficDistributionGroup](https://docs.aws.amazon.com/connect/latest/APIReference/API_TrafficDistributionGroup.html) data type. You use `SignInConfig` to choose which backend sign-in servers are used to help the agent signing in to their Connect Customer instance. For example, if you call `UpdateTrafficDistribution` with a modified `SignInConfig` and a non-default `TrafficDistributionGroup`, an `InvalidRequestException` is returned.
