...
Default colors provide sufficient contrast to make sure that content is readable and usable by all individuals, including those with visual impairments. _If colors are changed, they should be tested for accessibility_.
...
1. Test thoroughly to ensure the desired appearance, as well as accessibility (for example, sufficient contrast).
2. If a style element is inconsistent with your workspace configuration, check for component-level configurations in its view. Component styling takes precedence over workspace styling. (View styling is only considered in the absence of workspace or component styling).
