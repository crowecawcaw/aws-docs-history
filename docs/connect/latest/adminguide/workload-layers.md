...
You can separate Connect Customer workloads into the following layers: telephony, Connect Customer interface/API, flows/IVR, agent workstation, and metric and reporting. 
...
The Flow/IVR layer is the primary architectural vehicle for Connect Customer and serves as the point of entry and first line of communication with customers reaching out to your contact center. After a customer contacts your Connect Customer instance, a flow controls the interaction between Connect Customer, the contact, and the agent, allowing you to:
...
The agent workstation layer is not managed by AWS. It consists of any physical equipment and third-party technologies, services, and endpoints that help your agent’s voice, data, and access the Connect Customer interface layer. Components in the agent workstation layer include:
