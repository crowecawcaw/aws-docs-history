...
merge them into a unified profile based on consolidation criteria that you specify.

The attributes that you select are compared across all similar profiles in a match
group for exact match. For example, if you specify `email` as an
attribute in the criteria, then all similar profiles in a match group that have
exact same value of `email address` are merged into a unified profile.

###### Tip

If you want to set up your own merging logic, use the [MergeProfiles](../../../customerprofiles/latest/APIReference/API_MergeProfiles.md "../../../customerprofiles/latest/APIReference/API_MergeProfiles.md") API.

## Limits

You can select any attribute from the [standard profile](standard-profile-definition.md "standard-profile-definition.md") to compare
similar profiles. For example, you might choose phone number, email address, and
...
- Select attributes that can uniquely identify a customer and are not
  likely to be the same across customers, such as an account number or a
...
- Select all attributes within a specific attribute group, when
  ...
  name attributes: **First name, Middle name, Last
  name**. If you want to use business address, select all the
...
3. In the **Profile conflicts** section, choose how
   ...
   conflicts.
4. In the **Consolidation criteria** section, create one
...
consolidation criteria, however, a default timestamp of when the records were
ingested into Customer Profiles is used. We recommend adding the new attributes before
creating your consolidation criteria.
...
existing profiles with the `Fields.sourceLastUpdatedTimestamp`. To
opt in to the scheduled backfill:
...
2. After you update your custom profile object type, open an [Support ticket](https://console.aws.amazon.com/support/home "https://console.aws.amazon.com/support/home") and
   we’ll schedule the backfill for you. The scheduled backfill runs until
   end of February 2022.

Alternatively, you can delete and then re-create the ingestion/connector you
have for your domain that uses the custom object type. All of your data will be
re-ingested using your updated object type and
...
- **Resolve profile conflicts** is set to **Use
  last updated timestamp**. This means when two fields have
...
Because there's a conflict between the phone numbers, Identity Resolution uses the last
timestamp to choose the 555-555-555 number.
...
Identity Resolution uses the First name, Last name, and Email from Profile C because they
have the most recent timestamps.
