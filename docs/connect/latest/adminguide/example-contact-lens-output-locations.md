...
The date and timestamp in the output file path correspond to the `ConnectedToAgentTimestamp`, not the `InitiationTimestamp`. If the contact spans a UTC day boundary, the file appears under the date when the agent connected, not the date when the contact started.
...
**Deleting recordings**  
To fully remove a recording, you must delete both the redacted and unredacted files. If you delete only one version, the other version remains accessible. 