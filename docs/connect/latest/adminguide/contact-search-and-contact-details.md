...
**Important**  
When you onboard to this feature, your Connect Customer alias will be updated to a new sub-domain with the format `{{region}}.{{sourcealias}}.my.connect.aws`. For example, if your ACGR instance is deployed in US East (N. Virginia) and US West (Oregon) with source alias configured as `source.my.connect.aws` and replica alias as `replica.my.connect.aws`, after onboarding your Connect Customer instances to this feature, the new sub-domains will be `us-east-1.source.my.connect.aws` and `us-west-2.source.my.connect.aws`.   
...
When you access the Contact Search page, you see contacts from your paired Connect Customer Global Resiliency (ACGR) instances by default, giving you a complete view of customer interactions across your organization. This provides a seamless search experience regardless of which Region contacts originate from or which Region you are currently logged into.
