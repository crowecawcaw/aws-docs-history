...
**Topics**
+ [When is a conversation recorded?](#when-conversation-recorded)
+ [Where are recordings and transcripts stored?](#where-are-recordings-stored)
+ [When are recordings available?](#when-are-recordings-available)
+ [Prevent agents from accessing recordings](#recording-prevent-access)
+ [Headset requirements for listening to recordings](#recording-headset-requirements)
...
If a recording is moved from one S3 bucket to another for any reason, such as the retention period has expired, then the recording will no longer be accessible by Connect Customer.
