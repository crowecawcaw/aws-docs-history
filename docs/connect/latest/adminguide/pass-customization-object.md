...
| `composer.hide`                                    | Boolean | Hides the composer (`true`) or shows it<br>(`false`). To toggle the composer based on<br>events (such as when an agent joins), use<br>`registerCallback` with the<br>`hideComposer` method. For more information, see<br>[Supported widget snippet fields in Amazon Connect that are customizable](supported-snippet-fields.md "supported-snippet-fields.md").<br>`<br>document.getElementById("amazon-connect-chat-widget-iframe").contentWindow.connect.ChatInterface.hideComposer(false)<br>` |
