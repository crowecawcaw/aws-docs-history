...
For more information about contact attributes, see [Use Connect Customer contact attributes](connect-contact-attributes.md). 
...
+ **Current contact**: The attributes are set on the contact that this flow is running on. The attributes are accessible by other areas of Connect Customer, such as other flows, modules, Lambdas, contact records, and the GetMetricDataV2 API. 
...
+ For the JSON syntax for each attribute, see [List of available contact attributes in Connect Customer and their JSONPath references](connect-attrib-list.md).
