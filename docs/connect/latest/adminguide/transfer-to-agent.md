...
+ Use the **Network** tab of the browser debugger to retrieve the agent ID. For example:
  ...
  1. In Connect Customer, in the navigation menu, choose **Users**, **User management**, and then select an agent. Monitor the content of the **Network** tab. In the **Name** list, choose the GUID. 
...
![The preview tab, the agent ID.](https://docs.aws.amazon.com/connect/latest/adminguide/images/find-agent-id.png)
...
+ Find the user ID for an agent by using [Connect Customer agent event streams](agent-event-streams.md). The agent events, which are included in the agent event data stream, include the agent ARN. The user ID is included in the agent ARN after **`agent/`**. 
