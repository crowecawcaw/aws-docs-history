...
To enable case assignment in your Connect Customer instance, configure the following resources:
...
1. To enable agents to assign case ownership in the agent workspace: 
   ...
   + **Quick connects**. Create user and queue quick connects for each user and queue that you want to appear in the dropdown lists. For instructions, see [Create quick connects in Connect Customer](quick-connects.md).
   + **Queues**. Add the quick connects to the agent's queue. For instructions, see [Create a queue using the Connect Customer admin website](create-queue.md).
   + **Routing profile**. Add the queue to the agent's routing profile. For instructions, see [Create a routing profile in Connect Customer to link queues to agents](routing-profiles.md). 
