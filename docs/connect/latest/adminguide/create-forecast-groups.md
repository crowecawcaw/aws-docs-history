...
- You must create at least one forecast group before you can generate
  any forecast.
...
- If a queue is associated with a forecast group and is later disabled,
  you don't have to remove this queue from the forecast group. This is
  because: Although the queue is included by the forecast group and the
  historical data associated with it is included in the forecast, over
...
not yet associated with a forecast group are listed. If no queues are
listed, it means they are all associated with a forecast group
already. 5. Drag and drop one or more queues to the forecast group, as shown in
the following image. You can press and hold CTRL (COMMAND for macOS
users) or SHIFT to select multiple queues at a time.

![The Create forecast groups page, a list of queues.](images/wfm-drag-drop-queues-to-forecast-group.png) 6. Choose **Save**. The following image shows the new
forecasting group, along with the number of queues in the group and the
date it was last changed.

![The Forecasting page, the forecast groups tab, the new forecast group.](images/wfm-forecasting-group.png) 7. After creating a forecast group, you can add or remove queues.
...
**Remove** function.
    ...
    1. delete the forecasts that are associated with the forecast
     group you want to delete.


    For example, in the following image, a forecast group named
     *Network\_Issues* cannot be deleted
     because this forecast group was used to create forecasts.



    ![The Forecasting page, the forecasts groups tab, a message that the group cannot be removed.](images/wfm-forecasting-delete-forecast-group.png)

    Therefore, go to **Forecasts** tab to delete
     those associated forecasts.
    2. Delete the forecast group.

## Next steps

Now you're ready to create a forecast. For instructions, see [Create forecasts](create-forecasts.md "create-forecasts.md").
