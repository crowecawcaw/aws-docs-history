# Customer Profiles standard order

object fields

The following table lists all the fields in the Customer Profiles standard order
object.
...
| StatusCode            | String               | The order's status code. Valid values: DRAFT      | <br>ACTIVATED |
...
The standard order objects are indexed by the keys in the following
table.
...
For example, you can use `_orderId` as a key name with the
[SearchProfiles API](../../../customerprofiles/latest/APIReference/API_SearchProfiles.md "../../../customerprofiles/latest/APIReference/API_SearchProfiles.md") to find a profile that has an order whose
OrderId matches with the search value. You can find the standard order
