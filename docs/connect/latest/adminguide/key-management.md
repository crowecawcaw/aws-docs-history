...
Connect Customer agent assist stores knowledge documents that are encrypted at rest in S3 using a
BYOK or a service-owned key. The knowledge documents are encrypted at rest in
Amazon OpenSearch Service using a service-owned key. agent assist stores agent queries and call
