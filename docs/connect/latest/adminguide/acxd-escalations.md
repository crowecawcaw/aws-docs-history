...
1. Create a new application build and deploy it to the environment used by the Connect Customer flow.
...
1. Select the returned context value, such as `summary`.
...
Use clear attribute names so the values are easy to reference later in the Connect Customer flow or agent workspace.

After setting contact attributes, continue the Connect Customer flow by routing the customer to the appropriate queue.
...
1. **Transfer to queue** — Transfers the customer to the selected queue.
...
This reduces repeat questions and helps the agent continue the conversation with more context.
...
1. In a flow of Connect Customer, connect the Agentic CX block's Escalation path to **Set contact attributes**.

1. Store returned values from the Agentic CX namespace.
