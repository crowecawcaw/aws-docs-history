...
For information about how queue priority and delay work, see [Queues: priority and delay examples](concepts-routing-profiles-priority.md).
  ...
  For example:
  + An agent named Joe is idle. He is third in line to receive an inbound contact. He would rather handle an inbound contact than an outbound contact because he knows he will speak to a customer, whereas an outbound contact might not pick up the phone. Talking to an inbound contact increases his odds of getting recognition in his role. 
  + Because he is idle, Joe decides to make an outbound contact to chip away at the backlog. He might or might not reach someone.
...
+ A routing profile might assign a priority to one queue over another, but the priority within the queue is always set by the order the contact was added to the queue.
