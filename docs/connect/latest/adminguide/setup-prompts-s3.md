...
To allow Connect Customer to play prompts from an S3 bucket, when you set up your S3 bucket, you must update the bucket policy to grant `connect.amazonaws.com` (the Connect Customer service principal) permission to call `s3:ListBucket` and `s3:GetObject`. 
