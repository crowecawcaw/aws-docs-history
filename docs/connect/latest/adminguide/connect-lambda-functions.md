...
Connect Customer can interact with your own systems and take different paths in flows dynamically. To achieve this, invoke AWS Lambda functions in a flow, fetch the results, and call your own services or interact with other AWS data stores or services. For more information, see the [AWS Lambda Developer Guide](https://docs.aws.amazon.com/lambda/latest/dg/).

To invoke a Lambda function from a flow, complete the following tasks.

**Topics**
+ [Create a Lambda function](#create-lambda-function)
+ [Add a Lambda function to your Connect Customer instance](#add-lambda-function)
+ [Invoke a Lambda function from a flow](#function-contact-flow)
+ [Best practice for invoking multiple Lambda functions](#invoke-multiple-functions)
...
If you create the Lambda function in the same Region as your contact center, you can use the Connect Customer console to add the Lambda function to your instance as described in the next task, [Add a Lambda function to your Connect Customer instance](#add-lambda-function). This automatically adds resource permissions that allow Connect Customer to invoke the Lambda function. Otherwise, if the Lambda function is in a different Region, you can add it to your flow using the flow designer and add the resource permissions using the [add-permission](https://docs.aws.amazon.com/cli/latest/reference/lambda/add-permission.html) command, with a principal of `connect.amazonaws.com` and the ARN of your Connect Customer instance. For more information, see [Using Resource-Based Policies for AWS Lambda](https://docs.aws.amazon.com/lambda/latest/dg/access-control-resource-based.html) in the *AWS Lambda Developer Guide*.

## Add a Lambda function to your Connect Customer instance
<a name="add-lambda-function"></a>

Before you can use an Lambda function in a flow, you need to add it to your Connect Customer instance.
...
To use a Lambda in a different Region or account, in the [AWS Lambda function](invoke-lambda-function-block.md), under **Select a function**, you can enter the ARN of a Lambda. Then set up the corresponding resource-based policy on that Lambda to allow the flow to call it.   
...
1. Add an [AWS Lambda function](invoke-lambda-function-block.md) block (in the **Integrate** group) to the grid. Connect the branches to and from the block.

1. Choose the title of the [AWS Lambda function](invoke-lambda-function-block.md) block to open its properties page.
...
When a synchronous invocation returns an error, Connect Customer retries up to 3 times, for a maximum of 8 seconds. At that point, the flow will progress down the Error branch. 
...
**Tip**  
Referencing an array is not supported in a flow. Arrays can be used only in another Lambda function.
