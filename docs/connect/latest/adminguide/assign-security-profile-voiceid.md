...
# Security profile permissions for Connect Customer Voice ID
...
**Note**  
The functionality to enter or update the `SpeakerID` is not available with the default Voice ID widget in the CCP. To include the option for updating the `SpeakerID`, implement the `updateVoiceIdSpeakerId` [Connect Customer Streams](https://github.com/aws/amazon-connect-streams) API in your custom CCP.
