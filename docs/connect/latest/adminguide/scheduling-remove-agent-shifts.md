...
Contact center managers, supervisors, and schedulers can remove agent shifts, for either one agent at a time or multiple agents simultaneously. For example: 
+ Remove all shifts for next Monday.
+ Remove future shifts for an agent who is no longer with the organization.

You can remove up to 30 days of shifts for an agent. You can remove shifts for up to 400 agents for a day.
...
The following procedure does not remove any time-offs that are scheduled for the selected date range. 
     ...
     1. One the left side of the **Scheduling** - **Published schedule calendar** page, select the agent.

     1. Use the **Actions** dropdown list to select **Remove shift**.
...
The following procedure does not remove any time-offs that are scheduled for the selected date range. 
     ...
     1. On the left side of the page, select up to 400 agents. If you select more than 400 agents, only the first 400 agents will be part of the remove shift operation. 
