...
+ When used in a callback scenario, Connect Customer calls the agent first. After the agent accepts the call in the CCP, Connect Customer calls the customer.
...
You can configure the **Transfer to queue** block using the Connect Customer admin website. Or you can use the Connect Customer Flow language. Depending on the use case you use one of the following actions: 
...
**Topics**
+ [Transfer to queue](#transfer-to-queue-tab)
+ [Transfer to Callback (scheduling callbacks)](#transfer-to-queue-callback)
+ [Flow block branches](#transfer-to-queue-branches)
+ [Additional configuration tips](#transfer-to-queue-tips)
+ [Data generated by the block](#transfer-to-queue-data)
...
+ **Maximum number of retries**: If this were set to 1, then Connect Customer would try to callback the customer at most two times: the initial callback, and 1 retry.
**Tip**  
