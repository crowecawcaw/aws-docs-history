...
Use the **Next status** feature to pause new contacts being routed to you, while you finish your current contacts. When all your slots are cleared, Connect Customer automatically sets your CCP to the next status, such as **Lunch**.

The following images of the Contact Control Panel (CCP) show how to use this feature.

![Four images of the CCP with status in different states.](https://docs.aws.amazon.com/connect/latest/adminguide/images/next-status-example-new.png)
...
You can easily switch from **Next status** back to **Available**. The ability to switch your status is useful, for example, if you accidentally choose **Next status: Lunch**, or if you decide not to go to **Lunch** before Connect Customer automatically sets to that status. 

The following images show this workflow.

![The CCP set to next status, the CCP set to available.](https://docs.aws.amazon.com/connect/latest/adminguide/images/next-status-example-cancel.png)
...
1. Because all the ACWs have been cleared, Connect Customer immediately starts the automatic transition that sets the agent's status to **Lunch**.
...
1. Connect Customer automatically sets the agent's status to **Offline**.