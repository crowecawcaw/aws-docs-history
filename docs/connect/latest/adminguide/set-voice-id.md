...
+ Sends audio to Connect Customer Voice ID to verify the caller's identity and match against fraudsters in watch list, as soon as the call is connected to a flow.
...
When this option is selected, Connect Customer begins streaming audio from the customer's channel to Voice ID.
