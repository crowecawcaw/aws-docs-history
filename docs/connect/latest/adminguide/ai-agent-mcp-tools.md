...
Connect Customer includes prebuilt tools for common tasks such as updating contact attributes and retrieving case information, enabling immediate functionality without additional configuration.
...
You can use third-party integrations through Amazon Bedrock AgentCore Gateway. By registering AgentCore Gateways in the AWS Management Console, similar to how third-party applications are registered to Connect Customer today, you gain access to whatever tools are available on those servers, including remote MCP servers.  
For instructions on linking an AgentCore gateway to your Connect Customer instance, see [Integrate an MCP server with Connect Customer](https://docs.aws.amazon.com/connect/latest/adminguide/3p-apps-mcp-server.html). That topic covers how to add the MCP server integration in the Connect Customer console and how to configure the gateway's Discovery URL and allowed audiences.  
