...
**Topics**
+ [How callbacks keep their place in queue](#callback-how-it-works)
+ [Steps to set up queued callbacks](#setup-queued-callback-overview)
...
+ [Callbacks from a chat, task, or email contact](#queued-callback-chat-task)
...
1. After an agent accepts the callback in the CCP, Connect Customer calls the customer.

   If no agents are available to work on callbacks, the callbacks can stay in queue for up to 7 days after they are created before Connect Customer automatically removes them.
**Tip**  
...
1. If there is no answer when the Connect Customer calls the customer, it retries based on the number of times you've specified. 

1. If the call goes to **voicemail**, it's considered connected.

1. If the customer calls again while in the callback queue, it's treated as a new call and will be handled as usual. To avoid duplicate callback requests in a callback queue, see this blog: [Preventing duplicate callback requests in Connect Customer](https://aws.amazon.com/blogs/contact-center/preventing-duplicate-callback-requests-in-amazon-connect/).
...
To see what a flow looks like with queued callback, in new Connect Customer instances see [Sample queue configurations flow in Connect Customer](sample-queue-configurations.md). In previous instances, see [Sample queued callback flow in Connect Customer](sample-queued-callback.md).
...
![The Transfer to callback queue tab on the Properties page of the Transfer to queue block.](http://docs.aws.amazon.com/connect/latest/adminguide/images/transfer-to-callback-queue-tab.png)
   ...
   + **Maximum number of retries**: If this is set to 2, then Connect Customer tries to call back the customer a maximum of three times: the initial callback, and two retries. 

     A retry only happens if it rings but there's no answer. If the callback goes to voicemail, it's considered connected and Connect Customer does not retry again.
**Tip**  
...
1. To save and test this flow, configure the other branches and add error handling. To see an example of how this is done, see [Sample queue configurations flow in Connect Customer](sample-queue-configurations.md). For previous instances, see [Sample queued callback flow in Connect Customer](sample-queued-callback.md). 

1. For information about how callbacks appear in real-time metrics reports and contact records, see [Queued callbacks in real-time metrics in Connect Customer](about-queued-callbacks.md). 
