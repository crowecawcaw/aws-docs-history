...
- The queues of customers that the agents will handle. You can use a single
  queue to handle all incoming contacts, or you can set up multiple queues.
  Queues are linked to agents through a routing profile.
- Priority and delay of the queues.

The following image shows a graphic of a group of agents mapped to a routing
profile. The routing profile specifies multiple channels and queues for the
agents.

![A graphic that shows a group of agents mapped to a routing profile.](images/routing-profile-3.png)
