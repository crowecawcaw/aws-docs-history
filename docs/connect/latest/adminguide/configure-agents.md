...
1. Under Contact handling choose whether you want to auto accept contacts and set the After Contact timeout duration.
   + **Auto-accept**: Auto-accept automatically connects you to contacts on the respective channel when you turn it on. Each channel has its own **Auto-accept** setting, so there is no single checkbox that applies to all channels. For more information, see [Enable auto-accept](enable-auto-accept.md).
