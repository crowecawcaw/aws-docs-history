...
1. If voice, the second **Check contact attributes** block is triggered. It checks the match conditions of **State**, which is an external attribute. It uses an external contact attribute because it's getting data by using a process that's external to Connect Customer
...
For more information about using attributes, see [Store a value from a Lambda functions as a contact attribute in Connect Customer](attribs-with-lambda.md).