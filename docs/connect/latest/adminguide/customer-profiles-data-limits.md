...
Each profile can store a limited total number of objects across all object types (1000 by default; you can request a limit increase, see [Connect Customer service quotas](amazon-connect-service-limits.md)). When a profile reaches this total limit, Connect Customer removes (evicts) an existing object to make room. The per-object-type limits you set determine which object is evicted:
...
1. If no object type is over its limit, Connect Customer falls back to *self-eviction*. The incoming object evicts the oldest object of its own object type. The total object count for the profile stays the same, and no other object type is reduced.
