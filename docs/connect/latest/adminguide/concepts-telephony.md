...
Connect Customer provides a variety of choices to enable your company to make and receive telephone calls. One of the great advantages of Connect Customer is AWS manages the telephony infrastructure for you: carrier connections, redundancy, and routing. And, it's designed to scale. 

This topic explains the options that Connect Customer provides for telephony, which helps you build a solution to meet your business requirements.
...
In this case, simply select new numbers using the claim a number process. For instructions, see [Get a local toll-free or DID Connect Customer phone number](get-connect-number.md).

### Migrating to Connect Customer from another provider/platform
<a name="concepts-migrate-platforms"></a>

If you're migrating to Connect Customer from other platform, we recommend starting with a proof of concept, and migrating to Connect Customer over time.
+ A best practice is to forward your existing numbers to a new number (or numbers) claimed in Connect Customer until you are fully converted. 
+ After fully converting, use the [porting process](port-phone-number.md) to bring your numbers into Connect Customer. 
