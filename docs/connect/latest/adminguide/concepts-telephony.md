# The voice channel in Connect Customer
...
Connect Customer provides a variety of choices to enable your company to make and receive
telephone calls. One of the great advantages of Connect Customer is AWS manages the telephony
infrastructure for you: carrier connections, redundancy, and routing. And, it's designed
to scale.

This topic explains the options that Connect Customer provides for telephony, which helps you
...
Connect Customer provides capabilities to host both toll-free and direct dial numbers (DID)
in all AWS Regions supported by Connect Customer. You can use both types of numbers in a
single instance. A complete list of supported countries/regions and costs is located
on the [Connect Customer pricing](https://aws.amazon.com/connect/pricing/ "https://aws.amazon.com/connect/pricing/")
page.
...
To learn more about Connect Customer architecture, see [Architectural guidance for Connect Customer](architecture-guidance.md "architecture-guidance.md").
...
Because DID numbers are threaded to single carrier, Connect Customer doesn't offer
...
### Starting fresh with Connect Customer

In this case, simply select new numbers using the claim a number process. For
instructions, see [Get a local toll-free or DID Connect Customer phone number](get-connect-number.md "get-connect-number.md").

### Migrating to Connect Customer from another provider/platform

If you're migrating to Connect Customer from other platform, we recommend starting with a
proof of concept, and migrating to Connect Customer over time.

- A best practice is to forward your existing numbers to a new number
  (or numbers) claimed in Connect Customer until you are fully converted.
- After fully converting, use the [porting process](port-phone-number.md "port-phone-number.md") to bring your numbers into Connect Customer.
...
- If Connect Customer is the primary call handling platform, you can port or claim
...
- If the external platform is the primary call handler, you will need to
  configure that platform to forward calls to a number you claim in Connect Customer.
  ...
  the call into Connect Customer.
