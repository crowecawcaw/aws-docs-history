...
# Launch the Contact Control Panel (CCP) in Connect Customer
<a name="launch-ccp"></a>
...
As the administrator, you can also launch the CCP directly from the Connect Customer console. Just choose the phone icon in the upper right corner.
...
+ Add agents as users to the instance. For more information, see [Manage users that you add to Connect Customer](manage-users.md).
+ Configure permissions for the agents. By default, agents assigned to the Agent security profile can access the CCP and make outbound calls. But you can create a custom security profile and add additional permissions. For more information, see [Security profiles for Connect Customer and Contact Control Panel (CCP) access](connect-security-profiles.md).
