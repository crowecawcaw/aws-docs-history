...
| `disableAsyncViewResponse` | Boolean | Disable sending text messages while a view is pending a response in chat. Set this to `true` if the text message is expected to be processed by the flow block (for example, the no match branch). | Default: false. When false, sending messages is allowed while a view is pending, and text messages are not processed by the flow block. | 
