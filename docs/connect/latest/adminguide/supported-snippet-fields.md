...
# Supported widget snippet fields in Connect Customer that are customizable
...
| `language` | String | Connect Customer can do translations for supported ISO-639 format language codes. For more information, see [ https://en.wikipedia.org/wiki/List\_of\_ISO\_639-1\_codes](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes).This would not translate custom text overrides and message content (both sent and received). | Default: en\_US. Supported: 'cs\_CZ', 'da\_DK', 'de\_DE', 'en\_AU', 'en\_CA', 'en\_GB', 'en\_US', 'es\_ES', 'fi\_FI', 'fr\_FR', 'hu\_HU', 'id\_ID', 'it\_IT', 'ja\_JP', 'ko\_KR', 'nl\_NL', 'nn\_NO' 'pt\_BR', 'pt\_PT', 'sk\_SK', 'sv\_SE', 'zh\_CN', 'zh\_TW' | 
| `disableCSM` | Boolean | Disable tracking of the client-side metrics from the communication widget. | Default: false | 
| `disableAsyncViewResponse` | Boolean | Controls how a customer's text message is handled while a view is awaiting a response. When `true`, text the customer sends while a view is displayed is delivered to the flow as the view's response—typically taking the view's no match branch—instead of being sent as a regular chat message. This does not prevent the customer from typing or sending; the message composer remains enabled. | Default: `false`. When `false`, text messages sent while a view is pending are delivered as normal chat messages and are not processed by the view's flow block. Set to `true` only when your flow is designed to consume free-text input as a view response. | 
...
| `customizationObject` | Object | Customize the widget layout and transcript | For more information, see [Additional customizations for your Connect Customer chat widget](pass-customization-object.md), later in this section. | 
...
| `registerCallback` | Object | This allows to execute callbacks for the exposed lifecycle events. <br />For more information, see [amazon-connect-chatjs](https://github.com/amazon-connect/amazon-connect-chatjs).  | +  System exposed events are `'PARTICIPANT_IDLE'` , `'CHAT_ENDED'`, ` 'PARTICIPANT_RETURNED'`, `'PARTICIPANT_JOINED'`, `'PARTICIPANT_LEFT'`, ` 'CONNECTION_ESTABLISHED'`, `'CONNECTION_LOST'`, and ` 'AUTHENTICATION_INITIATED'`. `AUTHENTICATION_INITIATED` callback is executed when the contact reaches the [Authenticate Customer](authenticate-customer.md) flow block.  <br />+  Custom exposed events are `WIDGET_FRAME_CLOSED`.   | 
