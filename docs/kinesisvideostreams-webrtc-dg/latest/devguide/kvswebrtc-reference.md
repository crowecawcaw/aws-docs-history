...
| **framesSent**                         | This is incremented only if media stream tack type is<br>MEDIA\_STREAM\_TRACK\_KIND\_VIDEO.                                                                                                                                                                                                                                                                                                |
...
| **state**                 | State of the data channel when the stats are queried. Currently, the two states<br>supported are RTC\_DATA\_CHANNEL\_STATE\_CONNECTING (when the channel is created) and<br>RTC\_DATA\_CHANNEL\_STATE\_OPEN (Set in the onOpen() event). |
