...
balancer, you must delete those services before deleting the cluster. If you
don't, the system can't delete the load balancers. Use the following procedure
to find and delete services before you run the `AWS-DeleteEKSCluster`
runbook.
...
2. Run the following command to list all services running in your
   cluster.
...
3. Run the following command to delete any services that have an associated
   EXTERNAL-IP value. These services are fronted by a load balancer, and you
   must delete them in Kubernetes to allow the load balancer and associated
...
[Run this Automation (console)](https://console.aws.amazon.com/systems-manager/automation/execute/AWS-DeleteEKSCluster "https://console.aws.amazon.com/systems-manager/automation/execute/AWS-DeleteEKSCluster")
...
Description: (Required) The name of the Amazon EKS Cluster to be
deleted.
...
- `aws:executeScript` - DeleteNodeGroups: Find and delete all node
  groups in the EKS cluster.
