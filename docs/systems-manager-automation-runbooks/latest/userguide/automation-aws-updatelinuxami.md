...
Allowed Pattern:
`^$|^arn:aws[a-z0-9-]*:iam::(\d{12}|\{\{global:ACCOUNT_ID\}\}):role/[\w/.@+=,-]{1,1017}$`

    + Must be a valid IAM role ARN or an empty string.
     System variable `{{global:ACCOUNT_ID}}` can be used in place of the AWS Account ID in the arn.
...
`^(none|[a-zA-Z0-9\s,._+:=<>()\[\]/*-]+)$`
    ...
    + Must be "none" OR a comma-separated list of items consisting of letters, numbers, spaces,
...
`^[\w+=,.@-]{1,128}$`
    ...
    + Must be between 1 and 128 characters and contain only letters, numbers, and these characters:
     `+ = , . @ - _`
...
`^(all|[a-zA-Z0-9\s,._+:=<>()\[\]/*-]+)$`
    ...
    + Must be "all" OR a comma-separated list of items consisting of letters, numbers, spaces,
     and the following characters: `, . _ + : = < > ( ) [ ] / * -`
...
`^[a-z0-9]+(-[a-z0-9]+)*\.[a-z0-9]+$`
    ...
    + Must be in the format prefix.suffix where both parts contain lowercase letters and numbers,
     and the prefix may include hyphens
...
`^[a-zA-Z0-9()\[\]\{\} ./'@_:-]{3,128}$`
    ...
    + Must be between 3 and 128 characters and contain only letters, numbers, spaces, and these characters:
     `( ) [ ] { } . / ' @ _ : -`
