...
Description: (Required) The interval in seconds when Enhanced Monitoring
metrics are collected from the DB instance.
...
that allows Amazon RDS to send Enhanced Monitoring metrics to Amazon CloudWatch Logs.
...
want to enable Enhanced Monitoring on.
...
- `aws:assertAwsResourceProperty` - Confirms the DB Instance is in
  an `AVAILABLE` state.
- `aws:executeAwsApi` - Enables Enhanced Monitoring on your DB
  instance.
- `aws:executeScript` - Confirms that Enhanced Monitoring is
  enabled on your DB instance.
