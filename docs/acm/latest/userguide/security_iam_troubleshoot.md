...
have the fictional `acm:`GetWidget`` permissions.
...
configured with a `DENY` by the organization administrator.
...
- To learn whether ACM supports these features, see [How AWS Certificate Manager works with IAM](security_iam_service-with-iam.md "security_iam_service-with-iam.md").
