...
The following examples use Amazon Elastic Kubernetes Service orchestration. To use a custom AMI with Slurm orchestration, replace `--orchestrator 'Eks={ClusterArn=<eks_cluster_arn>}'` with `--orchestrator 'Slurm={}'`. Both orchestrators use the same values for the other fields, including `ImageId` and `--node-provisioning-mode Continuous`.
