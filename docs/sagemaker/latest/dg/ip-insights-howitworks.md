...
Amazon SageMaker AI IP Insights is an unsupervised algorithm that consumes observed data in the form
...
_collisions_, which occur when distinct entities are mapped to
...
Large Scale Multitask Learning](https://alex.smola.org/papers/2009/Weinbergeretal09.pdf "https://alex.smola.org/papers/2009/Weinbergeretal09.pdf"). For representing IP addresses, on the other
hand, IP Insights uses a specially designed encoder network to uniquely represent each
possible IPv4 address by exploiting the prefix structure of IP addresses.

During training, IP Insights automatically generates negative samples by randomly
...
More specifically, the model is trained to minimize the _cross
entropy_, also known as the _log loss_, defined as
follows:

![An image containing the equation for log loss.](images/ip-insight-image-cross-entropy.png)
yn is the label that indicates whether the sample is from the
real distribution governing observed data (yn=1) or from the
distribution generating negative samples (yn=0).
pn is the probability that the sample is from the real
...
Given an nth (entity, IP address pair), the IP Insights model outputs a
_score_, Sn , that indicates how
...
![An image containing the equation for the score, a log odds ratio.](images/ip-insight-image-log-odds.png)
The score is essentially a measure of the similarity between the vector
...
probability of a sample coming from the real distribution, pn, to
use in the cross entropy minimization, where:

![An image showing the equation for probability that the sample is from a real distribution.](images/ip-insight-image-sample-probability.png)
