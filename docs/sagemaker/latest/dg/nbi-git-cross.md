...
1. In the AWS account that contains the CodeCommit repository, create an IAM
   ...
   contains your notebook instance. For information, see [Step 1: Create a Policy for Repository Access in AccountA](../../../codecommit/latest/userguide/cross-account-administrator-a.md#cross-account-create-policy-a "../../../codecommit/latest/userguide/cross-account-administrator-a.md#cross-account-create-policy-a") in
   the _CodeCommit User Guide_.
2. In the AWS account that contains the CodeCommit repository, create an IAM
   role, and attach the policy that you created in the previous step to that
   role. For information, see [Step 2: Create a Role for Repository Access in AccountA](../../../codecommit/latest/userguide/cross-account-administrator-a.md#cross-account-create-role-a "../../../codecommit/latest/userguide/cross-account-administrator-a.md#cross-account-create-role-a") in the
...
3. Create a profile in the notebook instance that uses the role that you
   ...
   Where `CodeCommitAccount` is the account
   that contains the CodeCommit repository,
   `CrossAccountAccessProfile` is the name
   of the new profile, and
   `CrossAccountRepositoryContributorRole`
   is the name of the role you created in the previous step.
...
4. On the notebook instance, configure git to use the profile you created in
   ...
   Where `CrossAccountAccessProfile` is the
   name of the profile that you created in the previous step.
