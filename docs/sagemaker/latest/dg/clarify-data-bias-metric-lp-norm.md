...
non-negative and so cannot detect reverse bias.
...
The 2-norm is the Euclidean norm. Assume you have an outcome distribution with
three categories, for example, yi =
{y0, y1,
y2} = {accepted, waitlisted, rejected} in a college
admissions multicategory scenario. You take the sum of the squares of the
differences between the outcome counts for facets _a_ and _d_. The resulting Euclidean
...
- nd(i) is number of the
  ith category outcomes in facet _d_: for
  example nd(2) is number of
  facet _d_ rejections.

The range of LP values for binary, multicategory, and continuous outcomes
is [0, √2), where:
