...
The following parameters must be provided in a JSON file. The path to this JSON file must be provided in the `ConfigUri` parameter of the [`ModelBiasAppSpecification`](https://docs.aws.amazon.com/sagemaker/latest/APIReference/API_ModelBiasAppSpecification) API.
...
The other parameters should be provided in `EndpointInput` (for real-time endpoints) or `BatchTransformInput` (for batch transform jobs) of the [`ModelBiasJobInput`](https://docs.aws.amazon.com/sagemaker/latest/APIReference/API_ModelBiasJobInput) API.
