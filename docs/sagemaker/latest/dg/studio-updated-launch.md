...
+ [Service quotas for Studio](#studio-updated-launch-quotas)
...
<a name="studio-updated-launch-quotas-view"></a>
...
1. From the AWS services list, search for and choose **Amazon SageMaker AI**.

1. In the Service quotas list, review the quota name, applied value, AWS default quota, and whether you can adjust the quota.

1. Search for quotas relevant to your Studio workload. For example, search for the instance type you plan to use.
...
<a name="studio-updated-launch-quotas-increase"></a>
...
1. If the quota is adjustable, choose **Request increase at account level**.

1. For **Increase quota value**, enter the new value. The new value must be greater than the current value.

1. Choose **Request**.

1. To view pending or recently resolved requests, choose the **Request history** tab.
