...
from the dataset, resize this image to match the model’s original input, and then
you will make a prediction.

1. **Download your compiled model from Amazon S3 to your device
   and extract it from the compressed tarfile.**
...
2. **Import DLR and an initialized `DLRModel`
   object.**
...
3. **Download an image for inferencing and format it based on
   how your model was trained**.

For the `coco_ssd_mobilenet` example, you can download an image
from the [COCO dataset](https://cocodataset.org/#home "https://cocodataset.org/#home") and
then reform the image to `300x300`:
...
Finally, you can use DLR to make a prediction on the image you just
downloaded:
...
For more examples using DLR to make inferences from a Neo-compiled model on an edge
device, see the [neo-ai-dlr Github
repository](https://github.com/neo-ai/neo-ai-dlr "https://github.com/neo-ai/neo-ai-dlr").
