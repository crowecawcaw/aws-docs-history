...
To create a cluster with the inference operator pre-installed, see [Create an EKS-orchestrated SageMaker HyperPod cluster](sagemaker-hyperpod-quickstart.md#sagemaker-hyperpod-quickstart-eks "sagemaker-hyperpod-quickstart.md#sagemaker-hyperpod-quickstart-eks"). To install the inference
...
After the add-on is successfully installed, you can deploy models using the model deployment documentation or navigate to [Verify the inference operator is working](#sagemaker-hyperpod-model-deployment-setup-verify "#sagemaker-hyperpod-model-deployment-setup-verify").
...
1. [Connect to your cluster and set up environment variables](#sagemaker-hyperpod-model-deployment-setup-connect-addon "#sagemaker-hyperpod-model-deployment-setup-connect-addon")
2. [Configure IAM roles for inference operator](#sagemaker-hyperpod-model-deployment-setup-prepare-addon "#sagemaker-hyperpod-model-deployment-setup-prepare-addon")
3. [Create the ALB Controller role](#sagemaker-hyperpod-model-deployment-setup-alb-addon "#sagemaker-hyperpod-model-deployment-setup-alb-addon")
4. [Create the KEDA operator role](#sagemaker-hyperpod-model-deployment-setup-keda-addon "#sagemaker-hyperpod-model-deployment-setup-keda-addon")
5. [Install the dependency EKS Add-Ons](#sagemaker-hyperpod-model-deployment-setup-install-dependencies "#sagemaker-hyperpod-model-deployment-setup-install-dependencies")

###### Note

Alternatively, you can use CloudFormation templates to automate the prerequisite setup. For more information, see [Using CloudFormation templates to create the prerequisite stack](#sagemaker-hyperpod-model-deployment-setup-cfn "#sagemaker-hyperpod-model-deployment-setup-cfn").
...
2. Apply Tags (`kubernetes.io.role/elb`) to all subnets in the Amazon EKS
...
For detailed installation instructions including required IAM permissions, see [Amazon FSx for Lustre CSI driver](../../../eks/latest/userguide/workloads-add-ons-available-eks.md#add-ons-aws-fsx-csi-driver "../../../eks/latest/userguide/workloads-add-ons-available-eks.md#add-ons-aws-fsx-csi-driver"). 3. **Metrics Server** (minimum version: v0.7.2-eksbuild.4)
...
For detailed installation instructions, see [Metrics Server](../../../eks/latest/userguide/metrics-server.md "../../../eks/latest/userguide/metrics-server.md"). 4. **Cert Manager** (minimum version: v1.18.2-eksbuild.2)
...
For detailed installation instructions, see [cert-manager](../../../eks/latest/userguide/community-addons.md#addon-cert-manager "../../../eks/latest/userguide/community-addons.md#addon-cert-manager").
...
2. Create the add-on configuration file with all required settings:
...
3. Install the inference operator add-on (minimum version: v1.0.0-eksbuild.1):
...
4. Monitor the installation progress and verify successful completion:
...
For detailed troubleshooting of installation issues, see [HyperPod inference troubleshooting](sagemaker-hyperpod-model-deployment-ts.md "sagemaker-hyperpod-model-deployment-ts.md").

To verify the inference operator is working correctly, continue to [Verify the inference operator is working](#sagemaker-hyperpod-model-deployment-setup-verify "#sagemaker-hyperpod-model-deployment-setup-verify").
...
2. Derive cluster and network information:
...
3. Create parameters file and deploy stack:
...
4. Monitor the stack creation status:
...
5. Once the stack is created successfully, retrieve the output values for use in the inference operator installation:
...
After the CloudFormation stack is created, continue with [Installing the Inference Operator with EKS add-on](#sagemaker-hyperpod-model-deployment-setup-install-inference-operator-addon "#sagemaker-hyperpod-model-deployment-setup-install-inference-operator-addon") to install the inference operator.
...
Verify that you've created a HyperPod cluster with [Creating a SageMaker HyperPod cluster with Amazon EKS orchestration](sagemaker-hyperpod-eks-operate-console-ui-create-cluster.md "sagemaker-hyperpod-eks-operate-console-ui-create-cluster.md") .
...
2. Deploy the model and clean up the configuration file.
...
3. Verify the service account configuration to ensure the operator can assume AWS permissions.
...
2. If modifying the **Instance type**, ensure compatibility with your HyperPod cluster. Contact your admin if compatible instances aren't available.
3. For GPU-partitioned instances with MIG enabled, select an appropriate **GPU partition** from available MIG profiles to optimize GPU utilization. For more information, see [Using GPU partitions in Amazon SageMaker HyperPod](sagemaker-hyperpod-eks-gpu-partitioning.md "sagemaker-hyperpod-eks-gpu-partitioning.md").
4. If using task governance, configure priority settings for model deployment preemption capabilities.
5. Enter the namespace provided by your admin. Contact your admin for the correct namespace if needed.
...
[Setting up an Amazon EKS cluster in Studio](sagemaker-hyperpod-studio-setup-eks.md "sagemaker-hyperpod-studio-setup-eks.md") and [Setting up Kubernetes role-based access control](sagemaker-hyperpod-eks-setup-rbac.md "sagemaker-hyperpod-eks-setup-rbac.md").
