...
You can create an Autopilot image classification experiment programmatically by calling the
...
`CreateAutoMLJobV2` API action used in image classification.

## Required parameters

When calling `CreateAutoMLJobV2` to create an Autopilot experiment for image classification,
...
your image classification AutoML job.
