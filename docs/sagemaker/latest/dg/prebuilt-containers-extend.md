...
+ [Extend SageMaker AI Containers to Run a Python Script](#prebuilt-containers-extend-tutorial)
...
1. On the **Create notebook instance** page, provide the following information: 
   ...
   1. In the **Permissions and encryption** section, do the following:
      ...
      1. On the **Create an IAM role** page, choose **Specific S3 buckets**, specify an Amazon S3 bucket named **sagemaker-run-script**, and then choose **Create role**.
         ...
         SageMaker AI creates an IAM role named `AmazonSageMaker-ExecutionRole-{{YYYYMMDD}}T{{HHmmSS}}`, such as `AmazonSageMaker-ExecutionRole-20190429T110788`. Note that the execution role naming convention uses the date and time when the role was created, separated by a `T`.
...
1. Set `role` to the role used to create your Jupyter notebook. This is used to configure your SageMaker AI ModelTrainer.
...
1. Paste the following example script into the notebook code cell to configure a SageMaker AI ModelTrainer using your extended container.
...
1. Use the `ecr_image` retrieved from the previous step to configure a SageMaker AI ModelTrainer object. The following code sample configures a SageMaker AI PyTorch ModelTrainer.
