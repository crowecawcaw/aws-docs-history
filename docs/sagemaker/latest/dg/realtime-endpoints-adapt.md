...
+ {{adapter\_ic\_name}} – A unique name for your adapter inference component.
+ {{endpoint\_name}} – The name of the endpoint that hosts the base inference component.
+ {{base\_inference\_component\_name}} – The name of the base inference component that contains the foundation model to adapt.
+ {{adapter\_s3\_uri}} – The S3 URI that locates the `tar.gz` file with your LoRA adapter artifacts.
...
```
When you use this example in your own code, replace the placeholder values as follows:  
+ {{endpoint\_name}} – The name of the endpoint that hosts the base and adapter inference components.
