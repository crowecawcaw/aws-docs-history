...
Tier 1 uses SageMaker AI's native `DataCaptureConfig` to capture inference input and output payloads at the endpoint level. This is the outermost capture point and is compatible with SageMaker AI Model Monitor for automated data quality monitoring. For more information about SageMaker AI data capture, see [Capture data from real-time endpoint](model-monitor-data-capture.md "model-monitor-data-capture.md").
