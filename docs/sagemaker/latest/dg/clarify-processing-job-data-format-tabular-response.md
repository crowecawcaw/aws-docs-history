...
| Two records | '"dog",0.6,"[\\'cat\\', \\'dog\\', \\'fish\\']","[0.1, 0.6, 0.3]"\\n""cat",0.7,[\\'cat\\', \\'dog\\', \\'fish\\']","[0.7, 0.2, 0.1]"' | 
...
For bias analysis, the previous example can be configured as **one** of the following.
+ Set the `label` parameter of the `predictor` configuration to JMESPath expression "predicted\_label" to extract the predicted label.
...
Set the `label` parameter of the `predictor` configuration to JMESPath expression "predicted\_labels", and then the SageMaker Clarify processing job can extract the predicted labels for bias analysis.
