...
1. Create a custom `ModelRunner` and implement a predict method. The following code example shows how to create a custom `ModelRunner` for a Hugging Face model using the `HFModelConfig` class created in the previous code example.
   ...
   For more `ModelRunner` examples, see the [model\_runner](https://github.com/aws/fmeval/tree/main/src/fmeval/model_runners) section of the `fmeval` library.
   ...
   + The model is set to a pre-trained model from the Hugging Face [Auto Class](https://huggingface.co/transformers/v3.5.1/model_doc/auto.html) that is specified using the model\_name parameter upon instantiation.
...
1. Test your `ModelRunner`. Send a sample request to your model.
   ...
   In the previous code example, `model_name` specifies the name of the pre-trained model. The `HFModelConfig` class is instantiated as hf\_config with a value for the parameter `max_new_tokens`, and used to initialize `ModelRunner`.
