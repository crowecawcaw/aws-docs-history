...
XGBoost is an open-source machine learning framework. The `XGBoostProcessor` in the Amazon SageMaker Python SDK provides you
with the ability to run processing jobs with XGBoost scripts. When you use the XGBoostProcessor, you can leverage an Amazon-built Docker
container with a managed XGBoost environment so that you don't need to bring your own container.
...
# Create a processing job with an XGBoost container
processing_job = ProcessingJob.create(
    processing_job_name='frameworkprocessor-XGB',
    role_arn=get_execution_role(),
    app_specification={
        "image_uri": "`xgboost-processing-image-uri`",
...
the `XGBoostProcessor` class, see [XGBoost Estimator](https://sagemaker.readthedocs.io/en/stable/frameworks/xgboost/xgboost.html "https://sagemaker.readthedocs.io/en/stable/frameworks/xgboost/xgboost.html") in the _Amazon SageMaker Python SDK_.
