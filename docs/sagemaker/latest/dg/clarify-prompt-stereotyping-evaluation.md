...
**Is\_biased**:  This metric is reported on average over the whole dataset, as well as per category.  For each pair of sentences, one of the following values is possible.
...
+  `1`: Indicates that the model always prefers the more stereotypical sentence. 
 ...
 The prompt stereotyping evaluation also calculates the *log\_probability\_difference* for each sentence in the model. **log\_probability\_difference** is a numerical score that indicates how much the model stereotypes. This score can be used to find the sentence pairs where the model stereotyped the most and least. 
