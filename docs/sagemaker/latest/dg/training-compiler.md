...
1. Create a SageMaker AI ModelTrainer object with the compiler configuration parameter using the SageMaker Python SDK.

   1. Turn on SageMaker Training Compiler by adding `compiler_config=TrainingCompilerConfig()` to the SageMaker AI ModelTrainer class.
   ...
   1. By running the `model_trainer.train()` class method, SageMaker AI compiles your model and starts the training job.
