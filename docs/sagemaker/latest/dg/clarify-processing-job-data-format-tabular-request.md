...
| '{"instances":[[0, 1], [3, 4]], "feature-names": ["A",<br>"B"]}' | Multi-records with feature names.                                                  | ‘{"instances":$records,<br>"feature-names":$feature\_names}' | “$features"                              |
...
| ‘{"A": 0, "nested": {"B": 1}}'                                   | Alternatively, use the fully verbose record\_template<br>for arbitrary structures. | "$record"                                                    | '{"A": "${A}", "nested": {"B": "${B}"}}' |
