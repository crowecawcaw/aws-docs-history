...
supported AMI versions. In some cases, an AMI version might need to be designated end of
...
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------- |
| **Major** | HyperPod AMI major version releases involve upgrading core<br>components (EFA, NVIDIA driver, NCCL, CUDA, OS kernel) to new major<br>versions. These releases might introduce breaking changes, such as<br>NVIDIA driver changes from 570.x to 580.x, that require workload<br>validation. Major versions are denoted by the first number in the<br>version string (for example, 1.0, 2.0, 3.0). | 12 months                             |
...
versions from the following table. The **Supported package
...
security patches. You are responsible for managing any vulnerabilities that might
...
version reaches end of support. If auto-patching is enabled, HyperPod applies patch updates
automatically without disrupting running workloads.
