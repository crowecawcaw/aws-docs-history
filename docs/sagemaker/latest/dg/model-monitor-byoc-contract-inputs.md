...
| dataset\_source | If you are using a real-time endpoint, the local path in which the data corresponding to the monitoring period, as specified by `start_time` and `end_time`, are available. At this path, the data is available in` /{endpoint-name}/{variant-name}/yyyy/mm/dd/hh`.<br />We sometimes download more than what is specified by the start and end times. It is up to the container code to parse the data as required. | 
...
| \*sagemaker\_endpoint\_datacapture\_prefix\* | If you are using a real-time endpoint, the prefix specified in the `DataCaptureConfig` parameter of the `Endpoint`. The container can use this if it needs to directly access more data than already downloaded by SageMaker AI at the `dataset_source` path. | 
