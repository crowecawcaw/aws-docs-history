...
| cluster_max_task_wait_time   | Maximum wait time for Slurm jobs in the cluster     | N/A                                       | ## Instance level metrics The following instance-level metrics are available for HyperPod. These metrics also use the `ClusterId` dimension to identify the specific HyperPod cluster. |
| CloudWatch metric name       | Notes                                               | Amazon EKS Container Insights metric name |
| ---                          | ---                                                 | ---                                       |
