...
text/csv;label\_size=1 where the first column of each row represents the anomaly label:
...
The `sagemaker.inputs.s3_input` method is replaced by
the `Channel` and `S3DataSource` shapes in
