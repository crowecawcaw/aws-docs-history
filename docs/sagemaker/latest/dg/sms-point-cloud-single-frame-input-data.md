...
| `source-ref-metadata` | Yes | JSON object<br />**Accepted parameters**: <br />`format`, `unix-timestamp`, `ego-vehicle-pose`, `position`, `prefix`, `images` | Use this parameter to include additional information about the point cloud in `source-ref`, and to provide camera data for sensor fusion.  | 
...
| `unix-timestamp` | Yes | Number<br />A unix timestamp.  | The unix timestamp is the number of seconds since January 1st, 1970 until the UTC time that the data was collected by a sensor.  | 
...
| `heading` | Yes | JSON Object<br />**Required Parameters**:<br />`qx`, `qy`, `qz`, and `qw`. Enter numbers for these parameters.  | The orientation of the frame of reference of the camera mounted on the vehicle capturing images, measured using [quaternions](https://en.wikipedia.org/wiki/Quaternion), (`qx`, `qy`, `qz`, `qw`), in the world coordinate system.  | 
