...
`prefix` is the path within the bucket where SageMaker AI stores the data for the
...
`sagemaker`, and be globally unique. The bucket must be in the same
...
The name of the bucket doesn't need to contain `sagemaker` if the
