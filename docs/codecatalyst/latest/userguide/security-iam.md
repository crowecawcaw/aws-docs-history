...
| UntagResource                                                 | Required to remove tags associated with an account connection. This is an IAM policy permission only, not an API action.                                                          | `arn:aws:codecatalyst:region:`account_ID`:/connections/`connection_ID``                                   | ### Required permissions for IAM Identity Center applications The following permissions are required for working with IAM Identity Center applications. |
