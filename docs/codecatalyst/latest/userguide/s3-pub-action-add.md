...
Use the following instructions to add the **Amazon S3 publish** action to
your workflow.
...
9. Search for the **Amazon S3 publish** action, and do one of the
   following:
   - Choose the plus sign (**+**) to add the action to the
     workflow diagram and open its configuration pane.

   Or
   - Choose **Amazon S3 publish**. The action details dialog box
...
10. In the **Inputs**, **Configuration**, and
    **Outputs** tabs, complete the fields according to your needs.
    For a description of each field, see the ['Amazon S3 publish' action YAML](s3-pub-action-ref.md "s3-pub-action-ref.md"). This reference provides detailed information
    on each field (and corresponding YAML property value) as it appears in both the YAML
    and visual editors.
...
10. Modify the properties in the YAML code according to your needs. An explanation
    of each available property is provided in the ['Amazon S3 publish' action YAML](s3-pub-action-ref.md "s3-pub-action-ref.md").
11. (Optional) Choose **Validate** to validate the workflow's YAML
    code before committing.
12. Choose **Commit**, enter a commit message, and then choose
    **Commit** again.
