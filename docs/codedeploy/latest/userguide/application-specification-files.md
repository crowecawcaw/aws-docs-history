...
For information about how to create a well-formed AppSpec file, see [CodeDeploy AppSpec file reference](reference-appspec-file.md "reference-appspec-file.md").
...
console. The AppSpec file is used to specify:

- The name of the Amazon ECS service and the container name and port used to direct traffic
  to the new task set.
...
list of scripts to execute. The scripts are run sequentially, in the order in which they
appear in the file. The status of each script is logged in the CodeDeploy agent log file on the
instance.
...
The CodeDeploy agent is not used in an AWS Lambda or an
Amazon ECS deployment.

During the **Install** event, the CodeDeploy agent uses the
mappings defined in the **files** section of the AppSpec file
to determine which folders or files to copy from the revision to the instance.
...
For information about CodeDeploy agent log files, see [Working with the CodeDeploy agent](codedeploy-agent.md "codedeploy-agent.md").
