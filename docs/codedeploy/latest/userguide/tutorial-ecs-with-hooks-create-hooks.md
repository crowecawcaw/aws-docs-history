...
the updated Amazon ECS application is installed. For this tutorial, the Lambda function returns
`Succeeded`. During a real world deployment, validation tests return
`Succeeded` or `Failed`, depending on the result of the validation
...
1. Create a file named `AfterAllowTestTraffic.js` with the following
   contents.
...
4. Make a note of your Lambda function ARN in the `create-function` response.
   You use this ARN when you update your CodeDeploy deployment's AppSpec file in the next step.
