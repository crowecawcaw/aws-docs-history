...
When you create a connection, you also create or use an existing connector app installation for the auth handshake with the third-party provider.
...
You use the [CreateConnection](https://docs.aws.amazon.com/codeconnections/latest/APIReference/API_CreateConnection.html) API to create a connection using the AWS Command Line Interface (AWS CLI), SDK, or CloudFormation. After it is created, the connection is in a `pending` state. You complete the process by using the console **Set up pending connection** option. The console prompts you to create an installation or use an existing installation for the connection. You then use the console to complete the handshake and move the connection to an `available` state by choosing **Complete connection** on the console.
