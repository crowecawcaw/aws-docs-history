...
Custom data sources optionally support document-level access control. Unlike crawled connectors, a custom data source has no native permission system, so you supply the access control list (ACL) for each document when you ingest it with the `IngestKnowledgeBaseDocuments` operation. For the overview of ACL awareness across all connectors, see [Access Control Lists awareness enablement](kb-managed-acl.md "kb-managed-acl.md").
...
To enable ACL awareness for a custom data source, set `aclEnabled` to `true` in the `connectorParameters` when you create or update the data source. For the data source configuration structure, see [Custom](kb-managed-ds-custom.md "kb-managed-ds-custom.md").
...
The `.metadata.json` file uses the same format as the [per-document metadata files](kb-managed-ds-s3-acl.md#kb-managed-ds-s3-acl-perdoc "kb-managed-ds-s3-acl.md#kb-managed-ds-s3-acl-perdoc") for Amazon S3 data sources. This file uses capitalized ACL field names (`Name`, `Type`, `Access`), which differ from the lowercase field names (`name`, `type`, `access`) used in the inline request.
...
2. Confirm each ingested document includes an `accessControlList` under `metadata`, and that `metadata.type` matches the ACL source (`IN_LINE_ATTRIBUTE` for inline, `S3_LOCATION` for an S3 file).
3. Confirm the ACL entry field casing matches the source: lowercase `name`/`type`/`access` for inline ACLs, capitalized `Name`/`Type`/`Access` for the S3 `.metadata.json` file.
4. Confirm a test user's email exactly matches the `name` in an `ALLOW` entry for a document you expect them to retrieve.
