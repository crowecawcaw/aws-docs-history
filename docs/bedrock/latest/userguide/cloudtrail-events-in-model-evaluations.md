...
known as data plane operations. Data events are often high-volume activities that CloudTrail
...
- **Amazon Bedrock**: Data events for all model inference run
  during the model evaluation job.
- **Amazon SageMaker AI**: Data events for all human-based model
  evaluation jobs.
- **Amazon S3**: Data events for reading and writing data to
  the Amazon S3 bucket specified when the model evaluation job was created.
- **AWS Key Management Service**: Data events related to using customer
