...
2. **Embedded template as fallback** — If you do not provide a separate chat template file, Amazon Bedrock will use the `chat_template` field from your `tokenizer_config.json` if present.
...
For a complete example with multimodal support, see the [Qwen2-VL chat template](https://huggingface.co/Qwen/Qwen2-VL-72B-Instruct/blob/main/chat_template.json "https://huggingface.co/Qwen/Qwen2-VL-72B-Instruct/blob/main/chat_template.json") on Hugging Face.
...
2. Add the custom chat template to the tokenizer configuration
3. Apply the chat template to a conversation history
4. Generate the formatted text that can be sent to the model
