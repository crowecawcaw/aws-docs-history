...
We recommend [Set up Microsoft Entra ID App-Only authentication for SharePoint](kb-managed-sharepoint-entra-setup.md "kb-managed-sharepoint-entra-setup.md") instead. OAuth 2.0 authentication has two limitations:
...
4. For **Name**, enter a descriptive name, such as `bedrock-sharepoint-oauth2`.
5. For **Supported account types**, select **Accounts in this organizational directory only (Single tenant)**.
6. Leave **Redirect URI** blank, and choose **Register**.
7. On the application **Overview** page, record the **Application (client) ID** and the **Directory (tenant) ID**.
...
3. Choose **Add a permission** again. Choose **SharePoint** (under **Microsoft APIs**), then **Delegated permissions**. Select `AllSites.Read` (Read items in all site collections), then choose **Add permissions**.
...
After you store the secret, create the data source with `authType` set to `OAUTH2_APP`. You do not provide a certificate for this method. See [Connect a SharePoint data source](kb-managed-ds-sharepoint-connect.md "kb-managed-ds-sharepoint-connect.md").
