...
Atlassian Confluence is a collaborative work-management tool for sharing, storing,
and working on documents and pages. You can connect your Confluence Cloud instance as a
data source for your managed knowledge base to crawl pages, blog posts, and their
attachments. Only Confluence Cloud (SaaS) is supported; Confluence Server and Data
Center are not.
...
- Note your Confluence Cloud instance URL (for example,
  `https://example.atlassian.net`). The URL is the
  base URL ending with `.atlassian.net`.
...
- For the exact key-value pairs to store in the secret, see the setup page
  for your authentication method: [Set up Basic authentication for Confluence](kb-managed-confluence-basic-setup.md "kb-managed-confluence-basic-setup.md") or [Set up OAuth 2.0 authentication for Confluence](kb-managed-confluence-oauth2-setup.md "kb-managed-confluence-oauth2-setup.md").
...
3. **(Optional) Enable document-level access
   control.** Filter query results by each user's Confluence
   permissions. Requires Basic authentication with Atlassian organization
   admin credentials. See [Document-level access controls](kb-managed-ds-confluence-acl.md "kb-managed-ds-confluence-acl.md").
