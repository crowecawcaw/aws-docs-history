...
Watermark detection for the Amazon Bedrock console and API is available in public preview release and will detect watermarks generated from Titan Image Generator G1 and Amazon Nova Canvas. This feature is currently only available in the `us-west-2` and `us-east-1` Regions. Watermark detection is a highly accurate detection of
the watermark generated by these models. Images that are modified from the original image may produce less accurate detection results.

These models add an invisible watermark to all generated images to reduce the spread of misinformation, assist with
...
Titan Image Generator G1 or Amazon Nova Canvas, which checks for the existence of this watermark.
...
4. Select **Upload image** and locate a file that is in JPG or PNG format. The maximum file size allowed is 18 MB.
