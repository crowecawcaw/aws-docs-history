...
| Migrating from OpenAI APIs | Use the bedrock-runtime endpoint with the [Responses API](inference-responses-api.md#bedrock-mantle-responses) or [Chat Completions API](inference-chat-completions.md) — same OpenAI shape, plus Amazon Bedrock features such as [Guardrails](guardrails.md) and [cross-Region inference](cross-region-inference.md). | 
