...
Reward functions evaluate response quality and provide feedback signals for model training. You can set up
reward functions using custom Lambda functions. Choose the approach that matches your task requirements.
...
- **Objective tasks** – For objective tasks like code generation or math reasoning, use verifiable rule-based graders that check correctness against
  known standards or test cases.
- **Subjective tasks** – For subjective tasks like instruction following or chatbot interactions, call Amazon Bedrock foundation models as judges within your
  Lambda function to evaluate response quality based on your criteria.
...
- The Lambda execution role needs permissions to invoke the Lambda function as described in [Lambda permissions for reward functions](rft-open-weight-access-security.md#openai-fine-tuning-lambda-permissions "rft-open-weight-access-security.md#openai-fine-tuning-lambda-permissions").
