...
API (see Amazon Bedrock [supported SDKs and AWS CLI](../APIReference/welcome.md "../APIReference/welcome.md")). Currently, only Amazon OpenSearch Serverless vector store is available to use with this data source.
...
There are limits to how many web page content items and MB per content item that can be crawled. See [Quotas for knowledge bases](quotas.md "quotas.md").

###### Topics

- [Supported features](#supported-features-webcrawl-connector "#supported-features-webcrawl-connector")
- [Prerequisites](#prerequisites-webcrawl-connector "#prerequisites-webcrawl-connector")
- [Connection configuration](#configuration-webcrawl-connector "#configuration-webcrawl-connector")
...
by changing the crawling configuration - see [Connection configuration](#configuration-webcrawl-connector "#configuration-webcrawl-connector").
...
- Check the path to robots.txt corresponding to your source URLs doesn't block
  ...
  website. The Web Crawler respects robots.txt in accordance with the [RFC 9309](https://www.rfc-editor.org/rfc/rfc9309.html "https://www.rfc-editor.org/rfc/rfc9309.html"). You can
  also specify custom User Agent header suffix to set rules for your own crawler.
  For more information, see Web Crawler URL access in [Connection configuration](#configuration-webcrawl-connector "#configuration-webcrawl-connector") instructions on this
  page.
- [Enable CloudWatch Logs delivery](knowledge-bases-logging.md "knowledge-bases-logging.md") and follow examples of Web Crawler logs to view the status of your data ingestion job for
...
[Supported file types](knowledge-base-ds.md "knowledge-base-ds.md") are crawled regardless of scope and if there's
...
first look for `bedrockbot-UUID` rules and then for generic
`bedrockbot` rules in the robots.txt file.
...
To sync your data source with your knowledge base, use the [StartIngestionJob](../APIReference/API_agent_StartIngestionJob.md "../APIReference/API_agent_StartIngestionJob.md") API or select your knowledge
...
`bedrock:Retrieve` permissions to retrieve the data. This can also include any data with controlled
...
1. Follow the steps at [Create a knowledge base by connecting to a data source in Amazon Bedrock Knowledge Bases](knowledge-base-create.md "knowledge-base-create.md") and choose **Web Crawler** as the data source.
...
6. Configure the following in the **Sync scope** section:
   ...
   [Supported file types](knowledge-base-ds.md "knowledge-base-ds.md") are crawled regardless of scope and if there's
   no exclusion pattern for the file type. 2. Enter **Maximum throttling of crawling speed**. Ingest URLs between 1 and 300 URLs per host per minute. A higher crawling speed increases the load but takes less time. 3. Enter **Maximum pages for data source sync** between 1 and 25000. Limit the maximum number of web pages crawled from your source URLs. If web pages exceed this number the data source sync will fail and no web pages will be ingested. 4. For **URL Regex** patterns (optional) you can add **Include patterns** or **Exclude patterns** by entering the regular expression pattern in the box.
...
API
To connect a knowledge base to a data source using WebCrawler, send a [CreateDataSource](../APIReference/API_agent_CreateDataSource.md "../APIReference/API_agent_CreateDataSource.md") request with an [Agents for Amazon Bedrock build-time endpoint](../../../general/latest/gr/bedrock.md#bra-bt "../../../general/latest/gr/bedrock.md#bra-bt"), specify `WEB` in the `type` field of the [DataSourceConfiguration](../APIReference/API_agent_DataSourceConfiguration.md "../APIReference/API_agent_DataSourceConfiguration.md"), and include the `webConfiguration` field. The following is an example of a configuration of Web Crawler for your Amazon Bedrock
knowledge base.
