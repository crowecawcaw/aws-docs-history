...
| nextToken  | If there are more results than the number you specified in the `maxResults` field, the response returns a `nextToken` value. To see the next batch of results, send the `nextToken` value in another request. | [See code examples](bedrock-agent_example_bedrock-agent_ListAgentKnowledgeBases_section.md "bedrock-agent_example_bedrock-agent_ListAgentKnowledgeBases_section.md") |
