...
_Helpfulness_ – Evaluates whether the response is helpful and useful to the user. Responses are graded on a 7-point Likert scale, and then normalized in the output and the job's report card. The `{{chat_history}}` contains the conversation history, the `{{prompt}}` will contain the prompt sent to the generator from your dataset, the `{{context}}` contains the retrieved context passages, and the `{{prediction}}` is the generator model's responses.
...
_Faithfulness_ – Evaluates whether the response is faithful to the retrieved contexts. Responses are graded on a 5-point Likert scale, and then normalized in the output and the job's report card. The `{{chat_history}}` contains the conversation history, the `{{prompt}}` will contain the prompt sent to the generator from your dataset, the `{{context}}` contains the retrieved context passages, and the `{{prediction}}` is the generator model's responses.

```
You are given a task in some context (Input), and a candidate answer, which you need to assess whether it is faithful to the context.

Does the candidate answer contain information that is not included in the Input (task description and passages), or that cannot be easily inferred from the Input via common sense knowledge?
...
Evaluate how much of the information in the answer is contained in the available context (or can be easily inferred from the context via common sense knowledge).
