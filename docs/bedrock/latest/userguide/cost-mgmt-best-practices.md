...
+ **Per-user or per-team dollars on your bill.** Use [IAM principal attribution](cost-mgmt-iam-principal-tracking.md). The caller identity is captured automatically on every Amazon Bedrock inference call on the `bedrock-runtime` and `bedrock-mantle` endpoints, and you can layer on principal or session tags for team, department, or cost center.
