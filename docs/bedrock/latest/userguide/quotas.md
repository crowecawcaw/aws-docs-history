...
Amazon Bedrock offers two inference endpoints – `bedrock-runtime` and `bedrock-mantle` – each with its own per-model quota allocations. Traffic to the two endpoints is tracked against separate quotas, even when calling the same underlying model. For details, see [Quotas for the bedrock-runtime endpoint](quotas-runtime.md "quotas-runtime.md") and [Quotas for the bedrock-mantle endpoint](quotas-mantle.md "quotas-mantle.md").
...
- [Quotas for the bedrock-mantle endpoint](quotas-mantle.md "quotas-mantle.md")
