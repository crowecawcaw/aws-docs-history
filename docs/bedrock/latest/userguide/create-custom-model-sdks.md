...
To create a custom model from a SageMaker AI-trained Amazon Nova model stored in Amazon S3, you use the [CreateCustomModel](../APIReference/API_CreateCustomModel.md "../APIReference/API_CreateCustomModel.md") API operation. You can use the following code to create a custom model with
the SDK for Python (Boto3). The code creates a custom model and then checks its status until the model is
...
- **s3Uri** – Specify the path to the Amazon-managed Amazon S3 bucket that stores your model artifacts. SageMaker AI creates this bucket when you run your first SageMaker AI
  training job.
- **roleArn** – Specify the Amazon Resource Name (ARN) of an IAM service role that Amazon Bedrock assumes to perform tasks on
  your behalf. For more information about creating this role, see [Create a service role for importing pre-trained models](model-import-iam-role.md "model-import-iam-role.md").
- **modelKmsKeyArn** (optional) – Specify a AWS KMS key to encrypt the model in Amazon Bedrock. If you don't
  provide a AWS KMS key, Amazon Bedrock uses an AWS-managed AWS KMS key to encrypt the model. For information about
  encryption, see [Encryption of imported custom models](encryption-import-model.md "encryption-import-model.md").
  After you create the custom model, the model appears in the [ListCustomModels](../APIReference/API_ListCustomModels.md "../APIReference/API_ListCustomModels.md") response with a
  `customizationType` of `imported`. To track the status of the new model, you use
  the [GetCustomModel](../APIReference/API_GetCustomModel.md "../APIReference/API_GetCustomModel.md") API operation.
