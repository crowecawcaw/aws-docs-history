...
- **Destination Region** – A Region where the Amazon Bedrock service can
  route the guardrail inference request.
  The guardrail profiles that you can use depend on the source Region where your guardrail
  resides.
...
To use a guardrail profile in the US geographic boundary, specify the following
guardrail profile ID or Amazon Resource Name (ARN) in one of the source Regions:
