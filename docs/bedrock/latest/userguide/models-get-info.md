...
Amazon Bedrock provides two separate inference endpoints, each with its own API for listing the models available on it. For an overview of the endpoints, see [Endpoints supported by Amazon Bedrock](endpoints.md "endpoints.md").
...
- **`bedrock-runtime`** – Use the AWS-native [ListFoundationModels](../APIReference/API_ListFoundationModels.md "../APIReference/API_ListFoundationModels.md") API. The response returns a [FoundationModelSummary](../APIReference/API_FoundationModelSummary.md "../APIReference/API_FoundationModelSummary.md") object for each model, including its ARN, model ID, supported modalities and features, and deprecation status. To get details for a specific model, use [GetFoundationModel](../APIReference/API_GetFoundationModel.md "../APIReference/API_GetFoundationModel.md"), specifying the [model ID](models-supported.md "models-supported.md").

###### Note

The response also returns model IDs that aren't in the [base model ID](models-supported.md "models-supported.md") or base model IDs for Provisioned Throughput charts. These model IDs are deprecated or for backwards compatibility.
