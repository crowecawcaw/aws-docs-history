...
data when you delete your data source, but **doesn't delete the vector store
itself**) or "Retain" (retains all data when you delete
your data source). If you delete a data source or knowledge base, the
...
API
To delete a data source from a knowledge base, send a [DeleteDataSource](../APIReference/API_agent_DeletDataSource.md "../APIReference/API_agent_DeletDataSource.md") request, specifying the `dataSourceId` and `knowledgeBaseId`.
...
itself**) or `RETAIN` (retains all data when you delete
your data source). If you delete a data source or knowledge base, the
**vector store itself is not deleted**. If the data source data
deletion policy is set to `DELETE`, it's possible for the data source to unsuccessfully
complete the process of deletion due to issues with the configuration or access to the vector store.
You can view `failureReasons` if the data source status is `DELETE_UNSUCCESSFUL` to see
the reason why the data source could not successfully delete.
