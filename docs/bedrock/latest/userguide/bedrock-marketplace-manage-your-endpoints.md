...
4. Choose **Register**, **Edit**, or
   **Delete** from the **Action** bar.
5. To view
   additional details,
   choose
   the
   endpoint.
   You can use the following AWS Command Line Interface command to delete the
   endpoint.
...
You can use the following AWS Command Line Interface command to update the
endpoint.
...
`UpdateMarketplaceModelEndpoint` operations are the same.
