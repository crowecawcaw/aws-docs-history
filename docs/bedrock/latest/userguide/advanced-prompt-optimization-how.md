...
By default, the optimizer rewrites any part of your entire prompt template. To limit optimization to specific sections of your template, use `<advpo:optimize>` or `<advpo:exclude>` tags. For example, the following template optimizes only the response instructions while preserving the rest:
