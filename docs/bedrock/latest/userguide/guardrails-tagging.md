...
In the preceding example, the content _`How many objects do I have in my S3
 bucket?`_ and "_"How do I download files from my S3
bucket?_" is tagged for guardrails processing using the tag
`<amazon-bedrock-guardrails-guardContent_xyz>`. Note that the prefix
`amazon-bedrock-guardrails-guardContent` is reserved by
guardrails.

**Tag Suffix**

The tag suffix (`xyz` in the preceding example) is a dynamic value that
you must provide in the `tagSuffix` field in
`amazon-bedrock-guardrailConfig` to use input tagging. It is
...
tag and appending malicious content after the tag closure, resulting in an
_injection attack_. You are limited to alphanumeric
...
XML tags with your suffix:
`<amazon-bedrock-guardrails-guardContent_xyz>``your
...
exception is
[Detect prompt attacks with Amazon Bedrock Guardrails](guardrails-prompt-attack.md "guardrails-prompt-attack.md")
filters, which require input tags to be present.
