...
You can use a guardrail to guard conversational apps that you create with the Converse
API. For example, if you create a chat app with Converse API, you can use a guardrail to block
inappropriate content entered by the user and inappropriate content generated by the model.
For information about the Converse API, see [Inference using Converse API](conversation-inference.md "conversation-inference.md").

###### Topics

- [Call the Converse API with guardrails](#guardrails-use-converse-api-call "#guardrails-use-converse-api-call")
- [Processing the response when using the Converse API](#guardrails-use-converse-api-response "#guardrails-use-converse-api-response")
- [Code example for using Converse API with guardrails](#converse-api-guardrail-example "#converse-api-guardrail-example")
...
- [Configure a guardrail to work with the Converse API](#guardrails-use-converse-api-call-configure "#guardrails-use-converse-api-call-configure")
- [Evaluate only specific content in a message](#guardrails-use-converse-api-call-message "#guardrails-use-converse-api-call-message")
...
With the `Converse` operation, `guardrailConfig` is a
[GuardrailConfiguration](../APIReference/API_runtime_GuardrailConfiguration.md "../APIReference/API_runtime_GuardrailConfiguration.md") object, as shown in the following
example.
...
If you use `ConverseStream`, you pass a [GuardrailStreamConfiguration](../APIReference/API_runtime_GuardrailStreamConfiguration.md "../APIReference/API_runtime_GuardrailStreamConfiguration.md") object. Optionally, you can use the
`streamProcessingMode` field to specify that you want the model to
...
assessment in the background. For more information, see [Configure streaming response behavior to filter content](guardrails-streaming.md "guardrails-streaming.md").

### Evaluate only specific content in a message

When you pass a [Message](../APIReference/API_runtime_Message.md "../APIReference/API_runtime_Message.md") to a model, your guardrail assesses the content in the message.
You also can asses specific parts of a message by using the
`guardContent` ([GuardrailConverseContentBlock](../APIReference/API_runtime_GuardrailConverseContentBlock.md "../APIReference/API_runtime_GuardrailConverseContentBlock.md")) field.

###### Tip

Using the `guardContent` field is similar to using input tags with
[InvokeModel](../APIReference/API_runtime_InvokeModel.md "../APIReference/API_runtime_InvokeModel.md") and [InvokeModelWithResponseStream](../APIReference/API_runtime_InvokeModelWithResponseStream.md "../APIReference/API_runtime_InvokeModelWithResponseStream.md"). For more information, see [Apply tags to user input to filter content](guardrails-tagging.md "guardrails-tagging.md").

For example, the following guardrail evaluates only the content in the
`guardContent` field and not the rest of the message. This is useful
for having the guardrail assess only the most recent message in a conversation, as
shown in the following example.

###### Important

Once you include a `guardContent` block anywhere in your messages, the guardrail evaluates _only_ the content within `guardContent` blocks. All other content blocks that are not wrapped in `guardContent` are skipped entirely and not evaluated by the guardrail.
...
Another use case of `guardContent` is providing additional context for
...
If content isn't in a `guardContent` block, that doesn't necessarily mean
it won't be evaluated. This behavior depends on what filtering polices the guardrail
uses.

The following example shows two `guardContent` blocks with [contextual grounding checks](guardrails-contextual-grounding-check.md "guardrails-contextual-grounding-check.md")
(based on the `qualifiers` fields). The contextual grounding checks in the
guardrail will only evaluate the content in these blocks. However, if the guardrail also
has a [word filter](guardrails-content-filters.md "guardrails-content-filters.md") that blocks the word
"background", the text "Some additional background information." will still be
evaluated, even though it's not in a `guardContent` block.
...
`guardContent` ([SystemContentBlock](../APIReference/API_runtime_SystemContentBlock.md "../APIReference/API_runtime_SystemContentBlock.md")) field in the system prompt that you pass to the API, as shown
