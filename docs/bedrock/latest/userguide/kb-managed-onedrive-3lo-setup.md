...
<a name="kb-managed-onedrive-3lo-setup"></a>
...
**Important**  
User-managed setup does not support document-level access control (ACL). All indexed content is accessible to any user who has access to query the knowledge base. Individual permissions in OneDrive are not enforced. Carefully review which content you include when creating a knowledge base. If you require document-level access control, use Microsoft Entra App ID authentication instead. See [Set up Microsoft Entra App ID authentication for OneDrive](kb-managed-onedrive-entra-setup.md). For more information about document-level access control, see [Access Control Lists awareness enablement](kb-managed-acl.md).

## How your credentials are stored
<a name="kb-managed-onedrive-3lo-secret"></a>
...
You can create 3LO secrets and complete the user consent flow through the Amazon Bedrock Knowledge Bases console. You can then reference these AWS Secrets Manager entries when creating a third-party data connector (see [Connect a OneDrive data source](kb-managed-ds-onedrive-connect.md)). These entries are not associated with any knowledge base and can be used across separate knowledge bases as needed.

## Before you begin
<a name="kb-managed-onedrive-3lo-prereqs"></a>
...
<a name="kb-managed-onedrive-3lo-admin-consent"></a>
...
<a name="kb-managed-onedrive-3lo-consent-dialog"></a>
...
<a name="kb-managed-onedrive-3lo-consent-entra"></a>
...
1. Locate the enterprise application for the Amazon Bedrock KB feature.
**Note**  
...
### Verifying consent
<a name="kb-managed-onedrive-3lo-verify-consent"></a>
...
<a name="kb-managed-onedrive-3lo-tenant-settings"></a>
...
<a name="kb-managed-onedrive-3lo-permissions"></a>
...
## Troubleshooting
<a name="kb-managed-onedrive-3lo-troubleshooting"></a>

If you run into problems during user-managed setup, match the symptom against the following.


**User-managed setup issues**  

| Issue | Cause and resolution | 
| --- | --- | 
| App blocked by administrator | Your Microsoft 365 tenant restricts third-party app access. Ask your Microsoft 365 administrator to grant admin consent. For more information, see [Grant organization-wide admin consent](#kb-managed-onedrive-3lo-admin-consent). | 
...
## Next steps
<a name="kb-managed-onedrive-3lo-next"></a>

After you complete user-managed setup, create the data source with `authType` set to `MANAGED_OAUTH2`. See [Connect a OneDrive data source](kb-managed-ds-onedrive-connect.md).