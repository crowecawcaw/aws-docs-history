...
- You can use guardrails with Amazon Bedrock Agents and Amazon Bedrock Knowledge Bases.
  When using a guardrail in the `InvokeModel`,
  `InvokeModelWithResponseStream`, `Converse`, or
  `ConverseStream` operations, it works as follows during the inference
...
- If the input evaluation results in a guardrail intervention, a configured
  _blocked message_ response is returned and the foundation
...
- If the response results in a guardrail intervention or violation, it will be
  overridden with _pre-configured blocked messaging_ or
  _masking_ of the sensitive information based on your
  policy configuration.
- If the response's evaluation succeeds, the response is returned to the
  application without any modifications.
  For information on Amazon Bedrock Guardrails pricing, see the [Amazon Bedrock pricing](https://aws.amazon.com/bedrock/pricing/ "https://aws.amazon.com/bedrock/pricing/").
