...
**Important**  
User-managed setup does not support document-level access control (ACL). All indexed content is accessible to any user who has access to query the knowledge base. Individual permissions in SharePoint are not enforced. Carefully review which content you include when creating a knowledge base. If you require document-level access control, use Microsoft Entra ID App-Only authentication instead. See [Set up Microsoft Entra ID App-Only authentication for SharePoint](kb-managed-sharepoint-entra-setup.md). For more information about document-level access control, see [Access Control Lists awareness enablement](kb-managed-acl.md).
...
You can create 3LO secrets and complete the user consent flow through the Amazon Bedrock Knowledge Bases console. You can then reference these AWS Secrets Manager entries when creating a third-party data connector (see [Connect a SharePoint data source](kb-managed-ds-sharepoint-connect.md)). These entries are not associated with any knowledge base and can be used across separate knowledge bases as needed.
...
| --- | --- | 
| App blocked by administrator | Your Microsoft 365 tenant restricts third-party app access. Ask your Microsoft 365 administrator to grant admin consent. For more information, see [Grant organization-wide admin consent](#kb-managed-sharepoint-3lo-admin-consent). | 
