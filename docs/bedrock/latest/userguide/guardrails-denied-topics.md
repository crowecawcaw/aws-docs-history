...
- **Definition** – Up to 200 characters
  summarizing the topic content. The definition should describe the content of the
...
4. When you get to the **Add denied topics**
   ...
   5. (Optional) Expand **Add sample
      ...
      topic. You can enter up to five phrases. For each phrase
      you include, select **Add
      phrase**.
   6. For **Denied topics tier**, choose
...
API
Add denied topics to your guardrail by sending a [CreateGuardrail](../APIReference/API_CreateGuardrail.md "../APIReference/API_CreateGuardrail.md") request. Here's what the request format
looks like:
...
- Specify topics for the guardrail to deny in the
  ...
  - Set `inputEnabled` and
    ...
    responses.
  - (Optional) In the `examples` list, specify
    up to five sample phrases that you are representative of
    prompts or responses related to this topic.

- (Optional) Specify a safeguard tier for your guardrail in the
  `tierConfig` object. Options include
...
- (Optional) To enable [cross-Region inference](guardrails-cross-region.md "guardrails-cross-region.md"), specify a guardrail profile
  in the `crossRegionConfig` object. This is required
