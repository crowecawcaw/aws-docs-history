...
- **Amazon Nova Micro, Lite, and Pro prompt guide:**
  [Prompting
  best practices for Amazon Nova understanding models](../../../nova/latest/userguide/prompting.md "../../../nova/latest/userguide/prompting.md")
- **Amazon Nova Canvas prompt guide:**
  [Generating images with Amazon Nova](../../../nova/latest/userguide/image-generation.md "../../../nova/latest/userguide/image-generation.md")
- **Amazon Nova Reel prompt guide:**
  [Generating videos with Amazon Nova](../../../nova/latest/userguide/video-generation.md "../../../nova/latest/userguide/video-generation.md")
- **Anthropic Claude model prompt
  guide:**
  [https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview "https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview")
- **Cohere prompt guide:**
...
the model would not have enough information to produce a sensible summary. The
_instruction_ tells the LLM what to do, and the
text is the _input_ on which the LLM operates. The
_context_ (`The following is text
...
In the example below, the text `Context: Climate change threatens people
 with increased flooding ...` is the _input_ which the LLM can use to perform the _task_ of answering the question `Question: What
...
calibrate
their output to meet your expectations, also known as _few-shot prompting_ or _in-context
learning_, where a _shot_ corresponds
...
the examples to avoid confusion with the delimiters `Human:` and
`Assistant:` for the whole prompt. Notice that for the last few-shot
