...
For greater control of session context, you can modify the [SessionState](../APIReference/API_agent-runtime_SessionState.md#bedrock-Type-agent-runtime_SessionState "../APIReference/API_agent-runtime_SessionState.md#bedrock-Type-agent-runtime_SessionState") object in your agent. The [SessionState](../APIReference/API_agent-runtime_SessionState.md#bedrock-Type-agent-runtime_SessionState "../APIReference/API_agent-runtime_SessionState.md#bedrock-Type-agent-runtime_SessionState") object contains information that can be maintained across turns (separate [InvokeAgent](../APIReference/API_agent-runtime_InvokeAgent.md "../APIReference/API_agent-runtime_InvokeAgent.md") request and responses). You can use this information to provide conversational context for the agent during user conversations.

The general format of the [SessionState](../APIReference/API_agent-runtime_SessionState.md#bedrock-Type-agent-runtime_SessionState "../APIReference/API_agent-runtime_SessionState.md#bedrock-Type-agent-runtime_SessionState") object is as follows.
...
Select a topic to learn more about fields in the [SessionState](../APIReference/API_agent-runtime_SessionState.md#bedrock-Type-agent-runtime_SessionState "../APIReference/API_agent-runtime_SessionState.md#bedrock-Type-agent-runtime_SessionState") object.

###### Topics

- [Session and prompt session attributes](#session-state-attributes "#session-state-attributes")
- [Session attribute example](#session-attribute-ex "#session-attribute-ex")
- [Prompt session attribute example](#prompt-session-attribute-ex "#prompt-session-attribute-ex")
- [Action group invocation results](#session-state-return-control "#session-state-return-control")
- [Knowledge base retrieval configurations](#session-state-kb "#session-state-kb")
...
- **sessionAttributes** – Attributes that persist over a [session](advanced-prompts.md#advanced-prompts-terminology "advanced-prompts.md#advanced-prompts-terminology") between a user and agent. All [InvokeAgent](../APIReference/API_agent-runtime_InvokeAgent.md "../APIReference/API_agent-runtime_InvokeAgent.md") requests made with the same `sessionId` belong to the same session, as long as the session time limit (the `idleSessionTTLinSeconds`) has not been surpassed.
...
- When you set up an action group and [write the Lambda function](agents-lambda.md "agents-lambda.md"), include `sessionAttributes` or `promptSessionAttributes` in the [response event](agents-lambda.md#agents-lambda-response "agents-lambda.md#agents-lambda-response") that is returned to Amazon Bedrock.
- During runtime, when you send an [InvokeAgent](../APIReference/API_agent-runtime_InvokeAgent.md "../APIReference/API_agent-runtime_InvokeAgent.md") request, include a `sessionState` object in the request body to dynamically change the session state attributes in the middle of the conversation.
...
2. Write your application code to send an [InvokeAgent](../APIReference/API_agent-runtime_InvokeAgent.md "../APIReference/API_agent-runtime_InvokeAgent.md") request with the following body:
...
3. When a user uses your application and provides their first name, your code will send the first name as a session attribute and the agent will store their first name for the duration of the [session](advanced-prompts.md#advanced-prompts-terminology "advanced-prompts.md#advanced-prompts-terminology").
4. Because session attributes are sent in the [Lambda input event](agents-lambda.md#agents-lambda-input "agents-lambda.md#agents-lambda-input"), you can refer to these session attributes in a Lambda function for an action group. For example, if the action [API schema](agents-api-schema.md "agents-api-schema.md") requires a first name in the request body, you can use the `firstName` session attribute when writing the Lambda function for an action group to automatically populate that field when sending the API request.
...
3. Write your application to send an [InvokeAgent](../APIReference/API_agent-runtime_InvokeAgent.md "../APIReference/API_agent-runtime_InvokeAgent.md") request with the following body:
...
If you configured an action group to [return control in an InvokeAgent response](agents-returncontrol.md "agents-returncontrol.md"), you can send the results from invoking the action group in the `sessionState` of a subsequent [InvokeAgent](../APIReference/API_agent-runtime_InvokeAgent.md "../APIReference/API_agent-runtime_InvokeAgent.md") response by including the following fields:
