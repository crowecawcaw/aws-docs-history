...
If you've created your agent _after_ January 22, 2025, follow instructions for [Encryption of agent resources](encryption-agents-new.md "encryption-agents-new.md")

Amazon Bedrock encrypts your agent's session information. By default,
Amazon Bedrock encrypts this data using an AWS managed key. Optionally, you can encrypt the
agent artifacts using a customer managed key.
...
2. Attach the following resource-based policy to your KMS key. Change the scope of the permissions as necessary.
   Replace the `${region}`, `${account-id}`, `${agent-id}`, and `${key-id}` with the appropriate values.
