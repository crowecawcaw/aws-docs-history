...
Our simple calculator API exposes three methods (GET, POST, GET) to invoke the [Simple calculator Lambda function](simple-calc-nodejs-lambda-function.md "simple-calc-nodejs-lambda-function.md"). A graphical
representation of this API is shown as follows:

![Simple calculator API for generated SDK](images/simple-calc-api-console-hierarchy-new-console.png)
...
You can, and should, override the default SDK method names by specifying the [operationName](../api/API_Method.md#operationName "../api/API_Method.md#operationName") property on each
API method. You can do so when [creating
the API method](../api/API_PutMethod.md "../api/API_PutMethod.md") or [updating the API method](../api/API_UpdateMethod.md "../api/API_UpdateMethod.md") using the API Gateway REST API. In the API Swagger
...
let's recall briefly how to set them up. For detailed instructions, see [Develop REST APIs in API Gateway](rest-api-develop.md "rest-api-develop.md"). If you're new to API Gateway,
see [Choose an AWS Lambda integration tutorial](getting-started-with-lambda-integration.md "getting-started-with-lambda-integration.md") first.
...
2. Choose **URL query string parameters** and do the following:
   ...
   4. Keep **Caching** turned off.Repeat the same steps and create a query string named `b` and a query string named `op`.
