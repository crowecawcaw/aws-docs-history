...
AWS Lambda and Lambda functions, see the [AWS Lambda Developer Guide](../../../lambda/latest/dg.md "../../../lambda/latest/dg.md").
...
For more information, see the [AWS Lambda Developer Guide](../../../lambda/latest/dg/welcome.md "../../../lambda/latest/dg/welcome.md").
...
see [Control access to a REST API with IAM permissions](permissions.md "permissions.md").
...
API**. Under **REST API**, choose **Build**. 3. For **API name**, enter `LambdaNonProxyAPI`. 4. (Optional) For **Description**, enter a description. 5. Keep **API endpoint type** set to **Regional**. 6. For **IP address type**, select **IPv4**. 7. Choose **Create API**.
...
`time` query string parameter and a `day` header. 7. For **Request validator**, select **Validate query string parameters and headers**. 8. Choose **URL query string parameters** and do the following:
...
The input must be UTF-8 encoded (i.e., unlocalized) text. 4. (Optional) For **Description**, enter a description. 5. Choose **Deploy**.
...
as [POSTMAN](https://www.postman.com/ "https://www.postman.com/") or [cURL](https://curl.se/ "https://curl.se/").
