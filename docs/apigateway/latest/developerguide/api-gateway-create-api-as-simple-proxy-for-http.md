...
which are listed in [Amazon API Gateway important notes](api-gateway-known-issues.md "api-gateway-known-issues.md").
...
API**. Under **REST API**, choose **Build**. 3. For **API name**, enter `HTTPProxyAPI`. 4. (Optional) For **Description**, enter a description. 5. Keep **API endpoint type** set to **Regional**. 6. For **IP address type**, select **IPv4**. 7. Choose **Create API**.
...
![Create a child resource.](images/api-gateway-simple-proxy-create-proxy-resource-new-console.png)
...
![Use the test feature to test a method.](images/api-gateway-simple-proxy-petstore-call-proxy-resource-new-console.png)
...
Your browser sends a `GET` request to the API. 5. The result should be the same as returned when you use **Test**
