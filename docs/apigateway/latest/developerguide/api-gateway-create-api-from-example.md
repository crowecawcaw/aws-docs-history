...
3. Under **Create REST API**, choose **Example
   API** and then choose **Create API** to create the example
   API.

![Example REST API in the API Gateway console.](images/api-gateway-create-api-by-importing-example-new-console.png)

You can scroll down the OpenAPI definition for details of this example API before
choosing **Create API**. 4. In the main navigation pane, choose **Resources**. The newly created API is shown as follows:

![The example API after importing it into the API Gateway console.](images/api-gateway-create-api-by-importing-example-result-new-console.png)
...
integration type, authorization type, and API key requirement. 5. To view the details of a method, to modify its set-up, or to test the method
...
![The POST /pets method for the example API in the API Gateway console.](images/api-gateway-create-api-by-importing-example-post-method-execution-new-console.png)
...
testing the API to provide an end-to-end user experience. 6. Select the **Test** tab. You might need to choose the right arrow button to show the tab. 7. For example, to test the `POST /pets` method, enter
the following `{"type": "dog","price": 249.99}` payload into the **Request body**, and then choose
**Test**.

![Test the POST method in the API Gateway console.](images/api-gateway-create-api-by-importing-example-post-method-test-new-console.png)

The input specifies the attributes of the pet that we want to add to the list of
pets on the PetStore website. 8. The results display as follows:

![The result of testing the POST method in the API Gateway console.](images/api-gateway-create-api-by-importing-example-post-method-test-result-new-console.png)
...
you must first deploy the API to a stage. 9. To deploy the sample API,
choose **Deploy API**.

![Use the deploy button to deploy your API, so API callers can invoke your API.](images/api-gateway-create-api-by-importing-example-deploy-api-new-console.png) 10. For **Stage**, select **New stage**, and then enter `test`. 11. (Optional) For **Description**, enter a description. 12. Choose **Deploy**. 13. In the resulting **Stages** pane, under **Stage details**, the **Invoke
URL** displays the URL to invoke the API's `GET /` method
request.

![After you create your REST API, the console shows your API's invoke URL.](images/getting-started-rest-invoke-url.png) 14. Choose the copy icon to copy your API's invoke URL, and then enter your API's invoke URL in a web browser. A successful
response return the result, generated from the mapping template in the integration
response. 15. In the **Stages** navigation pane, expand the
