...
For more information about the model, see [Data models for REST APIs](models-mappings-models.md "models-mappings-models.md").
...
7. Under **URL query string parameters**, do the following:
   ...
   tutorial](set-up-data-transformations-in-api-gateway.md#mapping-example-console "set-up-data-transformations-in-api-gateway.md#mapping-example-console"). 4. Keep **Caching** turned off.
...
For information about how to transform this output data, see the [data mapping tutorial.](set-up-data-transformations-in-api-gateway.md#mapping-example-console "set-up-data-transformations-in-api-gateway.md#mapping-example-console") 4. Remove `petType=dog` and choose **Test**. 5. The method test will return a `400` error with the following error message:
...
Choose **Test**. 3. The method test will return `200 OK` and a success message. 4. For **Request body** paste the following into the code editor:
...
Choose **Test**. 5. The method test will return a `400` error with the following error message:
...
- For information about how to transform output data and perform more data mapping, see the [data mapping tutorial](set-up-data-transformations-in-api-gateway.md#mapping-example-console "set-up-data-transformations-in-api-gateway.md#mapping-example-console").
- Follow the [Set up basic request validation using the AWS CLI](#api-gateway-request-validation-setup-cli "#api-gateway-request-validation-setup-cli") tutorial, to do similar steps using the AWS CLI.
...
Note the ID of the `params-only` validator. 2. Use the following command to
...
Note the ID of the `body-only` validator. 3. A model is required to use request validation on the body of an incoming request. Use the following
...
The result will return a `200 OK` and list of dogs. 2. Use the following command to test without including the query string `petType`
...
The result will return a `400` error. 3. To test the `POST` method, which will perform request validation for the request body, use the following
command:
...
The result will return a `200 OK` and a success message. 4. Use the following command to test using an invalid body.
...
To turn a request validator on all methods of an API, specify an [x-amazon-apigateway-request-validator property](api-gateway-swagger-extensions-request-validator.md "api-gateway-swagger-extensions-request-validator.md") property at the API level of the OpenAPI
...
To import the OpenAPI example into API Gateway, see the following instructions to [Import a Regional API into API Gateway](import-export-api-endpoints.md "import-export-api-endpoints.md") or to
