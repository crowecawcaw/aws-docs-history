...
You can use the [`CreateAuthorizer`](../api/API_CreateAuthorizer.md "../api/API_CreateAuthorizer.md") action to create a `COGNITO_USER_POOLS`
...
4. To configure the new authorizer to use a user pool, do the following:
   ...
   `arn:aws:cognito-idp:`us-east-2`:`111122223333`:userpool/${stageVariables.`MyUserPool`}`. 4. For **Token source**, enter
   ...
   successfully. 5. (Optional) Enter a regular expression in the **Token
   ...
   field. 6. Choose **Create authorizer**.
...
calling the [Amazon Cognito Identity SDK](../../../cognito/latest/developerguide/cognito-integrate-apps.md "../../../cognito/latest/developerguide/cognito-integrate-apps.md") to
perform user sign-in. You can also use the [`InitiateAuth`](../../../cognito-user-identity-pools/latest/APIReference/API_InitiateAuth.md "../../../cognito-user-identity-pools/latest/APIReference/API_InitiateAuth.md") action.
...
5. To use an access token, do the following:
   1. For **Authorization Scopes**, enter one or more full names of a
      ...
      example, following the example given in [Create an Amazon Cognito user pool for a REST API](apigateway-create-cognito-user-pool.md "apigateway-create-cognito-user-pool.md"),
...
5. If needed, you can set other API configuration settings by using the
   appropriate OpenAPI definitions or extensions. For more information, see [OpenAPI extensions for API Gateway](api-gateway-swagger-extensions.md "api-gateway-swagger-extensions.md").
