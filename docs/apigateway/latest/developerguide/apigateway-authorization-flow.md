...
![Authorization flow of a resource policy only.](images/apigateway-auth-resource-policy-only.png)
...
document](api-gateway-lambda-authorizer-output.md "api-gateway-lambda-authorizer-output.md"), which is evaluated in conjunction with the resource policy. If your authorizer uses caching,
API Gateway might return the cached policy document. The result is determined based on [Table A](#apigateway-resource-policies-iam-policies-interaction "#apigateway-resource-policies-iam-policies-interaction").
...
![Authorization flow for a resource policy and a Lambda authorizer.](images/apigateway-auth-lambda-resource-policy.png)
...
explicitly allow the caller to proceed. For more information, see [Table B](#apigateway-resource-policies-iam-policies-interaction "#apigateway-resource-policies-iam-policies-interaction").
...
A](#apigateway-resource-policies-iam-policies-interaction "#apigateway-resource-policies-iam-policies-interaction").

![Authorization flow for a resource policy and IAM authentication.](images/apigateway-auth-iam-resource-policy.png)
...
`vpc-2f09a348`. For more information, see [Table B](#apigateway-resource-policies-iam-policies-interaction "#apigateway-resource-policies-iam-policies-interaction").
...
[JWT token](../../../cognito/latest/developerguide/amazon-cognito-user-pools-using-tokens-with-identity-providers.md "../../../cognito/latest/developerguide/amazon-cognito-user-pools-using-tokens-with-identity-providers.md") that is provided by the caller. If authentication is successful,
...
![Authorization flow for a resource policy and a Amazon Cognito authorizer.](images/apigateway-auth-cognito-resource-policy.png)

The following is an example of a resource policy that allows calls only from specified
source IPs, assuming that the Amazon Cognito authentication token contains an allow. For more information, see [Table B](#apigateway-resource-policies-iam-policies-interaction "#apigateway-resource-policies-iam-policies-interaction").
