...
The role you create will need to have Lambda [InvokeFunction](../../../lambda/latest/api/API_Invoke.md "../../../lambda/latest/api/API_Invoke.md") permission. Otherwise, the API caller will receive a
...
8. For **Action type**, select **Use path override**. This option allows us to specify the ARN of the
   [Invoke](../../../lambda/latest/dg/API_Invoke.md "../../../lambda/latest/dg/API_Invoke.md") action to
...
This setting will
cause an error message to return if the client does not specify the required parameters. 14. Choose **URL query string parameters**.
...
To create the query string parameters do the following:
    ...
    3. Turn on **Required**.
    4. Keep **Caching** turned off.Repeat the same steps and create a query string named `operand2` and a query string named `operator`.
...
regardless of the content type, enter `$default`. 5. For **Model schema**, enter the following model:
...
If no matching content type is found, request validation is not performed. To use the same model
regardless of the content type, enter `$default`. 4. For **Model schema**, enter the following model:
...
If no matching content type is found, request validation is not performed. To use the same model
regardless of the content type, enter `$default`. 4. For **Model schema**, enter the following model with your
...
If no matching content type is found, request validation is not performed. To use the same model
regardless of the content type, enter `$default`. 6. For **Model**, select **input**. 7. Choose **Save**.
...
![Create an API in API Gateway as a Lambda proxy](images/aws_proxy_lambda_create_api_resources_new_console.png)
...
![Test the GET method in the API Gateway console.](images/aws_proxy_lambda_calc_get_method_test_path_parm_new_console.png)
...
5. Choose **Deploy**.
6. (Optional) Under **Stage details**, for **Invoke URL**, you can choose the copy icon to copy your API's invoke URL. You can use this with tools such as [Postman](https://www.postman.com "https://www.postman.com") and [cURL](https://curl.se/ "https://curl.se/") to test your API.
