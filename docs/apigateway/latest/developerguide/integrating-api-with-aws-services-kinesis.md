...
and Kinesis services are available. For region availability, see [Service Endpoints and Quotas](../../../general/latest/gr/aws-service-information.md "../../../general/latest/gr/aws-service-information.md").
...
4. The `ListStreams` action in Kinesis
5. The `CreateStream`, `DescribeStream`, or
   `DeleteStream` action
6. The `GetRecords` or `PutRecords` (including
...
- Expose an HTTP GET method on the API's `/streams` resource and
  integrate the method with the [ListStreams](../../../kinesis/latest/APIReference/API_ListStreams.md "../../../kinesis/latest/APIReference/API_ListStreams.md") action in Kinesis to list the streams in the caller's account.
- Expose an HTTP POST method on the API's `/streams/{stream-name}`
  resource and integrate the method with the [CreateStream](../../../kinesis/latest/APIReference/API_CreateStream.md "../../../kinesis/latest/APIReference/API_CreateStream.md") action in Kinesis to create a named stream in the caller's
  account.
- Expose an HTTP GET method on the API's `/streams/{stream-name}`
  resource and integrate the method with the [DescribeStream](../../../kinesis/latest/APIReference/API_DescribeStream.md "../../../kinesis/latest/APIReference/API_DescribeStream.md") action in Kinesis to describe a named stream in the
  caller's account.
- Expose an HTTP DELETE method on the API's `/streams/{stream-name}`
  resource and integrate the method with the [DeleteStream](../../../kinesis/latest/APIReference/API_DeleteStream.md "../../../kinesis/latest/APIReference/API_DeleteStream.md") action in Kinesis to delete a stream in the caller's account.
- Expose an HTTP PUT method on the API's `/streams/{stream-name}/record`
  resource and integrate the method with the [PutRecord](../../../kinesis/latest/APIReference/API_PutRecord.md "../../../kinesis/latest/APIReference/API_PutRecord.md") action in Kinesis. This enables the client to add a single data
...
- Expose an HTTP PUT method on the API's
  ...
  with the [PutRecords](../../../kinesis/latest/APIReference/API_PutRecords.md "../../../kinesis/latest/APIReference/API_PutRecords.md") action in Kinesis. This enables the client to add a list of
  data records to the named stream.
- Expose an HTTP GET method on the API's
  `/streams/{stream-name}/records` resource and integrate the method
  with the [GetRecords](../../../kinesis/latest/APIReference/API_GetRecords.md "../../../kinesis/latest/APIReference/API_GetRecords.md") action in Kinesis. This enables the client to list data records
...
- Expose an HTTP GET method on the API's
  ...
  method with the [GetShardIterator](../../../kinesis/latest/APIReference/API_GetShardIterator.md "../../../kinesis/latest/APIReference/API_GetShardIterator.md") action in Kinesis. This helper method must be supplied to
  ...
  Reference](../../../kinesis/latest/APIReference/Welcome.md "../../../kinesis/latest/APIReference/Welcome.md").
...
API**. Under **REST API**, choose **Build**. 3. Choose **New API**. 4. In **API name**, enter `KinesisProxy`.
Keep the default values for all other fields. 5. (Optional) For **Description**, enter a description. 6. For **IP address type**, select **IPv4**. 7. Choose **Create API**.
...
5. Choose the `/streams` resource, and then choose **Create method**, and then do the following:
   ...
   here, because listing streams is intuitively a READ operation. 2. For **Integration type**, select **AWS service**. 3. For **AWS Region**, select the AWS Region where you created your Kinesis stream. 4. For **AWS service**, select **Kinesis**. 5. Keep **AWS subdomain** blank. 6. For **HTTP method**, choose
   **POST**.

   ###### Note

   We chose `POST` here because Kinesis requires that the
   `ListStreams` action be invoked with it. 7. For **Action type**, choose **Use action
   name**. 8. For **Action name**, enter
   `ListStreams`. 9. For **Execution role**, enter the ARN for your
   execution role. 10. Keep the default of **Passthrough** for
   **Content Handling**. 11. Choose **Create method**.
...
8. Choose **URL request headers parameters**, and then do the following:
   ...
   3. For **Mapped from**, enter
      `'application/x-amz-json-1.1'`. We use a request parameter mapping to set the `Content-Type`
...
9. Choose **Mapping templates**, and then choose **Add mapping template**, and do the following:
   ...
   3. Choose **Save**. The [ListStreams](../../../kinesis/latest/APIReference/API_ListStreams.md#API_ListStreams_RequestSyntax "../../../kinesis/latest/APIReference/API_ListStreams.md#API_ListStreams_RequestSyntax") request takes a payload of the following JSON format:
...
an empty JSON payload here. 10. Test the GET method on the **/streams** resource to invoke the
...
for the `GET /streams` method. 14. Add the following body mapping template to map data from the `GET
...
`stream-name` path parameter value. 15. To test the `GET /stream/{stream-name}` method to invoke the
`DescribeStream` action in Kinesis, choose the **Test** tab. 16. For **Path**, under **stream-name**, enter the name of an existing Kinesis stream. 17. Choose **Test**. If the test is successful, a 200 OK response is
...
The task follows the same procedure to set up the request parameter mapping
for the `GET /streams` method. 14. Add the following body mapping template to map data from the `POST
 /streams/{stream-name}` method request to the `POST
...
value of 5 if the client does not specify a value in the method request payload. 15. To test the `POST /stream/{stream-name}` method to invoke the
`CreateStream` action in Kinesis, choose the **Test** tab. 16. For **Path**, under **stream-name**, enter the name of a new Kinesis stream. 17. Choose **Test**. If the test is successful, a 200 OK response is
...
The task follows the same procedure to set up the request parameter mapping
for the `GET /streams` method. 14. Add the following body mapping template to map data from the `DELETE
...
of `stream-name`. 15. To test the `DELETE /stream/{stream-name}` method to invoke the
`DeleteStream` action in Kinesis, choose the **Test** tab. 16. For **Path**, under **stream-name**, enter the name of an existing Kinesis stream. 17. Choose
...
Reading data in Kinesis amounts to calling the [GetRecords](../../../kinesis/latest/APIReference/API_GetRecords.md#API_GetRecords_Examples "../../../kinesis/latest/APIReference/API_GetRecords.md#API_GetRecords_Examples") action:
...
![Create Records:GET|PUT|PUT|GET method for the API.](images/api-gateway-kinesis-proxy-setup-streams-stream-record-method-new-console.png)
...
The task follows the same procedure to set up the request parameter mapping
for the `GET /streams` method. 14. Add the following body mapping template to map data from the `PUT
...
without using any model. 15. To test the `PUT /streams/{stream-name}/record` method, set the
...
The task follows the same procedure to set up the request parameter mapping
for the `GET /streams` method. 14. Add the following mapping template to map data from the `PUT
...
procedure). In production environment, you should keep both formats consistent. 15. To test the `PUT /streams/{stream-name}/records` method, set the
...
following step. 13. Choose **Add query string**. 14. For **Name**, enter `shard-id`. 15. Keep **Required** and **Caching** turned off. 16. Choose **Create method**. 17. In the **Integration request** section, add the following mapping template to generate the required input
...
header parameter to the method request. 13. Choose **Add header**. 14. For **Name**, enter `Shard-Iterator`. 15. Keep **Required** and **Caching** turned off. 16. Choose **Create method**. 17. In the **Integration request** section, add the following body mapping template to map the `Shard-Iterator`
