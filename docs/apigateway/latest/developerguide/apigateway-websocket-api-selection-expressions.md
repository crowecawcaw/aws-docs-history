...
+ In some cases, a pattern format is required. In this case, the expression should be wrapped with forward slashes (`"/"`), e.g. `"/2\d\d/"` to match `2{{XX}}` status codes.
...
| Route.ModelSelectionExpression | Key for Route.RequestModels | Optional.<br />If provided for non-proxy integration, model validation occurs.<br />`$default` is supported as a catch-all. | Perform request validation dynamically within the same route. | 
| Integration.TemplateSelectionExpression | Key for Integration.RequestTemplates | Optional.<br />May be provided for non-proxy integration to manipulate incoming payloads.<br />`${request.body.jsonPath}` and static values are supported.<br />`$default` is supported as a catch-all. | Manipulate the caller's request based on dynamic properties of the request. | 
| Integration.IntegrationResponseSelectionExpression | IntegrationResponse.IntegrationResponseKey | Optional. May be provided for non-proxy integration.<br />Acts as a pattern match for error messages (from Lambda) or status codes (from HTTP integrations).<br />`$default` is required for non-proxy integrations to act as the catch-all for successful responses. | Manipulate the response from the backend.<br />Choose the action to occur based on the dynamic response of the backend (e.g., handling certain errors distinctly). | 
