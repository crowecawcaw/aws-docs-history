...
4. To specify request validation, select a value from the **Request Validator**
   ...
   about each option, see [Request validation for REST APIs in API Gateway](api-gateway-method-request-validation.md "api-gateway-method-request-validation.md").
5. Select **API key required** to require an API key. When enabled, API keys are used in [usage plans](api-gateway-api-usage-plans.md "api-gateway-api-usage-plans.md") to throttle
   client traffic.
6. (Optional) To assign an operation name in a Java SDK of this API, generated by API Gateway, for **Operation name**,
...
9. To declare the payload format of a method request with the
   ...
   3. For **Model**, select a model from the dropdown menu. The currently available
      ...
      any models you have created and added to the [Models](../api/API_Model.md "../api/API_Model.md") collection of the API. For more information about creating a model, see [Data models for REST APIs](models-mappings-models.md "models-mappings-models.md").
...
error response. 4. Choose **Add header**. 5. For **Header name**, enter a name.

To return a header from the backend to the client, add the header in the method response. 6. Choose **Add model** to define a format of the method response body.

Enter the media type of the response payload for **Content
type** and choose a model from the
**Models** dropdown menu. 7. Choose **Save**.
