...
more information, see [Set up API keys for REST APIs in API Gateway](api-gateway-setup-api-keys.md "api-gateway-setup-api-keys.md").
...
you must use the [`UsagePlan`](../api/API_UsagePlan.md "../api/API_UsagePlan.md")
`apiStages` property to associate specified API stage values
(`apiId` and `stage`) with included API keys (via [`UsagePlanKey`](../api/API_UsagePlanKey.md "../api/API_UsagePlanKey.md")), instead of using the [ApiKey](../api/API_ApiKey.md "../api/API_ApiKey.md")
`stageKeys` property.

To check whether you've already migrated to default usage plans, use the [`get-account`](../../../cli/latest/reference/apigateway/get-account.md "../../../cli/latest/reference/apigateway/get-account.md") CLI command. In the command output, the
...
1. Call this CLI command: [`update-account`](../../../cli/latest/reference/apigateway/update-account.md "../../../cli/latest/reference/apigateway/update-account.md").
...
AWS Management Console###### To create a usage plan
...
![API usage plan entities](images/api-gateway-new-console-usage-plan-keys-create-setup.png) 3. For **Name**, enter a name. 4. (Optional) For **Description**, enter a description. 5. By default, usage plans enable throttling. Enter a **Rate** and a
**Burst** for your usage plan. Choose **Throttling** to turn off throttling. 6. By default, usage plans enable a quota for a time period. For **Requests**, enter the total number of
requests that a user can make in the time period of your usage plan. Choose **Quota** to turn off the quota. 7. Choose **Create usage plan**.

AWS CLIThe following
[create-usage-plan](../../../cli/latest/reference/apigateway/create-usage-plan.md "../../../cli/latest/reference/apigateway/create-usage-plan.md") command creates a usage plan that resets at the beginning of the month:
...
REST APICall [`usageplan:create`](../api/API_CreateUsagePlan.md "../api/API_CreateUsagePlan.md") to create a usage plan.
...
![Add an API stage to a usage plan.](images/api-gateway-new-console-usage-plan-keys-create-add-stage.png) 3. For **API**, select an API. 4. For **Stage**, select a stage. 5. (Optional) To turn on method-level throttling, do the following:
...
AWS CLI
The following [update-usage-plan](../../../cli/latest/reference/apigateway/update-usage-plan.md "../../../cli/latest/reference/apigateway/update-usage-plan.md")
...
REST APICall [`usageplan:update`](../api/API_UpdateUsagePlan.md "../api/API_UpdateUsagePlan.md") to update a usage plan.
...
![API usage plan entities](images/api-gateway-new-console-usage-plan-keys-create-add-key.png) 2. 1. To associate an existing key to your usage plan, select **Add existing key**, and then select your existing key from the dropdown menu. 2. To create a new API key, select **Create and add new key**, and then create a new key. For more information on how to create a new key, see [Create an API key](api-gateway-setup-api-keys.md#api-gateway-usage-plan-create-apikey "api-gateway-setup-api-keys.md#api-gateway-usage-plan-create-apikey"). 3. Choose **Add API key**.

AWS CLI
The following [create-usage-plan-key](../../../cli/latest/reference/apigateway/create-usage-plan-key.md "../../../cli/latest/reference/apigateway/create-usage-plan-key.md")
...
REST API
Call [`usageplankey:create`](../api/API_CreateUsagePlanKey.md "../api/API_CreateUsagePlanKey.md") to associate an existing API key with a usage plan.

You can also directly associate your APIs keys with a usage plan when you import them.
Call [`apikey:import`](../api/API_ImportApiKeys.md "../api/API_ImportApiKeys.md") to add one or more API
