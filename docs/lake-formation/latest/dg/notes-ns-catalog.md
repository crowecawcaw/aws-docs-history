...
+ The read\_committed isolation level on RMS tables through Spark will be supported. 
+ Redshift database names are treated as case-insensitive in the AWS Glue Data Catalog, restricted to 128 characters, and can be alphanumeric with dashes (-) and underscores (\_). 
+ The catalog names are case-insensitive, restricted to 50 characters, and can be alphanumeric with dashes (-) and underscores (\_). 
