...
1. Next, create a new table within `tag_database`.
   ...
   1. For Include path, enter the path to `tag_database` created by the CloudFormation template `(s3://lf-tagbased-demo{{Account-ID}}/tag_database/)`.
...
1. Next, attach LF-Tags at the database level.
   ...
   This completes the LF-Tag assignment to the tag\_database database.
...
1. Follow these steps to grant permissions to the `lf-data-analyst` user on the objects associated with the LF-Tag `Confidential=True` (Database:tag\_database) to have `Describe` the database and `Select` permission on tables.
