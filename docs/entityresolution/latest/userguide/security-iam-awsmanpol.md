...
perform entity resolution actions. Additionally, this policy grants access to Connect Customer Customer Profiles APIs to enable integration for automated match result processing. Some resources are narrowed down to contain the
...
- `CustomerProfilesIntegrationAccess` – Grants access to Amazon Connect Customer
  and Connect Customer Customer Profiles APIs to enable integration between AWS Entity Resolution
  and Connect Customer Customer Profiles for automated match result processing.
