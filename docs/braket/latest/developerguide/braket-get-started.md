# Getting started with Amazon Braket

###### Tip

**Learn the foundations of quantum computing with AWS!**
Enroll in the [Amazon Braket Digital Learning Plan](https://skillbuilder.aws/learning-plan/EH35DWGU3R/amazon-braket--knowledge-badge-readiness-path-includes-labs "https://skillbuilder.aws/learning-plan/EH35DWGU3R/amazon-braket--knowledge-badge-readiness-path-includes-labs")
and earn your own Digital badge after completing a series of learning courses and a digital assessment.

After you have followed the instructions in [Enable Amazon Braket](braket-enable-overview.md "braket-enable-overview.md") , you can get started with Amazon Braket.

###### The steps to get started include:

- [Enable Amazon Braket](braket-enable-overview.md "braket-enable-overview.md")
- [Create an Amazon Braket notebook
  instance](braket-get-started-create-notebook.md "braket-get-started-create-notebook.md")
- [Create a Braket notebook instance using AWS CloudFormation](braket-cloudformation.md "braket-cloudformation.md")
