...
+ Hdrive,k​(t)=( 1/2 ​Ω(t)eiϕ(t)S−,k​ \+ 1/2 ​Ω(t)e−iϕ(t) S\+,k​) \+ (−Δglobal​(t)nk​),
  ...
  + nk ​is the projection operator on the Rydberg state of atom k (that is, n=\|r⟩⟨r\|)
...
**Note**  
The user cannot control which levels are involved (that is, S−​,S\+​, n operators are fixed) nor the strength of the Rydberg-Rydberg interaction coefficient (C6​).
...
| setup.ahs\_register.filling | List[int] | Marks atoms that occupy the trap sites with 1, and empty sites with 0 | 
...
| hamiltonian.drivingFields[].detuning.pattern | str | spatial pattern of driving detuning, Delta\_global(t); must be 'uniform' | 
 ...
 **braket.tasks.analog\_hamiltonian\_simulation\_quantum\_task\_result.AnalogHamiltonianSimulationQuantumTaskResult** (example)
...
| Task result field | type | description | 
| --- | --- | --- | 
| braketSchemaHeader.name | str | name of the schema; must be 'braket.task\_result.analog\_hamiltonian\_simulation\_task\_result' | 
| braketSchemaHeader.version | str | version of the schema | 
...
| additionalMetadata.action | braket.ir.ahs.program\_v1.Program | (See the [Braket AHS program schema](#braket-quera-ahs-program-schema) section) | 
 ...
 **braket.device\_schema.quera.quera\_device\_capabilities\_v1.QueraDeviceCapabilities** (example)
...
| service.qpu\_capabilities.service.deviceCost.unit | str | unit for charging the price, e.g: 'minute', 'hour', 'shot', 'task' | 
...
| service.braketSchemaHeader.name | str | name of the schema; must be 'braket.device\_schema.device\_service\_properties' | 
