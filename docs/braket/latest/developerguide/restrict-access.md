...
To adapt this code, substitute the Amazon Resource Number (ARN) of the restricted device for the string shown in the previous example. This string provides the **Resource** value. In Braket, a device represents a QPU or simulator that you can call to run quantum tasks. The devices available are listed on the [Devices page](https://docs.aws.amazon.com/braket/latest/developerguide/braket-devices.html). There are two schemas used to specify access to these devices:
