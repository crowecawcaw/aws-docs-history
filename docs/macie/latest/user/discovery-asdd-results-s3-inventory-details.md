...
[Sensitivity](#discovery-asdd-results-s3-inventory-sensitivity-details) \| [Bucket details](#discovery-asdd-results-s3-inventory-bucket-details) \| [Object samples](#discovery-asdd-results-s3-inventory-sample-details) \| [Sensitive data discovery](#discovery-asdd-results-s3-inventory-sdd-details)
...
+ **Sensitive data type** – The unique identifier (ID) for the managed data identifier that detected the data, or the name of the custom data identifier that detected the data.
  ...
  A managed data identifier's ID describes the type of sensitive data that it's designed to detect—for example, **USA\_PASSPORT\_NUMBER** for US passport numbers. For details about each managed data identifier, see [Using managed data identifiers](managed-data-identifiers.md).
