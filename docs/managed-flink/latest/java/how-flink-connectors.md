...
**Note**  
Starting with version 4.0.0-2.0, the JDBC connector is modularized. The monolithic `flink-connector-jdbc` artifact is replaced by a core artifact (`flink-connector-jdbc-core`) and database-specific artifacts such as `flink-connector-jdbc-postgres` and `flink-connector-jdbc-mysql`. Update your Maven or Gradle dependencies accordingly when upgrading to Flink 2.x.
