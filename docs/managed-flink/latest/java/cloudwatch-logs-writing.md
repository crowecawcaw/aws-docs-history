...
using the Apache [`log4j`](https://logging.apache.org/log4j/ "https://logging.apache.org/log4j/") library or the [`Simple Logging Facade for Java (SLF4J)`](https://www.slf4j.org/ "https://www.slf4j.org/") library.

###### Topics

- [Write to CloudWatch logs using Log4J](#cloudwatch-logs-writing-log4j "#cloudwatch-logs-writing-log4j")
- [Write to CloudWatch logs using SLF4J](#cloudwatch-logs-writing-slf4j "#cloudwatch-logs-writing-slf4j")

## Write to CloudWatch logs using Log4J

1. Add the following dependencies to your application's
   `pom.xml` file:
...
3. Instantiate the `Logger` object, passing in your application
   class:
...
4. Write to the log using `log.info`. A large number of messages are
   written to the application log. To make your custom messages easier to filter,
   use the `INFO` application log level.
...
1. Add the following dependency to your application's
   `pom.xml` file:
