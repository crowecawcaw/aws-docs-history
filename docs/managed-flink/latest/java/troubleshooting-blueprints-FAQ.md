...
- **original\_data**: The data that was used to
...
- **embedded\_data**: A vector float array of
...
2. When `source.msk.data.type` is `JSON`
   ...
   - **embedded\_data**: Embedding vector
     ...
     applied) or created from `original_data` if no chunking was
     applied.
   - **chunk\_key**: Only present when the
