...
#### [ CLI ]
   ...
   Run the following AWS CLI command to start people pathing in a video.
   ...
   + Replace the value of `profile-name` in the line that creates the Rekognition session with the name of your developer profile.
   ...
   After running the proceeding code example, copy down the returned `jobID` and provide it to the following `GetPersonTracking` command below to get your results, replacing `job-id-number` with the `jobID` you previously received: 
