...
The following example shows how you can use `DetectCustomLabels` with
frames extracted from a video. The code has been tested with video files in
_mov_ and _mp4_ format.
...
2. Make sure you have `rekognition:DetectCustomLabels` and
   `AmazonS3ReadOnlyAccess` permissions. For more information, see
   [Step 4: Set up the AWS CLI and AWS SDKs](su-awscli-sdk.md "su-awscli-sdk.md").
3. Use the following example code. Change the value of `videoFile` to
   the name of a video file. Change the value of `projectVersionArn` to
   the Amazon Resource Name (ARN) of your Amazon Rekognition Custom Labels model.
