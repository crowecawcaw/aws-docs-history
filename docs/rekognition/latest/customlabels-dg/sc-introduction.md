...
The identity used to detect custom labels might be different from the identity that manages
Amazon Rekognition Custom Labels models.

You can secure access an identity’s access to `DetectCustomLabels` by applying a
policy to the identity. The following example restricts access to `DetectCustomLabels` only and to
...
We provide the `AmazonRekognitionCustomLabelsFullAccess` AWS managed policy that you can use to control access
to Amazon Rekognition Custom Labels. For more information, see
[AWS managed policy: AmazonRekognitionCustomLabelsFullAccess](../dg/security-iam-awsmanpol.md#security-iam-awsmanpol-custom-labels-full-access "../dg/security-iam-awsmanpol.md#security-iam-awsmanpol-custom-labels-full-access").
