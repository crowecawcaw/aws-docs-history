...
[Regions](../../../whitepapers/latest/aws-fault-isolation-boundaries/regions.md "../../../whitepapers/latest/aws-fault-isolation-boundaries/regions.md").
Each Region is physically and logically independent and consists of
three or more
[Availability
Zones (AZs)](../../../whitepapers/latest/aws-fault-isolation-boundaries/availability-zones.md "../../../whitepapers/latest/aws-fault-isolation-boundaries/availability-zones.md"). Availability Zones are geographically close to
...
instances, [AWS Fargate](https://aws.amazon.com/fargate/ "https://aws.amazon.com/fargate/") tasks, and VPC-attached
[AWS Lambda](https://aws.amazon.com/lambda/ "https://aws.amazon.com/lambda/") functions. AWS compute services, including
[EC2
Auto Scaling](https://aws.amazon.com/ec2/autoscaling/ "https://aws.amazon.com/ec2/autoscaling/"),
[Amazon Elastic Container Service (ECS)](https://aws.amazon.com/ecs/ "https://aws.amazon.com/ecs/"), and
[Amazon Elastic Kubernetes Service (EKS)](https://aws.amazon.com/eks/ "https://aws.amazon.com/eks/"), provide ways for you to launch
...
[Amazon Elastic File
Service (EFS)](https://aws.amazon.com/efs/ "https://aws.amazon.com/efs/"),
[Amazon Aurora](https://aws.amazon.com/aurora/ "https://aws.amazon.com/aurora/"),
[Amazon DynamoDB](https://aws.amazon.com/dynamodb/ "https://aws.amazon.com/dynamodb/"),
[Amazon Simple Queue Service (SQS)](https://aws.amazon.com/sqs/ "https://aws.amazon.com/sqs/"), and
[Amazon Kinesis Data Streams](https://aws.amazon.com/kinesis/data-streams/ "https://aws.amazon.com/kinesis/data-streams/") replicate data in multiple
...
[Amazon Relational Database Service (RDS)](https://aws.amazon.com/rds/ "https://aws.amazon.com/rds/"),
[Amazon Redshift](https://aws.amazon.com/redshift/ "https://aws.amazon.com/redshift/"), and
[Amazon ElastiCache](https://aws.amazon.com/elasticache/ "https://aws.amazon.com/elasticache/"), you must enable multi-AZ replication. Once
...
[Amazon Elastic Block Store (EBS)](https://aws.amazon.com/ebs/ "https://aws.amazon.com/ebs/") volumes or Amazon EC2 instance storage,
you must manage multi-AZ replication yourself.

![Diagram showing multi-tier architecture deployed across three Availability Zones. Note that Amazon S3 and Amazon DynamoDB are always Multi-AZ automatically. The ELB also is deployed to all three zones.](images/multi-tier-architecture.png)
_Figure 9: Multi-tier architecture deployed across three
...
Regions (also known as _Regional services_). A
...
[AWS CloudFormation StackSets](../../../AWSCloudFormation/latest/UserGuide/what-is-cfnstacksets.md "../../../AWSCloudFormation/latest/UserGuide/what-is-cfnstacksets.md") can replicate your entire AWS
infrastructure across multiple Regions.
[AWS CloudFormation](https://aws.amazon.com/cloudformation/ "https://aws.amazon.com/cloudformation/") can also detect configuration drift and
...
[EC2 Image Builder](https://aws.amazon.com/image-builder/ "https://aws.amazon.com/image-builder/") can publish your EC2 machine images (AMIs) after
every build to each Region you use.
[Amazon Elastic Container Registry (ECR)](https://aws.amazon.com/ecr/ "https://aws.amazon.com/ecr/") can replicate your container
...
and Amazon EFS.
[Amazon DynamoDB global tables](https://aws.amazon.com/dynamodb/global-tables/ "https://aws.amazon.com/dynamodb/global-tables/") accept writes in any supported
...
[Amazon Route 53](https://aws.amazon.com/route53/ "https://aws.amazon.com/route53/") to direct traffic to the closest available Region to the
...
[Route 53 health checks](../../../Route%C2%A053/latest/DeveloperGuide/dns-failover.md "../../../Route%C2%A053/latest/DeveloperGuide/dns-failover.md") to detect unhealthy endpoints and perform
automatic failover and additionally use
[Amazon Application Recovery Controller (ARC)](https://aws.amazon.com/application-recovery-controller/ "https://aws.amazon.com/application-recovery-controller/") to provide a
...
infrastructure components and data in a _warm
standby_ or _pilot light_
...
1. Work with business stakeholders and data residency experts
   to determine which AWS Regions can be used to host your
   resources and data.
2. Work with business and technical stakeholders to evaluate
...
3. Deploy your compute resources into multiple Availability
   ...
   1. In your VPC, create multiple subnets in different
      ...
      availability](rel_planning_network_topology_ip_subnet_allocation.md "rel_planning_network_topology_ip_subnet_allocation.md").
   2. If you are using Amazon EC2 instances, use
      [EC2
      Auto Scaling](https://aws.amazon.com/ec2/autoscaling/ "https://aws.amazon.com/ec2/autoscaling/") to manage your instances. Specify
      the subnets you chose in the previous step when you
      create your Auto Scaling groups.
   3. If you are using AWS Fargate compute for
      [Amazon ECS](../../../AmazonECS/latest/developerguide/AWS_Fargate.md "../../../AmazonECS/latest/developerguide/AWS_Fargate.md") or
      [Amazon EKS](../../../eks/latest/userguide/fargate.md "../../../eks/latest/userguide/fargate.md"), select the subnets you chose in the first
      step when you create an ECS Service, launch an ECS task,
      or create a
      [Fargate
      profile](../../../eks/latest/userguide/fargate-profile.md "../../../eks/latest/userguide/fargate-profile.md") for EKS.
   4. If you are using AWS Lambda functions that need to run
   ...
   5. Place traffic directors such as load balancers in front
      of your compute resources. If cross-zone load balancing
      is enabled,
      [AWS Application Load Balancers](../../../elasticloadbalancing/latest/application/introduction.md "../../../elasticloadbalancing/latest/application/introduction.md") and
      [Network
      Load Balancers](../../../elasticloadbalancing/latest/network/introduction.md "../../../elasticloadbalancing/latest/network/introduction.md") detect when targets such as EC2
...
4. Replicate your workload's data across multiple Availability
   ...
   1. If you use an AWS-managed data service such as Amazon RDS, Amazon ElastiCache, or Amazon FSx, study its user
      ...
      if necessary.
   2. If you use AWS-managed storage services such as Amazon S3, Amazon EFS, and Amazon FSx, avoid using single-AZ or
   ...
   3. If you run a self-managed database, queue, or other
...
5. Configure your DNS service to detect AZ impairment and
...
2. Deploy your compute resources and configuration metadata
...
3. Replicate your data from your primary Region into your
   ...
   1. Amazon DynamoDB global tables provide global replicas of
      ...
      replication.
   2. If you are running a self-managed database, arrange for
   ...
   3. If your workload uses AWS EventBridge, you may need to
...
4. Consider whether and to what extent you want to use
   ...
   [AWS Key Management Service (KMS)](https://aws.amazon.com/kms/ "https://aws.amazon.com/kms/") supports
   [multi-region
   keys](../../../kms/latest/developerguide/multi-region-keys-overview.md "../../../kms/latest/developerguide/multi-region-keys-overview.md") to securely distribute and protect keys shared
   across Regions.
5. Consider AWS Global Accelerator to improve the availability
...
- [REL13-BP01
  Define recovery objectives for downtime and data loss](rel_planning_for_recovery_objective_defined_recovery.md "rel_planning_for_recovery_objective_defined_recovery.md")
...
- [White
  paper: AWS Fault Isolation Boundaries](../../../whitepapers/latest/aws-fault-isolation-boundaries/availability-zones.md "../../../whitepapers/latest/aws-fault-isolation-boundaries/availability-zones.md")
- [Resilience
  in Amazon EC2 Auto Scaling](../../../autoscaling/ec2/userguide/disaster-recovery-resiliency.md "../../../autoscaling/ec2/userguide/disaster-recovery-resiliency.md")
- [Amazon EC2 Auto Scaling: Example: Distribute instances across
  Availability Zones](../../../autoscaling/ec2/userguide/auto-scaling-benefits.md#arch-AutoScalingMultiAZ "../../../autoscaling/ec2/userguide/auto-scaling-benefits.md#arch-AutoScalingMultiAZ")
- [How
  EC2 Image Builder works](../../../imagebuilder/latest/userguide/how-image-builder-works.md#image-builder-distribution "../../../imagebuilder/latest/userguide/how-image-builder-works.md#image-builder-distribution")
- [How
  Amazon ECS places tasks on container instances (includes
  Fargate)](../../../AmazonECS/latest/developerguide/task-placement.md "../../../AmazonECS/latest/developerguide/task-placement.md")
- [Resilience
  in AWS Lambda](../../../lambda/latest/dg/security-resilience.md "../../../lambda/latest/dg/security-resilience.md")
- [Amazon S3: Replicating objects overview](../../../AmazonS3/latest/userguide/replication.md "../../../AmazonS3/latest/userguide/replication.md")
- [Private
  image replication in Amazon ECR](../../../AmazonECR/latest/userguide/replication.md "../../../AmazonECR/latest/userguide/replication.md")
- [Global
  Tables: Multi-Region Replication with DynamoDB](../../../amazondynamodb/latest/developerguide/GlobalTables.md "../../../amazondynamodb/latest/developerguide/GlobalTables.md")
- [Amazon
  Elasticache for Redis OSS: Replication across AWS Regions
  using global datastores](../../../AmazonElastiCache/latest/red-ug/Redis-Global-Datastore.md "../../../AmazonElastiCache/latest/red-ug/Redis-Global-Datastore.md")
- [Resilience
  in Amazon RDS](../../../AmazonRDS/latest/UserGuide/disaster-recovery-resiliency.md "../../../AmazonRDS/latest/UserGuide/disaster-recovery-resiliency.md")
- [Using
  Amazon Aurora global databases](../../../AmazonRDS/latest/AuroraUserGuide/aurora-global-database.md "../../../AmazonRDS/latest/AuroraUserGuide/aurora-global-database.md")
- [AWS Global Accelerator Developer Guide](../../../global-accelerator/latest/dg/what-is-global-accelerator.md "../../../global-accelerator/latest/dg/what-is-global-accelerator.md")
- [Multi-Region
  keys in AWS KMS](../../../kms/latest/developerguide/multi-region-keys-overview.md "../../../kms/latest/developerguide/multi-region-keys-overview.md")
- [Amazon Route 53: Configuring DNS failover](../../../Route%C2%A053/latest/DeveloperGuide/dns-failover-configuring.md "../../../Route%C2%A053/latest/DeveloperGuide/dns-failover-configuring.md")
- [Amazon Application Recovery Controller (ARC) Developer
  Guide](../../../r53recovery/latest/dg/what-is-route53-recovery.md "../../../r53recovery/latest/dg/what-is-route53-recovery.md")
- [Sending
  and receiving Amazon EventBridge events between AWS Regions](../../../eventbridge/latest/userguide/eb-cross-region.md "../../../eventbridge/latest/userguide/eb-cross-region.md")
- [Creating
  a Multi-Region Application with AWS Services blog
  series](https://aws.amazon.com/blogs/architecture/tag/creating-a-multi-region-application-with-aws-services-series/ "https://aws.amazon.com/blogs/architecture/tag/creating-a-multi-region-application-with-aws-services-series/")
- [Disaster
...
- [Disaster
  ...
  Warm Standby](https://aws.amazon.com/blogs/architecture/disaster-recovery-dr-architecture-on-aws-part-iii-pilot-light-and-warm-standby/ "https://aws.amazon.com/blogs/architecture/disaster-recovery-dr-architecture-on-aws-part-iii-pilot-light-and-warm-standby/")

**Related videos:**

- [AWS re:Invent
  2018: Architecture Patterns for Multi-Region Active-Active
  Applications](https://youtu.be/2e29I3dA8o4 "https://youtu.be/2e29I3dA8o4")
- [AWS re:Invent
  2019: Innovation and operation of the AWS global network
  infrastructure](https://youtu.be/UObQZ3R9_4c "https://youtu.be/UObQZ3R9_4c")
