...
<a name="scenario-ad-supported-content-monetization"></a>
...
1.  The AIS consolidates requests for the manifest from all players and forwards only one request to the origin. 

1.  The origin responds back with the content manifest, optionally including ad markers. 

1.  If the manifest has ad markers, the ad insertion service makes a call to the ads decision server (ADS) to get a list of personalized ads to insert for the playback session. 

1.  The ADS returns a Video Ad Serving Template (VAST) response with a list of personalized ads. 

1.  The AIS inserts the ads and returns a personalized manifest back to the CDN. 
...
+  Use a CDN to cache content and ad segments. However, personalized manifest responses must *not* be cached or shared between viewers. For more information, refer to [CDN Best Practices](https://docs.aws.amazon.com/mediatailor/latest/ug/cdn-bp.html). 
