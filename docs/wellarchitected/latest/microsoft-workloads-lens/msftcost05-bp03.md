...
1.  Set up Storage Virtual Machines (SVMs) and configure storage volumes with proper protocols (SMB/iSCSI) based on your Microsoft application needs 

1.  Connect Windows Server instances to FSx storage using native tools (File Explorer for SMB, iSCSI Initiator for block storage) 

1.  Configure Windows Server Failover Clustering if high availability is required, using FSx for ONTAP as the shared storage 

1.  Migrate existing data to FSx storage and implement backup/recovery procedures using ONTAP's snapshot and replication capabilities 
...
+  [Provisioning iSCSI for Windows](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/mount-iscsi-windows.html) 