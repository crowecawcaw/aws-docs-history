...
+  [Experiment more often](https://docs.aws.amazon.com/wellarchitected/latest/framework/rel-dp.html) - Identify the best model for your use case, developing a mechanism to update the appropriate model quickly. 
+  [Consider mechanical sympathy](https://docs.aws.amazon.com/wellarchitected/latest/framework/perf-dp.html) - Not all foundation models are created equal, and some have significant advantages over others. Select the appropriate model for your use case by understanding how models perform on different tasks. 
...
1.  Implement tests to facilitate rapid testing for each of the models. 

1.  Test each model against the ground truth data set, and identify which models surpass the minimum performance and response quality thresholds. 

1.  Select the model which performs best on average for the given use case. 

1.  Consider elevating model performance situationally, and use techniques like prompt routing or customization where appropriate. 

1.  Document results in an AI usage document to track model usage and encourage data-driven decision-making within the organization. 
