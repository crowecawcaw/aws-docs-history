...
<a name="implementation-guidance-32"></a>
 ...
 To benefit from the reliability provided by Availability Zones, you can deploy applications on multiple Outposts, each attached to a different Availability Zone. By doing so, you can build additional application resilience and avoid a dependence on a single Availability Zone. For more information about Regions and Availability Zones, see [AWS Global Infrastructure](https://aws.amazon.com/about-aws/global-infrastructure/). 

 This makes this connectivity to the AWS Region from AWS Outposts important. Where the data does need to flow to the AWS Region and has high availability requirements, we recommend using redundant connectivity back to the AWS Region. [AWS Direct Connect](https://docs.aws.amazon.com/directconnect/latest/UserGuide/Welcome.html) can be set up with redundant tunnels and connections to AWS Region. 
