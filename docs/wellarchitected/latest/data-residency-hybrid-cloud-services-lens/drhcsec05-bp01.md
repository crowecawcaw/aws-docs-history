...
<a name="drhcsec05-bp01"></a>

 Automate the detection and notification to security operations teams if data is stored is locations out of compliance with your data residency compliance requirements. 
...
+  Create detective controls, such as [AWS Config Rules](https://docs.aws.amazon.com/config/latest/developerguide/evaluate-config.html) or open source solutions like [Cloud Custodian](https://cloudcustodian.io/) rules, and configure them to detect and notify when resources are found in unapproved locations. 
  ...
  1.  Implement rules for EC2 instances, Auto Scaling groups (ASGs), NetworkInterfaces, RDS instances, and Application Load Balancers (ALB) to detect when the value of the SubnetId attribute value is not present in an allowlist.  
+  Automate remediation of findings in scenarios where precise rules without exceptions can be enforced. One example would be to automatically end an instance launched in an in-Region subnet when data residency requirements prohibit data storage in that Region. Another example would be to turn on default encryption for S3 buckets and EBS volumes. 
  ...
  1.  Automated remediation is an example of a control. Automated remediation can be implement using [AWS Security Hub CSPM custom actions](https://docs.aws.amazon.com/securityhub/latest/userguide/securityhub-cloudwatch-events.html), AWS Config integration with [Systems Manager Automation](https://docs.aws.amazon.com/systems-manager/latest/userguide/systems-manager-automation.html) documents, and [Cloud Custodian](https://cloudcustodian.io/docs/actions.html) actions. 
...
+  [AWS Security Hub CSPM custom actions](https://docs.aws.amazon.com/securityhub/latest/userguide/securityhub-cloudwatch-events.html) 

 **Related partner solutions:** 
+  [Cloud Custodian](https://cloudcustodian.io/docs/actions.html) actions 