...
1. **Determine a DR strategy that will
...
![Graph showing choosing a DR strategy based on RTO and cost](images/choosing-a-dr-strategy.png)

_Figure 18: Choosing a DR strategy based on RTO and
cost_

To learn more, see [Business Continuity Plan (BCP)](../../../whitepapers/latest/disaster-recovery-workloads-on-aws/business-continuity-plan-bcp.md "../../../whitepapers/latest/disaster-recovery-workloads-on-aws/business-continuity-plan-bcp.md"). 2. **Review the patterns for how the selected DR strategy can be
implemented.**
...
_Backup and restore_ is the least complex strategy to implement, but
...
![Diagram showing a backup and restore architecture](images/backup-restore-architecture.png)
...
Recovery](https://aws.amazon.com/blogs/architecture/disaster-recovery-dr-architecture-on-aws-part-ii-backup-and-restore-with-rapid-recovery/ "https://aws.amazon.com/blogs/architecture/disaster-recovery-dr-architecture-on-aws-part-ii-backup-and-restore-with-rapid-recovery/").

**Pilot light**

With the _pilot light_ approach, you replicate your data from your
...
![Diagram showing a ilot light architecture](images/pilot-light-architecture.png)
...
capacity, then this is known as _hot standby_.

![Diagram showing a Figure 21: Warm standby architecture](images/warm-standby-architecture.png)

_Figure 21: Warm standby architecture_

Using warm standby or pilot light requires scaling up resources in the recovery
Region. To verify capacity is available when needed, consider the use for [capacity reservations](../../../AWSEC2/latest/UserGuide/ec2-capacity-reservations.md "../../../AWSEC2/latest/UserGuide/ec2-capacity-reservations.md") for EC2 instances. If using AWS Lambda, then [provisioned
concurrency](../../../lambda/latest/dg/provisioned-concurrency.md "../../../lambda/latest/dg/provisioned-concurrency.md") can provide runtime environments so that they are prepared to
...
![Diagram showing a multi-site active/active architecture](images/multi-site-active-active-architecture.png)
...
Active/Active](https://aws.amazon.com/blogs/architecture/disaster-recovery-dr-architecture-on-aws-part-iv-multi-site-active-active/ "https://aws.amazon.com/blogs/architecture/disaster-recovery-dr-architecture-on-aws-part-iv-multi-site-active-active/").
...
![Architecture diagram describing how AWS Elastic Disaster Recovery operates.](images/drs-architecture.png)
...
zone such as [Amazon EBS volumes](../../../AWSEC2/latest/UserGuide/ebs-volumes.md "../../../AWSEC2/latest/UserGuide/ebs-volumes.md") or [Amazon Redshift clusters](../../../redshift/latest/mgmt/working-with-clusters.md "../../../redshift/latest/mgmt/working-with-clusters.md"). If an
...
Part 1: Single-Region stack](https://aws.amazon.com/blogs/networking-and-content-delivery/building-highly-resilient-applications-using-amazon-route-53-application-recovery-controller-part-1-single-region-stack/ "https://aws.amazon.com/blogs/networking-and-content-delivery/building-highly-resilient-applications-using-amazon-route-53-application-recovery-controller-part-1-single-region-stack/"). Here, the strategy is to maintain as much isolation
...
disasters. 3. **Assess the resources of your workload, and what their configuration
will be in the recovery Region prior to failover (during normal operation).**

For infrastructure and AWS resources use infrastructure as code such as [AWS CloudFormation](https://aws.amazon.com/cloudformation "https://aws.amazon.com/cloudformation") or third-party tools like
Hashicorp Terraform. To deploy across multiple accounts and Regions with a single
operation you can use [AWS CloudFormation
StackSets](../../../AWSCloudFormation/latest/UserGuide/what-is-cfnstacksets.md "../../../AWSCloudFormation/latest/UserGuide/what-is-cfnstacksets.md"). For Multi-site active/active and Hot Standby strategies, the deployed
...
additional actions to become production ready. Using CloudFormation [parameters](../../../AWSCloudFormation/latest/UserGuide/parameters-section-structure.md "../../../AWSCloudFormation/latest/UserGuide/parameters-section-structure.md") and [conditional logic](../../../AWSCloudFormation/latest/UserGuide/intrinsic-function-reference-conditions.md "../../../AWSCloudFormation/latest/UserGuide/intrinsic-function-reference-conditions.md"), you can control whether a deployed stack is active or
standby with [a single template](https://aws.amazon.com/blogs/architecture/disaster-recovery-dr-architecture-on-aws-part-iii-pilot-light-and-warm-standby/ "https://aws.amazon.com/blogs/architecture/disaster-recovery-dr-architecture-on-aws-part-iii-pilot-light-and-warm-standby/"). When using Elastic Disaster Recovery, the service will replicate and orchestrate
...
and then those backups are copied to the recovery Region. [AWS Backup](https://aws.amazon.com/backup/ "https://aws.amazon.com/backup/") provides a centralized view where you can configure,
...
(Amazon RDS)](https://aws.amazon.com/rds "https://aws.amazon.com/rds") DB instances or [Amazon DynamoDB](https://aws.amazon.com/dynamodb "https://aws.amazon.com/dynamodb") tables. These data resources are therefore live and ready to serve
requests in the recovery Region.

To learn more about how AWS services operate across Regions, see this blog series
on [Creating a Multi-Region Application with AWS Services](https://aws.amazon.com/blogs/architecture/tag/creating-a-multi-region-application-with-aws-services-series/ "https://aws.amazon.com/blogs/architecture/tag/creating-a-multi-region-application-with-aws-services-series/"). 4. **Determine and implement how you will make your recovery Region ready
...
One option is to use [Amazon Route 53](https://aws.amazon.com/route53 "https://aws.amazon.com/route53"). Using
...
Some AWS services will do this automatically. If using [Amazon DynamoDB global tables](https://aws.amazon.com/dynamodb/global-tables/ "https://aws.amazon.com/dynamodb/global-tables/"), even if the table in the
primary Region had become not available, when it comes back online, DynamoDB resumes
propagating any pending writes. If using [Amazon Aurora Global Database](https://aws.amazon.com/rds/aurora/global-database/ "https://aws.amazon.com/rds/aurora/global-database/") and using [managed planned failover](../../../AmazonRDS/latest/AuroraUserGuide/aurora-global-database-disaster-recovery.md#aurora-global-database-disaster-recovery.managed-failover "../../../AmazonRDS/latest/AuroraUserGuide/aurora-global-database-disaster-recovery.md#aurora-global-database-disaster-recovery.managed-failover"), then Aurora global database's existing replication
...
failback process. For more details, see [Performing a failback](../../../drs/latest/userguide/failback-performing-main.md "../../../drs/latest/userguide/failback-performing-main.md").
...
- [Build
  ...
  an hour](https://read.acloud.guru/building-a-serverless-multi-region-active-active-backend-36f28bed4ecf "https://read.acloud.guru/building-a-serverless-multi-region-active-active-backend-36f28bed4ecf")
- [Multi-region
  serverless backend — reloaded](https://medium.com/@adhorn/multi-region-serverless-backend-reloaded-1b887bc615c0 "https://medium.com/@adhorn/multi-region-serverless-backend-reloaded-1b887bc615c0")
- [RDS:
  Replicating a Read Replica Across Regions](../../../AmazonRDS/latest/UserGuide/USER_ReadRepl.md#USER_ReadRepl.XRgn "../../../AmazonRDS/latest/UserGuide/USER_ReadRepl.md#USER_ReadRepl.XRgn")
- [Route 53: Configuring DNS Failover](../../../Route53/latest/DeveloperGuide/dns-failover-configuring.md "../../../Route53/latest/DeveloperGuide/dns-failover-configuring.md")
- [S3:
  Cross-Region Replication](../../../AmazonS3/latest/dev/crr.md "../../../AmazonS3/latest/dev/crr.md")
- [What
...
- [HashiCorp
  Terraform: Get Started - AWS](https://learn.hashicorp.com/collections/terraform/aws-get-started "https://learn.hashicorp.com/collections/terraform/aws-get-started")
- [APN
...
- [Get
  Started with AWS Elastic Disaster Recovery | Amazon Web Services](https://www.youtube.com/watch?v=GAMUCIJR5as "https://www.youtube.com/watch?v=GAMUCIJR5as")
