...
1.  Fleet Provisioning service creates new certificate and private key assigned with AWS CA. 

1.  Device writes the unique private key and certificate to secure storage. 

1.  With the parameters published from the device, Fleet Provisioning service triggers Pre-Provisioning lambda function. 

1.  Lambda function performs additional verification logic such as checking the hardware secret against a DynamoDB table with verified devices. 

1.  Fleet provisioning service create IoT Thing, Policy, and activates certificate based on Provisioning template and publishes this to the device. 

1.  Device applies the new configuration and connects with the unique private key, certificates and configuration. 