...
<a name="iotsec09-bp01"></a>

 Having an automated, monitored, and audited mechanism for deploying, managing, and maintaining device software in IoT devices allows for making changes to this device software over time. The device software installed in each device should be maintained using a software bill of materials (SBOM). New features and fixes or updates can be applied to the device which extend its useful lifetime, address security vulnerabilities, or enable the device to perform more actions. Use [AWS IoT Device Management Software Package Catalog (SPC)](https://docs.aws.amazon.com/iot/latest/developerguide/software-package-catalog.html) to aid in maintaining device software inventories. 
