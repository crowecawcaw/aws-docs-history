...
information, and auto-record-to-S3) and assign that policy to users. You can either
assign the permissions when creating a [new
user](#iam-permissions-new-user "#iam-permissions-new-user") or add permissions to an [existing user](#iam-permissions-existing-user "#iam-permissions-existing-user"). Both procedures are given below.
...
- [Creating an IAM User](../../../IAM/latest/UserGuide/id_users_create.md#Using_CreateUser_console "../../../IAM/latest/UserGuide/id_users_create.md#Using_CreateUser_console") in the _IAM User
  Guide_
- The information in [Amazon IVS Security](security.md "security.md") on IAM
  and "Managed Policies for IVS."
- For record-to-S3 functionality: [Using Service-Linked Roles](security-service-linked-roles.md "security-service-linked-roles.md") and [Auto-Record to Amazon S3](record-to-s3.md "record-to-s3.md") in the _Amazon
  IVS User Guide_
...
described fully in the [Managed Policies for
IVS](security-iam-awsmanpol.md "security-iam-awsmanpol.md") section of _IVS Security_.
...
3. In the **Specify permissions** window, choose
   ...
   add/delete (Allow/Deny) operation access permissions as needed. See [IVS Low-Latency
   Streaming API Reference](../LowLatencyAPIReference/Welcome.md "../LowLatencyAPIReference/Welcome.md") for details on IVS operations.)
...
2. In the **Specify user details**
   ...
   2. Check **Provide user access to the AWS
      Management Console**.
   3. When prompted, select **I want to create
      an IAM user**.
   4. Under **Console password**,
      select **Autogenerated
      password**.
   5. Check **Users must create a new password
      at next sign-in**.
   6. Choose **Next**. A **Set permissions** window opens.
