...
- `ADVANCED` channels have two, selectable transcode presets:
  ...
  - _Higher bandwidth delivery_ uses a higher
    ...
    and/or complex video content (e.g., flashes and quick scene changes). This
    is the default.
...
- **Transcode presets**: There is one, default
  transcode-preset ladder.
- **Audio**: For renditions 360p and below, audio
  is transcoded. For other renditions: original audio is passed through.
...
`STANDARD` channels are transmuxed when the input is multitrack video. The highest video resolution produced is limited by the `multitrackInputConfiguration.maximumResolution` property. The specific renditions are dynamic depending on the [broadcaster’s system and environmental requirements](multitrack-video-setup.md#multitrack-video-setup-broadcaster-system "multitrack-video-setup.md#multitrack-video-setup-broadcaster-system").
