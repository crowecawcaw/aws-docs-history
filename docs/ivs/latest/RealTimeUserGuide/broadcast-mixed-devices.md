...
![IVS broadcasting mixed devices terminology.](https://docs.aws.amazon.com/ivs/latest/RealTimeUserGuide/images/Broadcast_SDK_Mixer_Glossary.png)
...
| Mixed Device | A `Device` that can be attached to a `BroadcastSession` like any other `Device`, but with additional APIs that allow `Source` objects to be added. Mixed devices have internal mixers that composite audio or images, producing a single output audio and image stream.<br />Mixed devices come in either audio or image flavors. | 
...
| Transition | To move a slot to a new position or change some of its properties, use `MixedDevice.transitionToConfiguration()`. This method takes:+  A new source configuration that represents the next state for the source. <br />+  A duration that specifies how long the animation should take, relative to the timeline of the video. If the duration is set to 0, the transition happens on the next frame that is mixed. <br />+  An optional callback that informs you when the animation is complete. The callback may be useful for chaining animations.  | 
...
![Configuring a broadcast session for mixing.](https://docs.aws.amazon.com/ivs/latest/RealTimeUserGuide/images/Broadcast_SDK_Mixer_Configuring.png)
