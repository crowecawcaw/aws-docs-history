...
1. From the AWS services list, search for and select **Amazon Interactive Video Service Chat**.
...
Alternately, you can get to these metrics through the CloudWatch console. Under **AWS Namespaces**, choose **Usage**. Then, from the **Service** list, choose **IVS Chat**. (See [Monitoring Amazon IVS Chat](chat-health.md).)
...
| Resource | The name of the AWS resource. Valid value: `ConcurrentChatConnections`.<br />The ConcurrentChatConnections usage metric is a copy of the one in the **AWS/IVSChat** namespace (with the None dimension), as described in [Monitoring Amazon IVS Chat](chat-health.md). | 
