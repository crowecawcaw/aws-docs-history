# Pricing transition

The following table shows how pricing components map from AWS Resilience Hub v1 to
the next generation of Resilience Hub.

| Component            | AWS Resilience Hub v1 | Next generation Resilience Hub          |
| -------------------- | --------------------- | --------------------------------------- |
| Base fee             | $15/application/month | $15/service/month                       |
| Assessments included | Unlimited             | 2 per month                             |
| Resource overage     | None                  | $0.10/resource above 150 per assessment |
| Dependency discovery | Not available         | $10/service/month (optional)            |

The $15 entry price is preserved. Most customers (services with 150 or fewer resources
using 2 or fewer assessments per month) pay the same amount.
