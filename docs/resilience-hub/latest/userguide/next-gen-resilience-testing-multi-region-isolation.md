...
The **Multi-Region: isolation** test blocks connectivity between
two Regions, including network traffic, cross-Region data replication, and selected dependencies.
It can help you validate that your service operates independently when resources in another Region
are not accessible, and surface cross-Region dependencies that could impact availability.

###### What makes this test unique

- Focuses on Region isolation (loss of cross-Region connectivity) rather than recovery to
  another Region.
- Blocks network traffic, pauses cross-Region replication (Amazon S3, DynamoDB, MemoryDB), and
  optionally blocks dependencies between the two Regions.
...
- This is a sustained test. The test passes if all success alarms remain in
  `OK` state until the test actions end.

###### Things to think about

- Choose success alarms that measure the isolated Region's health – validate it
  continues serving traffic independently.
- This test applies to both active/active and active/passive architectures.
- A key use case: validate your recovery Region can operate standalone. For example, if
  your primary is `us-east-1` and secondary is `us-west-2`,
  use this to validate `us-west-2` operates independently when isolated from
  `us-east-1`.
- This test is also a good precursor to **Multi-Region: recovery**
  – validate independence before testing full failover. The test can help discover
  cross-Region dependencies that are critical.
...
- **Isolated Region** – The Region you are testing for
  independent operation.
- **Destination Region** – The Region being blocked
  (connectivity cut to this Region).
- **Duration** – The length of time the test actions
  ...
  ends. Default is 3 hours when you first create the test.
- **Dependencies to block** (optional) – If dependency
  discovery is enabled, the list shows dependencies from the isolated Region to the destination
  Region. You can select from this list or add manually by DNS domain name. No dependencies are
  selected by default. Other actions in this test (network connectivity, replication pausing)
  run regardless. Additional dependencies added here are only used for this test and will not be
  saved to the service's dependency discovery. These defaults apply in the console; when using
  the API, you provide the dependencies explicitly.
...
| Action                                                          | Description                                                                           |
| --------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `aws:network:transit-gateway-disrupt-cross-region-connectivity` | Blocks cross-Region traffic via Transit Gateway peering to the destination<br>Region. |
