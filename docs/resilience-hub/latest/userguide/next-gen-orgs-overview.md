...
Next generation Resilience Hub enables centralized resilience governance across your AWS organization.
With Organizations integration, you can:
...
| Org-level policies      | Resilience policies created by the DA, visible and assignable across all member<br>accounts         |
...
- Creates and publishes organization-wide resilience policies by associating them with user
  journeys on shared systems.
- Views aggregated resilience posture dashboards.
  Service-Linked Roles (SLRs) are automatically created in all member accounts when trusted
  access is enabled, providing the DA with read-only cross-account visibility without manual
  IAM setup.
