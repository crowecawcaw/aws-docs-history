...
The service event log provides a chronological history of all events that have occurred for
a service. You can use the event log to track changes, troubleshoot issues, and audit activity.
...
3. Select **View event log**.
   You can filter events by **Event type** and
   **Time range**. The following event types are recorded:
...
- **Workflow updated** – A workflow configuration was updated.
  Each event entry shows the timestamp, event type, and a summary of what changed. Choose
  **Show details** to view additional information about a specific
  event.
