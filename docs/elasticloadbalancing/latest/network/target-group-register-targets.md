...
group. If you detach the load balancer from the Auto Scaling group, the instances are
...
- QUIC server IDs are always 8 bytes. When registering the target, the server ID must be in the form `0x` followed by 16 hexadecimal
  characters.
- Connection IDs generated by your targets must be exactly 17 bytes.
  Connection IDs of any other length are dropped. This requirement does
  not apply to QUIC Initial packets.
- Once a target is registered with a server ID, the ID is immutable. To change a target server ID, it must be deregistered first
