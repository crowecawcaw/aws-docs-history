...
Indicates whether the Gateway Load Balancer sends a TCP reset (RST) to the sender of
traffic. If enabled, the Gateway Load Balancer sends the RST when a TCP flow's idle timeout
expires, or when it receives a non-SYN TCP packet for a flow that is not in
the flow table. The possible values are `true` and
`false`. The default is `false`. After sending the RST,
the Gateway Load Balancer removes the flow entry from its flow table.
