...
Some AWS services allow you to pass an existing role to that service, instead of
creating a new service role or service-linked role. To do this, you must have
...
a policy to the entity that grants them the correct permissions in CodePipeline. After the permissions are granted, provide the credentials to the user
