...
+ Configure and attach an IAM role ({{CrossAccount\_Role}}) with a trust relationship policy that allows the CodePipeline service role in {{AccountA}} to assume the role.
...
1. In **Define Key Administrative Permissions**, choose the role or roles you want to act as administrators for this key, and then choose **Next**.
...
1. In the **Bucket Policy Editor** window, create a policy that will allow {{AccountB}} access to the pipeline artifacts, and will give {{AccountB}} the ability to add output artifacts if an action, such as a custom source or build action, creates them. 
