...
changes when you make a change to the source files in your source repository. The pipeline
then uses Amazon S3 to deploy the files to your bucket. Each time you modify or add your website
files in your source location, the deployment creates the website with your latest files.
...
Even if you delete files from the source repository, the S3 deploy action does not
delete S3 objects corresponding to deleted files.
...
Next, you use the AWS CodePipeline console to create your pipeline and specify an Amazon S3
...
- An S3 bucket configured for website hosting. See [Hosting a static website on Amazon
  S3](../../../AmazonS3/latest/userguide/WebsiteHosting.md "../../../AmazonS3/latest/userguide/WebsiteHosting.md"). Make sure you create your bucket in the same Region as the
  pipeline.

###### Note

To host a website, your bucket must have public read access, which
gives everyone read access. With the exception of website hosting, you
...
1. Extract the downloaded sample files. Do not upload the ZIP file to your
   repository.
2. Push or upload the files to your CodeCommit repository. These files are the
   source artifact created by the **Create Pipeline** wizard
   for your deployment action in CodePipeline. Your files should look like this in
...
3. You can use Git or the CodeCommit console to upload your files:
   ...
   2. To use the CodeCommit console to upload your files:
      ...
      2. Choose **Add file**, and then choose
         **Upload file**.
      3. Select **Choose file**, and then browse
         for your file. Commit the change by entering your user name
         and email address. Choose **Commit
         changes**.
...
- A source stage with a CodeCommit action where the source artifacts are the
...
2. On the **Welcome** page, **Getting
   started** page, or **Pipelines** page, choose
   **Create pipeline**.
...
4. In **Step 2: Choose pipeline settings**, in
   **Pipeline name**, enter
   `MyS3DeployPipeline`.
5. In **Pipeline type**, choose **V2**. For
   more information, see [Pipeline types](pipeline-types.md "pipeline-types.md"). Choose
   **Next**.
...
8. In **Step 3: Add source stage**, in **Source
   ...
   **Repository name**, choose the name of the CodeCommit
   repository you created in [Step 1: Create a CodeCommit repository](tutorials-simple-codecommit.md#codecommit-create-repository "tutorials-simple-codecommit.md#codecommit-create-repository"). In **Branch name**, choose the name of the branch that contains
   your latest code update. Unless you created a different branch on your own,
   only `main` is available.

After you select the repository name and branch, the Amazon CloudWatch Events rule to be
created for this pipeline is displayed.

Choose **Next**. 9. In **Step 4: Add build stage**, choose **Skip
build stage**, and then accept the warning message by choosing
**Skip** again.
...
Choose **Next**. 11. In **Step 6: Add deploy stage**:
    ...
    1. In **Deploy provider**, choose **Amazon
     S3**.
    2. In **Bucket**, enter the name of your public
     bucket.
    ...
    The deployment fails if you do not select **Extract file before deploy**. This is
     because the AWS CodeCommit action in your pipeline zips source
     artifacts and your file is a ZIP file.


    When **Extract file before deploy** is selected,
     **Deployment path** is displayed. Enter the
     name of the path you want to use. This creates a folder structure in
     Amazon S3 to which the files are extracted. For this tutorial, leave this
     field blank.



    ![The Step 6: Add deploy stage page for an S3 deploy action with an AWS CodeCommit source](images/tutorial-s3deploy-stage-codecommit.png)
...
14. Access your endpoint to test the website. Your endpoint follows this
    format:
...
timestamped folder. First, you create TypeScript code and a buildspec.yml file. After
you combine the source files in a ZIP file, you upload the source ZIP file to your S3
source bucket, and use a CodeBuild stage to deploy a built application ZIP file to your S3
target bucket. The compiled code is retained as an archive in your target bucket.

### Prerequisites

You must already have the following:

- An S3 source bucket. You can use the bucket you created in [Tutorial: Create a simple pipeline (S3 bucket)](tutorials-simple-s3.md "tutorials-simple-s3.md").
- An S3 target bucket. See [Hosting a static website on Amazon S3](../../../AmazonS3/latest/userguide/WebsiteHosting.md "../../../AmazonS3/latest/userguide/WebsiteHosting.md"). Make sure you create
  your bucket in the same AWS Region as the pipeline you want to
  create.

###### Note

This example demonstrates deploying files to a private bucket. Do not
...
- A `buildspec.yml` file, which is used for CodeBuild build
  projects.
- An `index.ts` file.
...
- Create a file named `buildspec.yml` with the following
  contents. These build commands install TypeScript and use the TypeScript
  compiler to rewrite the code in `index.ts` to JavaScript
  code.
...
- Create a file named `index.ts` with the following
  contents.
...
Zip the files and name the file `source.zip`. 2. In the Amazon S3 console, for your source bucket, choose
**Upload**. Choose **Add files**, and
then browse for the ZIP file you created. 3. Choose **Upload**. These files are the source artifact
created by the **Create Pipeline** wizard for your
deployment action in CodePipeline. Your file should look like this in your
bucket:
...
- A source stage with an Amazon S3 action where the source artifacts are the
  files for your downloadable application.
- A deployment stage with an Amazon S3 deployment action.
...
4. In **Step 2: Choose pipeline settings**, in
   **Pipeline name**, enter
   `MyS3DeployPipeline`.
5. In **Service role**, choose **New service
   role** to allow CodePipeline to create a service role in IAM.
6. Leave the settings under **Advanced settings** at their
   defaults, and then choose **Next**.
7. In **Step 3: Add source stage**, in **Source
   provider**, choose **Amazon S3**. In
   **Bucket**, choose the name of your source bucket. In
   **S3 object key**, enter the name of your
   source ZIP file. Make sure you include the .zip file extension.

Choose **Next**. 8. In **Step 4: Add build stage**:

    1. In **Build provider**, choose
     **CodeBuild**.
    2. Choose **Create build project**. On the
     **Create project** page:
    3. In **Project name**, enter a name for this build
     project.
    4. In **Environment**, choose **Managed
     image**. For **Operating system**,
     choose **Ubuntu**.
    5. For **Runtime**, choose
     **Standard**. For **Runtime
     version**, choose
     **aws/codebuild/standard:1.0**.
    6. In **Image version**, choose **Always use
     the latest image for this runtime version**.
    7. For **Service role**, choose your CodeBuild service
     role, or create one.
    8. For **Build specifications**, choose
     **Use a buildspec file**.
    9. Choose **Continue to CodePipeline**. A message is
     displayed if the project was created successfully.
    10. Choose **Next**.
...
deployed output files are available in the `js-application` folder as
shown:

![Sample ZIP download](images/tutorial-s3deploy-pipeline-appzip.png)
