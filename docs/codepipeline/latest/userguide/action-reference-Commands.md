...
Some compute types are not compatible with certain environment types. For example, WINDOWS\_SERVER\_2022\_CONTAINER is not compatible with BUILD\_GENERAL1\_SMALL. Using incompatible combinations causes the action to fail and generates a runtime error.
...
The following is a JSON example of the action with configuration fields shown for environment and compute type, along with an example environment variable.
