...
When requests are throttled in ElastiCache Serverless, you should see an increase in the `ThrottledCmds` [ElastiCache Serverless](serverless-metrics-events-redis.md#serverless-metrics) metric. If you are noticing a high number of throttled requests, consider the following:
