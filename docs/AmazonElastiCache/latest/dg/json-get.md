...
- INDENT/NEWLINE/SPACE (optional) – Controls the format of the returned JSON string, that
  is, "pretty print". The default value of each one is an empty string.
  They can be overridden in any combination. They can be specified in any
  order.
- NOESCAPE - Optional, allowed to be present for legacy compatibility and has no other
  effect.
- path (optional) – Zero or more JSON paths, defaults to the root if none is given. The
  path arguments must be placed at the end.
  **Return**
