...
| CPUCreditBalance | The number of earned CPU credits that an instance has accrued since it was launched or started.<br />Credits are accrued in the credit balance after they are earned, and removed from the credit balance when they are spent. The credit balance has a maximum limit, determined by the instance size. After the limit is reached, any new credits that are earned are discarded.<br />The credits in the CPUCreditBalance are available for the instance to spend to burst beyond its baseline CPU utilization.<br />CPU credit metrics are available at a five-minute frequency only. | Credits (vCPU-minutes)  | 
| CPUCreditUsage | The number of CPU credits spent by the instance for CPU utilization. One CPU credit equals one vCPU running at 100% utilization for one minute or an equivalent combination of vCPUs, utilization, and time (for example, one vCPU running at 50% utilization for two minutes or two vCPUs running at 25% utilization for two minutes).<br />CPU credit metrics are available at a five-minute frequency only. If you specify a period greater than five minutes, use the Sum statistic instead of the Average statistic. | Credits (vCPU-minutes)  | 
...
| MajorPageFaults | The number of major page faults incurred by the engine process. High values indicate that the engine is actively swapping and experiencing memory pressure that can degrade performance. | Count | 
| NetworkBaselineUsageInPercentage | The average inbound network bandwidth utilization as a percentage of the baseline bandwidth for the instance type. On instance types that support network burst (typically 4xlarge and smaller), values above 100 percent indicate that the host is consuming burst credits, a leading indicator that a sustained workload will eventually lead to credit exhaustion and throttling. | Percent | 
| NetworkBaselineUsageOutPercentage | The average outbound network bandwidth utilization as a percentage of the baseline bandwidth for the instance type. On instance types that support network burst (typically 4xlarge and smaller), values above 100 percent indicate that the host is consuming burst credits, a leading indicator that a sustained workload will eventually lead to credit exhaustion and throttling. | Percent | 
| NetworkBaselineMaxUsageInPercentage | The maximum per-second inbound network bandwidth utilization as a percentage of the baseline bandwidth for the instance type. This metric captures microbursts that averaged metrics can hide. On instance types that support network burst (typically 4xlarge and smaller), values above 100 percent indicate that the host is consuming burst credits, a leading indicator that a sustained workload will eventually lead to credit exhaustion and throttling. | Percent | 
| NetworkBaselineMaxUsageOutPercentage | The maximum per-second outbound network bandwidth utilization as a percentage of the baseline bandwidth for the instance type. This metric captures microbursts that averaged metrics can hide. On instance types that support network burst (typically 4xlarge and smaller), values above 100 percent indicate that the host is consuming burst credits, a leading indicator that a sustained workload will eventually lead to credit exhaustion and throttling. | Percent | 