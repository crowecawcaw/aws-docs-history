...
2. In the navigation pane, choose the appropriate engine.
3. Choose the cluster that you want to add a policy to (choose the
   cluster name and not the button to its left).
4. Choose the **Auto Scaling policies** tab.
5. Under **Scaling policies**, choose the button to the
   left of the Auto Scaling policy you wish to change, and then choose
   **Modify**.
...
- When using the AWS CLI, specify the name of the policy you want to edit
  in the `--policy-name` parameter. Specify new values for the
