...
- When you delete an ElastiCache cluster, your manual snapshots are retained.
  You can also create a final snapshot before the cluster is deleted. Automatic cache snapshots are not retained.
  For more information, see [Snapshot and restore](backups.md "backups.md").
...
2. In the ElastiCache engine dashboard, choose Valkey or Redis OSS.

A list of all caches running on that engine appears. 3. To choose the cluster to delete, choose the cluster's name from the list of
clusters. In this case, the name of the cluster you created at [Step 2: Create a cluster](SubnetGroups.designing-cluster-pre.valkey.md#GettingStarted.CreateCluster.valkey "SubnetGroups.designing-cluster-pre.valkey.md#GettingStarted.CreateCluster.valkey").

###### Important

You can only delete one cluster at a time from the ElastiCache console. Choosing multiple clusters
disables the delete operation. 4. For **Actions**, choose **Delete**. 5. In the **Delete Cluster** confirmation screen, type the name of the cluster and choose
**Final Backup**. Then choose
...
The following code deletes the cluster `my-cluster`. In this case, substitute `my-cluster` with the name of the cluster you created at [Step 2: Create a cluster](SubnetGroups.designing-cluster-pre.valkey.md#GettingStarted.CreateCluster.valkey "SubnetGroups.designing-cluster-pre.valkey.md#GettingStarted.CreateCluster.valkey").
