...
###### To replace any node in a Valkey or Redis OSS (cluster mode enabled) cluster
    ...
    2. Scale in: Delete the shard with the node to be replaced. For more
     information, see [Removing shards with online resharding](scaling-redis-cluster-mode-enabled.md#redis-cluster-resharding-online-remove "scaling-redis-cluster-mode-enabled.md#redis-cluster-resharding-online-remove").
    3. Update the endpoints in your application.
