...
Valkey does not support the `no-password-required` authentication mode. When you use Valkey, create the initial user with a temporary password. Set the access string to `off` (disabled) instead of using the `--no-password` flag shown in Step 1 below.
