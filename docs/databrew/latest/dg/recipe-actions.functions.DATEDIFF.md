...
- `units` – A unit of measure for describe the difference
  between the dates. Valid values are `MONTHS`, `YEARS`,
  `MILLISECONDS`, `QUARTERS`, `HOURS`,
  `MICROSECONDS`, `WEEKS`, `SECONDS`,
  `DAYS`, and `MINUTES`.
...
- One of `sourceColumn1` or `sourceColumn2` and one of
  `value1` or `value2`.
