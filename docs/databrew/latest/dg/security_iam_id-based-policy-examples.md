...
AWS APIs. An administrator must create IAM policies that grant users and roles
...
DataBrew resources in your AWS account. If you create an identity-based policy
that is more restrictive than the minimum required permissions, the console doesn't
function as intended for users or roles with that policy.

To ensure that users and roles can use the DataBrew console, also attach
...
example shows a policy that denies the deletion of a project. However, deletion is denied only if the
project tag _Owner_ has the value of _admin_. This policy also
grants the permissions necessary to deny this action on the console.
...
_richard-roe_ attempts to delete a DataBrew project, the resource must not be
tagged _Owner=admin_ or _owner=admin_. Otherwise, the
user is denied permission to delete the project. The condition tag key _Owner_
matches both _Owner_ and _owner_ because condition key
names are not case-sensitive. For more information, see
...
ListDatasets, ListJobs, ListProjects, ListRecipes, ListRulesets, and ListSchedules
do not support tag-based access control.
