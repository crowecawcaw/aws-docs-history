...
represent the entire JSON string. The “unnestLevel” parameter specifies how many levels
of JSON objects to convert to structs.

###### Parameters

- `sourceColumns` – A list of source columns.
- `regexColumnSelector –` A regular expression to select the columns.
- `removeSourceColumn` – A Boolean value. If `true` then remove
  the source column; otherwise, keep it.
