...
In this scenario, there are four distinct values for `shirt_size`. Therefore, `ONE_HOT_ENCODING` generates four new columns. Each new column is named `shirt_size_{{x}}`, where `{{x}}` represents a distinct `shirt_size `value.
