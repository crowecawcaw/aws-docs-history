...
to have a mean (μ) of 0 and standard deviation (σ) of 1. We recommend this action for
handling outliers.
...
- `threshold` – The threshold value to use when detecting
  outliers. The `sourceColumn` value is identified as an outlier if the
  score that's calculated with the `outlierStrategy` exceeds this number.
  The default is 3.
...
member of an example `RecipeStep` for a DataBrew [Recipe](API_Recipe.md "API_Recipe.md")
operation, using JSON syntax. For syntax examples showing a list of recipe
...
The following shows an example `RecipeAction` to use as
member of an example `RecipeStep` for a DataBrew [Recipe](API_Recipe.md "API_Recipe.md")
operation, using YAML syntax. For syntax examples showing a list of recipe
actions, see [Defining a recipe structure](recipes.md#recipes.structure "recipes.md#recipes.structure").
