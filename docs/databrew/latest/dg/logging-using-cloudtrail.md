...
All DataBrew actions are logged by CloudTrail and are documented in the [API Reference](https://docs.aws.amazon.com/databrew/latest/APIReference/Welcome.html). For example, calls to the `CreateDataset`, `UpdateRecipe` and `StartJobRun` actions generate entries in the CloudTrail log files. 
