...
- `targetColumn` – The name of a new column to be
  created.
- `flagType` – A value that must be set to
  `Pattern`.
- `pattern` – A regular expression, indicating the pattern to
  be evaluated.
- `trueString` – A value for the new column, if a null value
  is found in the source. If no value is specified, the default is
  `True`.
- `falseString` – A value for the new column, if a non-null
  value is found in the source. If no value is specified, the default is
  `False`.
