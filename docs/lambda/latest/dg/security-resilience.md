...
- **Versioning** – You can use versioning in Lambda to save your function's
  ...
  and rolling deployments. For details, see [Manage Lambda function versions](configuration-versions.md "configuration-versions.md").
- **Scaling** – When your function receives a request while it's
  ...
  Lambda automatically scales to handle 1,000 concurrent executions per Region, a [quota](gettingstarted-limits.md "gettingstarted-limits.md") that can be increased if needed. For details, see
  [Understanding Lambda function scaling](lambda-concurrency.md "lambda-concurrency.md").
- **High availability** – Lambda runs your function in multiple
  ...
  subnets in multiple Availability Zones to ensure high availability. For details, see [Giving Lambda functions access to resources in an Amazon VPC](configuration-vpc.md "configuration-vpc.md").
- **Reserved concurrency** – To make sure that your function can always
  ...
  details, see [Configuring reserved concurrency for a function](configuration-concurrency.md "configuration-concurrency.md").
- **Retries** – For asynchronous invocations and a subset of invocations
  ...
  [Understanding retry behavior in Lambda](invocation-retries.md "invocation-retries.md").
- **Dead-letter queue** – For asynchronous invocations, you can configure
  ...
  Amazon SQS queue that receives events for troubleshooting or reprocessing. For details, see [Adding a dead-letter queue](invocation-async-retain-records.md#invocation-dlq "invocation-async-retain-records.md#invocation-dlq").
