...
[AWS CloudFormation Guard](../../../cfn-guard/latest/ug/what-is-guard.md "../../../cfn-guard/latest/ug/what-is-guard.md") is an open-source, general-purpose, policy-as-code evaluation tool.
...
rules](../../../cfn-guard/latest/ug/writing-rules.md "../../../cfn-guard/latest/ug/writing-rules.md") and implement your controls with a domain-specific language as follows.

![Guard rules include resource type, property name, operator, expression value, and optional comment](images/governance-cloudformation-guard.png)

For example, suppose you want to ensure that developers choose only the latest runtimes. You
could specify two different policies, one to identify [runtimes](lambda-runtimes.md "lambda-runtimes.md")
...
After [installing](../../../cfn-guard/latest/ug/setting-up.md "../../../cfn-guard/latest/ug/setting-up.md")
...
[Globals](../../../serverless-application-model/latest/developerguide/sam-specification-template-anatomy-globals.md "../../../serverless-application-model/latest/developerguide/sam-specification-template-anatomy-globals.md")
section. Properties that are defined in the Globals section are not validated with your
Guard rules.

As outlined in the Guard troubleshooting [documentation](../../../cfn-guard/latest/ug/troubleshooting.md "../../../cfn-guard/latest/ug/troubleshooting.md"),
be aware that Guard doesn't support short-form intrinsics like `!GetAtt` or `!Sub` and instead requires using the expanded
forms: `Fn::GetAtt` and `Fn::Sub`. (The [earlier example](#guard-iac-yaml "#guard-iac-yaml") doesn't evaluate the Role property, so the short-form intrinsic was used for simplicity.)
