...
- [Lambda execution environment lifecycle](#runtimes-lifecycle "#runtimes-lifecycle")
- [Cold starts and latency](#cold-start-latency "#cold-start-latency")
- [Reducing cold starts with Provisioned Concurrency](#cold-starts-pc "#cold-starts-pc")
- [Optimizing static initialization](#static-initialization "#static-initialization")
...
- [Failures during the Init phase](#runtimes-lifecycle-init-errors "#runtimes-lifecycle-init-errors")
- [Restore phase (Lambda SnapStart only)](#runtimes-lifecycle-restore "#runtimes-lifecycle-restore")
- [Invoke phase](#runtimes-lifecycle-invoke "#runtimes-lifecycle-invoke")
- [Failures during the invoke phase](#runtimes-lifecycle-invoke-with-errors "#runtimes-lifecycle-invoke-with-errors")
- [Shutdown phase](#runtimes-lifecycle-shutdown "#runtimes-lifecycle-shutdown")
...
- Run any before-checkpoint [runtime hooks](snapstart-runtime-hooks.md "snapstart-runtime-hooks.md") (Lambda SnapStart only)
...
If the `Init` phase is successful, Lambda doesn't emit the `INIT_REPORT` log unless [SnapStart](snapstart.md "snapstart.md") or
[provisioned concurrency](provisioned-concurrency.md "provisioned-concurrency.md") is enabled. SnapStart and provisioned concurrency functions always emit `INIT_REPORT`.
For more information, see [Monitoring for Lambda SnapStart](snapstart-monitoring.md "snapstart-monitoring.md").

### Restore phase (Lambda SnapStart only)

When you first invoke a [SnapStart](snapstart.md "snapstart.md") function and as the function scales up, Lambda resumes new execution environments from the persisted snapshot instead of initializing the function from scratch. If you have an after-restore [runtime hook](snapstart-runtime-hooks.md "snapstart-runtime-hooks.md"), the code runs at the end of the `Restore` phase. You are charged for the duration of after-restore runtime hooks. The runtime must load and after-restore runtime hooks must complete within the timeout limit (10 seconds). Otherwise, you'll get a SnapStartTimeoutException. When the `Restore` phase completes, Lambda invokes the function handler (the [Invoke phase](#runtimes-lifecycle-invoke "#runtimes-lifecycle-invoke")).
...
For more information about the `RESTORE_REPORT` log, see [Monitoring for Lambda SnapStart](snapstart-monitoring.md "snapstart-monitoring.md").
...
[Accessing real-time telemetry data for extensions using the Telemetry API](telemetry-api.md "telemetry-api.md"). The Telemetry API emits
...
[Implement statelessness in functions](concepts-application-design.md#statelessness-functions "concepts-application-design.md#statelessness-functions").
...
- Each execution environment provides between 512 MB and 10,240 MB, in 1-MB increments, of disk space in the `/tmp`
  ...
  that you stored. For more information on deployment size limits, see [Lambda quotas](gettingstarted-limits.md "gettingstarted-limits.md").
...
concurrency](provisioned-concurrency.md "provisioned-concurrency.md") is the recommended solution to ensure the lowest possible latency. This feature pre-initializes
