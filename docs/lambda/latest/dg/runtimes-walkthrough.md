...
`LAMBDA_TASK_ROOT` tells it where the package was extracted, and `_HANDLER` includes the
name of the script.
...
the handler. For details, see [Requirements](runtimes-custom.md#runtimes-custom-build "runtimes-custom.md#runtimes-custom-build"). 3. Create a script for the function. The following example script defines a handler function that takes event data, logs it to `stderr`, and returns it.
...
5. Create a function named `bash-runtime`. For `--role`, enter the ARN of your Lambda [execution role](lambda-intro-execution-role.md "lambda-intro-execution-role.md").
...
with multiple functions. For more information, see [Managing Lambda dependencies with layers](chapter-layers.md "chapter-layers.md").
...
2. Create a layer with the [publish-layer-version](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/lambda/publish-layer-version.html?highlight=nodejs16%20x "https://awscli.amazonaws.com/v2/documentation/api/latest/reference/lambda/publish-layer-version.html?highlight=nodejs16%20x") command.
...
This adds the runtime to the function in the `/opt` directory. To ensure that Lambda uses the runtime in the layer, you must remove the `boostrap` from the function's deployment package, as shown in the next two steps. 2. Create a .zip file that contains the function code.
...
Delete the function with the [delete-function](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/lambda/delete-function.html "https://awscli.amazonaws.com/v2/documentation/api/latest/reference/lambda/delete-function.html") command.
