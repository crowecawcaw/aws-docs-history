...
The following design principles can help you build workloads that meet these goals. Not every principle might apply to every architecture, but they should guide you in general architecture decisions.
...
Because the same event might be received more than once, functions should be designed to be [idempotent](https://en.wikipedia.org/wiki/Idempotence). This means that receiving the same event multiple times does not change the result beyond the first time the event was received.
