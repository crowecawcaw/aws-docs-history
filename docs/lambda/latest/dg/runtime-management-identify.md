...
The [runtime version number](runtimes-update.md "runtimes-update.md") and ARN are logged in the `INIT_START` log line,
which Lambda emits to CloudWatch Logs each time that it creates a new [execution environment](concepts-basics.md#gettingstarted-concepts-runtime "concepts-basics.md#gettingstarted-concepts-runtime"). Because the execution environment uses the same runtime
version for all function invocations, Lambda emits the `INIT_START` log line only
...
Insights](../../../AmazonCloudWatch/latest/monitoring/ContributorInsights-CreateRule.md "../../../AmazonCloudWatch/latest/monitoring/ContributorInsights-CreateRule.md") to identify transitions between runtime versions. The following rule counts
the distinct runtime versions from each `INIT_START` log line. To use the rule,
...
![Graph showing the transition from one runtime version to another.](images/runtime_version_graph.png)
