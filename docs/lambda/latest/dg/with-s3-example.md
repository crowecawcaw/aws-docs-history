...
![Data flow between an S3 bucket, a Lambda function, and CloudWatch Logs](images/services-s3-example/s3_tut_config.png)
...
![First step: Create the Amazon S3 bucket](images/services-s3-example/s3trigger_tut_steps1.png)
...
![Next step: Upload a test object](images/services-s3-example/s3trigger_tut_steps2.png)

###### To upload a test object

1. Open the [Buckets](https://console.aws.amazon.com/s3/buckets "https://console.aws.amazon.com/s3/buckets") page of the Amazon S3 console and choose the bucket you created during the
   previous step.
2. Choose **Upload**.
...
![Next step: Create the permissions policy for Lambda](images/services-s3-example/s3trigger_tut_steps3.png)
...
![Next step: Create the execution role](images/services-s3-example/s3trigger_tut_steps4.png)

An [execution role](lambda-intro-execution-role.md "lambda-intro-execution-role.md") is an AWS Identity and Access Management (IAM) role that grants a Lambda function permission to access AWS services and resources. In this step, create an execution role using the permissions policy that you created in the previous step.
...
![Next step: Create the S3 trigger](images/services-s3-example/s3trigger_tut_steps7.png)

###### To create the Amazon S3 trigger

1. In the **Function overview** pane, choose **Add trigger**.

![Lambda console Function overview section.](images/overview-trigger.png) 2. Select **S3**. 3. Under **Bucket**, select the bucket you created earlier in the tutorial. 4. Under **Event types**, be sure that **All object create events** is selected. 5. Under **Recursive invocation**, select the check box to acknowledge that using the same Amazon S3 bucket for input and
output is not recommended. 6. Choose **Add**.

###### Note

When you create an Amazon S3 trigger for a Lambda function using the Lambda console, Amazon S3 configures an [event notification](../../../AmazonS3/latest/userguide/EventNotifications.md "../../../AmazonS3/latest/userguide/EventNotifications.md")
...
![Tutorial workflow diagram showing you are in the testing step testing with a dummy event](images/services-s3-example/s3trigger_tut_steps8.png)
...
1. Open the [Buckets](https://console.aws.amazon.com/s3/buckets "https://console.aws.amazon.com/s3/buckets") page of the Amazon S3 console and choose the bucket that you created earlier.
2. Choose **Upload**.
...
1. Open the [CloudWatch](https://console.aws.amazon.com/cloudwatch/home "https://console.aws.amazon.com/cloudwatch/home") console.
...
In [Tutorial: Using an Amazon S3 trigger to create thumbnail images](with-s3-tutorial.md "with-s3-tutorial.md"), the Amazon S3 trigger invokes a function that creates a thumbnail image for each image file that is uploaded to a
