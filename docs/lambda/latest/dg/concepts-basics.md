...
Lambda functions are the basic building blocks you use to build Lambda applications. To write functions, it's essential to understand the core concepts and components that make up the Lambda programming model.
This section will guide you through the fundamental elements you need to know to start building serverless applications with Lambda.
...
- **[Lambda execution environment and runtimes](#gettingstarted-concepts-runtime "#gettingstarted-concepts-runtime")** - Lambda execution environments manage the resources required to run your function. Run times are the language-specific environments your functions run in.
- **[Events and triggers](#gettingstarted-concepts-event "#gettingstarted-concepts-event")** - how other AWS services invoke your functions in response to specific events.
- **[Lambda permissions and roles](#gettingstarted-concepts-permissions "#gettingstarted-concepts-permissions")** - how you control who can access your functions and what other AWS services your functions can interact with.

###### Tip

If you want to start by understanding serverless development more generally, see [Understanding the difference between traditional and serverless development](../../../serverless/latest/devguide/serverless-shift-mindset.md "../../../serverless/latest/devguide/serverless-shift-mindset.md")
in the _AWS Serverless Developer Guide_.
...
of code that runs in response to events, such as a user clicking a button on a website or a file being uploaded to an Amazon Simple Storage Service (Amazon S3) bucket.
You can think of a function as a kind of self-contained program with the following properties. A Lambda **function handler** is the method in your function code that processes events.
...
[.zip file archives](configuration-function-zip.md "configuration-function-zip.md") and [container images](images-create.md "images-create.md").
...
Lambda functions run inside a secure, isolated _[execution environment](lambda-runtime-environment.md "lambda-runtime-environment.md")_ which Lambda manages for you. This
execution environment manages the processes and resources that are needed to run your function. When a function is first invoked, Lambda
...
The Lambda execution environment also contains a _runtime_, a language-specific environment that relays event information and responses between Lambda and your function.
Lambda provides a number of [managed runtimes](lambda-runtimes.md#runtimes-supported "lambda-runtimes.md#runtimes-supported") for the most popular programming languages, or you can create your own.

For managed runtimes, Lambda automatically applies security updates and patches to functions using the runtime.

## Events and triggers

You can also invoke a Lambda function directly by using the Lambda console, [AWS CLI](https://aws.amazon.com/cli/ "https://aws.amazon.com/cli/"), or one of the [AWS Software Development Kits (SDKs)](https://aws.amazon.com/developer/tools/ "https://aws.amazon.com/developer/tools/").
...
or for a general overview of using triggers and instructions on creating a trigger using the Lambda console, see [Integrating other services](lambda-services.md "lambda-services.md").

## Lambda permissions and roles

For Lambda, there are two main types of [permissions](permissions-granting-access.md "permissions-granting-access.md") that you need to configure:
...
object in an S3 bucket, or write to an Amazon SQS queue. To give functions the permissions they need to perform these actions, you use an _[execution role](lambda-intro-execution-role.md "lambda-intro-execution-role.md")_.

A Lambda execution role is a special kind of AWS Identity and Access Management (IAM) [role](../../../IAM/latest/UserGuide/id_roles.md "../../../IAM/latest/UserGuide/id_roles.md"), an identity you create in your account
that has specific permissions associated with it defined in a _policy_.
...
To grant other AWS service permission to access your Lambda function, you use a _[resource-based policy](access-control-resource-based.md "access-control-resource-based.md")_.
...
You can also use an _[identity-based policy](access-control-identity-based.md "access-control-identity-based.md")_ that's associated with the user.

### Best practices for Lambda permissions

When you set permissions using IAM policies, [security best practice](../../../IAM/latest/UserGuide/best-practices.md "../../../IAM/latest/UserGuide/best-practices.md") is to grant only the permissions required to perform a task. This is known as the principle of _least privilege_.
