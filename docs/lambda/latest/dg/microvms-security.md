...
This section describes the IAM roles, authentication tokens, and access
controls for AWS Lambda MicroVMs.
...
This role is used at MicroVM runtime to provide access permissions to
write logs to CloudWatch and use other AWS services.
...
Tokens control inbound access to running MicroVMs. They are port-scoped;
each token specifies which ports it grants access to.
...
`create-microvm-shell-auth-token` fails with a
`ValidationException`:
...
For a full walkthrough of connecting to a MicroVM shell, see [Shell access](microvms-troubleshooting.md#microvms-troubleshooting-shell "microvms-troubleshooting.md#microvms-troubleshooting-shell").
...
Lambda MicroVMs defines the following IAM actions. Use these in IAM
policies to control access to MicroVM operations.
