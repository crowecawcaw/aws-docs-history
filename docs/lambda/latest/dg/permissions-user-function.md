...
Use [identity-based policies](access-control-identity-based.md "access-control-identity-based.md") to allow users, user groups, or roles to perform operations on Lambda functions.
...
For an example, see [Amazon ECR repository policies](images-create.md#configuration-images-permissions "images-create.md#configuration-images-permissions").
...
The permissions in the policy are organized into statements based on the [resources and conditions](lambda-api-permissions-ref.md "lambda-api-permissions-ref.md") that they support.
...
- `DevelopFunctions` – Use any Lambda action that operates on functions prefixed with
  ...
  [resource-based policy](access-control-resource-based.md "access-control-resource-based.md") on the function and can have
...
- `DevelopEventSourceMappings` – Manage event source mappings on functions that are
  ...
  by function with a _condition_.
