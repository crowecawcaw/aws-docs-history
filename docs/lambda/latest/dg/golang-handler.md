...
SDK calls themselves might require the context object as an input. For example, the `s3Client.PutObject` call accepts the context object as its first argument:
...
+ The handler can take between 0 and 2 arguments. If there are two arguments, the first argument must implement `context.Context`.
+ The handler can return between 0 and 2 arguments. If there is a single return value, it must implement `error`. If there are two return values, the second value must implement `error`.
...
After loading this configuration into the `cfg` variable, you can pass this variable into client instantiations. The example code creates an Amazon S3 client as follows:
...
To avoid creating new resources every time you invoke your function, you can declare and modify global variables outside of your Lambda function's handler code. You define these global variables in a `var` block or statement. In addition, your handler can declare an `init()` function that is executed during the [initialization phase](lambda-runtime-environment.md#runtimes-lifecycle-ib). The `init` method behaves the same in AWS Lambda as it does in standard Go programs.
