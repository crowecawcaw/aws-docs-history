...
[Deploy Go Lambda functions with .zip file archives](golang-package.md "golang-package.md") or [Deploy Go Lambda functions with container images](go-image.md "go-image.md").

###### Topics

- [Setting up your Go handler project](#golang-handler-setup "#golang-handler-setup")
- [Example Go Lambda function code](#golang-example-code "#golang-example-code")
- [Handler naming conventions](#golang-handler-naming "#golang-handler-naming")
- [Defining and accessing the input event object](#golang-example-input "#golang-example-input")
- [Accessing and using the Lambda context object](#golang-example-context "#golang-example-context")
- [Valid handler signatures for Go handlers](#golang-handler-signatures "#golang-handler-signatures")
- [Using the AWS SDK for Go v2 in your handler](#golang-example-sdk-usage "#golang-example-sdk-usage")
- [Accessing environment variables](#golang-example-envvars "#golang-example-envvars")
- [Using global state](#golang-handler-state "#golang-handler-state")
- [Code best practices for Go Lambda functions](#go-best-practices "#go-best-practices")

## Setting up your Go handler project

A Lambda function written in [Go](https://golang.org/ "https://golang.org/") is authored
...
[github.com/aws/aws-lambda-go/lambda](https://github.com/aws/aws-lambda-go/tree/master/lambda "https://github.com/aws/aws-lambda-go/tree/master/lambda") package, which implements the Lambda
...
- `func init() {}`: Include any code you want Lambda to run during the
  during the [initialization phase](lambda-runtime-environment.md#runtimes-lifecycle-ib "lambda-runtime-environment.md#runtimes-lifecycle-ib") in
  this `init()` method.
...
`RECEIPT_BUCKET` environment variable. After a successful invocation, the Amazon S3 bucket
...
For Go functions deployed using a [.zip deployment package](golang-package.md "golang-package.md"),
...
[container image](go-image.md#go-image-provided "go-image.md#go-image-provided"), you can use any name for the executable file.
...
[encoding/json standard library](https://pkg.go.dev/encoding/json "https://pkg.go.dev/encoding/json").
You can then deserialize it into your struct using the [func Unmarshal](https://golang.org/pkg/encoding/json/#Unmarshal "https://golang.org/pkg/encoding/json/#Unmarshal") function.
...
information about accepted handler signatures, see [Valid handler signatures for Go handlers](#golang-handler-signatures "#golang-handler-signatures").
...
For more information about the context object, see [Using the Lambda context object to retrieve Go function information](golang-context.md "golang-context.md").
...
compatible with the _encoding/json_ standard library. For more information, see
[func Unmarshal](https://golang.org/pkg/encoding/json/#Unmarshal "https://golang.org/pkg/encoding/json/#Unmarshal") to learn how these types
are deserialized.
...
Often, you'll use Lambda functions to interact with or make updates to other AWS resources. The simplest
 way to interface with these resources is to use the [AWS SDK for Go v2](https://pkg.go.dev/github.com/aws/aws-sdk-go-v2 "https://pkg.go.dev/github.com/aws/aws-sdk-go-v2").
...
When using the SDK in your handler, configure your clients with the right settings. The simplest way
 ...
 provider chain](../../../sdkref/latest/guide/standardized-credentials.md#credentialProviderChain "../../../sdkref/latest/guide/standardized-credentials.md#credentialProviderChain"). This example illustrates one way to load this configuration:
...
In this example, we initialized our Amazon S3 client in the `init()` function to
 ...
 described in [Using the context in AWS SDK client initializations and calls](golang-context.md#golang-context-sdk "golang-context.md#golang-context-sdk").
...
In your handler code, you can reference any [environment variables](configuration-envvars.md "configuration-envvars.md")
 by using the `os.Getenv()` method. In this example, we reference the defined
 `RECEIPT_BUCKET` environment variable using the following line of code:
