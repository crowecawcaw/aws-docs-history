...
- [Using an AWS base image for Ruby](#ruby-image-instructions "#ruby-image-instructions")
...
- [AWS base images for Ruby](#ruby-image-base "#ruby-image-base")
- [Using an AWS base image for Ruby](#ruby-image-instructions "#ruby-image-instructions")
- [Using an alternative base image with the runtime interface client](#ruby-image-clients "#ruby-image-clients")
...
2. Create a new file called `Gemfile`. This is where you list your application's required RubyGems packages. The AWS SDK for Ruby is available from RubyGems. You should choose specific AWS service gems to install. For example, to use the [Ruby gem for Lambda](https://rubygems.org/gems/aws-sdk-lambda/ "https://rubygems.org/gems/aws-sdk-lambda/"), your Gemfile should look like this:
...
Alternatively, the [aws-sdk](https://rubygems.org/gems/aws-sdk/ "https://rubygems.org/gems/aws-sdk/") gem contains every available AWS service gem. This gem is very large. We recommend that you use it only if you depend on many AWS services. 3. Install the dependencies specified in the Gemfile using [bundle install](https://bundler.io/v2.4/man/bundle-install.1.html "https://bundler.io/v2.4/man/bundle-install.1.html").
...
5.  Create a new Dockerfile. The following is an example Dockerfile that uses an [AWS base image](images-create.md#runtimes-images-lp "images-create.md#runtimes-images-lp"). This Dockerfiles uses the following configuration:
...
Install the [Lambda runtime interface client for Ruby](https://rubygems.org/gems/aws_lambda_ric "https://rubygems.org/gems/aws_lambda_ric") using the RubyGems.org package manager:

```
gem install aws_lambda_ric
```

You can also download the [Ruby runtime interface client](https://github.com/aws/aws-lambda-ruby-runtime-interface-client "https://github.com/aws/aws-lambda-ruby-runtime-interface-client") from GitHub.
...
5.  Create a new Dockerfile. The following Dockerfile uses a Ruby base image instead of an [AWS base image](images-create.md#runtimes-images-lp "images-create.md#runtimes-images-lp"). The Dockerfile includes the [runtime interface client for Ruby](https://github.com/aws/aws-lambda-ruby-runtime-interface-client "https://github.com/aws/aws-lambda-ruby-runtime-interface-client"), which makes the image compatible with Lambda. Alternatively, you can add the runtime interface client to your application's Gemfile.
