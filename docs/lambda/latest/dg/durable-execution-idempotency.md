...
| 4        | Yes            | Running                                           | No                 | **Error:*<br>• Lambda returns `DurableExecutionAlreadyExists` error because an execution with this name is already running with different payload                                       |
...
| 6        | Yes            | Closed (succeeded, failed, stopped, or timed out) | No                 | **Error:*<br>• Lambda returns `DurableExecutionAlreadyExists` error because an execution with this name already completed with different payload                                        |
