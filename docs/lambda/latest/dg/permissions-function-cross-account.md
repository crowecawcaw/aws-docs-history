...
To share a function with another AWS account, add a cross-account permissions statement to the function's
[resource-based policy](access-control-resource-based.md "access-control-resource-based.md"). We recommend using `put-resource-policy`
to create a full JSON policy. With `put-resource-policy`, you can specify multiple principals, add conditions,
and create deny statements. You can also use `add-permission` for simple use cases.
...
The following example policy grants two AWS accounts permission to invoke a function, with one restricted to a specific
IAM role:
...
Save the policy to a file named `policy.json` and apply it. The `resource-arn` can specify a function,
version, or alias:
...
You can also restrict cross-account access to specific versions using an alias. The following policy grants
invoke access only through the `prod` alias:
...
You can grant cross-account access for most API actions that operate on an existing function. For example, you could grant access to
`lambda:ListAliases` to let an account get a list of aliases, or `lambda:GetFunction` to let them download your function
code.
...
For simple use cases, you can use `add-permission` to add individual statements. The following example grants
account `111122223333` permission to invoke `my-function` with the `prod` alias:
