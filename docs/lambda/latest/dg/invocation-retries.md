...
- **Asynchronous invocation** – Lambda retries function errors twice. If
  ...
  events that weren't successfully processed. For more information, see [Adding a dead-letter queue](invocation-async-retain-records.md#invocation-dlq "invocation-async-retain-records.md#invocation-dlq").
- **Event source mappings** – Event source mappings that read from streams
  ...
  resolved or the items expire. To detect stalled shards, you can monitor the [Iterator Age](monitoring-metrics.md "monitoring-metrics.md") metric.
...
more information, see [How Lambda processes records from stream and queue-based event sources](invocation-eventsourcemapping.md "invocation-eventsourcemapping.md") and the service-specific topics under [Invoking Lambda with events from other AWS services](lambda-services.md "lambda-services.md").
