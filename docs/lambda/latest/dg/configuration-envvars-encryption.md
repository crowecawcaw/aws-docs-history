...
database credentials. For more information, see [Use Secrets Manager secrets in Lambda functions](with-secrets-manager.md "with-secrets-manager.md").
...
3. (Optional) Enable console encryption helpers to use client-side encryption to protect your data in
   ...
   Save this policy to use in the last step of this procedure. 5. Add code to your function that decrypts the environment variables. To see an example, choose
...
Lambda](access-control-identity-based.md "access-control-identity-based.md"). Key policies grant the remaining permissions to users in the **Key users**
group.
...
For details on managing key permissions, see [Key policies in AWS KMS](../../../kms/latest/developerguide/key-policies.md "../../../kms/latest/developerguide/key-policies.md") in the
_AWS Key Management Service Developer Guide_.
