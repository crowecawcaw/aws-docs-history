...
2. To [Set up a CloudWatch Logs subscriptions filter to send Lambda function logs to Amazon S3](#using-cwl-subscription-filter-lambda-s3 "#using-cwl-subscription-filter-lambda-s3"). This filter defines which log events are delivered to your Amazon S3 bucket.
...
`amzn-s3-demo-bucket2` is an example Amazon S3 bucket name. It is _reserved_. For this procedure to work, you must to replace it with your unique Amazon S3 bucket name.
