...
- Calls to AWS services, such as Amazon Simple Queue Service (Amazon SQS), to read from an event source that's set up as an
  [event source mapping](invocation-eventsourcemapping.md "invocation-eventsourcemapping.md").
  ...
  in an IAM identity-based policy or [service control policy (SCP)](../../../organizations/latest/userguide/orgs_manage_policies_scps.md "../../../organizations/latest/userguide/orgs_manage_policies_scps.md").
...
and `aws:SourceArn` condition keys. The `lambda:FunctionArn` condition key applies only to [event source mappings](invocation-eventsourcemapping.md "invocation-eventsourcemapping.md") and helps define which functions your
...
condition operators](../../../IAM/latest/UserGuide/reference_policies_elements_condition_operators.md#Conditions_ARN "../../../IAM/latest/UserGuide/reference_policies_elements_condition_operators.md#Conditions_ARN"). The value of the key must be a valid ARN.
...
`arn:aws:lambda:*:123456789012:function:source_lambda`, or unless they come from the specified VPC.
