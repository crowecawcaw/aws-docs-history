...
[Lambda extensions](runtimes-extensions-api.md "runtimes-extensions-api.md") can use the Lambda Runtime Logs API to subscribe
to log streams directly from within the Lambda [execution environment](lambda-runtime-environment.md "lambda-runtime-environment.md"). Lambda
streams the logs to the extension, and the extension can then process, filter, and send the logs to any preferred
destination.

![The Extensions API and the Logs API connect Lambda and external extensions.](images/logs-api-concept-diagram.png)
...
- [Subscribing to receive logs](#runtimes-logs-api-subscribing "#runtimes-logs-api-subscribing")
- [Memory usage](#runtimes-logs-api-memory "#runtimes-logs-api-memory")
- [Destination protocols](#runtimes-logs-api-dest "#runtimes-logs-api-dest")
- [Buffering configuration](#runtimes-logs-api-buffering "#runtimes-logs-api-buffering")
- [Example subscription](#runtimes-logs-api-subs-example "#runtimes-logs-api-subs-example")
- [Sample code for Logs API](#runtimes-logs-api-samples "#runtimes-logs-api-samples")
- [Logs API reference](#runtimes-logs-api-ref "#runtimes-logs-api-ref")
- [Log messages](#runtimes-logs-api-msg "#runtimes-logs-api-msg")
...
First [register the extension](runtimes-extensions-api.md#extensions-registration-api-a "runtimes-extensions-api.md#extensions-registration-api-a") to receive the extension
identifier. Then subscribe to the Logs API during [initialization](lambda-runtime-environment.md#runtimes-lifecycle-ib "lambda-runtime-environment.md#runtimes-lifecycle-ib").
...
adjust the [buffering configuration](#runtimes-logs-api-buffering "#runtimes-logs-api-buffering"). Buffer memory usage counts
...
1. **HTTP** (recommended) – Lambda delivers logs to a local HTTP endpoint
   (`http://sandbox.localdomain:${PORT}/${PATH}`) as an array of records in JSON format. The
   `$PATH` parameter is optional. Note that only HTTP is supported, not HTTPS. You can choose to
   receive logs through PUT or POST.
2. **TCP** – Lambda delivers logs to a TCP port in [Newline delimited JSON (NDJSON) format](https://github.com/ndjson/ndjson-spec "https://github.com/ndjson/ndjson-spec").

We recommend using HTTP rather than TCP. With TCP, the Lambda platform cannot acknowledge when it delivers logs
...
- Lambda retries the attempt to send the logs (with backoff) if there is no listener, or if the POST or PUT
...
- If the subscriber cannot process incoming logs quickly enough, Lambda might drop logs to keep memory utilization
  ...
  log. For more information, see [Lambda: Not all of my function's logs appear](troubleshooting-execution.md#troubleshooting-execution-missinglogs "troubleshooting-execution.md#troubleshooting-execution-missinglogs").
...
send logs to custom destinations](https://aws.amazon.com/blogs/compute/using-aws-lambda-extensions-to-send-logs-to-custom-destinations/ "https://aws.amazon.com/blogs/compute/using-aws-lambda-extensions-to-send-logs-to-custom-destinations/") on the AWS Compute Blog.

For Python and Go code examples showing how to develop a basic Lambda extension and subscribe to the Logs API,
see [AWS Lambda Extensions](https://github.com/aws-samples/aws-lambda-extensions "https://github.com/aws-samples/aws-lambda-extensions") on the AWS
Samples GitHub repository. For more information about building a Lambda extension, see [Using the Lambda Extensions API to create extensions](runtimes-extensions-api.md "runtimes-extensions-api.md").
...
`destination` – See [Destination protocols](#runtimes-logs-api-dest "#runtimes-logs-api-dest"). Required: yes. Type:
strings.

`buffering` – See [Buffering configuration](#runtimes-logs-api-buffering "#runtimes-logs-api-buffering"). Required: no. Type:
strings.
...
extension to receive [platform.runtimeDone](#runtimes-logs-api-ref-done "#runtimes-logs-api-ref-done")
messages.
...
- [Function logs](#runtimes-logs-api-msg-function "#runtimes-logs-api-msg-function")
- [Extension logs](#runtimes-logs-api-msg-extension "#runtimes-logs-api-msg-extension")
- [Platform logs](#runtimes-logs-api-msg-platform "#runtimes-logs-api-msg-platform")
