...
- [Step 1: Create a SAML identity provider in AWS IAM](#create-saml-identity-provider "#create-saml-identity-provider")
- [Step 2: Create a SAML 2.0 federation IAM role](#create-saml-iam-role "#create-saml-iam-role")
- [Step 3: Embed an inline policy for the IAM role](#embed-inline-policy "#embed-inline-policy")
- [Step 4: Configure your SAML 2.0 identity provider](#configure-saml-id-provider "#configure-saml-id-provider")
- [Step 5: Create assertions for the SAML authentication response](#create-assertions-saml-auth "#create-assertions-saml-auth")
- [Step 6: Configure the relay state of your federation](#configure-relay-state "#configure-relay-state")
- [Step 7: Enable integration with SAML 2.0 on your WorkSpaces directory](#enable-integration-saml "#enable-integration-saml")
...
`&client=`. When the parameter is specified in a relay state URL, the corresponding sessions
will automatically start in the specified client.
...
7. For the **User Access URL** and **IdP deep link parameter name**,
   enter values that are applicable to your IdP and the application you have configured in Step 1.
   The default value for the IdP deep link parameter name is “RelayState“ if you omit this parameter.
...
for your IdP, choose **Test**. Copy and paste the test URL to a private window in your current browser
or another browser to test the SAML 2.0 logon without disrupting your current AWS management console session.
When IdP-initiated flow opens, you can register your WorkSpaces client. For more information, see
[Identity provider (IdP)-initiated flow](amazon-workspaces-saml.md "amazon-workspaces-saml.md"). 8. Manage fallback settings by checking or unchecking **Allow clients that do not support SAML 2.0 to login**.
Enable this setting to continue providing your users access to WorkSpaces using client types or versions that do not support
SAML 2.0 or if users need time to upgrade to the latest client version.

###### Note

This setting allows users to bypass SAML 2.0 and log in using directory authentication using older client versions. 9. To use SAML with the web client, enable Web Access. For more information, see
[Enable and configure Amazon WorkSpaces Web Access](web-access.md "web-access.md").

###### Note

PCoIP with SAML is not supported on Web Access. 10. Choose **Save**. Your WorkSpaces directory is now enabled with SAML 2.0 integration. You can use the
IdP-initiated and client application-initiated flows to register WorkSpaces client applications and sign in to WorkSpaces.
