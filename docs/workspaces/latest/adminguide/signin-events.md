...
specific credential challenge or factor, as well as the status of that particular
credential verification request. A user is signed in only after completing all required
credential challenges, which results in a `UserAuthentication` event being
logged.
...
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `CredentialChallenge`    | Notifies that AWS sign-in has requested that the user solve a<br>specific credential challenge and specifies the<br>`CredentialType` that is required (for example,<br>SMARTCARD).                                                                                                               |
