...
| --- | --- | --- | 
| Enabled | DWORD | **0** – (Default) Disables in-place upgrade<br />**1** – Enables in-place upgrade | 
| PostUpgradeRestoreProfileOnD | DWORD | **0** – (Default) Does not attempt to restore the user profile path after the in-place upgrade<br />**1** – Restores the user profile path (**ProfileImagePath**) after the in-place upgrade | 
| UserShellFoldersRedirection | DWORD | **0** – Does not enable redirection of user shell folders<br />**1** – (Default) Enables redirection of user shell folders to `D:\Users\%USERNAME%` after the user profile is regenerated on `C:\Users\%USERNAME%` | 
| NoReboot | DWORD | **0** – (Default) Allows you to control when a reboot occurs after modifying the registry for the user profile<br />**1** – Does not allow the script to reboot the WorkSpace after modifying the registry for the user profile | 
...
| Registry key | Type | Values | 
| --- | --- | --- | 
| Enabled | DWORD | **0** – (Default) Disables AWS PV drivers update<br />**1** – Enables AWS PV drivers update | 
