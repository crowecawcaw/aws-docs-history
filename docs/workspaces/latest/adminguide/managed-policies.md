...
This policy is used in the workspaces\_DefaultRole, which WorkSpaces uses to access required resources in the customer AWS account for WorkSpaces Pools. For more information see [Create the workspaces\_DefaultRole Role](workspaces-access-control.md#create-default-role). It provides the following permissions:
