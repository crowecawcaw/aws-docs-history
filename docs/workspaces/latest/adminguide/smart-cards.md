...
- To use a smart card with a Windows or Linux WorkSpace, the user must use the Amazon WorkSpaces Windows
  client version 3.1.1 or later or the WorkSpaces macOS client version 3.1.5 or later. For more
...
- For in-session authentication and pre-session authentication on Linux or Windows WorkSpaces,
  only one smart card is currently allowed at a time.
...
In addition to configuring your AD Connector directory, you must also make sure
that the certificates that are issued to the domain controllers for your on-premises
...
Linux WorkSpaces on DCV currently have the following limitations:

- Clipboard, audio-in, video-in, and time zone redirection aren't supported.
- Multiple monitors aren't supported.
- You must use the WorkSpaces Windows client application to connect to Linux WorkSpaces on DCV.

To enable the use of smart cards on Linux WorkSpaces, you need to include a root CA
...
- Performs a default configuration, which includes enabling `pkinit` during WorkSpace
  ...
  your Linux WorkSpaces and to enable smart cards for your Linux WorkSpaces.
...
Linux WorkSpaces assume that the certificates on the smart cards are issued for the user's
...
be performed individually on each user's WorkSpace. 3. (Optional) By default, all services are enabled to use smart card authentication on Linux WorkSpaces. To limit smart
