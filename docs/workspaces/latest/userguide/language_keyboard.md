...
To use the same language and keyboard settings on your client device and your WorkSpace, use one of the following methods, depending on which protocol your WorkSpace is using: PCoIP or DCV. 
...
+ To select the **Server keyboard layout** mode, enable the **Configure server keyboard layout usage** GPO (see *Amazon WorkSpaces Administration Guide* for more details). Then, if you are using a language-specific keyboard you can follow the same instructions for provided for [WorkSpaces using the PCoIP protocol](#lang-keyboard-pcoip).
