...
- **Step 1.** Through a game server group, the
  linked Auto Scaling group is set up to launch instances of types m4.large, c4.large,
  and c5.large with game server protection enabled. The Auto Scaling group has
...
- **Step 2.** Amazon GameLift Servers FleetIQ evaluates the current
  ...
  game server interruption. Amazon GameLift Servers FleetIQ immediately updates the Auto Scaling group
...
- **Step 3.** Amazon GameLift Servers FleetIQ identifies existing
  ...
  In addition, an Auto Scaling group with draining instances is triggered to launch
  new instances to replace them.
- **Step 4.** As new viable instances come
  online, the Auto Scaling group terminates draining instances. This replacement
...
- **Step 5.** Draining instances with game
  ...
  c4.large is again considered a viable instance type, the Auto Scaling group is updated to
