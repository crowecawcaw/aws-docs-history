...
key ways: first by allowing only viable instance types in the Auto Scaling group, and second by
...
demand. To do this, the Auto Scaling group relies on a list of your desired instance types.
The job of Amazon GameLift Servers FleetIQ is to continually check the viability of these desired instance
types and update the list for the Auto Scaling group. This process is called instance
balancing. It ensures that instances in the Auto Scaling group are continually refreshed so
that only currently viable instance types are used at all times.

Amazon GameLift Servers FleetIQ affects how the Auto Scaling group selects optimal instance types in the
following ways:
...
- **It determines usage of Spot and/or On-Demand
  ...
  balancing strategy, which influences how the Auto Scaling group uses Spot and/or
...
As the Auto Scaling group launches and retires instances, it maintains a collection that
...
1. Where possible, Amazon GameLift Servers FleetIQ places new game sessions on instances that are
   ...
   instances and keeping others idle, the Auto Scaling group is able to quickly scale
...
Auto Scaling group from terminating instances with actively running game sessions.
