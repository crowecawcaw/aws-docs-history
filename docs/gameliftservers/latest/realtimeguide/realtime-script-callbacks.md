...
Invoked when the server process is being terminated by the Amazon GameLift Servers service. This can
...
Invoked when a player disconnects from the game session, either by sending a
disconnect request or by other means.
...
Invoked when a server process is started. This callback allows the script to perform
any custom tasks needed to prepare to host a game session.
...
Invoked when a message is received on the server from one player to be delivered to
another player. This process runs before the message is delivered.
...
Invoked when a message is received on the server from one player to be delivered to a
group. This process runs before the message is delivered.
