...
Using a serverless client service architecture, the backend can view the status of matchmaking tickets from a highly scalable database instead of by directly accessing the Amazon GameLift Servers API.
...
1. The Lambda function requests a match through Amazon GameLift Servers FlexMatch matchmaking.

1. FlexMatch matches a group of players with suitable latency, and then requests a game session placement through an Amazon GameLift Servers queue. The queue has fleets with one or more AWS Region locations in it.
...
1. The game client connects to the game server using TCP or UDP by using the port and IP address that the backend service provides. The game client then sends the player session ID to the game server, which then validates the ID using the server SDK for Amazon GameLift Servers.