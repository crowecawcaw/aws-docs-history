...
+ **Multi-region queues** – Amazon GameLift Servers game session queues are used to place new game sessions with available hosting resources. Queues that span multiple Regions are able to redirect game session placements in the event of a regional outage. For more information and best practices on creating game session queues, see [Customize a game session queue](queues-design.md).
+ **Automatic capacity scaling** – Maintain the health and availability of your hosting resources by using Amazon GameLift Servers scaling tools. These tools provide a range of options that let you adjust fleet capacity to fit the needs of your game and players. For more information on scaling, see [Scaling game hosting capacity with Amazon GameLift Servers](fleets-manage-capacity.md).
