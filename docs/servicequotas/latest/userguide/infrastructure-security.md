...
As a managed service, Service Quotas is protected by AWS global network security. For
...
You use AWS published API calls to access Service Quotas through the network. Clients must
...
- Cipher suites with perfect forward secrecy (PFS) such as DHE (Ephemeral
  ...
  You can call these API operations from any network location, but Service Quotas does support resource-based access policies, which can include restrictions
