...
**Topics**
+ [Submit the simple\_job sample](#submit-a-simplejob-sample)
+ [Submit a simple\_job with a parameter](#submit-with-parameter)
+ [Create a simple\_file\_job job bundle with file I/O](#create-job-bundle-with-file-io)
+ [Next steps](#submit-a-job-next)

## Submit the simple\_job sample
<a name="submit-a-simplejob-sample"></a>

After you create a farm and run the worker agent, you can submit the simple\_job sample to Deadline Cloud. 
...
1. Navigate to the job bundle samples directory.

   ```
   cd ~/deadline-cloud-samples/job_bundles/
   ```

1. Submit the simple\_job sample.

   ```
   deadline bundle submit simple_job
   ```

1. Choose your second CloudShell tab to view the logging output about calling `BatchGetJobEntities`, getting a session, and running a session action.
...
1. Choose your first tab, then inspect the log files that the worker agent writes.
   ...
   1. Print the first log file that the worker agent creates. 
   ...
   1. Print the log file output when the worker agent joins the fleet. 
   ...
   1. Print the log files for each session in a directory that is named the same as the queue resource id.
...
1. Print information about the job. 
...
You can submit jobs with parameters. In the following procedure, you edit the simple\_job template to include a custom message, submit the simple\_job, then print the session log file to view the message. 
...
1. Print the contents of the simple\_job template.
...
1. Submit the simple\_job sample with a parameter value, then wait for the job to finish running.
...
1. To see the custom message, view the most recent session log file.
...
1. Make a copy of `simple_job` with the new name `simple_file_job`.

   ```
   cp -r simple_job simple_file_job
   ```

1. Edit the job template as follows:
**Note**  
We recommend that you use nano for these steps. If you prefer to use Vim, you must set its paste mode using `:set paste`.
   ...
   1. Add the following `type`, `objectType`, and `dataFlow` `parameterDefinitions`. 
   ...
   1. Add the following `bash` script command to the end of the file that reads from the input file and writes to the output file. 
...
1. Provide parameter values for the input and output files to submit the simple\_file\_job.
...
+ [Submit jobs with job attachments in Deadline Cloud](run-jobs-job-attachments.md) to learn how to run jobs on hosts running different operating systems.
