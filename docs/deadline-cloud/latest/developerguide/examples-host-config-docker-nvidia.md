...
1. Installs Docker through `dnf` and starts the service.

1. Adds `job-user` to the `docker` group so that jobs can run containers without sudo.

1. Installs the NVIDIA Container Toolkit from the official repository.

1. Configures the Docker daemon to use the NVIDIA runtime.

1. Generates the CDI (Container Device Interface) spec for GPU access.

1. Restarts Docker and verifies the setup.

The fleet AMI must have NVIDIA GPU drivers already installed, which Deadline Cloud GPU AMIs provide, and the fleet must use a GPU instance type such as `g6.xlarge`, `g6e.xlarge`, or `p4d.24xlarge`.
