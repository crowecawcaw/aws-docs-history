...
`onEnter` and `onExit` actions run the
...
significantly improve performance when running many jobs with the
same package requirements. By default, persistent environments are
stored under `~/.persistent_envs`; modify the
`onEnter` and `onExit` actions to reference a
different path.

To get equivalent functionality on customer-managed fleets, see
