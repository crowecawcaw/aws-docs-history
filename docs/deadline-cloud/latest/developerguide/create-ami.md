...
Finally, we recommend you enable the worker agent auto shutdown. Auto shutdown allows the worker fleet to scale up when needed and to shutdown when the rendering job finishes. This auto scaling helps ensure you're only using resources as needed.
