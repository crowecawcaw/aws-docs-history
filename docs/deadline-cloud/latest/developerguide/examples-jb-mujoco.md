...
1. **Datagen** — Scripted
   ...
   Each episode is verified by a cube-height check and discarded if it
   fails.
2. **Train** — Finetunes a LeRobot
   ACT policy on the generated dataset (CUDA).
3. **Render** — Drives a MuJoCo
   rollout with the finetuned policy and records the result as an MP4
   video and PNG frames.
   The bundle requires a Linux x86\_64 GPU fleet (steps render headless
