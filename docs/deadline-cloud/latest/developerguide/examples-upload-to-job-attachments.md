...
Python script on the GitHub website uploads files and directories from your workstation to a
...
them again. The script is useful for pre-populating job attachments with
large datasets.
...
- Two configuration modes: direct Amazon S3 specification or queue
  lookup.
...
For a job-based alternative that copies files between Amazon S3 buckets,
see [Copy an S3 prefix to job attachments on Deadline Cloud](examples-jb-copy-s3-to-attachments.md "examples-jb-copy-s3-to-attachments.md").
