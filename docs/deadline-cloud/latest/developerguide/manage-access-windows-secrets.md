...
It is highly recommended to follow the principle of least-privilege for these secrets. Following least privilege means that access to fetch the secret value of a queue's `jobRunAsUser` → `windows` → `passwordArn` should be:
...
1. In the Resource permissions section, add a policy statement of the form:
...
1. In the Resource permissions section, remove the policy statement of the form:
