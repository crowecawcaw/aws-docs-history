...
job bundle on the GitHub website evaluates multiple LLMs against multiple
benchmarks in a single Deadline Cloud job. Each model becomes one task in a parameter sweep, and tasks run
in parallel across workers. A final step aggregates per-model results into
a ranked leaderboard (CSV and Markdown).

Each task in the `EvalModels` step starts a local
vLLM server, runs every benchmark with EleutherAI's lm-evaluation-harness
against the local endpoint, then stops vLLM. For more information, see
[vLLM](https://github.com/vllm-project/vllm "https://github.com/vllm-project/vllm") and
[lm-evaluation-harness](https://github.com/EleutherAI/lm-evaluation-harness "https://github.com/EleutherAI/lm-evaluation-harness") on
the GitHub website. Models load directly from Hugging Face Hub, so job attachments
are not required.
...
service-managed fleet (A10G or L4) and a queue with a conda queue
environment that this bundle uses without modification. If you already have
a farm, you need an SMF fleet with NVIDIA GPUs, at least 32 GB of RAM, and
at least 4 vCPUs.
