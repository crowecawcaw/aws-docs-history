...
Running this tutorial incurs charges for the GPU worker instances that process the job. The [Understand the cost of your training jobs](#tutorial-gsplat-cost) section shows how to review those costs.
...
1. Capture a video of a subject.

1. Submit the `gsplat_pipeline` job.

1. Monitor the job.

1. Download and view the Gaussian Splatting `.ply` file.
...
1. From the `job_bundles` directory, run the following command:

   ```
   deadline bundle gui-submit gsplat_pipeline
   ```

1. Switch to the **Job-specific settings** tab and select paths for both the **Input Video File** and the **Output Ply File**.

1. Choose **Submit**, or customize the settings first. If the input video has higher complexity, you may need to increase the **Approximate Image Count** value.
...
1. Modify the **GSplat Simple Trainer Options** text from `--strategy.cap-max 1000000` to `--strategy.cap-max 2000000` to get 2 million splats, and from `--no-use-bilateral-grid` to `--use-bilateral-grid` to enable the bilateral grid option.
