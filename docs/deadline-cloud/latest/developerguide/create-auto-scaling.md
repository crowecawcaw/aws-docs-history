...
Use the AWS CloudFormation YAML template below to create an Amazon EC2 Auto Scaling (Auto Scaling) group, an Amazon Virtual Private Cloud (Amazon VPC)
...
* Credentials from the IAM role that are attached to your worker's Amazon EC2 instance are
 available to *all* processes running on that worker,
...
You can set up an EventBridge rule to filter events, a Lambda to consume the events and
 control Auto Scaling, and an SQS queue to store unprocessed events. Use the following AWS CloudFormation
