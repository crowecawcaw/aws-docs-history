...
into a partitioned table, you can use the [MSCK REPAIR TABLE](msck-repair-table.md "msck-repair-table.md") command, which works only with Hive-style
partitions.
...
- If you issue queries against Amazon S3 buckets with a large number of objects and
  ...
  design patterns: Optimizing Amazon S3 performance](../../../AmazonS3/latest/userguide/request-rate-perf-considerations.md "../../../AmazonS3/latest/userguide/request-rate-perf-considerations.md") .
...
- Make sure that the Amazon S3 path is in lower case instead of camel case (for
  ...
  AWS Glue Data Catalog. For more information, see [MSCK REPAIR TABLE](msck-repair-table.md "msck-repair-table.md").
...
- If you are using the AWS Glue Data Catalog with Athena, see [AWS Glue endpoints and quotas](../../../general/latest/gr/glue.md "../../../general/latest/gr/glue.md") for service
  ...
  - Although Athena supports querying AWS Glue tables that have 10 million
    ...
    indexes](https://aws.amazon.com/blogs/big-data/improve-amazon-athena-query-performance-using-aws-glue-data-catalog-partition-indexes/ "https://aws.amazon.com/blogs/big-data/improve-amazon-athena-query-performance-using-aws-glue-data-catalog-partition-indexes/").
...
[`aws s3 ls`](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/s3/ls.html "https://awscli.amazonaws.com/v2/documentation/api/latest/reference/s3/ls.html") command, which lists the S3 objects
under a specified prefix:

```
aws s3 ls s3://elasticmapreduce/samples/hive-ads/tables/impressions/`PRE dt=2009-04-12-13-00/
...
In the following example, the `aws s3 ls` command shows [ELB](elasticloadbalancer-classic-logs.md "elasticloadbalancer-classic-logs.md") logs stored in
...
```
aws s3 ls s3://athena-examples-`myregion`/elb/plaintext/ --recursive`2016-11-23 17:54:46 11789573 elb/plaintext/2015/01/01/part-r-00000-ce65fca5-d6c6-40e6-b1f9-190cc4f93814.txt
...
create it. Instead, you can use the [ALTER TABLE ADD PARTITION](alter-table-add-partition.md "alter-table-add-partition.md") command to add each
...
For more information, see [Use partition projection with Amazon Athena](partition-projection.md "partition-projection.md").
