...
1. (Optional) Choose **Encrypt query results** if you want your query results to be encrypted.
   + For **Encryption type**, choose one of the following options:
     + **SSE\_S3** – This option uses server-side encryption (SSE) with Amazon S3-managed encryption keys.
     + **SSE\_KMS** – This option uses server-side encryption (SSE) with AWS KMS-managed keys. 
