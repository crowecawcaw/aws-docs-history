...
+ `MSCK REPAIR TABLE` does not add partition column names that begin with an underscore (\_). To work around this limitation, use [ALTER TABLE ADD PARTITION](alter-table-add-partition.md). 
