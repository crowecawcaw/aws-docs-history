...
| DPUConsumed              | The number of DPUs actively consumed by queries in a `RUNNING` state at a given time in a reservation. This metric is emitted only when workgroup is associated with a capacity reservation and includes all workgroups associated with a reservation. If you move a workgroup from one reservation to another, the metric includes data from the time when the workgroup belonged to the first reservation. For more information about capacity reservations, see [Manage query processing capacity](capacity-management.md "capacity-management.md"). |
...
| TotalExecutionTime       | The number of milliseconds that Athena took to run a DDL or DML query. TotalExecutionTime includes QueryQueueTime, QueryPlanningTime, EngineExecutionTime, and ServiceProcessingTime.                                                                                                                                                                                                                                                                                                                                                                   | These metrics for Athena have the following dimensions. |
