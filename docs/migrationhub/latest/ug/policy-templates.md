...
# AWS Migration Hub roles and policies

Access to AWS Migration Hub requires credentials that AWS can use to authenticate
your requests as well as have permissions to access AWS resources. The following
sections demonstrate how the various permissions policies can be attached to IAM
identities (that is, users, groups, and roles) and thereby grant permissions to perform
...
The policy templates have been organized in the following hierarchy as shown below.
You can click on any policy to go directly to its template.
