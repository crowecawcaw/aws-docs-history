...
The Resolver contains endpoints that you configure to answer DNS queries to and
...
You also can integrate DNS resolution between Resolver and DNS resolvers on your network by configuring forwarding rules.
...
Route 53 Resolver. Inbound endpoints allow your DNS resolvers to easily resolve
...
Resolver chooses the rule with the most specific match (acme.example.com) and
forwards the query to the IP addresses that you specified in that rule.
There are three types of rules, **forward**,
**system**, and **delegation**. For more information, see [How Route 53 Resolver endpoint forwards DNS
...
[Using Resolver in VPCs that are configured for dedicated instance tenancy](resolver-choose-vpc.md#resolver-considerations-dedicated-instance-tenancy "resolver-choose-vpc.md#resolver-considerations-dedicated-instance-tenancy").
...
queries to originate from. For each IP address and delegation that you specify, Resolver
...
![Conceptual graphic that shows the path of a DNS query from your network to Route 53 Resolver.](images/Resolver-outbound-endpoint.png)
