...
**Note**  
 Forwarding private DNS queries to any VPC CIDR \+ 2 address from on-premises or other VPC DNS servers is not supported, and can cause unstable results. Instead, we recommend that you use a Resolver inbound endpoint.
...
**Inbound endpoint: DNS resolvers on your network can forward DNS queries to Route 53 VPC Resolver through this endpoint**  
