...
but none of these name servers has ever hosted `example.com`. Route 53 cannot protect against this because no hosted zone has ever existed
to establish a hold on those name servers for that domain name. To mitigate the risk, only delegate to Route 53 name servers that belong to a public hosted zone that you control.
