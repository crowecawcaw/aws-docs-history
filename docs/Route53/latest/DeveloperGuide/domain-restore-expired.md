...
<a name="domain-restore-expired"></a>
...
**Important**  
For generic TLD domains that are deleted within the first 5 days after registration, the standard 30-day redemption period (to check for the actual redemption period number of days, see [Domains that you can register with Amazon Route 53](registrar-tld-list.md)) does not apply. Instead, these domains become immediately available for new registration.
...
1. Using the AWS account that the domain was registered to, sign in to the [AWS Support Center](https://console.aws.amazon.com/support/cases#/create?issueType=customer-service&serviceCode=service-domains&categoryCode=restoration). 
...
1. When AWS Support determines whether the domain can be restored, a representative contacts you. In addition, if Route 53 restores your domain, the domain reappears in the console. The expiration date depends on whether the domain expired or was accidentally deleted:  
**The domain expired**  
