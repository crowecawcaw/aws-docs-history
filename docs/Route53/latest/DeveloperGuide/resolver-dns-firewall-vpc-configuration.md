...
The DNS Firewall configuration for your VPC determines whether Route 53 Resolver allows queries
...
- By default, the failure mode is closed, which means that Resolver blocks any queries for
  which it doesn't receive a reply from DNS Firewall and sends a
  `SERVFAIL` DNS response. This approach favors security over
  availability.
...
2. In the navigation pane under **Resolvers**, choose
   **VPCs**.
...
- Update your VPC firewall configuration by calling [UpdateFirewallConfig](../APIReference/API_route53resolver_UpdateFirewallConfig.md "../APIReference/API_route53resolver_UpdateFirewallConfig.md") and enabling or disabling
  `FirewallFailOpen`.
