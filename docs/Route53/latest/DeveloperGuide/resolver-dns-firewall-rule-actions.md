...
You are required to specify one of the following options in each rule that you
create:

- **Allow** – Stop
  inspecting the query and permit it to go through. Not available for
...
Reply with the configured block response, from the following:
    ...
    + **NODATA**  –
     ...
     available for it.
    + **NXDOMAIN** –
     Respond indicating that the query's domain name doesn't exist.
    + **OVERRIDE** – Provide
     a custom override in the response. This option requires the
     following additional settings:
    	...
    	- **Record
    	 ...
    	 record value. This must be `CNAME`.
    	- **Time to live in
    	 seconds** –
    	 The recommended amount of time for the DNS resolver or web
    	 browser to cache the override record and use it in response
