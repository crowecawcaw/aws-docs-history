...
**Note**  
To create records for complex routing setups, you can also use the Traffic Flow visual editor and save the setup as a traffic policy. You can then link the traffic policy to one or more domain names (such as example.com) or subdomain names (such as www.example.com), in the same hosted zone or in multiple hosted zones. You can also roll back updates if the new setup isn't working as expected. For more information, see [Using Traffic Flow to route DNS traffic](traffic-flow.md).<a name="resource-record-sets-creating-procedure"></a>
