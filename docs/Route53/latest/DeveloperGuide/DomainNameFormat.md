...
**Note**  
If you're using Route 53 with AWS Certificate Manager, note that ACM wildcard certificates only protect one subdomain level. For example, an ACM certificate for \*.example.com protects login.example.com and test.example.com, but not test.login.example.com. For more information, see [AWS Certificate Manager certificate characteristics and limitations](https://docs.aws.amazon.com/acm/latest/userguide/acm-certificate-characteristics.html).
