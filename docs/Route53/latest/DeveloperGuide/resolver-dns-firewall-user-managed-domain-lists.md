...
You can create your own domain lists to specify domain categories that you either
don't find in the managed domain list offerings or that you prefer to handle on your
own.

In addition to the procedures described in this section, in the console, you can
...
- With the exception of the optional starting asterisk and a period, as a delimiter between
  labels, it must only contain the following characters: `A-Z`,
  `a-z`, `0-9`, `-` (hyphen).
...
non-production environment, with the rule action set to `Alert`.
Evaluate the rule using Amazon CloudWatch metrics and the Resolver logs. The logs provide
the domain list name for all alerts and blocking actions. When you're satisfied
that the domain list is matching your DNS queries the way you want it to, change
the rule action setting as needed. For information about CloudWatch metrics and the
query logs, see [Monitoring Route 53 Resolver DNS Firewall rule
...
Choose **DNS Firewall** in the navigation pane to open the DNS Firewall
**Rule groups** page on the Amazon VPC console. Continue to
step 2.
...
the Amazon VPC console under [https://console.aws.amazon.com/vpc/](https://console.aws.amazon.com/vpc/ "https://console.aws.amazon.com/vpc/"). 2. In the navigation pane, under **DNS Firewall**, choose **Domain lists**. In the **Domain
lists** page, you can select and edit existing domain lists and
you can add your own. 3. To add a domain list, choose **Add domain list**. 4. Provide a name for your domain list, and then enter your domain
specifications in the text box, one per line.

If you slide **Switch to bulk upload** to
**on**, enter the URI of the Amazon S3 bucket
where you created a domain list. This domain list should have one domain
name per line.

###### Note

Duplicate domain names will cause the bulk import to fail. 5. Choose **Add domain list**. The **Domain
lists** page lists your new domain list.
After you create the domain list, you can reference it by name from your DNS Firewall
rules.
...
3. Select the domain list that you want to delete, then choose
