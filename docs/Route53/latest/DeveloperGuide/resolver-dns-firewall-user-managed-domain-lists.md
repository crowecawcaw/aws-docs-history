...
- With the exception of the optional starting asterisk and a period, as a
  delimiter between labels, it must only contain the following characters: `A-Z`
  ,
  `a-z`, `0-9`, `-` (hyphen).
...
the navigation pane to open the DNS Firewall **Rule groups** page on
the Amazon VPC console. Continue to step 2.
...
the Amazon VPC console under [https://console.aws.amazon.com/vpc/](https://console.aws.amazon.com/vpc/ "https://console.aws.amazon.com/vpc/"). 2. In the navigation pane, under **DNS Firewall**, choose **Domain
lists**. In the **Domain
...
If you slide **Switch to bulk upload** to **on**,
enter the URI of the Amazon S3 bucket where you created a domain list. This
domain list should have one domain name per line.
...
After
you create the domain list, you can reference it by name from your DNS Firewall
rules.
