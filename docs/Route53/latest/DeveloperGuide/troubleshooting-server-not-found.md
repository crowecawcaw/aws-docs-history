...
If your browser displays a "Server not found" error when you try to browse to a domain (example.com) or a subdomain (www.example.com),
...
"Server not found" error, we recommend that you confirm that the value is correct.

## The resource that you're routing traffic to is unavailable

If a record specifies a resource such as a web server that's unavailable, a browser will return a
"Server not found" error. We recommend that you check the status of the resource that you're routing traffic to.
