...
If the domain name in a query matches the domain name in a default forward rule, VPC Resolver forwards
...
record names in private hosted zones. This rule causes VPC Resolver to forward DNS queries to public name servers if the
...
**Forwarding** for **Type**, and specify the IP addresses of those resolvers. 3. VPC Resolver returns the response to the application that submitted the query.
