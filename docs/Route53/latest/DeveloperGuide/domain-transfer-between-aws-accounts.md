...
If you don't sign in either by using the root account or by using a user that has the required permissions, Route 53 can't perform the transfer. This requirement prevents unauthorized users from transferring domains to other AWS accounts.
...
1. On the **Generate password** dialog, copy the password, and forward it to the receiving AWS account owner.
   ...
   On the **Requests** page, the **Status** for the domain displays **In progress**, and the **Type** displays **Internal transfer out**.<a name="domain-transfer-between-aws-accounts-accept-procedure"></a>
