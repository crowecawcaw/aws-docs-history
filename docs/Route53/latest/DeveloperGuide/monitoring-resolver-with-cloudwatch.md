...
For **inbound endpoints** the traffic load balancing is customer-dependent. Therefore capacity warnings or critical alerts might indicate a "hot spot" where a subset of elastic network interfaces is disproportionately utilized.
...
For **outbound endpoints** the traffic is automatically balanced across elastic network interfaces. Capacity issues might be due to problems with the target name server, or because high-latency queries of timeouts overwhelm the Resolver network interfaces.
