...
The CloudWatch metric (`cloudwatchMetric`) action captures an Amazon CloudWatch
...
- An IAM role that AWS IoT can assume to perform the `cloudwatch:PutMetricData` operation.
...
(Optional) A string that contains the timestamp, expressed in
seconds in Unix epoch time. Defaults to the current Unix epoch
time.
...
The IAM role that allows access to the CloudWatch metric. For more
information, see [Requirements](#cloudwatch-metrics-rule-action-requirements "#cloudwatch-metrics-rule-action-requirements").

Supports [substitution templates](iot-substitution-templates.md "iot-substitution-templates.md"): No

## Examples

The following JSON example defines a CloudWatch metric action in an AWS IoT
rule.
...
The following JSON example defines a CloudWatch metric action with substitution
