...
1. To resolve the location information, choose **Resolve**.
   ...
   + The confidence level, which indicates the uncertainty in the location estimate response. The default value is 0.68, which indicates a 68% probability that the actual device location is within the uncertainty radius of the estimated location. You can customize this value using the `AdvancedConfiguration` parameter. For more information, see [Advanced configuration for AWS IoT Core Device Location](device-location-advanced-configuration.md).
...
The optional `--advanced-configuration` parameter contains the `WiFiCellular` object, which includes the `ConfidencePercent` field to customize the confidence level for Wi-Fi and Cellular measurement data. In this example, the `ConfidencePercent` is set to {{68}}, which is also the default value when this parameter is not specified. For more information, see [Advanced configuration for AWS IoT Core Device Location](device-location-advanced-configuration.md).
