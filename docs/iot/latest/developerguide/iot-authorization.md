...
- **MQTT over TLS/TCP, TLS mutual authentication (port 8883 or 443)[†](protocols.md#protocol-port))**
  - **SDK:** AWS IoT Device SDK
  - **Identity type:** X.509 certificates
  - **Policy type:** AWS IoT Core policy

- **MQTT over HTTPS/WebSocket, AWS SigV4 authentication (port 443)**
  - **SDK:** AWS Mobile SDK
  - **Identity type:** Authenticated Amazon Cognito identity / **Policy type:** IAM and AWS IoT Core policies
  - **Identity type:** Unauthenticated Amazon Cognito identity / **Policy type:** IAM policy
  - **Identity type:** IAM, or federated identity / **Policy type:** IAM policy

- **HTTPS, AWS Signature Version 4 authentication (port 443)**
  - **SDK:** AWS CLI
  - **Identity type:** Amazon Cognito, IAM, or federated identity
  - **Policy type:** IAM policy

- **HTTPS, TLS mutual authentication (port 8443)**
  - **SDK:** No SDK support
  - **Identity type:** X.509 certificates
  - **Policy type:** AWS IoT Core policy

- **HTTPS over custom authentication (Port 443)**
  - **SDK:** AWS IoT Device SDK
  - **Identity type:** Custom authorizer
  - **Policy type:** Custom authorizer policy
...
- **HTTPS AWS Signature Version 4 authentication (port 443)**
  - **SDK:** AWS CLI
  - **Identity type:** Amazon Cognito identity / **Policy type:** IAM policy
  - **Identity type:** IAM, or federated identity / **Policy type:** IAM policy
