...
If the payload is a JSON object, you can reference keys in the object. Your outgoing
...
You can select multiple items including '\*' to add items to the incoming payload. For
example:
...
You can use the `"VALUE"` keyword to produce outgoing payloads that are not
JSON objects. With SQL version `2015-10-08`, you can select only one item.
...
You can use `'.'` syntax to drill into nested JSON objects in the incoming
payload. For example:
...
For information about how to use JSON object and property names that include reserved
characters, such as numbers or the hyphen (minus) character, see [JSON extensions](iot-sql-json.md "iot-sql-json.md")

You can use functions (see [Functions](iot-sql-functions.md "iot-sql-functions.md")) to transform the incoming payload. You can use
