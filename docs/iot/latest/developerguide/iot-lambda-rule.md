...
AWS SDK to send the formatted message to the Amazon SNS topic you created in the tutorial about how
to [Tutorial: Sending an Amazon SNS notification](iot-sns-rule.md "iot-sns-rule.md").

In the tutorial about how to [Tutorial: Sending an Amazon SNS notification](iot-sns-rule.md "iot-sns-rule.md"), the JSON document that
resulted from the rule's query statement was sent as the body of the text message. The result
...
In this tutorial, you'll use an AWS Lambda rule action to call an AWS Lambda function that
...
The AWS Lambda function you'll create in this tutorial formats the message string by using
the data from the rule query statement and calls the [SNS publish](https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/sns.html#SNS.Client.publish "https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/sns.html#SNS.Client.publish") function of the AWS SDK to create the notification.

###### What you'll learn in this tutorial

- How to create and test an AWS Lambda function
- How to use the AWS SDK in an AWS Lambda function to publish an Amazon SNS
  notification
...
This tutorial requires the SNS notification topic you created in the tutorial about
how to [Tutorial: Sending an Amazon SNS notification](iot-sns-rule.md "iot-sns-rule.md"). It also assumes that you've completed the other
...
Unlike the tutorial about how to [Tutorial: Sending an Amazon SNS notification](iot-sns-rule.md "iot-sns-rule.md"), which used an AWS IoT
rule action to send the notification, this tutorial sends the notification from the Lambda
function by using a function of the AWS SDK. The actual Amazon SNS notification topic used in
this tutorial, however, is the same one that you used in the tutorial about how to [Tutorial: Sending an Amazon SNS notification](iot-sns-rule.md "iot-sns-rule.md").
...
1.  Create a new AWS Lambda function.
    ...
    and then choose **Configure**. 3. In **Basic information**:
        ...
        2. In **Execution role**, choose **Create a new role
         from AWS policy templates**.
        3. In Role name, enter the name of the new role,
         `format-high-temp-notification-role`.
        4. In **Policy templates - *optional***, search for and select **Amazon SNS
         publish policy**.
...
If you didn't get a text message on the phone, check the result of the operation. In the
**Function code** panel, in the **Execution result**
...
In this rule, you'll use the rule query statement to create a message payload for the
...
This contains all the information the Lambda function needs to format and send the
...
4.  In **Rule query statement** of **Create a
    ...
    This statement:
        ...
        * Selects the second element from the topic string, converts it to a decimal
         number, and then assigns it to the `device_id` field.
        * Selects the value of the `temperature` field from the message
         payload and assigns it to the `reported_temperature` field.
...
3. Confirm that the text message was sent.
   ...
   **device/+/data** to check that the message payload matches what
   ...
   If you change the topic ID element in the message topic, remember that casting
   the `topic(2)` value to a numeric value will only work if that element in
...
Every time you publish your input message to the `device/32/data`
...
- You created an AWS IoT rule to call a Lambda function that sent an Amazon SNS notification
...
- Change the fields selected in the rule query statement, update the Lambda function to
...
- Change the test in the rule query statement to test for a minimum temperature
  instead of a maximum temperature. Update the Lambda function to format a new message and
  remember to change the name of `max_temperature`.
- To learn more about how to find errors that might occur while you're developing and
  using AWS IoT rules, see [Monitoring AWS IoT](monitoring_overview.md "monitoring_overview.md").
