...
Specify queries in a query string using a query syntax. The queries are passed to the [`SearchIndex`](https://docs.aws.amazon.com/iot/latest/apireference/API_SearchIndex.html) API. The following table lists some example query strings.
...
|  thingName:\*my  | Queries for things with names that end in "my". Leading wildcard use is limited to 1 query term per query. Queries that include a leading wildcard have a max query length of 100 characters. | 
|  thingName:\*my\*  | Queries for things that contain the substring "my". Leading wildcard use is limited to 1 query term per query. Queries that include a leading wildcard have a max query length of 100 characters. | 
