# Scenarios for AWS IoT using AWS SDKs

The following code examples show you how to implement common scenarios in AWS IoT
with AWS SDKs. These scenarios show you how to accomplish specific tasks by calling multiple functions
within AWS IoT or combined with other AWS services.
Each scenario includes a link to the complete source code, where you can find instructions on how to set up and run the code.

Scenarios target an intermediate level of experience to help you understand service actions in context.

###### Examples

- [Getting started with internet of things device protection](example_iot_GettingStarted_079_section.md "example_iot_GettingStarted_079_section.md")
- [Getting started with internet of things messaging](example_iot_GettingStarted_063_section.md "example_iot_GettingStarted_063_section.md")
