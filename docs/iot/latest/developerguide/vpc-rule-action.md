...
For pricing purposes, a VPC rule action is metered in addition to the
action that sends a message to a resource when the resource is in your VPC.
...
A list of security groups to apply to the network
interfaces.

roleArn

The Amazon Resource Name (ARN) of a role that has permission
to create network interfaces on your behalf.
...
3. Select a role that has the permissions required to create
