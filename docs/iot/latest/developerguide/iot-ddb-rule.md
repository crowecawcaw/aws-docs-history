...
This tutorial demonstrates how to create an AWS IoT rule that sends message data to a DynamoDB
table.

In this tutorial, you create a rule that sends message data from an imaginary weather
sensor device to a DynamoDB table. The rule formats the data from many weather sensors such that
...
- [Step 1: Create the DynamoDB table for this tutorial](#iot-ddb-rule-ddb-table "#iot-ddb-rule-ddb-table")
- [Step 2: Create an AWS IoT rule to send data to the DynamoDB table](#iot-ddb-rule-topic-rule "#iot-ddb-rule-topic-rule")
- [Step 3: Test the AWS IoT rule and DynamoDB table](#iot-ddb-rule-test "#iot-ddb-rule-test")
- [Step 4: Review the results and next steps](#iot-ddb-rule-review "#iot-ddb-rule-review")
...
- ###### Reviewed the [Amazon DynamoDB](../../../amazondynamodb/latest/developerguide/Introduction.md "../../../amazondynamodb/latest/developerguide/Introduction.md") overview

If you've not used DynamoDB before, review [Getting Started with DynamoDB](../../../amazondynamodb/latest/developerguide/GettingStartedDynamoDB.md "../../../amazondynamodb/latest/developerguide/GettingStartedDynamoDB.md") to become familiar with the
basic concepts and operations of DynamoDB.

## Step 1: Create the DynamoDB table for this tutorial

In this tutorial, you'll create a DynamoDB table with these attributes to record the data
from the imaginary weather sensor devices:

- `sample_time` is a primary key and describes the time the sample was
  recorded.
- `device_id` is a sort key and describes the device that provided the
  sample
- `device_data` is the data received from the device and formatted by the
...
2. In **Create table**:
   ...
   1. In **Table name**, enter the table name:
      `wx_data`.
   2. In **Partition key**,
      enter `sample_time`, and in the option list next to the field,
      choose `Number`.
   3. In **Sort key**, enter
      `device_id`, and in the option list next to the field, choose
      `Number`.
   4. At the bottom of the page, choose **Create**.

You'll define `device_data` later, when you configure the DynamoDB rule
action.
...
For the database entry, you'll use the rule query statement to flatten the structure of
...
In this rule, you'll also use a couple of [Substitution templates](iot-substitution-templates.md "iot-substitution-templates.md"). Substitution templates are expressions that
...
5. In **SQL statement**:
   ...
   This statement:
        ...
        * Formats the elements of the `wind` attribute as individual
         attributes.
...
To test the new rule, you'll use the MQTT client to publish and subscribe to the MQTT
...
2. Now, publish a message to the input topic with a specific device ID,
   ...
   2. For **Topic name**, enter the input topic name,
      `device/22/data`.
   3. For **Message payload**, enter the following
      sample data.
...
The first thing to do is to refresh the display by choosing the refresh icon in
the upper-right corner of the table's header. If that doesn't display the data
you're looking for, check the following.
...
###### Things to check
    ...
    The field names used in the topic rule must match those found in the JSON
     message payload of the published message.


    Open the rule you created in the console and check the field names in the rule
    ...
    The policies that authorize the rule to receive message data and update the
    ...
    If you suspect this is the problem, edit the rule action and create a new
...
After you send a few messages to the DynamoDB table with this rule, try experimenting with
it to see how changing some aspects from the tutorial affect the data written to the table.
...
- Change the fields selected in the rule query statement and observe the effect on the
  data. You could use this to filter the data stored in the table.
- Add a republish rule action to send an MQTT message for each row added to the table.
  You could use this for debugging.

After you have completed this tutorial, check out [Tutorial: Formatting a notification by using an AWS Lambda function](iot-lambda-rule.md "iot-lambda-rule.md").
