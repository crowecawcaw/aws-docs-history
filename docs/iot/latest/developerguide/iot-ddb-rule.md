...
Make sure that you choose DynamoDB and not DynamoDBv2 as the rule action.
   ...
   1. In the **Create role** dialog box, for **Role name**, enter **wx\_ddb\_role**. This new role will automatically contain a policy with a prefix of "aws-iot-rule" that will allow the **wx\_data\_ddb** rule to send data to the **wx\_data** DynamoDB table you created.
