...
Returns a `Boolean` result. Returns true if the left operand is less
than the right operand. Both operands are converted to a `Decimal`, and
then compared.
...
Returns a `Boolean` result. Returns true if both left and right
...
The "+" is an overloaded operator. It can be used for string concatenation or
addition.
