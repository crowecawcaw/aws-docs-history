...
The expression `v` is evaluated and matched for
equality against the `t[i]` value of each
`WHEN` clause. If a match is found, the corresponding
`r[i]` expression becomes the result of
the `CASE` statement. The `WHEN` clauses are evaluated in order so
