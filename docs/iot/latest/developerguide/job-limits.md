...
**Topics**
+ [Job executions limits](#job-execution-limits)
+ [Active and concurrent job limits](#job-limits-active-concurrent)
...
- **`SCHEDULED`**
  - **Phase:** Non-concurrent phase: AWS IoT Jobs waits for the scheduled startTime of the job to begin job execution notifications to your devices. Jobs in this phase only count towards the active jobs limit and will have the IsConcurrent property set to false.
  - **Active jobs limit:** Applies
  - **Job concurrency limit:** Does not apply

- **`IN_PROGRESS`**
  - **Phase:** Concurrent phase: AWS IoT Jobs accepts the request for creating the job and starts rolling out job execution notifications to your devices. Jobs in this phase are concurrent, as denoted by the IsConcurrent property set to true, and count towards both the active jobs and the job concurrency limits. / **Active jobs limit:** Applies / **Job concurrency limit:** Applies
  - **Phase:** Non-concurrent phase: AWS IoT Jobs waits for devices to report the results of their job executions. Jobs in this phase only count towards the active jobs limit and will have the IsConcurrent property set to false. / **Active jobs limit:** Applies / **Job concurrency limit:** Does not apply

- **`Canceled`**
  - **Phase:** Concurrent phase: AWS IoT Jobs accepts the request for canceling the job and starts canceling job executions previously created for your devices. Jobs in this phase are concurrent and will have the IsConcurrent property set to true. Once the job and job executions have been canceled, the job is no longer concurrent and does not count towards the job concurrency limit.
  - **Active jobs limit:** Does not apply
  - **Job concurrency limit:** Applies
