...
1. Configure network connectivity and routing between Oracle Database@AWS and Amazon S3.
...
1. Select the ODB network for which you want to enable Amazon S3 access.

1. Choose **Modify**.

1. Select **Amazon S3**.

1. (Optional) Configure an Amazon S3 policy document to control access to Amazon S3. If you don't specify a policy, the default policy grants full access.
...
1. Choose **ODB networks**.

1. Choose the name of the ODB network.

1. Choose **OCI resources**.

1. Choose the **Service integrations** tab.

1. Under **Amazon S3**, note the following pieces of information:
...
1. In the left navigation pane, choose Exadata VM clusters and then choose your VM cluster name.

1. At the top of the page, choose the **Summary** tab.

1. Choose **Virtual machines** and then choose the name of your VM.

1. Note the value in **DNS Name**. This is the host name that you specify when you connect to your VM using `ssh`.

1. In the top right, choose **Manage in OCI**. This opens the OCI console.

1. On the **Virtual Cloud Networks** list page, choose the VCN that contains the network security group (NSG) for the ODB network client subnet (`exa_static_nsg`). For more information, see [Managing Security Rules for an NSG](https://docs.oracle.com/en-us/iaas/Content/Network/Concepts/manage-nsg-security-rules.htm) in the OCI documentation.

1. On the details page, perform one of the following actions depending on the option that you see:
   + On the **Security** tab, go to **Network Security Groups**.
   + Under **Resources**, choose **Network Security Groups**. 

1. Choose the NSG for the client subnet (`exa_static_nsg`).

1. Add an egress rule for the VPC endpoint address that you noted earlier.
...
1. Run the following commands to make sure that the VM can access the Amazon S3 Amazon VPC endpoint. Use the S3 domain name that you noted down previously.
...
1. Deploy and activate an Amazon S3 file gateway, using an Amazon EC2 instance as the hub. Follow the instructions in [Deploy a customized Amazon EC2 host for S3 File Gateway](https://docs.aws.amazon.com/filegateway/latest/files3/ec2-gateway-file.html) in the *Storage Gateway User Guide*.
...
1. Create a Amazon VPC endpoint for private connectivity between your ODB network and the Storage Gateway. For more information, see [Access an AWS service using an interface VPC endpoint](https://docs.aws.amazon.com/vpc/latest/privatelink/create-interface-endpoint.html).

1. Create a file share for your Amazon S3 bucket through the Storage Gateway console. For more information, see [Creating a file share](https://docs.aws.amazon.com/filegateway/latest/files3/GettingStartedCreateFileShare.html).

**To back up your database to Amazon S3 using Storage Gateway**

1. In a terminal, use `ssh` to connect to the DNS name of the Exadata VM. To find the DNS name, see [Prerequisites for user-managed backups to Amazon S3 in Oracle Database@AWS](#manual-backups-prerequisites).

1. Create a directory on the Exadata VM cluster server for the NFS mount. The following example creates the directory `/home/oracle/sgw_mount/`.

   ```
   mkdir /home/oracle/sgw_mount/
   ```

1. Mount the NFS share on the directory that you just created. The following example creates the share on the directory `/home/oracle/sgw_mount/`. Replace {{SG-IP-address}} with your Storage Gateway IP address and {{your-bucket-name}} with the name of your S3 bucket.

   ```
   sudo mount -t nfs -o nolock,hard {{SG-IP-address}}:/{{your-bucket-name}} /home/oracle/sgw_mount/
   ```

1. Connect to RMAN and back up the database to the mounted directory. The following example creates the channel `rman_local_bkp` and uses the mount point path to format the backup pieces.
...
1. Verify that the backup files are created in the mount directory. The following example shows two backup pieces.
...
1. Install the Amazon S3 mount point on the Exadata VM cluster server. For more information about installation and configuration, see [Mountpoint for Amazon S3](https://docs.aws.amazon.com/AmazonS3/latest/userguide/mountpoint.html) in the *Amazon S3 User Guide*.

   ```
   $ sudo yum install ./mount-s3.rpm
   ```

1. Verify the installation by running the `mount-s3` command.
...
1. Create an intermediate backup directory on the Exadata VM cluster server local storage. You will back up your database to this local directory and then copy the backup to your S3 bucket. The following example creates directory `/u02/rman_bkp_local`.

   ```
   mkdir /u02/rman_bkp_local
   ```

1. Create a directory for the Amazon S3 mount point. The following example creates directory `/home/oracle/s3mount`.

   ```
   $ mkdir /home/oracle/s3mount
   ```

1. Mount your Amazon S3 bucket using the mount point. The following example mounts an S3 bucket on directory `/home/oracle/s3mount`. Replace {{your-s3-bucket-name}} with your actual Amazon S3 bucket name.

   ```
   $ mount-s3 s3://{{your-s3-bucket-name}} /home/oracle/s3mount
   ```

1. Verify that you can access the Amazon S3 bucket contents.

   ```
   $ ls -lart /home/oracle/s3mount
   ```

1. Connect RMAN to your target database and back it up to your local staging directory. The following example creates the channel `rman_local_bkp` and uses the path `/u02/rman_bkp_local/` to format the backup pieces.
...
1. Verify that the backups are created in the local directory:
...
1. Copy the backup files from the local staging directory to the Amazon S3 mount point.

   ```
   cp /u02/rman_bkp_local/* /home/oracle/s3mount/
   ```

1. Verify that you copied the files successfully to Amazon S3.
...
1. Select the ODB network for which you want to disable Amazon S3 access.

1. Choose **Modify**.

1. Clear the **Enable S3 access** checkbox.

1. Choose **Modify ODB network**.
