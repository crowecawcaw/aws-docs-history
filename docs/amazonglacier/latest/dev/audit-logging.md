...
All Amazon Glacier actions are logged by CloudTrail and are documented in the [API Reference for Amazon Glacier](amazon-glacier-api.md). For example, calls to the [Create Vault (PUT vault)](api-vault-put.md), [Delete Vault (DELETE vault)](api-vault-delete.md), and [List Vaults (GET vaults)](api-vaults-get.md) actions generate entries in the CloudTrail log files. 
