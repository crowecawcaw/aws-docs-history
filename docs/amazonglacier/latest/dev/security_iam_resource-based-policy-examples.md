...
A Amazon Glacier vault can have one vault access policy and one Vault Lock policy associated
with it. A Amazon Glacier _vault access policy_ is a resource-based policy
that you can use to manage permissions to your vault. A _Vault Lock
policy_ is vault access policy that can be locked. After you lock a Vault Lock
policy, the policy can't be changed. You can use a Vault Lock Policy to enforce compliance
controls.

###### Topics

- [Vault Access Policies](vault-access-policy.md "vault-access-policy.md")
