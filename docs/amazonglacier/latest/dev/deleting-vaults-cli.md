...
the specified SNS topic anytime the specific event occurs. 4. When it's complete, use the `get-job-output` command to download the retrieval
...
archive ID in quotation marks in order to run this command. 6. Use the `initiate-job` command to start a new inventory retrieval
job.
