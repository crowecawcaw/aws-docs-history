...
The following C\# code example uses the high-level API of the AWS SDK for .NET to download the archive that you uploaded previously in [Upload an Archive to a Vault in Amazon Glacier by Using the AWS SDK for .NET](getting-started-upload-archive-dotnet.md). In the code example, note the following:
