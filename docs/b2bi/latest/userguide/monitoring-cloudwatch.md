...
+ One log group for all transformers that you create (logging for transformers is always enabled).

  This log group is named `/aws/vendedlogs/b2bi/transformers`.
...
| --- | --- | --- | --- | 
| **Capability Match**<br />The service attempts to match the incoming file with one of the customer’s existing trading capability resources | The attempt to match is successful | Attempt to match the incoming file failed | System is searching for a trading capability match | 
