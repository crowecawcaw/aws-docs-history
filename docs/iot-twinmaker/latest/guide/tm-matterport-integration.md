...
<a name="tm-matterport-integration"></a>
...
**Topics**
+ [Integration overview](#tm-matterport-integration-overview)
+ [Matterport integration prerequisites](#tm-matterport-integration-prereqs)
+ [Generate and record your Matterport credentials](#tm-matterport-integration-sdk-key)
+ [Store your Matterport credentials in AWS Secrets Manager](#tm-matterport-integration-secrets)
+ [Import Matterport spaces into AWS IoT TwinMaker scenes](#tm-matterport-integration-tm-console)
+ [Use Matterport spaces in your AWS IoT TwinMaker Grafana dashboard](#tm-matterport-grafana)
+ [Use Matterport spaces in your AWS IoT TwinMaker web application](#tm-matterport-app-kit)

## Integration overview
<a name="tm-matterport-integration-overview"></a>
...
1. **[Generate and record your Matterport credentials](#tm-matterport-integration-sdk-key)**

1. **[Store your Matterport credentials in AWS Secrets Manager](#tm-matterport-integration-secrets)**

1. **[Import Matterport spaces into AWS IoT TwinMaker scenes](#tm-matterport-integration-tm-console)**
...
<a name="tm-matterport-integration-prereqs"></a>
...
+ You have updated your AWS IoT TwinMaker workspace role. For more information on creating a workspace role, see [ Create and manage a service role for AWS IoT TwinMaker](https://docs.aws.amazon.com/iot-twinmaker/latest/guide/twinmaker-gs-service-role.html).
...
+ You must contact Matterport to configure the necessary licensing for enabling the integration. Matterport will also enable a Private Model Embed (PME) for the integration.
  ...
  1. Open the **[Matterport and AWS IoT TwinMaker](https://matterport.com/partners/amazon-web-services)** page.

  1. Press the **Contact us** button, to open the contact form.

  1. Fill out the required information on the form.

  1. When you're ready, choose **SAY HELLO** to send your request to Matterport.
...
<a name="tm-matterport-integration-sdk-key"></a>
...
1. Log in to your [Matterport account](https://authn.matterport.com).

1. Navigate to your account settings page.

1. Once in the settings page, select the **Developer tools** option.

1. On the **Developer tools** page, go to the **SDK Key Management** section.

1. Once in the **SDK Key Management** section, select the option to add a new SDK key.

1. Once you have the Matterport SDK key, add domains to the key for AWS IoT TwinMaker and your Grafana server. If you are using the AWS IoT TwinMaker app kit, then make sure to add your custom domain as well.

1. Next, find the **Application integration Management** section, you should see your **PME application listed**. Record the following information:
   + The **Client ID**
   + The **Client Secret**
**Note**  
Since the **Client Secret** is only presented to you once, we strongly recommend that you record your **Client Secret**. You must present your **Client Secret** in the AWS Secrets Manager console to continue with the Matterport integration.
   ...
   These credentials are automatically created when you have purchased the necessary components and the PME for your account has been enabled by Matterport. If these credentials do not appear, contact Matterport. To request contact, see the **[Matterport and AWS IoT TwinMaker](https://matterport.com/partners/amazon-web-services)** contact form.

For more information on Matterport SDK credentials, see Matterport's official SDK documentation [SDK Docs Overview](https://matterport.github.io/showcase-sdk/index.html).

## Store your Matterport credentials in AWS Secrets Manager
<a name="tm-matterport-integration-secrets"></a>

Use the following procedure to store your Matterport credentials in AWS Secrets Manager.

**Note**  
 You need the **Client Secret** created from the procedure in the [Generate and record your Matterport credentials](#tm-matterport-integration-sdk-key) topic to continue with the Matterport integration.

1. Log in to the AWS Secrets Manager console.

1. Navigate to the **Secrets** page and select **Store a new secret**.

1. For the **Secret type**, select **Other type of secret**.

1. In the **Key/value pairs** section, add in the following key-value pairs, with your Matterport credentials as the values:
...
1. Choose **Next** to move on to the **Configure secret** page.

1. Fill out the field for **Secret name** and the **Description**.
...
<a name="tm-matterport-integration-tm-console"></a>
...
1. Create or open an existing AWS IoT TwinMaker scene in which you want to use a Matterport space.
