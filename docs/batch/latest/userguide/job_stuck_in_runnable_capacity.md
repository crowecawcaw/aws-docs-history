...
All connected compute environments have insufficient capacity errors. When requested,
AWS Batch detects Amazon EC2 instances that experience insufficient capacity errors. Manually
canceling the job will allow the subsequent job to move to the head of the queue.
...
1. The AWS Batch service role requires `autoscaling:DescribeScalingActivities`
   permission for this detection to work. If you use the [Using service-linked roles for AWS Batch](using-service-linked-roles.md "using-service-linked-roles.md")
   service-linked role (SLR) or the [AWS managed policy: AWSBatchServiceRole policy](security-iam-awsmanpol.md#security-iam-awsmanpol-AWSBatchServiceRolePolicy "security-iam-awsmanpol.md#security-iam-awsmanpol-AWSBatchServiceRolePolicy") managed policy, then
   you don't need to take any action because their permission policies are updated.
