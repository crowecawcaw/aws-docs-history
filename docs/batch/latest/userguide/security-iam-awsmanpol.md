...
+ `ecs` – Allows AWS Batch to create, update, and manage Amazon ECS clusters, task definitions, and tasks for job execution.
+ `iam` - Allows AWS Batch to validate and pass roles provided by owner to Amazon EC2, Amazon EC2 Auto Scaling and Amazon ECS.
+ `logs` – Allows AWS Batch to create and manage log groups and log streams for AWS Batch jobs.
...
| --- | --- | --- | 
| [AWSBatchServiceRole](#security-iam-awsmanpol-AWSBatchServiceRolePolicy) policy updated | Added the `ecs:UpdateCluster` permission so that AWS Batch can update Amazon ECS cluster configurations. | August 25, 2026 | 
