...
**Note**  
In the AWS Batch console, you can configure the `awslogs` log driver in the **Logging configuration** section when you create a job definition.
...
By default, AWS Batch enables the `awslogs` log driver. This section describes how to customize the `awslogs` log configuration for a job. For more information, see [Create a single-node job definition](create-job-definition.md).
