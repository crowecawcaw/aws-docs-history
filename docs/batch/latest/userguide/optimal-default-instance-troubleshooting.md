...
The `optimal` option now selects instance types from modern m, c, and r
instance families based on regional availability. AWS Batch periodically updates
the pool with newer generations within these families. If you are using `optimal`, no
action is needed on your part.

However, please be aware that only `ENABLED` and `VALID` Compute Environments (CEs) will be
updated with new instance types. If you have any `DISABLED` or `INVALID` CEs, they
