...
If AWS Batch detects that you have a `RUNNABLE` job blocking the front of the
queue, you'll receive [Service job queue blocked events](batch-service-job-queue-blocked-events.md "batch-service-job-queue-blocked-events.md") from with the reason. The
same reason is also updated into the `statusReason` field as a part of
`ListServiceJobs` and
`DescribeServiceJob` API calls.
...
The following sections each describe a distinct type of blocked reason that AWS Batch can
infer. Each section includes:

- The `statusReason` set on `ListServiceJobs` and
  `DescribeServiceJob` API actions.
- The `reason` value for `jobStateTimeLimitActions`
  configuration.
- The `statusReason` displayed if `jobStateTimeLimitActions`
