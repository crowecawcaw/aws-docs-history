...
memory. It runs as a collection of related yet separate basic jobs that might be distributed
across multiple hosts and might run concurrently. Array jobs are the most efficient way to run
extremely parallel jobs such as Monte Carlo simulations, parametric sweeps, or large rendering
jobs.
...
parent ID, such as ``example_job_ID`:0` for the first child
job of the array.
...
information about job dependencies, see [Job dependencies](job_dependencies.md "job_dependencies.md").
...
- [Example of an array job workflow](example_array_job.md "example_array_job.md")
- [Use the array job index to control job differentiation](array_index_example.md "array_index_example.md")
