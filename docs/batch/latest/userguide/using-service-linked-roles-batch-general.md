...
**Important**  
Service-linked role auto-creation only applies to `MANAGED` compute environments. For `UNMANAGED` compute environments, you must explicitly specify a `serviceRole`. Omitting the role for an `UNMANAGED` compute environment results in a `ServiceRole is required` error.
...
**Note**  
This automatic service-linked role creation only applies to `MANAGED` compute environments. For `UNMANAGED` compute environments, you must explicitly provide a `serviceRole` when calling `CreateComputeEnvironment`.
