...
As a managed service, AWS Service Catalog is protected by AWS global network security. For
...
You use AWS published API calls to access AWS Service Catalog through the network. Clients must
...
- Cipher suites with perfect forward secrecy (PFS) such as DHE (Ephemeral
  ...
  With AWS Service Catalog, you can control the Regions in which data is stored. Portfolios and products are
  only available in the Regions in which you have made them available. You can use the
  `CopyProduct` API to copy a product to another Region.
