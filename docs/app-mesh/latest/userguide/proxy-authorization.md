...
Proxy authorization authorizes the [Envoy](envoy.md "envoy.md") proxy running within
...
(TLS)](tls.md "tls.md") and for virtual gateways (with or without TLS). For customer accounts who want
...
configuration for all mesh endpoints, then skip to [Create IAM role](#create-iam-role "#create-iam-role"). If you want to limit the
...
editing an IAM policy, see [Creating IAM Policies](../../../IAM/latest/UserGuide/access_policies_create.md "../../../IAM/latest/UserGuide/access_policies_create.md") and
[Edit
IAM Policies](../../../IAM/latest/UserGuide/access_policies_manage-edit.md "../../../IAM/latest/UserGuide/access_policies_manage-edit.md").
...
- **Amazon EKS** – If you want to use a singe role, then
  ...
  for Service Accounts on your Cluster](../../../eks/latest/userguide/enable-iam-roles-for-service-accounts.md "../../../eks/latest/userguide/enable-iam-roles-for-service-accounts.md"). Create the IAM roles and associate the
  roles with Kubernetes service accounts. For more information, see [Creating an
  IAM Role and Policy for your Service Account](../../../eks/latest/userguide/create-service-account-iam-policy-and-role.md "../../../eks/latest/userguide/create-service-account-iam-policy-and-role.md") and [Specifying an IAM Role for
  your Service Account](../../../eks/latest/userguide/specify-service-account-role.md "../../../eks/latest/userguide/specify-service-account-role.md").
...
For more information about how to create an IAM role, see [Creating a Role for an AWS Service](../../../IAM/latest/UserGuide/id_roles_create_for-service.md#roles-creatingrole-service-console "../../../IAM/latest/UserGuide/id_roles_create_for-service.md#roles-creatingrole-service-console").
...
custom or managed IAM policy to an IAM role, see [Adding IAM Identity Permissions](../../../IAM/latest/UserGuide/access_policies_manage-attach-detach.md#add-policies-console "../../../IAM/latest/UserGuide/access_policies_manage-attach-detach.md#add-policies-console").
...
- **Amazon ECS** – Attach an Amazon ECS Task Role to the task
  ...
  attach it to a task, see [Specifying an IAM Role for your Tasks](../../../AmazonECS/latest/developerguide/task-iam-roles.md#create_task_iam_policy_and_role "../../../AmazonECS/latest/developerguide/task-iam-roles.md#create_task_iam_policy_and_role").
- **Amazon EC2** – The IAM role must be attached to the
  ...
  role, and now I want to assign it to an EC2 instance](https://aws.amazon.com/premiumsupport/knowledge-center/assign-iam-role-ec2-instance "https://aws.amazon.com/premiumsupport/knowledge-center/assign-iam-role-ec2-instance").
...
deployment. You'll need to assign one. For more information, see [Configure Service Accounts for Pods](https://kubernetes.io/docs/tasks/configure-pod-container/configure-service-account/ "https://kubernetes.io/docs/tasks/configure-pod-container/configure-service-account/") in the Kubernetes
documentation. 2. View the details of the service account returned in the previous step. The
...
Role for your Service Account](../../../eks/latest/userguide/specify-service-account-role.md "../../../eks/latest/userguide/specify-service-account-role.md"). 3. Sign in to the AWS Management Console and open the IAM console at [https://console.aws.amazon.com/iam/](https://console.aws.amazon.com/iam/ "https://console.aws.amazon.com/iam/"). 4. In the left navigation, select **Roles**. Select the name of
the IAM role that you noted in a previous step. 5. Confirm that either the custom policy you created previously, or the
`AWSAppMeshEnvoyAccess` managed policy is listed. If neither
policy is attached, [attach an IAM policy](#attach-iam-policy "#attach-iam-policy")
to the IAM role. If you want to attach a custom IAM policy but don't have one,
then you need to [create a custom IAM
policy](#create-iam-policy "#create-iam-policy") with the required permissions. If a custom IAM policy is attached,
...
Policies](../../../IAM/latest/UserGuide/access_policies_manage-edit.md "../../../IAM/latest/UserGuide/access_policies_manage-edit.md") and [Create IAM policy](#create-iam-policy "#create-iam-policy"). 6. Repeat the previous steps for each Kubernetes pod that contains the Envoy
proxy.
...
4. On the **Task Definition** page, select the link of the IAM
   ...
   role](#create-iam-role "#create-iam-role") and attach it to your task by [updating your
   task definition.](../../../AmazonECS/latest/developerguide/update-task-definition.md "../../../AmazonECS/latest/developerguide/update-task-definition.md")
5. In the **Summary** page, on the
   **Permissions** tab, confirm that either the custom policy you
   ...
   policy](#create-iam-policy "#create-iam-policy"). If a custom IAM policy is attached, select the policy and confirm
   ...
   [Edit IAM
   Policies](../../../IAM/latest/UserGuide/access_policies_manage-edit.md "../../../IAM/latest/UserGuide/access_policies_manage-edit.md") and [Create IAM policy](#create-iam-policy "#create-iam-policy").
...
3. In the **Description** tab, select the link of the IAM role
   ...
   role](#create-iam-role "#create-iam-role").
4. In the **Summary** page, on the
   **Permissions** tab, confirm that either the custom policy you
   created previously, or the `AWSAppMeshEnvoyAccess` managed policy is listed. If neither
   policy is attached, [attach the IAM policy](#attach-iam-policy "#attach-iam-policy")
   to the IAM role. If you want to attach a custom IAM policy but don't have one,
   then you need to [create the custom IAM
   policy](#create-iam-policy "#create-iam-policy"). If a custom IAM policy is attached, select the policy and confirm
   that it contains `"Action": "appmesh:StreamAggregatedResources"`. If it
