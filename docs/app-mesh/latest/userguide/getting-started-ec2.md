...
7. Run one of the following commands to start the App Mesh Envoy container on your
   ...
   -u `1337` --network host 840364872350.dkr.ecr.`region-code`.amazonaws.com/aws-appmesh-envoy:v1.34.4.0-prod`
   ```
   ...
   ```
   `sudo docker run --detach --env APPMESH_RESOURCE_ARN=`mesh/`apps`/virtualNode/`serviceB`` \
   -u `1337` --network host 772975370895.dkr.ecr.me-south-1.amazonaws.com/aws-appmesh-envoy:v1.34.4.0-prod`
   ```
   ...
   ```
   `sudo docker run --detach --env APPMESH_RESOURCE_ARN=`mesh/`apps`/virtualNode/`serviceB`` \
   -u `1337` --network host 856666278305.dkr.ecr.ap-east-1.amazonaws.com/aws-appmesh-envoy:v1.34.4.0-prod`
   ```
