...
This topic helps you use AWS App Mesh with an actual service that is running on Amazon EC2. This
tutorial covers basic features of several App Mesh resource types.
...
App Mesh supports Linux services that are registered with DNS, AWS Cloud Map, or both. To use
...
with Amazon EC2 Linux instances](../../../AWSEC2/latest/UserGuide/ec2-lamp-amazon-linux-2.md "../../../AWSEC2/latest/UserGuide/ec2-lamp-amazon-linux-2.md") in the Amazon EC2 User Guide. The remaining steps
assume that the actual services are named `serviceA`, `serviceB`,
...
`serviceb.apps.local`. Alternately, you can discover actual services using
...
`serviceA` and `serviceBv2` in a later step.
...
- Authorize the Envoy proxy that you deploy with each service to read the
  configuration of one or more virtual nodes. For more information about how to
  authorize the proxy, see [Envoy Proxy authorization](proxy-authorization.md "proxy-authorization.md").
...
7. Run one of the following commands to start the App Mesh Envoy container on your
   ...
   -u `1337` --network host 840364872350.dkr.ecr.`region-code`.amazonaws.com/aws-appmesh-envoy:v1.34.13.3-prod`
   ```
   ...
   ```
   `sudo docker run --detach --env APPMESH_RESOURCE_ARN=`mesh/`apps`/virtualNode/`serviceB`` \
   -u `1337` --network host 772975370895.dkr.ecr.me-south-1.amazonaws.com/aws-appmesh-envoy:v1.34.13.3-prod`
   ```
   ...
   ```
   `sudo docker run --detach --env APPMESH_RESOURCE_ARN=`mesh/`apps`/virtualNode/`serviceB`` \
   -u `1337` --network host 856666278305.dkr.ecr.ap-east-1.amazonaws.com/aws-appmesh-envoy:v1.34.13.3-prod`
   ```
