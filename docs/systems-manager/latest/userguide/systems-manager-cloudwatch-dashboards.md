...
- A single view for selected metrics and alarms to help you assess the health of
  your resources and applications across one or more AWS Regions. You can select the
  color used for each metric on each graph, so that you can track the same metric
...
- A common view of critical resource and application measurements that can be shared
  ...
  You can create dashboards by using the console, the AWS Command Line Interface (AWS CLI), or by using the
  CloudWatch `PutDashboard` API. For more information, see [Using Amazon CloudWatch
