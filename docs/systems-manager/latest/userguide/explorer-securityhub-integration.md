...
AWS accounts, services, and supported third-party products. Security Hub CSPM findings can
...
Security Hub CSPM sends findings to Amazon EventBridge, which uses an event rule to send the findings to
Explorer. After you enable integration, as described here, you can view Security Hub CSPM
findings in an Explorer widget and view finding details in OpsCenter OpsItems. The
widget provides a summary of all Security Hub CSPM findings based on severity. New findings in
Security Hub CSPM are usually visible in Explorer within seconds of being created.
...
- Log into the AWS Organizations management account. Systems Manager requires access to
  ...
  access and Explorer can't create OpsItems from Security Hub CSPM findings.
...
5. Select **AWS Security Hub CSPM**.
...
**Medium,Low**. 7. In the **OpsItems created by Security Hub CSPM findings** section,
...
1. Open the AWS Systems Manager console at [https://console.aws.amazon.com/systems-manager/](https://console.aws.amazon.com/systems-manager/ "https://console.aws.amazon.com/systems-manager/").
2. In the navigation pane, choose **Explorer**.
3. Find the **AWS Security Hub CSPM findings summary** widget. This
...
If the option to disable Security Hub CSPM findings is grayed out in the console, you
...
to stop receiving Security Hub CSPM findings in Explorer.
