...
Depending on the amount of data collected, the system can take several minutes to
report the data to the output you specified. After the information is collected, the
data is sent over a secure HTTPS channel to a plain-text AWS store that is accessible
only from your AWS account.

You can view the data in the Systems Manager console on the **Inventory** page,
which includes several predefined cards to help you query the data.

![Systems Manager Inventory cards in the Systems Manager console.](images/inventory-cards.png)

###### Note

Inventory cards automatically filter out Amazon EC2 managed instances with a state of
_Terminated_ and _Stopped_. For on-premises
and AWS IoT Greengrass core device managed nodes, Inventory cards automatically filter out nodes
with a state of _Terminated_.
