...
| South America (São Paulo) | sa-east-1      | aws-patch-manager-sa-east-1-ddf4b6a09             | ## Buckets containing SSM Command documents for patching operations (macOS) Buckets with the format `aws-patchmanager-macos-`region`-`unique-suffix``contain the following documents used by Patch Manager patching operations on the macOS operating system: <br>•`AWS-RunPatchBaseline`<br>•`AWS-RunPatchBaselineAssociation`<br>•`AWS-RunPatchBaselineWithHooks`<br>•`AWS-InstanceRebootWithHooks`<br>•`AWS-PatchAsgInstance`<br>•`AWS-PatchInstanceWithRollback`            |
