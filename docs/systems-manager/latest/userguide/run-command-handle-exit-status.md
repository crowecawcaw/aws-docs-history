...
In some cases, you might need to manage how your commands are handled by using exit
codes.
...
To fail the entire script at the first command failure, you can include a
shell conditional statement to exit the script if any command before the final
one fails. Use the following approach.
...
In the following example, the entire script fails if the first command
fails.
