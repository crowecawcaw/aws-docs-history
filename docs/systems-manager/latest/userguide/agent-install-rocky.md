...
The Amazon Machine Images (AMIs) for Rocky Linux that are provided by AWS do not come
with AWS Systems Manager Agent (SSM Agent) preinstalled by default. For a list of AWS
managed AMIs on which the agent might be preinstalled, see [Find AMIs with the SSM Agent preinstalled](ami-preinstalled-agent.md "ami-preinstalled-agent.md").

Use the information in this section to help you manually install or reinstall
SSM Agent on an Rocky Linux instance.
...
- [Quick installation commands for SSM Agent on Rocky Linux](#quick-install-rocky "#quick-install-rocky")
- [Create custom agent installation commands for Rocky Linux in your Region](#custom-url-rocky "#custom-url-rocky")
...
1. Connect to your Rocky Linux instance using your preferred method, such
   as SSH.
...
are the correct global installation files for Rocky Linux.
...
You can also replace a global URL in the procedure [Quick installation commands for SSM Agent on Rocky Linux](#quick-install-rocky "#quick-install-rocky")
