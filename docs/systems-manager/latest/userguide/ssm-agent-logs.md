...
1. Locate the **seelog.xml.template** file.
   ...
   **Linux**:
   ...
   On Ubuntu Server 20.04 and 18.04 LTS, the file is located in the directory `/snap/amazon-ssm-agent/current/seelog.xml.template`. Copy this file from the `/snap/amazon-ssm-agent/current/` directory to the `/etc/amazon/ssm/` directory before making any changes.
