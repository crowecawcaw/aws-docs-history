...
document `schemaVersion` must be 1.0, and the `sessionType` of
...
4. For **Name**, enter a descriptive name for the
   document.
5. For **Document type**, choose **Session
   document**.
6. Enter your document content that defines the command a user can run in
   a Session Manager session using JSON or YAML, as shown in the following
   example.
...
2. Run the following commands to create an SSM document using your
   content that defines the command a user can run in a Session Manager
   session.
...
```
$json = Get-Content -Path "C:\`path`\`to`\`file`\`documentContent`.json" | Out-String
New-SSMDocument `
    -Content $json `
    -Name "`exampleAllowedSessionDocument`" `
...
machine that you use to connect to managed nodes with the AWS CLI, the syntax you
provide for commands that contain special or escape characters might differ. The
following examples show some of the different ways you can provide command
parameters when using the AWS CLI, and how to handle special or escape
characters.

Parameters stored in Parameter Store can be referenced in the AWS CLI for your command
...
The following example shows how you can use a shorthand syntax with the AWS CLI
to pass parameters.
...
You can also provide parameters in JSON as shown in the following
example.
...
Parameters can also be stored in a JSON file and provided to the AWS CLI as
shown in the following example. For more information about using AWS CLI
parameters from a file, see [Loading AWS CLI
parameters from a file](../../../cli/latest/userguide.md "../../../cli/latest/userguide.md") in the
...
You can also generate an AWS CLI skeleton from a JSON input file as shown in the
following example. For more information about generating AWS CLI skeletons from
JSON input files, see [Generating AWS CLI
skeleton and input parameters from a JSON or YAML input file](../../../cli/latest/userguide.md "../../../cli/latest/userguide.md") in the
_AWS Command Line Interface User Guide_.
...
To escape characters inside quotation marks, you must add additional
backslashes to the escape characters as shown in the following example.
...
For information about using quotation marks with command parameters in the
AWS CLI, see [Using
...
`Session` type SSM documents.

**Allow a user to run an interactive command on a
single managed node**
...
**Allow a user to run an interactive command on all
managed nodes**
...
**Allow a user to run multiple interactive commands
