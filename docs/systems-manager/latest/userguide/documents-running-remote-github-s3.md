...
**Allowlist approved sources**: If you need to use nested document execution, restrict access to only approved sources. Use IAM policies to control `ssm:GetDocument` for SSM document sources. Use IAM and Amazon S3 bucket policies for Amazon S3 sources. Use network settings (such as VPC endpoints or security groups) for public Internet sources.
