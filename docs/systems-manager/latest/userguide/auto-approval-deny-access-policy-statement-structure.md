...
| like                            | (string, string) → Boolean                                     | Infix operator. `t like p` checks if the text<br>`t` matches the pattern `p`, which<br>may include wildcard characters `*` that match 0<br>or more of any character. To match a literal star<br>character in `t`, you can use the special escaped<br>character sequence `\*` in<br>`p`. |
