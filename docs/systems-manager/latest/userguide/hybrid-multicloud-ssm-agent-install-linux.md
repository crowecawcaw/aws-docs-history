...
You can use the [setupcli\_data\_integrity\_linux.sh](https://github.com/aws/amazon-ssm-agent/blob/mainline/Tools/src/setupcli_data_integrity_linux.sh) script to validate the signature of `ssm-setup-cli`.
   ...
   + `provider` (Optional) - Specifies the cloud provider of the machine being registered. Valid values are `Azure` and `EC2`. When specified, SSM Agent reads instance metadata from the cloud provider's Instance Metadata Service (IMDS) and associates the managed node with the source compute instance. If omitted, the default behavior is unchanged and no cloud provider metadata is collected.
...
+ **Using Snap packages**
  ...
  ```
**Important**  
The *candidate* channel in the Snap store contains the latest version of SSM Agent; not the stable channel. If you want to track SSM Agent version information on the candidate channel, run the following command on your Ubuntu Server 18.04 LTS 64-bit managed nodes.  
...
**Solution**: `DeliveryTimedOut` is the expected response code for this scenario. The command to install SSM Agent on the target node changes the node ID of the source node. Because the node ID has changed, the source node can't reply to the target node that the command failed, completed, or timed out while executing.
