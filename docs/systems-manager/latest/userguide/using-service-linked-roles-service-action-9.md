...
for Systems Manager just-in-time node access notifications

The `AWSServiceRoleForSystemsManagerNotifications` service-linked role trusts the following
...
`AWSServiceRoleForSystemsManagerNotifications` role is
`AWSSystemsManagerNotificationsServicePolicy`. For details about
the permissions it grants, see [AWS managed policy: AWSSystemsManagerNotificationsServicePolicy](security-iam-awsmanpol.md#security-iam-awsmanpol-AWSSystemsManagerNotificationsServicePolicy "security-iam-awsmanpol.md#security-iam-awsmanpol-AWSSystemsManagerNotificationsServicePolicy").
...
`AWSServiceRoleForSystemsManagerNotifications` service-linked role for
Systems Manager
...
the `AWSServiceRoleForSystemsManagerNotifications` role in your account. To learn more, see
...
**AWS service role that allows Systems Manager to send notifications to access request approvers.** use case. In the AWS CLI or the
...
## Editing the `AWSServiceRoleForSystemsManagerNotifications`

service-linked role for Systems Manager

Systems Manager doesn't allow you to edit the `AWSServiceRoleForSystemsManagerNotifications`
...
**To manually delete the `AWSServiceRoleForSystemsManagerNotifications`
service-linked role using IAM**

Use the IAM console, the AWS CLI, or the AWS API to delete the
`AWSServiceRoleForSystemsManagerNotifications` service-linked role. For more information, see
...
Systems Manager  `AWSServiceRoleForSystemsManagerNotifications` service-linked role
