...
In later steps, you choose the targets, or resources, that it updates and the tasks
that run when the maintenance window runs.
...
4. For **Name**, enter a descriptive name to help you
   identify this maintenance window.
5. (Optional) For **Description**, enter a description to
   identify how this maintenance window will be used.
6. (Optional) If you want to allow a maintenance window task to run on
   managed nodes, even if you haven't registered those nodes as targets, choose
...
If you don't choose this option, then you must choose previously
registered targets when you register a task with the maintenance
...
minus the number of hours you specify for **Stop initiating
tasks** in the next step.

For example, if the maintenance window starts at 3 PM, the duration is
three hours, and the **Stop initiating tasks** value is one
hour, no maintenance window tasks can start after 5 PM. 9. For **Stop initiating tasks**, enter the number of hours
...
You can't specify a start date and time that occurs in the
past. 11. (Optional) For **Window end date**, specify a date and
time, in ISO-8601 Extended format, for when you want the maintenance window
...
days to wait after the date and time specified by a cron or rate expression
before running the maintenance window. You can specify between one and six
days.

###### Note

This option is available only if you specified a schedule by entering
a cron or rate expression manually. 14. (Optional) In the **Manage tags** area, apply one or more
...
identify the type of tasks it runs, the types of targets, and the
