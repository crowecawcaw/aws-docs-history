...
including those for AWS Systems Manager Run Command and Automation, AWS Lambda, and AWS Step Functions.

The examples in this section use the following Systems Manager actions for updating a
maintenance window:

- [UpdateMaintenanceWindow](../APIReference/API_UpdateMaintenanceWindow.md "../APIReference/API_UpdateMaintenanceWindow.md")
- [UpdateMaintenanceWindowTarget](../APIReference/API_UpdateMaintenanceWindowTarget.md "../APIReference/API_UpdateMaintenanceWindowTarget.md")
...
1. Open the AWS CLI and run the following command to update a target to
...
2. Run the following command to use the `replace` option to
   remove the description field and add an additional target. The
   description field is removed, because the update doesn't include the
   field (a null value). Be sure to specify an additional node that has
...
Run the following command to specify a date and time range for
regularly scheduled maintenance window executions.
...
If your target is an Amazon Elastic Compute Cloud (Amazon EC2) instance for Windows Server,
change `df` to `ipconfig`, and
`AWS-RunShellScript` to
`AWS-RunPowerShellScript` in the following
command.
...
6. If you're updating a Step Functions task, adapt and run the following command
   to update its task-invocation-parameters.
...
7. Run the following command to unregister a target from a maintenance
   window. This example uses the `safe` parameter to determine
   if the target is referenced by any tasks and therefore safe to
   unregister.
...
8. Run the following command to unregister a target from a maintenance
   window even if the target is referenced by a task. You can force the
   unregister operation by using the `no-safe` parameter.
...
9. Run the following command to update a Run Command task. This example uses
   a Systems Manager Parameter Store parameter called `UpdateLevel`, which is
   formatted as follows: '`{{ssm:UpdateLevel}}`'
...
10. Run the following command to update an Automation task to specify
    `WINDOW_ID` and `WINDOW_TASK_ID` parameters
    for the `task-invocation-parameters` parameter:
