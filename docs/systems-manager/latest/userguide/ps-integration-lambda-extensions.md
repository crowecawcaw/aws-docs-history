...
| `PARAMETERS_SECRETS_EXTENSION_CACHE_ENABLED`   | Determines whether the cache for the extension is enabled. Value values: `TRUE                                                                                                                                                                                                                    | FALSE`   | No                               | TRUE                                                                                                          | FALSE                                                                                                | TRUE |
| `PARAMETERS_SECRETS_EXTENSION_CACHE_SIZE`      | The maximum size of the cache in terms of number of items. A value of 0 (zero) indicates that the cache is bypassed. This variable is ignored if both cache TTL values are 0 (zero).                                                                                                              | No       | 0 (zero) to 1000                 | 1000                                                                                                          |
| `PARAMETERS_SECRETS_EXTENSION_HTTP_PORT`       | The port for the local HTTP server.                                                                                                                                                                                                                                                               | No       | 1 - 65535                        | 2773                                                                                                          |
...
| `PARAMETERS_SECRETS_EXTENSION_LOG_LEVEL`       | The level of detail reported in logs for the extension. We recommend using `DEBUG` for the most detail about your cache configuration as you set up and test the extension. Logs for Lambda operations are automatically pushed to an associated CloudWatch Logs log group.                       | No       | `DEBUG                           | WARN                                                                                                          | ERROR                                                                                                | NONE | INFO` | `INFO` | ## Sample commands for using the AWS Systems Manager Parameter Store and AWS Secrets Manager Extension The examples in this section demonstrate API actions for use with the AWS Systems Manager Parameter Store and AWS Secrets Manager extension. ### Sample commands for Parameter Store The Lambda extension uses read-only access to the **GetParameter** API action. To call this action, make an HTTP GET call similar to the following. This command format provides access to parameters in the standard parameter tier. ``` GET http://localhost:`port`/systemsmanager/parameters/get?name=`parameter-name`&version=`version`&label=`label`&withDecryption={true | false} ``In this example, `parameter-name` represents the full parameter name, such as `MyParameter`, for a parameter not in a hierarchy, or `%2FDev%2FProduction%2FEast%2FProject-ABC%2FMyParameter` for a parameter named `/Dev/Production/East/Project-ABC/MyParameter` that is part of a hierarchy. ###### Note When using GET calls, parameter values must be encoded for HTTP to preserve special characters. For example, instead of formatting a hierarchical path like `/a/b/c`, encode characters that could be interpreted as part of the URL, such as `%2Fa%2Fb%2Fc`. `version` and `label` are the selectors available for use with the `GetParameter` action.`` GET http://localhost:`port`/systemsmanager/parameters/get/?name=MyParameter&version=5 `To call a parameter in a hierarchy, make an HTTP GET call similar to the following.` GET http://localhost:`port`/systemsmanager/parameters/get?name=%2Fa%2Fb%2F&label=release `To call a public (global) parameter, make an HTTP GET call similar to the following.` GET http://localhost:`port`/systemsmanager/parameters/get/?name=%2Faws%2Fservice%20list%2F… `To make an HTTP GET call to a Secrets Manager secret by using Parameter Store references, make an HTTP GET call similar to the following.` GET http://localhost:`port`/systemsmanager/parameters/get?name=%2Faws%2Freference%2Fsecretsmanager%2F… `To make a call using the Amazon Resource Name (ARN) for a parameter, make an HTTP GET call similar to the following.` GET http://localhost:`port`/systemsmanager/parameters/get?name=arn:aws:ssm:us-east-1:123456789012:parameter/MyParameter ``To make a call that accesses a `SecureString` parameter with decryption, make an HTTP GET call similar to the following.`` GET http://localhost:`port`/systemsmanager/parameters/get?name=MyParameter&withDecryption=true ```You can specify that parameters aren't decrypted by omitting`withDecryption`or explicitly setting it to`false`. You can also specify either a version or a label, but not both. If you do, only the first of these that is placed after question mark (`?`) in the URL is used. ## AWS Parameters and Secrets Lambda Extension ARNs The following tables provide extension ARNs for supported architectures and Regions. ###### Topics <br>• [Extension ARNs for the x86_64 and x86 architectures](#intel "#intel") <br>• [Extension ARNs for ARM64 and Mac with Apple silicon architectures](#arm64 "#arm64") ### Extension ARNs for the x86_64 and x86 architectures Last updated: October 22, 2025 |
| Region                                         | ARN                                                                                                                                                                                                                                                                                               |          | ---                              | ---                                                                                                           |
| US East (Ohio)                                 | `arn:aws:lambda:us-east-2:590474943231:layer:AWS-Parameters-and-Secrets-Lambda-Extension:25`                                                                                                                                                                                                      |          | US East (N. Virginia)            | `arn:aws:lambda:us-east-1:177933569100:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                  |
| US West (N. California)                        | `arn:aws:lambda:us-west-1:997803712105:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                                                                                                                                                                                                      |          | US West (Oregon)                 | `arn:aws:lambda:us-west-2:345057560386:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                  |
| Africa (Cape Town)                             | `arn:aws:lambda:af-south-1:317013901791:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                                                                                                                                                                                                     |          | Asia Pacific (Hong Kong)         | `arn:aws:lambda:ap-east-1:768336418462:layer:AWS-Parameters-and-Secrets-Lambda-Extension:23`                  |
| Asia Pacific (Taipei)                          | `arn:aws:lambda:ap-east-2:890742577149:layer:AWS-Parameters-and-Secrets-Lambda-Extension:9`                                                                                                                                                                                                       |          | Asia Pacific (Hyderabad) Region  | `arn:aws:lambda:ap-south-2:070087711984:layer:AWS-Parameters-and-Secrets-Lambda-Extension:18`                 |
| Asia Pacific (Jakarta)                         | `arn:aws:lambda:ap-southeast-3:490737872127:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                                                                                                                                                                                                 |          | Asia Pacific (Melbourne)         | `arn:aws:lambda:ap-southeast-4:090732460067:layer:AWS-Parameters-and-Secrets-Lambda-Extension:11`             |
| Asia Pacific (Malaysia)                        | `arn:aws:lambda:ap-southeast-5:381492012281:layer:AWS-Parameters-and-Secrets-Lambda-Extension:10`                                                                                                                                                                                                 |          | Asia Pacific (New Zealand)       | `arn:aws:lambda:ap-southeast-6:995508174458:layer:AWS-Parameters-and-Secrets-Lambda-Extension:5`              |
| Asia Pacific (Mumbai)                          | `arn:aws:lambda:ap-south-1:176022468876:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                                                                                                                                                                                                     |          | Asia Pacific (Osaka)             | `arn:aws:lambda:ap-northeast-3:576959938190:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`             |
| Asia Pacific (Seoul)                           | `arn:aws:lambda:ap-northeast-2:738900069198:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                                                                                                                                                                                                 |          | Asia Pacific (Singapore)         | `arn:aws:lambda:ap-southeast-1:044395824272:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`             |
| Asia Pacific (Sydney)                          | `arn:aws:lambda:ap-southeast-2:665172237481:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                                                                                                                                                                                                 |          | Asia Pacific (Thailand)          | `arn:aws:lambda:ap-southeast-7:941377119484:layer:AWS-Parameters-and-Secrets-Lambda-Extension:9`              |
| Asia Pacific (Tokyo)                           | `arn:aws:lambda:ap-northeast-1:133490724326:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                                                                                                                                                                                                 |          | Canada (Central)                 | `arn:aws:lambda:ca-central-1:200266452380:layer:AWS-Parameters-and-Secrets-Lambda-Extension:23`               |
| Canada West (Calgary)                          | `arn:aws:lambda:ca-west-1:243964427225:layer:AWS-Parameters-and-Secrets-Lambda-Extension:11`                                                                                                                                                                                                      |          | China (Beijing)                  | `arn:aws-cn:lambda:cn-north-1:287114880934:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`              |
| China (Ningxia)                                | `arn:aws-cn:lambda:cn-northwest-1:287310001119:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                                                                                                                                                                                              |          | Europe (Frankfurt)               | `arn:aws:lambda:eu-central-1:187925254637:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`               |
| Europe (Ireland)                               | `arn:aws:lambda:eu-west-1:015030872274:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                                                                                                                                                                                                      |          | Europe (London)                  | `arn:aws:lambda:eu-west-2:133256977650:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                  |
| Europe (Milan)                                 | `arn:aws:lambda:eu-south-1:325218067255:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                                                                                                                                                                                                     |          | Europe (Paris)                   | `arn:aws:lambda:eu-west-3:780235371811:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                  |
| Europe (Spain) Region                          | `arn:aws:lambda:eu-south-2:524103009944:layer:AWS-Parameters-and-Secrets-Lambda-Extension:20`                                                                                                                                                                                                     |          | Europe (Stockholm)               | `arn:aws:lambda:eu-north-1:427196147048:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                 |
| Israel (Tel Aviv)                              | `arn:aws:lambda:il-central-1:148806536434:layer:AWS-Parameters-and-Secrets-Lambda-Extension:11`                                                                                                                                                                                                   |          | Europe (Zurich) Region           | `arn:aws:lambda:eu-central-2:772501565639:layer:AWS-Parameters-and-Secrets-Lambda-Extension:18`               |
| Mexico (Central) Region                        | `arn:aws:lambda:mx-central-1:241533131596:layer:AWS-Parameters-and-Secrets-Lambda-Extension:8`                                                                                                                                                                                                    |          | Middle East (Bahrain)            | `arn:aws:lambda:me-south-1:832021897121:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                 |
| Middle East (UAE)                              | `arn:aws:lambda:me-central-1:858974508948:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                                                                                                                                                                                                   |          | South America (São Paulo)        | `arn:aws:lambda:sa-east-1:933737806257:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                  |
| AWS GovCloud (US-East)                         | `arn:aws-us-gov:lambda:us-gov-east-1:129776340158:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`                                                                                                                                                                                           |          | AWS GovCloud (US-West)           | `arn:aws-us-gov:lambda:us-gov-west-1:127562683043:layer:AWS-Parameters-and-Secrets-Lambda-Extension:21`       | ### Extension ARNs for ARM64 and Mac with Apple silicon architectures Last updated: October 22, 2025 |
| Region                                         | ARN                                                                                                                                                                                                                                                                                               |          | ---                              | ---                                                                                                           |
| US East (Ohio)                                 | `arn:aws:lambda:us-east-2:590474943231:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:25`                                                                                                                                                                                                |          | US East (N. Virginia)            | `arn:aws:lambda:us-east-1:177933569100:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:21`            |
| US West (N. California) Region                 | `arn:aws:lambda:us-west-1:997803712105:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:18`                                                                                                                                                                                                |          | US West (Oregon)                 | `arn:aws:lambda:us-west-2:345057560386:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:21`            |
| Africa (Cape Town) Region                      | `arn:aws:lambda:af-south-1:317013901791:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:18`                                                                                                                                                                                               |          | Asia Pacific (Hong Kong) Region  | `arn:aws:lambda:ap-east-1:768336418462:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:20`            |
| Asia Pacific (Taipei)                          | `arn:aws:lambda:ap-east-2:890742577149:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:5`                                                                                                                                                                                                 |          | Asia Pacific (Hyderabad) Region  | `arn:aws:lambda:ap-south-2:070087711984:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:10`           |
| Asia Pacific (Jakarta) Region                  | `arn:aws:lambda:ap-southeast-3:490737872127:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:18`                                                                                                                                                                                           |          | Asia Pacific (Melbourne)         | `arn:aws:lambda:ap-southeast-4:090732460067:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:10`       |
| Asia Pacific (Malaysia)                        | `arn:aws:lambda:ap-southeast-5:381492012281:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:10`                                                                                                                                                                                           |          | Asia Pacific (New Zealand)       | `arn:aws:lambda:ap-southeast-6:995508174458:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:3`        |
| Asia Pacific (Mumbai)                          | `arn:aws:lambda:ap-south-1:176022468876:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:21`                                                                                                                                                                                               |          | Asia Pacific (Osaka)             | `arn:aws:lambda:ap-northeast-3:576959938190:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:18`       |
| Asia Pacific (Seoul) Region                    | `arn:aws:lambda:ap-northeast-2:738900069198:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:18`                                                                                                                                                                                           |          | Asia Pacific (Singapore)         | `arn:aws:lambda:ap-southeast-1:044395824272:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:21`       |
| Asia Pacific (Sydney)                          | `arn:aws:lambda:ap-southeast-2:665172237481:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:21`                                                                                                                                                                                           |          | Asia Pacific (Thailand)          | `arn:aws:lambda:ap-southeast-7:941377119484:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:8`        |
| Asia Pacific (Tokyo)                           | `arn:aws:lambda:ap-northeast-1:133490724326:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:21`                                                                                                                                                                                           |          | Canada (Central) Region          | `arn:aws:lambda:ca-central-1:200266452380:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:20`         |
| Canada West (Calgary)                          | `arn:aws:lambda:ca-west-1:243964427225:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:10`                                                                                                                                                                                                |          | China (Beijing)                  | `arn:aws-cn:lambda:cn-north-1:287114880934:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:10`        |
| China (Ningxia)                                | `arn:aws-cn:lambda:cn-northwest-1:287310001119:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:10`                                                                                                                                                                                        |          | Europe (Frankfurt)               | `arn:aws:lambda:eu-central-1:187925254637:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:21`         |
| Europe (Ireland)                               | `arn:aws:lambda:eu-west-1:015030872274:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:21`                                                                                                                                                                                                |          | Europe (London)                  | `arn:aws:lambda:eu-west-2:133256977650:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:21`            |
| Europe (Milan) Region                          | `arn:aws:lambda:eu-south-1:325218067255:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:18`                                                                                                                                                                                               |          | Europe (Paris) Region            | `arn:aws:lambda:eu-west-3:780235371811:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:18`            |
| Europe (Spain) Region                          | `arn:aws:lambda:eu-south-2:524103009944:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:12`                                                                                                                                                                                               |          | Europe (Stockholm) Region        | `arn:aws:lambda:eu-north-1:427196147048:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:18`           |
| Israel (Tel Aviv)                              | `arn:aws:lambda:il-central-1:148806536434:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:10`                                                                                                                                                                                             |          | Europe (Zurich) Region           | `arn:aws:lambda:eu-central-2:772501565639:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:10`         |
| Mexico (Central) Region                        | `arn:aws:lambda:mx-central-1:241533131596:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:7`                                                                                                                                                                                              |          | Middle East (Bahrain) Region     | `arn:aws:lambda:me-south-1:832021897121:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:18`           |
| Middle East (UAE)                              | `arn:aws:lambda:me-central-1:858974508948:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:10`                                                                                                                                                                                             |          | South America (São Paulo) Region | `arn:aws:lambda:sa-east-1:933737806257:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:18`            |
| AWS GovCloud (US-East)                         | `arn:aws-us-gov:lambda:us-gov-east-1:129776340158:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:10`                                                                                                                                                                                     |          | AWS GovCloud (US-West)           | `arn:aws-us-gov:lambda:us-gov-west-1:127562683043:layer:AWS-Parameters-and-Secrets-Lambda-Extension-Arm64:10` |
