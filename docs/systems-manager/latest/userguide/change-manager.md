...
In addition to restricting the changes that can be made when a runbook workflow runs, Change Manager helps you control concurrency and error thresholds. You choose how many resources a runbook workflow can run on at once and how many accounts it can run in simultaneously. You also set how many failures to allow before the process stops and, if the runbook includes a rollback script, rolls back. You can monitor progress by using CloudWatch alarms.
