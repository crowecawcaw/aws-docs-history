...
If you need to create a service role for Automation complete the following tasks. For more information about when a service role is required for Automation, see [Setting up Automation](automation-setup.md).
...
1. On the **Roles** page, choose the role you just created to open the **Summary** page. Note the **Role Name** and **Role ARN**. Specify the role ARN when you attach the **iam:PassRole** policy to your IAM account in the next procedure. You can also specify the role name and the ARN in runbooks.
