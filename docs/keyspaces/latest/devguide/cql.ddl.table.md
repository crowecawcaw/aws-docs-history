...
and the available options, see [Configure automatic scaling on an existing table](autoscaling.configureTable.md "autoscaling.configureTable.md").
...
- `*column\_definition*` consists of the
  ...
  - `*cql\_type*` – An Amazon Keyspaces
    data type (see [Data types](cql.elements.md#cql.data-types "cql.elements.md#cql.data-types")).
  - `*FROZEN*` – Designates
    ...
    `BLOB`. For more information, see [Collection types](cql.elements.md#cql.data-types.collection "cql.elements.md#cql.data-types.collection").
  - `*STATIC*` –
...
- `*table\_options*` consist of the
  ...
  - `*AUTOSCALING\_SETTINGS*` includes the optional auto scaling
    ...
    examples, see [Configure automatic scaling on an existing table](autoscaling.configureTable.md "autoscaling.configureTable.md").
  - _`cdc`_ – A boolean that specifies if Amazon Keyspaces
    creates a change data capture (CDC) stream for the table. The default is `false`.
    To specify the `view type` when enabling a stream, set the `cdc_specification` with `CUSTOM_PROPERTIES` .
  - _`CUSTOM_PROPERTIES`_ – A
