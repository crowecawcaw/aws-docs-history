...
4. Choose **Create UDT**
5. Under **UDT details**, enter the name for the UDT. Under **UDT fields** you
   define the schema of the UDT.
...
2. Create the first type. This type stores _BISAC_
   codes, which are used to define the genre of books. A BISAC code
   consists out of an alpha-numeric code and up to four subject matter
   areas.
...
4. Create a table with a column for the author's name and uses a list
   type for the book awards. Note that the UDT used in the list has to
   be frozen.
...
2. The output of that command looks similar to this example. Note that `typeName`
   returns the formatted name of the UDT.
