...
In this step, you create a dual-stack VPC endpoint for Amazon Keyspaces using the AWS CLI. To create the VPC
endpoint using the VPC console, you can follow the [Create a VPC endpoint](../../../vpc/latest/privatelink/create-interface-endpoint.md#create-interface-endpoint "../../../vpc/latest/privatelink/create-interface-endpoint.md#create-interface-endpoint") instructions in the _AWS PrivateLink Guide_. When filtering for the **Service
...
The output shows a list of Amazon Keyspaces tables that are contained in the specified
...
2. Verify that Amazon Keyspaces is an available service for creating VPC endpoints in the
   current AWS Region. (The command is shown in bold text, followed by example
   output.)
...
If Amazon Keyspaces is one of the available services in the output of the command, you can
proceed with creating a VPC endpoint. 3. To connect to Amazon Keyspaces using IPv6 enabled dual-stack endpoints, confirm that your VPC supports IPv6 and configure subnets
with IPv6 support. To add IPv6 support to an existing VPC that currently only supports IPv4, see [IPv6 support for your VPC](../../../vpc/latest/userguide.md "../../../vpc/latest/userguide.md") in the _Amazon VPC User Guide;_. 4. Determine your VPC identifier.
...
In the example output, the VPC ID is `vpc-a1234bcd`. 5. Use a filter to gather details about the subnets of the VPC.
...
`subnet-70b24b16` and `subnet-c63ffbe7`. 6. Create the VPC endpoint. For the `--vpc-id` parameter, specify the
VPC ID from the previous step. For the `--subnet-ids` parameter,
specify the subnet IDs from the previous step. Use the
`--vpc-endpoint-type` parameter to define the endpoint as an
interface. To create a dual-stack endpoint, use `--ip-address-type dualstack`.
For more information about the command, see [`create-vpc-endpoint`](../../../cli/latest/reference/ec2/create-vpc-endpoint.md "../../../cli/latest/reference/ec2/create-vpc-endpoint.md") in the _AWS CLI Command
Reference_.
