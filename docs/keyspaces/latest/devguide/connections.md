...
![A diagram showing how a connection that is established by a driver first reaches the endpoint of the Amazon Keyspaces service, then continues to a load balancer, and after authentication and authorization the CQL request reaches the storage layer.](http://docs.aws.amazon.com/keyspaces/latest/devguide/images/keyspaces_bp-connections.png)
...
![A screenshot that shows metrics on a Cloudwatch dashboard for connections going through a VPC endpoint. The metrics used are ActiveConnections and BytesProcessed.](http://docs.aws.amazon.com/keyspaces/latest/devguide/images/keyspaces_bp_cw.png)
