...
appropriate `cqlsh` parameter values and required table settings to ensure that
the data upload is successful.

###### Randomize the data

The `cqlsh COPY FROM` command reads and writes data in the same order
that it appears in the CSV file. If you use the `cqlsh COPY TO` command
