...
`INSERT`, `UPDATE`, and `DELETE`. The following topics
outline some of the more complex options available when working with queries. For the
complete language syntax with examples, see [DML statements (data manipulation language) in Amazon Keyspaces](cql.dml.md "cql.dml.md").

###### Topics

- [Use the IN operator with the SELECT statement in a query in Amazon Keyspaces](in.select.md "in.select.md")
