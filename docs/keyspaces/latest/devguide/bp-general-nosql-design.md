...
NoSQL database systems like Amazon Keyspaces use alternative models for data management, such as
key-value pairs or document storage. When you switch from a relational database management
system to a NoSQL database system like Amazon Keyspaces, it's important to understand the key differences
...
- In RDBMS, data can be queried flexibly, but queries are relatively
  expensive and don't scale well in high-traffic situations (see [Data modeling best practices: recommendations for designing data models](data-modeling.md "data-modeling.md")).
- In a NoSQL database such as Amazon Keyspaces, data can be queried efficiently in a limited
...
- In Amazon Keyspaces, you design your schema specifically to make the most common and important
...
- By contrast, you shouldn't start designing your schema for Amazon Keyspaces until you know the
  questions it needs to answer. Understanding the business problems and the
...
- You should maintain as few tables as possible in an Amazon Keyspaces application. Having fewer
  ...
  overhead for your Amazon Keyspaces application. It can also help keep backup costs lower overall.

## Approaching NoSQL design

_The first step in designing your Amazon Keyspaces application is to identify the specific
query patterns that the system must satisfy._
...
- **Data size**: Knowing how much data will be stored and
  requested at one time helps to determine the most effective way to partition the
  data.
...
- **Data velocity**: Amazon Keyspaces scales by increasing the
...
- **Keep related data together.**   Research on
  routing-table optimization 20 years ago found that "locality of reference" was the
  single most important factor in speeding up response time: keeping related data together
  in one place. This is equally true in NoSQL systems today, where keeping related data in
  close proximity has a major impact on cost and performance. Instead of distributing
  related data items across multiple tables, you should keep related items in your NoSQL
...
model data efficiently in Amazon Keyspaces.
