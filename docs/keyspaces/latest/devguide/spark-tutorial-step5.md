...
You can confirm the schema of the data in the `DataFrame` as shown
...
4. Use the following command to write the data in the `DataFrame` to
   the Amazon Keyspaces table.

```
df.write.cassandraFormat("book_awards", "catalog").mode("APPEND").save()
```

5. To confirm that the data was saved, you can read it back to a dataframe, as
