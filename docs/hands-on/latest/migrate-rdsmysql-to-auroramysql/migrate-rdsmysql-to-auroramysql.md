...
![GitHub repository page showing file list, branch set to master, and a "Code" dropdown with options to clone or download as ZIP.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/github-repository-page-file-list-set.png)
...
![AWS CloudFormation interface with a "Create stack" button to model and provision cloud infrastructure.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/cfnlong-interface-stack-button-model.png)
...
![AWS CloudFormation "Create stack" interface showing step 1, where a template is prepared and specified by uploading a file named "CloudFormation.yaml.".](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/cfnlong-stack-interface-where-template.png)
...
![AWS CloudFormation "Specify stack details" page with fields for stack name, parameters like DBInstanceID, DatabaseName, KeyName, and VPC, and a "Next" button highlighted.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/cfnlong-specify-stack-details-page-fields.png)
...
![AWS CloudFormation interface showing a stack named "rds2aurora" with status "CREATE_IN_PROGRESS" and event details.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/cfnlong-interface-stack-named-aurora.png)

1. (Optional) Refresh status

   You can click on the refresh icon to see the progress of resources creation.   
![A table of events showing timestamps, logical IDs, and statuses, with statuses including "CREATE_COMPLETE" and "CREATE_IN_PROGRESS.".](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/table-events-timestamps-logical-ids.png)
...
![AWS CloudFormation console showing a stack named "rds2aurora" with a status of "CREATE_COMPLETE" and outputs including an RDS DB instance endpoint and a webserver URL.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/cfnlong-console-stack-named-aurora-status.png)
...
![Amazon Web Services RDS load test interface showing instance ID, availability zone, and current CPU load at 1%.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/load-test-interface-instance-availability.png)
...
![Amazon Web Services interface showing database connection fields for endpoint, database name, username, and password with a "Submit" button.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/interface-database-connection-fields.png)
...
![Amazon Web Services page showing a command execution for MySQL and a message about writing configuration to rds.conf.php, with a redirect notice.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/page-command-execution-message-writing.png)
...
![A web interface displaying an address book with entries for Alice and Bob, including their phone numbers, email addresses, and options to edit or remove each contact.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/interface-displaying-address-book-entries.png)
...
![A web-based address book interface hosted on Amazon Web Services, displaying names, phone numbers, emails, and options to edit or remove contacts.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/based-address-book-interface-hosted.png)
...
![Amazon EC2 dashboard showing a running t2.micro instance named "WebServer" in the us-west-2c availability zone with its public and private IP details.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/dashboard-running-micro-instance-named.png)
...
![EC2 Instance Connect interface showing connection method options, with 'EC2 Instance Connect' selected and a 'Connect' button highlighted.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/instance-connect-interface-connection.png)
...
![Terminal window showing navigation to the `/var/www/html` directory on an Amazon Linux 2 instance, listing files including `rds.conf.php`, and displaying database connection details in the `rds.conf.php` file.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/terminal-window-navigation-var-directory.png)
...
![AWS RDS interface showing a database named "rdsdb" with the "Actions" dropdown menu expanded, highlighting the "Create Aurora read replica" option.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/interface-database-named-actions-dropdown.png)
...
![The AWS console showing the "Create Aurora read replica" page with options for DB engine, instance class, Multi-AZ deployment, and a DB instance identifier set to "aurora-db".](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/console-aur-read-replica-page-options.png)

1. Create read replica

   Choose **Create read replica**   
![Database settings interface showing IAM role, log recommendations, maintenance options, and a 'Create read replica' button.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/database-settings-interface-iam-role-log.png)
...
![AWS RDS console showing a list of databases, including an Aurora MySQL cluster with two instances, both marked as "Available.".](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/console-list-databases-including-aur.png)
...
![AWS RDS dashboard showing the configuration details of an Aurora DB cluster, including DB cluster role as Replica, replication source ARN, and availability settings.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/dashboard-configuration-details-aur.png)
...
![AWS RDS dashboard showing details of an Aurora DB cluster, including roles (Regional, Writer, Reader), endpoints, and connectivity & security settings.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/dashboard-details-aur-cluster-including.png)
...
![AWS RDS console showing the "Parameter groups" section with a selected custom parameter group and the "Edit" option highlighted in the dropdown menu.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/console-parameter-groups-section-selected.png)
...
![Amazon RDS parameter group interface showing a search for "read_only" with editable values and allowed options for parameters.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/parameter-group-interface-search-read-only.png)
...
![Amazon RDS interface showing parameter group settings with "read_only" parameter set to 1 and highlighted "Save changes" button.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/interface-parameter-group-settings-read.png)
...
![AWS RDS dashboard showing details of an Aurora DB cluster with monitoring tab selected and a CloudWatch graph displaying Aurora Binlog Replica Lag in seconds.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/dashboard-details-aur-cluster-monitoring.png)
...
![Terminal screenshot showing a MySQL "show slave status\G" command output, highlighting "Seconds Behind Master: 0" and the connection to an Amazon RDS Aurora database.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/terminal-screenshot-show-slave-status.png)
...
![Amazon RDS dashboard showing a list of databases, with 'aurora-db-cluster' selected and the 'Promote' option highlighted in the Actions dropdown menu.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/dashboard-list-databases-aurora-cluster.png)

1. Confirm promotion

   Choose **Promote Read Replica**.   
![AWS interface showing the option to promote a read replica of "aurora-db-cluster" to a primary instance, with a warning about replication stopping during the process and buttons for "Cancel" and "Promote Read Replica.".](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/interface-option-promote-read-replica.png)
...
![AWS RDS dashboard showing details of an Aurora MySQL database cluster with instances listed as available.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/dashboard-details-aur-database-cluster.png)
...
![AWS RDS console showing no auto-scaling policies or activities, with one recent event indicating a Read Replica cluster was promoted to a standalone database cluster.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/console-auto-scaling-policies-activities.png)
...
![Amazon RDS dashboard showing the configuration details of an Aurora DB cluster, with the 'Master' role highlighted under the DB cluster role.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/dashboard-configuration-details-aur-1.png)
...
![Amazon RDS dashboard showing a list of databases with identifiers, roles, engines, and regions, highlighting "rdsdb" as an instance.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/dashboard-list-databases-identifiers-roles.png)
...
![Amazon RDS interface showing a list of databases with the "Actions" dropdown menu expanded, highlighting the "Stop" option for the selected database "rdsdb".](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/interface-list-databases-actions-dropdown.png)
...
![Confirmation dialog to stop a database instance with options to create a snapshot and a warning about automatic restart after seven days, featuring 'Cancel' and 'Yes, Stop Now' buttons.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/confirmation-dialog-stop-database-instance.png)

1. Verify the stopped state

   You will see the RDS DB instance **rdsdb** is now in **Stopped** state.   
![AWS RDS console showing a list of databases, with one MySQL instance named "rdsdb" highlighted and its status marked as "Stopped.".](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/console-list-databases-one-instance-named.png)
...
![Terminal interface showing an Amazon Linux 2 AMI session, navigating to the /var/www/html directory and editing the rds.conf.php file using the vi editor.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/terminal-interface-linux-ami-session.png)
...
![A code snippet displayed in an Amazon EC2 Instance Connect terminal, showing PHP variables for an RDS database connection including URL, database name, username, and password.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/code-snippet-displayed-instance-connect-1.png)
...
![A web-based address book interface hosted on Amazon Web Services, displaying a table with names, phone numbers, emails, and options to edit or remove entries.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/based-address-book-interface-hosted-1.png)
...
![A web-based address book interface hosted on Amazon Web Services, displaying a table of names, phone numbers, emails, and options to edit or remove entries.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/based-address-book-interface-hosted-2.png)
...
![AWS CloudFormation interface showing a stack named "rds2aurora" with status "CREATE_COMPLETE" and a highlighted "Delete" button.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/cfnlong-interface-stack-named-aurora-1.png)

1. Delete stack

   Click **Delete** stack and check the CloudFormation console to ensure the stack that you selected is deleted.   
![Confirmation dialog in AWS CloudFormation asking to delete the "rds2aurora" stack, with options to cancel or delete the stack.](https://docs.aws.amazon.com/hands-on/latest/migrate-rdsmysql-to-auroramysql/images/confirmation-dialog-cfnlong-asking-delete.png)
