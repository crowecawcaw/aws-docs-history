...
tries to use the console to view details about a sampling rule but
does not have `xray:GetSamplingRules`
permissions.
...
access the sampling rule resource using the
...
X-Ray. However, the action requires the service to have permissions that are granted by a service role. Mary does not have permissions to pass the
role to the service.
...
a policy to the entity that grants them the correct permissions in X-Ray. After the permissions are granted, provide the credentials to the user
