...
See [Instrumenting your application](xray-instrumenting-your-app.md "xray-instrumenting-your-app.md") for more information.

AWS services that are [integrated with X-Ray](xray-services.md "xray-services.md") can add tracing headers to
...
![How the X-Ray SDK works](images/xray-how-it-works.png)
Instead of sending trace data directly to X-Ray, each client SDK sends JSON segment documents to a
daemon process listening for UDP traffic. The [X-Ray daemon](xray-daemon.md "xray-daemon.md") buffers segments in a queue and uploads them to X-Ray in
...
a detailed _trace map_. The trace map shows the client,
