...
If you use Rails, use the Rails middleware to instrument incoming HTTP requests. When you
add the middleware to your application and configure a segment name, the X-Ray SDK for Ruby creates
a segment for each sampled request. Any segments created by additional instrumentation become
...
The middleware creates a segment for each incoming request with an `http` block
...
To use the middleware, update your gemfile to include the required [railtie](http://api.rubyonrails.org/classes/Rails/Railtie.html "http://api.rubyonrails.org/classes/Rails/Railtie.html").
...
If you don't use Rails, create segments manually. You can create a segment for each
incoming request, or create segments around patched HTTP or AWS SDK clients to provide context
for the recorder to add subsegments.
...
you configure the recorder, as shown in the [previous sections](#xray-sdk-ruby-middleware-rails "#xray-sdk-ruby-middleware-rails").
...
dynamically, specify a naming pattern in the config hash.
...
You can use '\*' in the pattern to match any string, or '?' to match any single
character.
...
[environment variable](xray-sdk-ruby-configuration.md#xray-sdk-ruby-configuration-envvars "xray-sdk-ruby-configuration.md#xray-sdk-ruby-configuration-envvars").
