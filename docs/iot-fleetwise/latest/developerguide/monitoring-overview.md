...
performance of AWS IoT FleetWise and your other AWS solutions. AWS provides the following monitoring
tools to watch AWS IoT FleetWise, report when something is wrong, and take automatic actions when
appropriate:
...
- _Amazon CloudWatch_ monitors your AWS resources and the applications that
  ...
  dashboards, and set alarms that notify you or take actions when a metric reaches a threshold
...
- _Amazon CloudWatch Logs_ can be used to monitor, store, and access your log files
...
- _AWS CloudTrail_ captures API calls and related events made by or on behalf
  of your AWS account. Then, it delivers the log files to an Amazon S3 bucket that you specify.
  You can identify which users and accounts called AWS, the source IP address from which the
