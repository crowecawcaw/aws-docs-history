...
| `IN_PROGRESS` | When the vehicle starts executing the command, it can publish<br>a message to the response topic to update the status to<br>`IN_PROGRESS`. | Device | No | • IN\_PROGRESS<br>• SUCCEEDED<br>• FAILED<br>• REJECTED<br>• TIMED\_OUT | `COMMAND_STATUS_COMMAND_IN_PROGRESS` |
...
response received from the device within the specified duration, the cloud
reports a `TIMED_OUT` status. In this case, the command execution in
`TIMED_OUT` status is non-terminal.

The device can override this status to a terminal status, such as
`SUCCEEDED`, `FAILED`, or `REJECTED`. It
can also report that a timeout occurred when running the command. In this case,
the command execution status stays at `TIMED_OUT` but the fields of
the `StatusReason` object are updated based on the information
reported by the device. The command execution in the `TIMED_OUT`
status now becomes terminal.

For additional information, see [Command execution timeout considerations](../../../iot/latest/developerguide/iot-remote-command-execution-start-monitor.md#iot-command-execution-timeout "../../../iot/latest/developerguide/iot-remote-command-execution-start-monitor.md#iot-command-execution-timeout") in the _AWS IoT Core
developer guide_.
