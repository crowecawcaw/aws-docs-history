...
a device along the path, the receiving host or device drops the packet. It then returns the
following ICMP message: `Destination Unreachable:
...
packet that's larger than the MTU of the receiving host or a device along the path, the
receiving host or device drops the packet. It then returns the following ICMP message:
`ICMPv6 Packet Too Big (PTB)` (Type 2).
