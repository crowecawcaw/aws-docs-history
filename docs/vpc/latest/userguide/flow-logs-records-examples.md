...
For information about flow log record format, see [Flow log records](flow-log-records.md "flow-log-records.md"). For information about how to create flow logs, see
[Work with flow logs](working-with-flow-logs.md "working-with-flow-logs.md").
...
In this example, SSH traffic (destination port 22, TCP protocol) from IP address
172.31.16.139 to network interface with private IP address is 172.31.16.21 and ID
...
Security groups are stateful — this means that responses to allowed traffic
...
For example, you use the **ping** command from your home computer
...
For more information about the tcp-flags option and an explanation of each of the TCP flags, see [Available fields](flow-log-records.md#flow-logs-fields "flow-log-records.md#flow-logs-fields").
...
during the previous flow is now closed. The server sent a FIN flag (1)
to the client for the connection on port 43418. The client responded with a FIN
to the server on port 43418.
