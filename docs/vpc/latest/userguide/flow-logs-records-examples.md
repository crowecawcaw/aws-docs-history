...
You create another custom flow log using the same set of fields as above. You create the flow log for the network interface for the instance in the private subnet. In this case, the instance-id field returns the ID of the instance that's associated with the network interface. There is no difference between the dstaddr and pkt-dstaddr fields or the srcaddr and pkt-srcaddr fields. Unlike the network interface for the zonal NAT gateway, this network interface is not an intermediate network interface for traffic.
...
The next two lines show the traffic from the regional NAT gateway (public IP address 107.22.182.139) to the target host on the internet. They also show the response traffic from the host to the regional NAT gateway.
