...
<a name="network-access-analyzer-troubleshooting"></a>

The following error messages are returned by Network Access Analyzer:

**The request failed due to insufficient permissions**  
Verify that you have the required permissions. For more information, see [Required API permissions for Network Access Analyzer](security_iam_required-API-permissions.md).

**The network configuration is not supported**  
Verify that you are using resources that are supported by Network Access Analyzer. For more information, see [Supported path resources](how-network-access-analyzer-works.md#path-components).
