...
When you create a Active Directory with Simple AD, AWS Directory Service performs the following tasks on your
behalf:
...
retrieved. However, you can reset a password from the AWS Directory Service console or by using the
...
- Creates an account with the name `AWSAdminD-`xxxxxxxx`` that has domain admin
  privileges. This account is used by AWS Directory Service to perform automated operations for directory
