...
A two-way trust is required for AWS Enterprise Apps such as Amazon Chime, Connect Customer, Quick, AWS IAM Identity Center, WorkDocs, Amazon WorkMail, Amazon WorkSpaces, and the AWS Management Console. AWS Managed Microsoft AD must be able to query the users and groups in your self-managed Active Directory.
...
1. For **Conditional forwarder**, type the IP address of your self-managed DNS server (for example, 172.16.10.153 for IPv4 or 2001:db8::1 for IPv6). If you have previously created conditional forwarders, you can type the FQDN of your self-managed domain instead of a DNS IP address.

1. (Optional) Choose **Add another IP address** and type the IP address of an additional self-managed DNS server. You can repeat this step for each applicable DNS server address for a total of four addresses. Both IPv4 and IPv6 addresses are supported.
