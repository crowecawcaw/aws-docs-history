...
2. Train a multi-variate time series anomaly detection model on the data.
...
have its own unique data pattern. All detectable failures are preceded by behavior or
conditions that fall out of the normal behavior of the equipment, so Lookout for Equipment is designed to
look for those behaviors or conditions.

If you have the data available, you can highlight abnormal equipment behavior using
"labels." The trained model can use the anomalous behavior in the dataset to improve its
performance.
