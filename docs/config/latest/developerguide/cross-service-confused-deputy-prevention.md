...
policies to limit the permissions that AWS Config gives another service to the resource.
...
`aws:SourceAccount` global condition context keys in AWS Config to prevent the
confused deputy problem: [Granting AWS Config access
to the Amazon S3 Bucket](s3-bucket-policy.md "s3-bucket-policy.md").
