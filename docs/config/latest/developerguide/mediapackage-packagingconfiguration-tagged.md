...
Checks if AWS Elemental MediaPackage packaging configuration resources have tags. The rule is NON_COMPLIANT if there are no tags or if the specified tag keys are not present. The rule does not check for tags starting with 'aws:'.
