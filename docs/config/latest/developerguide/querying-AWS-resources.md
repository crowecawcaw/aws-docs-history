...
+ [Supported Resource Types](#supported-resources)
...
The first condition `configuration.configRuleList.complianceType = 'non_compliant'` is applied to ALL elements in R.configRuleList, because R has a rule (rule B) with complianceType = 'non\_compliant', the condition is evaluated as true.
...
Not all resource types that AWS Config supports are queryable in Advanced Query. Additionally, some fields in a configuration item are unsearchable. Unsearchable fields may be returned in results and used in the `SELECT` portion of the query, but cannot be used in other parts of the query, such as `WHERE`, `GROUP BY`, or `ORDER BY`.

For a full list of searchable fields and resource types, see [Supported Resource Types for Advanced Queries](https://github.com/awslabs/aws-config-resource-schema/tree/master/config/properties/resource-types).
