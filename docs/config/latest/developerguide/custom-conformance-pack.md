...
1. Replace `{{NameOfRuleParameter}}` with the same name you used in the `Parameters` section. After the colon, copy and paste the same concatenated string of `!Ref` \+ {{NameOfRule}} \+ Param \+ {{NameOfRuleParameter}} that you created in `Parameters` section.
