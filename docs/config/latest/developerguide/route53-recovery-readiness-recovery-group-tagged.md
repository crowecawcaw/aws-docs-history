...
Checks if Amazon Route 53 Recovery Readiness recovery groups have tags. The rule is NON\_COMPLIANT if there are no tags or if the specified tag keys are not present. The rule does not check for tags starting with 'aws:'. 



**Identifier:** ROUTE53\_RECOVERY\_READINESS\_RECOVERY\_GROUP\_TAGGED
