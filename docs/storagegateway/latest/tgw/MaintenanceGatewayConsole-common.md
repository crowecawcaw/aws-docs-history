...
| sslcheck             | Returns output with certificate issuerNoteStorage Gateway uses certificate issuer verification and does<br>not support ssl inspection. If this command returns an<br>issuer other than aws-appliance@amazon.com, then it is<br>likely that an application performing an ssl inspection.<br>In that case, we recommend bypassing ssl inspection for<br>the Storage Gateway appliance.        |
