...
![an upload buffer formula based on application and network throughput compression and write duration.](https://docs.aws.amazon.com/storagegateway/latest/vgw/images/WorkingStorageFormula-diagram.png)
...
You can initially use this approximation to determine the disk size that you want to allocate to the gateway as upload buffer space. Add more upload buffer space as needed using the Storage Gateway console. Also, you can use the Amazon CloudWatch operational metrics to monitor upload buffer usage and determine additional storage requirements. For information on metrics and setting the alarms, see [Monitoring the upload buffer](PerfUploadBuffer-common.md).
