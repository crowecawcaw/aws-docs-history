...
Volume Gateway AMIs use the following naming convention. The version
...
1. Start setting up a new gateway using the Storage Gateway console. For instructions, see
   [Set up a Volume Gateway](create-volume-gateway.md#set-up-gateway-volume "create-volume-gateway.md#set-up-gateway-volume"). When you reach the **Platform
   options** section, choose **Amazon EC2** as the
   **Host platform**, then use the following steps to launch the
   Amazon EC2 instance that will host your Volume Gateway.

###### Note

The Amazon EC2 host platform supports **Cached volumes** only.
Stored volume gateways cannot be deployed on EC2 instances. 2. Choose **Launch instance** to open the AWS Storage Gateway AMI template
...
cause problems when you start or stop Volume Gateway; for
...
8. In the **Inbound security groups rules** subsection, add firewall
   ...
   information on the ports required for Volume Gateway, see
   [Port
...
Volume Gateway requires TCP port 80 to be open for inbound traffic and for
...
resume setting up your Volume Gateway.
You can determine the AMI ID to use for launching a Volume Gateway by using the
...
- Query the Systems Manager parameter store. You can use the AWS CLI or
  ...
  `/aws/service/storagegateway/ami/CACHED/latest` for Cached
  Volume Gateways or `/aws/service/storagegateway/ami/STORED/latest` for
  Stored Volume Gateways. For example, using the following CLI command returns the ID
  of the current AMI in the AWS Region you specify.
