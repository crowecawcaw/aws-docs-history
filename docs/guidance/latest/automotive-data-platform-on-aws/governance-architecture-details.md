...
2. **EU Producer Region**: Vehicle data collection, PII classification, anonymization processing, and dual data stores (PII + anonymized)
3. **Global Consumer Regions**: R&D analytics access to anonymized data only through Lake Formation resource links
4. **Governance Layer**: Cross-region policies enforced through Lake Formation resource shares and IAM roles
5. **Audit Layer**: Centralized CloudTrail logging with S3 Object Lock for tamper-proof audit trails
