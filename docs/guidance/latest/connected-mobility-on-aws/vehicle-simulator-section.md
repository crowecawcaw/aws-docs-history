...
![Cloud-Native Simulator for Dynamic Vehicle Data Collection](images/cloud-native-simulator-dynamic-data-collection.png)
...
2. **API Gateway → Simulation Lambda** — The request hits the Simulation API Lambda, which acts as a thin orchestrator. It validates the configuration, generates a simulation ID, and launches ECS tasks.
3. **ECS task launch** — The Lambda calls `ecs:RunTask` to start worker containers. In MQTT Direct mode, this is a single Fargate task. In FWE mode, this is two separate EC2-backed tasks on the same host (explained below).
4. **State tracking** — The Lambda records the simulation ID, task ARN, and configuration in the simulations DynamoDB table with a 24-hour TTL.
...
2. The container starts the [AWS IoT FleetWise Edge Agent](https://github.com/aws/aws-iot-fleetwise-edge "https://github.com/aws/aws-iot-fleetwise-edge") (open source, Apache 2.0 license). The solution uses a custom-built container image based on the official source.
3. At startup, the agent writes the vehicle’s X.509 certificate and private key (retrieved from DynamoDB) to the filesystem.
4. The agent is configured with a custom MQTT topic prefix (`cms/fleetwise/`) so telemetry publishes to `cms/fleetwise/vehicles/{vin}/signals` instead of the default `$aws/iotfleetwise/` path.
5. The agent connects to IoT Core, receives its decoder manifest and collection scheme (pushed by the CampaignSyncProcessor), and begins reading CAN frames from the assigned `vcanN` interface.
6. A health check verifies the agent has received both `DecoderManifest.bin` and `CollectionSchemeList.bin` before marking the task healthy.
...
2. The container runs `realtime_telemetry_simulator.py --mode can`.
3. The simulator generates CAN frames matching the decoder manifest’s signal definitions (message IDs, start bits, lengths, factors, offsets) and writes them to `vcanN` using SocketCAN.
4. GPS coordinates are injected into the FWE agent via a Unix domain socket (FWE’s location API), since GPS is not a CAN bus signal.
5. When the trip completes, the simulator container exits. The FWE agent remains running for the next trip.
