...
2. IoT Rule routes the checkin to the `fw-checkin` Kafka topic
3. CampaignSyncProcessor consumes the checkin and extracts the vehicle VIN
4. Processor queries the `cms-dev-campaigns` DynamoDB table for active campaigns targeting the vehicle
5. Processor generates protobuf decoder manifest and collection scheme messages
6. Processor publishes the schemes to `cms/fleetwise/vehicles/{vin}/collection_schemes_and_decoder_manifests` via IoT Core MQTT
7. Agent receives the schemes and begins collecting the specified signals
...
2. IoT Rule routes to MSK `cms-telemetry-raw` topic
3. SimulatorPreprocessor decodes and decompresses
4. Clean JSON written to `cms-telemetry-preprocessed`
5. Downstream processors (EventDrivenTelemetryProcessor, TripProcessor, SafetyProcessor, MaintenanceProcessor) consume from `cms-telemetry-preprocessed`
...
2. Extracts vehicle ID, latitude, and longitude
3. Queries the `cms-dev-storage-geofences` DynamoDB table for active geofences (vehicle-specific and global)
4. Calculates distance from vehicle position to geofence center using the Haversine formula
5. Compares distance against the geofence radius
6. On boundary crossing (enter or exit), generates a safety event and writes to the `cms-dev-storage-safety-events` DynamoDB table
7. Deduplicates events: only fires once per boundary crossing direction (enter or exit), preventing repeated alerts while a vehicle remains inside or outside a geofence
...
2. Extracts the `oem_source` field to identify which OEM sent the data
3. Loads the corresponding transform manifest from S3 (cached in memory per OEM)
4. Applies field mappings, unit conversions, and data type transformations defined in the manifest
5. Outputs standard CMS-format JSON to the `cms-telemetry-raw` topic
