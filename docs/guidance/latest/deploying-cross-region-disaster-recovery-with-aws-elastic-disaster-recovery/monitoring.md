...
<a name="configure-replication-monitoring-and-alerting"></a>

Elastic Disaster Recovery can utilize [Amazon CloudWatch](https://aws.amazon.com/cloudwatch/) to assist with monitoring. CloudWatch is a monitoring service that helps you monitor AWS resources as they are being consumed within your account. When these two services are integrated, you can monitor Elastic Disaster Recovery events with CloudWatch to build a customizable and detailed dashboard for Elastic Disaster Recovery. These services can be extended further with [Amazon EventBridge](https://aws.amazon.com/eventbridge/) and [Amazon Simple Notification Service](https://aws.amazon.com/sns/) (Amazon SNS), to get real-time alerts and automate responses.
...
1. Under Dashboards, select **Automatic dashboards** 

1. Filter for and select **Elastic Recovery Service** 
...
1. Choose **Create Topic**.
   ...
   1. Under **Name** enter a name for this topic. (for example: drs-replication-monitoring)
      +  *Optional:* Enter a display name for SMS messages to mobile devices.
        + Note: As of June 1, 2021, US telecom providers no longer support person-to-person long codes for applications-to-person communications. See the [Amazon SNS Developer Guide](https://docs.aws.amazon.com/sns/latest/dg/sms_publish-to-phone.html)for more information.
...
1. Under **Name** and **Description**, enter the name for this rule (we use "drs-replication-monitoring" for this step).

1. Under Define pattern, choose **Event pattern**.

1. Select Pre-defined pattern by service.

1. From the dropdown menu for Service provider, choose **AWS**.

1. Under the Service name dropdown, choose **Elastic Disaster Recovery Service**.

1. Under Event type, choose **DRS Source Server Data Replication Stalled Change**.

1. Under Select targets and Target, choose **SNS topic**.

1. For Topic, choose the SNS topic created earlier: **drs-replication-monitoring**.
...
<a name="configure-cost-monitoring"></a>
...
1. Locate and select **Cost Allocation Tags**.

1. Under **User-defined cost allocation tags**, find the AWSElasticDisasterRecoveryManaged tag.

1. Select the checkbox for this tag, and choose **Activate**.

1. Choose **Activate** from the pop up. It may take a couple of hours before the tags are available.
...
1. Locate and select **Cost Categories** and select **Create cost category**.

1. Provide a Name (for example, DRSCost) to the cost category and select **Next**.

1. Choose Rule type as Inherited value, and Dimension as Cost Allocation Tag, Tag key as AWSElasticDisasterRecoveryManaged, and select **Add rule**.

 **Modify rule** 

1. Choose **Rule type** as **Regular** and **DRSCost *Value* ** as **License**.

1. Under **Dimension 1**, choose **Service**. For **Operator** select **Is** and for **Service Code** choose **AWSElasticDisasterRecovery**.

1. Select **Next**.

1. Select **Create cost category**. It will take up to 24 hours for the cost category to be available in AWS Cost Explorer.
...
1. Open the AWS Cost Management dashboard and select **Cost Explorer**.

1. Locate **Cost Organization** and select Cost Category.

1. Select the cost category (for example: DRSCost) which was created in the Create Cost Categories section.

1. Select two checkboxes: License and drs.amazonaws.com
...
1. Search for AWS Cost Explorer and open the **AWS Cost Management** dashboard.

1. Select **Reports**.

1. Select the report that was previously saved. The total cost of Elastic Disaster Recovery is included in that report.
...
1. Select **Download CSV** to export your data for further analysis.
 ...
 *Note that if you have compliance requirements and require your snapshots for long-term retention, you should use a long-term storage and backup solution like [AWS Backup](https://aws.amazon.com/backup/). Elastic Disaster Recovery is not intended to act as a backup or archive storage service, and hence is not a suitable solution for long-term retention of your snapshots and other data* 
