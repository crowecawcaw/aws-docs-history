...
All data replicated by Elastic Disaster Recovery is encrypted in transit using [TLS 1.2 or later](https://docs.aws.amazon.com/drs/latest/userguide/infrastructure-security.html).
...
### Separate DR account
<a name="separate-dr-account"></a>

A best practice for Elastic Disaster Recovery is to use [separate AWS accounts](https://docs.aws.amazon.com/drs/latest/userguide/multi-account.html) for the Elastic Disaster Recovery staging network (VPC and subnet) and recovery network. Using a separate AWS account specifically for your disaster recovery solution allows for better segmentation and separation of your critical replicated data.
