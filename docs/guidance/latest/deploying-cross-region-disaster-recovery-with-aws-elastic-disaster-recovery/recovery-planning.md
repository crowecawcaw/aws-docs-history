...
<a name="preparing-for-recovery"></a>

1. Configure your [launch templates](https://docs.aws.amazon.com/drs/latest/userguide/launching-target-servers.html) for each server you want to protect.
...
1. Under the **Data replication status** column, the server should show the **Healthy** status, but you can also launch the source server if the system is undergoing **Lag** or even **Stall**, but in that case the data may not be up to date. You can still launch a drill instance from a previous Point In Time.

1. Under the **Pending actions** column, the server should show **Initiative recovery drill** if no drill instances have ever been launched for the server. Otherwise, the column will be blank. This helps you identify whether the server has had a recent drill or recovery launch.

## Performing recovery
<a name="performing-recovery"></a>
...
<a name="group-launch-templates"></a>
