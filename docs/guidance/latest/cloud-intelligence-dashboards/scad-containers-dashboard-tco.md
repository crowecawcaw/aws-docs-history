...
<a name="scad-containers-dashboard-tco"></a>
...
<a name="add-cost-allocation-tags-to-the-athena-view"></a>
...
<a name="how-to-use-eks-k8s-pods-labelsecs-tasks-tags-for-cost-allocation"></a>
...
<a name="cost-allocation-using-k8s-pods-labelsecs-tasks-tags"></a>

Let’s start with a simple use-case - allocating costs to pods/tasks. This is a common use-case that is relevant to many organizations that would like to allocate cost to pods/tasks using their own definitions that aren’t necessarily available using the common Kubernetes (K8s) objects or ECS constructs. For example, you may want to allocate costs to projects, teams, business units, applications, and more. Let’s walk through an example of using the "Workloads Explorer" sheet to allocate costs by K8s pods labels.
...
![SCAD - Containers Cost Allocation Dashboard - Group By App Name Label - Chart](https://docs.aws.amazon.com/guidance/latest/cloud-intelligence-dashboards/images/scad_containers_cost_allocation_workloads_explorer_group_by_app_name_label_chart.png)


![SCAD - Containers Cost Allocation Dashboard - Group By App Name Label - Pivot](https://docs.aws.amazon.com/guidance/latest/cloud-intelligence-dashboards/images/scad_containers_cost_allocation_workloads_explorer_group_by_app_name_label_pivot.png)
...
![SCAD - Containers Cost Allocation Dashboard - Add Parameter for Label](https://docs.aws.amazon.com/guidance/latest/cloud-intelligence-dashboards/images/scad_add_parameter_for_label.png)
...
![SCAD - Containers Cost Allocation Dashboard - Add Control for Label](https://docs.aws.amazon.com/guidance/latest/cloud-intelligence-dashboards/images/scad_add_control_for_label.png)
...
![SCAD - Containers Cost Allocation Dashboard - Add Filter for Label](https://docs.aws.amazon.com/guidance/latest/cloud-intelligence-dashboards/images/scad_add_filter_for_label.png)
...
<a name="total-cost-of-ownership-tco-for-k8s-applications"></a>
...
![SCAD - Containers Cost Allocation Dashboard - AWS Secrets Manager Secret Tags](https://docs.aws.amazon.com/guidance/latest/cloud-intelligence-dashboards/images/scad_aws_secrets_manager_secret_tags.png)
...
![SCAD - Containers Cost Allocation Dashboard - TCO - App to Workload Mapping](https://docs.aws.amazon.com/guidance/latest/cloud-intelligence-dashboards/images/scad_tco_app_resource_workload_type_mapping.png)


From this visual, we can learn that for the label key we selected, the label value `cost-analyzer` (on the left part of the visual) is applied to EKS pods (designated as "EKS Pods" on the right side of the visual) and AWS resources (designated as "Other AWS Services", also on the right side of the visual). We can also learn that other values of this label key are only applied to EKS pods. If you hover with your mouse on a section of the visual, you can see the cost value. On the pie chart right next to the Sankey diagram visual, you can see the cost distribution between EKS pods and other AWS services. The Sankey diagram visual is interactive - click on the `cost-analyzer` value, and it’ll filter other visuals in this sheet.
...
![SCAD - Containers Cost Allocation Dashboard - TCO - App to AWS Service Mapping](https://docs.aws.amazon.com/guidance/latest/cloud-intelligence-dashboards/images/scad_tco_app_aws_services_mapping.png)
