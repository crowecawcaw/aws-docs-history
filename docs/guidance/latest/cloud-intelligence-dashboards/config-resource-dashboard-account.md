...
<a name="config-resource-dashboard-account"></a>
...
<a name="installation-on-dedicated-dashboard-account"></a>
...
1. On the AWS Config account, configure the S3 replication rule that copies AWS Config files from the AWS Config Logs bucket to the Dashboard bucket using a CloudFormation stack.
...
<a name="step-1-in-dashboard-account"></a>

1. Log into the AWS Management Console for your Dashboard account.

1. Ensure you are in the same Region as the AWS Config Logs bucket.
...
<a name="step-2-in-aws-config-account"></a>
...
### Click here if you need to manually set up S3 replication
<a name="collapsible-section-id-config-resource-dashboard-account-1"></a>

 **Manual setup of S3 replication** 

1. Log onto the AWS Config account and open the Amazon S3 console.

1. You can replicate files from the centralized AWS Config Logs bucket to the Dashboard bucket through an Amazon S3 Replication configuration, follow these [instructions](https://docs.aws.amazon.com/AmazonS3/latest/userguide/replication-walkthrough-2.html).
...
<a name="step-3-in-dashboard-account"></a>
