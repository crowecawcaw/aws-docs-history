...
- [Step 7: Create links to the Let’s Encrypt certificate files in the Nginx
  server directory](#link-the-lets-encrypt-certificate-files-in-the-nginx-directory-nginx "#link-the-lets-encrypt-certificate-files-in-the-nginx-directory-nginx")
- [Step 8:
...
![SSH quick connect on the Lightsail home page.](images/amazon-lightsail-nginx-ssh-quick-connect.png) 3. After your Lightsail browser-based SSH session is connected, enter the following
...
![Update the packages on your instance.](images/amazon-lightsail-nginx-ssh-lets-encrypt-update-packages.png) 4. Enter the following command to install the software properties package. Certbot’s
...
Go back to the Lightsail browser-based SSH session for your Nginx instance and complete
...
1. In the Lightsail browser-based SSH session for your Nginx instance, press
...
![Instance services stopped.](images/amazon-lightsail-nginx-ssh-stop-services.png) 2. Enter the following command to set an environment variable for your domain. You can
...
![Instance services started.](images/amazon-lightsail-nginx-ssh-start-services.png)

Your Nginx instance is now configured to use SSL encryption. However, traffic is not
automatically redirected from HTTP to HTTPS. 7. Continue to the [next section](#configure-http-to-https-redirection-nginx "#configure-http-to-https-redirection-nginx") of this tutorial.

## Step 8: Configure HTTP to HTTPS

redirection for your web application

You can configure an HTTP to HTTPS redirect for your Nginx instance. Automatically
...
(save) your edits, and quit Vim. 5. Enter the following command to modify the server section of the Nginx configuration
file:
...
1. In the Lightsail browser-based SSH session for your Nginx instance, enter the
   following command to modify the server section of the Nginx configuration
   file:
...
1. In the Lightsail browser-based SSH session for your Nginx instance, enter the
   following command to edit the Nginx web server configuration file using the Vim text
   editor:
...
![Nginx configuration file edited for HTTP to HTTPS redirection.](images/amazon-lightsail-nginx-ssh-lets-encrypt-config-file.png) 4. Press the **ESC** key, and then enter `:wq` to write
(save) your edits, and quit Vim. 5. Enter the following command to restart the underlying services and make your edits
effective:
...
Your Nginx instance is now configured to automatically redirect connections from HTTP
