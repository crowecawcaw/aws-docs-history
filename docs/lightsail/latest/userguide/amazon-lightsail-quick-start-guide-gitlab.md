...
Read the Bitnami documentation to learn how to configure your GitLab CE application. For more information, see the [GitLab CE Packaged By Bitnami For AWS Cloud](https://docs.bitnami.com/general/apps/gitlab/).
...
1. Enter the following command to create symbolic links for your new Let's Encrypt certificates in the `/etc/gitlab/ssl` directory, which is the default certificates directory on your GitLab CE instance.
