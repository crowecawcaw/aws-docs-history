...
Amazon Lightsail. However, the action requires the service to have permissions that are granted by a service role. Mary does not have permissions to pass the
role to the service.
...
a policy to the entity that grants them the correct permissions in Amazon Lightsail. After the permissions are granted, provide the credentials to the user
...
- To learn whether Amazon Lightsail supports these features, see [How Amazon Lightsail works with IAM](security_iam_service-with-iam.md "security_iam_service-with-iam.md").
