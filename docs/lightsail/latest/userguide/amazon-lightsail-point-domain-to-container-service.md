...
zone of each of the domains specified on the certificates that you're using with your container
service. All of the records that you add should point to the default domain (e.g.,
`https://<ServiceName>.<RandomGUID>.<AWSRegion>.cs.amazonlightsail.com`)
of your container service.

In this guide, we provide you with the procedure to point your domains to your container
service using a Lightsail DNS zone. For more information about Lightsail DNS zones, see [DNS in Amazon Lightsail](understanding-dns-in-amazon-lightsail.md "understanding-dns-in-amazon-lightsail.md").
...
If you're using Route 53 to host the DNS of your domain, then you should add the alias record
to the hosted zone of your domain in Route 53. For more information, see [Routing traffic for a
domain in Route 53 to an Amazon Lightsail container service](amazon-lightsail-route-53-alias-record-for-container-service.md "amazon-lightsail-route-53-alias-record-for-container-service.md").

## Prerequisite

Before you get started, you should enable custom domains for your Lightsail container
...
3. Choose the name of a container service for which want get the default domain
   name.
4. In the header section of your container service management page, make note of your
   default domain name. Your container service default domain name is similar to
   ``<ServiceName>`.`<RandomGUID>`.`<AWSRegion>`.cs.amazonlightsail.com`.

You must add this value as part of a canonical name (CNAME) record in the DNS of your
domains. We recommend that you copy and paste this value into a text file that you can
...
4. Complete one of the following steps depending on the current state of your DNS
   ...
   - If you previously added an A, AAAA, or CNAME record, choose the edit icon next to
     the existing A, AAAA, or CNAME record listed on the page, and then skip to step 5 of
     this procedure.
...
5. Choose **A record**, **AAAA record**, or
   **CNAME record** in the **Record type** dropdown
   ...
   - Add an A record to map the apex of your domain (e.g., `example.com`) or
     ...
     IPv4 network.
   - Add an AAAA record to map the apex of your domain (e.g., `example.com`)
     or a subdomain (e.g., `www.example.com`) to your container service under
     the IPv6 network.
   - Add a CNAME record to map a subdomain (e.g., `www.example.com`) to the
     public domain (default DNS) of your container service.
...
6. In the **Record name** text box, enter one of the following
   ...
   - For an A record or AAAA record, enter `@` to route traffic for the apex
     ...
     `www.example.com`) to your container service.
   - For a CNAME record, enter a subdomain (e.g., `www`) to route traffic
...
you are using with your container service. Allow time for changes to propagate through the
