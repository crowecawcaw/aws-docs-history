...
details about the Lightsail resources in your AWS account. If you create an
identity-based policy that is more restrictive than the minimum required permissions
(i.e., that is not full-access), the console won't function as intended for entities
(IAM users or roles) with that policy.

To ensure that those entities can use the Lightsail console, attach the following
...
users from creating new Lightsail resources unless a key tag of `allow` and
a value of `true` is defined with the create request. This policy also
restricts users from deleting resources unless they have the `allow/true`
key-value tag.
...
The following example restricts users from changing the tag for resources that have a
...
You can attach these policies to the IAM users in your account.
