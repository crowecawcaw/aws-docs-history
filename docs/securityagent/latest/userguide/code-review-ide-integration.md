...
1. The archive is uploaded to an S3 bucket that the server auto-provisions in your AWS account.

1. The server calls the AWS Security Agent API to start a scan.

1. AWS Security Agent analyzes the code for security vulnerabilities and compliance with your organization’s security requirements.

1. Findings are returned to the IDE with code locations, descriptions, severity ratings, and remediation suggestions.
...
+  [uv](https://docs.astral.sh/uv/getting-started/installation/) installed (Python package runner)
+ Python 3.10 or later
+ One of the following IDEs:
  +  [Kiro](https://kiro.dev) (install the AWS Security Agent Power)
...
**Tip**  
You can also run the MCP server via Docker if you prefer not to install Python locally. See the [MCP server documentation](https://github.com/awslabs/mcp) for Docker configuration.
...
1. Uploads the archive to S3

1. Starts a full code review job

1. Polls for completion (typically around 1 hour)

1. Presents findings grouped by severity
