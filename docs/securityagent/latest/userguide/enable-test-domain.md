...
<a name="_step_1_add_a_target_domain"></a>
...
<a name="_step_2_verify_domain_ownership"></a>
...
<a name="_verify_using_a_dns_txt_record"></a>
...
1. Add the TXT record with your DNS registrar.

1. Return to the Target Domains table, select the domain, and choose **Verify**.

### Verify using HTTP route validation
<a name="_verify_using_http_route_validation"></a>
...
1. Place the token provided by AWS Security Agent in the file using this format:
...
1. Return to the Target Domains table, select the domain, and choose **Verify**. AWS Security Agent sends an HTTPS GET request to the verification URL and validates the token.

1. If the domain is accessible on the public internet, make sure that your domain has a valid SSL certificate before running verification.
