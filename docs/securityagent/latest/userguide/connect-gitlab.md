...
<a name="_how_gitlab_integration_works"></a>
...
<a name="_register_a_gitlab_connection"></a>
...
1. Under **Choose an account type**, select one of the following:
...
1. In the **Registration name** field, enter a descriptive name for this connection, such as `Engineering-Team-GitLab`. Valid characters are letters, numbers, periods, underscores, and hyphens.
...
1. Verify the token has not expired. GitLab returns `401 Unauthorized` for expired, revoked, or malformed tokens.

1. Verify the token has the scopes required for the operations you use (see [Missing or insufficient token scope](#gitlab-token-scopes)).

1. If the token has expired or was revoked, create a new token and update the integration in the AWS Console.
