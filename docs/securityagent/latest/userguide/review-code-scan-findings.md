...
1. Select the code review you want to examine.

1. In the **All runs** table, select the completed run by clicking its start time link. Alternatively, choose **Monitor run** to view the latest run.

## Step 2: Monitor run progress
<a name="_step_2_monitor_run_progress"></a>
...
1. Review the status of each phase:
...
<a name="_step_3_review_the_run_summary"></a>

Navigate to the **Code review run** tab to view the high-level results.

1. Select the **Code review run** tab.

1. The **Run summary** section provides run status, duration and other high level details. It also provides a dashboard of security findings categorized by severity level and risk-types.

1. Application overview by AWS Security Agent provides a summary of the code review scan when the run is completed

## Step 4: Review preflight results
<a name="_step_4_review_preflight_results"></a>

Navigate to the **Preflight** tab to verify that all source access checks passed.

1. Select the **Preflight** tab.

1. Review the **Preflight progress** indicator showing the number of checks completed.

1. Verify that each check shows a success status:
...
1. Browse the list of tasks in the left panel.

1. Select a task to view its detailed log in the right panel.
...
<a name="_step_6_navigate_to_findings"></a>
...
1. The findings display in a split view with the findings list on the left and the selected finding’s details on the right.

1. Review the information displayed on each finding card:
   +  **Finding title** – A descriptive name summarizing the security issue
   +  **Severity badge** – Color-coded risk level (Critical, High, Medium, or Low). For what each level means, see [Severity ratings and CVSS metrics](severity-and-cvss.md).
   +  **Last updated** – Timestamp of when the finding was last updated

## Step 7: Review finding details
<a name="_step_7_review_finding_details"></a>
...
1. Review the available actions:
...
1. Provide feedback using **Was this finding accurate?** – Select **Yes** or **No** to help improve future analysis accuracy.

1. Review the key attributes in the **Overview** section:
...
1. Expand the **Code locations** section to view:
...
1. Expand the **Evidence** section to review:
...
1. Expand the **Suggested fix** section to view:
...
1. Plan a follow-up code review run to verify the fix is effective.
...
1. Consider whether multiple medium-severity findings together create higher risk.
...
1. If vulnerability is valid, prioritize based on severity and risk tolerance and consider remediation tasks.
...
1. Run a new code review to verify that fixes resolve the findings and don’t introduce new issues.

1. Review the **All runs** table on the code review detail page to compare findings across runs over time.
