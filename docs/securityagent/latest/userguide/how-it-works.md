...
| --- | --- | 
|  **Administrator**  | Works in the **AWS Management Console** — sets up Agent Spaces, defines security requirements, configures capabilities, and manages user access. | 
|  **User**  | Works in the **Security Agent Web Application** — runs penetration tests, code reviews, threat models, and design reviews, and reviews the resulting findings. | 
|  **Developer**  | Works in **GitHub or an IDE** (such as Kiro or Claude Code) — receives automated security findings on pull requests and runs code scans without leaving the development environment. | 

We use these role names throughout to indicate who performs each task. A single person can hold more than one role.

For how AWS Security Agent organizes resources (what’s shared across your organization versus scoped to each Agent Space) and how each capability is run and re-run, see [Understand the resource hierarchy and lifecycle](understand-lifecycle.md).
