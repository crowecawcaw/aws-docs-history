...
<a name="_aws_security_agent_information_in_cloudtrail"></a>
...
All AWS Security Agent actions are logged by CloudTrail and are documented in the [AWS Security Agent API Reference](https://docs.aws.amazon.com/security-agent/latest/api/). For example, calls to the `CreatePentest`, `StartPentestExecution`, and `ListFindings` actions generate entries in the CloudTrail log files.
...
<a name="_understanding_aws_security_agent_log_file_entries"></a>
...
<a name="_monitoring_aws_security_agent_with_amazon_cloudwatch"></a>
...
1. Configure the alarm conditions, notification, and other settings.
