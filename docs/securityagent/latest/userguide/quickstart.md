...
When you choose Set up, AWS Security Agent creates your Agent Space and establishes a web application where your users can carry out design reviews and penetration tests.
...
1. After you complete the pentest setup, a banner appears with an option to connect GitHub for penetration testing, choose **Add** in the right side of the banner.
2. Choose **Create new registration**
3. Select **GitHub** and then **Next**
4. Choose **Install and authorize**. You are redirected to GitHub to complete the installation.
...
1. Select the **Web app** tab and then **Admin access** to launch the AWS Security Agent Web Application with administrator privileges. This works only if you had setup your agent using **IAM-only access** under _User access configuration_. Alternatively, you will need to add users and create a login.
2. In the left sidebar, choose **Penetration Test**, then select **Create your first penetration test**.
...
6. Choose **Create and execute**. You are redirected to the penetration test detail screen.

   - To save the configuration for future use without running it immediately, choose **Create penetration test** instead.
