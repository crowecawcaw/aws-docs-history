...
Skip this task if your target endpoints use certificates issued by a publicly trusted CA. For a private endpoint hosted in a VPC, provide the trust anchor in addition to the VPC configuration. For more information, see [Connect agent to private VPC resources](connect-agent-vpc.md "connect-agent-vpc.md").
...
3. For **Source**, choose one of the following:
...
4. Review the parsed certificate details (subject, issuer, type, and validity), and then choose **Save**.
