...
provisioned or registered. The events provide you information about the different
...
AWS IoT Wireless. The registration process starts, where the encryption and
session keys are set up with AWS IoT. When the device is registered, the
`eventType` becomes `registered`, and your device is
ready to use.
...
Reserved MQTT topics for Sidewalk device registration state events use the
following format:
...
{eventType} can be `provisioned` or
`registered`.
...
be notified of all possible event types (`provisioned` and
`registered`) and for all devices registered to a particular
...
The type of event that occurred. Can be `provisioned`
or `registered`.
