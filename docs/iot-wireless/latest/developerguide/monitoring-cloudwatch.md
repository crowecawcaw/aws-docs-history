...
+ Network analyzer enables you to monitor your LoRaWAN resources, which includes LoRaWAN devices and gateways. The network analyzer reduces the time that it takes to set up a connection to start receiving trace messages, providing you with just-in-time log information. For more information, see [Monitoring of LoRaWAN resources using network analyzer](network-analyzer-overview.md).
...
1. Create a logging role to log your AWS IoT Wireless resources, as described in [Create logging role and policy for AWS IoT Wireless monitoring](create-logging-role-policy.md).

1. Log messages in the CloudWatch Logs console have a default log level of `ERROR`, which is less verbose and contains only error information. If you want to view more verbose messages, we recommend that you first configure logging, as described in [Configure resource logging for AWS IoT Wireless resources](configure-resource-logging.md). 
