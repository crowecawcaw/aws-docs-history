...
request of type 0 or type 2 is received, and the device has joined with
AWS IoT Core for LoRaWAN.

## How join events work

When you onboard your LoRaWAN devices with AWS IoT Core for LoRaWAN, AWS IoT Core for LoRaWAN
performs a _join_ procedure for your device with
...
AWS account can receive the notification:
...
{resourceID} can be `dev_eui` or
`wireless_device_id`.

For example, you can subscribe to the following topics to receive an event
notification when AWS IoT Core for LoRaWAN has accepted a join request from your
devices.
...
The type of event that occurred. Can be one of the following
values:

- `join_req_received`: This field will show the
  EUI parameters `JoinEui` or
  `AppEui`
- `rejoin_req_0_received`
- `rejoin_req_2_received`
- `join_accepted`: This field will show the
  `NetId` and `DevAddr`.
...
The unique identifier of the device found on the device label or
device documentation.

**DevAddr and EUIs (optional)**

These fields are the optional device address and the EUI
parameters `JoinEUI` or `AppEUI`.
