...
The AMS Accelerate monitoring system relies on internal tools, such as Resource Tagger and Alarm Manager, and leverages AWS services, such as [AWS AppConfig](https://docs.aws.amazon.com/appconfig/latest/userguide/what-is-appconfig.html), Amazon CloudWatch (CloudWatch), Amazon EventBridge (formerly known as CloudWatch), Amazon GuardDuty, Amazon Macie, and AWS Health.
...
+ [Alerts from baseline monitoring in AMS](monitoring-default-metrics.md)
+ [Application aware incident notifications in AMS](app-aware-inc-notifications.md)
+ [Accelerate Alarm Manager](acc-mem-tag-alarms.md)
+ [AMS automatic remediation of alerts](auto-remediation.md)
+ [Using Amazon EventBridge Managed Rules in AMS](how-event-router-works.md)
+ [Trusted Remediator in AMS](trusted-remediator.md)

For information about monitoring Amazon EKS, see [Monitoring and incident management for Amazon EKS in AMS Accelerate](acc-mon-inc-mgmt-eks.md)
...
+ A monitoring baseline so that you have a default level of protection, even if you don’t configure any other monitoring for your managed accounts. For more information, see [Alerts from baseline monitoring in AMS](monitoring-default-metrics.md).
+ The ability to customize the baseline resource alarms to meet your requirements.
+ Automatic remediation of alerts by AMS Operations, when possible, to prevent or reduce the impact to your applications. For example, if you are using a standalone Amazon EC2 instance and it fails the system health check, AMS attempts to recover the instance by stopping and restarting it. For more information, see  [AMS automatic remediation of alerts](auto-remediation.md).
