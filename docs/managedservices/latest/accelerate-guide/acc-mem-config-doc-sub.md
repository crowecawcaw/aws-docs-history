...
- In an **AWS::EC2::Instance::Disk** resource type block:
  ...
  - ${EC2::Disk::FSType}: Replaced by the file system type of the disk. (Linux only, on instances managed by the
  ...
  - ${EC2::Disk::Path}: Replaced by the disk path. (Windows only, this is the drive label (for example, c:/), only on an instance managed by the
    [CloudWatch Agent](../../../AmazonCloudWatch/latest/monitoring/Install-CloudWatch-Agent.md "../../../AmazonCloudWatch/latest/monitoring/Install-CloudWatch-Agent.md")).
