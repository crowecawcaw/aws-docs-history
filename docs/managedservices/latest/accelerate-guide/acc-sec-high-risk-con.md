...
**Logging**
+ High\_Risk-LOG-001: Disable CloudTrail.
+ High\_Risk-LOG-002: Disable VPC Flow Logs.
+ High\_Risk-LOG-003: Log forwarding via any method (S3 event notification, SIEM agent pull, SIEM agent push etc) from an AMS managed account to third party account (not owned by customer)
