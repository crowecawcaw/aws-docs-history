...
RFC delete stack failures: If you use the Management \| Standard stacks \| Stack \| Delete CT, you will see the detailed events in the CloudFormation Console for the stack with the AMS stack name. You can identify your stack by checking it against the name it has in the AMS Console. The CloudFormation Console provides more details about failure causes.
...
Most AMS CTs that create a resource are based on CloudFormation templates. As a customer you have read-only access to all AWS services including CloudFormation, you can quickly identify the CloudFormation stack that represents your stack based on the stack description using the CloudFormation Console. The failed stack will likely be in a state of DELETE\_COMPLETE. Once you have identified the CloudFormation stack, the events will show you the specific resource that failed to create, and why.
...
![Request for change details showing rejected status due to no domain trust found.](https://docs.aws.amazon.com/managedservices/latest/onboardingguide/images/rfcReason.png)
...
![EC2 instance details showing IAM role field with customer-mc-ec2-instance-profile value.](https://docs.aws.amazon.com/managedservices/latest/onboardingguide/images/ec2ConsoleWCircle.png)
...
The `InternalError` message is from CloudFormation; a CREATION\_FAILED status reason. You can find details on the stack failure in CloudWatch stack events by following these steps:
