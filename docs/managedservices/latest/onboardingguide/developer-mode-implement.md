...
1. Submit a request for change (RFC) using the change type (CT) Management \| Managed account \| Developer mode \| Enable (managed automation). For an example of how to use this CT, see [ Developer Mode \| Enable (Managed Automation)](https://docs.aws.amazon.com/managedservices/latest/ctref/management-managed-developer-mode-enable-review-required.html).
