...
1. To access RDP (Windows) bastions, use DNS records like this: `rdp-{{Username}}.{{Your_Domain}}.com`.
...
1. To access SSH (Linux) bastions, use DNS records like this: `sshbastion{{(1-4)}}.A{{AccountNumber}}.amazonaws.com.`
...
1. To access RDP (Windows) bastions, use DNS records like this: `rdpbastion{{(1-4)}}.A{{ACCOUNT_NUMBER}}.amazonaws.com`.
...
AMS customers can use SSH and RDP bastions, either the [DNS friendly bastion names](#dns-bastions) described previously, or bastion IP addresses.
