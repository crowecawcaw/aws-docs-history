...
<a name="deployment-advanced-kms-key-create-managed-automation"></a>

Request a KMS key by describing key permissions or submitting a key policy document.

**Full classification:** Deployment \| Advanced stack components \| KMS key \| Create (managed automation)
...
<a name="deployment-advanced-kms-key-create-managed-automation-info"></a>

### Create KMS Key (Managed Automation)
<a name="ex-kms-key-create-rr-col"></a>

#### Creating an AWS KMS Key (Managed Automation) with the Console
<a name="kms-key-create-rr-con"></a>
...
#### Creating an AWS KMS Key (Managed Automation) with the CLI
<a name="kms-key-create-rr-cli"></a>
...
```
aws amscm create-rfc --change-type-id "ct-2epp05svrlwod" --change-type-version "3.0" --title "{{TITLE}}" --execution-parameters "{\"KeyDescription\": \"{{Example description}}\", \"KeyPermissions\": \"{{key permissions}}\", \"Operation\": \"Create\"}"
```
...
1. Modify and save the CreateKmsKeyParams file. For example, you can replace the contents with something like this:
...
1. Output the RFC template JSON file to a file; this example names it CreateKmsKeyRfc.json:

   ```
   aws amscm create-rfc --generate-cli-skeleton > CreateKmsKeyRfc.json
   ```

1. Modify and save the CreateKmsKeyRfc.json file. For example, you can replace the contents with something like this:
...
1. Create the RFC, specifying the CreateKmsKey Rfc file and the CreateKmsKeyParams file:
...
#### Tips
<a name="ex-kms-key-create-rr-tip"></a>
...
<a name="deployment-advanced-kms-key-create-managed-automation-input"></a>

For detailed information about the execution input parameters, see [Schema for Change Type ct-2epp05svrlwod](schemas.md#ct-2epp05svrlwod-schema-section).

## Example: Required Parameters
<a name="deployment-advanced-kms-key-create-managed-automation-ex-min"></a>
...
<a name="deployment-advanced-kms-key-create-managed-automation-ex-max"></a>
