...
![Enable TGW Propagation change type showing description, ID ct-1f9hi4bephqa9, and version 1.0.](https://docs.aws.amazon.com/managedservices/latest/ctref/images/guiManLzNetAcctTgwPropEnableCT.png)
...
1. Modify and save the TgwPropagationEnableParams file. For example, you can replace the contents with something like this:
...
1. Output the RFC template JSON file to a file; this example names it TgwPropagationEnableRfc.json:

   ```
   aws amscm create-rfc --generate-cli-skeleton > TgwPropagationEnableRfc.json
   ```

1. Modify and save the TgwPropagationEnableRfc.json file. For example, you can replace the contents with something like this:
...
1. Create the RFC, specifying the TgwPropagationEnableRfc file and the TgwPropagationEnableParams file:
...
For detailed information about the execution input parameters, see [Schema for Change Type ct-1f9hi4bephqa9](schemas.md#ct-1f9hi4bephqa9-schema-section).
