...
1. Modify and save the UpdateScheduleParams file.
...
1. Output the RFC template to a file in your current folder; this example names it UpdateScheduleRfc.json:

   ```
   aws amscm create-rfc --generate-cli-skeleton > UpdateScheduleRfc.json
   ```

1. Modify and save the UpdateScheduleRfc.json file. For example, you can replace the contents with something like this:
...
1. Create the RFC, specifying the UpdateScheduleRfc file and the UpdateScheduleParams file:
...
For detailed information about the execution input parameters, see [Schema for Change Type ct-3u61cd4edns0x](schemas.md#ct-3u61cd4edns0x-schema-section).
