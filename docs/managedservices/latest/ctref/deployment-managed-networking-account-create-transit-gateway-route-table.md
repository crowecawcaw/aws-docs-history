...
![Create Application Account Route Table interface showing ID, execution mode, version, and classification details.](/images/managedservices/latest/ctref/images/guiMalzNetAcctCreateAppAcctRouteTableRrCT.png)
How it works:
...
- This change type is manual. To use the automated version of this change type, see
  [Networking Account | Create Application Route Table](deployment-managed-networking-account-create-application-route-table.md "deployment-managed-networking-account-create-application-route-table.md").
- By default, the route table does not connect to on-premise network, but contains preset routes. To request connections to the on-premise network,
  submit a Deployment | Managed landing zone | Networking account | Add static route change type, with the route table ID, to add routes to it.

If you set the **AddPresetStaticRoutes** parameter to False, the route table that created is empty and you must file a
Deployment | Managed landing zone | Networking account | Add static route change type, with the route table ID, to add routes to it.
...
[Schema for Change Type ct-3dscwaeyi6cup](schemas.md#ct-3dscwaeyi6cup-schema-section "schemas.md#ct-3dscwaeyi6cup-schema-section").
