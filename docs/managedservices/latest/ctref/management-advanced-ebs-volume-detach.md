...
1. Modify and save the DetachEbsVolParams file. For example, you can replace the contents with something like this:
...
1. Output the RFC template JSON file to a file; this example names it DetachEbsVolRfc.json:

   ```
   aws amscm create-rfc --generate-cli-skeleton > DetachEbsVolRfc.json
   ```

1. Modify and save the DetachEbsVolRfc.json file. For example, you can replace the contents with something like this:
...
1. Create the RFC, specifying the DetachEbsVolRfc file and the DetachEbsVolParams file:
...
+ For Linux instances, volumes can't be detached if managed by Logical Volume Manager (LVM). Only volumes with filesystem type "ext3", "ext4" and "xfs" can be detached from Linux instances using this change type.
...
For detailed information about the execution input parameters, see [Schema for Change Type ct-2d55p1d7z6w3d](schemas.md#ct-2d55p1d7z6w3d-schema-section).
