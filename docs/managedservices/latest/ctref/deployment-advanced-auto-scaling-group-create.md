...
![Change type details showing Auto Scaling group creation with ID, version 3.0, and automated execution mode.](images/guiAsgCreateCT.png)
How it works:
...
1. Issue the create RFC command with execution parameters provided inline
   (escape quotation marks when providing execution parameters inline) and then submit the
   returned RFC ID. For example:
...
1. Output the execution parameters JSON schema for this change type to a file in your
   current folder; this example names it CreateAsgParams.json:
...
The RFC waits up to six hours for all of the UserData script commands to run
before returning a final status of success or failure.
...
You can add up to 50 tags, but to do so you must enable the
...
[Schema for Change Type ct-2tylseo8rxfsc](schemas.md#ct-2tylseo8rxfsc-schema-section "schemas.md#ct-2tylseo8rxfsc-schema-section").
