...
1. Modify and save the CreateStoreGatewayFromBackupParams file.
...
1. Output the RFC template to a file in your current folder; this example names it CreateStoreGatewayFromBackupRfc.json:

   ```
   aws amscm create-rfc --generate-cli-skeleton > CreateStoreGatewayFromBackupRfc.json
   ```

1. Modify and save the CreateStoreGatewayFromBackupRfc.json file. For example, you can replace the contents with something like this:
...
1. Create the RFC, specifying the CreateStoreGatewayFromBackupRfc file and the CreateStoreGatewayFromBackupParams file:
...
For detailed information about the execution input parameters, see [Schema for Change Type ct-0cupn1txog5tk](schemas.md#ct-0cupn1txog5tk-schema-section).
