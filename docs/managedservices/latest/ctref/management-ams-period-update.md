...
![Update Resource Scheduler Period page showing ID ct-2pkdckieh62ps and Version 1.0.](https://docs.aws.amazon.com/managedservices/latest/ctref/images/guiResSchedPeriodUpdateCT.png)
...
1. Modify and save the UpdatePeriodParams file.
...
1. Output the RFC template to a file in your current folder; this example names it UpdatePeriodRfc.json:

   ```
   aws amscm create-rfc --generate-cli-skeleton > UpdatePeriodRfc.json
   ```

1. Modify and save the UpdatePeriodRfc.json file. For example, you can replace the contents with something like this:
...
1. Create the RFC, specifying the UpdatePeriodRfc file and the UpdatePeriodParams file:
...
For detailed information about the execution input parameters, see [Schema for Change Type ct-2pkdckieh62ps](schemas.md#ct-2pkdckieh62ps-schema-section).
