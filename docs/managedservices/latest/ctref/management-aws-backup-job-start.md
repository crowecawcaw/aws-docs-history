...
1. Modify and save the StartBackupJobParams file.
...
1. Output the RFC template to a file in your current folder; this example names it StartBackupJobRfc.json:

   ```
   aws amscm create-rfc --generate-cli-skeleton > StartBackupJobRfc.json
   ```

1. Modify and save the StartBackupJobRfc.json file. For example, you can replace the contents with something like this:
...
1. Create the RFC, specifying the StartBackupJobRfc file and the StartBackupJobParams file:
...
For detailed information about the execution input parameters, see [Schema for Change Type ct-2hhud2lx01tq7](schemas.md#ct-2hhud2lx01tq7-schema-section).
