...
1. Modify and save the StopBackupJobParams file.
...
1. Output the RFC template to a file in your current folder; this example names it StopBackupJobRfc.json:

   ```
   aws amscm create-rfc --generate-cli-skeleton > StopBackupJobRfc.json
   ```

1. Modify and save the StopBackupJobRfc.json file. For example, you can replace the contents with something like this:
...
1. Create the RFC, specifying the StopBackupJobRfc file and the StopBackupJobParams file:
...
For detailed information about the execution input parameters, see [Schema for Change Type ct-1895yr1p87noq](schemas.md#ct-1895yr1p87noq-schema-section).
