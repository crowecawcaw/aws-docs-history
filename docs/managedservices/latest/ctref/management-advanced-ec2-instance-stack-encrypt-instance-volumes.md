...
#### Encrypting Amazon EC2 instance volumes with the console
...
#### Encrypting Amazon EC2 instance volumes with the CLI
...
1. Modify and save the EncryptEC2sParams file, retaining only the parameters that you want to change. For example, you can replace the contents with something like this: 
...
1. Output the RFC template to a file in your current folder; this example names it EncryptEC2sRfc.json:

   ```
   aws amscm create-rfc --generate-cli-skeleton > EncryptEC2sRfc.json
   ```

1. Modify and save the EncryptEC2sRfc.json file. For example, you can replace the contents with something like this:
...
1. Create the RFC, specifying the EncryptEC2sRfc file and the EncryptEC2sParams file:
...
For detailed information about the execution input parameters, see [Schema for Change Type ct-0hahohe17csnc](schemas.md#ct-0hahohe17csnc-schema-section).
