...
1. Modify and save the DescribeScheduleParams file.
...
1. Output the RFC template to a file in your current folder; this example names it DescribeScheduleRfc.json:

   ```
   aws amscm create-rfc --generate-cli-skeleton > DescribeScheduleRfc.json
   ```

1. Modify and save the DescribeScheduleRfc.json file. For example, you can replace the contents with something like this:
...
1. Create the RFC, specifying the DescribeScheduleRfc file and the DescribeScheduleParams file:
...
For detailed information about the execution input parameters, see [Schema for Change Type ct-2ptn20pq7ur3x](schemas.md#ct-2ptn20pq7ur3x-schema-section).
