...
<a name="management-advanced-kms-key-update-managed-automation"></a>
...
<a name="management-advanced-kms-key-update-managed-automation-info"></a>

### Update KMS key (Managed Automation)
<a name="ex-kms-key-update-rr-col"></a>

#### Updating an AWS KMS Key (Managed Automation) with the Console
<a name="kms-key-update-rr-con"></a>
...
#### Updating an AWS KMS Key (Managed Automation) with the CLI
<a name="kms-key-update-rr-cli"></a>
...
```
aws amscm create-rfc --change-type-id "ct-3ovo7px2vsa6n" --change-type-version "3.0" --title "{{TITLE}}" --execution-parameters "{\"KeyDescription\": \"{{Example description}}\", \"KeyPermissions\": \"{{key permissions}}\", \"PolicyAction\": \"Replace\", \"Operation\": \"Update\"}"
```
...
1. Output the execution parameters JSON schema for this change type to a file; this example names it UpdateKmsKeyParams.json:
   ...
   ```
   aws amscm get-change-type-version --change-type-id "ct-3ovo7px2vsa6n" --query "ChangeTypeVersion.ExecutionInputSchema" --output text > UpdateKmsKeyParams.json
   ```

1. Modify and save the UpdateKmsKeyParams file. For example, you can replace the contents with something like this:
...
1. Output the RFC template JSON file to a file; this example names it UpdateKmsKeyRfc.json:

   ```
   aws amscm create-rfc --generate-cli-skeleton > UpdateKmsKeyRfc.json
   ```

1. Modify and save the UpdateKmsKeyRfc.json file. For example, you can replace the contents with something like this:
...
1. Create the RFC, specifying the UpdateKmsKey Rfc file and the UpdateKmsKeyParams file:
...
#### Tips
<a name="ex-kms-key-update-rr-tip"></a>
...
<a name="management-advanced-kms-key-update-managed-automation-input"></a>

For detailed information about the execution input parameters, see [Schema for Change Type ct-3ovo7px2vsa6n](schemas.md#ct-3ovo7px2vsa6n-schema-section).

## Example: Required Parameters
<a name="management-advanced-kms-key-update-managed-automation-ex-min"></a>
...
<a name="management-advanced-kms-key-update-managed-automation-ex-max"></a>
