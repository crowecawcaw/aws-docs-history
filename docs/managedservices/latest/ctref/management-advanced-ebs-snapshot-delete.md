...
1. Modify and save the DeleteEbsSnpshtParams file. For example, you can replace the contents with something like this:
...
1. Output the RFC template JSON file to a file; this example names it DeleteEbsSnpshtRfc.json:

   ```
   aws amscm create-rfc --generate-cli-skeleton > DeleteEbsSnpshtRfc.json
   ```

1. Modify and save the DeleteEbsSnpshtRfc.json file. For example, you can replace the contents with something like this:
...
1. Create the RFC, specifying the DeleteEbsSnpshtRfc file and the DeleteEbsSnpshtParams file:
...
For detailed information about the execution input parameters, see [Schema for Change Type ct-30bfiwxjku1nu](schemas.md#ct-30bfiwxjku1nu-schema-section).
