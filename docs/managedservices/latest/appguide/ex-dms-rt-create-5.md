...
1. Output the JSON template to a file in your current folder; this example names it CreateDmsRtRfc.json:

   ```
   aws amscm create-rfc --generate-cli-skeleton > CreateDmsRtRfc.json
   ```

1. Modify and save the CreateDmsRtRfc.json file. For example, you can replace the contents with something like this:
...
1. Create the RFC, specifying the execution parameters file and the CreateDmsRtRfc file:
