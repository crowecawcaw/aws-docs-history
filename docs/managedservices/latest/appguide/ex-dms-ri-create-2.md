...
1. Modify and save the execution parameters CreateDmsRiParams.json file. For example, you can replace the contents with something like this:
...
1. Output the JSON template to a file in your current folder; this example names it CreateDmsRiRfc.json:

   ```
   aws amscm create-rfc --generate-cli-skeleton > CreateDmsRiRfc.json
   ```

1. Modify and save the CreateDmsRiRfc.json file. For example, you can replace the contents with something like this:
...
1. Create the RFC, specifying the execution parameters file and the CreateDmsRiRfc file:
