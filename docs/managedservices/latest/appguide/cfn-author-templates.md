...
To reduce CloudFormation errors while performing CloudFormation ingest, follow these
guidelines:
...
- **Follow CloudFormation guidelines when writing CloudFormation resource templates** –
...
- **Configure your custom CloudFormation templates to use parameters defined in the AMS CloudFormation ingest CT** –
  ...
  (ct-361tlo1k7339x). For an example, see [CloudFormation Ingest examples: Defining resources](cfn-ingest-ex-define-resource.md "cfn-ingest-ex-define-resource.md").
...
- **Wait Condition requires signal logic** –
  ...
  the Wait Condition resource in the template, CloudFormation waits for a success signal, and it marks stack creation as a failure if the number of success
