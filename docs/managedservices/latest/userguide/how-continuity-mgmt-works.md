...
<a name="how-continuity-mgmt-works"></a>

AMS uses [AWS Backup](https://aws.amazon.com/backup/) for continuity management.

When starting to work with AWS Backup in AMS:

1. Run an on-demand backup

1. Create a backup plan (optional, AMS provides default backup plans)

1. Use the default AMS a backup vaults (optional)

1. Manage (run, refine, delete, and so forth) your backup plans and recovery points

## AMS backup plans
<a name="ams-backup-plans"></a>
...
AWS Backup can operate at the EBS volume level or at the Amazon EC2 instance level, but do not do both at the same time, as this can lead to a race condition where the backups may clash.

### Default backup plans, multi-account landing zone
<a name="backup-plans-default"></a>
...
<a name="acc-backup-plan-default-enhanced"></a>

This plan is a blueprint for AWS Backup best practices to protect against ransomware attacks. It implements a daily, weekly, monthly, and yearly backup strategy. AWS Backup [continuous backup](https://docs.aws.amazon.com/aws-backup/latest/devguide/point-in-time-recovery.html) is enabled with maximum retention (31 days) on [ supported resources](https://docs.aws.amazon.com/aws-backup/latest/devguide/point-in-time-recovery.html#point-in-time-recovery-supported-services).
...
<a name="acc-backup-plan-default-data-sen"></a>

This plan is a blueprint for AWS Backup best practices to protect against ransomware attacks for data-sensitive applications. It implements an hourly, daily, weekly, monthly, and yearly backup strategy. AWS Backup [continuous backup](https://docs.aws.amazon.com/aws-backup/latest/devguide/point-in-time-recovery.html) is enabled with maximum retention (31 days) on [supported resources](https://docs.aws.amazon.com/aws-backup/latest/devguide/point-in-time-recovery.html#point-in-time-recovery-supported-services).
...
You can control backup vault notifications at individual vault-level using tags. You can opt out of notifications for a specific vault by adding the tag `AMSNotificationOptOut` and setting the value to `True` on a specific vault. To resume getting notifications from the vault, remove the tag.
 ...
 To view a list of your AMS backups, open the [AWS Backup console](https://console.aws.amazon.com/backup). In the navigation pane, choose **Backup vaults** and select the one of the AMS backup vaults from the following tables. In the **Backups** section, view the list of all the backups in the backup vault. Select a backup to edit, delete, or restore.
...
<a name="ams-backup-change-types"></a>
...
+ [ Backup plan: Create](https://docs.aws.amazon.com/managedservices/latest/ctref/deployment-aws-backup-plan-create.html)
+ [Backup Job: Start](https://docs.aws.amazon.com/managedservices/latest/ctref/management-aws-backup-job-start.html)
+ [Backup Job: Stop](https://docs.aws.amazon.com/managedservices/latest/ctref/management-aws-backup-job-stop.html)
+ [Recovery Point: Delete](https://docs.aws.amazon.com/managedservices/latest/ctref/management-aws-recovery-point-delete.html)
+ [DynamoDB \| Create from Backup](https://docs.aws.amazon.com/managedservices/latest/ctref/deployment-advanced-dynamodb-create-from-backup.html)
+ [EBS Volume: Create From Backup](https://docs.aws.amazon.com/managedservices/latest/ctref/deployment-advanced-ebs-volume-create-from-backup.html)
+ [ Amazon Elastic File System (EFS): Create From Backup](https://docs.aws.amazon.com/managedservices/latest/ctref/deployment-advanced-elastic-file-system-efs-create-from-backup.html)
