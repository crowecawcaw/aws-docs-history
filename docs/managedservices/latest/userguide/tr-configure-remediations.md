...
Configurations are stored in AWS AppConfig as part of the Trusted Remediator application. Each Trusted Advisor check category has a separate configuration profile. For more information on Trusted Advisor categories, see [View check categories](https://docs.aws.amazon.com/awssupport/latest/user/get-started-with-aws-trusted-advisor.html#view-check-categories).
...
+ **execution-mode:** Determines how Trusted Remediator performs default remediation:
  ...
  + **Manual:** An OpsItem is created, but the SSM document isn't executed automatically. You review the OpsItem and run remediation using the automated RFC. For more information, see [Work with remediations in Trusted Remediator](tr-remediation.md).
  + **Conditional:** Remediation is disabled by default. You can enable it for specific resources using tags. For more information, see the following sections [Customize remediation with resource tags](#tr-con-rem-customize-tags) and [Customize remediation with resource override tags](#tr-con-rem-resource-override).
...
+ **preconfigured-parameters:** Enter values for SSM document parameters that are required for automated remediation, in the format of `Parameter=Value` , separated by a comma (,). See [Trusted Advisor checks supported by Trusted Remediator](tr-supported-checks.md) for supported preconfigured parameters for the associated SSM document for each check.
...
**Note**  
The `alternative-automation-document` attribute doesn't support custom automation documents. You can use the existing supported Trusted Remediator automation documents listed in [Trusted Advisor checks supported by Trusted Remediator](tr-supported-checks.md).  
...
![An illustration of Trusted Remediator execution mode decision workflow.](https://docs.aws.amazon.com/managedservices/latest/userguide/images/tr-exe-mode-man-new.png)
...
1. Determine the Trusted Advisor check ID for the check that you want to override. You can find the check IDs for supported Trusted Advisor checks in [Trusted Advisor checks supported by Trusted Remediator](tr-supported-checks.md).

1. Add a tag to the resources with the following key and value using the [Tag \| Update](https://docs.aws.amazon.com/managedservices/latest/ctref/management-advanced-tag-update.html) or [Tag \| Bulk Update](https://docs.aws.amazon.com/managedservices/latest/ctref/management-advanced-tag-bulk-update.html) change type:
   ...
   + **Tag value:** Use one of the following values for the tag value:
     ...
     + **Manual:** An OpsItem is created for the resource, but remediation isn't performed automatically. You review and run the remediation using the automated. For more information, see [Work with remediations in Trusted Remediator](tr-remediation.md).
...
![An example of the Tags section on the console.](https://docs.aws.amazon.com/managedservices/latest/userguide/images/tr-tags-example.png)
