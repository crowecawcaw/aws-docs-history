...
- If iSCSI LUN is configured on top of the volume, you must manually expand the iSCSI LUN at the OS level after the automatic remediation completes.
  For more information, see [Why is my FSx for ONTAP LUN in read-only mode?](https://repost.aws/knowledge-center/fsx-ontap-lun-in-read-only-mode "https://repost.aws/knowledge-center/fsx-ontap-lun-in-read-only-mode")
