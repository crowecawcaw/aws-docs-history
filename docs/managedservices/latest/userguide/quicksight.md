...
Use AMS Self-Service Provisioning (SSP) mode to access Quick capabilities directly in your AMS managed account. Quick is a fast, cloud-powered business intelligence service that delivers insights to everyone
in your organization. As a fully managed service, Quick lets you easily create and publish interactive dashboards
that include machine learning (ML) insights.
To learn more, see [Amazon Quick](https://aws.amazon.com/quicksight/ "https://aws.amazon.com/quicksight/").
...
- AWS resource settings on Quick won’t be accessible to you
...
- Quick service offers two editions: Enterprise and Standard. Both provide a single sign-on
  ...
  Edition has an option to integrate Quick with Active Directory
  (AD). Quick on AMS does not support integration with AD due to
  incompatibilities between AMS account structure and the Quick trust requirements.
...
- When AMS receives this RFC to add Quick, you are sent a service request for
  ...
  - The AWS Region in which to enable the Quick service (defaults to your AMS AWS
    Region).
  - A notification email address for Quick account.
  - (Optional) The S3 bucket where data files to be analyzed are located.
  - The VPC and subnet IDs that connect to Quick support a feature to
    add a VPC connection, which enables private connectivity between Quick and resources inside
    the account.
