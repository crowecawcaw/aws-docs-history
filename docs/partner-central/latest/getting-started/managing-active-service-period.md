...
1. **Customer notification** – A new service period channel handshake is automatically created and the end-customer's AWS management account receives an email notification with a unique link to review the proposed replacement terms.

1. **Customer review and response** – An authorized user from the customer's AWS management account must:
...
1. **Seamless transition** – When the customer accepts the replacement handshake:
