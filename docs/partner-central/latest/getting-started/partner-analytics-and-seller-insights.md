# Tracking progress with Partner Analytics and Marketplace Insights

The Partner analytics and Marketplace insights dashboards provide Partners and Sellers with real-time insights into their product performance, including customer usage patterns, revenue trends, and subscription metrics through interactive visualizations and customizable reports. Sellers can track key performance indicators like monthly recurring revenue, customer acquisition costs, conversion rates, and geographic distribution of customers, enabling data-driven decisions to optimize their marketplace strategy.

###### Note

Partner analytics pages without sufficient data will not work as expected. Users will see an error. To determine if the error is due to IAM permissions vs insufficient data, users can navigate to specific dashboards with different data sources.

For more information, see [Navigating AWS Partner Central](../../../awsconsolehelpdocs/latest/gsg/using-search.md "../../../awsconsolehelpdocs/latest/gsg/using-search.md"). You can reference the [AWS managed policies for AWS Partner Central users](managed-policies.md "managed-policies.md") documentation to review access policies for the dashboards.

###### Topics

- [Partner Analytics Dashboard](partner-analytics.md "partner-analytics.md")
- [Seller Insights Dashboard](seller-insights.md "seller-insights.md")
- [Partner Analytics and Seller Insights Frequently Asked Questions (FAQs)](partner-analytics-faq.md "partner-analytics-faq.md")
