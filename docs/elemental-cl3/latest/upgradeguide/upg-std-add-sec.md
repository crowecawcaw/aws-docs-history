...
<a name="upg-std-add-sec"></a>

If you have only one Conductor Live, skip this step and go to [Step K: Start channels](upg-std-start.md).

Add the secondary node back to the cluster, and then to the redundancy group.

To avoid errors when you're adding the secondary Conductor Live back to the cluster, wait approximately three minutes after the upgrade before performing these steps. This wait ensures that the elemental\_se service has restarted and is running.
...
1. In the **Add Nodes to Cluster** pop-up, complete the node information for the secondary Conductor Live and choose **Add**.
...
1. In the **Add** pop-up, use the **Node** drop-down to select the secondary Conductor Live. Choose **Add**.
