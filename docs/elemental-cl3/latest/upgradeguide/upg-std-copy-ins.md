...
1. Copy the download file from your workstation to `/home/elemental/` on one of the nodes. For example:
   + Use SFTP protocol and an FTP client application on your workstation computer. Connect to the IP address for Elemental Live with the *elemental* user credentials and transfer the file.
