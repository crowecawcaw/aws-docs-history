...
<a name="conductor-live-config-nodes-add"></a>
...
If you are adding a secondary Conductor Live, make sure that the primary and secondary Conductor Live nodes have the same [firewall settings](network-firewall.md). If they don't, you won't be able to add the secondary Conductor Live. 
...
1. On the **Nodes** page, choose **Add Node**. The **Add Nodes to Cluster** dialog appears showing two fields:
   + **Node IP Addresses**
   + **Lookup Node IP Address**

1. Specify the addresses of the nodes to add. You can enter one address, several comma-delimited addresses, or a range of addresses. If your network has a DNS server, you can look up an address by entering a hostname.

1. When you've entered all the nodes to add, choose **Add**. 
