...
- If the node is a Conductor Live node and if HA is currently enabled, disable it now. Conductor Live
  ...
  before you configure network interfaces. For instructions, see [Disabling Conductor Live HA (high availability)](conductor-live-config-ha-chg.md "conductor-live-config-ha-chg.md").
