...
<a name="config-conductor-live-checklist-add-redundancy"></a>
...
1. As your first step, you should identify your redundancy requirements. See [Designing the cluster](ready-conductor-live-cg.md).

1. [Verify the firewall setup](network-firewall.md) is the primary Conductor Live:
   + Make sure that the firewall is enabled on both Conductor Live nodes. 
   + Accept port 5432 TCP. 

1. Configure the secondary Conductor Live, perform the following tasks. Perform these tasks in any order:
   + [Configure DNS servers](config-cluster-dns.md).
   + [Configure Ethernet interfaces](config-conductor-live-config-ethernet-add.md) and bonds (optional).
...
1. [Recruit (add)](conductor-live-config-nodes-add.md) the secondary Conductor Live into the existing cluster.
