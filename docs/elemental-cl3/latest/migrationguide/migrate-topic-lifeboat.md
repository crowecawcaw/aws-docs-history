...
# Backing up data
<a name="migrate-topic-lifeboat"></a>

You back up data using the special lifeboat script.

**Topics**
+ [About the backup process](#migrate-topic-lifeboat-backup-about)
+ [Step A: Verify hostnames](#migrate-topic-verify-hostnames)
+ [Step B: Download the lifeboat script](#migrate-topic-lifeboat-download)
+ [Step C: Create the backup](#migrate-topic-lifeboat-backup)
...
<a name="migrate-topic-lifeboat-backup-about"></a>
...
<a name="migrate-topic-verify-hostnames"></a>

RHEL 9 (specifically `systemd`) doesn't support underscores in hostnames. If any of the nodes in the cluster contain underscores, there are two ways to proceed:
...
<a name="migrate-topic-lifeboat-download"></a>

Perform this procedure on every node in the cluster, to copy the lifeboat script onto every node.

1. Download the latest version of the lifeboat script from [https://a.co/ElementalRHEL9Lifeboat](https://a.co/ElementalRHEL9Lifeboat) to your laptop. The lifeboat file is called `elemental_lifeboat_el.tar`.
**Important**  
...
1. Copy the lifeboat file to the `/home/elemental` directory on every node in the cluster.

1. From the Linux prompt, use the *elemental* user to start a remote terminal session with the node. Don’t log in as sudo.

1. Untar the lifeboat file:

   ```
   [elemental@hostname ~]$ cd /home/elemental && tar xvf elemental_lifeboat_el9.tar
   ```

1. Change to the `elemental_lifeboat_el9` directory:
...
<a name="migrate-topic-lifeboat-backup"></a>

**Important**  
Make sure that you have stopped the node. We recommend that you don't run the script on an active node. The script temporarily stops elemental\_se and httpd services. 

### Step C1: Run the backup command
<a name="migrate-topic-lifeboat-backup-create"></a>
...
<a name="migrate-topic-lifeboat-backup-store.title"></a>
...
<a name="migrate-topic-lifeboat-backup-verify"></a>

Verify the integrity of the backup archive. This step is optional but we strongly recommend that you follow it because the [restore operation](migrate-topic-restore-database.md) that you later perform might fail if the backup file is corrupted. 
...
1. Enter the `cat` command to view the checksums currently listed in the checksum file.:
   ...
   The `cat` command simply displays the file contents on your screen. 

1. Now run a checksum command on the `lifeboat-archive.zip` file. 
...
1. Compare the results from step 1 to the results from step 2. If the checksums don’t match, copy the archive file again. 