...
<a name="migrate-split-c-upgrade-primary"></a>
...
1. In [Step C: Split the cluster](migrate-split-c-split.md), you created a backup of the database on node X. You can now restore the backup onto the node. See [Restoring the database](migrate-topic-restore-database.md). 
