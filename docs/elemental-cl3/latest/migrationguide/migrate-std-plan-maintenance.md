...
- [Step F: Rebuild the cluster](migrate-std-rebuild-cluster.md "migrate-std-rebuild-cluster.md")
  These steps upgrade all the nodes in one maintenance window. You must perform the
  upgrade in this way because you can't have a cluster where some nodes are on the
  previous version of the AWS Elemental software and some are on the new version.
