...
1. Disable high availability (HA) on the cluster. You must disable HA before you
   can remove the secondary Conductor node. See [Enabling or disabling high availability (HA)](migrate-topic-disable-ha.md "migrate-topic-disable-ha.md"). After you disable HA, only the primary Conductor can control the cluster.
...
to the secondary Conductor node. See [Removing a Conductor node from the cluster](migrate-topic-remove-c-node.md "migrate-topic-remove-c-node.md").
...
Conductor node. See [Removing a worker node from the cluster](migrate-topic-remove-worker.md "migrate-topic-remove-worker.md").
