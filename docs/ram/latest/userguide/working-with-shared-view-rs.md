...
4. The AWS RAM console displays the following information:
   ...
   - **ID** – The ID of the resource share. Choose
     the ID to view the details page for the resource share.
   - **Owner** – The ID of the
   ...
   - **Status** – The current status of the
     ...
     - `Pending` – An invitation to accept
       the resource share is waiting for a response.

AWS CLI

###### To view the resource shares

Use the [get-resource-shares](../../../cli/latest/reference/ram/get-resource-shares.md "../../../cli/latest/reference/ram/get-resource-shares.md") command with the
`--resource-owner` parameter set to
`OTHER-ACCOUNTS`.

The following example shows the list of resource shares shared in the specified
AWS Region with the calling account by other AWS accounts.
