...
For details about actions and resource types defined by MediaConvert, including the format of the ARNs for each of the resource types, see [Actions, resources, and condition keys for AWS Elemental MediaConvert](../../../service-authorization/latest/reference/mediaconvert.md "../../../service-authorization/latest/reference/mediaconvert.md") in the _Service Authorization Reference_.
...
To ensure that users and roles can still use the MediaConvert console, also attach the
MediaConvert `ConsoleAccess` or `ReadOnly` AWS managed policy to
...
The following sample policy grants the IAM user permissions to all AWS Elemental MediaConvert
actions (such as `ListJobs`, `CreateJob`, and so on) on all
MediaConvert resources (such as jobs, queues, and output presets). It also grants the IAM
actions required for the IAM user to specify the service role that MediaConvert will assume in
order to run the job. It also grants Amazon S3 actions that allow the IAM user to use the
**Browse** button to choose input and output locations. The Amazon S3
permissions aren't required to run the job; without them, the IAM user can specify the URL of
the bucket instead. In this example, the account number is
...
`012345678910`. It allows the following actions, in any partition
and Region:
...
- Create a job and submit it to the queue "myQueue", referencing any presets with
  names that start with "allow" and referencing any job template.
- Create a job template referencing the queue "myQueue" and any presets with names
  that start with "allow".

###### Note

You can't grant cross-account permissions to AWS Elemental MediaConvert resources, such as
queues, output presets, and jobs. You can grant cross-account permissions to your input
and output media files stored in Amazon S3.
...
`DenyMediaConvertWithResourceTag` is tagged with the key
`access` and has a value `denied` or a value that starts with
the string "`deny`".

###### Note

This policy demonstrates the IAM principle that, in the case where policies
both allow and deny action on a resource, the denial takes precedence. Therefore, the
IAM user with this attached policy can't perform the denied actions, even if they have a
different policy that allows administrative permissions to all MediaConvert
resources.
