...
| `warnings` code count              | array integer integer | Any warning codes seen in the job and the number of times they occurred. For more information, see [Warning codes](warning_codes.md "warning_codes.md").                                                                                                                                                                                                                |
...
| `averageBitrate`                   | integer               | The average bitrate of your video output, calculated by dividing the duration by the total bits.                                                                                                                                                                                                                                                                        | You can use the following sample JSON to create an EventBridge event pattern for jobs with a status of `COMPLETE`. `{ "source": ["aws.mediaconvert"], "detail-type": ["MediaConvert Job State Change"], "detail": { "status": ["COMPLETE"] } }` |
