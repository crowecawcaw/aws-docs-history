...
XML documents in your AWS Elemental MediaConvert job settings. When you do, MediaConvert
conditions your outputs
...
manifests for your HLS outputs accordingly.

###### Note

To put SCTE-35 markers in your MPEG2-TS outputs, in addition to supplying the ESAM
XML documents, you must also enable **ESAM SCTE-35** on each
output. For more information, see the console procedure following this
overview.

###### About Timecodes in Your ESAM Documents

Specify the insertion points in your XML documents relative to the timing of the
final output, after input clipping and stitching. Start your timing from
00:00:00:00, regardless of your timecode settings. Use the following 24-hour format with a frame number: HH:MM:SS:FF.

For example, a job has the following three inputs: a five-minute preroll, a one-hour
film, and a five-minute postroll. You use input clipping to clip adjust the final 20
minutes of your one-hour input. So your output with preroll and postroll is 30 minutes
long. If you want your first insertion point to appear three minutes into the main
content, you would specify it at eight minutes—three minutes after your
five-minute preroll.
...
4. In the **Job** pane on the left, in the **Job
   settings** section, choose **Settings**.
...
By default, MediaConvert adds a four-second preroll to the ESAM payload.
This might result in MediaConvert placing the SCTE-35 message one segment
earlier than the cue marker designates in the HLS manifest. To remove the
preroll, set `responseSignalPreroll` to zero. This setting is a
child of [EsamSettings](../apireference/jobs-id.md#jobs-id-model-esamsettings "../apireference/jobs-id.md#jobs-id-model-esamsettings"). 7. If you want to include information about your SCTE-35 markers in
your HLS manifests, for **Manifest confirm condition notification
XML**, enter your ESAM manifest conditional XML document as text.
MediaConvert doesn't include information about your SCTE-35 markers in your
DASH manifests.

To insert SCTE-35 markers in the transport stream in addition to the manifest,
set the `dataPassThrough` attribute in your MCC document to
...
markers:
     ...
     **Output groups**, **File group**,
     choose the output.
    2. Confirm that it is an MPEG2-TS output. In the **Output
     settings** section, make sure that
     **Container** is set to **MPEG-2 Transport
     Stream**.
    3. Choose **Container settings**, and then scroll down
     to find the **PID controls** section.
    4. For **ESAM SCTE-35** choose
     **Enabled**.
    5. For **SCTE-35 source**, keep the default
     **None**.
...
in their default state:
    ...
    1. Make sure that **Manifest confirm condition notification
     XML**, discussed in a previous step of this procedure, is
     empty.
    2. For each **Apple HLS** output group in your job,
     confirm that you have kept **Ad Markers**
     unchecked.
    	 ...
    	 **Output groups**, choose **Apple
    	 HLS**.
    	2. In the **Apple HLS group settings** section,
    	 choose **Advanced**.
    	3. In the **Ad markers** section, clear the
    	 **Elemental** and **SCTE-35
    	 enhanced** check boxes.
    3. For each output in your **Apple HLS** output groups,
     confirm that **SCTE-35 source** is set to
     **None**:
...
1. Include the [`esam` property](../apireference/jobs.md#jobs-prop-jobsettings-esam "../apireference/jobs.md#jobs-prop-jobsettings-esam") and its children at the root of the
   ...
   2. Optionally, include a manifest confirm condition XML notification
      document as a string in the setting `mccXml`:
...
2. For each M2TS (MPEG2 Transport Stream) output in your job, set your JSON job
   specification as shown in the following example. Include the property [`scte35Esam`](../apireference/jobs.md#jobs-prop-m2tssettings-scte35esam "../apireference/jobs.md#jobs-prop-m2tssettings-scte35esam"). Set [`scte35Source`](../apireference/jobs.md#jobs-prop-m2tssettings-scte35source "../apireference/jobs.md#jobs-prop-m2tssettings-scte35source") to `NONE`.
...
4. Submit your job as usual.

For information about submitting AWS Elemental MediaConvert jobs programmatically, see
[Getting started with AWS Elemental MediaConvert using the AWS SDKs or the AWS CLI](../apireference/custom-endpoints.md "../apireference/custom-endpoints.md") and [Getting started with AWS Elemental MediaConvert using the API](../apireference/getting-started.md "../apireference/getting-started.md").
