...
- Sidecar captions container options.
  If your output captions are IMSC, TTML or WebVTT format, set them up in your
...
For **DASH ISO** output groups, you can choose from
these:

- Fragmented MP4 (`.fmp4`)
- Raw (`.xml` for IMSC, `.ttml`
  for TTML)
...
3. In the **Output settings** section on the right,
   choose **Container settings**, and then enable
   **DASH container settings**.
4. For **Captions container**, keep the default
   **Raw** or choose **Fragmented
   MPEG-4**.
