...
begins at 75 seconds in. These cue manifest tags also contain data that can be used
by a downstream client for ad replacement and insertion. Outputs generated with this
MCC document contain a SCTE-35 message embedded into the transport stream file at
the first ad break, but not at the second one.

Note the `dataPassThrough` attribute on the first ad break. When this
attribute is present and set to `"true"`, MediaConvert inserts SCTE-35
markers in the transport stream in addition to the manifest. For any ad break that
you want marked only in the manifest, keep out the `dataPassThrough`
attribute.

MediaConvert supports manifest conditioning with ESAM only in HLS
packages.
