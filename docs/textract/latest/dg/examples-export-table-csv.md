...
These Python examples show how to export tables from an image of a document into a
...
- `get_rows_columns_map` – Gets the rows and
...
3. At the command prompt, enter the following command. Replace
   `file` with the name of the document image file that
   you want to analyze.
...
Asynchronous
In this example, you will use make use of two different scripts. The first
script starts the process of asynchronoulsy analyzing documents with
`StartDocumentAnalysis` and gets the `Block`
information returned by `GetDocumentAnalysis`. The second script
takes the returned `Block` information for each page, formats the
data as a table, and saves the tables to a CSV file.
...
3. In the following code example, replace the value of
   `roleArn` with the Arn assigned to the role that you
   created in Step 2. Replace the value of `bucket` with the
   name of the S3 bucket containing your document. Replace the value of
   `document` with the name of the document in your S3
   bucket. Replace the value of `region_name` with the name
...
4. Run the code. The code will print a JobId. Copy this JobId
   down.
5. Wait for your job to finish processing, and after it has
   ...
   value of `file_name` with the name you want to give the
   output CSV.
...
After you have obtained you results, be sure to delete the
associated SNS and SQS resources, or else you may accrue charges for
them.
