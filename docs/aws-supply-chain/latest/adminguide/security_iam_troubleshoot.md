...
have the fictional `scn:`GetWidget``
permissions.
...
AWS Supply Chain. However, the action requires the service to have permissions that are granted by a service role. Mary does not have permissions to pass the
role to the service.
...
- To learn whether AWS Supply Chain supports these features, see [How AWS Supply Chain works with IAM](security_iam_service-with-iam.md "security_iam_service-with-iam.md").
