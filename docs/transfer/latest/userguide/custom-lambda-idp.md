...
| `HomeDirectory` | The landing directory for a user when they log in to the server using the client. The format depends on your storage backend:+  **For Amazon S3:** `/bucket-name/user-home-directory` <br />Example: `/amzn-s3-demo-bucket/users/john` <br />+  **For Amazon EFS:** `/fs-12345/user-home-directory` <br />Example: `/fs-faa1a123/users/john`  The bucket name or Amazon EFS file system ID must be included in the path. Omitting this information will result in "File not found" errors during file transfers.  | Optional | 
