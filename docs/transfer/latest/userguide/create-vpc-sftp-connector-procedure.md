...
This topic provides step-by-step instructions for creating SFTP connectors with VPC connectivity. VPC\_LATTICE-enabled connectors use Amazon VPC Lattice to route traffic through your Virtual Private Cloud, enabling secure connections to private endpoints or using your own NAT gateways for internet access.
...
Before creating a VPC\_LATTICE-enabled SFTP connector, you must complete the following prerequisites:
...
When you create a VPC\_LATTICE-enabled connector, AWS Transfer Family uses Forward Access Session (FAS) to temporarily obtain your credentials and associate your Resource Configuration with our service network.
...
1. Choose **Create SFTP connector** to create the VPC\_LATTICE-enabled SFTP connector.
...
VPC\_LATTICE-enabled connectors have an asynchronous setup process. After creation, monitor the connector status:
