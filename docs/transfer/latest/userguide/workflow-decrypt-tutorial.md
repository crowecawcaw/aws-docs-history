...
+ [Step 3: Add the workflow to a server and create a user](#add-workflow-to-server)
...
Create an AWS Identity and Access Management (IAM) execution role that Transfer Family can use to launch a workflow. The process of creating an execution role is described in [IAM policies for workflows](workflow-execution-role.md).
...
1. In the left navigation pane, choose **Workflows**, and then choose **Create workflow**. 

1. Enter the following details:
   + Enter a description, for example **Decrypt workflow example**.
   + In the **Nominal steps** section, choose **Add step**.

1. For **Choose step type**, choose **Decrypt file**, and then choose **Next**.

1. <a name="configure-destination-details"></a>In the **Configure parameters** dialog box, specify the following:
...
1. Review the details for the step. If everything is correct, choose **Create step**.

1. Your workflow needs only the single decrypt step, so there are no additional steps to configure. Choose **Create workflow** to create the new workflow.
...
1. In the left navigation pane, choose **Servers**, and then choose a server from the list. Make sure that this server supports the SFTP protocol.

1. On the details page for the server, scroll down to the **Additional details** section, and then choose **Edit**. 

1. On the **Edit additional details** page, in the **Managed workflows** section, choose your workflow, and choose a corresponding execution role.
   + For **Workflow for complete file uploads**, choose the workflow that you created in [Step 2: Create a managed workflow](#create-example-workflow), for example, **w-1234abcd5678efghi**.
   + For **Managed workflows execution role**, choose the IAM role that you created in [Step 1: Configure an execution role](#create-example-execution-role).

1. Scroll to the bottom of the page, and choose **Save** to save your changes.
...
1. In the left navigation pane, choose **Servers**, and then choose the server that you're using for the decrypt workflow.

1. On the server details page, scroll down to the **Users** section, and choose **Add user**. 
...
1. For your new user, enter the following details:
   ...
   + For **SSH public keys**, paste in a public key that corresponds to a private key that you have. For details, see [Generate SSH keys for service-managed users](sshkeygen.md).

1. Choose **Add** to save your new user.
...
Use one of the [supported PGP clients](pgp-key-clients.md) to generate a PGP key pair. This process is described in detail in [Generate PGP keys](generate-pgp-keys.md).
...
1. Export the private key by running the following command. Replace `{{user@example.com}}` with the email address that you used when you generated the key.
...
1. Choose **Add row**, and in the **Key/value pairs** section, choose the **Key/value** tab.
   + **Key** – Enter **PGPPassphrase**.
   + **value** – Enter the passphrase that you used when you generated your PGP key pair in [Step 4: Create a PGP key pair](#create-example-pgp-key-pair).

1. Choose **Next**.

1. On the **Configure secret** page, enter a name and description for your secret. You can create a secret for a specific user or one that can be used by all users. If your server ID is {{`s-11112222333344445`}}, you name the secret as follows.
...
For more information about adding your PGP private key to Secrets Manager, see [Use AWS Secrets Manager to store your PGP key](manage-pgp-keys.md#store-pgp-key-details).
...
+ The `--openpgp` flag is optional. This flag makes the encrypted file conform to the [OpenPGP RFC4880](https://www.rfc-editor.org/rfc/rfc4880) standard.
...
To run the workflow, you connect to the Transfer Family server with the user that you created in Step 3. Then you can look in the Amazon S3 bucket that you specified in [Step 2.5, configure destination parameters](#configure-destination-details) to see the decrypted file.

**To run the decrypt workflow**

1. Open a command terminal.

1. Run the following command, replacing `{{your-endpoint}}` with your actual endpoint, and `{{transfer-key}}` with your user's SSH private key:
...
1. Run the `pwd` command. If successful, this command will return the following:
...
1. For the destination of the decrypted files, you specified the `decrypted-files/` folder when you created the workflow. Now, you can navigate to that folder and list the contents.
