...
To look up your account-specific endpoint, use the [aws iot describe-endpoint --endpoint-type iot:Data-ATS](../../../cli/latest/reference/iot/describe-endpoint.md "../../../cli/latest/reference/iot/describe-endpoint.md")
command.
...
Legacy Verisign endpoints are currently supported for [some Regions](#greengrass-legacy-endpoints "#greengrass-legacy-endpoints"), but we recommend
that you use ATS endpoints with ATS root certificate authority (CA)
...
The following table contains AWS Region-specific ATS endpoints for device
discovery operations using the [AWS IoT Greengrass
Discovery API](../../../greengrass/v1/developerguide/gg-discover-api.md "../../../greengrass/v1/developerguide/gg-discover-api.md"). This is a data plane API.
...
Legacy Verisign endpoints are currently supported for [some Regions](#greengrass-legacy-endpoints "#greengrass-legacy-endpoints"),
but we recommend that you use ATS endpoints with ATS root CA certificates.
For more information, see
[Server authentication](../../../iot/latest/developerguide/server-authentication.md "../../../iot/latest/developerguide/server-authentication.md")
...
CA certificates. For backward compatibility, AWS IoT Greengrass currently supports legacy Verisign
endpoints in the following AWS Regions. This support is expected to end in the
future. For more information, see [Server authentication](../../../iot/latest/developerguide/server-authentication.md "../../../iot/latest/developerguide/server-authentication.md") in the
_AWS IoT Developer Guide_.
...
To look up your account-specific legacy endpoint, use the [aws iot describe-endpoint --endpoint-type iot:Data](../../../cli/latest/reference/iot/describe-endpoint.md "../../../cli/latest/reference/iot/describe-endpoint.md") command.
...
| Maximum number of transactions per second (TPS) on the AWS IoT Greengrass<br>APIs. | See [TPS](#gg_core_limits_tps "#gg_core_limits_tps"). |
| Maximum number of subscriptions per group.                                         | 10000                                                 |
| Maximum number of subscriptions that specify `Cloud`<br>as the source per group.   | 50                                                    |
| Maximum length of a core thing name.                                               | 124 bytes of UTF-8 encoded characters.                |

#### TPS

The default quota for the maximum number of transactions per second on the
AWS IoT Greengrass APIs depends on the API and the AWS Region where AWS IoT Greengrass is used.
...
`ListFunctionDefinitions`) has a quota of 30 TPS. This includes
...
| Description                                                                    | Default                                                                                                                                                           |
| ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Maximum number of routing table entries that specify<br>`Cloud` as the source. | 50 (matches AWS IoT subscription quota)                                                                                                                           |
...
| Maximum number of forward slashes (/) in a topic or topic<br>filter.           | 7                                                                                                                                                                 |
| Minimum disk space needed to run the Greengrass Core<br>software.              | 128 MB<br>400 MB when using [OTA<br>updates](../../../greengrass/v1/developerguide/core-ota-update.md "../../../greengrass/v1/developerguide/core-ota-update.md") |
| Minimum RAM to run the Greengrass Core software.                               | 128 MB<br>198 MB when using [stream manager](../../../greengrass/v1/developerguide/stream-manager.md "../../../greengrass/v1/developerguide/stream-manager.md")   |

The Greengrass Core software provides a service to detect the IP addresses of your
Greengrass core devices. It sends this information to the AWS IoT Greengrass cloud service and
allows AWS IoT devices to download the IP address of the Greengrass core they need to
connect to.

Do not use this feature if any of the following is true:

- The IP address of a Greengrass core device changes frequently.
- The Greengrass core device is not always available to AWS IoT devices in its
  group.
- The Greengrass core has multiple IP addresses and an AWS IoT device is
  unable to reliably determine which address to use.
- Your organization's security policies don't allow you to send devices' IP
  addresses to the AWS Cloud.
