...
## Prerequisites
<a name="aws-agent-registry-prerequisites"></a>
...
+ **Administrator access** – The administrator signs in to the Quick administration console with an IAM identity that has permission to list and view registries. For more information, see [IAM permissions](#aws-agent-registry-permissions).

## IAM permissions
<a name="aws-agent-registry-permissions"></a>
...
<a name="aws-agent-registry-admin-setup"></a>
...
1. Review the list of AWS Agent Registries in your account. Only registries that meet the requirements in [Prerequisites](#aws-agent-registry-prerequisites) appear.

1. Turn on the toggle for the registry that you want to connect. Only one registry can be active for each account.

1. Confirm the selection. Quick grants the account's service role the read-only Agent Registry permissions that it needs and links the registry to your Quick account.
...
<a name="aws-agent-registry-user-setup"></a>

After an administrator links a registry, you can create a connector from it.

1. Open the **Connectors** page.

1. Choose the **Create for your team** tab. Registry-sourced connectors appear on this tab, not on the **Available** tab.

1. Find the MCP connector sourced from the registry by searching or browsing. Each connector comes pre-populated with its MCP server URL, name, and description.

1. Choose the connector to begin setup.

1. Complete the **Authenticate** step by configuring or signing in with the required credentials, such as OAuth. The Agent Registry does not store authentication credentials, so each user provides or authenticates with their own credentials.

1. On the **Manage permissions** step, choose which tools to turn on or turn off.

1. Choose **Create and continue**.

1. Share the connector with the appropriate user groups.

**Note**  
Registry-sourced connectors are classified as **Custom MCP** connectors. This classification matters for settings such as [custom permissions](https://docs.aws.amazon.com/quick/latest/userguide/custom-permissions.html).
...
To edit, share, or delete a connector that you created from the Agent Registry, see [Managing existing integrations](integration-workflows.md#managing-existing-integrations).
...
For more information about account, Region, authorization, and registry-count requirements, see [Prerequisites](#aws-agent-registry-prerequisites).

## IAM policy reference
<a name="aws-agent-registry-policy-reference"></a>
