...
<a name="standalone-billing-access"></a>

The Plan & Billing page is located in the Amazon Quick account settings portal.
...
1. In the left navigation of the Manage Account page, choose **Plan and billing**.

   The Plan & Billing settings page opens in a new browser tab at `account.global.app.aws`.

The settings page has its own navigation panel with the following sections:
+ **Plan & Billing** – View your current plan, estimated charges, and billing history.
+ **My account** – View your account profile and account details. For more information, see [Account settings for Amazon Quick Free and Plus accounts](https://docs.aws.amazon.com/quicksuite/latest/userguide/standalone-account-settings.html).
+ **Support** – Access AWS Support (opens externally).
...
+ **Payment method** – The card type and last four digits of the payment method on file. A **Change** link is available to update your payment method.
+ **Estimated charges breakdown** – An expandable section that provides a detailed breakdown of the charges for the current billing period.

## Billing history
<a name="standalone-billing-history"></a>

The **Billing history** section displays a table of past billing records with the following columns:


| Column | Description | 
| --- | --- | 
| ID | A unique identifier for the billing record. | 
| Type | The type of charge (for example, subscription). | 
| Status | The payment status (for example, paid, pending). | 
...
| Action | Available actions for the billing record, such as downloading an invoice. | 

**Note**  
New accounts will not have any billing records until the first billing cycle completes. The table displays "No billing records to display" until charges are processed.

## Billing preferences
<a name="standalone-billing-preferences"></a>
