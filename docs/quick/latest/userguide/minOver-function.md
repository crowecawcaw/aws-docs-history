...
The `minOver` function calculates the minimum of a measure or date
...
The following example calculates the min `sum(Sales)`, partitioned by
`City` and `State`.
...
The following example shows the minimum `Billed Amount` over
