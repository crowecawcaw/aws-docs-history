...
authentication, skip this section and see [Setting up the connector in Amazon Quick](#google-drive-action-quicksuite-setup "#google-drive-action-quicksuite-setup").
...
If a Google Drive connector already exists, a dialog
    ...
    2. For **Custom OAuth app**,
     configure the following fields:
    	...
    	* **Base URL** (Optional) – The
    	 Google Drive API base URL. For example:
...
- **Google Drive API not
  enabled** – Verify that the Google Drive API
