...
The `periodToDateStDevP` function calculates the population standard
deviation of the set of numbers in the specified measure, for a given time granularity
(for instance, a quarter) up to a point in time, based on a sample in that
period.

## Syntax

```
periodToDateStDevP(*measure,
 dateTime,
 period,
 endDate (optional)*)
```
...
![This is an image of the results from the example calculation.](images/PTDStDevPResults.png)
