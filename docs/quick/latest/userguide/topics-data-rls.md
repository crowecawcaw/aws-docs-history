...
For more information about adding RLS to a dataset, see [Using row-level security in Amazon Quick](row-level-security.md "row-level-security.md"). 3. On the data preparation page, choose the field menu (the three dots) for a
field that you want to allow , and then choose **Row level security** . 4. On the **Row level security for Quick** page
