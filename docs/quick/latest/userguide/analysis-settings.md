...
+ Use this setting to control the message displayed when a visual in the analysis returns no data. Customizing this can give readers of the dashboards reasons why no data is shown and potential fixes. Parameters are supported for all text input.
+ **Title and subtitle** – Use these settings to customize the text shown on the message title or subtitle and to toggle visibility of either option.
