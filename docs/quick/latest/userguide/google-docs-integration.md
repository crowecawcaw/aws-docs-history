...
Amazon Quick supports multiple authentication methods for Google Docs. Choose
...
authentication, skip this section and see [Setting up the connector in Amazon Quick](#google-docs-quicksuite-setup "#google-docs-quicksuite-setup").
...
and enable the **Google Docs API**.
...
3. Find and choose **Google Docs**.

###### Note

If a Google Docs connector already exists, a dialog
    ...
    2. For **Custom OAuth app**,
     configure the following fields:
    	...
    	* **Base URL** (Optional) – The
    	 Google Docs API base URL. For example:
...
The actions that you can use depend on the documents accessible
...
- **Google Docs API not
  enabled** – Verify that the Google Docs API
