...
You set the geospatial type on the **Output** tab, not through **Change data type** on the **Transform** tab. Geospatial types (such as latitude and longitude) are not part of the regular data types. Because Amazon Quick Sight does not apply further transformations to these columns, it places them on the **Output** tab.

To set the geospatial type of a column:

1. On the data preparation page, choose the **Output** tab at the top (next to **Transform**).

1. In the **Schema** pane on the left, choose the column you want to set (for example, **City**).

1. Open the **Geospatial type** dropdown and choose a value, such as **Country**, **State**, **County**, **City**, **Postcode**, **Latitude**, or **Longitude**. To clear the geospatial type, choose **Unspecified**.
...
| Configuration Options | ["Start from row" in file upload settings](choosing-file-upload-settings.md) | 
