...
The SPICE ingestion history page includes the following
information:
...
- The job type for the refresh: scheduled, full refresh, and so on
  Use the following procedure to view a dataset's SPICE ingestion
  history.

###### To view a dataset's SPICE ingestion history

1. From the homepage, choose **Data** at left.
2. On the **Datasets** tab, choose the dataset that you want to
   examine.
3. On the dataset details page that opens, choose the
   **Refresh** tab.

SPICE ingestion history is shown at bottom. 4. (Optional) Choose a time frame to filter the entries from the last hour to the
last 90 days. 5. (Optional) Choose a specific job status to filter the entries, for example
**Running** or **Completed**. Otherwise,
you can view all entries by choosing **All**.
