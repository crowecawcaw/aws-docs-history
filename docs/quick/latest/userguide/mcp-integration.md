...
- For custom MCP connectors, tool lists do not update automatically. To pick up server-side tool changes, open the connector details page and choose **Sync**. Built-in MCP connectors sync automatically. For more information, see [MCP Sync](#mcp-integration-sync "#mcp-integration-sync").
