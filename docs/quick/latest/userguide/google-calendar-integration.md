...
Amazon Quick supports multiple authentication methods for Google Calendar.
...
authentication, skip this section and go to [Setting up the connector in Amazon Quick](#google-calendar-quicksuite-setup "#google-calendar-quicksuite-setup").
...
3. Find and choose **Google Calendar**.

###### Note

If a Google Calendar connector already exists, a dialog
    ...
    2. For **Custom OAuth app**,
     configure the following fields:
    	...
    	* **Base URL** (Optional) – The
    	 Google Calendar API base URL. Example:
...
The actions that you can use depend on the calendars accessible
...
- **Google Calendar API not
  enabled** – Verify that the Google Calendar
