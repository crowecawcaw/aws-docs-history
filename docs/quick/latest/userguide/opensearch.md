...
Quick administrator to complete with the cooperation of a person who has
...
- The start hour for the daily automated snapshot (if your users want to
  know).
  Before you proceed, the Amazon Quick administrator enables authorized connections from
  Amazon Quick to OpenSearch Service. This process is required for every AWS service that you connect
  to from Amazon Quick. You need to do this only once per AWS account for each AWS
...
1. Within Amazon Quick, choose **Administrator** and
   **Manage Amazon Quick**.
...
permissions for Amazon Quick to access OpenSearch.
...
the Amazon VPC service. If so, make sure to determine if Amazon Quick is already
...
If a connection isn't already defined in Amazon Quick for the VPC that you want to
use, you can create one. This task is a multistep process that you need to complete
before you proceed. To learn how to add Amazon Quick to a VPC and add a connection
from Amazon Quick to the VPC, see [Connecting to a Amazon VPC with Amazon Quick](../../../quicksight/latest/user/working-with-aws-vpc.md "../../../quicksight/latest/user/working-with-aws-vpc.md").

## Using OpenSearch permissions

After you configure Amazon Quick to connect to OpenSearch Service, you might need to enable
...
domain is accessible from Amazon Quick. Perform these steps for each domain that you
want to use.
...
5. In the **Backend roles** section, enter the ARN of the
   AWS-managed IAM role for Amazon Quick. Following is an example.
