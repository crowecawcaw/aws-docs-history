...
Amazon Quick supports multiple authentication methods for Gmail. Choose the
...
authentication, skip this section and see [Setting up the connector in Amazon Quick](#gmail-quicksuite-setup "#gmail-quicksuite-setup").
...
3. Find and choose **Gmail**.

###### Note

If a Gmail connector already exists, a dialog appears with
    ...
    2. For **Custom OAuth app**,
     configure the following fields:
    	...
    	* **Base URL** (Optional) – The
    	 Gmail API base URL. For example:
...
The actions that you can use depend on the data accessible to the
authenticated user.
...
- **Gmail API not enabled** –
  Verify that the Gmail API is enabled in your Google Cloud
  project under **APIs & Services**,
  **Library**.
