...
authentication, skip this section and proceed to [Setting up the connector in Amazon Quick](#onedrive-action-integration-setup "#onedrive-action-integration-setup").
...
This integration uses delegated permissions, which allow the app to act on
behalf of a signed-in user. For more information, see [Overview of Microsoft Graph permissions](https://learn.microsoft.com/en-us/graph/permissions-overview "https://learn.microsoft.com/en-us/graph/permissions-overview") in the Microsoft
documentation.
...
2. Choose **Add a permission**, then choose
   **Microsoft Graph**.
3. Choose **Delegated permissions** and add the
...
If a Microsoft OneDrive connector already exists, a dialog
...
**Perform actions in Microsoft OneDrive** and
choose **Next**. 5. Enter a **Name** for your connector. Optionally,
...
[Admin consent for Microsoft 365](#onedrive-action-admin-consent "#onedrive-action-admin-consent"). 10. On the **Review** page, review the available
...
access Microsoft OneDrive on behalf of the signed-in user. Most users can
...
- **What to do:** Grant admin consent
  for the Amazon Quick Microsoft OneDrive integration
  application.
- **Why:** Amazon Quick needs
  delegated access to OneDrive files, folders, and Excel workbooks
