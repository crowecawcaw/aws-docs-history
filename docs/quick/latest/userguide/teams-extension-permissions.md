...
The Amazon Quick Microsoft Teams extension uses
Microsoft's Resource-Specific Consent (RSC) framework. The bot only
gets access to the specific meetings and chats where it is installed — not
tenant-wide access to all Teams data. These permissions are granted
automatically when the app is added to a meeting or chat; no additional admin action
is needed.

The following table lists the RSC permissions used by the Teams
extension.

| Permission                             | Purpose                                                                            | Feature                |
| -------------------------------------- | ---------------------------------------------------------------------------------- | ---------------------- |
| `ChatMessage.Read.Chat`                | Read chat messages for AI assistant context                                        | General chat assistant |
| `ChatMember.Read.Chat`                 | Resolve attendee emails and identify the meeting organizer for<br>summary delivery | Meeting Summary        |
...
| `ChannelMessage.Read.Group`            | Read thread context when the bot is @mentioned in a<br>channel                     | General chat assistant |
...
After installation, go to Microsoft Entra >
**Enterprise Applications**, find the app starting with
"qbs" and ending with "teams", navigate to
**Permissions**, and revoke
`AppCatalog.ReadWrite.All`.

## Where to review permissions

Use the following locations to review permissions for the Microsoft
Teams extension:
