...
from the appropriate IP address range for the Quick servers in that
AWS Region. To learn more about authorizing Quick connections, see [Manually
...
Enabling connection from Amazon Quick servers to your instance is just one of several
...
Amazon Quick) or that has Internet Gateways attached, see [Connecting to a VPC with Amazon Quick](../../../quicksight/latest/user/working-with-aws-vpc.md "../../../quicksight/latest/user/working-with-aws-vpc.md").
...
Use the following values:
    ...
    * For **Source**, choose
    ...
    For example, for Europe (Ireland) you would enter
     ...
     address ranges for Amazon Quick in supported AWS Regions, see
    ...
    If you have activated Amazon Quick in multiple AWS Regions,
     you can create inbound rules for each Amazon Quick endpoint
     CIDR. Doing this allows Amazon Quick to have access to the Amazon RDS
     DB instance from any AWS Region defined in the inbound rules.

    Anyone who uses Amazon Quick in multiple AWS Regions is
     treated as a single user. In other words, even if you are using
     Amazon Quick in every AWS Region, both your Amazon Quick
...
Amazon Quick in supported AWS Regions, see [AWS
