...
With the Google Sheets connector, you can access Google Sheets directly in Amazon Quick through natural language. You can create and edit spreadsheets, read and write cell ranges, format cells, and search for data without leaving Amazon Quick.

Amazon Quick supports multiple authentication methods for Google Sheets. Choose the method that best fits your organization's security requirements.
...
If you use **Default OAuth app** authentication, skip this section and see [Setting up the connector in Amazon Quick](#google-sheets-quicksuite-setup).
...
The following scopes are requested when you connect to Google Sheets.


**Google Sheets recommended scopes**  

| Scope | Description | 
| --- | --- | 
| https://www.googleapis.com/auth/spreadsheets | Reads and writes Google Sheets spreadsheets. | 
...
1. On the **Available** tab, find **Google Sheets** and choose **Connect**.
...
1. Find and choose **Google Sheets**.
**Note**  
If a Google Sheets connector already exists, a dialog appears with your existing connectors. To use an existing connector, choose it. To create a new one, choose **No, create new**.
...
1. For **OAuth Configuration**, choose one of the following authentication methods and configure the required fields.
   ...
   1. For **Custom OAuth app**, configure the following fields:
      + **Base URL** (Optional) – The Google Sheets API base URL. For example: `https://sheets.googleapis.com`
...
| Values | Batch Get Value | Reads values from multiple cell ranges in a single request. | 
| Values | Get Values By Filter | Reads values using a data filter. | 
| Values | Update Values | Writes values to a cell range. | 
| Values | Batch Update Value | Writes values to multiple cell ranges in a single request. | 
| Values | Append Values | Appends values after the last row of data in a range. | 
| Values | Delete Values | Clears values from a cell range. | 
| Values | Update Text | Finds and replaces text across a spreadsheet. | 
...
| Sheets | Update Sheet Properties | Updates sheet properties such as name or visibility. | 
| Formatting | Update Cell Format | Formats cells in a range. | 
| Formatting | Update Conditional Format Rules | Creates or updates conditional formatting rules. | 

**Note**  
The actions that you can use depend on the spreadsheets accessible to the authenticated user.
...
+ **Google Sheets API not enabled** – Verify that the Google Sheets API is enabled in your Google Cloud project under **APIs & Services**, **Library**.