...
Amazon Quick supports multiple authentication methods for Google Sheets.
...
authentication, skip this section and see [Setting up the connector in Amazon Quick](#google-sheets-quicksuite-setup "#google-sheets-quicksuite-setup").
...
3. Find and choose **Google Sheets**.

###### Note

If a Google Sheets connector already exists, a dialog
    ...
    2. For **Custom OAuth app**,
     configure the following fields:
    	...
    	* **Base URL** (Optional) – The
    	 Google Sheets API base URL. For example:
...
The actions that you can use depend on the spreadsheets accessible
...
- **Google Sheets API not
  enabled** – Verify that the Google Sheets API
  is enabled in your Google Cloud project under
  **APIs & Services**,
  **Library**.
