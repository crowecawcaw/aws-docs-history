...
Amazon Quick supports multiple authentication methods for Cisco Webex Video
Messaging. Choose the method that best fits your organization's security
requirements.
...
- An active Cisco Webex account with access to Webex Video Messaging
  (Vidcast).
...
step-by-step instructions, see [Vidcast MCP Server](https://developer.webex.com/mcp/docs/vidcast-mcp-server "https://developer.webex.com/mcp/docs/vidcast-mcp-server") in the Webex developer documentation.
Record the Client ID and Client Secret — you need them when you
configure Amazon Quick. For API Key authentication, generate a Webex
personal access token or API key with the required scopes.
...
3. Find and choose **Cisco Webex Video Messaging**.
...
6. For **OAuth Configuration**, choose one of the
   ...
   1. For **Custom OAuth app**,
      ...
      - **Base URL** – The Webex Video
        Messaging MCP base URL. Default:
...
Video Messaging are available. To see the current set of actions for your
...
- **API Key authentication fails** –
  ...
  required scopes for Webex Video Messaging operations.
