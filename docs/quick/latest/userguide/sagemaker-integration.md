...
You can augment your Amazon Quick Enterprise edition data with Amazon SageMaker AI machine learning
models. You can run inferences on data stored in SPICE imported from any data
source supported by Quick. For a full list of supported data sources, see [Supported data sources](supported-data-sources.md "supported-data-sources.md").

Using Quick with SageMaker AI models can save the time that you might otherwise spend
...
Quick serverless approach, the process scales seamlessly, so you don't need
to worry about inference or query capacity.

Amazon Quick supports SageMaker AI models that use regression and classification algorithms. You
...
sales leads, and assessing credit risks. To use Quick to provide predictions,
...
Quick doesn’t support multiple values inside a single column.
...
1. An Amazon Quick administrator adds permissions for Quick to access
   SageMaker AI. To do this, open **Security & Permissions** settings
   from the **Manage Quick** page. Go to
   **Quick access to AWS services**, and add
   SageMaker AI.

When you add these permissions, Quick is added to an AWS Identity and Access Management
...
get the schema file from the owner of the model. 4. In Quick, you begin by creating a new dataset with the data that
...
When you run SageMaker AI integration, Quick passes a request to SageMaker AI to
run batch transform jobs with inference pipelines. Quick starts
...
To make it easier for you to identify them, Quick names all its
...
- Your Quick subscription costs are as follows:
  ...
  - Quick subscriptions for the authors or admins who build
    the datasets.
...
In Amazon Quick, the following usage guidelines apply to this Enterprise edition
feature:
...
- Only Quick admins or authors can augment datasets with ML models.
...
contains the metadata that Amazon Quick Sight needs to process the model. The Amazon Quick author
...
transform job. The list is provided to the Amazon Quick user to choose from.
