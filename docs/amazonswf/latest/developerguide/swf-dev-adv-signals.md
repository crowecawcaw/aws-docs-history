...
To use signals, define the signal name and data to be passed to the signal—if any. Then, program the decider to recognize the signal event ([WorkflowExecutionSignaled](https://docs.aws.amazon.com/amazonswf/latest/apireference/API_WorkflowExecutionSignaledEventAttributes.html)) in the history and process it appropriately. When a process wants to signal a workflow execution, it makes a call to Amazon SWF (using the [SignalWorkflowExecution](https://docs.aws.amazon.com/amazonswf/latest/apireference/API_SignalWorkflowExecution.html) action or, in the case of a decider, using the [SignalExternalWorkflowExecution](https://docs.aws.amazon.com/amazonswf/latest/apireference/API_SignalExternalWorkflowExecutionInitiatedEventAttributes.html) decision) that specifies the identifier for the target workflow execution, the signal name, and the signal data. Amazon SWF then receives the signal, records it in the history of the target workflow execution, and schedules a decision task for it. When the decider receives the decision task, it also receives the signal inside the workflow execution history. The decider can then take appropriate actions based on the signal and its data.
...
1. When a decision task is received, check the history to see if the signal has arrived or if the timer has fired.

1. If a signal has arrived, cancel the timer using a `CancelTimer` decision and process the signal. Depending on the timing, the history may contain both `TimerFired` and `WorkflowExecutionSignaled` events. In such cases, you can rely on the relative order of the events in the history to determine which occurred first.

1. If the timer has fired, before a signal is received, then the decider has timed out waiting for the signal. You can fail the execution or do whatever other logic is appropriate to your use case.
