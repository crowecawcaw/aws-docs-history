...
_application policy_ (or [ActorPolicy](../APIReference/API_IamAuthenticationMethod.md#API_IamAuthenticationMethod_Contents "../APIReference/API_IamAuthenticationMethod.md#API_IamAuthenticationMethod_Contents") in the APIs), defines which [IAM principals](../../../IAM/latest/UserGuide/reference_policies_elements_principal.md#Principal_specifying "../../../IAM/latest/UserGuide/reference_policies_elements_principal.md#Principal_specifying") are authorized to call IAM authentication method API actions
...
- `sso-oauth:IntrospectTokenWithIAM` [permission only]: Grants permission to validate and retrieve information about active OAuth 2.0 access tokens and refresh tokens, including their associated scopes and permissions. This permission is used only by AWS managed applications and is not documented in the _IAM Identity Center OIDC API Reference_.
- `RevokeTokenWithIAM` [permission only]: Grants permission to revoke OAuth 2.0 access tokens and refresh tokens, invalidating them before their normal expiration. This permission is used only by AWS managed applications and is not documented in the _IAM Identity Center OIDC API Reference_.
