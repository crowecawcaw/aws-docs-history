...
You can only end active sessions that are loaded. If a user has many sessions, choose **Load more active sessions** to display additional sessions. 6. Select the check box next to each session that you want to end, and then choose **End sessions**. 7. A dialog box appears that confirms you are ending active sessions for this user. Review the information, and if you want to continue, type `confirm`, and then choose **End sessions**. 8. You are returned to the user's page. A green notification message appears to indicate that the selected sessions were successfully ended.
