...
access token. Ensure you copy these values before moving forward. 4. Choose **Close**.
...
suspending a user's account. 6. Choose **Finish.** You are returned to the Amazon Web Services app
page. 7. In the **Auto-provisioning** section, turn on the toggle
...
the slider. 8. In the confirmation dialog box, choose **Turn
on**. 9. To verify that users are successfully synchronized to IAM Identity Center, return to the
