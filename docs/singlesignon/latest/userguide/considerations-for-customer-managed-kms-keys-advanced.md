...
statements provide finer-grained control over who can use the key and for what purposes;
however, they require ongoing maintenance as your IAM Identity Center configuration evolves. For
example, if you restrict the use of the KMS key to specific AWS managed application
deployments, you'll need to update the key policy whenever your organization wants to
...
restrict use of the KMS key to a specific IAM Identity Center instance.

When enabling a new IAM Identity Center instance with a customer managed KMS key, the IAM Identity Center and
Identity Store ARNs are not available until after setup. You have the following options:
...
Don't apply this configuration to a KMS key already in use in an existing IAM Identity Center
instance, as it may disrupt its normal operations.

- Omit the encryption context condition from the KMS key policy until after the
  instance is enabled.
