...
<a name="encryption-in-transit"></a>
 ...
 IAM Identity Center protects data in transit, as it travels to and from the service, by automatically encrypting all inter-network data using the Transport Layer Security (TLS) 1.2 or TLS 1.3 encryption protocol. Direct HTTPS requests authenticated with IAM and sent to the IAM Identity Center APIs, Identity Store API, or OIDC API, are signed using the [AWS Signature Version 4 Algorithm](https://docs.aws.amazon.com/general/latest/gr/sigv4_signing.html) to establish a secure connection. 

## Data privacy
<a name="data-privacy"></a>

 With IAM Identity Center, you retain control of your organization’s data. Your user and group identities stored in IAM Identity Center are shared with other AWS services such as [AWS managed applications](https://docs.aws.amazon.com/singlesignon/latest/userguide/awsapps.html) only if you enable them with IAM Identity Center, and if needed by those services. 
