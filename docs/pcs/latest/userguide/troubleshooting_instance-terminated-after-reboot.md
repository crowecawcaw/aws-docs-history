...
**What to do**  
 You can check your `slurmctld` or `slurmd` logs to see if your instance was rebooted. For more information, see [Scheduler logs in AWS PCS](monitoring_scheduler-logs.md) and [Monitoring AWS PCS instances using Amazon CloudWatch](monitoring-cloudwatch_instances.md). The following example `slurmctld` log entry indicates that the instance rebooted: 
