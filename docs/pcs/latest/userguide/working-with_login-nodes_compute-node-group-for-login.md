...
+  The IAM instance profile should have only the AWS permissions you want your end users to have. See [IAM instance profiles for AWS Parallel Computing Service](security-instance-profiles.md) for details. 
...
+  Use the same (or a derivative) AMI as for your other compute node groups to help ensure all instances have the same software installed. For more information about customizing AMIs, see [Amazon Machine Images (AMIs) for AWS PCS](working-with_ami.md)
+  Configure the same network file system (Amazon EFS, Amazon FSx for Lustre, etc.) mounts on your login nodes as on your compute instances. For more information, see [Using network file systems with AWS PCS](working-with_file-systems.md).

**Access your login nodes**  
Once your new compute node group reaches ACTIVE status, you can find the EC2 instance(s) it has created and log into them. For more information, see [Finding compute node group instances in AWS PCS](working-with_compute-instances.md).
