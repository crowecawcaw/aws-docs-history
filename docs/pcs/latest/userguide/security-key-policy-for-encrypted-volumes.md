...
1. From the account that you want to create the AWS PCS cluster in, create a grant that delegates the relevant permissions to the AWS PCS service-linked role. The value of `grantee-principal` is the ARN of the service-linked role. The value of `key-id` is the ARN of the key.

   The following example [create-grant](https://docs.aws.amazon.com/cli/latest/reference/kms/create-grant.html) CLI command gives the service-linked role named `AWSServiceRoleForPCS` in account {{111122223333}} permissions to use the customer managed key in account {{444455556666}}.
