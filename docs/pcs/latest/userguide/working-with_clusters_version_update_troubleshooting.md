...
- [Compute nodes fail to connect after update](#update-troubleshooting-nodes-fail "#update-troubleshooting-nodes-fail")
- [Update request rejected with ValidationException](#update-troubleshooting-validation-error "#update-troubleshooting-validation-error")
- [Cluster remains in UPDATING state](#update-troubleshooting-stuck-updating "#update-troubleshooting-stuck-updating")
- [Cluster enters UPDATE\_FAILED state after update](#update-troubleshooting-update-failed "#update-troubleshooting-update-failed")
- [Compute nodes fail to start due to configuration parsing errors](#update-troubleshooting-config-parse "#update-troubleshooting-config-parse")
- [Cluster unavailable after update due to invalid QoS configuration](#update-troubleshooting-qos-bootloop "#update-troubleshooting-qos-bootloop")
...
These errors indicate that a compute node with an incompatible scheduler version is trying to connect to the controller. For information about setting up scheduler logging, see [Scheduler logs in AWS PCS](monitoring_scheduler-logs.md "monitoring_scheduler-logs.md").
...
For more information about retrieving instance logs, see [Retrieve instance logs](troubleshooting-compute-node-bootstrap.md#troubleshooting-compute-node-bootstrap-retrieve-logs "troubleshooting-compute-node-bootstrap.md#troubleshooting-compute-node-bootstrap-retrieve-logs").
...
To determine which scheduler versions are compatible with your cluster, see [Version compatibility](working-with_clusters_version_update.md#version_update-cluster-compatibility "working-with_clusters_version_update.md#version_update-cluster-compatibility").

For information about building custom AMIs with the correct scheduler version, see [Amazon Machine Images (AMIs) for AWS PCS](working-with_ami.md "working-with_ami.md").
...
- The target version is outside the [compatibility window](https://slurm.schedmd.com/upgrades.html#compatibility_window "https://slurm.schedmd.com/upgrades.html#compatibility_window") of the current version.
...
If the target version is EOL, choose a newer supported version instead. For information about supported versions, see [Slurm versions in AWS PCS](slurm-versions.md "slurm-versions.md").
...
This occurs when compute nodes running scheduler version 23.11 receive configuration from a newer cluster version. Scheduler versions after 23.11 introduced new configuration directives that 23.11 cannot parse. Unlike other versions within the [compatibility window](https://slurm.schedmd.com/upgrades.html#compatibility_window "https://slurm.schedmd.com/upgrades.html#compatibility_window"), 23.11 compute nodes cannot connect to a newer cluster because they fatally error on unrecognized configuration keys.
...
For information about building custom AMIs, see [Amazon Machine Images (AMIs) for AWS PCS](working-with_ami.md "working-with_ami.md"). For information about AWS PCS agent versions, see [AWS PCS agent versions](pcs-agent-versions.md "pcs-agent-versions.md").
