...
- Under **Computing configuration**, enter or select these values:
  ...
  - **Subnets** – Select the subnet where the name starts with
    `hpc-networking:PublicSubnetA`. Login nodes run in a public subnet so that you
    can reach them to log in and submit work.
  - **Instances** – Select `c6i.xlarge`.
  - **Scaling configuration** – For **Min. instance
    count**, enter `1`. For **Max. instance count**, enter
    `1`. Fixing both counts at `1` keeps a single login node running at
    all times as a stable entry point to the cluster.
