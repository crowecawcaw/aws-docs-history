...
+ **Cancel and Reboot** – First, cancel affected jobs using `scancel <job_id>`, then initiate an immediate reboot using `scontrol reboot ASAP <nodename>`. Running jobs will be terminated and need to be resubmitted after the node recovers.
