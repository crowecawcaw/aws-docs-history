...
- If the source volume is encrypted, make sure that the Amazon Data Lifecycle Manager default roles
...
- If you enable **Cross Region copy** for
  ...
  to enable encryption in the destination Region, make sure that the default
  roles have permission to use the KMS key needed to perform the encryption
  in the destination Region.
- If you enable **Cross Region copy** for
  encrypted snapshots or AMIs backed by encrypted snapshots, make sure that the
  default roles have permission to use both the source and destination
  KMS keys.
- If you enable snapshot archiving for encrypted snapshots, make sure that
