...
To access an encrypted snapshot, you must have access to the KMS key used to encrypt the snapshot, and the AWS KMS decrypt action. See the [Control access to EBS direct APIs using IAM](ebsapi-permissions.md) section earlier in this guide for the AWS KMS policy to assign to a user.
