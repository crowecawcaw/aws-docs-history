...
+ [Monitor real-time I/O performance statistics](#monitor-io-nvme)
...
- **SSD**
  - **Maximum I/O size:** 256 KiB
  - **I/O operations from your application:** 1 x 1024 KiB I/O operation / **Number of IOPS:** 4 (1,024÷256=4) / **Notes:** Amazon EBS splits the 1,024 KiB I/O operation into four smaller 256 KiB operations.
  - **I/O operations from your application:** 8 x sequential 32 KiB I/O operations / **Number of IOPS:** 1 (8x32=256) / **Notes:** Amazon EBS merges the eight sequential 32 KiB I/O operations into a single 256 KiB operation.
  - **I/O operations from your application:** 8 random 32 KiB I/O operations / **Number of IOPS:** 8 / **Notes:** Amazon EBS counts random I/O operations separately.

- **HDD**
  - **Maximum I/O size:** 1,024 KiB
  - **I/O operations from your application:** 1 x 1024 KiB I/O operation / **Number of IOPS:** 1 / **Notes:** The I/O operation is already equal to the maximum I/O size. It is not merged or split.
  - **I/O operations from your application:** 8 x sequential 128 KiB I/O operations / **Number of IOPS:** 1 (8x128=1,024) / **Notes:** Amazon EBS merges the eight sequential 128 KiB I/O operations into a single 1,024 KiB I/O operation.
  - **I/O operations from your application:** 8 random 32 KiB I/O operations / **Number of IOPS:** 8 / **Notes:** Amazon EBS counts random I/O operations separately.
...
If your workload uses sequential I/Os on HDD-backed `st1` and `sc1` volumes, you may experience a higher than expected number of IOPS as measured from inside your instance. This happens when the instance operating system merges sequential I/Os and counts them in 1,024 KiB-sized units. If your workload uses small or random I/Os, you may experience a lower throughput than you expect. This is because each random, non-sequential I/O counts toward the total IOPS count, which can cause you to hit the volume's IOPS limit sooner than expected.

Whatever your EBS volume type, if you are not experiencing the IOPS or throughput you expect in your configuration, make sure that your EC2 instance bandwidth is not the limiting factor. You should always use a current-generation, EBS-optimized instance (or one that includes 10 Gb/s network connectivity) for optimal performance. Another possible cause for not experiencing the expected IOPS is that you are not driving enough I/O to the EBS volumes.
