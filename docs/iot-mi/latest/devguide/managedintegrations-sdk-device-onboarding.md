...
air purifier, both using CMake as the build system.
    ...
    1. Simple security camera application
    ...
    ```
    2. Simple air purifier application
    ...
    This demo implements low-level C-Functions for a simulated air purifier with 2
     endpoints and the following supported clusters:




    Supported clusters for air purifier endpoint| Endpoint | Clusters |
    | --- | --- |
    ...
    The output is similar to the camera demo application, with different supported
     clusters.
...
With this setup, you can leverage the Managed integrations service to control your AWS IoT devices.
    ...
    Before implementing the hardware callback functions, understand how the API works.
     This example uses the On/Off cluster and OnOff attribute to control a
     device function. For API details, see [Low level C-Function APIs](managedintegrations-sdk-device-api.md "managedintegrations-sdk-device-api.md").
    ...
    ```
    2. ###### Set up endpoints and hook hardware callback functions
    ...
    After implementing the functions, create endpoints and register your callbacks.
     Complete the following tasks:
    	...
    	1. Create a device agent.
    		...
    		1. Create a device agent using `iotmiDev_Agent_new()` before you invoke any other SDK functions.
    		2. At minimum, your configuration must include the **thingId** and **clientId** parameters.
    		3. Use the `iotmiDev_Agent_initDefaultConfig()` function to set reasonable default values
    		 for parameters such as queue sizes and maximum endpoints.
    		4. When you finish using resources, free them with the `iotmiDev_Agent_free()` function.
    		 This prevents memory leaks and ensures proper resource management in your application.
    	2. Fill callback function pointers for each cluster structure that you want to support.
    	3. Set up endpoints and register your supported clusters.


    	Create endpoints with `iotmiDev_Agent_addEndpoint()`, which requires:


    		1. A unique endpoint ID.
    		2. A descriptive endpoint name
    		3. One or more device types that conform to AWS data model definitions.
    		4. After creating endpoints, register clusters using the appropriate cluster-specific registration functions.
    		5. Each cluster registration requires callback functions for attributes and commands.
    		 The system passes your user context pointer to callbacks to maintain state between calls.
    ```
    struct DeviceState
    {
        struct iotmiDev_Agent * agent;
    ...
    ```
    3. ###### Use the jobs handler to obtain the job document


    	1. Initiate a call to your OTA application:
