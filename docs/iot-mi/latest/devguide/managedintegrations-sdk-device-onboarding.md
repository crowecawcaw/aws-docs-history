...
**Setup Amazon EC2 instance**
   ...
   Sign in to the AWS Management Console and launch an Amazon EC2 instance using an Amazon Linux AMI. See [Get started with Amazon EC2](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/EC2_GetStarted.html) in the *[Amazon Elastic Container Registry User Guide](https://docs.aws.amazon.com/AmazonECR/latest/userguide/)*. 
...
**Build and run the demo applications**
   ...
   1. Simple security camera application
      ...
      This demo implements low-level C-Functions for a simulated camera with RTC Session Controller and Recording clusters. Complete the flow mentioned in [Provisionee workflow](managedintegrations-sdk-device-provisionee.md#managedintegrations-sdk-device-provisionee-how) before running.
   ...
   1. Simple air purifier application
...
**Develop hardware callback functions**
      ...
      Before implementing the hardware callback functions, understand how the API works. This example uses the On/Off cluster and OnOff attribute to control a device function. For API details, see [Low level C-Function APIs](managedintegrations-sdk-device-api.md).
...
**Set up endpoints and hook hardware callback functions**
      ...
      1. Create a device agent.
         ...
         1.  At minimum, your configuration must include the **thingId** and **clientId** parameters.

         1.  Use the `iotmiDev_Agent_initDefaultConfig()` function to set reasonable default values for parameters such as queue sizes and maximum endpoints. 

         1. When you finish using resources, free them with the `iotmiDev_Agent_free()` function. This prevents memory leaks and ensures proper resource management in your application. 

      1. Fill callback function pointers for each cluster structure that you want to support.

      1. Set up endpoints and register your supported clusters.

         Create endpoints with `iotmiDev_Agent_addEndpoint()`, which requires:

         1. A unique endpoint ID.

         1. A descriptive endpoint name

         1. One or more device types that conform to AWS data model definitions.

         1. After creating endpoints, register clusters using the appropriate cluster-specific registration functions.

         1. Each cluster registration requires callback functions for attributes and commands. The system passes your user context pointer to callbacks to maintain state between calls.
...
**Use the jobs handler to obtain the job document**
      ...
      1. Call `iotmi_JobsHandler_start` to initialize the jobs handler.

      1. Call `iotmi_JobsHandler_getJobDocument` to retrieve the job Document from Managed Integrations.

      1. When the Jobs Document is obtained successfully, write your custom OTA operation in the `processOTA` function and return a `JobSucceeded` status.
