...
| --- | --- | 
| AWS4-HMAC-SHA256 | The algorithm that was used to calculate the signature. You must provide this value when you use AWS Signature Version 4 for authentication.<br />The string specifies AWS Signature Version 4 (`AWS4`) and the signing algorithm (`HMAC-SHA256`). | 
| Credential | Your access key ID and the scope information, which includes the date, Region, and service that were used to calculate the signature. <br />This string has the following form:<br /> <pre><your-access-key-id>/<date>/<aws-region>/<aws-service>/aws4_request</pre><br />Where:<br /> +  {{<date>}} value is specified using `YYYYMMDD` format. <br />+  {{<aws-service>}} value is `s3` when sending request to Amazon S3.  | 
