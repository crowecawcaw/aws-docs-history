...
You can add tags to Amazon S3 vector buckets and modify these tags. There is no
additional charge for using tags on vector buckets beyond the standard S3 API request rates.
For more information, see [Amazon S3
pricing](https://aws.amazon.com/s3/pricing/ "https://aws.amazon.com/s3/pricing/"). For more information
...
If you encounter an error when attempting to add a tag to a vector bucket, you can do
the following:

- Verify that you have the required [Permissions](#add-bucket-tag-permissions "#add-bucket-tag-permissions") to add a tag to a vector
  bucket.
...
You can add tags to vector buckets by using the Amazon S3 console, the AWS Command Line Interface
...
The following CLI example shows you how to add tags to a vector bucket by using the
AWS CLI. To use the command replace the `user input placeholders`
