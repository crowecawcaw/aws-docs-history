...
**Note**  
This page covers delivering server access logs to an Amazon S3 general purpose bucket. You can also deliver logs to Amazon CloudWatch Logs for structured querying, cross-account aggregation, and optional mirroring to S3 Tables. For more information, see [Delivering server access logs to CloudWatch Logs](sal-cw-enabling.md).
...
1. Choose **Save changes**.
   ...
   You can view the logs in the destination bucket. After you enable server access logging, it might take a few hours before the logs are delivered to the target bucket.
...
**Example – `logging.json` with the log object key format set to S3 event time**  
   ...
   The following `logging.json` file changes the log object key format to S3 event time. 
