...
| `Range` (query parameter) | `userRequest.url` (query parameter
 `Range`) |
| `partNumber` | `userRequest.url` (query parameter
 `partNumber`) |
...
To do this, use the provided presigned URL to fetch the entire object from Amazon S3
 ...
 a `Range` parameter in this way, see  [this sample](https://github.com/aws-samples/amazon-s3-object-lambda-default-configuration/blob/main/function/nodejs_20_x/src/response/range_mapper.ts "https://github.com/aws-samples/amazon-s3-object-lambda-default-configuration/blob/main/function/nodejs_20_x/src/response/range_mapper.ts") in the AWS Samples GitHub repository.
