...
upload the object. 3. To configure other additional properties, choose **Properties**. 4. Under **Storage class**, choose the storage class for the files that
you're uploading.

For more information about storage classes, see [Understanding and managing Amazon S3 storage classes](storage-class-intro.md "storage-class-intro.md"). 5. To update the encryption settings for your objects, under **Server-side encryption
...
like to use to verify your data. For more information about additional checksums, see [Checking object integrity in Amazon S3](checking-object-integrity.md "checking-object-integrity.md"). 7. To add tags to all of the objects that you are uploading, choose **Add
...
Amazon S3 uploads your object. When the upload completes, you can see a success message on the **Upload: status** page. 10. Choose **Exit**.
You can send a `PUT` request to upload an object of up to 5 GB in a single
operation. For more information, see the [`PutObject`](../../../cli/latest/reference/s3api/put-object.md#examples "../../../cli/latest/reference/s3api/put-object.md#examples") example
