...
| CRC-32 (`CRC32`)<br>CRC-32C (`CRC32C`)<br>SHA-1 (`SHA1`)<br>SHA-256 (`SHA256`) | Composite     | Required headers:<br>`x-amz-checksum-algorithm`                          | Required headers:<br>`x-amz-checksum-crc32`<br>`x-amz-checksum-crc32c`<br>`x-amz-checksum-sha1`<br>`x-amz-checksum-sha256` | Required headers:<br>All part-level checksums need to be included in the `CompleteMultiPartUpload` request.<br>Optional headers:<br>`x-amz-crc32`<br>`x-amz-crc32c`<br>`x-amz-sha1`<br>`x-amz-sha256` |
