...
AWS managed table buckets are specialized S3 table buckets designed to store AWS managed tables, such as
 [Discovering your data with S3 Metadata tables](metadata-tables-overview.md "metadata-tables-overview.md") journal and live
 inventory tables. Unlike customer-managed table buckets that you create and manage directly,
 AWS managed table buckets are automatically provisioned by AWS when you configure features that require AWS
 managed tables. When managed tables are created, they belong to a predefined namespace based on the source
 bucket that can't be modified. Each AWS account has one AWS managed table bucket per Region, following the
 naming convention `aws-s3`. This bucket serves as a centralized location for all managed tables
...
| **Encryption** | You can only change the default encryption (SSE-S3) settings if you encrypted the initial table with a customer manged AWS KMS key. | You can set bucket-level default encryption and modify it anytime |
| **Maintenance** | Managed by AWS services | Automated maintenance can be customized at bucket level |


## Permissions to create AWS managed table buckets


To work with AWS managed table buckets, you need permissions to create AWS managed table buckets and tables, specify encryption settings for AWS managed tables as well as basic read permissions for querying tables.


The following is an example policy that will allow you to create an AWS managed table bucket through service configurations:
...
The following is an example policy that will allow you to query tables in AWS managed table buckets:
...
```
SELECT *
FROM "s3tablescatalog/aws-s3"."b_``amzn-s3-demo-source-bucket``"."inventory"
LIMIT 10;
```
...
```
SELECT *
FROM "s3tablescatalog/aws-s3"."b_``amzn-s3-demo-source-bucket``"."inventory" `a`
...
By default, AWS managed table buckets are encrypted with server-side encryption using Amazon S3
 managed keys (SSE-S3). After your AWS managed table bucket is created, you can use [PutTableBucketEncryption](../API/API_s3Buckets_PutTableBucketEncryption.md "../API/API_s3Buckets_PutTableBucketEncryption.md") to set the bucket's default encryption setting to
