...
* **Using the Amazon S3 console** — You can use
 ...
 write any code. For more information, see [Enabling and configuring event notifications using the Amazon S3 console](enable-event-notifications.md "enable-event-notifications.md").
* **Programmatically using the AWS SDKs**
 — Internally, both the console and the SDKs call the Amazon S3 REST API to
 manage *notification* subresources that are associated with
...
Regardless of the method that you use, Amazon S3 stores the notification configuration as
 XML in the *notification* subresource that's associated with a
 bucket. For information about bucket subresources, see [General purpose buckets configuration options](UsingBucket.md#bucket-config-options-intro "UsingBucket.md#bucket-config-options-intro").
 ...
 *notification* subresource initially stores an empty
 configuration.
...
When Amazon S3 detects an event of the specific type, it publishes a message with the
 event information. For more information, see [Event message structure](notification-content-structure.md "notification-content-structure.md"). 
