...
![Diagram showing an AWS account granting permissions.](../../../images/AmazonS3/latest/userguide/images/access-policy-ex1.png)
	...
	2. In Account A, create an administrator user `AccountAadmin`. Using
	 ...
	 do the following: 
		 ...
		 **AccountAadmin**. You will need to use
		 ...
		 about where to find the sign-in URL, see [Sign in to the AWS Management Console as an IAM user](https://docs.aws.amazon.com/IAM/latest/UserGuide/getting-started_how-users-sign-in.html "https://docs.aws.amazon.com/IAM/latest/UserGuide/getting-started_how-users-sign-in.html") in
		 *IAM User Guide*. Note the URL for each
		 of the accounts.
...
1. Create resources of a bucket and an IAM user
	...
	2. In the
	 ...
	 do the following: 
		...
		3. Note the Amazon Resource Name (ARN) for user Dave. In the [IAM Console](https://console.aws.amazon.com/iam/ "https://console.aws.amazon.com/iam/"), select
...
Because the bucket owner and the parent account to which the user belongs are
 ...
 policy (or an IAM policy).
	...
	For instructions, see [Managing IAMpolicies](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_inline-using.html "https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_inline-using.html") in the
	 *IAM User Guide*.
	 Note
