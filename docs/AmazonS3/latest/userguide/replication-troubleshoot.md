...
**Note**  
If your replication rule uses the change object ownership to the destination bucket owner option and the object exists in the destination bucket but shows a `FAILED` replication status, S3 Batch Replication might not resolve this status. If you encounter this situation, contact AWS Support to verify the replication state of the affected objects.
