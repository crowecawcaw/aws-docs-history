...
* **`ExpiredObjectDeleteMarker` action
 ...
 with zero noncurrent versions is referred to as an *expired object delete marker*. You can use this lifecycle action
...
When a specified date or time period in the object's lifetime
 is reached. | Amazon S3 transitions the object to the specified storage
...
| `Expiration`
When a specified date or time period in the object's lifetime
 is reached. | The `Expiration` action deletes the object, and the
...
For noncurrent versions in a versioning enabled or versioning
 ...
 rule. | `NoncurrentVersionTransition` has no effect. | Amazon S3 transitions the noncurrent object versions to the
 specified storage class. | Same behavior as a versioning-enabled bucket. |
...
For noncurrent versions in a versioning enabled or versioning
 ...
 newer versions to retain**
 (`<NewerNoncurrentVersions>`) in the
 rule. | `NoncurrentVersionExpiration` has no effect. | The `NoncurrentVersionExpiration` action permanently
