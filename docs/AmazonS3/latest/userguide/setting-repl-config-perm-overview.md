...
This section explains the trust policy and the minimum required permissions policy that
 ...
 to create an IAM role. For more information, see [Examples for configuring live replication](replication-example-walkthroughs.md "replication-example-walkthroughs.md").
...
The *trust policy* identifies which principal
 identities can assume the IAM role. The *permissions
 policy* specifies which actions the IAM role can perform, on which resources,
 and under what conditions. 



* The following example shows a *trust policy* where
...
* The following example shows the *permissions
 policy*, where you grant the IAM role permissions to perform replication
...
The permissions policy grants permissions for the following actions:
	...
	+ `s3:ReplicateObject` and `s3:ReplicateDelete` –
	 ...
	 markers, see [How delete operations affect replication](replication-what-is-isnot-replicated.md#replication-delete-op "replication-what-is-isnot-replicated.md#replication-delete-op"). 
...
For a list of Amazon S3 actions, see [Actions, resources, and condition keys for Amazon S3](https://docs.aws.amazon.com/service-authorization/latest/reference/list_amazons3.html#list_amazons3-actions-as-permissions "https://docs.aws.amazon.com/service-authorization/latest/reference/list_amazons3.html#list_amazons3-actions-as-permissions") in the *Service Authorization Reference*.
...
For example, suppose that the source bucket contains objects owned by another
 ...
 fails. For information about ACL permissions, see [Access control list (ACL) overview](acl-overview.md "acl-overview.md").
...
You can also use the Amazon S3 console to automatically generate this bucket policy for you.
 ...
 source bucket](#receiving-replicated-objects "#receiving-replicated-objects").
...
The ARN format of the role might appear different. If the role was created by using
 ...
 roles](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_identifiers.html "https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_identifiers.html") in the *IAM User Guide*. 
...
When different AWS accounts own the source and destination buckets, you can tell Amazon S3 to change
 ...
 see [Changing the replica owner](replication-change-owner.md "replication-change-owner.md").
