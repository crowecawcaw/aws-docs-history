...
**Note**  
The `PutBucketEncryption` API does not support setting SSE-C as a default encryption method for a bucket, objects won't automatically use SSE-C. SSE-C is an object-level encryption method where you provide the encryption key with each upload or download request. Amazon S3 uses this key to encrypt or decrypt the object during the request and then discards the key.   
