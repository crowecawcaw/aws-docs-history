...
+ **S3 Object Lock retain until date** – The date until which the locked object cannot be deleted. For objects with an active event hold, this date is dynamic, based on the report generation time. For objects with a released event hold or fixed retention, this date is fixed. For more information, see [Locking objects with Object Lock](object-lock.md).
...
+ **S3 Object Lock event hold status** – Set to `ON` or `OFF`, reflecting the event hold status of the object. This field is empty when the object has no event hold configured.
+ **S3 Object Lock event hold duration** – The variable retention duration that is configured for the object. This field is empty when the object has no event hold configured.
**Note**  
When you include the S3 Object Lock event hold duration field, the report also includes an S3 Object Lock event hold duration unit column (`DAYS` or `YEARS`) to express the unit of the duration value.
