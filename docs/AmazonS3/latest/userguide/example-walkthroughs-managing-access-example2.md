...
![An AWS account granting another AWS account permission to access its resources.](images/access-policy-ex2.png)
...
permissions. 2. Account B administrator user attaches user policy to the user delegating the
permissions it received from Account A. 3. User in Account B then verifies permissions by accessing an object in the bucket
owned by Account A.
...
For instructions, see [Creating a general purpose bucket](create-bucket-overview.md "create-bucket-overview.md"). 2. Upload a sample object to the bucket.
