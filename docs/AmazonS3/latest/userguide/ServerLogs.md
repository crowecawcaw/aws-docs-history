...
To enable log delivery, perform the following basic steps. For details, see [Enabling Amazon S3 server access logging](enable-server-access-logging.md "enable-server-access-logging.md").



1. **Provide the name of the destination bucket** (also
 known as a *target bucket*). This bucket is where you
...
If you choose to save access logs in the source bucket, we recommend that you specify
 a destination prefix (also known as a *target prefix*)
...
2. **(Optional) Assign a destination prefix to all Amazon S3 log object
 keys.** The destination prefix (also known as a *target
 prefix*) makes it simpler for you to locate the log objects. For example, if
...
[Prefixes](https://docs.aws.amazon.com/general/latest/gr/glos-chap.html#keyprefix "https://docs.aws.amazon.com/general/latest/gr/glos-chap.html#keyprefix") are also useful to distinguish between source buckets when multiple
...
This prefix can also help when you delete the logs. For example, you can set a
 ...
 information, see [Deleting Amazon S3 log files](deleting-log-files-lifecycle.md "deleting-log-files-lifecycle.md").
3. **(Optional) Set permissions so that
 ...
 you can't grant permissions in destination grants (also known as *target grants*) that use ACLs. However, you can update your bucket policy for
 the destination bucket to grant access to others. For more information, see [Identity and Access Management for Amazon S3](security-iam.md "security-iam.md") and [Permissions for log delivery](enable-server-access-logging.md#grant-log-delivery-permissions-general "enable-server-access-logging.md#grant-log-delivery-permissions-general").
...
When you enable server access logging and grant access for access log delivery through
 ...
 for server access log delivery, see [Permissions for log delivery](enable-server-access-logging.md#grant-log-delivery-permissions-general "enable-server-access-logging.md#grant-log-delivery-permissions-general"). 
 ...
 *it might not be delivered at all*. It is possible that you might even
