...
If you are using an access point attached to an S3 recovery point in AWS Backup, data plane requests (such as `GetObject` and `ListObjectsV2`) are logged in CloudTrail the same way as requests through standard S3 access points. For example:
