...
5. Choose the **View in CloudWatch** in any metrics pane to navigate to the CloudWatch console and see your available metrics in the AWS/S3Tables namespace.
   To list metrics for S3 Tables using the AWS CLI, use the `list-metrics` command with the `--namespace` parameter set to "AWS/S3Tables":

```
aws cloudwatch list-metrics --namespace AWS/S3Tables
```
