...
+ **AWS Command Line Interface (CLI) `s3 sync` command** – The `s3 sync` command is not supported with directory buckets. Use `s3 cp --recursive` as an alternative to transfer files to or from directory buckets.
