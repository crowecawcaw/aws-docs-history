...
| Protocol | The protocol, `http` or `https`, to be used in the `Location` header that is returned in the response. <br />If one of its siblings is supplied, `Protocol` is not required. | 
| HostName | The hostname to be used in the `Location` header that is returned in the response.<br />If one of its siblings is supplied, `HostName` is not required. | 
| ReplaceKeyPrefixWith | The prefix of the object key name that replaces the value of `KeyPrefixEquals` in the redirect request. <br />If one of its siblings is supplied, `ReplaceKeyPrefixWith` is not required. It can be supplied only if `ReplaceKeyWith` is not supplied. | 
| ReplaceKeyWith | The object key to be used in the `Location` header that is returned in the response. <br />If one of its siblings is supplied, `ReplaceKeyWith` is not required. It can be supplied only if `ReplaceKeyPrefixWith` is not supplied. | 
| HttpRedirectCode | The HTTP redirect code to be used in the `Location` header that is returned in the response.<br />If one of its siblings is supplied, `HttpRedirectCode` is not required. | 
