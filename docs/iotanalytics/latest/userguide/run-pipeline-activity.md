...
Here is an example of how you would use the `RunPipelineActivity` command to test
a pipeline activity. For this example, we test a math activity.

1. Create a `maths.json` file, which contains the definition of the pipeline
   activity you want to test.
...
2. Create a file `payloads.json` file, which contains the example payloads
   that are used to test the pipeline activity.
...
The payloads listed in the results are Base64-encoded strings. When these strings are
decoded, you get the following results.
