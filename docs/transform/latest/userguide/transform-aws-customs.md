...
### Runtime upgrades
<a name="transform-aws-customs-runtime"></a>
...
| AWS/dotnet-modernization | .NET | Generally available | Migrate .NET Framework solutions to modern .NET (net10.0 default; net8.0 or later supported) with controlled user interaction. Assesses complexity and dependencies, presents a migration plan with parallel execution, transforms projects with system-enforced exit-criteria validation, and generates a summary with next steps. | 

### SDK migrations
<a name="transform-aws-customs-sdk"></a>
...
<a name="transform-aws-customs-framework"></a>
...
<a name="transform-aws-customs-genai"></a>
...
<a name="transform-aws-customs-language"></a>
...
### Database migrations
<a name="transform-aws-customs-db"></a>

No database-specific AWS-managed transformations are currently available. To modernize Microsoft SQL Server databases and their associated .NET applications to Amazon Aurora PostgreSQL, see [SQL Server modernization](sql-server-modernization.md).

### Observability
<a name="transform-aws-customs-observability"></a>
...
### Architecture
<a name="transform-aws-customs-architecture"></a>
...
| AWS/mulesoft-to-aws-native-java | Java / MuleSoft to serverless | Early access | Transform MuleSoft Mule 3.x and 4.x applications into AWS serverless architectures (Amazon API Gateway, AWS Lambda, and AWS Step Functions) targeting Java 17 on AWS Lambda with SnapStart. Maps flow triggers to AWS event sources, converts DataWeave and MEL to tiered AWS equivalents (VTL, Step Functions, Lambda), replaces connectors with AWS SDK for Java v2 calls, and orchestrates flows with Step Functions. Produces SAM templates, Java Lambda handler code, Maven pom.xml, architecture docs, and JUnit 5 tests. | 
| AWS/payshield-hsm-to-aws-payment-cryptography | Java (HSM to AWS Payment Cryptography) | Generally available | Migrate Java applications from the Thales PayShield HSM socket-based command protocol to the AWS Payment Cryptography Service (APC) SDK v2. Covers command mapping, key migration (TR-31 and TR-34 import, KCV verification), SDK dependency setup (Maven, Gradle, and Ant), infrastructure removal (socket pools, response parsers), test infrastructure update, and parity testing. | 
| AWS/websphere-jaxrs-to-bedrock-agentcore | Java (WebSphere to AgentCore) | Generally available | Generate AgentCore tool definitions from the REST endpoints in a WebSphere or Liberty application, so existing business logic can be called by AI agents. The transformation reads your source code and writes new files to an output directory without changing anything in place. It finds endpoints that use JAX-RS in both the javax.ws.rs and jakarta.ws.rs namespaces, including EJB exposed through a JAX-RS facade. For each endpoint it maps Java parameters and types to a JSON schema, resolves the HTTP method, and rates its confidence so you know what to review. Output includes a discovery report, a JSON schema per endpoint, tool implementations in Python, and a manifest for registering them with an AgentCore gateway. | 

### Codebase analysis
<a name="transform-aws-customs-analysis"></a>
...
### Security
<a name="transform-aws-customs-security"></a>

Fix security vulnerabilities in source code based on scanner findings.


| Transformation | Language / stack | Status | Description | 
| --- | --- | --- | --- | 
| AWS/security-issue-fixer | Multiple languages | Generally available | Fix security vulnerabilities in source code based on third-party scanner results (SARIF, JSON, CSV, or plain-text findings with CWE IDs, file paths, and line numbers). Does not perform scanning or discovery; it only remediates supplied findings while preserving functional behavior. Covers injection, broken authentication, cryptographic failures, authorization, CSRF, insecure deserialization, SSRF, path traversal, XXE, and more. Supports Python, JavaScript, TypeScript, Java, Go, C\#, .NET, C, C\+\+, and Ruby. | 
