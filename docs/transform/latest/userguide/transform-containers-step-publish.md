...
not yet configured Amazon ECR access, AWS Transform prompts you to provide your Amazon ECR
...
1. **Connector configuration** — If you
   have not already configured Amazon ECR access, AWS Transform presents a connector
   form where you provide your Amazon ECR repository ARN and connect to your
   AWS account.
2. **Approval request** — AWS Transform
...
3. **Image push** — After approval,
   AWS Transform publishes the container images to your Amazon ECR repository using
   . If you have multiple services, AWS Transform publishes all images
   in a single batch operation.
4. **Vulnerability scanning** — Amazon ECR
   automatically scans the published images for known vulnerabilities.
   AWS Transform reports the scan results. For more comprehensive analysis,
...
- Approve or reject the publishing operation. If you reject, AWS Transform
...
After publishing, AWS Transform provides the image URIs that are used in the
