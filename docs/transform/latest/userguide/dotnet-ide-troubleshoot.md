...
You should see regular activity in the Worklog window. If AWS Transform appears to be spending a long time on a step in the **AWS Transform Job Plan** window, you can check whether the job is still active in the output logs. If diagnostic messages are being generated, the job is still active. 
