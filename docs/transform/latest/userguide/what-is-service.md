...
production environments. Administrators can mutate [workspaces](#glossary_workspace "#glossary_workspace"), [connectors](#glossary_connector "#glossary_connector"), and users.
...
permissions to mutate workspaces, connectors, or users. Approvers cannot mutate [workspaces](#glossary_workspace "#glossary_workspace"), [connectors](#glossary_connector "#glossary_connector"), or users.

**Asset**

Input for a transformation [job](#glossary_job "#glossary_job"). For example,
customer's source code, server, database, network. Assets are accessed via a [connector](#glossary_connector "#glossary_connector").
...
given in [the connector acceptor policy](security_iam_id-based-policy-examples.md#id-based-policy-examples-admin-connector "security_iam_id-based-policy-examples.md#id-based-policy-examples-admin-connector").
...
- The account that will be on the receiving end of your transformation. This account must
  be assigned an IAM role that allows it to use a [connector](#glossary_connector "#glossary_connector").

**Contributor**

Contributors can read everything in the workspace. They can begin chats with AWS
Transform, start and stop jobs, upload or download artifacts, and interact with running [jobs](#glossary_job "#glossary_job") for HITL actions. However, they cannot perform critical
HITL actions such as merging to main, performing graph decomposition, or deploying code to
production environments. Contributors also cannot mutate [workspaces](#glossary_workspace "#glossary_workspace"), [connectors](#glossary_connector "#glossary_connector"), or users.
...
fulfill an [objective](#glossary_objective "#glossary_objective") defined by a user. Made up of
multiple [tasks](#glossary_task "#glossary_task") and [collaborator requests](#glossary_collaborator-request "#glossary_collaborator-request").

**Plan**

A list of [tasks](#glossary_task "#glossary_task") that AWS Transform undertakes
(with help from human users) in pursuit of an [objective](#glossary_objective "#glossary_objective").

**Reader**

Readers can view the status and outcomes of AWS Transform jobs, but cannot
make any changes. They can read everything in the [workspace](#glossary_workspace "#glossary_workspace"), download artifacts, view [jobs](#glossary_job "#glossary_job"), and
view human-in-the-loop (HITL) actions. However, readers cannot perform mutating actions in AWS Transform.

**Task**

An individual unit of work that is part of a [job](#glossary_job "#glossary_job").

**Worklog**

A log of what actions AWS Transform and users have performed as part of a [job](#glossary_job "#glossary_job").

**Workspace**

A AWS Transform resource that contains other resources like [connectors](#glossary_connector "#glossary_connector") and [jobs](#glossary_job "#glossary_job").
A [workspace](#glossary_workspace "#glossary_workspace") serves as a permissions boundary.
