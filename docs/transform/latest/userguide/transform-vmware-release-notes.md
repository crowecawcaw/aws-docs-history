...
+ AWS Transform for migrations now lets you refine the generated AWS security groups while reviewing your target network. You can create, configure, and delete security groups and their rules. You can also associate a rule with a CIDR range or a source , and enable security group referencing for a specific rule. [Learn more about reviewing and optimizing your network](https://docs.aws.amazon.com/transform/latest/userguide/transform-vmware-migrate-network.html#transform-vmware-review-and-optimize-network).
+ AWS Transform for migrations now supports for as a generally available block storage target, alongside Amazon EBS. You can migrate block storage directly to for within the same migration wave that handles compute and networks. [Learn more about configuring for as a target storage type](https://docs.aws.amazon.com/transform/latest/userguide/transform-vmware-migrate-storage.html#transform-vmware-storage-fsx-ontap).
+ AWS Transform for migrations now lets you apply your source security posture to Amazon VPCs you have already provisioned in AWS, instead of creating new Amazon VPCs. You upload a source network file that contains firewall rules, tag the existing Amazon VPCs you want in scope, and AWS Transform matches your source subnets to those Amazon VPCs by and generates the corresponding security groups. [Learn more about applying security posture to existing Amazon VPCs](https://docs.aws.amazon.com/transform/latest/userguide/transform-vmware-migrate-network.html#transform-vmware-brownfield-network).
+ AWS Transform for migrations now identifies unused inbound firewall rules migrated from your on-premises environment and suggests removing them as part of guided network recommendations. This helps you avoid carrying forward security exposure, such as open inbound access, that no longer serves a purpose. Removal is limited to unused ingress rules. [Learn more about guided network recommendations](https://docs.aws.amazon.com/transform/latest/userguide/transform-vmware-migrate-network.html#transform-vmware-guided-network-recommendations).
+ AWS Transform for migrations now flags Amazon VPCs that host applications from multiple environments, such as development and production, as part of guided network recommendations. When the environments can be cleanly separated, AWS Transform suggests splitting the Amazon VPC to isolate them. [Learn more about guided network recommendations](https://docs.aws.amazon.com/transform/latest/userguide/transform-vmware-migrate-network.html#transform-vmware-guided-network-recommendations).
+ AWS Transform for rehost now allows you to modify server configurations through the chat interface during the inventory validation step. In addition to editing the inventory file directly, you can specify changes for individual columns on specific source servers without downloading and editing the entire inventory file. This provides a more convenient way to make targeted updates to your server configurations. [Learn more about inventory validation and modifications](https://docs.aws.amazon.com/transform/latest/userguide/transform-vmware-migrate-servers.html#transform-vmware-ms-validate-inventory).

## July 2026
<a name="transform-vmware-release-notes-july-2026"></a>
+ AWS Transform for migrations now offers expert mode, which lets you provide your inputs up front so the agent completes migration steps automatically with minimal back-and-forth. This helps you move faster through large migrations while keeping control of the decisions that matter. [Learn more about expert mode](https://docs.aws.amazon.com/transform/latest/userguide/transform-app-vmware-expert-mode.html).
