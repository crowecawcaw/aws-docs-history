...
+ AWS Transform for migrations now lets you refine the generated AWS security groups while reviewing your target network. You can create, configure, and delete security groups and their rules. You can also associate a rule with a CIDR range or a source VM, and enable security group referencing for a specific rule. [Learn more about reviewing and optimizing your network](https://docs.aws.amazon.com/transform/latest/userguide/transform-vmware-migrate-network-new-vpcs.html#transform-vmware-optimization-operations).
+ AWS Transform for migrations now supports Amazon FSx for NetApp ONTAP as a generally available block storage target, alongside Amazon Elastic Block Store (Amazon EBS). You can migrate block storage directly to FSx for ONTAP within the same migration wave that handles compute and networks. [Learn more about configuring FSx for ONTAP as a target storage type](https://docs.aws.amazon.com/mgn/latest/ug/fsx-ontap.html).
