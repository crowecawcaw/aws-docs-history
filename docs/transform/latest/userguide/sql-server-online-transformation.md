...
<a name="sql-server-online-transformation"></a>
...
1. **Connect** — You provide connection details, and AWS Transform establishes a connection to the source database.

1. **Validate** — AWS Transform validates connectivity and permissions.

1. **Discovery and assessment** — AWS Transform discovers the databases on the server, you choose which to modernize, and AWS Transform assesses them.

1. **Per-database transformation** — For each selected database, AWS Transform provisions an Aurora PostgreSQL target, converts the schema, and migrates data. Choose to migrate real data, generate synthetic data, or skip data migration.

1. **Code assessment and transformation** (optional) — If a source code repository is connected, AWS Transform assesses and transforms the associated .NET application.

1. **Deploy** (optional) — AWS Transform deploys the transformed application.
