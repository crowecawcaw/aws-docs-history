...
The agent deploys the infrastructure templates generated in the previous step to
your AWS account. For Amazon EKS deployments, this uses Helm. For Amazon ECS deployments,
this uses Terraform.

Before deploying, the agent presents an infrastructure values form where you can
configure deployment parameters such as resource names, replica counts, and
environment-specific settings. The agent also generates a deployment preview that
...
2. Review the deployment preview that shows the resources that will be
   created or modified.
3. Approve the infrastructure deployment when the agent requests it.
4. Wait for the deployment to complete. The agent displays the deployment
   results.
...
6. Tell the agent whether you are satisfied with the deployment, or whether
...
1. Tell the agent you want to modify the IaC.
2. Upload a zip file containing your modified infrastructure templates.
3. The agent replaces the templates and performs an incremental update
