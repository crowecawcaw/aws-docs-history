...
This topic outlines the process for collecting mainframe source code and related
artifacts to support mainframe modernization initiatives using AWS Transform. Proper collection
and preparation of source artifacts are critical for successful analysis, migration
...
If you are not sure of the extension, leave it as blank or .txt and AWS Transform
classifies it for you.
...
Use secure FTP clients (WinSCP/FileZilla) for interactive source code
extraction from mainframe PDS libraries.

### DB2 schema extraction

The DB2 LUW (Linux, UNIX, and Windows) client software includes the
`db2look` utility as part of its installation package. The
`db2look` utility extracts DDL (Data Definition Language) statements
from a DB2 database.
...
The LISTCAT command in IDCAMS is a utility for retrieving catalog information
about datasets on the mainframe. This is useful during mainframe modernization
projects to inventory datasets and understand their characteristics.
...
The IBM-provided DFHCSDUP utility can extract CICS resource definitions from
...
To extract IMS definitions, you can generate the IMS Transaction file by using
the DFSURDD0 utility. This utility creates stage 1 macro statements that you can
use for migration purposes. Only IMS Stage 1 is currently supported.
...
CA-7 job schedule information is comprised in LJOB reports. Use the LJOB utility
with the `LIST=NODD` parameter to generate the reports on the mainframe.
The reports must have a `.ca7` extension. Each report must have ANSI
carriage control characters in column 1. You can create a report with carriage
control characters in column 1 by specifying `DCB=RECFM=FBA` (or FA)
in the JCL used to run the LJOB utility.

For example: `LJOB,JOB=ZBN*,LIST=ALL`

For more information, see the
[Broadcom documentation](https://www.broadcom.com/products/mainframe/job-scheduling/workload-automation "https://www.broadcom.com/products/mainframe/job-scheduling/workload-automation").

### Extracting SMF data

System Management Facility (SMF) records provide activity metrics used by
AWS Transform to analyze batch job and CICS transaction usage. The following SMF record
types are supported: 14, 15, 30 (sub type 5), 64, 102, 110 (sub type 1 with
data class 1, 3, and 4).
...
Provide a minimum of 13 months of SMF records to capture annual
batch cycles and seasonal patterns that shorter timeframes might miss.
...
2. DUMPIN: Point to your SMF datasets
3. DUMPOUT: Specify output dataset name and HLQ
4. TYPE: Select record types (14, 15, 30, 110 or others)
5. DATE: Format yyyy/ddd (for example, 2026/055,2026/056)
6. SPACE: Adjust based on expected data volume
7. Remove SORT step if not needed

#### Option 2: Extract SMF data with DFSORT

You can use DFSORT to consolidate and report historical SMF data. This is
helpful if you store your SMF records in a GDG by day or week. The following
job uses DFSORT to consolidate SMF records from the SORTIN file and filter
based on the records specified in the INCLUDE COND parameter.
...
To download the data while keeping the RDW data, you can use an FTP server
to receive the extracted file. Make sure the data is transferred in binary
format and the RDW option is used so the RDW is included in the data transfer.
The following job shows a sample FTP job to transfer this file.
...
2. Compress the top-level folder into a single .zip file.
3. Upload the .zip file to the Amazon S3 bucket connected to your AWS Transform workspace.

###### Note

SMF records must be placed in a separate folder from your source code within
the Amazon S3 bucket, and must be in .tar.gz or .gz format.

For detailed instructions on connecting your Amazon S3 bucket to AWS Transform, see
[Set up a connector](transform-app-mainframe-workflow.md#transform-app-mainframe-workflow-setup-connector "transform-app-mainframe-workflow.md#transform-app-mainframe-workflow-setup-connector").
