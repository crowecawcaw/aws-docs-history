...
- **Date and time operators** – You can
  perform arithmetic operations on dates, times, and interval data types. For more
  information, see [Date,
  Timestamp, and Interval Operators](../sqlref/sql-reference-date-timestamp-interval.md "../sqlref/sql-reference-date-timestamp-interval.md") in the
...
- **SQL Functions** – These include the
  ...
  - `CURRENT_TIME` – Returns the time when the query
    executes (UTC).
  - `CURRENT_DATE` – Returns the date when the query
    executes (UTC).
  - `CURRENT_TIMESTAMP` – Returns the time stamp when the
    query executes (UTC).
  - `LOCALTIME` – Returns the current time when the query
    executes as defined by the environment on which Kinesis Data Analytics is running
    (UTC).
  - `LOCALTIMESTAMP` – Returns the current time stamp as
    defined by the environment on which Kinesis Data Analytics is running (UTC).
...
![Console screenshot showing the in-application schema with event time and ticker columns..](images/ex_datetime_convert_0.png)
...
3. Choose **Create Kinesis stream**, and create a stream
   with one shard.
4. Run the following Python code to populate the stream with sample data.
   This simple code continuously writes a record with a random ticker
   symbol and the current time stamp to the stream.
...
2. Choose **Create application**, type an application
   name, and choose **Create application**.
...
4. On the **Connect to source** page, do the
   ...
   3. Choose **Discover schema**. Wait for the
      console to show the inferred schema and the sample records that
      are used to infer the schema for the in-application stream
...
5. On the application details page, choose **Go to SQL
   editor**. To start the application, choose **Yes,
