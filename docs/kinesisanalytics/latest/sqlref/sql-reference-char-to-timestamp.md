...
Note that each string must be enclosed in single quotes and each element of the <input\_date\_time\_string> must be in the range for its corresponding element in the template, otherwise no result is returned.
