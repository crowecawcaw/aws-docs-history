...
- When the input set has no non-null data, or when applied to an empty set, `VAR_POP` returns
  `NULL`.
- If you don't use the `OVER` clause, `VAR_POP` is calculated as an
...
- If you use the `OVER` clause, `VAR_POP` is calculated as an analytic
...
The following example demonstrates how to use the `VARPOP` function to determine the population variance of the values
...
The following example demonstrates how to use the `VARPOP` function to determine the population variance of the values in a
