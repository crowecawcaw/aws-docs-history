...
| --- | --- | --- | --- | 
| Aggregate Functions | One output row per group of input rows. | All output columns are calculated over the same window or same group of rows. | COUNT DISTINCT is not allowed in streaming aggregation. Statements of the following type are not allowed:<br />SELECT COUNT(DISTINCT x) ... FROM ... GROUP BY ... | 
...
+ [COUNT\_DISTINCT\_ITEMS\_TUMBLING Function](count-distinct-items.md)
+ [EXP\_AVG](sql-reference-exp-avg.md)
+ [FIRST\_VALUE](sql-reference-first-value.md)
+ [LAST\_VALUE](sql-reference-last-value.md)
+ [MAX](sql-reference-max.md)
+ [MIN](sql-reference-min.md)
+ [SUM](sql-reference-sum.md)
+ [TOP\_K\_ITEMS\_TUMBLING Function](top-k.md)
