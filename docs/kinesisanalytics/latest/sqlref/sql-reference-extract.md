...
| `<br>EXTRACT (HOUR FROM CURRENT_ROW_TIMESTAMP)<br>`<br>where CURRENT\_ROW\_TIMESTAMP is 2016-09-23 04:29:26.234 | 4      |

### Use in Function

EXTRACT can be used for conditioning data, as in the following function which returns a 30
minute floor when [CURRENT\_ROW\_TIMESTAMP](sql-reference-current-row-timestamp.md "sql-reference-current-row-timestamp.md") is input for p\_time.
