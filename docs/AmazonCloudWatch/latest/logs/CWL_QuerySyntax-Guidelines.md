...
For system fields that start with `@` and contain special
characters in the remaining name, place the `@` symbol outside
the backticks and enclose only the field name portion. For example, resource
tag fields use the pattern `@aws.tag.<tagKey>`. If the tag
key contains special characters such as colons, you must use the following
syntax:
