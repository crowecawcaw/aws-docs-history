...
1. Add the following statement to the key policy statement list. When you do, replace {{us-east-1}} with the Region of your logs, {{account-ARN}} with the ARN of the account that owns the KMS key, {{123456789012}} with the account number that owns the KMS key,{{key\_id}} with the kms-key Id and {{role\_name}} with the role used for creating export task.
