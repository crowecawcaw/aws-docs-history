...
filter against the recipient destination.

Perform the steps in this section in the sender account. The role must
exist in the sender account, and you specify the ARN of this role in the
...
1. Create the following trust policy in a file
   ...
   Use a text editor to create this policy file; do not use the IAM
   console.
...
2. Create the IAM role that uses this policy. Take note of the
   `Arn` value that is returned by the command, you will
   need it later in this procedure. In this example, we use
   `CWLtoSubscriptionFilterRole` for the name of the
   role we're creating.
