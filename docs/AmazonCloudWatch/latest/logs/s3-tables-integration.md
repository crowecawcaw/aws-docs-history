...
To get started with S3 Tables Integration, you need to set up the integration between your CloudWatch Logs and S3 Tables. This process involves configuring data source associations and setting up appropriate IAM permissions.<a name="create-s3-table-integration"></a>
...
1. Choose **Settings**, **Global**, **Create S3 Table Integration**.

1. Customize how logs will be encrypted in S3 Tables, and the role that CloudWatch Logs will use to write your logs into S3 Tables.

1. If you want all data sources to be automatically associated with the integration, leave the **Enable all log sources and types to be available in the S3 table** checkbox selected (it is selected by default). If you want to associate only specific data sources, clear this checkbox.

1. Choose **Create S3 Table Integration**.

**Note**  
If you selected **Enable all log sources and types to be available in the S3 table** during creation, all data sources are automatically associated, including any data sources added in the future. You can stop here. The following steps are only needed if you cleared the checkbox and want to associate specific data sources.<a name="associate-sources-s3-table-integration"></a>

**To associate sources to an S3 Table Integration**

1. Open the CloudWatch Logs console at [https://console.aws.amazon.com/cloudwatch/](https://console.aws.amazon.com/cloudwatch)".

1. Choose **Settings**, **Global**, **Manage S3 Table Integration**.

1. Choose **Associate data source**.

1. Select the data source name and data source type that you want to enable integration for.

1. Choose **Associate data source**.<a name="associate-sources-to-s3-tables-integration-via-log-mgmt"></a>

**To associate sources to an S3 Table Integration from the Log Management Page**

1. Open the CloudWatch Logs console at [https://console.aws.amazon.com/cloudwatch/](https://console.aws.amazon.com/cloudwatch)".

1. Choose **Log Management** in the navigation pane.

1. Select **Data Sources** tab.

1. Choose the data source name and data source type that you want to integrate.

1. Choose **Data source actions**.

1. Select **Associate with S3 Tables Integration**.

1. Review the data sources, and then choose **Associate Data source**.
...
1. Connect with Analytics Tools

### Integrating Amazon Amazon S3 Tables with AWS analytics services - Using the Amazon S3 console ([Link](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-tables-integrating-aws.html))
...
1. Choose the **Enable integration** on the top.

1. The first time that you integrate table buckets in any Region, Amazon Amazon S3 creates a new IAM service role on your behalf. This role allows Lake Formation to access all table buckets in your account and federate access to your tables in AWS Glue Data Catalog.

### Configure Lake Formation Permissions
<a name="configure-lake-formation-permissions"></a>

While CloudWatch Logs has permission to write to the table (configured in previous steps), users and analytics roles do not automatically have permission to read the data. You must explicitly grant access using AWS Lake Formation. You have to do this for every IAM principal you want to provide access to the table.<a name="grant-query-access"></a>
...
1. **Principals**: Select the IAM users or roles that require access (e.g., your data analysts or the Admin role you are currently using).

1. **LF-Tags or Catalog resources**: Select **Named Data Catalog resources**.
...
1. **Table Permissions**: Select **Select** and **Describe**.
...
Amazon Athena interacts with S3 Tables through the Amazon S3 Tables catalog.<a name="setup-athena-query"></a>
...
1. If you do not see the catalog, ensure you have completed the Lake Formation permission steps above for your specific user role.

1. Once the catalog is selected, your log tables will appear in the database list. You can now run standard SQL queries against your log data.

**Example Query:** `SELECT * FROM "amazon_vpc__flow" LIMIT 100;`

Learn more about connecting Analytics services with S3 Tables at [https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-tables-integrating-aws.html](https://docs.aws.amazon.com/AmazonS3/latest/userguide/s3-tables-integrating-aws.html).