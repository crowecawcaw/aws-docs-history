...
Additionally, the instance must have an NVIDIA driver installed. NVIDIA drivers on
 ...
 Linux instances](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/install-nvidia-driver.html "https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/install-nvidia-driver.html"). 
...
| `clocks_current_video` | `nvidia_smi_clocks_current_video` | The current frequency of the video (encoder plus decoder) clocks. | All of these metrics are collected with the following dimensions:
| Dimension | Description | | --- | --- |
| `index` | A unique identifier for the GPU on this server. Represents the NVIDIA Management Library (NVML) index of the device. | | `name` | The type of GPU. For example, `NVIDIA Tesla A100` |
