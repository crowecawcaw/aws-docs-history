...
**Note**  
For logs pipelines, `PipelineErrorsByErrorType` includes additional dimensions (`ErrorSource`, `ErrorComponent`) and `PipelineWarningsByWarningType` includes additional dimensions (`WarningSource`, `WarningComponent`).

### Logs pipeline metrics
<a name="logs-pipeline-metrics"></a>
...
<a name="metrics-pipeline-operational-metrics"></a>
...
| `PipelineDataPointsUnprocessed` | Number of metric datapoints that matched selection criteria but passed through unchanged because all processors were skipped. For example, this occurs when `UnsupportedTemporality` or `DestructiveOnVended` restrictions apply. You can create an alarm on this metric to detect metrics in your selection criteria that cannot be processed. | PipelineName, dataset | Count | 
...
A destructive processor attempted to modify a vended metric (`instrumentation_scope.name` starting with `cloudwatch.aws/`). The pipeline skipped the operation and counted the affected datapoints in `PipelineDataPointsUnprocessed`.
