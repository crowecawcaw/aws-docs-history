...
1. Choose **Add** > **Access Key**.

1. Note the Access Key ID and Secret Key.

1. Identify the CSPM API URL based on your tenant region (for example, `https://api.prismacloud.io`).

1. Identify the Compute Console URL by navigating to Compute > Manage > System > Utilities > Path to Console.

1. Identify the Compute API version (available from the bell icon in the Console).

1. Store the Access Key ID as `username` and Secret Key as `password` in AWS Secrets Manager.
