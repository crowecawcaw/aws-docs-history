...
To compile lists of top contributors, Network Flow Monitor determines the network flows
 ...
 are the *top contributors* for each metric type.
...
Network Flow Monitor classifies metrics into designated destination categories. Metrics are 
...
* Between AZs (`INTER_AZ`)
* Within AZs (`INTRA_AZ`)
* Between VPCs (`INTER_VPC`)
* Between Regions
 (`INTER_REGION`, which means performance for network flows between resources 
