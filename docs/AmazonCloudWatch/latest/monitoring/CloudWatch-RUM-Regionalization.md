...
- Deploy one app monitor in one Region, in one account, serving all
  Regions.
...
The advantage of using one app monitor is that all data will be centralized into
one visualization, and all logs are written to the same log group in CloudWatch Logs. With a
single app monitor there is a small amount of extra latency for requests, and a
...
benefits. End users can be anywhere and still have their sessions included if
you have set up an app monitor in the Region that they are connecting to.

However, CloudWatch RUM is not yet launched in any Regions in China. You are not
able to send data to CloudWatch RUM from these Regions.
