...
The SageMaker AI Insights dashboard is organized into three tabs: **Performance**,
**Capacity**, and **Reliability**. Each tab provides a focused view
of your inference endpoint health.
...
![SageMaker AI Insights dashboard header with summary bar showing Invocations, Instances, Inference Components, and Avg AZ Skew.](images/smai_New_Insights_Dashbaord.png)
...
![Endpoints list page showing the View in SageMaker AI Insights button.](images/smai_Observability_Enabled_page.png)

- **From the endpoint detail page:** Choose **View in SageMaker AI Insights** to open the dashboard filtered to that endpoint.

![Endpoint detail page showing the View in SageMaker AI Insights button.](images/smai_Endpoint_detail_page.png)
...
![Inference component detail page showing the View in SageMaker AI Insights button.](images/smai_IC_Detaild_page.png)

- **Direct navigation:** CloudWatch console → **Infrastructure monitoring** → **SageMaker AI Insights**.

## Performance tab

The **Performance** tab answers "is it healthy?" and "why is it slow?"—flowing
from fleet‐wide health at the top to per‐IC diagnostics at the bottom.

Performance health

Honeycombs grouped by AZ showing alarm state for instances, IC copies, and endpoints at a glance.

![Performance health honeycombs showing alarm state by AZ.](images/smai_Performance_health_AZ_honey_comb.png)
...
![Instance performance table showing TTFT, output TPS, concurrent requests, and KV cache.](images/smai_Instance_health_performance_tab.png)

Token streaming

Time‐series chart showing TTFT and inter‐token latency (ITL) with a P50/P99 toggle, broken down by framework.

![Token streaming chart showing TTFT and ITL P50/P99 over time.](images/smai_Token_streaming.png)

Token throughput

Input and output tokens per second by framework, with toggles for Input/Output, Percentiles, and By instance views.

![Token throughput chart showing input and output tokens per second.](images/smai_Token_throughput.png)
...
![Engine and request pressure panel showing KV cache, running requests, and waiting requests.](images/smai_Engine_and_request_pressure.png)
...
![Traffic distribution table showing invocations per minute, 4XX rate, and 5XX rate by instance.](images/smai_Traffic_distribution.png)

Error mix over time

Line chart showing 4XX, 5XX, and mid‐stream error rates over time per IC.

![Error mix over time line chart showing 4XX, 5XX, and mid-stream errors.](images/smai_Error_mix_over_time.png)
...
![Latency breakdown over time stacked area chart showing model latency and overhead latency.](images/smai_Latency_breakdown_over_time.png)

## Capacity tab

The **Capacity** tab answers "do I have headroom?" and "is my hardware
healthy?"—showing actual utilization compared to reserved capacity.

Capacity health

The same honeycomb visualization as the **Performance** tab, showing alarm state for instances, IC copies, and endpoints.

![Capacity health honeycombs showing alarm state by AZ.](images/smai_Capacity_health_honey_comb.png)

Instance capacity table

Per‐instance utilization bars for GPU, GPU memory, CPU, memory, and disk.

![Instance capacity table showing GPU, GPU memory, CPU, memory, and disk utilization.](images/smai_Instance_health_capacity_tab.png)
...
![Fleet utilization chart showing CPU, GPU, GPU memory, memory, and disk utilization per instance over time.](images/smai_Fleet_Util.png)

## Reliability tab

The **Reliability** tab answers "is it resilient?" and "why did scaling
fail?"—covering AZ distribution, scaling behavior, and provisioning events.

Availability Zone distribution

Bar chart showing instance or IC copy count per AZ to validate high availability (HA) compliance.

![Availability Zone distribution bar chart showing instance count per AZ.](images/smai_AZ_distribution.png)
...
![Cold start anatomy showing model download, GPU load, container start, and platform overhead.](images/smai_Cold_start_anatomy.png)
...
![ICE diagnostics panel showing ICE count over time and an event table.](images/smai_ICE_events.png)
