...
include this data in the report, see [executeHttpStep(stepName, requestOptions, [callback], [stepConfig])](CloudWatch_Synthetics_Canaries_Library_Nodejs.md#CloudWatch_Synthetics_Library_executeHttpStep "CloudWatch_Synthetics_Canaries_Library_Nodejs.md#CloudWatch_Synthetics_Library_executeHttpStep").
...
see [Synthetics runtime versions](CloudWatch_Synthetics_Canaries_Library.md "CloudWatch_Synthetics_Canaries_Library.md").
...
- Install the latest version of the AWS CLI. For more information, see [AWS CLI
  install and update instructions](../../../cli/latest/userguide/getting-started-install.md#getting-started-install-instructions "../../../cli/latest/userguide/getting-started-install.md#getting-started-install-instructions") .
...
runtime version changes](../../../lambda/latest/dg/runtimes-update.md#runtime-management-identify.html "../../../lambda/latest/dg/runtimes-update.md#runtime-management-identify.html") to find the ARN of the
...
manual mode. Be sure to replace the `function-name` and `qualifier` with the Lambda function ARN and Lambda function version number
respectively, using the values you found in Step 1. Also replace the the `runtime-version-arn` with the version ARN that you found in Step 2.
...
- Check that the canary's IAM role has the `s3:ListAllMyBuckets`
  ...
  stores its artifacts. If the canary performs visual monitoring, the role also needs the `s3:GetObject` permission for the bucket. These same permissions are also required
...
- If the canary uses an AWS KMS customer managed key for encryption instead of the standard AWS
  ...
  or decrypt using that key. For more information, see [Encrypting canary artifacts](CloudWatch_Synthetics_artifact_encryption.md "CloudWatch_Synthetics_artifact_encryption.md").
...
If the canary performs visual monitoring, see [Updating artifact location and encryption when using visual monitoring](CloudWatch_Synthetics_artifact_encryption.md#CloudWatch_Synthetics_artifact_encryption_visual "CloudWatch_Synthetics_artifact_encryption.md#CloudWatch_Synthetics_artifact_encryption_visual") for more information.
...
that you set up CloudWatch Synthetics to use VPC. For more information, see [Running a canary on a VPC](CloudWatch_Synthetics_Canaries_VPC.md "CloudWatch_Synthetics_Canaries_VPC.md").
...
version, it may be a cross-origin request sharing (CORS) issue. For more information, see [Cross-origin request sharing (CORS) issue](#CloudWatch_Synthetics_Canaries_CORS "#CloudWatch_Synthetics_Canaries_CORS").
...
availability of functions, see [Writing a canary script](CloudWatch_Synthetics_Canaries_WritingCanary.md "CloudWatch_Synthetics_Canaries_WritingCanary.md").
...
In a UI canary, if some network requests are failing with `403` or `net::ERR_FAILED`, check whether the canary has active tracing enabled and also uses
...
You can override launch parameters used by CloudWatch Synthetics and pass additional `disable-web-security` flag parameters by using the CloudWatch Synthetics launch function.
For more information, see [Library functions available for Node.js canary scripts using Puppeteer](CloudWatch_Synthetics_Canaries_Library_Nodejs.md "CloudWatch_Synthetics_Canaries_Library_Nodejs.md").
...
permissions. To run on a VPC, a canary must have the permissions `ec2:CreateNetworkInterface`, `ec2:DescribeNetworkInterfaces`, and `ec2:DeleteNetworkInterface`. These permissions are all contained in the `AWSLambdaVPCAccessExecutionRole` managed policy. For more information, see [Execution Role and User
Permissions](../../../lambda/latest/dg/configuration-vpc.md#vpc-permissions "../../../lambda/latest/dg/configuration-vpc.md#vpc-permissions").
