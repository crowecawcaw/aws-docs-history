...
You can publish your own custom metrics to CloudWatch using the `PutMetricData` API or the AWS CLI. This approach uses CloudWatch namespaces, metric names, and dimensions.

**Note**  
For new implementations, we recommend using OpenTelemetry to publish custom metrics. For more information, see [Publish custom metrics with OpenTelemetry](metrics-otel-send.md).
