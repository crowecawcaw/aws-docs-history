...
After you download the template file, open it and replace {{PATH\_TO\_CloudFormation\_TEMPLATE}} with the path where you saved the template file. Then export the following parameters and run the CloudFormation command, as shown in the following command.
+ **Cluster\_Name**– The Amazon ECS cluster name
+ **AWS\_Region**– The Region where the data will be sent
+ **PATH\_TO\_CloudFormation\_TEMPLATE**– The path where you saved the CloudFormation template file.
