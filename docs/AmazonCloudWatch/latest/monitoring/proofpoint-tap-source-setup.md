...
2. Store the `servicePrincipal` and `secret` in AWS Secrets Manager.
3. Create a CloudWatch Logs log group for the pipeline output.
4. Create an IAM role with AWS Secrets Manager access and a trust relationship for the pipeline.
5. Configure and create the CloudWatch pipeline.
...
2. Navigate to Settings > Connected Applications > Service Credentials.
3. Generate a new service credential.
4. Note the Service Principal and Secret values.
5. Store the `servicePrincipal` and `secret` in AWS Secrets Manager.
