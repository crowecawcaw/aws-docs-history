...
1. Install the Amazon ECS CLI if you haven't already done so. For more information,
 see  [Installing
 the Amazon ECS CLI](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ECS_CLI_installation.html "https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ECS_CLI_installation.html").
...
```
3. (Optional) If you don't already have an Amazon ECS Fargate cluster where you
 want to install the sample Redis OSS workload and CloudWatch agent, you can create one
...
1. Set your VPC and subnet ID of the Amazon ECS cluster. If you created a new
 ...
 final command. Otherwise, use the IDs of the existing cluster that you are going
 to use with Redis.
...
2. In this tutorial, we are going to install the Redis OSS application and the
 ...
 security groups for the Redis OSS application and CloudWatch agent and set customized
...
Then set the Fargate cluster deafult security group variable by entering
 the following command, replacing
 `my-default-security-group` with the value you found
...
```
2. Set the IAM role names to be created for Redis OSS by entering the
 following commands.
...
```
3. Install the sample Redis OSS workload by entering the following
 command.
...
* The primary container runs a simple Redis OSS application and opens port
 6379 for access.
...
1. Download the latest version of
 `cwagent-ecs-prometheus-metric-for-awsvpc.yaml` by entering
...
```
2. Open the file with a text editor, and find the full CloudWatch agent configuration
 behind the `value` key in the
 `resource:CWAgentConfigSSMParameter` section.
...
For the `metric_declaration` section, the default setting does
 not allow any Redis OSS metrics. Add the following section to allow Redis OSS
...
5. Review the newly created changeset `redis-scraping-support`. You
 ...
 resource. Execute the changeset and restart the CloudWatch agent task by entering the
 following commands.
...
```
7. If you are installing the CloudWatch agent with Prometheus metric collecting for
 the cluster for the first time, please enter the following commands:
