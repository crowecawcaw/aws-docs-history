...
cluster at the time you re-create it. You can enable it by re-creating it, and then
...
replaced all the placeholders such as `cluster-name` and
`region-name` in the commands that you are using with the
...
Bottlerocket uses a different `containerd` path on the host, so you need to
change the volumes to its location. If you don't, you see an error in the logs that
includes `W! No pod metric collected`. See the following
example.
...
limited to scenarios where customers collected the logs of the CloudWatch agent itself and were
...
resources based on the number of cores and amount of RAM required by the agents. Enter the
following command to describe the pod:
