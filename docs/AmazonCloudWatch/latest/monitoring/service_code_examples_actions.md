...
The following code examples demonstrate how to perform individual CloudWatch actions with AWS SDKs. Each example includes a link to GitHub, where you can find instructions for setting up and running the code. 

These excerpts call the CloudWatch API and are code excerpts from larger programs that must be run in context. You can see actions in context in [Scenarios for CloudWatch using AWS SDKs](service_code_examples_scenarios.md). 
