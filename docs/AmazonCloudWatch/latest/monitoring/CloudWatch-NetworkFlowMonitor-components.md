...
**Agents**
An *agent* in Network Flow Monitor is a software application that you install on your Amazon EC2
...
**Top contributors**
*Top contributors* are the network flows that have the highest values for
 ...
 *workload insights*. In addition, if you create a monitor, 
...
A *local resource* is the host location—or
...
A *remote resource* is the other end of a network flow. In this example
...
**Workload insights**
*Workload insights* includes the performance metrics returned for all the network
...
**Monitors**
You create a *monitor* so that you can monitor, on an ongoing basis, the
...
The *network health indicator* (NHI) is a binary value that informs you 
...
**Scope**
In Network Flow Monitor, the *scope* is the account or accounts that you
...
**Query ID**
Network Flow Monitor generates a unique *query ID* for each query that
...
**Performance metrics**
Network Flow Monitor gathers and calculates end-to-end *performance metrics*, including TCP 
