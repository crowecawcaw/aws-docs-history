...
rolling intervals.
    ...
    + *Calendar intervals* are aligned with the calendar, such as an SLO that is tracked
    ...
    + *Rolling intervals* are calculated on a rolling basis. Rolling intervals are better
...
interval that can breach the threshold but still let the SLO be met. The _total
error budget_ is the total amount of breaching time that can be tolerated
...
Creating alarms incurs charges. For more information about CloudWatch pricing, see [Amazon CloudWatch Pricing](https://aws.amazon.com/cloudwatch/pricing/ "https://aws.amazon.com/cloudwatch/pricing/").
...
see [What is AppRegistry?](../../../servicecatalog/latest/arguide/intro-app-registry.md "../../../servicecatalog/latest/arguide/intro-app-registry.md") 13. Choose **Create SLO**. If you also chose to create one or more alarms,
...
4. For **Set Service Level Indicator (SLI)**, do one of the following:
   ...
   - To set the SLO on a service operation, all operations, or the dependency of a service,
     ...
     6. For **Select a calculation method**, choose **Requests**. 7. The **Select service** and **Select operation**
     drop-downs are populated by services and operations that have been active within
     the past 24 hours. 8. Choose either **Availability** or **Latency**.
...
**Services** option, see [Monitor the operational health of your applications with Application Signals](Services.md "Services.md").
...
see [SLO concepts](#CloudWatch-ServiceLevelObjectives-concepts "#CloudWatch-ServiceLevelObjectives-concepts").
...
For more information, see [Managing alarms](Manage-CloudWatch-Alarm.md "Manage-CloudWatch-Alarm.md").
