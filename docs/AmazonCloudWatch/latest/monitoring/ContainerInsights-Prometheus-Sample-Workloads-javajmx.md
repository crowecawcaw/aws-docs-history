...
an Amazon EKS or Kubernetes cluster. This is done by `role: pod` discovery
...
The fields to change are the `/metrics` and `regex:
 '.*:9404$'` fields, as highlighted in the following example.
...
When you have built the image, load it into Amazon EKS or Kubernetes, and then run
the following command to verify that Prometheus metrics are exposed by
`JMX_EXPORTER` on port 9404. Replace
`$JAR_SAMPLE_TRAFFIC_POD` with the running pod name and
replace `$JAR_SAMPLE_TRAFFIC_NAMESPACE` with your
application namespace.

If you are running JMX Exporter on a cluster with the Fargate launch type,
you also need to set up a Fargate profile before doing the steps in this
procedure. To set up the profile, enter the following command. Replace
...
example Dockerfile shows the steps to build a testing image:
...
- `setenv.sh` is a Tomcat startup script to start the JMX
  exporter along with Tomcat and expose Prometheus metrics on port 9404 of the
  localhost. It also provides the JMX Exporter with the
...
- your web application.war is your web application `war`
  file to be loaded by Tomcat.
...
- `start_exporter_example.sh` is the script to start the
  JAR application with the Prometheus metrics exported. It also provides the JMX
  Exporter with the `config.yaml` file path.
...
- SampleJavaApplication-1.0-SNAPSHOT.jar is the sample Java application jar
  file. Replace it with the Java application that you want to monitor.
