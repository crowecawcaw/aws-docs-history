...
**Note**  
Don't enter a reserved or AWS-internal IP address as a destination, such as an address in the link-local range `169.254.0.0/16` or the address `10.0.0.2`. Network Synthetic Monitor can't correctly measure round-trip time and packet loss for these addresses, and reports round-trip time values below 1 millisecond. For more information, see [Network Synthetic Monitor requirements and limitations](what-is-network-monitor.md#nw-monitor-limitations).
