...
name that starts with `cw-agent-prometheus`.

```
kubectl describe pod `pod-name` -n amazon-cloudwatch
```

If you have CloudWatch Container Insights
installed, you can use CloudWatch Logs Insights to query the logs from the CloudWatch agent collecting
...
3. Select the log group for the application logs,
   **/aws/containerinsights/`cluster-name`/application**
4. Replace the search query expression with the following query, and choose
   **Run query**
...
You can also confirm that Prometheus metrics and metadata are being ingested as
...
they are histogram metrics. You can also log a list of the first 500 Prometheus
metrics that are dropped and not sent to CloudWatch because they are histogram
metrics.
...
If you see those lines and want to know what metrics are being dropped, use the
following steps.
...
1. Change the CloudWatch agent to debug mode by adding the following bold lines to your
   `prometheus-eks.yaml` or
   `prometheus-k8s.yaml`file, and save the file.
...
2. Reinstall the CloudWatch agent to enable debug mode by entering the following
   commands:
...
The dropped metrics are logged in the CloudWatch agent pod. 3. To retrieve the logs from the CloudWatch agent pod, enter the following
command:
...
Or, if you have Container Insights Fluentd logging installed, the logs are
also saved in the CloudWatch Logs log group
...
`prometheus-k8s.yaml` files, the line `job_name:
 'kubernetes-pod-appmesh-envoy'` scrapes App Mesh metrics. The Prometheus target
is defined as `kubernetes-pod-appmesh-envoy`. So all App Mesh Prometheus
metrics are ingested as CloudWatch Logs events in the log stream
...
For more information about embedded metric format, see [Specification: Embedded metric format](CloudWatch_Embedded_Metric_Format_Specification.md "CloudWatch_Embedded_Metric_Format_Specification.md").
