...
Under **Configure Checks**, you will see two tabs, **Checks** and **Canary configuration**. 4. Select the runtime version **syn-nodejs-3.0** or later. 5. Follow the procedure under [Writing a JSON configuration for Node.js multi Checks blueprint](CloudWatch_Synthetics_WritingCanary_Multichecks.md "CloudWatch_Synthetics_WritingCanary_Multichecks.md") to describe the check you want to perform. Alternatively, the console provides you a default JSON configuration that you can build on. 6. Choose **Create Canary**.
...
`DEBUG` logs such that the following run will provide more additional details on why the Checks are failing. To do so, provide the Environment Variable CW_SYNTHETICS_LOG_LEVEL: "DEBUG".
