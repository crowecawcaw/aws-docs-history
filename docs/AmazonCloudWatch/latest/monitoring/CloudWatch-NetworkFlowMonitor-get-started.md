...
- Deploy _agents_ on your instances, by using AWS Systems Manager or
...
- Review top contributor metrics for network flows returned by the agents,
  to gain _workload insights_. Workload insights provide a high-level
...
- Based on the network flows that you want to see detailed network information
  about, create one or more _monitors_. For each monitor, specify the
