...
<a name="Concepts_NotificationsArticle"></a>
...
<a name="setting-notifications"></a>
...
To set up an Amazon SQS queue for use with Mechanical Turk, follow the setup instructions found in [Notification Handling Using Amazon SQS](https://docs.aws.amazon.com/AWSMechTurk/latest/AWSMturkAPI/ApiReference_NotificationReceptorAPI_SQSTransportArticle.html). To set up an Amazon SNS topic, follow the steps in [Notification Handling Using Amazon SNS](https://docs.aws.amazon.com/AWSMechTurk/latest/AWSMturkAPI/ApiReference_NotificationReceptorAPI_SNSTransportArticle.html). Note that in both cases, you need to configure permissions properly to allow Mechanical Turk to send messages to your topic or queue. 
...
1. Select the **Role name**. This redirects you to summary page for that role in the IAM console. 

1. Choose **Attach Policies**.

1. In the search field, enter AmazonMechanicalTurkFullAccess and select the check box next to that policy. 
...
1. Choose **Add trigger** to add your Amazon SNS topic as a trigger for your Lambda function. 

1. When prompted to select a trigger service, search for and select **SNS**, then select the topic you created for notifications. 

1. Enter the code that you want to use to process Amazon SNS messages. The following Python code provides a template for getting started. 
...
1. After you've added the code to your Lambda function and edited it to meet your needs, you can select **Deploy** and begin using it. 
...
1. Select the **Create new test event** radio button. 

1. Enter a name for your event in the text box.

1. Enter the test event. The following can be used for your test parameters after replacing the `HITId` with a completed HIT in your account. 
...
<a name="notifications-and-reliability"></a>
