...
Because the sandbox is a mirror of production environment, you need to complete account setup a second time, as described in [Set up Amazon Mechanical Turk](SetUpMturk.md). To enable easy switching between testing and production, you can link to the same AWS account and use the same AWS credentials. To switch to the sandbox environment, simply specify that you want the CLI or SDK to use the sandbox endpoint (`https://mturk-requester-sandbox.us-east-1.amazonaws.com`).

## API calls using the CLI
<a name="mturk-use-sandbox-use-cli"></a>
...
<a name="mturk-use-sandbox-use-boto3"></a>
...
## Testing HITs
<a name="mturk-use-sandbox-test-hits"></a>
