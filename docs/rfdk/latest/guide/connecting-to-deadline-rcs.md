...
<a name="connecting-to-deadline-rcs"></a>
...
1. Select the instance with a name that ends in ` <render-queue-construct-id>/Cluster/RCS Capacity`.
...
1. Copy the "ID" of the container that is running the Deadline RCS. This container should be using an image from an ECR repository in your account or from the [AWS Thinkbox ECR Public account](https://gallery.ecr.aws/aws-thinkbox/deadline/rcs). The `entrypoint` of the container should be `./configure-rcs.sh`.

1. Create a bash shell inside the Docker container by running the following command, replacing `<container-id>` with the value from the previous step.
