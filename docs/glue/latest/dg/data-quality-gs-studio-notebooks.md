...
+  A new notebook (see [Getting started with notebooks in AWS Glue Studio](https://docs.aws.amazon.com/glue/latest/ug/notebook-getting-started.html)) 
...
1.  Copy the following code and run it in the cell. 
...
1.  In the next cell, import the `EvaluateDataQuality` class that evaluatesAWS Glue Data Quality. 

   ```
   from awsgluedq.transforms import EvaluateDataQuality
   ```

1. In the next cell, read in the source data by using the .csv file that's stored in the public Amazon S3 bucket. 
...
1.  Convert the data to an AWS Glue DynamicFrame. 
...
1.  Validate the dataset against the ruleset. 
...
1.  Filter passed rows and review the failed rows from the Data Quality row-level results. 
...
1.  Write the output to an Amazon S3 bucket to analyze the data and visualize the results.
