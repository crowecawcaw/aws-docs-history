...
```
 ...
 In the above script, the `Phone Number` was partially redacted with `#`. The `Passport No` was changed into a SHA256 hash. The `DL NO# `was detected as a USA driver license number and was redacted to “USA\_DL” just like it was stated in the `detectionParameters`. 
