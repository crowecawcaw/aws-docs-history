...
To add a classifier in the AWS Glue console, choose **Add classifier**.
...
built-in patterns provided by AWS Glue and custom patterns you write
and include in the **Custom patterns** field.
Although grok debugger results might not match the results from
AWS Glue exactly, we suggest that you try your pattern using some
sample data with a grok debugger. You can find grok debuggers on the
web. The named built-in patterns provided by AWS Glue are generally
...
that when the AWS Glue crawler runs your grok pattern, your data can be
parsed.
...
expression (regex)](http://en.wikipedia.org/wiki/Regular_expression "http://en.wikipedia.org/wiki/Regular_expression") syntax.
...
brackets `< >`. The name must comply with XML rules
for a tag.

For more information, see [Writing XML custom classifiers](custom-classifier.md#custom-classifier-xml "custom-classifier.md#custom-classifier-xml").
...
name in either dot or bracket JSON syntax using AWS Glue supported
operators.

For more information, see the list of operators in [Writing JSON custom classifiers](custom-classifier.md#custom-classifier-json "custom-classifier.md#custom-classifier-json").
...
`Other` to enter a custom delimiter.
...
in the CSV file. You can choose `Has headings`, `No
 headings`, or `Detect headings`. If your custom
...
For more information, see [Writing custom classifiers for diverse data formats](custom-classifier.md "custom-classifier.md").

## Viewing classifiers

To see a list of all the classifiers that you have created, open the AWS Glue console at
[https://console.aws.amazon.com/glue/](https://console.aws.amazon.com/glue/ "https://console.aws.amazon.com/glue/"), and choose the **Classifiers** tab.
...
From the **Classifiers** list in the AWS Glue console, you can add,
