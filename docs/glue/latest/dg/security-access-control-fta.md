...
```
 ...
 Key configurations: 
+  `spark.sql.catalog.glue_catalog`: Registers a catalog named "glue\_catalog" that uses the GlueCatalog implementation 
...
+  The catalog name ("glue\_catalog" in this example) can be customized, but must be consistent in both configuration settings 
