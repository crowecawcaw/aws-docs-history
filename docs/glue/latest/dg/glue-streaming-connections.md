...
**Topics**
+ [Working with Kafka connections](#glue-streaming-kafka-connections)
+ [Working with Kinesis connections](#glue-streaming-kinesis-connections)
...
**Topics**
+ [Configure Kafka](#glue-streaming-etl-connect-kafka-configure)
+ [Example: Reading from Kafka streams](#glue-streaming-etl-connect-kafka-read)
+ [Example: Writing to Kafka streams](#glue-streaming-etl-connect-kafka-write)
+ [Kafka connection option reference](#glue-streaming-etl-connect-kafka)
...
+  `"includeHeaders"`: (Optional) Whether to include the Kafka headers. When the option is set to "true", the data output will contain an additional column named "glue\_streaming\_kafka\_headers" with type `Array[Struct(key: String, value: String)]`. The default value is "false". This option is available in AWS Glue version 3.0 or later. 
...
+ `"addRecordTimestamp"`: (Optional) Used for Read. When this option is set to 'true', the data output will contain an additional column named "\_\_src\_timestamp" that indicates the time when the corresponding record received by the stream. The default value is 'false'. This option is supported in AWS Glue version 4.0 or later.
