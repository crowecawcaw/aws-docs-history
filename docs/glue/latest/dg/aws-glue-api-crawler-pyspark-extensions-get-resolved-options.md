...
by importing it from the AWS Glue `utils` module, along with the `sys`
module:
...
To retrieve the arguments that are passed, you can use the `getResolvedOptions`
function as follows:
...
Note that each of the arguments are defined as beginning with two hyphens, then referenced in
the script without the hyphens. The arguments use only underscores, not
hyphens. Your arguments need to follow this convention to be resolved.
