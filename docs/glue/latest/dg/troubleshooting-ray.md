...
AWS Glue provides access to logs that are emitted by Ray processes during the job run. If you encounter errors or unexpected behavior in Ray jobs, first gather information from the logs to determine the cause of failure. We also provide similar logs for interactive sessions. Sessions logs are provided with the `/aws-glue/ray/sessions` prefix.
...
For more information about Ray architecture, see [Architecture Whitepapers]( https://docs.ray.io/en/latest/ray-contribute/whitepaper.html) in the Ray documentation.
...
In CloudWatch, you can filter down your logs to inspect those specific to this IP address by identifying this suffix. For example, given {{FAILED\_IP}} and {{JOB\_RUN\_ID}}, you can identify the suffix with: 
