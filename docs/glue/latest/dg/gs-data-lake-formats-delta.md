...
1.  AWS Glue Studio displays the format type as Delta Lake and the Amazon S3 URL. 
**Note**  
 If your Delta Lake source is not registered as the AWS Glue Data Catalog table yet, you have two options:   
 Create a AWS Glue crawler for the Delta Lake data store. For more information, see [ How to specify configuration options for a Delta Lake data store](https://docs.aws.amazon.com/glue/latest/dg/crawler-configuration.html#crawler-delta-lake). 
