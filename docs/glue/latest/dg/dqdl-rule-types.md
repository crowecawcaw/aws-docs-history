...
| Distribution | Computes the frequency distribution of a column. For numeric columns, generates a binned histogram. For string and date columns, generates a value distribution sorted in descending order by frequency. | A single column name, or "AllColumns". Optional: with bins = <BIN\_COUNT> | Column.[Column].Distribution | No | Yes | No | No | Yes | No | 
