...
to a development endpoint. You do this so that you can interactively run, debug, and test AWS Glue
extract, transform, and load (ETL) scripts before deploying them. This tutorial uses Secure
Shell (SSH) port forwarding to connect your local machine to an AWS Glue development endpoint. For
...
If you're installing on macOS, you must have Xcode installed before you can install
Sparkmagic.
...
3. Change your directory to the one returned for `Location`, and install the
   kernels for Scala and PySpark.
...
In this configuration file, you can configure Spark-related parameters like
`driverMemory` and `executorCores`.
...
1. Open a separate terminal window that gives you access to SSH. In Microsoft Windows,
...
2. Run the following SSH command, modified as follows:
   ...
   - If you're forwarding a different port than `8998`, replace
     `8998` with the port number that you're actually using locally. The
     address `169.254.76.1:8998` is the remote port and isn't changed by
     you.
   - Replace `dev-endpoint-public-dns` with the
     public DNS address of your development endpoint. To find this address, navigate to
     your development endpoint in the AWS Glue console, choose the name, and copy the
     **Public address** that's listed on the **Endpoint
     details** page.
...
Enter `yes` and leave the terminal window open while you use
JupyterLab. 3. Check that SSH port forwarding is working with the development endpoint
correctly.
...
1. Check that Spark is running successfully. The following command instructs Spark to
   calculate `1` and then print the value.
...
The following script uses the `persons_json` table metadata in the
AWS Glue Data Catalog to create a `DynamicFrame` from your sample data. It then prints
out the item count and the schema of this data.
