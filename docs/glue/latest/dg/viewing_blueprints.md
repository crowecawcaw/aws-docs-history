...
View a blueprint to review the blueprint description, status, and parameter
specifications, and to download the blueprint ZIP archive.

You can view a blueprint by using the AWS Glue console, AWS Glue API, or AWS Command Line Interface
(AWS CLI).
...
- Enter the following command to view just the blueprint name,
  description, and status. Replace `<blueprint-name>` with the name of the
  blueprint to view.
...
Add the `--include-blueprint` argument to include a URL in the output that you
can paste into your browser to download the blueprint ZIP archive that AWS Glue stored.
