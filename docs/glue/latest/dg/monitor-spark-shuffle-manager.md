...
![Spark Map Stage writes to disk, shuffles data to S3, then Spark Reduce Stage reads from S3.](https://docs.aws.amazon.com/glue/latest/dg/images/gs-s3-shuffle-diagram.png)
...
![Job parameters section showing write-shuffle-files key with optional value field and Remove button.](https://docs.aws.amazon.com/glue/latest/dg/images/gs-s3-shuffle.png)
...
**Important**  
To disable Amazon S3 shuffling, you must remove the `--write-shuffle-files-to-s3` parameter entirely from your job configuration. Setting the value to `false` does not disable Amazon S3 shuffling — the parameter acts as a presence-based flag, meaning that any value (including `false`) enables Amazon S3 shuffling when the parameter is present.
