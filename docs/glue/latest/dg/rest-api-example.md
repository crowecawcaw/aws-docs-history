...
1.  Create the AWS Glue connection by calling the CreateConnection API using the AWS API, CLI, or SDK. 
   ...
   1.  Provide the **SecretArn** and other **AuthenticationProperties** that are configured. Because this example uses `OAUTH2` as the AuthenticationType, set the "OAuth2Properties" in the CreateConnectionInput. This requires properties such as "OAuth2GrantType", "TokenUrl", and "OAuth2ClientApplication". 
