...
```
 ...
 The `IpToInt` transformation takes the `source\_column` as `"source\_column\_ip"` and the `target\_column` as `"target\_column"` and converts the valid IPv4 address strings in the `source\_column\_ip` column to their corresponding 32-bit integer representation and stores the result in the `target\_column` column. 
