...
If
you're running AWS Glue ETL jobs that read files or partitions from Amazon Simple Storage Service (Amazon S3), you can
exclude some Amazon S3 storage class types.

The following storage classes are available in Amazon S3:

- `STANDARD` — For general-purpose storage of frequently accessed
  data.
- `INTELLIGENT_TIERING` — For data with unknown or changing access
  patterns.
- `STANDARD_IA` and `ONEZONE_IA` — For long-lived, but less
...
Archived Objects](../../../AmazonS3/latest/dev/restoring-objects.md "../../../AmazonS3/latest/dev/restoring-objects.md") in the _Amazon S3 Developer Guide_.)
...
uses its own Amazon S3 `Lister` implementation to list and exclude files corresponding
to the specified storage classes.

The following Python and
Scala examples show how to exclude the `GLACIER` and `DEEP_ARCHIVE`
...
[Table Structure](aws-glue-api-catalog-tables.md#aws-glue-api-catalog-tables-Table "aws-glue-api-catalog-tables.md#aws-glue-api-catalog-tables-Table") and [CreateTable](../webapi/API_CreateTable.md "../webapi/API_CreateTable.md").
...
3. Choose the table name in the list, and then choose **Edit
   table**.
4. In **Table properties**, add
   `excludeStorageClasses` as a key and
