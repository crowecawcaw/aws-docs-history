...
The Development endpoints API describes the AWS Glue API related
to testing using a custom DevEndpoint.

## Data types

- [DevEndpoint structure](#aws-glue-api-dev-endpoint-DevEndpoint "#aws-glue-api-dev-endpoint-DevEndpoint")
- [DevEndpointCustomLibraries structure](#aws-glue-api-dev-endpoint-DevEndpointCustomLibraries "#aws-glue-api-dev-endpoint-DevEndpointCustomLibraries")
...
- `RoleArn` – UTF-8 string, matching the [AWS IAM ARN string pattern](aws-glue-api-common.md#aws-glue-api-regex-aws-iam-arn-id "aws-glue-api-common.md#aws-glue-api-regex-aws-iam-arn-id").
...
VPC if the `DevEndpoint` is created within one. The `PrivateAddress`
field is present only when you create the `DevEndpoint` within your
VPC.
...
Accepts a value of Standard, G.1X, or G.2X.
    ...
    + For the `G.1X` worker type, each worker maps to 1 DPU (4 vCPU,
     16 GB of memory, 64 GB disk), and provides 1 executor per worker. We recommend this
     worker type for memory-intensive jobs.
    + For the `G.2X` worker type, each worker maps to 2 DPU (8 vCPU,
     32 GB of memory, 128 GB disk), and provides 1 executor per worker. We recommend
...
by using the `Arguments` parameter in the `CreateDevEndpoint`
...
Libraries that rely on C extensions, such as the [pandas](http://pandas.pydata.org/ "http://pandas.pydata.org/")
...
This attribute is provided for backward compatibility because the recommended
...
attribute, and the list of new keys in the `addPublicKeys` attribute.
...
(Amazon S3) bucket that should be loaded in your `DevEndpoint`.
...
- `EndpointName` – _Required:_ UTF-8 string.

The name to be assigned to the new `DevEndpoint`.

- `RoleArn` – _Required:_ UTF-8 string, matching the [AWS IAM ARN string pattern](aws-glue-api-common.md#aws-glue-api-regex-aws-iam-arn-id "aws-glue-api-common.md#aws-glue-api-regex-aws-iam-arn-id").
...
If you previously created an endpoint with a public key, you must remove
that key to be able to set a list of public keys. Call the `UpdateDevEndpoint`
API with the public key content in the `deletePublicKeys` attribute,
...
the DevEndpoint. For more information about tags in AWS Glue, see
[AWS Tags in AWS Glue](monitor-tags.md "monitor-tags.md") in the developer guide.
...
Path to one or more Java `.jar` files in an S3 bucket that will
...
The name of the `SecurityConfiguration` structure being
used with this `DevEndpoint`.
...
- `CustomLibraries` – A [DevEndpointCustomLibraries](#aws-glue-api-dev-endpoint-DevEndpointCustomLibraries "#aws-glue-api-dev-endpoint-DevEndpointCustomLibraries") object.
...
field is not populated. When you create a non-VPC development endpoint, AWS Glue returns only a public IP address.
...
- `DevEndpoint` – A [DevEndpoint](#aws-glue-api-dev-endpoint-DevEndpoint "#aws-glue-api-dev-endpoint-DevEndpoint") object.
...
- `DevEndpoints` – An array of [DevEndpoint](#aws-glue-api-dev-endpoint-DevEndpoint "#aws-glue-api-dev-endpoint-DevEndpoint") objects.
...
call this operation to access the data to which you have been granted permissions.
...
- `DevEndpointNames` – _Required:_ An array of UTF-8 strings, not less than 1 or more than 25 strings.
...
allows you to see which resources are available in your account, and their names.
