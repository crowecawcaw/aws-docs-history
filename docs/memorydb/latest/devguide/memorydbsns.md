...
+ Do not use an encrypted Amazon SNS topic for event notifications.
...
| MemoryDB:AddNodeFailed due to insufficient free IP addresses |  "Failed to create db node %s because the subnets in Availability Zone %s do not have sufficient free IP addresses. Free up IP addresses by removing unused resources, or add a subnet with available addresses to the subnet group, then retry the operation."  | MemoryDB could not add a node because the subnet does not have enough available IP addresses. | 
