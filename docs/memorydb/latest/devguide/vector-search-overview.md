...
**Topics**
+ [Indexes and the Valkey and Redis OSS keyspace](#vector-search-indexes-keyspaces)
+ [Index field types](#vector-search-index-field-types)
+ [Vector index algorithms](#vector-search-index-algorithms)
+ [Vector search query expression](#vector-search-query-expression)
+ [INFO command](#vector-search-ft.info)
...
| Name | Description | 
| --- | --- | 
| search\_background\_indexing\_status | Status of ingestion. NO\_ACTIVITY means idle. Other values indicate there are keys in the process of being ingested. | 
