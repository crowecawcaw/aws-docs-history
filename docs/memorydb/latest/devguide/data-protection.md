...
MemoryDB provisions every cluster with a TLS certificate for the cluster endpoint that includes the cluster name. AWS Certificate Manager records publicly trusted TLS certificates in public, append-only Certificate Transparency logs. As a result, MemoryDB cluster names appear in public Certificate Transparency logs. Don't include confidential or sensitive information in cluster names.
