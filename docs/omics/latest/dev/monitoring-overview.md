...
+ *Run metrics* report near real-time resource utilization for your runs and tasks, including CPU, GPU, memory, and file system usage. AWS HealthOmics vends these metrics to CloudWatch as OpenTelemetry metrics, and you query them with PromQL. For more information, see [Run metrics for Private Workflows](monitoring-run-metrics.md).
