...
To get started with HealthOmics, ensure that you have properly set up your [IAM permissions and roles for HealthOmics](https://docs.aws.amazon.com/omics/latest/dev/setting-up-new.html#setting-up-create-iam-user). 
...
1. Select the navigation pane (≡) in the top left, and select **Ready2Run workflows**.

1. On the **Ready2Run workflows** page, choose the **ESMFold for up to 800 residues** workflow. The console opens the details page for that workflow.

1. The details tab provides information about the workflow. To try out the workflow, in the top right of the page select **Start run**.

1. In the **Specify run details** page, enter a run name.

1. Enter or select an Amazon S3 location for the run output. 

1. For **Run metadata retention mode**, choose whether to retain or remove runmeta data. 

1. In the **Service role** panel, choose **Create and use a new service role**.

1. Choose **Next**.

1. On the **Add parameter values** page, choose **Run workflow with Ready2Run test data**.
