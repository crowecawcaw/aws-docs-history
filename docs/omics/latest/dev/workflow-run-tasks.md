...
+ You can monitor task status using EventBridge events. For more information, see [Using EventBridge with AWS HealthOmics](eventbridge.md).
...
+ You can adjust run priorities so that specific queued runs and their tasks start before other queued runs. For more information about run priority, see [Run priority](creating-run-groups.md#run-priority)
