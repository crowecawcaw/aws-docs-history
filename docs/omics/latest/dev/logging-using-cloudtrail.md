...
All HealthOmics actions are logged by CloudTrail and are documented in the [AWS HealthOmics API Reference](https://docs.aws.amazon.com/omics/latest/api/Welcome.html). For example, calls to the `CreateReferenceeStore`, `StartVariantImportJob` and `CreateWorkflow` actions generate entries in the CloudTrail log files.
