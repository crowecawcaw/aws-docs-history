...
**Topics**
+ [Create a workflow version using the console](#workflow-versions-console-create)
+ [Create a workflow version using the CLI](#workflow-versions-api-create)
+ [Create a workflow version using an SDK](#workflow-versions-sdk)
+ [Verify the status of a workflow version](#using-get-workflow)
...
1. On the **Private workflows** page, choose the workflow for the new version.

1. On the **Workflow details** page, choose **Create new version**.

1. On the **Create version** page, provide the following information:

   1. **Version name**: Enter a name for the workflow version that is unique across the workflow. 

   1. **Version description** (optional): You can use the description field to add notes about this version.
...
1. **Tags** (optional): You can associate up to 50 tags with this workflow version.
...
1. Review the version configuration, then choose **Create version**.
...
| Main  | Determined based on the contents of the workflow definition folder. For details, see [HealthOmics workflow definition requirements](workflow-defn-requirements.md). | 
| Accelerators | none | 
| Tags | none | 
...
If your workflow definition file is located in an Amazon S3 folder, enter the location using the `definition-uri` parameter instead of `definition-zip`. For more information, see [CreateWorkflowVersion](https://docs.aws.amazon.com/omics/latest/api/API_CreateWorkflowVersion.html) in the AWS HealthOmics API Reference.
