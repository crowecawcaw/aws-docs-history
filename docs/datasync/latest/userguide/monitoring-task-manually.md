...
1. After you [start your DataSync task](run-task.md#starting-task "run-task.md#starting-task"),
...
Copy the following `describe-task-execution` command. To use this
...
This command returns information about a task execution similar to that shown
following.
...
The `watch` utility doesn't recognize the DataSync alias. The following
example shows how to invoke the CLI directly. To use this example command, replace
