...
To activate your DataSync agent, make sure that you have the following
information:

- The [DataSync service endpoint](choose-service-endpoint.md "choose-service-endpoint.md")
...
How you find this depends on the type of agent that you [deploy](deploy-agents.md "deploy-agents.md"). For example, if your agent is an
Amazon EC2 instance, you can find its IP address by going to the instance's page
...
the DataSync console](#activate-agent-how-to "#activate-agent-how-to"), DataSync can get the activation key for you
...
1. Log in to the [local
   console](local-console-vm.md#local-console-login "local-console-vm.md#local-console-login") of your agent virtual machine (VM) or Amazon EC2
   instance.
2. On the **AWS DataSync Activation -
   Configuration** main menu, enter
   `0` to get an activation key.
3. Enter the AWS Region that you're activating your agent
   in.
4. Enter the type of service endpoint type that your agent is
   using.
...
agent](#activate-agent-how-to "#activate-agent-how-to").

CLI
With standard Unix tools, you can run a `curl` request to
...
open the [Amazon VPC
console](https://console.aws.amazon.com/vpc/ "https://console.aws.amazon.com/vpc/"), choose **Endpoints**, and
select your DataSync VPC service endpoint. On the
**Subnets** tab, locate the IP address for
your [VPC service endpoint's
subnet](choose-service-endpoint.md#datasync-in-vpc "choose-service-endpoint.md#datasync-in-vpc"). This is the endpoint's IP address.

This command returns an activation key. For example:

`F0EFT-7FPPR-GG7MC-3I9R3-27DOH`

You specify this key when [activating your agent](#activate-agent-how-to "#activate-agent-how-to").

## Activating your agent

You have several options for activating your DataSync agent. Once activated, AWS
[manages the agent](managing-agent.md "managing-agent.md") for you.
...
4. In the **Activation key** section, do one of
   ...
   - Choose **Automatically get the activation key
     ...
     configuration](datasync-network.md "datasync-network.md") based on the type of service
...
5. (Recommended) For **Agent name**, give your
   agent a name that you can remember.
6. (Optional) For **Tags**, enter values for the
   **Key** and **Value**
   fields to tag your agent.

Tags help you manage, filter, and search for your AWS
resources. 7. Choose **Create agent**. 8. On the **Agents** page, verify that your
...
2. For `--activation-key`, specify your [agent activation
   key](#get-activation-key "#get-activation-key").
3. (Recommended) For `--agent-name`, specify a name
...
6. Verify that your agent is activated by running the
   `list-agents` command:
...
`Status` is `OFFLINE`. This
happens briefly after activating an agent.

DataSync API
Once you [get your activation
key](#get-activation-key "#get-activation-key"), activate your agent by using the [CreateAgent](API_CreateAgent.md "API_CreateAgent.md") operation.
