...
Package vulnerability findings can also have a severity of
**Untriaged**. This means that the vendor hasn't yet set a
vulnerability score for the detected vulnerability. In this case, we recommend using
the reference URLs for the finding to research that vulnerability and respond
accordingly.

Package vulnerability findings include the following scores and associated scoring
vectors as part of their finding details:
...
following table defines these severity ratings. The value in the **Open path rating** column represents open paths from
virtual gateways, peered VPCs, and AWS Direct Connect networks. All other
exposed services, ports, and protocols have an Informational severity rating.
