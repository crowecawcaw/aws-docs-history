...
All Amazon EVS actions are logged by CloudTrail and are documented in the [Amazon EVS API Reference](https://docs.aws.amazon.com/evs/latest/APIReference/). For example, calls to the `CreateEnvironment`, `GetEnvironment` and `DeleteEnvironment` actions generate entries in the CloudTrail log files.
