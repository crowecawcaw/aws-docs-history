...
<a name="replatform-to-ecs-combining-applications"></a>
...
1. Run the `combine_applications.py` script while specifying values for the following parameters:
   ...
   1. **workflow\_id** – The workflow ID.

   1. **s3\_input\_path** – The S3 path that has the S3 artifacts uploaded that need to be combined.

   1. **s3\_output\_path** – The output path given when creating the workflow.

   1. **temp\_workspace** – The workspace directory to use. The default is `/tmp/`.
