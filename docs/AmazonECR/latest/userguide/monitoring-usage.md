...
| `ResourceId` | The identifier for the resource that incurred the usage.<br>Currently, ResourceId is only relevant to `ImagesPerRepositoryCount`<br>and its value is formatted as repository/your\_repository\_name. For example:<br>"repository/my-repo" returns the number of images in repository with name "my-repo". |
