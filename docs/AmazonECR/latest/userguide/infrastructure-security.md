...
As a managed service, Amazon Elastic Container Registry is protected by AWS global network security. For
...
You use AWS published API calls to access Amazon ECR through the network. Clients must
...
- Cipher suites with perfect forward secrecy (PFS) such as DHE (Ephemeral
  ...
  address. You can also use Amazon ECR policies to control access from specific Amazon Virtual
  Private Cloud (Amazon VPC) endpoints or specific VPCs. Effectively, this isolates network
  access to a given Amazon ECR resource from only the specific VPC within the AWS network. For
  more information, see [Amazon ECR interface VPC endpoints (AWS PrivateLink)](vpc-endpoints.md "vpc-endpoints.md").
