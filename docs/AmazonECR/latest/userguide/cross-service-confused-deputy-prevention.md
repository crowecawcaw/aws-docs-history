...
policies to limit the permissions that Amazon ECR gives another service to the resource. Use
...
`arn:aws:`servicename`:`region`:`123456789012`:*`.
...
`aws:SourceAccount` global condition context keys in an Amazon ECR repository
policy to allow AWS CodeBuild access to the Amazon ECR API actions necessary for integration with
that service while also preventing the confused deputy problem.
