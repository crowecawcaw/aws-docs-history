...
<a name="troubleshooting-failed-contacts"></a>
...
**Note**  
This guide is specifically for the **FAILED** contact status - and is not intended for other failure statuses, such as **AWS\_FAILED**, **AWS\_CANCELLED**, or **FAILED\_TO\_SCHEDULE**. For more information on contact statuses, see [AWS Ground Station contact statuses](contacts.lifecycle.md#contact-statuses) 

## Dataflow endpoint FAILED use cases
<a name="dataflow-endpoint-failed-use-cases"></a>
...
+ Ensure subnets associated to your Dataflow Endpoint Group, through dataflows configured in [Set up and configure Amazon VPC](dataflows.vpc-configuration.md), remain active and available to AWS Ground Station. 

 See the section on [Troubleshoot contacts that deliver data to Amazon EC2](troubleshooting-contact.md) for more specific troubleshooting steps. 

## AWS Ground Station Agent FAILED use cases
<a name="agent-failed-use-cases"></a>
