...
<a name="security.encryption-during-transit.gsagent"></a>
 ...
 The AWS KMS key used for encrypting a stream is specified when creating a `MissionProfile` in the [ streamsKmsKey ](https://docs.aws.amazon.com/ground-station/latest/APIReference/API_CreateMissionProfile.html#groundstation-CreateMissionProfile-request-streamsKmsKey) parameter. All permissions granting AWS Ground Station access to the keys are handled through the AWS KMS key policy attached to `streamsKmsKey`. 
