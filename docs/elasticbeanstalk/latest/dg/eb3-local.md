...
| `--port {{hostport}}` | Maps a port on the host to the exposed port on the container. If you don't specify this option, the EB CLI uses the same port on both host and container.<br />This option works only with Docker platform applications. It doesn't apply to the Multicontainer Docker platform. | 
