...
Swaps the environment's CNAME with the CNAME of another environment (for example, to
avoid downtime when you update your application version).
...
suppress this, you can specify the name of the environment to use by including the
...
The `environment-name` is the environment for which you want
a different CNAME. If you don't specify `environment-name` as a
command line parameter when you run **eb swap**, EB CLI updates the CNAME of the
default environment.
