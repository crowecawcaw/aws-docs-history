...
After you enable log streaming, the environment health log group (`/aws/elasticbeanstalk/{{environment-name}}/environment-health.log`) appears in the Elastic Beanstalk console alongside your instance log groups, on the environment's **Logs** page. Choose it to view its log events in a built-in viewer.
