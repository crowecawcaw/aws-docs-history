...
Commands in a `Buildfile` are only run once and must terminate upon completion, whereas commands in a [Procfile](java-se-procfile.md "java-se-procfile.md") are expected to run for the life of the application and will be restarted if they terminate. To run the JARs in
