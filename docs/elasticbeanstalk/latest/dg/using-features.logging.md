...
When log streaming is enabled, you can view these log groups directly in the Elastic Beanstalk console, on the environment's **Logs** page. Choose a log group to open its log events in a built-in viewer, where you can switch between log streams, and search and filter events.
