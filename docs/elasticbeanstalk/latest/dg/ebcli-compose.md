...
`env.yaml`. For details on the `env.yaml` format, see
[Environment manifest (env.yaml)](environment-cfg-manifest.md "environment-cfg-manifest.md").

To use the `Compose Environments` API, first run **eb init** from the
...
component](eb-cli3-configuration.md "eb-cli3-configuration.md"), and then creates the `.elasticbeanstalk` directory in
...
Next, run the **eb create** command with a list of environments to create, one for
each component:
...
created by concatenating the `EnvironmentName` specified in the
`env.yaml` file with the group name, separated by a hyphen. The total
...
`branch-defaults` section of the EB CLI configuration file under
`/.elasticbeanstalk/config.yml`. To deploy your application to a different
group, use the `--env-group-suffix` option when you run **eb deploy**. If
...
`--ignore-links` option to override the default behavior:
