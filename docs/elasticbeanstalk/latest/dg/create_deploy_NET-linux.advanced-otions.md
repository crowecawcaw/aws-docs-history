...
For a complete list of namespaces and options, including default and supported values for each, see [Configuration options for Beanstalk Standard environments](command-options-general.md) and [.NET Core on Linux platform options](command-options-specific.md#command-options-dotnet-core-linux).
