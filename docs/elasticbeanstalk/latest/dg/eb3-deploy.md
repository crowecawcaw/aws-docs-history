...
| `-m` "{{version\_description}}"<br />or<br />`--message` "{{version\_description}}" | The description for the application version, enclosed in double quotation marks.<br />Type: String | 
| `--modules` {{component-a component-b}} | List of components to update. Only for use with [Compose Environments](ebcli-compose.md). | 
| `-p`<br />or<br />`--process` | Preprocess and validate the environment manifest and configuration files in the source bundle. Validating configuration files can identify issues prior to deploying the application version to an environment. | 
...
| `--version` {{version\_label}} | An existing application version to deploy.<br />Type: String | 
