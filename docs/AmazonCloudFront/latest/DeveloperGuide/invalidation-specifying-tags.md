...
from the origin. For more information, see [Invalidating content by cache tags](invalidation-by-tags.md "invalidation-by-tags.md").
...
specify `#product:electronics` in the `Paths.Items` list.

**Case sensitivity**

Tag values are case-insensitive. For example, `#Product:Electronics`
and `#product:electronics` refer to the same tag.
...
see [Quotas on invalidations](cloudfront-limits.md#limits-invalidations "cloudfront-limits.md#limits-invalidations").
