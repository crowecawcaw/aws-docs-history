...
single file (such as `/images/logo.jpg`) or for multiple files (such as `/images/*`). A path that includes the `*` wildcard counts as one
...
to create three distributions, and you submit 600 invalidation paths for _each
distribution_ in a given month (for a total of 1,800 invalidation paths),
...
For more information about invalidation pricing, see [Amazon CloudFront Pricing](https://aws.amazon.com/cloudfront/pricing/ "https://aws.amazon.com/cloudfront/pricing/"). For more information about
invalidation paths, see [Invalidation paths](invalidation-specifying-objects.md#invalidation-specifying-objects-paths "invalidation-specifying-objects.md#invalidation-specifying-objects-paths").
...
for 100 invalidation paths. A single tag invalidation item (for example, `#product:electronics`) counts as one path regardless of how many cached
objects it invalidates — the same as a wildcard path invalidation (for example, `/*`) counts as one path.

For more information about tag invalidation, see [Invalidating content by cache tags](invalidation-by-tags.md "invalidation-by-tags.md").
