...
**Contents**
+ [You specified a missing object from the Amazon S3 origin](#missing-object-in-s3-bucket)
+ [Your Amazon S3 origin is missing IAM permissions](#access-denied-origin-missing-iam-permissions)
+ [You're using invalid credentials or don't have sufficient permissions](#access-denied-missing-iam-permissions)
...
For example, if you follow the [CloudFront tutorial](GettingStarted.SimpleDistribution.md) to create a basic distribution, you create an Amazon S3 bucket as the origin and upload an example `index.html` file.
...
To resolve this, first create another distribution, then create a DNS entry pointing to the new distribution. Finally, add the more specific CNAME. For more information on how to add CNAMEs, see [Add an alternate domain name](CreatingCNAME.md).
...
To use Amazon CloudFront with an Amazon S3 origin, you must sign up for both CloudFront and Amazon S3, separately. For more information about signing up for CloudFront and Amazon S3, see [Set up your AWS account](setting-up-cloudfront.md).
