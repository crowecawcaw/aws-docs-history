...
If the cache doesn’t contain the requested range, CloudFront forwards the request to the origin. (To optimize performance, CloudFront might request a larger range than the client requested in the `Range GET`.) What happens next depends on whether the origin supports `Range GET` requests:
