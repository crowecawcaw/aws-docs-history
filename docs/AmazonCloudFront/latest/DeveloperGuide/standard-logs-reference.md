...
the connection is closed. 3. **`x-edge-location`**
...
change in the future.) 4. **`sc-bytes`**
...
`x-forwarded-for` field. 6. **`cs-method`**

The HTTP request method received from the viewer. 7. **`cs(Host)`**

The domain name of the CloudFront distribution (for example,
d111111abcdef8.cloudfront.net). 8. **`cs-uri-stem`**
...
not included in the log. 9. **`sc-status`**
...
came from a search engine, which search engine. 12. **`cs-uri-query`**
...
information about cookies, see [Cache content based on cookies](Cookies.md "Cookies.md"). 14. **`x-edge-result-type`**
...
this string in the `x-amz-cf-id` response header. 16. **`x-host-header`**
...
for the domain name that is associated with your distribution. 17. **`cs-protocol`**

The protocol of the viewer request (`http`,
`https`, `grpcs`, `ws`, or `wss`). 18. **`cs-bytes`**
...
of bytes sent from the client to the server on the connection. 19. **`time-taken`**
...
latency and TCP buffering. 20. **`x-forwarded-for`**
...
field is a hyphen (-). 21. **`ssl-protocol`**
...
`HTTP/1.1`, `HTTP/2.0`, and `HTTP/3.0`. 25. **`fle-status`**
...
this field is a hyphen (-). 26. **`fle-encrypted-fields`**
...
If field-level encryption is not configured for the distribution, the value of
this field is a hyphen (-). 27. **`c-port`**

The port number of the request from the viewer. 28. **`time-to-first-byte`**

The number of seconds between receiving the request and writing the
first byte of the response, as measured on the server. 29. **`x-edge-detailed-result-type`**
...
response. 31. **`sc-content-len`**

The value of the HTTP `Content-Length` header of the
response. 32. **`sc-range-start`**

When the response contains the HTTP `Content-Range` header,
this field contains the range start value. 33. **`sc-range-end`**
...
this field. For more information, see [Mutual TLS authentication with CloudFront (Viewer mTLS)](mtls-authentication.md "mtls-authentication.md").
...
One way to analyze your access logs is to use [Amazon Athena](https://aws.amazon.com/athena/ "https://aws.amazon.com/athena/"). Athena is an interactive query service that can help you analyze
data for AWS services, including CloudFront. To learn more, see [Querying Amazon CloudFront Logs](../../../athena/latest/ug/cloudfront-logs.md "../../../athena/latest/ug/cloudfront-logs.md") in the
_Amazon Athena User Guide_.

In addition, the following AWS blog posts discuss some ways to analyze access
logs.

- [Amazon CloudFront Request Logging](https://aws.amazon.com/blogs/aws/amazon-cloudfront-request-logging/ "https://aws.amazon.com/blogs/aws/amazon-cloudfront-request-logging/") (for content delivered via HTTP)
- [Enhanced CloudFront Logs, Now With Query Strings](https://aws.amazon.com/blogs/aws/enhanced-cloudfront-logs-now-with-query-strings/ "https://aws.amazon.com/blogs/aws/enhanced-cloudfront-logs-now-with-query-strings/")
