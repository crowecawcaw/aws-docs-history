...
| `source`      | The unique location of the affected system                                                                 | No                      | ###### Note The `url`, `service_key_file`, `routing_key_file`, and `http_config` options are not supported. The following topics describe how to configure PagerDuty as an alert receiver in Amazon Managed Service for Prometheus. ###### Topics <br>• [Configure AWS Secrets Manager and permissions](AMP-alertmanager-pagerduty-permissions.md "AMP-alertmanager-pagerduty-permissions.md") <br>• [Configure alert manager to send alerts to PagerDuty](AMP-alertmanager-pagerduty-configure-alertmanager.md "AMP-alertmanager-pagerduty-configure-alertmanager.md") |
