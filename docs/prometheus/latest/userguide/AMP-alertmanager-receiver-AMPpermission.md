...
When configuring your Amazon SNS topic for these opt-in Regions, ensure you use
...
`aws:SourceArn` value must use the same account ID when used
in the same policy statement.

The value of `aws:SourceArn` must be the ARN of the Amazon Managed Service for Prometheus
workspace.
