...
have the fictional `refactor-spaces:`GetWidget``
permissions.
...
Refactor Spaces. However, the action requires the service to have permissions that are granted by a service role. Mary does not have permissions to pass the
role to the service.
...
- To learn whether Refactor Spaces supports these features, see [How AWS Migration Hub Refactor Spaces works with IAM](security_iam_service-with-iam.md "security_iam_service-with-iam.md").
