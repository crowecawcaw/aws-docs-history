...
You can use the AWS CodeBuild console, AWS CLI,or AWS SDKs to manually retry either a single build or a
batch build in AWS CodeBuild.

###### Topics

- [Retry a build manually (console)](#retry-build-console "#retry-build-console")
- [Retry a build manually (AWS CLI)](#retry-build-cli "#retry-build-cli")
- [Retry a build manually (AWS SDKs)](#retry-build-sdks "#retry-build-sdks")
...
2. Do one of the following:
   ...
   - In the navigation pane, choose **Build projects**. In
     ...
     select the box for the build, and then choose **Retry
     build**.
...
In the preceding command, replace the following placeholder:
    ...
    + `<build-id>`: Required string. The ID of
     the build or batch build to retry. To get a list of build IDs, see the
     following topics:
     ...
     **retry-build** command with the option, a unique
     ...
     `retry-build` request. The token is valid for 5 minutes
     after the request. If you repeat the `retry-build` request
     with the same token, but change a parameter, CodeBuild returns a parameter
     mismatch error.
