...
You can use the AWS CodeBuild console, AWS CLI, or AWS SDKs to view a list of build projects
in CodeBuild.
...
- `sort-order`: Optional string used to indicate the
  order in which to list build projects, based on
  `sort-by`. Valid values include
...
- `next-token`: Optional string. During a previous run,
  ...
  command with each subsequent next token, until no more next tokens are
  returned.
