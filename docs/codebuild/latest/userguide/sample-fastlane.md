...
1.  In **Environment**: 
   ...
   + Provide the below environment variables.
     + Name: `MATCH_PASSWORD`, Value: {{<secrets arn>}}, Type: Secrets Manager (Secrets ARN created in step 5 for MATCH\_PASSWORD)
     + Name: `FASTLANE_SESSION`, Value: {{<secrets arn>}}, Type: Secrets Manager (Secrets ARN created in Step 5 for FASTLANE\_SESSION)
...
+ If you encounter issues with certificate decrypting, ensure you set correct passphrase in MATCH\_PASSWORD environment variable.
