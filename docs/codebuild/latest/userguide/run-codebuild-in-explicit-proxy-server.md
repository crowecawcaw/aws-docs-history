...
+  Remove the following default access control list (ACL) rules. 
  ...
  + If your source is Amazon S3, use the command **acl download\_src dstdom\_regex .\*s3\\.us-west-1\\.amazonaws\\.com**to grant access to S3 buckets in the `us-west-1` Region.
