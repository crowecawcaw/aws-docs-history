...
policies to limit the permissions that AWS CodeBuild gives another service to the
...
`aws:SourceAccount` global condition context keys in CodeBuild to prevent
