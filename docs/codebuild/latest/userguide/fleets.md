...
The VPC that your CodeBuild fleet will access. For more information, see [Use AWS CodeBuild with Amazon Virtual Private Cloud](vpc-support.md "vpc-support.md").

###### Note

If a fleet override is specified when calling the
StartBuild API, CodeBuild will ignore the project VPC
configuration.
...
The VPC security groups that CodeBuild uses with your VPC. Ensure
that your security groups allow outbound connections.
...
Proxy configurations that apply network access control to your reserved capacity instances. For more information, see [Use AWS CodeBuild with a managed proxy server](run-codebuild-in-managed-proxy-server.md "run-codebuild-in-managed-proxy-server.md").
