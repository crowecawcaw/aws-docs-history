...
**Resource:**
`arn:aws:codebuild:`region-ID`:`account-ID`:report-group/`report-group-name``
...
Required to delete a set of `SourceCredentialsInfo` objects
that contain information about credentials for a GitHub, GitHub Enterprise Server,
...
Required to import a set of `SourceCredentialsInfo` objects
that contain information about credentials for a GitHub, GitHub Enterprise Server,
or Bitbucket repository.
...
Required to get information about all Docker images that are managed by
AWS CodeBuild.

**Resource:**
`*`
(required, but does not refer to an addressable AWS
resource)
...
**Actions:**
`codebuild:UpdateProjectVisibility`, `iam:PassRole`
...
¹ Used for permission only. There is no API for this
action.
