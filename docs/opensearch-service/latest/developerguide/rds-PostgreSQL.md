...
Complete the following steps to configure an OpenSearch Ingestion pipeline with Amazon RDS for PostgreSQL.
...
+ [Recommended CloudWatch Alarms](#rds-postgres-pipeline-metrics)
...
<a name="rds-postgres-pipeline-metrics"></a>
...
| {{pipeline-name}}.rds.exportRecordProcessingErrors | This metric indicates number of processing errors in an OpenSearch Ingestion pipeline while reading the data from an Amazon RDS for PostgreSQL instance. | 
