...
A per-account endpoint targets every collection in your AWS account in a Region through a single hostname. Because all collections share the same hostname, you identify the target collection using one of these HTTP headers. At least one is required; if you include both, they must refer to the same collection.
