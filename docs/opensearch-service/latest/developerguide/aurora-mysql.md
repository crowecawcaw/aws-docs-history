...
default options, or select your own. The CIDR block must use a /24 prefix
length. You can use any /24 CIDR from a private address
...
- The integration does not currently support cross-account data ingestion;
  ...
  AWS account.
- Ensure that the Amazon Aurora cluster has authentication enabled using Secrets Manager,
...
- Foreign key references with cascading delete actions are not supported and
  can result in data inconsistency between Aurora MySQL and OpenSearch.
