...
- **In security policies** – Define the KMS key
  ...
  If you specify a KMS key in both locations, the KMS key that you provide in the
  CreateCollection request takes precedence over the security policy configuration.

This flexibility simplifies how you manage collections at scale, particularly when
you need to create multiple collections with unique KMS keys. Instead of creating and
managing thousands of encryption policies, you can specify the KMS key directly during
collection creation.
...
OpenSearch Serverless maintains security by encrypting each collection's data with its
