...
Snapshot recovery limitations: When data-stream backing indices include UltraWarm tier data, snapshot recovery may not fully restore the data stream state. Ensure all backing indices are in hot storage before taking snapshots intended for recovery of complete data streams.
