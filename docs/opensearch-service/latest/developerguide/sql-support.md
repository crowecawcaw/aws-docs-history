...
DSL](https://docs.opensearch.org/latest/opensearch/query-dsl/full-text/ "https://docs.opensearch.org/latest/opensearch/query-dsl/full-text/"). Querying with SQL is useful if you're already familiar with the language or
want to integrate your domain with an application that uses it. SQL support is available on
domains running OpenSearch or Elasticsearch 6.5 or higher.
...
documentation](https://opensearch.org/docs/latest/search-plugins/sql/sql/index/ "https://opensearch.org/docs/latest/search-plugins/sql/sql/index/") for information about the syntax for basic and complex
queries, functions, metadata queries, and aggregate functions.
...
To query your data with SQL, send HTTP requests to `_sql` using the
following format:
...
Calls to `_plugins/_sql` include index names in the request body, so they
have the same [access policy considerations](ac.md#ac-advanced "ac.md#ac-advanced") as the
bulk, mget, and msearch operations. As always, follow the principle of [least privilege](../../../IAM/latest/UserGuide/best-practices.md#grant-least-privilege "../../../IAM/latest/UserGuide/best-practices.md#grant-least-privilege") when you grant permissions to API operations.

For security considerations related to using SQL with fine-grained access control, see
[Fine-grained access control in Amazon OpenSearch Service](fgac.md "fgac.md").

The OpenSearch SQL plugin includes many [tunable
settings](https://docs.opensearch.org/latest/search-plugins/sql/settings/ "https://docs.opensearch.org/latest/search-plugins/sql/settings/"). In OpenSearch Service, use the `_cluster/settings` path, not the
...
For legacy Elasticsearch domains, replace `plugins` with
`opendistro`:
...
The SQL Workbench is an OpenSearch Dashboards user interface that lets you run on-demand
SQL queries, translate SQL into its REST equivalent, and view and save results as text,
JSON, JDBC, or CSV. For more information, see [Query
Workbench](https://docs.opensearch.org/latest/search-plugins/sql/workbench/ "https://docs.opensearch.org/latest/search-plugins/sql/workbench/").

## SQL CLI

The SQL CLI is a standalone Python application that you can launch with the
`opensearchsql` command. For steps to install, configure, and use, see
[SQL
CLI](https://docs.opensearch.org/latest/search-plugins/sql/cli/ "https://docs.opensearch.org/latest/search-plugins/sql/cli/").

## JDBC driver

The Java Database Connectivity (JDBC) driver lets you integrate OpenSearch Service domains with your
favorite business intelligence (BI) applications. To download the driver, click [here](https://artifacts.opensearch.org/opensearch-clients/jdbc/opensearch-sql-jdbc-1.1.0.1.jar "https://artifacts.opensearch.org/opensearch-clients/jdbc/opensearch-sql-jdbc-1.1.0.1.jar"). For more information, see the [GitHub
repository](https://github.com/opensearch-project/sql-jdbc "https://github.com/opensearch-project/sql-jdbc").
...
applications like [Microsoft Excel](https://github.com/opensearch-project/sql-odbc/blob/main/docs/user/microsoft_excel_support.md "https://github.com/opensearch-project/sql-odbc/blob/main/docs/user/microsoft_excel_support.md") to the SQL plugin.

For information about installing the driver, see the [SQL repository on
GitHub](https://github.com/opensearch-project/sql-odbc "https://github.com/opensearch-project/sql-odbc").
