...
+ The external provider must allow inbound traffic from public IP addresses, as ML Connector requests do not originate from the OpenSearch VPC.
+ You must add your external provider URL to the cluster setting `plugins.ml_commons.trusted_connector_endpoints_regex` to allow the connector to communicate with the external endpoint.
