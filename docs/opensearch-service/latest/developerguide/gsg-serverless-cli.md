...
1. Attach `TutorialPolicy` to the IAM role who will index and search data in the collection. In this example, the role is named `TutorialRole`.
   ...
   Before attaching the policy, create the `TutorialRole` if it doesn't already exist:
...
1. Use an HTTP tool such as [Postman](https://www.getpostman.com/) or curl to index data into the *books* collection. The following example creates an index called *books-index* and adds a single document. OpenSearch Serverless endpoints require AWS Signature Version 4 (SigV4) authentication. If you use curl, use the `--aws-sigv4` option or a tool like `awscurl` to sign your requests.
