...
Dashboards combine visualizations from logs, traces, and metrics into a single view.
You can use dashboards to monitor operational health, respond to incidents, and track
...
2. **Build a visualization** – When your query returns results, use the visualization tab to choose a chart type and configure the display. For log and trace queries, aggregation commands like `stats` automatically switch to the visualization view.
3. **Save to a dashboard** – Choose **Add to dashboard** to save the visualization to a new or existing dashboard. The panel stays live, updating as new data arrives.
4. **Iterate** – Repeat for each question you want the dashboard to answer. When something looks wrong on a dashboard, choose any panel to open the underlying query in Discover for further investigation.
...
2. Choose **Add filter** in the filter bar.
3. Select a field name from the dropdown list.
...
| Isolate errors              | `status_code`  | `is greater than or equal to` | `400`           |
