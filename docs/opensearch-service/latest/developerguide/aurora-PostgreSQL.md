...
- [Aurora PostgreSQL prerequisites](#aurora-PostgreSQL-prereqs "#aurora-PostgreSQL-prereqs")
- [Step 1: Configure the pipeline role](#aurora-mysql-pipeline-role "#aurora-mysql-pipeline-role")
- [Step 2: Create the pipeline](#aurora-PostgreSQL-pipeline "#aurora-PostgreSQL-pipeline")
- [Data consistency](#aurora-mysql-pipeline-consistency "#aurora-mysql-pipeline-consistency")
- [Mapping data types](#aurora-PostgreSQL-pipeline-mapping "#aurora-PostgreSQL-pipeline-mapping")
- [Limitations](#aurora-PostgreSQL-pipeline-limitations "#aurora-PostgreSQL-pipeline-limitations")
...
3. Set up username and password authentication on your Amazon Aurora cluster
   ...
   Secrets Manager secret](../../../secretsmanager/latest/userguide/create_secret.md "../../../secretsmanager/latest/userguide/create_secret.md").
4. If you use the full initial snapshot feature, create an AWS KMS key and
...
Configure an OpenSearch Ingestion pipeline like the following, which specifies
Aurora PostgreSQL cluster as the source.
...
You can use a preconfigured Amazon Aurora blueprint to create this pipeline. For
...
To provide a custom CIDR, select Other from the dropdown menu. To avoid a
collision in IP addresses between OpenSearch Ingestion and Amazon Aurora, ensure that the
Amazon Aurora VPC CIDR is different from the CIDR for OpenSearch Ingestion.
...
OpenSearch Ingestion pipeline maps Aurora PostgreSQL data types to representations that are
...
The table below lists Aurora PostgreSQL data types and corresponding OpenSearch field
...
These field types are more closely aligned with the data types in Aurora PostgreSQL and
