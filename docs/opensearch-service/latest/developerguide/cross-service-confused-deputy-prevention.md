...
policies to limit the permissions that Amazon OpenSearch Service gives another service to the resource. If
...
`arn:aws:`es`:*:`123456789012`:*`.

The following example shows how you can use the `aws:SourceArn` and
`aws:SourceAccount` global condition context keys in OpenSearch Service to prevent the
