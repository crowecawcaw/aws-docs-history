...
**Note**  
When UltraWarm is enabled on a domain, the `search.max_buckets` setting defaults to 10,000 instead of the standard OpenSearch default of 65,536. This setting limits the maximum number of buckets allowed in a single aggregation response. If your aggregations require more buckets, you can increase this value by updating the domain's advanced cluster settings.
