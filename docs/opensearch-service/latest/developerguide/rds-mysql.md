...
If automatic mappings fail, you can use `template_type` and
...
- The integration does not currently support cross-account data ingestion;
  ...
  AWS account.
- Ensure that the Amazon RDS instance has authentication enabled using Secrets Manager,
...
- If the MySQL tables to sync don't have primary keys defined, data
  consistency are not guaranteed. You will need to define custom
  `document_id` option in OpenSearch sink configuration properly
  to be able to sync updates/deletes to OpenSearch.
- Foreign key references with cascading delete actions are not supported and
  can result in data inconsistency between RDS for MySQL and OpenSearch.
