...
5. For the domain creation method, choose **Standard
   create**.
6. For **Use cases**, select a use case to configure your
   domain with recommended settings:

   - **Search** – Full-text search,
     e-commerce, and application search.
   - **Observability** – Log analytics,
     metrics, and distributed tracing.
   - **Vector search** – Store vector embeddings
     and run semantic and similarity searches.
   - **Custom** – Configure all settings
     manually.
     Each use case preconfigures your domain with recommended engine and
     storage settings. Choose **Custom** if you want to
     configure all settings yourself.

###### Engine mode is permanent

You cannot change the engine mode after you create the
domain. 7. For **Templates**, choose the option that best matches
...
8. For **Deployment Option(s)**, choose **Domain
...
type, replica count, and software update settings. 9. For **Version**, choose the version of OpenSearch or
...
with the service. 10. For **Instance type**, choose an instance type for your
...
current-generation instance types, such as R5 or I3. 11. For **Number of nodes**, choose the number of data
nodes.
...
data nodes (Intel based instances or Graviton based instances). 12. For **Storage type**, select Amazon EBS. The volume types
available in the list depend on the instance type that you've chosen. For
guidance on creating especially large domains, see [Petabyte scale in Amazon OpenSearch Service](petabyte-scale.md "petabyte-scale.md"). 13. For **EBS** storage, configure the following additional
...
14. (Optional) If you selected a `gp3` volume type, expand
...
15. (Optional) To enable [UltraWarm storage](ultrawarm.md "ultrawarm.md"),
...
16. (Optional) To enable [cold storage](cold-storage.md "cold-storage.md"),
    choose **Enable cold storage**. You must enable
    UltraWarm to enable cold storage.
17. If you use Multi-AZ with Standby, three [dedicated master
...
instances for your dedicated master nodes. 18. (Optional) For domains running OpenSearch or Elasticsearch 5.3 and later,
the **Snapshot configuration** is irrelevant. For more
information about automated snapshots, see [Creating index snapshots in Amazon OpenSearch Service](managedomains-snapshots.md "managedomains-snapshots.md"). 19. If you want to use a custom endpoint rather than the standard one of
...
certificate. For more information, see [Creating a custom endpoint for Amazon OpenSearch Service](customendpoint.md "customendpoint.md"). 20. Under **Network**, choose either **VPC
...
21. Enable or disable fine-grained access control:
...
levels. 22. (Optional) If you want to use SAML authentication for OpenSearch Dashboards,
choose **Enable SAML authentication** and configure SAML
options for the domain. For instructions, see [SAML authentication for OpenSearch Dashboards](saml.md "saml.md"). 23. (Optional) If you want to use Amazon Cognito authentication for OpenSearch Dashboards,
...
authentication. For guidance on creating these resources, see [Configuring Amazon Cognito authentication for OpenSearch Dashboards](cognito-auth.md "cognito-auth.md"). 24. (Optional) If you want to use IAM Identity Center (IDC) authentication
...
with IAM Identity Center** . For more information, see [Trusted identity propagation overview](../../../singlesignon/latest/userguide/trustedidentitypropagation-overview.md "../../../singlesignon/latest/userguide/trustedidentitypropagation-overview.md") in the _IAM Identity Center User Guide._ 25. (Optional) In the **Advanced features** section, leave
...
26. For **Access policy**, choose an access policy or
...
For more information, see [About access policies on VPC domains](vpc.md#vpc-security "vpc.md#vpc-security"). 27. (Optional) To require that all requests to the domain arrive over HTTPS,
...
Multi-AZ with Standby deployment option. 28. (Optional) Select **Use AWS owned key** to have OpenSearch Service
...
information, see [Encryption of data at rest for Amazon OpenSearch Service](encryption-at-rest.md "encryption-at-rest.md"). 29. For **Off-peak window**, select a start time to schedule
...
cluster's dedicated master nodes during high traffic periods. 30. For **Auto-Tune**, choose whether to allow OpenSearch Service to
...
recurring window during which Auto-Tune updates the domain. 31. (Optional) Select **Automatic software update** to enable
automatic software updates. 32. (Optional) For **Single sign-on authentication**,
select **Authenticate with IAM Identity Center** if you
want to create a single sign-on experience or work with Security Assertion Markup Language (SAML)-authenticated
applications. By default, the dashboard application is authenticated with
IAM to manage permissions for AWS resource users. 33. (Optional) For **OpenSearch UI admins management**,
configure administrator permissions for creating, editing, and deleting
Workspaces in an OpenSearch application:

    * **Grant administrators' permission to specific
     user(s)** – Select specific IAM principals or
     IAM Identity Center users to grant administrator access.
    * **Grant administrators' permission to all
     users** – Grant all users administrator
     access.

34. (Optional) Add tags to describe your domain so you can categorize and
filter on that information. For more information, see [Tagging Amazon OpenSearch Service domains](managedomains-awsresourcetagging.md "managedomains-awsresourcetagging.md"). 35. (Optional) Expand and configure **Advanced cluster
settings**. For a summary of these options, see [Advanced cluster settings](#createdomain-configure-advanced-options "#createdomain-configure-advanced-options"). 36. Choose **Create**.
...
for OpenSearch domains, see the [Amazon OpenSearch Service resource type reference](../../../AWSCloudFormation/latest/UserGuide/aws-resource-opensearchservice-domain.md "../../../AWSCloudFormation/latest/UserGuide/aws-resource-opensearchservice-domain.md") in the
