...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `cold_cluster` | • `cluster:monitor/nodes/stats`<br>• `cluster:admin/ultrawarm*`<br>• `cluster:admin/cold/*`                                                            |
| `cold_index`   | • `indices:monitor/stats`<br>• `indices:data/read/minmax`<br>• `indices:admin/ultrawarm/migration/get`<br>• `indices:admin/ultrawarm/migration/cancel` |
...
6. For **Index**, enter `*`.
7. For **Index permissions**, choose the
   `cold_index` group you created.
8. Choose **Create**.
...
`ColdStorageSpaceUtilization` metric in Amazon CloudWatch to see how much cold
storage space you're using.

## Cold storage pricing

Similar to UltraWarm storage, with cold storage you only pay for data storage. There's
no compute cost for cold data and you wont get billed if theres no data in cold
storage.
...
Service pricing](https://aws.amazon.com/opensearch-service/pricing/ "https://aws.amazon.com/opensearch-service/pricing/").

## Enabling cold storage

The console is the simplest way to create a domain that uses cold storage. While you
create the domain, first choose **Enable warm data nodes**, because you
must enable warm storage on the same domain. Then, choose **Enable cold
storage**.

The same process works on existing domains as long as you meet the [prerequisites](#coldstorage-pp "#coldstorage-pp"). Even after the domain state changes
from **Processing** to **Active**, cold storage might
...
dedicated master nodes, cold storage enabled, and fine-grained access control
enabled:
...
storage, and monitor index migration status, without using the CLI or configuration API.
For more information, see [Managing indexes in
OpenSearch Dashboards](dashboards.md#dashboards-indices "dashboards.md#dashboards-indices").

## Migrating indexes to cold storage

When you migrate indexes to cold storage, you provide a time range for the data to
...
The following request migrates a warm index to cold storage and provides start and end
times for the data in that index:
...
[metric](managedomains-cloudwatchmetrics.md#managedomains-cloudwatchmetrics-coldstorage "managedomains-cloudwatchmetrics.md#managedomains-cloudwatchmetrics-coldstorage"). The migration
...
You can use [Index State Management](ism.md "ism.md") to automate the
migration process after an index reaches a certain age or meets other conditions. See
the [sample policy](ism.md#ism-example-cold "ism.md#ism-example-cold"), which demonstrates how to
automatically migrate indexes from hot to UltraWarm to cold storage.

###### Note

An explicit `timestamp_field` is required in order to move indexes to
cold storage using an Index State Management policy.

## Canceling migrations to cold storage

If a migration to cold storage is queued or in a failed state, you can cancel the
migration using the following request:
...
Before querying, you can list the indexes in cold storage to decide which ones to
migrate to UltraWarm for further analysis. The following request lists all cold indexes,
...
You can filter cold indexes based on a prefix-based index pattern and time range
offsets.
...
The following request returns indexes with `start_time` and
`end_time` metadata fields between `2019-03-01` and
`2020-03-01`:
...
You can sort cold indexes by metadata fields such as index name or size. The
following request lists all indexes sorted by size in descending order:
...
`end_time:asc/desc`, and `index_name:asc/desc`.

### Pagination

You can paginate a list of cold indexes. Configure the number of indexes to be
returned per page with the `page_size` parameter (default is 10). Every
`_search` request on your cold indexes returns a
`pagination_id` which you can use for subsequent calls.

The following request paginates the results of a `_search` request of
...
To check the status of the migration and retrieve the migration ID, send the following
request:
...
status. Valid values are `_failed`, `_accepted`, and
`_all`.

The following command gets the status of all indexes in a single migration
request:
...
If you need to restore a deleted cold index, you can restore it back to the warm tier
by following the instructions in [Restoring warm indexes from snapshots](ultrawarm.md#ultrawarm-snapshot "ultrawarm.md#ultrawarm-snapshot") and then migrating
the index back to cold tier again. You can't restore a deleted cold index directly back
...
To cancel migration for a batch of indexes (maximum of 10 at a time), specify the
migration ID:
...
You can update the `start_time` and `end_time` fields for a cold
index:
...
OpenSearch Dashboards doesn't support the PATCH method. Use [curl](https://curl.haxx.se/ "https://curl.haxx.se/"), [Postman](https://www.getpostman.com/ "https://www.getpostman.com/"), or some other method to
update cold metadata.

## Deleting cold indexes

If you're not using an ISM policy you can delete cold indexes manually. The following
request deletes a cold index:
...
The OpenSearch Service console is the simplest way to disable cold storage. Select the domain and
choose **Actions**, **Edit cluster
configuration**, then deselect **Enable cold
storage**.

To use the AWS CLI or configuration API, under `ColdStorageOptions`, set
`"Enabled"="false"`.

Before you disable cold storage, you must either delete all cold indexes or migrate
them back to warm storage, otherwise the disable action fails.
