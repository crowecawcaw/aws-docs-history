...
1. Regardless of how you choose to sign your requests, the first step is to register the repository:
   ...
   ```
**Note**  
The `base_path` parameter specifies the path within the S3 bucket where snapshots are stored. Without `base_path`, OpenSearch Service cannot locate uploaded snapshots in the S3 bucket.
