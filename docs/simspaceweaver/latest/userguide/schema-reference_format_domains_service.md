...
To specify the properties for a service domain, replace
`service-domain-name` with a name of your choice.
The name must be 3-64 characters long and can contain the characters **A** -**Z** , **a** -**z** , **0** -**9** , and **\_ -** (hyphen). Specify the properties of the service
domain after the name. Repeat this process for each service domain.
...
The `launch_apps_per_worker` section (required) indicates that this
is a service domain configuration, and specifies the number of
service apps to launch per worker.
...
This property specifies the number of service apps to launch
per worker.
...
_Valid values:_
`{}` | `1` | `2`. A value of `{}` specifies the default value of `1`.
...
For example, `s3://weaver-myproject-111122223333-app-zips-us-west-2/MyServiceApp.zip`.
...
value of `ingress_ports` specifies the ports that
your service apps bind to for incoming client connections.
SimSpace Weaver maps dynamically allocated ports to your specified
ingress ports. Ingress ports are both TCP and UDP. Use the
DescribeApp API to find the
actual port number to connect your clients.

_Required:_ No. If you don't specify
endpoint configuration then your service apps in this domain
