...
| -------------------------------- | ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Three star feature compatibility | Four star automation level         | [Transaction Isolation](chap-sql-server-aurora-mysql.tools.actioncode.md#chap-sql-server-aurora-mysql.tools.actioncode.transactionisolation "chap-sql-server-aurora-mysql.tools.actioncode.md#chap-sql-server-aurora-mysql.tools.actioncode.transactionisolation") | Default isolation level is set to `REPEATABLE READ`. Default mechanism `CONSISTENT SNAPSHOT` is similar to `READ COMMITTED SNAPSHOT` isolation in SQL Server. Syntax and option differences. |

## SQL Server Usage

A _transaction_ is a unit of work performed against a database and typically represents a change in the database. Transactions serve the following purposes:
...
To set the transaction isolation level, you will need to set the `tx_isolation` parameter when using Aurora MySQL. For more information, see [Server Options](chap-sql-server-aurora-mysql.configuration.serveroptions.md "chap-sql-server-aurora-mysql.configuration.serveroptions.md").
...
| Set autocommit off or on      | `SET IMPLICIT_TRANSACTIONS OFF           | ON`                                                                       | `SET autocommit = 0                                                                                                           | 1`                | For more information, see [Session Options](chap-sql-server-aurora-mysql.configuration.sessionoptions.md "chap-sql-server-aurora-mysql.configuration.sessionoptions.md"). |
