...
![A partial graphical run plan](images/pb-sql-server-aurora-mysql-run-plans-ssms.png)
...
![MySQL Workbench graphical run plan](images/pb-sql-server-aurora-mysql-workbench.png)

###### Note

To instruct the optimizer to use a join order corresponding to the order in which the tables are specified in a `SELECT` statement, use `SELECT STRAIGHT_JOIN`. For more information, see [Query Hints and Plan Guides](chap-sql-server-aurora-mysql.tuning.queryhints.md "chap-sql-server-aurora-mysql.tuning.queryhints.md").
