...
To instruct the optimizer to use a join order corresponding to the order in which the tables are specified in a `SELECT` statement, use `SELECT STRAIGHT_JOIN`. For more information, see [Query Hints and Plan Guides](chap-sql-server-aurora-mysql.tuning.md "chap-sql-server-aurora-mysql.tuning.md").
