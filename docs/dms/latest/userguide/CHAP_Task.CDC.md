...
**Note**  
AWS DMS CDC does not provide real-time replication. Replication latency varies based on source workload, network conditions, replication instance resources, target ingestion capacity, and data characteristics. There are no SLAs for CDC latency. Latency can increase to several minutes or longer depending on these factors. For more information, see [Ongoing replication](CHAP_BestPractices.md#CHAP_BestPractices.OnGoingReplication).
