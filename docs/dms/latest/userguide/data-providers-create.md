...
| **Service ID (SID) or service name** | Enter the Oracle System ID (SID). To find the Oracle SID, submit the following query to your Oracle database: `SELECT sys_context('userenv','instance_name') AS SID FROM dual;`                                                                                                                                                                             |
...
| **Secure Socket Layer (SSL) mode**   | Choose an SSL mode if you want to turn on connection encryption for this data provider. Depending on the mode that you select, you might need to provide certificate and server certificate information. For further details, see [Using SSL with AWS Database Migration Service](CHAP_Security.md#CHAP_Security.SSL "CHAP_Security.md#CHAP_Security.SSL"). |
...
| **Authentication mechanism**         | For a MongoDB source, the authentication method that MongoDB uses to encrypt the password.                                                                                                                                                                                                                                                                  | 4. Choose **Create data provider**. After you create a data provider, make sure that you add database connection credentials in AWS Secrets Manager. |
