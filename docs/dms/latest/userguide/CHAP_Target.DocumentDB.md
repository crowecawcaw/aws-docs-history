...
To coerce a data type, you can prefix the source column name with `json_` (that is, `json_{{columnName}}`) either manually or using a transformation. In this case, the column is created as a nested JSON document within the target document, rather than as a string field.
...
To coerce a data type, you can prefix a column name with `array_` (that is, `array_{{columnName}}`), either manually or using a transformation. In this case, AWS DMS considers the column as a JSON array, and creates it as such in the target document.
