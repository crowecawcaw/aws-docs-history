...
This premigration assessment validates if Dblink is created for the Oracle standby database source. AWSDMS\_DBLINK is a prerequisite for using a standby database as a source. When using Oracle Standby as a source, AWS DMS does not validate open transactions by default.
