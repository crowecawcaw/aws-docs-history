...
PostgreSQL endpoint, see [Using SSL with AWS Database Migration Service](CHAP_Security.SSL.md "CHAP_Security.SSL.md").
...
###### Note
    ...
    + If your PostgreSQL source database is self-managed, see [Working with self-managed PostgreSQL databases as a source in AWS DMS](#CHAP_Source.PostgreSQL.Prerequisites "#CHAP_Source.PostgreSQL.Prerequisites") for more
     information.
    + If your PostgreSQL source database is managed by Amazon RDS, see [Working with AWS-managed PostgreSQL databases as a DMS source](#CHAP_Source.PostgreSQL.RDSPostgreSQL "#CHAP_Source.PostgreSQL.RDSPostgreSQL") for more
     information.
...
CDC task), see [Enabling CDC using a self-managed PostgreSQL database as a AWS DMS source](#CHAP_Source.PostgreSQL.Prerequisites.CDC "#CHAP_Source.PostgreSQL.Prerequisites.CDC") or [Enabling CDC with an AWS-managed PostgreSQL DB instance with AWS DMS](#CHAP_Source.PostgreSQL.RDSPostgreSQL.CDC "#CHAP_Source.PostgreSQL.RDSPostgreSQL.CDC").

###### Topics

- [Working with self-managed PostgreSQL databases as a source in AWS DMS](#CHAP_Source.PostgreSQL.Prerequisites "#CHAP_Source.PostgreSQL.Prerequisites")
- [Working with AWS-managed PostgreSQL databases as a DMS source](#CHAP_Source.PostgreSQL.RDSPostgreSQL "#CHAP_Source.PostgreSQL.RDSPostgreSQL")
- [Enabling change data capture (CDC) using logical replication](#CHAP_Source.PostgreSQL.Security "#CHAP_Source.PostgreSQL.Security")
- [Using native CDC start points to set up a CDC load of a PostgreSQL source](#CHAP_Source.PostgreSQL.v10 "#CHAP_Source.PostgreSQL.v10")
- [Migrating from PostgreSQL to PostgreSQL using AWS DMS](#CHAP_Source.PostgreSQL.Homogeneous "#CHAP_Source.PostgreSQL.Homogeneous")
...
- [Removing AWS DMS artifacts from a PostgreSQL source database](#CHAP_Source.PostgreSQL.CleanUp "#CHAP_Source.PostgreSQL.CleanUp")
- [Additional configuration settings when using a PostgreSQL database as a DMS source](#CHAP_Source.PostgreSQL.Advanced "#CHAP_Source.PostgreSQL.Advanced")
- [Read replica as a source for PostgreSQL](#CHAP_Source.PostgreSQL.ReadReplica "#CHAP_Source.PostgreSQL.ReadReplica")
- [Using the MapBooleanAsBoolean PostgreSQL endpoint setting](#CHAP_Source.PostgreSQL.ConnectionAttrib.Endpointsetting "#CHAP_Source.PostgreSQL.ConnectionAttrib.Endpointsetting")
...
- [Limitations on using a PostgreSQL database as a DMS source](#CHAP_Source.PostgreSQL.Limitations "#CHAP_Source.PostgreSQL.Limitations")
- [Source data types for PostgreSQL](#CHAP_Source-PostgreSQL-DataTypes "#CHAP_Source-PostgreSQL-DataTypes")
...
- If you're configuring a database as a source for logical
  replication using AWS DMS see [Enabling CDC using a self-managed PostgreSQL database as a AWS DMS source](#CHAP_Source.PostgreSQL.Prerequisites.CDC "#CHAP_Source.PostgreSQL.Prerequisites.CDC")
...
For more information about enabling CDC, see [Enabling change data capture (CDC) using logical replication](#CHAP_Source.PostgreSQL.Security "#CHAP_Source.PostgreSQL.Security").
...
account that you use. For information about creating these, see [Migrating an Amazon RDS for PostgreSQL database without using the master user account](#CHAP_Source.PostgreSQL.RDSPostgreSQL.NonMasterUser "#CHAP_Source.PostgreSQL.RDSPostgreSQL.NonMasterUser").
...
use. For more information, see [Migrating an Amazon RDS for PostgreSQL database without using the master user account](#CHAP_Source.PostgreSQL.RDSPostgreSQL.NonMasterUser "#CHAP_Source.PostgreSQL.RDSPostgreSQL.NonMasterUser"). 2. Set the `rds.logical_replication` parameter in your DB
...
identity as a workaround only after careful testing. For more information, see [Additional configuration settings when using a PostgreSQL database as a DMS source](#CHAP_Source.PostgreSQL.Advanced "#CHAP_Source.PostgreSQL.Advanced").
...
For logical decoding, DMS uses either test\_decoding or pglogical plugin. If
the pglogical plugin is available on a source PostgreSQL database, DMS creates a
replication slot using pglogical, otherwise a test\_decoding plugin is used. For more
information about the test\_decoding plugin, see [PostgreSQL
...
- For information about enabling logical replication for CDC on
  ...
  databases, see [Enabling CDC using a self-managed PostgreSQL database as a AWS DMS source](#CHAP_Source.PostgreSQL.Prerequisites.CDC "#CHAP_Source.PostgreSQL.Prerequisites.CDC")
- For information about enabling logical replication for CDC on
  _AWS-managed_ PostgreSQL source
  databases, see [Enabling CDC with an AWS-managed PostgreSQL DB instance with AWS DMS](#CHAP_Source.PostgreSQL.RDSPostgreSQL.CDC "#CHAP_Source.PostgreSQL.RDSPostgreSQL.CDC").
...
In the preceding command, the following options are set:
    ...
    * The `cdc-start-position` option is set to a start
     ...
     information, see [Determining a CDC native start point](CHAP_Task.CDC.md#CHAP_Task.CDC.StartPoint.Native "CHAP_Task.CDC.md#CHAP_Task.CDC.StartPoint.Native").
...
2. After the database creates the replication slot, get and note the **restart\_lsn**
   and **confirmed\_flush\_lsn** values for the slot:
...
the **confirmed\_flush\_lsn** value.

For information about the **restart\_lsn**
and **confirmed\_flush\_lsn** values, see
[pg\_replication\_slots](https://www.postgresql.org/docs/14/view-pg-replication-slots.html "https://www.postgresql.org/docs/14/view-pg-replication-slots.html") 3. Create a pglogical node.
...
The pg\_dump utility uses the COPY command to create a schema and data dump of
a PostgreSQL database. The dump script generated by pg\_dump loads data into a
...
PostgreSQL or Amazon Aurora PostgreSQL-Compatible Edition, see [https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/PostgreSQL.Procedural.Importing.html](../../../AmazonRDS/latest/UserGuide/PostgreSQL.Procedural.Importing.md "../../../AmazonRDS/latest/UserGuide/PostgreSQL.Procedural.Importing.md").
...
| TIMESTAMP WITH TIME ZONE |                       | X                  |                  | The time zone offset is always normalized to UTC. The original offset<br>literal is not retained.                             |
| DATE                     | X                     |                    |                  |                                                                                                                               |
| TIME                     | X                     |                    |                  |                                                                                                                               |
...
- Enable logical replication on both, your primary database instance and its
  ...
  a DMS source](CHAP_Source.PostgreSQL.md#CHAP_Source.PostgreSQL.RDSPostgreSQL "CHAP_Source.PostgreSQL.md#CHAP_Source.PostgreSQL.RDSPostgreSQL").
- For CDC-only tasks, create a replication slot on the primary (writer)
  ...
  of a PostgreSQL source](CHAP_Source.PostgreSQL.md#CHAP_Source.PostgreSQL.v10 "CHAP_Source.PostgreSQL.md#CHAP_Source.PostgreSQL.v10"). This action is necessary as read
...
when using PostgreSQL as a DMS source](CHAP_Source.PostgreSQL.md#CHAP_Source.PostgreSQL.ConnectionAttrib "CHAP_Source.PostgreSQL.md#CHAP_Source.PostgreSQL.ConnectionAttrib") and [Using native CDC start points to set up a CDC load of a
PostgreSQL source](CHAP_Source.PostgreSQL.md#CHAP_Source.PostgreSQL.v10 "CHAP_Source.PostgreSQL.md#CHAP_Source.PostgreSQL.v10").
...
- If your source is a PostgreSQL database that is on-premises or on an Amazon EC2
  instance, ensure that the test\_decoding output plugin is installed on your
...
[Spill files in Aurora PostgreSQL](CHAP_Troubleshooting_Latency_Source_PostgreSQL.md#CHAP_Troubleshooting_Latency_Source_PostgreSQL_Spill "CHAP_Troubleshooting_Latency_Source_PostgreSQL.md#CHAP_Troubleshooting_Latency_Source_PostgreSQL_Spill").
...
- AWS DMS does not support CDC for Amazon RDS Proxy for PostgreSQL as a
  source.
- When using [source filters](CHAP_Tasks.CustomizingTasks.Filters.md "CHAP_Tasks.CustomizingTasks.Filters.md") that
