...
<tbody>
  <tr><td><b>DB_SCHEMA</b></td><td><code>DB_SCHEMA</code></td><td>{{database}}_{{schema}} (for example, <code>pubs2_dbo</code>). Use when migrating multiple databases to the same PostgreSQL instance to avoid name collisions.</td></tr>
  <tr><td><b>DB</b></td><td><code>DB</code></td><td>{{database}} (for example, <code>pubs2</code>).</td></tr>
