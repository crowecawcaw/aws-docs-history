...
<a name="chap-oracle-aurora-mysql.configuration.memory"></a>
...
### Examples
<a name="chap-oracle-aurora-mysql.configuration.memory.mysql.examples"></a>
...
## Summary
<a name="chap-oracle-aurora-mysql.configuration.memory.summary"></a>
...
| Maximum amount of physical memory available for the instance |  `sga_max_size` or `memory_max_size`  | Configured by the Amazon Relational Database Service or Amazon Aurora instance class.<br />Consider the following example:<pre>db.r3.large: 15.25GB<br />db.r3.xlarge: 30.5GB</pre> | 
| Total amount of private memory for all sessions |  `pga_aggregate_target` and `pga_aggregate_limit`  |  `max_digest_length`  | 
| View values for all database parameters |  <pre>SELECT * FROM v$parameter;</pre>  |  <pre>SELECT * FROM information_schema.GLOBAL_VARIABLES</pre>  | 
