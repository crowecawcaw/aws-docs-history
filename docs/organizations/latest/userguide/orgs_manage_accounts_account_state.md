...
| SUSPENDED | In this state, the account is unusable because AWS has restricted access. You can't make API calls to AWS services from a suspended account. You can still sign in and access the AWS Billing and Account Management console using its direct link, where the account holder can view billing information and contact Support, who can explain why the account is suspended. | 
