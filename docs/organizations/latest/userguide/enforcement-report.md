...
You can generate the report from your organization's management account in the `us-east-1` Region, provided that it has access to an Amazon S3 bucket. The bucket must have an attached bucket policy as shown in [Amazon S3 Bucket Policy for Storing Report](../../../ARG/latest/userguide/tag-policies-prereqs.md#bucket-policy "../../../ARG/latest/userguide/tag-policies-prereqs.md#bucket-policy"). To generate the report, run the following command:
