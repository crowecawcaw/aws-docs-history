...
AWS Security Hub CSPM policies provide security teams with a centralized approach to managing security
...
Security Hub CSPM policies are fully integrated with AWS Organizations, allowing management accounts or
...
Security Hub CSPM policies provide a comprehensive set of capabilities that help you manage and
...
- Centrally [enable Security Hub CSPM](../../../securityhub/latest/userguide/security-hub-adv-getting-started-enable.md#security-hub-adv-getting-started-enable-org-account "../../../securityhub/latest/userguide/security-hub-adv-getting-started-enable.md#security-hub-adv-getting-started-enable-org-account") across accounts and Regions in your
  organization
...
## What are Security Hub CSPM policies?

Security Hub CSPM policies are AWS Organizations policies that provide centralized control over security
...
When you implement Security Hub CSPM policies, you gain the ability to define specific security
...
## How Security Hub CSPM policies work

When you attach an Security Hub CSPM policy to your organization or organizational unit, AWS Organizations
...
configurations, Security Hub CSPM will be disabled in that region.

When `ALL_SUPPORTED` is specified for enablement, Security Hub CSPM is enabled in all
...
This topic uses the following terms when discussing Security Hub CSPM policies.
...
| Delegated administrator         | An account designated to manage Security Hub CSPM policies on behalf of the<br>organization. |
| Service-linked role             | An IAM role that allows Security Hub CSPM to interact with other AWS<br>services.            |

## Use cases for Security Hub CSPM policies

Security Hub CSPM policies address common security management challenges in multi-account
...
A multinational corporation needs different Security Hub CSPM configurations for different
geographical regions. They create a parent policy enabling Security Hub CSPM in all regions
...
A software development organization implements Security Hub CSPM policies that enable
...
When creating Security Hub CSPM policies, the following validations occur:

- Region names must be valid AWS region identifiers
- Regions must be supported by Security Hub CSPM
...
Security Hub CSPM policies operate across multiple Regions, requiring careful consideration of
...
- Policies only apply to Regions where Security Hub CSPM is available

## Next steps

To get started with Security Hub CSPM policies:

1. Review the prerequisites in Getting started with Security Hub CSPM policies
