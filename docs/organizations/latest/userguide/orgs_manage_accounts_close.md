...
If the member account's root user email address uses a domain registered through Route 53 in that same account, see [Service specific considerations](https://docs.aws.amazon.com/accounts/latest/reference/manage-acct-closing.html#close-account-considerations) before proceeding.
