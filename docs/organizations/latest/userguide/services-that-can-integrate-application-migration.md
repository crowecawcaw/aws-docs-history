...
AWS Transform MGN simplifies, expedites, and reduces the cost of migrating applications to AWS.
By integrating with Organizations, you can use the global view feature to manage large-scale migrations across multiple accounts. For more information see [Setting up your AWS Organizations](../../../mgn/latest/ug/setting-up-organizations.md "../../../mgn/latest/ug/setting-up-organizations.md") in the _MGN user guide_.

Use the following information to help you integrate
AWS Transform MGN with AWS Organizations.
...
access. This role allows MGN to perform supported operations within your
...
MGN and Organizations, or if you remove the member account from the organization.
...
role. The service-linked roles used by MGN grant access to the following service
principals:
...
When you enable trusted access with MGN you can use the global view feature, which allows you to manage
...
For more information, see [Setting up your AWS Organizations](../../../mgn/latest/ug/setting-up-organizations.md "../../../mgn/latest/ug/setting-up-organizations.md") in the _AWS Transform MGN user guide_.
...
You can enable trusted access using either the AWS Transform MGN console or the AWS Organizations
console.

###### Important

We strongly recommend that whenever possible, you use the AWS Transform MGN console or
tools to enable integration with Organizations. This lets AWS Transform MGN perform any
...
provided by AWS Transform MGN. For more information, see [this note](orgs_integrate_services.md#important-note-about-integration "orgs_integrate_services.md#important-note-about-integration").

If you enable trusted access by using the AWS Transform MGN console or tools then you
...
3. Choose **AWS Transform MGN** in the list of services.
4. Choose **Enable trusted access**.
5. In the **Enable trusted access for AWS Transform MGN** dialog
...
6. If you are the administrator of only AWS Organizations, tell the
   administrator of AWS Transform MGN that they can now enable that service to work with AWS Organizations
...
Run the following command to enable AWS Transform MGN as a
...
We strongly recommend that whenever possible, you use the AWS Transform MGN console or
tools to disable integration with Organizations. This lets AWS Transform MGN perform any
...
integration using the tools provided by AWS Transform MGN.

If you disable trusted access by using the AWS Transform MGN console or tools then you
don’t need to complete these steps.
...
5. In the **Disable trusted access for AWS Transform MGN** dialog
   box, type **disable** to confirm, and then choose **Disable trusted
   access**.
6. If you are the administrator of only AWS Organizations, tell the
   administrator of AWS Transform MGN that they can now disable that service from working with AWS Organizations
...
Run the following command to disable AWS Transform MGN as a
trusted service with Organizations.
...
MGN that otherwise can be performed only by users or roles in the
organization's management account. This helps you to separate management of the
organization from management of MGN. For more information see [Setting up your AWS Organizations](../../../mgn/latest/ug/setting-up-organizations.md "../../../mgn/latest/ug/setting-up-organizations.md") in the _MGN user guide_.

###### Minimum permissions

Only a user or role in the Organizations management account can configure a member
account as a delegated administrator for MGN in the organization
...
administrator for MGN. You can remove the delegated administrator using the Organizations `DeregisterDelegatedAdministrator` CLI or SDK operation.
