...
| Replace your existing trail with a CloudTrail Lake event data store | Create an event data store with the same event selectors as your trail to ensure that the event data store has the same coverage as your trail. <br />To avoid duplicating events between the source trail and destination event data store, choose a date range for the copied events that is earlier than the creation of the event data store.<br />After your event data store is created, you can turn off logging for the trail to avoid additional charges. | 

**Topics**
+ [Considerations for copying trail events](#cloudtrail-trail-copy-considerations)
