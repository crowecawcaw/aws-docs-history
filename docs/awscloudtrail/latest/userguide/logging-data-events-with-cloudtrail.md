...
+ [Suppressing recursive events with recursive logging](cloudtrail-trail-recursive-logging.md)
  + [What is recursive logging?](cloudtrail-trail-recursive-logging.md#cloudtrail-recursive-logging-overview)
  + [How recursive logging suppression works](cloudtrail-trail-recursive-logging.md#cloudtrail-recursive-logging-how-it-works)
  + [Configuring recursive logging](cloudtrail-trail-recursive-logging.md#cloudtrail-recursive-logging-configure)
    + [Disabling recursive logging (suppressing recursive events)](cloudtrail-trail-recursive-logging.md#cloudtrail-recursive-logging-disable)
    + [Re-enabling recursive logging](cloudtrail-trail-recursive-logging.md#cloudtrail-recursive-logging-enable)
  + [Default behavior](cloudtrail-trail-recursive-logging.md#cloudtrail-recursive-logging-defaults)
...
To avoid logging data events for the Amazon S3 bucket where you receive log files if you configure a trail to log all Amazon S3 data events in your AWS account, consider one of the following options:  
Set `RecursiveLogging` to `false` on the trail to suppress events generated by CloudTrail delivery operations. For more information, see [Suppressing recursive events with recursive logging](cloudtrail-trail-recursive-logging.md).
Configure delivery of log files to an Amazon S3 bucket that belongs to another AWS account. For more information, see [Receiving CloudTrail log files from multiple accounts](cloudtrail-receive-logs-from-multiple-accounts.md).
