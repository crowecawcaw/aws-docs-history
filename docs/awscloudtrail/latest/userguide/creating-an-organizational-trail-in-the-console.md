...
You must be signed in using an IAM identity in the management or delegated
 ...
 permissions](creating-an-organizational-trail-prepare.md#org_trail_permissions "creating-an-organizational-trail-prepare.md#org_trail_permissions") to create an organization trail.
2. Choose **Trails**, and then choose **Create
 trail**.
3. On the **Create Trail** page, for **Trail
 name**, type a name for your trail. For more information, see [Naming requirements for CloudTrail resources, S3 buckets, and KMS keys](cloudtrail-trail-naming-requirements.md "cloudtrail-trail-naming-requirements.md").
4. Select **Enable for all accounts in my organization**. You
 ...
 organization trail, be sure that the user or role has [sufficient permissions](creating-an-organizational-trail-prepare.md#org_trail_permissions "creating-an-organizational-trail-prepare.md#org_trail_permissions").
5. For **Storage location**, choose **Create new S3
 ...
 **Browse** to choose a bucket. You can choose a bucket
 ...
 policy, see [Amazon S3 bucket policy for CloudTrail](create-s3-bucket-policy-for-cloudtrail.md "create-s3-bucket-policy-for-cloudtrail.md").


To make it easier to find your logs, create a new folder (also known as a
 *prefix*) in an existing bucket to store
 your CloudTrail logs. Enter the prefix in **Prefix**.
6. For **Log file SSE-KMS encryption**, choose
 ...
 **Enabled**. If you don't enable SSE-KMS encryption, your
 ...
 (SSE-KMS)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingKMSEncryption.html "https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingKMSEncryption.html"). For more information about SSE-S3 encryption, see [Using Server-Side Encryption with Amazon S3-Managed
 Encryption Keys (SSE-S3)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingServerSideEncryption.html "https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingServerSideEncryption.html").
...
7. In **Additional settings**, configure the following.
	 ...
	 **Enabled** to have log digests delivered to your
	 ...
	 [Validating CloudTrail log file integrity](cloudtrail-log-file-validation-intro.md "cloudtrail-log-file-validation-intro.md").
	 ...
	 **Enabled** to be notified each time a log is
	 ...
	 information, see [Configuring Amazon SNS notifications for CloudTrail](configure-sns-notifications-for-cloudtrail.md "configure-sns-notifications-for-cloudtrail.md").
	...
	If you create a topic, you must subscribe to the topic to be notified
	 ...
	 programmatically. For more information, see [Getting started with Amazon SNS](https://docs.aws.amazon.com/sns/latest/dg/sns-getting-started.html "https://docs.aws.amazon.com/sns/latest/dg/sns-getting-started.html") in the *Amazon Simple Notification Service Developer Guide*.
...
Only the management account can configure a CloudWatch Logs log group for an
 ...
 or `UpdateTrail` API operations.
	 ...
	 **Policy document**. For more information about
	 this role, see [Role policy document for CloudTrail to use CloudWatch Logs for monitoring](cloudtrail-required-policy-for-cloudwatch-logs.md "cloudtrail-required-policy-for-cloudwatch-logs.md").
...
9. For **Tags**, you can add up to 50 tag key pairs to help 
 ...
 CloudTrail resources. For more information, see [AWS Resource Groups](https://docs.aws.amazon.com/ARG/latest/userguide/resource-groups.html "https://docs.aws.amazon.com/ARG/latest/userguide/resource-groups.html") and [Tags](cloudtrail-concepts.md#cloudtrail-concepts-tags "cloudtrail-concepts.md#cloudtrail-concepts-tags").
10. On the **Choose log events** page, choose the event types
 ...
 following.
	...
	3. Choose **Exclude Amazon RDS Data API events** to filter
	 ...
	 API events, see [Logging Data API calls with AWS CloudTrail](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/logging-using-cloudtrail-data-api.html "https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/logging-using-cloudtrail-data-api.html") in
...
Steps 12-16 are for configuring data events using advanced event
 ...
 more [resource types](logging-data-events-with-cloudtrail.md#logging-data-events "logging-data-events-with-cloudtrail.md#logging-data-events") and offer
 ...
 are using basic event selectors, complete the steps in [Configure data event settings using basic event selectors](cloudtrail-create-a-trail-using-the-console-first-time.md#trail-data-events-basic-selectors "cloudtrail-create-a-trail-using-the-console-first-time.md#trail-data-events-basic-selectors"), then return to step
 17 of this procedure.


For **Resource type**, choose the resource type on which
 you want to log data events. For more information about available resource types, see [Data events](logging-data-events-with-cloudtrail.md#logging-data-events "logging-data-events-with-cloudtrail.md#logging-data-events").
	...
	1. Choose from the following fields.
		...
		* **`resources.ARN`** - You can use
		...
		For more information about the ARN formats of data event
		 ...
		 keys for AWS services](https://docs.aws.amazon.com/service-authorization/latest/reference/reference_policies_actions-resources-contextkeys.html "https://docs.aws.amazon.com/service-authorization/latest/reference/reference_policies_actions-resources-contextkeys.html") in the *Service Authorization Reference*.
...
CloudTrail Insights analyzes management events for unusual activity, and logs events
 ...
 more information about Insights events, see [Working with CloudTrail Insights](logging-insights-events-with-cloudtrail.md "logging-insights-events-with-cloudtrail.md"). Additional
 charges apply for logging Insights events. For CloudTrail pricing, see [AWS CloudTrail Pricing](https://aws.amazon.com/cloudtrail/pricing/ "https://aws.amazon.com/cloudtrail/pricing/").
...
* Change the configuration of your trail by editing it. For more
 information, see [Updating a trail with the CloudTrail console](cloudtrail-update-a-trail-console.md "cloudtrail-update-a-trail-console.md").
...
* Create a table and use it to run a query in Amazon Athena to analyze your
 AWS service activity. For more information, see [Creating a Table for CloudTrail Logs in the CloudTrail Console](https://docs.aws.amazon.com/athena/latest/ug/cloudtrail-logs.html#create-cloudtrail-table-ct "https://docs.aws.amazon.com/athena/latest/ug/cloudtrail-logs.html#create-cloudtrail-table-ct")
 in the [Amazon Athena User Guide](https://docs.aws.amazon.com/athena/latest/ug/ "https://docs.aws.amazon.com/athena/latest/ug/").
