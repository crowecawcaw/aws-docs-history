...
## Syntax
<a name="remote-analyze-syntax"></a>
...
<a name="remote-analyze-parameters"></a>Parameters

**--application-id {{id}}**  
The application ID *(required)*. After you run the [remote inventory](cmd-remote-inventory.md) command, you can find the application ID in the `inventory.json` file in one of the following locations:   
...
**--target {{IP/FQDN}}**  
Specifies the IP address or FQDN of the application server targeted for the inventory *(required)*.Options

**--help**  
Displays the command help.

## Output
<a name="remote-analyze-output"></a>
...
## Examples
<a name="remote-analyze-examples"></a>
...
```
$ sudo app2container remote analyze --target  {{192.0.2.0}} --application-id {{java-tomcat-9e8e4799}}
...
```
PS> app2container remote analyze --target  {{192.0.2.0}} --application-id {{iis-smarts-51d2dbf8}}
