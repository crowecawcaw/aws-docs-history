...
To ensure that users and roles can still use the readiness check console when you allow access to
only specific API operations, also attach a `ReadOnly` AWS managed policy for readiness check to
the entities. For more information, see the readiness check [Readiness check
...
that is associated with readiness check in ARC. To learn more, see
...
To give users full access to use readiness check features through the console, attach a policy like the following to
the user:
...
to the API operations that the user needs to work with, as described below.
