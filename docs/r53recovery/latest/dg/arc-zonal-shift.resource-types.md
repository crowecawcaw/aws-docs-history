...
You can start a zonal shift for a Network Load Balancer by using the AWS CLI, the AWS Management Console, or the ELB widget.
...
The Network Load Balancer disables the targets in the impacted zone so that they stop receiving traffic, and ELB treats these targets as disabled targets for zonal shift.
