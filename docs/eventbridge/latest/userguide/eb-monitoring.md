...
| `EventBusEncryptionFailed`             | The number of times a re-encryption operation has failed for an event bus. Updating the event bus configuration triggers a re-encryption of the static configuration stored by EventBridge.<br>NoteEventBridge only sends this metric to CloudWatch if it isn't zero.                                                                        | EventBusName                            | Count        |
