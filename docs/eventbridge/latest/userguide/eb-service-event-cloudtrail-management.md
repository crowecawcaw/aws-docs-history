...
- Certain read-only management events, such as AWS Key Management Service `GetKeyPolicy` and
  ...
  change events.
- You may already be receiving read-only management events even if those events don't start with
  `Describe`, `Get`, or `List`. One such example is `TestEventPattern` from EventBridge.
