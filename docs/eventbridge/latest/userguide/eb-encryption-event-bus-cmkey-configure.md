...
EventBridge uses the new key to re-encrypt the data keys that protect data stored
on the event bus. EventBridge does not re-encrypt the stored data itself.
