...
When you update a pipe to use a different AWS KMS key, EventBridge uses the new key to re-encrypt the data keys that protect data stored on the pipe. EventBridge does not re-encrypt the stored data itself.
