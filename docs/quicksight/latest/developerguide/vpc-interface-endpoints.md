...
to privately access the Quick Suite website without leaving the Amazon network. Instances in your VPC don't need public IP addresses to communicate with Quick Suite website, but still need access to certain domains other than Quick Suite so that static assets, reports, and other files can be downloaded. For a list of domains that Quick Suite needs to access, see [Domains accessed by Quick Suite](#vpc-interface-endpoints-supported-domains "#vpc-interface-endpoints-supported-domains").
...
- The VPC endpoint that you create for Quick Suite only works for the Quick Suite website. Quick Suite API calls are not supported through VPC endpoints.
...
- Quick Suite VPC endpoints are not available in China regions.
- Quick Suite VPC endpoints are not available in GovCloud regions.
...
Create a VPC endpoint for Quick Suite using the following service name:
...
###### Example: VPC endpoint policy for Quick Suite actions
...
the Quick Suite website after the user signs in.
