...
Kinesis Data Streams is not eligible for the AWS Free Tier. When you are finished with this tutorial,
delete your AWS resources to stop incurring charges. For more information, see [Step 4: Clean up](#clean-up "#clean-up").

###### Topics

- [Step 1: Create a stream](#create-stream "#create-stream")
- [Step 2: Put a record](#put-record "#put-record")
- [Step 3: Get the record](#get-records "#get-records")
- [Step 4: Clean up](#clean-up "#clean-up")

## Step 1: Create a stream

Your first step is to create a stream and verify that it was successfully created.
Use the following command to create a stream named "Foo":

```
aws kinesis create-stream --stream-name Foo
```

Next, issue the following command to
check on the stream's creation progress:
...
In this example, the stream has a status CREATING, which means it is not yet ready to
use. Check again in a few moments, and you should see output similar to the following
example:
...
important information for now is `"StreamStatus": "ACTIVE"`, which tells you
...
`list-streams` command, as shown here:
...
tutorial, you will use the simplest possible command, `put-record`, which
...
Congratulations, you just added data to a stream! Next you will see how to get data
out of the stream.
...
shard from which the consumer (`get-record` command in this case) will read.
You'll use the `get-shard-iterator` command as follows:

```
aws kinesis get-shard-iterator --shard-id shardId-000000000000 --shard-iterator-type TRIM_HORIZON --stream-name Foo
```

Recall that the `aws kinesis` commands have a Kinesis Data Streams API behind them, so if
you are curious about any of the parameters shown, you can read about them in the [`GetShardIterator`](../../../kinesis/latest/APIReference/API_GetShardIterator.md "../../../kinesis/latest/APIReference/API_GetShardIterator.md")
...
The `get-records` command gets data from the stream, and it resolves to a
call to [`GetRecords`](../../../kinesis/latest/APIReference/API_GetRecords.md "../../../kinesis/latest/APIReference/API_GetRecords.md") in
...
the portion of the shard that the iterator points to, `GetRecords` returns an
...
If you are running this tutorial from a Unix-type command processor such as bash, you
can automate the acquisition of the shard iterator using a nested command, like
this:
...
If you are running this tutorial from a system that supports PowerShell, you can
automate acquisition of the shard iterator using a command such as this:
...
The successful result of the `get-records` command will request records
...
Note that `get-records` is described above as a
_request_, which means you may receive zero or more records even
...
be garbage – and it's not the clear text `testdata` we sent. This is
due to the way `put-record` uses Base64 encoding to allow you to send binary
data. However, the Kinesis Data Streams support in the AWS CLI does not provide Base64
_decoding_ because Base64 decoding to raw binary content printed
to stdout can lead to undesired behavior and potential security issues on certain
platforms and terminals. If you use a Base64 decoder (for example, [https://www.base64decode.org/](https://www.base64decode.org/ "https://www.base64decode.org/")) to
manually decode `dGVzdGRhdGE=` you will see that it is, in fact,
`testdata`. This is sufficient for the sake of this tutorial because, in
...
(`describe-stream` and `list-streams`). For more information
about the KCL, see [Developing Custom Consumers with Shared Throughput Using KCL](shared-throughput-kcl-consumers.md "shared-throughput-kcl-consumers.md").

`get-records` doesn't always return all records in the stream/shard
specified. When that happens, use the `NextShardIterator` from the last
...
data using `get-records` each time. However, if you do not call
`get-records` using the next shard iterator within the 300 second shard
iterator lifetime, you will get an error message, and you must use the
`get-shard-iterator` command to get a fresh shard iterator.

Also provided in this output is `MillisBehindLatest`, which is the number
of milliseconds the [GetRecords](../../../kinesis/latest/APIReference/API_GetRecords.md "../../../kinesis/latest/APIReference/API_GetRecords.md") operation's response is from the tip of the
...
A successful `get-records` result will always have a
`NextShardIterator` even if there are no more records currently in the
...
If you call `get-records` until there are no more records in the stream and
...
Delete your stream to free up resources and avoid unintended charges to your account.
Do this any time you have created a stream and will not be using it, because charges
accrue per stream whether you are putting and getting data with it or not. The clean-up
...
Success results in no output. Use `describe-stream` to check on the
deletion progress:
...
If you execute this command immediately after the delete command, you will see output
...
After the stream is fully deleted, `describe-stream` will result in a "not
found" error:
