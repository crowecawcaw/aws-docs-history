...
Delete an Amazon S3 delivery when you no longer need it. Deleting a delivery stops
delivery from the source stream to the destination. Data already delivered to the Amazon S3
bucket is not removed.
...
and, if you no longer need them, the destination bucket and dead-letter queue bucket.
For a full cleanup walkthrough, see [Getting started with S3 general purpose delivery](data-delivery-s3-getting-started.md "data-delivery-s3-getting-started.md").
