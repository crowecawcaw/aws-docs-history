This guide documents the classic version of the AWS Wickr administration console, released before March
13, 2025. For documentation on the new AWS Wickr administration console, see [Administration Guide](../adminguide/what-is-wickr.md "../adminguide/what-is-wickr.md").

# Pinwheel (Quick Access) for ATAK

The pinwheel or quick access feature is used for one-one-one conversations or direct
messages.

Complete the following procedure to use the pinwheel.

1. Open the split screen view of the ATAK map and the Wickr for ATAK plugin
   simultaneously. The map displays your teammates or assets on the map
   view.
2. Choose the user icon to open the pinwheel.
3. Choose the Wickr icon to view the available options for the selected
   user.

![Wickr icon.](images/atak-pinwheel-wickr-icon.png) 4. On the pinwheel, choose one of the following icons:

    * **Phone**: Choose to call.



    ![Pinwheel call icon.](images/atak-pinwheel-call.png)
    * **Message**: Choose to chat.



    ![Pinwheel chat icon.](images/atak-pinwheel-message.png)
    * **File send**: Choose to send a file.



    ![Pinwheel send a file icon.](images/atak-pinwheel-send.png)
