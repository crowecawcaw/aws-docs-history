...
Support for multi-region federation. Enterprise customers can now federate with AWS Wickr
customers in AWS Canada (Central) and London regions in addition to Northern Virginia.
...
- Fixed not showing guest user restricted user interface (UI) as soon as the last licensed
  user leaves the conversation
- Added missing create password CTA analytics
- 6.22.3 - Addressed calling compatibility with Android 13, September 2023 security
  update
  Improvements:
...
- Fixed issue where users could not download an image when certificate pinning was
  enabled
  Improvements:
...
- Implemented retry logic on getUserInfo error handling in case network issues occur. The
  new logic will improve room recovery.
