...
**Topics**
+ [Wickr IO client provisioning logs](#provisioning-logs)
+ [Wickr IO client logs](#client-logs)
+ [Wickr IO integration logs](#integration-logs)

## Wickr IO client provisioning logs
<a name="provisioning-logs"></a>
...
In the example above, the host directory shared with the Docker container is `/opt/WickrIOOnHost`. As a result,the logs will be located at: `/opt/WickrIOOnHost/logs`.
...
In the above example, the host directory path shared with the docker container is `/opt/WickrIOOnHost`, then the logs path will be `/opt/WickrIOOnHost/clients/(client name)/logs`.
