...
<a name="host-machine-requirements"></a>

You should provision a host machine capable of running Docker to deploy the Wickr IO Docker container. Refer to the [Docker website](https://docs.docker.com/engine/install/) for instructions on how to install Docker on your system. Once the docker is installed, make sure the docker service is up and running using the `docker info` command.
...
<a name="host-os-specifications"></a>
...
<a name="host-resource-specifications"></a>
...
Wickr IO bot clients have the same networking requirements as traditional Wickr clients. The ports and domains needed for basic connectivity are in the AWS Wickr administration guide. For more information, see [ Ports and domains to allow list](https://docs.aws.amazon.com/wickr/latest/adminguide/allow-list-ports-domains.html) for your Wickr network.
...
If you plan to use other AWS services with a Wickr bot, you must ensure the host has the appropriate AWS Identity and Access Management (IAM) role and policy to access them. For more information, see [ What is IAM?](https://docs.aws.amazon.com/IAM/latest/UserGuide/introduction.html).
