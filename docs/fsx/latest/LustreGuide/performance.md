...
**Note**  
\* The traffic between an individual client instance and an individual FSx for Lustre object storage server is limited to 5 Gbps. Refer to the [IP addresses for file systems](using-fsx-lustre.md#ip-addesses-for-fs) for the number of object storage servers underpinning your FSx for Lustre file system.
