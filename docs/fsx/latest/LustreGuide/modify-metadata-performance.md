...
- To increase your metadata performance, use the `--lustre-configuration
 MetadataConfiguration` property. This property has two parameters, `Mode` and `Iops`.
  ...
  (which switches the file system mode to USER\_PROVISIONED in addition to increasing the Metadata IOPS value). 2. For SSD file systems, set `Iops` to a value of `1500`, `3000`,
