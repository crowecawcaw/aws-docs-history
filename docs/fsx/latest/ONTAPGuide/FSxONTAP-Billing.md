...
**Note**  
Like other AWS services, FSx for ONTAP charges you for only what you use. For more information, see [Amazon FSx for NetApp ONTAP Pricing](https://aws.amazon.com/fsx/netapp-ontap/pricing/).
...
1. Choose a **Billing period** (for example, August 2024).

1. To see Amazon FSx charges, on the **Charges by service** tab, enter **FSx** in the filter by service text field, and then expand **FSx** to view charges by AWS Region.

   Charges for FSx for ONTAP file systems appear under *Amazon FSx CreateFileSystem:ONTAP* entries in the report.

1. To download the detailed billing report in CSV format, choose **Download all to CSV** at the top of the **Bills** page.
...
1. Under the **Report parameters** section, choose the **Date range** and granularity for your report.

1. Leave **Group by > dimension** set to **Service**.

1. Under **Filters > Service**, choose **FSx**

1. Choose the **Usage type**. See the table following this procedure for a list of FSx for ONTAP usage types.

1. Make any additional filter selections for your report.

1. To download the report details to a file, choose **Download as CSV**.

The following table lists the FSx for ONTAP usages types that you can use to filter the report to view usage data for ONTAP file systems. For more information about using the Cost Explorer, see [Analyzing your costs and usage with AWS Cost Explorer](https://docs.aws.amazon.com/cost-management/latest/userguide/ce-what-is.html) in the AWS Cost Management User Guide.
