...
| `CopyObject`<br>(same-Region copies only)  | Supported, if source and destination are within the same access point. The `x-amz-object-annotation-directive` header is not supported. |
