...
1.  Create a DNS entry for your Active Directory by providing the full directory DNS name (`corp.example.com`) and at least one DNS server IP address.
   ...
   To verify the connection to your DNS servers, run the following command. Replace {{svm\_name}} with your own information. 
...
1. If you need to modify the Active Directory configuration itself, you can change existing fields by using the following command, replacing:
   + {{computer\_name}}, if you want to modify the NetBIOS (machine account) name of the SVM.
   + {{domain\_name}}, if you want to modify the name of the domain. This should correspond with the DNS domain entry noted in Step 3 of this section (`corp.example.com`).
