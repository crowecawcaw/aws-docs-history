...
After you create an access point attached to an FSx for ONTAP volume, you can access your data via the AWS CLI and S3 API, as well as S3-compatible
AWS and third-party services and applications. When referring to an access point in an AWS service or application you can use the Amazon Resource Name (ARN),
...
An access point alias cannot be changed. For an access point attached to an FSx for ONTAP volume,
...
The following shows the ARN and access point alias for an S3 access point attached to an FSx for ONTAP volume, returned as part of the response to a
`describe-s3-access-point-attachments` FSx CLI command. The access point in this example
is named `my-ontap-ap`.
...
The `-ext-s3alias` suffix is reserved for the aliases of S3 access points attached to an FSx for ONTAP volume,
...
see [Access point compatibility](access-points-for-fsxn-object-api-support.md "access-points-for-fsxn-object-api-support.md").
...
part of the domain name in the URL. To view the S3 URI for an access point attached to an FSx for ONTAP volume, in the access point details page under
