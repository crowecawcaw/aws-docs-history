...
- **Storage**
  - **If there's a warning for this metric:** Primary storage capacity utilization
  - **Do this:** Increase your file system's primary storage capacity if your file system isn't already at the maximum SSD storage capacity. For more information, see [Updating storage capacity and provisioned IOPS](increase-storage-capacity.md).<br />If your file system has multiple HA pairs and your primary storage capacity utilization is only higher for a subset of your file system's aggregates (the storage pools that make up your primary storage tier), then you can also rebalance your workload so that your primary storage capacity utilization is more evenly spread across your file system. For more information on rebalancing your workloads, see [Balancing workloads across HA pairs](monitor-workload-balance.md).<br />If you're currently performing an SSD decrease operation and utilization exceeds 80% on the new set of disks, you can either tier data to the capacity pool, delete data from volumes that have been redirected to the new disks, or submit a request to increase SSD capacity during the decrease operation. Amazon FSx will prioritize the increase request before resuming the decrease operation. For more information, see [To decrease SSD storage capacity for a file system (console)](increase-storage-capacity.md#decrease-storage-console).

- **File server performance**
  - **If there's a warning for this metric:**
    - Network throughput
    - Disk throughput
    - Disk IOPS
    - CPU utilization
  - **Do this:** Increase your file system's throughput capacity if your file system is not already at the maximum throughput capacity. For more information on updating throughput capacity, see [Updating throughput capacity](increase-throughput-capacity.md).<br />If your file system has multiple HA pairs and utilization is high for only a subset of file servers, then you can also rebalance your workload so that your workload is more evenly utilizing the performance capabilities of each of your file system's HA pairs. For more information on rebalancing your workloads, see [Balancing workloads across HA pairs](monitor-workload-balance.md).

- **Disk performance**
  - **If there's a warning for this metric:** Disk IOPS
  - **Do this:** Increase SSD IOPS if your file system is not already at the maximum SSD IOPS for your file system's current throughput capacity. For more information on updating your file system's provisioned IOPS, see [Updating storage capacity and provisioned IOPS](increase-storage-capacity.md).<br />If your file system has multiple HA pairs and your disk IOPS utilization is only higher for a subset of your file system's aggregates (the storage pools which make up your primary storage tier), then you can also rebalance your workload so that your disk IOPS are utilized more evenly across your file system. For more information on rebalancing your workloads, see [Balancing workloads across HA pairs](monitor-workload-balance.md).
