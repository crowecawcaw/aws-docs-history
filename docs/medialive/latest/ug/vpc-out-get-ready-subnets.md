...
An Amazon VPC user must set up the VPC and identify subnets and security
groups for the MediaLive channel.
...
2. Determine if you need to identify EIPs to associate with the channel.
   If the channel has output groups with destinations outside your VPC, you
   must provide a mechanism for the content to leave the VPC. One way to do
   this is to associate EIPs with the channel endpoints. These endpoints
   appear in the diagram in [How VPC delivery works](vpc-out-how-it-works.md "vpc-out-how-it-works.md") Speak to
   the Amazon VPC user about your requirements.

If you decide to associate EIPs with the channel endpoints, identify
those EIPs. 3. After the Amazon VPC user has performed the setup, obtain the following
information:

    * The ID of the VPC or
     VPCs.
    * The IDs of the subnets and Availability Zones for the channel
     endpoints.
    ...
    * The IDs of the security groups for the
     subnets.
    * The elastic IP address to associate with the elastic network
