...
First, you don't create separate captions selectors; all the languages are automatically
included. Second, if you are outputting to HLS, there is an opportunity to specify the
languages and the order in which they appear.

![Diagram showing captions selector, output with video and audio, and HLS output group.](images/captions_INembed_OUTembed_hls_result.png)
...
5. In that same output, create one captions asset with the following:
   ...
   - **Language code** and **Language
     description**: Keep the field blank. With embedded captions, all the
     languages are included.

6. In the HLS output group, in **Captions**, for **Captions
   language setting**, choose **Insert**.
7. For **HLS settings**, in **Captions language
   mappings**, choose **Add captions language mappings** twice
   (once for each language).
8. Complete the first group of mapping fields with `1`,
