...
This section provides very general information about pricing. For detailed information, see [https://aws.amazon.com/medialive/pricing/](https://aws.amazon.com/medialive/pricing/). 
...
+ There is a *running input charge* for each input that is attached to a channel that is running. The charge applies to both push and pull inputs. It applies even to the inputs in the channel that aren't currently active or that aren't receiving any content.
  ...
  Inputs from a Link device use a different pricing model. Link inputs are charged a flat rate that is not based on codec, bitrate, or resolution. Link UHD devices are charged at a higher rate than Link HD devices. A Link UHD device that is configured to send HD resolution is charged at the Link HD rate. For specific rates, see [https://aws.amazon.com/medialive/pricing/](https://aws.amazon.com/medialive/pricing/).
