...
encounter when working with MediaLive and IAM.

###### Topics

- [I am not authorized to perform an action in MediaLive](#security_iam_troubleshoot-no-permissions "#security_iam_troubleshoot-no-permissions")
- [I am not authorized to perform iam:PassRole](#security_iam_troubleshoot-passrole "#security_iam_troubleshoot-passrole")
- [I want to allow people outside of my AWS account to access my MediaLive resources](#security_iam_troubleshoot-cross-account-access "#security_iam_troubleshoot-cross-account-access")
...
have the fictional `medialive:`GetWidget`` permissions.
...
MediaLive. However, the action requires the service to have permissions that are granted by a service role. Mary does not have permissions to pass the
role to the service.
...
- To learn whether MediaLive supports these features, see [How AWS Elemental MediaLive works with IAM](security_iam_service-with-iam.md "security_iam_service-with-iam.md").
