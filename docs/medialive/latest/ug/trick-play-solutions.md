...
player. In AWS Elemental MediaLive, you can include track assets in the output group.
The downstream system for that output group can use these assets to
implement the visual cue in their trick-play implementation.
...
- Is the supported implementation required or optional? Both
  ...
  HLS manifest. If the lines are absent, the downstream system
  will fail to handle the output from MediaLive?
...
- If you choose the I-frame-only manifest method, confirm
  ...
  able to handle the output from MediaLive. MediaLive doesn't support
  customizations of the method.
- If you choose the image media playlist method, confirm
  ...
  system won't be able to handle the output from MediaLive. MediaLive
  doesn't support customizations of the implementation.
