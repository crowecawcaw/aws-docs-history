...
a channel. You can work with the schedule when the channel is
running or when it is idle.

The actions are performed in the channel when the channel is
running.

The console provides two views for working with actions:

- A list view that lists actions in tabular format
- A timeline view that shows a timeline representation of
  the actions
  In either view, you can do the following:

- Create individual actions.
- Delete individual actions.
- Delete several actions in one request (a batch
  command).
