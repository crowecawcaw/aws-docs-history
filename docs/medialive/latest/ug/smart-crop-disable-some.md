...
| Smart Subtitles                   | 1. Remove the `SmartSubtitleSourceSettings`<br>caption selectors from all input attachments.<br>2. Remove or update any caption descriptions that<br>reference those selectors.<br>3. Remove the captions-only output (WebVTT or TTML) if<br>it is no longer needed.                                                                                                                                                                                                                                                                                                                                                                                     |
...
| To disable Smart Subtitles                  | Use `UpdateFeed` to change the status parameter of<br>the `OutputConfig` that is of type<br>`SubtitlingConfig`. Change the status from<br>`ENABLED` to `DISABLED`. |
