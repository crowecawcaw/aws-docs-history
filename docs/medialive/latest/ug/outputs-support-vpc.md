...
The following table specifies which MediaLive containers can be delivered to a destination
in the VPC, when the channel that is set up for VPC delivery. For more information about
VPC delivery, see [Delivering outputs via your VPC](delivery-out-vpc.md "delivery-out-vpc.md").
...
| Multiplex                            | NoWhen the channel is set up for VPC delivery, it can't contain<br>a multiplex output. | NoWhen the channel is set up for VPC delivery, it can't contain<br>a multiplex output. |
| RTMP or RTMPS                        | A server on Amazon EC2                                                                 | Yes, if you associate Elastic IP addresses with the channel                            |
| SRT caller                           | With a specified IP address                                                            | Yes, if you associate Elastic IP addresses with the channel                            |
