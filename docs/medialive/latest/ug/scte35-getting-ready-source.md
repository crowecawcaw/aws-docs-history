...
+ `break_duration`:
  ...
  When a non-zero `duration` is provided in the inbound SCTE message, the `auto_return` value is ignored. MediaLive ends the break at the specified duration. The duration of a break in progress can be adjusted with subsequent SCTE messages that reference the same `splice_event_id`.
