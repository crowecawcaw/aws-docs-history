...
<a name="input-clipping"></a>

You can clip a file input so that MediaLive ingests only a portion of the file. The file must be an MP4 file that is stored on Amazon S3 or an HTTP server that supports HTTP range requests.
...
1. Create the MP4 file input in the usual way. See [Setting up an MP4 input](mp4-pull-input.md).

1. Attach the input to the channel in the usual way. See [Attach inputs to the channel](creating-a-channel-step2.md). 
