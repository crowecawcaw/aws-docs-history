...
The destination for an Archive output group is always in an Amazon S3 bucket. You and the
Amazon S3 operator must agree about the bucket to use.
...
2. We recommend that you design the full path of the destination — the Amazon S3
   bucket and all the folders. See [Fields for the output destination](archive-destinations.md "archive-destinations.md").
...
output. For more information, see [Controlling access to the output](#setting-dss-archive-canned-acl "#setting-dss-archive-canned-acl"), after this procedure.
