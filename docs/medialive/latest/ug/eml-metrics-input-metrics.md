...
+ [Input loss seconds](#eml-metrics-udp-input-loss)
...
+ Units: Count.
+ Meaning of zero: An RTP Push, SRT, or MediaConnect input was being ingested and no packets were lost.
+ Meaning of no datapoints: There are no RTP Push, SRT, or MediaConnect inputs active or being prepared (by the schedule). Or you included the ActiveInputFailoverLabel in a channel that isn't set up for automatic input failover. 
