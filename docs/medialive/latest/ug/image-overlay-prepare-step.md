...
Keep in mind that with the per-output insertion option, MediaLive inserts the image on the output video frame *after* it sets the output video resolution. This means that the image will not be resized with the output video.
