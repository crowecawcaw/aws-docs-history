...
This section provides an example of the audio portion of the manifest that MediaLive
produces when you set up an audio-only output. This sample manifest contains the
following elements:
...
- Two video outputs, as indicated by the presence of two
  ...
  - The first video output has a low bandwidth. As indicated by the
    ...
    _audio1_.
  - The second video output has a higher bandwidth. As indicated by the
    `AUDIO` parameter, it is associated with
    _audio2_.

- Four audio outputs, as indicated by the presence of four
  `EXT-X-MEDIA` lines with `TYPE=AUDIO`. In each audio
...
- Two audio rendition groups. The audio renditions groups don't have their own
  ...
  `GROUP-ID` parameters in the audio lines.
- Two captions streams, as indicated by the presence of two
  `EXT-X-MEDIA` lines with `TYPE=SUBTITLES`.
