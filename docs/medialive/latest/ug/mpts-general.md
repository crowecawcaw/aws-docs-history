...
The MPTS is an RTP output. MediaLive creates and delivers the MPTS to AWS Elemental MediaConnect in the account associated with the MediaLive that is creating the MPTS. AWS Elemental MediaConnect automatically sets up the output from MediaLive as an entitled source that uses the RTP protocol. You don't have to perform any steps to set up this entitled source. 

In order to complete the distribution of the MPTS, the MediaConnect operator must create a flow that uses that entitled source. The flow can be any protocol, it doesn't have to be RTP.
