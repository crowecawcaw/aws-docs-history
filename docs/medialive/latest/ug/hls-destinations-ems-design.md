...
- The user of the AWS service should have provided you with
  the container names.
...
Therefore, if the buckets or containers are different, the
folder path and file names for the two destinations can be
different from each other, or they can be the same. For
example:
...
- If the buckets or containers are the same, the folder path and
  file names for the two destinations must be different from each
  other. For example:
