...
You must also decide if you want to give some or all of your users
permission to set the retention policy for logs. If you decide not to give
this access to any user, an administrator must be responsible for setting
the policy.

Users don't need special permission to enable logging from within
MediaLive.

The following table shows the actions in IAM that relate to access
for setting up channel logs.
