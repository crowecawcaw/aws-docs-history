...
- Identify the types of output media that you plan to create in the channel, for
  example, Microsoft Smooth and HLS.
- Identify the combinations of video and audio that you plan to create for each output
  media.
- For each output media, identify which input captions will be converted to which
  output formats. For example, you will convert Teletext captions to TTML for the
  Microsoft Smooth output media, and those same Teletext captions to WebVTT for the HLS
  output media.

The output formats that are possible depend on the input formats and the type of
output media. To determine which output captions are possible given the input format,
...
- The output container must support all the formats. See [Captions supported in MediaLive](supported-captions.md "supported-captions.md").
