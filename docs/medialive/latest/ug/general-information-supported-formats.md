...
| Burn-in |  | Yes | From input: It is technically impossible for the encoder to read burn-in captions. Therefore, from an input viewpoint, they can't be considered to be captions.<br />For output: Burn-in captions are captions that are converted into text and then overlaid on top of the picture directly in the video stream. | 
