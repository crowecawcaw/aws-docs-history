...
1. [Stop the channel](starting-stopping-deleting-a-channel.md).

1. Change the channel class to standard class. See the steps after this list.
...
1. [Restart the channel](starting-stopping-deleting-a-channel.md).
...
1. Obtain a second destination address for each output group. Each address is at the downstream system of each output group.
   ...
   Plan these destinations now, in the same way as you planned the destination addresses when you originally set up the channel. For more information, see [Setup: Creating output groups and outputs](medialive-outputs.md), and read the information about coordinating for the type of output group that you are creating.
...
When you originally created the channel, might have been sure that you wouldn't need to convert the channel, to a standard channel. Therefore, you [set up the channel](single-pipeline-no-upgrade.md) as a single-pipeline channel with single-class inputs. 
...
1. Detach each single-class input. To detach the inputs, you must [edit the channel](editing-deleting-channel.md) and remove the attached inputs.

1. [Edit each input](edit-input.md) to convert it to standard class, and to add a second source.

1. Edit the channel to change the channel class to standard class. See the steps after this list.

1. [Edit the channel](editing-deleting-channel.md) to reattach each input.
