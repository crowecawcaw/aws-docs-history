...
You can share a single encode among several outputs within one MediaLive channel. You can clone an encode so that it serves as the basis for a new encode within the channel.

## Sharing encodes
<a name="encode-share"></a>

You can share an encode among several MediaLive outputs when you want these outputs to have identical encodes. When you share an encode, there is only one instance of the encode in the channel. All the affected outputs use that encode.
...
You can clone an encode so that it serves as the basis for a new encode within the MediaLive channel.
