...
When you followed the [guidelines](pipeline-redundancy-guidelines.md) for implementing pipeline redundancy in a MediaLive channel, you might have decided that you might want to implement pipeline redundancy. In this case, make sure that you set up the inputs as standard-class inputs and set up the channel as a standard channel. 
...
+ When you [create inputs](medialive-inputs.md), set up all the inputs as standard-class inputs.

  Some inputs are always set up as standard-class inputs. For all other inputs, set the **Input class** field to **Standard input**.
+ When you create the channel, do the following:
  + Set up the channel as a standard channel. See [Complete channel and input details](creating-a-channel-step1.md).
...
+ Contact the upstream system and request that they provide two content sources.
