...
+ [I don't see any messages from CodeGuru Profiler in my application logs.](#troubleshooting-agent-not-running)
...
+ [I don’t see any profiling data for my AWS Lambda function.](#troubleshooting-lambda)
+ [I have errors in my Lambda function log.](#troubleshooting-lambda-error)
+ [I received a `ValidationException` error in the agent.](#troubleshooting-lambda-agent)
+ [Why don't I see heap summary data?](#troubleshooting-heap-summary)

## Why are certain methods missing from my profile?
<a name="troubleshooting-missing-functions"></a>
...
<a name="troubleshooting-agent-not-running"></a>
...
<a name="troubleshooting-profiler-group-does-not-exist"></a>
...
<a name="troubleshooting-agent-permission"></a>
...
<a name="troubleshooting-not-enough-data"></a>
...
<a name="troubleshooting-lambda"></a>
...
<a name="troubleshooting-lambda-error"></a>

If you enabled profiling from the Lambda console and continuously see `CreateProfilingGroup` error messages or `ResourceNotFoundException` in your logs, your Lambda function did not run long enough for the `CreateProfilingGroup` API call to succeed. You can manually create a proﬁling group with Lambda as your compute platform from the CodeGuru Profiler console. Set the name to `aws-lambda-{{lambda-function-name}}`.

## I received a `ValidationException` error in the agent.
<a name="troubleshooting-lambda-agent"></a>

If you see the following error in your logs, it means you configured your profiling group for AWS Lambda, but not your agent. For more information on how to configure the agent with Lambda, see [Profiling your applications that run on AWS Lambda](https://docs.aws.amazon.com/codeguru/latest/profiler-ug/setting-up-lambda.html).
...
<a name="troubleshooting-heap-summary"></a>
