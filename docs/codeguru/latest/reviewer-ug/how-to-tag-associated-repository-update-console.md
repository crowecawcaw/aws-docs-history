...
the key, which is equivalent to removing the current tag and adding a different one
with the new name and the same value.
...
4. Enter new values in **key** and
   **value** to edit tags. Choose
   **Remove** next to a tag to remove it. Choose
