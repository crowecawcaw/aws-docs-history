...
| Key name       | Value                                                                                                                                          |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `displayName`  | The human-readable name to display and associate with the user (for<br>example, Mary Major or Saanvi Sarkar).                                  |
| `emailAddress` | The email address you want displayed for and associated with the user<br>(for example, mary\_major@example.com or saanvi\_sarkar@example.com). |

If this information is provided, CodeCommit includes it in events sent to Amazon EventBridge and
Amazon CloudWatch Events. For more information, see [Monitoring CodeCommit events in Amazon EventBridge and Amazon CloudWatch Events](monitoring-events.md "monitoring-events.md").

To use session tagging, roles must have policies that include the
`sts:TagSession` permission set to `Allow`. If you are using
...
You can use the AWS CLI to pass session tags for `displayName` and
`emailAddress` using **AssumeRole**. For example,
a
user who wants to assume a role named `Developer` who wants to
associate her name `Mary Major` might use the
...
For more information, see [AssumeRole](../../../IAM/latest/UserGuide/id_session-tags.md#id_session-tags_adding-assume-role "../../../IAM/latest/UserGuide/id_session-tags.md#id_session-tags_adding-assume-role").

You can use the `AssumeRoleWithSAML` operation to return a set of
temporary credentials that include `displayName` and
`emailAddress` tags. You can use these tags when you access CodeCommit
...
email address for a user named `Saanvi Sarkar` as session
tags:
...
For more information, see [Passing Session Tags using AssumeRoleWithSAML](../../../IAM/latest/UserGuide/id_session-tags.md#id_session-tags_adding-assume-role-saml "../../../IAM/latest/UserGuide/id_session-tags.md#id_session-tags_adding-assume-role-saml").

You can use the `AssumeRoleWithIdentity` operation to return a set of
temporary credentials that include `displayName` and
`emailAddress` tags. You can use these tags when you access CodeCommit
...
call `AssumeRoleWithWebIdentity` that includes the `displayName`
and `emailAddress` session tags for a user named `Li
 Juan`:
...
For more information, see [Passing Session Tags using AssumeRoleWithWebIdentity](../../../IAM/latest/UserGuide/id_session-tags.md#id_session-tags_adding-assume-role-idp "../../../IAM/latest/UserGuide/id_session-tags.md#id_session-tags_adding-assume-role-idp").

You can use the `GetFederationToken` operation to return a set of
temporary credentials that include `displayName` and
`emailAddress` tags. You can use these tags when you access CodeCommit
...
CodeCommit resources in your Amazon Web Services account. If you create an identity-based policy
...
To ensure that those entities can still use the CodeCommit console, also attach
...
a policy to the entity that grants them the correct permissions in CodeCommit. After the permissions are granted, provide the credentials to the user
