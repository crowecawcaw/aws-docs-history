...
1. To change the AWS KMS encryption key used to encrypt and decrypt data in the repository, in **Repository encryption key**, choose either **AWS managed key** or **Customer managed key** to specify the type of key to use. If choosing a customer managed key, enter the ARN of the key. Choose **Save**.
