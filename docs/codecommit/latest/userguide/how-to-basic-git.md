...
| Shows what has or hasn't been added to the pending commit in the local repo in a concise format.<br />(`M` = modified, `A` = added, `D` = deleted, and so on) | `git status -sb` | 
...
| Lists recent commits in the local repo in a custom format, with a graph.<br />(For more options, see [Understanding History: Commits](https://git-scm.com/docs/user-manual#understanding-commits) in the Git documentation.) | `git log --graph --pretty=format:"%H (%h) : %cn : %ar : %s"` | 
