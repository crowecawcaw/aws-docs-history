...
+ Review [Cedar design patterns](https://docs.cedarpolicy.com/overview/patterns.html) in the Cedar policy language Reference Guide.
+ Consider the [best practices](https://docs.cedarpolicy.com/bestpractices/bp-naming-conventions.html) in the Cedar policy language Reference Guide.
+ Consider the best practices included on this page.

**Topics**
+ [There isn't a canonical “correct” model](#design-no-canonical-correct-model)
+ [Return 403 forbidden errors rather than 404 not found errors](#resource-existence-errors)
+ [Focus on your resources beyond API operations](#design-focus-on-data-beyond-apis)
+ [Multi-tenancy considerations](#design-multi-tenancy-considerations)
...
You'll find that the [getting started](getting-started-first-policy-store.md) process in the Verified Permissions console includes the option to build your resources and actions directly from an API. This is a useful baseline: a direct mapping between your policy store and the API that it authorizes for.
...
When your application has multiple policy stores, your [policy templates](policy-templates.md) and a [policy store schema](schema.md) add a level of design and maintenance overhead in each policy store.
...
Verified Permissions has resource and request-rate quotas that might influence your multi-tenancy decision. For more information about quotas, see [Quotas for resources](quotas.md#quotas-resources).
