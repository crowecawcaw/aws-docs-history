...
1. In the [Verified Permissions console](https://console.aws.amazon.com/verifiedpermissions), select **Create new policy store**.
...
1. In the **Details** section, type a **Namespace for your schema**. For more information about namespaces, see [Namespaces](https://docs.cedarpolicy.com/schema/schema.html#namespace) in the Cedar documentation.
...
1. In the **Import resources and actions** step, under **API**, choose an API that will function as the model to your policy store resources and actions.
   ...
   1. Choose a **Deployment stage** from the stages configured in your API and select **Import API**. For more information about API stages, see [Setting up a stage for a REST API in the Amazon API Gateway Developer Guide](https://docs.aws.amazon.com/apigateway/latest/developerguide/set-up-stages.html).
...
1. After some time, typically about an hour, or when the **Deploy Lambda authorizer** step shows **Success**, if you chose to attach the authorizer manually, configure your authorizer.
   ...
   To learn how to assign a Lambda authorizer, see [Use API Gateway Lambda authorizers](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-use-lambda-authorizer.html) in the *Amazon API Gateway Developer Guide*.
...
You can create a policy store using the `CreatePolicyStore` API. For more information, see [CreatePolicyStore](https://docs.aws.amazon.com/verifiedpermissions/latest/apireference/API_CreatePolicyStore.html) in the Amazon Verified Permissions API Reference Guide.
