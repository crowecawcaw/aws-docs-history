...
- **`EndpointOverride`**   –  
  The endpoint override for the Timestream for LiveAnalytics service. This is an advanced option that overrides
...
- **`Region`**   –  
  The signing region for the Timestream for LiveAnalytics service endpoint.
...
- **`OktaApplicationID`**   –  
  The unique Okta-provided ID associated with the Timestream for LiveAnalytics application. A place
...
- **`MaxConnections`**   –  
  ...
  Timestream service. The value must be positive.
...
The AWS SDK log level is separate from the Timestream for LiveAnalytics ODBC driver log level.
...
- `TRACE`
- `FATAL`
  If `TS_AWS_LOG_LEVEL` is not set, the SDK log level is set to the default,
  which is `WARN`.

## Connecting through a proxy

The ODBC driver supports connecting to Amazon Timestream for LiveAnalytics through a proxy. To use this
