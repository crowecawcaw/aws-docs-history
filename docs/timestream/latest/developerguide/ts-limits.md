...
| Table and Database Name |  +   We recommend you do not use [System identifiers](#limits.system_identifier).  <br />+   Can contain a-z A-Z 0-9 \_ (underscore) - (dash) . (dot).  <br />+   All names must be encoded as UTF-8, and are case sensitive.    Table and database names are compared using UTF-8 binary representation. This means that comparison for ASCII characters is case sensitive.   | 
| Measure Name |  +   Must not contain [System identifiers](#limits.system_identifier) or colon ':'.  <br />+   Must not start with a reserved prefix (`ts_`, `measure_value`).    Table and database names are compared using UTF-8 binary representation. This means that comparison for ASCII characters is case sensitive.   | 
| Dimension Name |  +   Must not contain [System identifiers](#limits.system_identifier), colon ':' or double quote (").  <br />+   Must not start with a reserved prefix (`ts_`, `measure_value`).  <br />+   Must not contain Unicode characters [0,31] listed [here](https://www.asciitable.com/) or "\\u2028" or "\\u2029".    Dimension and measure names are compared using UTF-8 binary representation. This means that comparison for ASCII characters is case sensitive.   | 
