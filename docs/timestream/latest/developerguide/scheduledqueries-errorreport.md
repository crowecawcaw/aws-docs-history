...
This section describes the location, format, and reasons for error reports generated
by Timestream for LiveAnalytics when errors are encountered by running scheduled queries.

###### Topics

- [Reasons](#scheduled-queries-error-report-reasons "#scheduled-queries-error-report-reasons")
- [Location](#scheduled-queries-error-report-location "#scheduled-queries-error-report-location")
- [Format](#scheduled-queries-error-report-format "#scheduled-queries-error-report-format")
...
automatically when non-recoverable errors are encountered. These include:

- `AssumeRole` failure
- Any 4xx errors encountered when communicating with KMS when a
  customer-managed KMS key is specified
- Any 4xx errors encountered when a scheduled query runs
- Any 4xx errors encountered during ingestion of query results

For non-recoverable errors, Timestream for LiveAnalytics sends a failure notification with a
...
`Auto` indicates scheduled queries automatically
scheduled by Timestream for LiveAnalytics and `Manual` indicates scheduled queries
manually triggered by a user via `ExecuteScheduledQuery` API action in
