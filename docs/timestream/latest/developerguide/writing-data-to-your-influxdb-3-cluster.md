...
The `accept_partial` parameter controls behavior when write batches contain
errors:
...
Compatibility APIs enable seamless migration of existing InfluxDB v1 or v2 workloads to
InfluxDB 3. These endpoints work with existing InfluxDB client libraries, Telegraf, and
...
For existing v1 and v2 workloads, you can continue using legacy client libraries with the
...
- Strict type checking: Field types must remain consistent across all writes
  By leveraging the appropriate write API and following these best practices, you can
  efficiently ingest time-series data into your Timestream for InfluxDB 3 instance while maintaining
