...
| **Cloning** | You can create a new parameter group by cloning an existing one and modifying the parameters during the cloning process. This is a AWS Management Console only feature. **Note:*<br>• When cloning, `gen1-duration`, `compaction-gen2-duration`, `compaction-multipliers`, and `gen1-lookback-duration` must be copied unchanged — see [Detailed Parameter Reference](detailed-parameter-reference.md "detailed-parameter-reference.md") for details on parameters that must not change after initial setup. |
