...
The default name of the task instance, which can be modified: Invoke Lambda
Function
...
The name of the Lambda function to invoke. You can also specify the Amazon Resource Name (ARN) of
the function.
...
Either _Asynchronous execution_ or _Synchronous execution
returning the output from the function_.

### Synchronous Execution Output

#### Output Variable

The name of the variable that will contain the function output on task completion.
You can use the variable as `$(variableName)` to refer to the function
result in subsequent tasks.

#### Log Type

For synchronous execution, returns the base64-encoded last 4 KB of log data
