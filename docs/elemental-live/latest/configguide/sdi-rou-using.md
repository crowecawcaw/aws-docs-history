...
Note that you specify the input by identifying the router input, not by identifying the
SDI input. So you are saying “Use the input that is coming in on Input 1 using the Videohub
Ethernet Protocol." When you run the event, Elemental Live directs this input to a free
SDI card. Each time you run the event, a different SDI input could be used.
...
You should only use the direct inputs for inputs on the Elemental Live node that do
not connect to the router but are, indeed, still direct inputs.
