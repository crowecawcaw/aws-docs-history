...
<a name="performance-via-logs-viewing"></a>
...
<a name="performance-via-logs-messages"></a>
...
| SyncRepeat | The encoder is repeating frames.  | CPU: Not enough CPU power to encode the current frame. The current video might be very complex. Or the CPU usage might generally be at its maximum.<br />I/O: Not enough available I/O bandwidth<br />Network: Elemental Live can't ingest the input. Perhaps there are network issues. One way that Elemental Live recovers from an input failure is through repeat frames. | 
| C | A C beside a message identifies a critical error.  |  | 

## Dealing with issues
<a name="performance-via-logs-issues"></a>

The solution to dropped or repeated frames is typically to decrease density on the appliance. You might also fine tune the video quality. See [Encoding parameters that affect performance](performance-encoding-params.md).