...
| Elemental Live generates a key that it uses to encrypt the content. Elemental Live also puts that key at a customer-specified location; the client player retrieves the key from that location and decrypts the content. The end user plays the content on an iOS player. Strictly speaking, this is an encryption solution, not a DRM solution. | Self-Generated                 | Not applicable; key generated by Elemental Live | Static, Rotating |
