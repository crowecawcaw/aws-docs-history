...
2. In the body of the request, include one
   ...
   `live_event` tag. The XML is structured as
   follows.

You can configure some or all the information in the
non-running event, but you must at least set the following
tags:
...
For detailed information about the tags, see [Fields for a MOV asset](mov-set-up-event-fields.md "mov-set-up-event-fields.md").
...
myLiveEvent. The event includes a `motion_image_inserter` section that inserts the motion overlay at a specific
time:
