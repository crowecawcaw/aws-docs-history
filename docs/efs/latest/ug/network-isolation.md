...
As a managed service, Amazon Elastic File System is protected by AWS global network security. For
...
You use AWS published API calls to access Amazon EFS through the network. Clients must
...
- Cipher suites with perfect forward secrecy (PFS) such as DHE (Ephemeral
  ...
  These APIs are callable from any network location, but Amazon EFS does support resource-based access policies which can include restrictions based
  on the source IP address. You can also use Amazon EFS policies to control access from specific Amazon Virtual Private Cloud (Amazon VPC) endpoints, or
  specific VPCs. Effectively, this isolates network access to a given Amazon EFS resource from only the specific VPC within the AWS network.
