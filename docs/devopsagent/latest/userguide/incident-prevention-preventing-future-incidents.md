...
AWS DevOps Agent evaluates recent incident investigations to identify lasting improvements
to prevent future incidents and quicken the mean time to detection (MTTD).The agent analyzes
multiple incidents to identify recommendations that may prevent whole classes of incidents in the
future, focusing on the most impactful recommendations to ensure they are actionable. ​ The agent
automatically runs evaluations weekly. You can also manually trigger an evaluation at any time,
which is useful when a recent investigation warrants a quick turnaround on recommended
improvements. ​ The agent identifies improvements acrossfour areas:
