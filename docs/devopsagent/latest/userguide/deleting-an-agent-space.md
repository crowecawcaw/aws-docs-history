...
1. Open the AWS DevOps Agent console at [https://console.aws.amazon.com/aidevops/](https://console.aws.amazon.com/aidevops/ "https://console.aws.amazon.com/aidevops/").
2. Change your AWS Region to the Region where the agent space lives.
3. In the navigation pane, choose **Agent spaces**.
4. From the table, select the agent space you want to remove. The **Agent space details** page opens.
5. From **Actions**, choose **Delete agent space**.
6. In the dialog box that opens, review the information to make sure it is accurate, enter the name of the agent space to confirm, and then choose **Delete**.
...
3. From the table, search for roles with the name **DevOpsAgentRole**. Two roles are returned, with names of the form `DevOpsAgentRole-AgentSpace-<suffix>` and `DevOpsAgentRole-WebappAdmin-<suffix>`.
4. For each role in the table, select the check box for the role, choose **Delete**, then in the confirmation dialog enter the role name to confirm and choose **Delete**.
...
3. Filter by **Type: Customer managed**, and look for any policies whose name starts with `AIDevOps` or any other identifier you used during onboarding.
4. For each policy you want to remove, select the check box for the policy, choose **Actions**, and then choose **Delete** from the dropdown menu. In the dialog box that opens, review the information, enter the name of the policy to confirm, and then choose **Delete**.
...
If you removed AWS DevOps Agent resources by following the procedures above and you later want to enable AWS DevOps Agent again, see [Getting started with AWS DevOps Agent](getting-started-with-aws-devops-agent.md "getting-started-with-aws-devops-agent.md"). Keep the following in mind:
...
- [What are DevOps Agent Spaces?](about-aws-devops-agent-what-are-devops-agent-spaces.md "about-aws-devops-agent-what-are-devops-agent-spaces.md")
...
- [Getting started with AWS DevOps Agent](getting-started-with-aws-devops-agent.md "getting-started-with-aws-devops-agent.md")
