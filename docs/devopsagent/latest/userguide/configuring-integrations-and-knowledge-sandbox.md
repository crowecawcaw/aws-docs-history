...
For more information, see [Feature availability by Region](about-aws-devops-agent-supported-regions.md).
...
With Sandbox enabled, the agent reads your [DevOps Agent Skills](about-aws-devops-agent-devops-agent-skills.md) from the sandbox's filesystem rather than from a virtual filesystem. This changes how the agent works with skills in two ways:
...
1. Select your Agent Space and choose the **Capabilities** tab.

1. In the **Sandbox** section, choose **Configure**.

1. Configure a networking policy and pre-installed packages as described in the following sections.

1. Save your configuration.

After you configure Sandbox, the **Sandbox** section shows your pre-installed `pip` and `npm` packages and the number of allowlist entries. To remove the configuration later, see [Remove Sandbox](#remove-sandbox).
