...
For more information about OAuth support in AWS Sign-In, see [Sign-In with OAuth 2.0](oauth-sign-in-overview.md).
...
Grant the required permissions by attaching the [AWS managed policy](https://docs.aws.amazon.com/aws-managed-policy/latest/reference/AWSMCPSignInOAuthAccessPolicy.html):
...
| ChatGPT Connectors | https://chatgpt.com/connector\_platform\_oauth\_redirect | 
...
The steps to configure an OAuth-compatible agent vary depending on the application. For instructions for supported agents, see the [AWS MCP Server Guide](https://docs.aws.amazon.com/agent-toolkit/latest/userguide/getting-started-aws-mcp-server.html).

The general workflow is:

1. Configure the AWS MCP Server endpoint.

1. Authenticate through AWS Sign-In or obtain an OAuth access token programmatically.

1. Approve the authorization request (interactive flow only).

1. Begin invoking tools provided by AWS MCP Server.
...
1. The user authenticates using their AWS identity.

1. The user reviews and approves the authorization request.

1. AWS Sign-In issues OAuth access and refresh tokens.

1. The agent accesses AWS MCP Server on the user's behalf.
...
| --- | --- | --- | 
| AWS MCP Server | arn:aws:signin:{{region}}:{{account-id}}:service-principal/aws-mcp.amazonaws.com | Represents the AWS MCP Server. Use this resource to grant or control OAuth authorization for applications that access AWS MCP Server. | 
| OAuth public client (same-device) | arn:aws:signin:{{region}}:{{account-id}}:oauth2/public-client/localhost | Represents the AWS CLI public OAuth client for same-device authorization. Use this resource with aws login, where the browser and AWS CLI run on the same device. | 
| OAuth public client (cross-device) | arn:aws:signin:{{region}}:{{account-id}}:oauth2/public-client/remote | Represents the AWS CLI public OAuth client for cross-device authorization. Use this resource with aws login --remote, where authentication is completed on a separate device. | 
...
Additional audit events and logging details for calls made using OAuth access tokens to the AWS MCP Server can be found in the [AWS MCP Server CloudTrail logging guide](https://docs.aws.amazon.com/agent-toolkit/latest/userguide/logging-using-cloudtrail.html).
...
Verify that the condition key applies to the IAM action being evaluated. Some condition keys are supported only by specific OAuth actions. For more information, see [Sign-In with OAuth 2.0](oauth-sign-in-overview.md).
