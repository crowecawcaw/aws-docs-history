...
When you use AWS Cloud9 CLI commands or API actions to add, modify, or remove the tags that are attached to an AWS Cloud9 environment, those changes aren't automatically propagated to underlying resources such as the CloudFormation stack, the Amazon EC2 instance, and Amazon EC2 security groups. You must manually propagate those changes.
