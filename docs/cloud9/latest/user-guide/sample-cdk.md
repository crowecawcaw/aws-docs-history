...
If successful, the output contains the **`nvm`** version number, and you can skip ahead to [Step 1.2: Install Node.js](#sample-cdk-install-nodejs "#sample-cdk-install-nodejs"). 3. Download and install **`nvm`**. To do this, run the install script. In this example, v0.33.0 is
...
the version number is v16.17.0 skip ahead to [Step 1.3: Install TypeScript](#sample-cdk-install-typescript "#sample-cdk-install-typescript"). 2. Install Node.js 16 by running the **`nvm`** command with the **`install`** action.
...
terminal session is running. 4. To confirm that you're using Node.js 16 run the **`node --version`** command again. If the correct version is installed, the output
...
number. If TypeScript is installed, skip ahead to [Step 1.4: Install the AWS CDK](#sample-cdk-install-cdk "#sample-cdk-install-cdk"). 2. Install TypeScript. To do this, run the **`npm`** command with the **`install`** action, the **`-g`** option, and the name of the TypeScript package. This installs
...
numbers. Skip ahead to [Step 2: Add code](#sample-cdk-code "#sample-cdk-code"). 2. Install the AWS CDK by running the **`npm`** command along with the `install` action, the name of the
...
from the `lib/hello-cdk-stack.ts` file. 5. Use **`npm`** to run the TypeScript compiler to check for coding errors, and then
