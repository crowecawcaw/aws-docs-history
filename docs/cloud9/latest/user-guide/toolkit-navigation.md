...
| **Connect to AWS** | Connects AWS Toolkit to an AWS account using credentials that are stored in a *profile*. For more information, see [Managing access credentials for AWS Toolkit](toolkit-welcome.md#credentials-for-toolkit "toolkit-welcome.md#credentials-for-toolkit"). |
