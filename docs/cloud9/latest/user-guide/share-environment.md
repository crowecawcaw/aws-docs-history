...
A *shared environment* is an AWS Cloud9 development environment that multiple users were invited
...
* **Pair programming** (**also
 known as *peer programming***)**:** This is where two users work together on the same code in a single environment.
 ...
 *environment member access roles*.


A shared environment in AWS Cloud9 offers three environment member access roles: *owner*,
...
* The user is an **administrator user**. For more
 information, see [Creating Your First IAM Admin User and Group](https://docs.aws.amazon.com/IAM/latest/UserGuide/getting-set-up.html#create-an-admin "https://docs.aws.amazon.com/IAM/latest/UserGuide/getting-set-up.html#create-an-admin") in the
 *IAM User Guide*.
* The user is a **user who belongs to an IAM group**, a
 **user who assumes a role**, or a **federated user who assumes a role**, *and*
 ...
 Policies](security-iam.md#auth-and-access-control-managed-policies "security-iam.md#auth-and-access-control-managed-policies").
	...
	+ You can create a role in IAM with one of the preceding managed policies
	 ...
	 Roles](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create.html "https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create.html") in the *IAM User Guide*. To have a user
	 or a federated user assume the role, see coverage of assuming roles in [Using IAM
	 Roles](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_use.html "https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_use.html") in the *IAM User Guide*.
...
2. Open the IAM console. To do this, in the console navigation bar, choose
...
(If you don't see one of these policy names in the list, type the policy name
 in the **Search** box to display it.)
...
1. Suppose that the user that you want to invite *isn't* one of the following types of users. Make sure the user that you
 ...
 Member Access Roles](#share-environment-member-roles "#share-environment-member-roles").
	...
	* A **user who belongs to an IAM group**, a
	 ...
	 *and* that group or role has the AWS managed policy
...
Removing your permanent AWS access credentials from your environment and using
 ...
 take actions in AWS from the environment on your behalf.
7. Contact the user to let them know they can open this environment and begin using
 it.
...
* The **AWS account root user**.
* An **administrator user**.
* A **user who belongs to an IAM group**, a **user who assumes a role**, or a **federated user who assumes a role**, *and* that group
 or role has the AWS managed policy `AWSCloud9Administrator`
 attached.

Suppose that the invited user *isn't* one of the
