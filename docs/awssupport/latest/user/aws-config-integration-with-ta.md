...
**Contents**
+ [I just enabled recording and managed rules for AWS Config, but I don’t see corresponding Trusted Advisor checks.](#ta-checks-not-appearing)
+ [I deployed the same AWS Config managed rule twice, what will I see in Trusted Advisor?](#config-managed-rule-deployed-twice)
+ [I turned off recording for AWS Config in an AWS Region. What will I see in Trusted Advisor?](#turned-off-config-recording)
...
After the AWS Config rule generates evalution results, you see the results in Trusted Advisor in near real-time. If you have issues with this feature, create a technical support case in the [AWS Support Center](https://console.aws.amazon.com/support/home#/).
