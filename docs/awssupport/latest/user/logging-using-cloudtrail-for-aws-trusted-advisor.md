...
calls captured include calls from the Trusted Advisor console. If you create a trail, you can enable
continuous delivery of CloudTrail events to an Amazon Simple Storage Service (Amazon S3) bucket, including events for Trusted Advisor.
...
the request that was made to Trusted Advisor, the IP address from which the request was made, who made
...
activity occurs in the Trusted Advisor console, that activity is recorded in a CloudTrail event along with
...
For an ongoing record of events in your AWS account, including events for Trusted Advisor,
...
Trusted Advisor supports logging a subset of the Trusted Advisor console actions as events in CloudTrail log
files. CloudTrail logs the following actions:
...
For a complete list of Trusted Advisor console actions, see[Trusted Advisor actions](security-trusted-advisor.md#trusted-advisor-operations "security-trusted-advisor.md#trusted-advisor-operations").

###### Note

CloudTrail also logs the Trusted Advisor API operations in the [AWS Support API Reference](../APIReference.md "../APIReference.md"). For more information,
see[Logging AWS Support API calls with AWS CloudTrail](logging-using-cloudtrail.md "logging-using-cloudtrail.md").
...
`RefreshCheck`
action for the Amazon S3 Bucket Versioning check (ID
R365s2Qddf).
...
The following example shows a CloudTrail log entry that demonstrates the
`GenerateReport`
action. This action creates a report for your AWS
organization.
