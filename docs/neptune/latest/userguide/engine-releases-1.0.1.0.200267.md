...
Amazon Neptune 1.0.1.0.200267.0 is generally available. All new Neptune DB clusters, including
those restored from snapshots, will be created in Neptune 1.0.1.0.200267.0 after the engine update
...
Neptune DB clusters will automatically be upgraded to engine release 1.0.1.0.200267.0 during
...
- Neptune waits longer (within the query timeout specified)
  for any conflicts to get resolved. This reduces the number of concurrent
  modification exceptions that need to be handled by the client
  (see [Query Errors](errors-engine-codes.md#errors-query "errors-engine-codes.md#errors-query")).
- Fixed an issue where Gremlin cardinality enforcement sometimes
  caused the engine to restart.
- Improved Gremlin performance for `emit.times` repeat queries.
- Fixed a Gremlin issue where `repeat.until` was allowing `.emit`
  solutions through that should have been filtered.
