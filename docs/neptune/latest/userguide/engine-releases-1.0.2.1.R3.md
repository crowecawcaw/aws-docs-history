...
+ Fixed a bug where the `OSGP` index was disabled even though the feature was correctly enabled using in[Lab Mode](features-lab-mode.md) using the `ObjectIndex` value in the `neptune_lab_mode` parameter.
