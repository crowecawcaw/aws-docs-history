...
With the AWS SDK, you can run openCypher queries against your Neptune graph using
a programming language of your choice. The Neptune data API SDK (service name
`neptunedata`) provides the
[ExecuteOpenCypherQuery](../data-api/API_ExecuteOpenCypherQuery.md "../data-api/API_ExecuteOpenCypherQuery.md")
action for submitting openCypher queries.
...
run an openCypher query, and print the results. Replace
...
2. Use the following code to set up a `NeptunedataClient`, run an
   openCypher query, and print the result:
