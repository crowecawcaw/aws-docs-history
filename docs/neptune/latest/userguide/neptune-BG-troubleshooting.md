...
+ **`Cluster with id = {{(blue_green_deployment_id)}} already exists`**   –   There is an existing cluster with identifier {{(blue\_green\_deployment\_id)}}.
...
+ **`Streams should be enabled on the source Cluster for Blue Green Deployment`**   –   Enable [Neptune streams](streams-using-enabling.md) on the blue (source) cluster.
