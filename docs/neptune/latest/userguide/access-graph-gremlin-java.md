...
These terminal steps behave differently depending on whether you submit queries as strings (script mode) or as bytecode traversal objects (bytecode mode). The differences are especially important for transaction handling. For details, see [Test Gremlin code in the context where you will deploy it](best-practices-gremlin-console-glv-differences.md).
