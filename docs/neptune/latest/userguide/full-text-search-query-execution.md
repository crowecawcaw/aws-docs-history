...
You can prevent this re-ordering of query-step execution using the [noReordering query hint](gremlin-query-hints-noReordering.md):
...
Here again, Neptune executes the `SERVICE` part of the query first, and then joins the results with the `Person` data. You can suppress this behavior using the [joinOrder query hint](sparql-query-hints-joinOrder.md):
