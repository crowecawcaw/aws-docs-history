...
+ Bug fix for Gremlin queries with an optional traversal, e.g., for queries of the form `g.V().hasLabel('person').group().by(id()).by(\_\_.in('friend').id().fold())`, where no persons without friend edges got grouped.
