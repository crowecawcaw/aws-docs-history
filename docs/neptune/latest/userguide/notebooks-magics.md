...
+ [%load\_ids](#notebooks-line-magics-load-ids)
+ [%load\_status](#notebooks-line-magics-load-status)
+ [%cancel\_load](#notebooks-line-magics-cancel-load)
+ [%status](#notebooks-line-magics-status)
+ [%gremlin\_status](#notebooks-line-magics-gremlin-status)
+ [%opencypher\_status, or %oc\_status](#notebooks-line-magics-opencypher-status)
+ [`%stream_viewer`](#notebooks-line-magics-stream-viewer)
+ [%sparql\_status](#notebooks-line-magics-sparql-status)
+ [%graph\_notebook\_config](#notebooks-line-magics-graph-notebook-config)
+ [%graph\_notebook\_host](#notebooks-line-magics-graph-notebook-host)
+ [%graph\_notebook\_version](#notebooks-line-magics-graph-notebook-version)
+ [%graph\_notebook\_service](#notebooks-line-magics-graph-notebook-service)
+ [%graph\_notebook\_vis\_options](#notebooks-line-magics-graph-notebook-vis-options)
+ [%statistics](#notebooks-line-magics-statistics)
+ [%summary](#notebooks-line-magics-summary)
+ [%reset\_graph](#notebooks-line-magics-reset-graph)
+ [%get\_graph](#notebooks-line-magics-get-graph)
...
+ [%%graph\_notebook\_config](#notebooks-cell-magics-graph-notebook-config)
+ [%%graph\_notebook\_vis\_options](#notebooks-cell-magics-graph-notebook-vis-options)

There are also two magics, a line magic and a cell magic, for working with [Neptune machine learning](machine-learning.md):
+ [%neptune\_ml](#notebooks-line-magics-neptune_ml)
+ [%%neptune\_ml](#notebooks-cell-magics-neptune_ml)
...
Property values for these arguments can consist either of a single property key, or of a JSON string that can specify a different property for each label type. A JSON string can only be specified using [variable injection](#notebook-magics-variable-injection).
...
The `%load` line magic generates a form that you can use to submit a bulk load request to Neptune (see [Neptune Loader Command](load-api-reference-load.md)). The source must be an Amazon S3 path in the same region as the Neptune cluster.

## The `%load_ids` line magic
<a name="notebooks-line-magics-load-ids"></a>

The `%load_ids` line magic retrieves the load Ids that have been submitted to the notebook's host endpoint (see [Neptune Loader Get-Status request parameters](load-api-reference-status-requests.md#load-api-reference-status-parameters)). The request takes this form:
...
The `%load_status` line magic retrieves the load status of a particular load job that has been submitted to the notebook's host endpoint, specified by the line input (see [Neptune Loader Get-Status request parameters](load-api-reference-status-requests.md#load-api-reference-status-parameters)). The request takes this form:
...
The `%cancel_load` line magic cancels a particular load job (see [Neptune Loader Cancel Job](load-api-reference-cancel.md)). The request takes this form:
...
Retrieves [Gremlin query status information](gremlin-api-status.md).
...
Retrieves [SPARQL query status information](sparql-api-status.md).
...
You can change the configuration by copying the `%graph_notebook_config` output into a new cell and make changes to it there. Then if you run the [%%graph\_notebook\_config](#notebooks-cell-magics-graph-notebook-config) cell magic on the new cell, the configuration will be changed accordingly.
...
The `%graph_notebook_vis_options` line magic displays the current visualization settings that the notebook is using. These options are explained in the [vis.js](https://visjs.github.io/vis-network/docs/network/#options) documentation.
...
The `%statistics` line magic is used to retrieve or manage DFE engine statistics (see [Managing statistics for the Neptune DFE to use](neptune-dfe-statistics.md)). This magic can also be used to retrieve a [graph summary](neptune-graph-summary.md).

It accepts the following parameters:
+ **`--language`**   –   The query language of the statistics endpoint: either or `propertygraph` (or `pg`) or `rdf`.
...
The `%summary` line magic is used to retrieve [graph summary](neptune-graph-summary.md) information. It is available starting with Neptune engine version `1.2.1.0`.
...
Example of a visualization query with an `--expand-all` visualization hint parameter (see [SPARQL visualization](notebooks-visualization.md#notebooks-visualization-sparql)):
...
The `%%gremlin` cell magic issues a Gremlin query to the Neptune endpoint using WebSocket. It accepts an optional line input to toggle into [Gremlin `explain`](gremlin-explain.md) /> mode or [Gremlin `profile` API](gremlin-profile-api.md), and a separate optional visualization hint input to modify visualization output behavior (see [Gremlin visualization](notebooks-visualization.md#notebooks-visualization-Gremlin)).
...
+ **`--profile-chop`**   –   Specifies the maximum length of the profile results string. The default value if you don't supply this argument is 250.
+ **`--profile-serializer`**   –   Specifies the serializer to use for the results. Allowed values are any of the valid MIME type or TinkerPop driver "Serializers" enum values. The default value if you don't supply this argument is `application.json`.
+ **`--profile-no-results`**   –   Displays only the result count. If not used, all query results are displayed in the profile report by default.
+ **`--profile-indexOps`**   –   Shows a detailed report of all index operations.
...
+  **`--explain-type`**  –   Used to specify the explain mode to use (one of: dynamic, static, debug, or details). 
...
+ **`--query-parameters`** or **`qp`**   –   [Parameter definitions](opencypher-parameterized-queries.md) to apply to the query. This option can either accept a single variable name, or a string representation of the map.
...
**Example usage of `--query-parameters`**
  ...
  1.  Pass the parameters into `--query-parameters` in another cell with `%%oc`. 
...
The `%%graph_notebook_config` cell magic uses a JSON object containing configuration information to modify the settings that the notebook is using to communicate with Neptune, if possible. The configuration takes the same form returned by the [%graph\_notebook\_config](#notebooks-line-magics-graph-notebook-config) line magic.
...
These options are explained in the [vis.js](https://visjs.github.io/vis-network/docs/network/#options) documentation.
...
**Note**  
You can also initiate and manage some Neptune ML operations using the [%%neptune\_ml](#notebooks-cell-magics-neptune_ml) cell magic.
...
**Parameters**
  ...
  + **`--instance-type`** {{(instance type)}}   –   (*optional*) The instance size to use for this data-processing job.
...
For example, the `%%neptune_ml export start` cell magic can consume a JSON document in the cell body that contains all the parameters described in [Parameters used to control the Neptune export process](export-parameters.md).

In the [Neptune-ML-01-Introduction-to-Node-Classification-Gremlin](https://github.com/aws/graph-notebook/blob/main/src/graph_notebook/notebooks/04-Machine-Learning/Neptune-ML-01-Introduction-to-Node-Classification-Gremlin.ipynb) notebook, under **Configuring Features** in the **Export the data and model configuration** section, you can see how the following cell holds export parameters in a document assigned to a Jupyter variable named `export-params`:
