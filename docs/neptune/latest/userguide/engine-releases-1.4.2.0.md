...
As of 2024-12-19, engine version 1.4.2.0 is being generally deployed. Please note that it takes several days for a new release to become available in every region.
...
**General fixes**
+  Fixed execution plan and performance for queries with multiple property accesses over the same variable inside OPTIONAL MATCH and list comprehension. For example: 
...
Before upgrading a DB cluster to version 1.4.2.0, make sure that your project is compatible with these query-language versions:
