...
# Amazon Neptune Engine version 1.4.6.2 (2025-11-18)
<a name="engine-releases-1.4.6.2"></a>

As of 2025-11-18, engine version 1.4.6.2 is being generally deployed. Please note that it takes several days for a new release to become available in every region.

## Defects fixed in this engine release
<a name="engine-releases-1.4.6.2-defects"></a>

**General fixes**
+  Some exceptions that were erroneously classified as Internal Failure Exceptions (IFEs) are now correctly reported as Concurrent Modification Exceptions (CMEs) during the Data Flow Engine (DFE) execution. 

**Gremlin fixes**
+  Improved the engine stability for Gremlin over DFE 

**openCypher fixes**
+  Fixed an issue introduced in Neptune engine release 1.4.6.0 where openCypher MERGE queries incorrectly handled incoming relationship directions (←). Queries using patterns like MERGE (n)←[:type]-(m) now create relationships with the correct direction. 
+  Fixed a failed commit operation on the writer transactions that were not properly rolled back. 
+  Optimized query plans are now supported for MERGE queries that reference parametrized values. 

## Query-Language Versions Supported in This Release
<a name="engine-releases-1.4.6.2-query-versions"></a>

Before upgrading a DB cluster to version 1.4.6.2, make sure that your project is compatible with these query-language versions:
+ *Gremlin earliest version supported:* `3.7.1`
+ *Gremlin latest version supported:* `3.7.1`
+ *openCypher version:* `Neptune-9.0.20190305-1.0`
+ *SPARQL version:* `1.1`

## Upgrade paths to engine release 1.4.6.2
