...
1. In the navigation pane, choose **Clusters**, and then choose the DB cluster that you want to modify. 

1. Choose **Actions**, and then choose **Modify cluster**. The **Modify DB cluster** page appears.

1. Change any of the settings that you want.

   For example, you can change the **Network type** to **Dual-stack** to enable IPv6 support for the DB cluster. For more information, see [Dual-stack mode](neptune-dualstack-db-cluster.md).
**Note**  
 On the console, some instance level changes only apply to the current DB instance, whereas others apply to the entire DB cluster. To change a setting that modifies the entire DB cluster at the instance level on the console, follow the instructions in [Modifying a DB Instance in a DB Cluster](#manage-console-modify-instance).

1. When all the changes are as you want them, choose **Continue** and check the summary. 
...
1. Change any of the settings that you want.
**Note**  
Some settings apply to the entire DB cluster and must be changed at the cluster level. To change those settings, follow the instructions in [Modifying a Neptune DB Cluster Using the Console](#manage-console-modify).   
