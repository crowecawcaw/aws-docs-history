...
As of 2025-06-30, engine version 1.4.5.1 is being generally deployed. Please note
...
- Fixed an issue where an internal error was returned on some OpenCypher queries when running
  `collect(distinct())` on a list.
- Fixed an issue where a concurrent FTS query causes other concurrent FTS queries to be terminated
  when using OpenSearch serverless.

## Query-Language Versions Supported in This Release

Before upgrading a DB cluster to version 1.4.5.1, make sure that your project is compatible
