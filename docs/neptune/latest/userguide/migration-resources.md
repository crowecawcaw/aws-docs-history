...
- [neo4j-to-neptune](https://github.com/awslabs/amazon-neptune-tools/tree/master/neo4j-to-neptune "https://github.com/awslabs/amazon-neptune-tools/tree/master/neo4j-to-neptune") –
  ...
  - Perform some basic conversions of data during the conversion to Neptune format,
    such as renaming vertex or edge labels and generating elements.
  - Generate properties for nodes and edges using templates (for example, create an
    `~id` value using a template such as `Person_{personid}` for situations
    where you need to create the unique identifier for an element).
