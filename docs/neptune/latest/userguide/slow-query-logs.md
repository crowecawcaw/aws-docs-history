...
**Note**  
Slow-query logging was introduced in Neptune [engine release 1.2.1.0](engine-releases-1.2.1.0.md).

You enable slow-query logging using the [neptune\_enable\_slow\_query\_log](parameters.md#parameters-db-cluster-parameters-neptune_enable_slow_query_log) DB cluster parameter. By default, this parameter is set to `disabled`. Setting it to `info` or `debug` enables slow-query logging. The `info` setting logs a few useful attributes of each slow-running query, whereas the `debug` setting logs all available attributes.

To set the threshold for what is considered a slow-running query, use the [neptune\_slow\_query\_log\_threshold](parameters.md#parameters-db-cluster-parameters-neptune_slow_query_log_threshold) DB cluster parameter to specify the number of milliseconds after which a running query is considered slow and is logged when slow-query logging is enabled. The default value is 5000 milliseconds (5 seconds).

You can set these DB cluster parameters [in the AWS Management Console](parameter-groups.md#parameters-editgroup), or using the [modify-db-cluster-parameter-group](https://docs.aws.amazon.com/cli/latest/reference/neptune/modify-db-cluster-parameter-group.html) AWS CLI command, or the [ModifyDBClusterParameterGroup](api-parameters.md#ModifyDBClusterParameterGroup) management function.
...
- ****requestResponseMetadata****
  - **Attribute:** `requestId` / **Description:** Request id of query.
  - **Attribute:** `requestType` / **Description:** Request type, like HTTP or WebSocket.
  - **Attribute:** `responseStatusCode` / **Description:** Query response status code, like 200.
  - **Attribute:** `exceptionClass` / **Description:** Exception class of the error returned after query execution.

- ****queryStats****
  - **Attribute:** `query` / **Description:** Query string.
  - **Attribute:** `queryFingerprint` / **Description:** Fingerprint of the query.
  - **Attribute:** `queryLanguage` / **Description:** Query language, like Gremlin, SPARQL or openCypher.

- ****memoryStats****
  - **Attribute:** `allocatedPermits` / **Description:** Permits allocated to the query.
  - **Attribute:** `approximateUsedMemoryBytes` / **Description:** Approximate memory used by the query during execution.

- ****queryTimeStats****
  - **Attribute:** `startTime` / **Description:** Query start time (UTC).
  - **Attribute:** `overallRunTimeMs` / **Description:** Query total run time, in milliseconds.
  - **Attribute:** `parsingTimeMs` / **Description:** Query parsing time, in milliseconds.
  - **Attribute:** `waitingTimeMs` / **Description:** Query Gremlin/SPARQL/openCypher queue waiting time, in milliseconds
  - **Attribute:** `executionTimeMs` / **Description:** Query execution time, in milliseconds.
  - **Attribute:** `serializationTimeMs` / **Description:** Query serialization time, in milliseconds.
  - **Attribute:** `authTimeMs` / **Description:** Query authentication and authorization time, in milliseconds.

- ****statementCounters****
  - **Attribute:** `read` / **Description:** Number of statements read.
  - **Attribute:** `written` / **Description:** Number of statements written.
  - **Attribute:** `deleted` / **Description:** Number of statements deleted.

- ****transactionCounters****
  - **Attribute:** `committed` / **Description:** Number of transactions commited.
  - **Attribute:** `rolledBack` / **Description:** Number of transactions rolled back.

- ****vertexCounters****
  - **Attribute:** `added` / **Description:** Number of vertices added.
  - **Attribute:** `removed` / **Description:** Number of vertices removed.
  - **Attribute:** `propertiesAdded` / **Description:** Number of vertex properties added.
  - **Attribute:** `propertiesRemoved` / **Description:** Number of vertex properties removed.

- ****edgeCounters****
  - **Attribute:** `added` / **Description:** Number of edges added.
  - **Attribute:** `removed` / **Description:** Number of edges removed.
  - **Attribute:** `propertiesAdded` / **Description:** Number of edge properties added.
  - **Attribute:** `propertiesRemoved` / **Description:** Number of edge properties removed.

- ****resultCache****
  - **Attribute:** `hitCount` / **Description:** Result cache hit count.
  - **Attribute:** `missCount` / **Description:** Result cache miss count.
  - **Attribute:** `putCount` / **Description:** Result cache put count.

- ****concurrentExecutionStats****
  - **Attribute:** `acceptedQueryCountAtStart` / **Description:** Parallel queries accepted with the current query execution at start.
  - **Attribute:** `runningQueryCountAtStart` / **Description:** Parallel queries running with the current query execution at start.
  - **Attribute:** `acceptedQueryCountAtEnd` / **Description:** Parallel queries accepted with the current query execution at end.
  - **Attribute:** `runningQueryCountAtEnd` / **Description:** Parallel queries running with the current query execution at end.

- ****queryBatch****
  - **Attribute:** `queryProcessingBatchSize` / **Description:** Batch size during query processing.
  - **Attribute:** `querySerialisationBatchSize` / **Description:** Batch size during query serialization.
