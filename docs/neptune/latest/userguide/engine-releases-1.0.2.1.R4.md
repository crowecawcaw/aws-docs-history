...
+ Neptune now tries always to place any full-text-search call first in the execution pipeline. This reduces the volume of calls to OpenSearch, which can significantly improve performance. See [Full-text-search query execution](full-text-search-query-execution.md).
