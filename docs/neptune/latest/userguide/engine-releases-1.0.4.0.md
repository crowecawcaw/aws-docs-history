...
+ [Release: 1.0.4.0.R2 (2021-02-24)](engine-releases-1.0.4.0.R2.md)
...
A bug in this release has allowed HTTP connections and/or outdated TLS connections to continue to work for customers who previously set a DB cluster parameter to prevent enforcement of HTTPS connections. That bug was fixed in patch releases [1.0.4.0.R2](engine-releases-1.0.4.0.R2.md) and [1.0.4.1.R2](engine-releases-1.0.4.1.R2.md), but the fix has caused unexpected connection failures when the patches are automatically installed.  
...
+ Upgraded TinkerPop to version 3.4.8. This is a backwards compatible upgrade. See the [TinkerPop change log](https://github.com/apache/tinkerpop/blob/master/CHANGELOG.asciidoc#tinkerpop-348-release-date-august-3-2020) for what's new.
...
+ Added a new `allowEmptyStrings` setting in the bulk loader's `parserConfiguration` parameter that allows empty strings to be treated as valid property values in CSV loads (see [Neptune Loader Request Parameters](load-api-reference-load.md#load-api-reference-load-parameters)).
