...
To perform a write operations, you must connect to the DB cluster endpoint of the primary DB cluster. Only the primary cluster can perform write operations. Then, as shown in the diagram above, replication is performed by the [cluster storage volume](feature-overview-storage.md), not the database engine.
...
+ To recover from an outage in the primary region, use the [manual unplanned detach-and-promote](neptune-gdb-disaster-recovery.md#neptune-gdb-detach-and-promote) process, where you detach one the secondary clusters, turning it into a standalone cluster, and then promote it to be the new primary cluster.
+ For planned operational procedures such as maintenance, use [managed planned failover](neptune-gdb-disaster-recovery.md#neptune-gdb-managed-failover), where you relocate the primary cluster to one of its secondary regions with no data loss.
