...
**Contents**
+ [Things to keep in mind about restoring a Neptune DB cluster from a snapshot](#backup-restore-restore-snapshot-considerations)
  + [You cannot restore to an existing DB cluster](#backup-restore-restores-to-new-cluster)
  + [No instances are restored](#backup-restore-restore-no-restored-instances)
  + [No custom parameter group is restored](#backup-restore-restore-default-parameters)
  + [No custom security groups are restored](#backup-restore-restore-default-security-group)
  + [You can't restore from a shared encrypted snapshot](#backup-restore-restore-not-shared-and-encrypted)
  + [A restored DB cluster uses as much storage as before](#backup-restore-restore-same-storage-allocation)
+ [How to restore from a snapshot](#backup-restore-restore-snapshot-restoring)
  + [Using the Console to Restore from a Snapshot](#backup-restore-restore-snapshot-restoring-console)
...
As soon as the restore is complete and your new DB cluster is available, explicitly create the instances that you will need. You can do this on the Neptune console, or using the [CreateDBInstance](api-instances.md#CreateDBInstance) API.
...
As soon as the restore is complete and your new DB cluster is available, associate any custom DB parameter group that the instance you restored from was using. To do that, use the **Modify** command on the Neptune console, or the [ModifyDBInstance](api-instances.md#ModifyDBInstance) API.
...
As soon as the restore is complete and your new DB cluster is available, associate any custom security groups that the instance you restored from was using. To do that, use the **Modify** command on the Neptune console, or the [ModifyDBInstance](api-instances.md#ModifyDBInstance) API.
...
For more information about handling encryption when copying a snapshot, see [Handling Encryption When Copying Snapshots](backup-restore-copy-snapshot.md#backup-restore-copy-snapshot-encryption). For more information about sharing encrypted snapshots, see [Sharing an Encrypted DB Cluster Snapshot](backup-restore-share-snapshot.md#backup-restore-share-snapshot-encrypted).
...
1. Choose **Actions**, **Restore Snapshot**. 

1. On the **Restore DB Instance** page, in the **DB Instance Identifier** box, enter the name for your restored DB cluster. 

1. Choose **Restore DB Instance**. 

1. If you want to restore the functionality of the DB cluster to that of the DB cluster that the snapshot was created from, you must modify the DB cluster to use the security group. The next steps assume that your DB cluster is in a virtual private cloud (VPC). If your DB cluster is not in a VPC, use the Amazon EC2 console to locate the security group that you need for the DB cluster. 

   1. Open the Amazon VPC console at [https://console.aws.amazon.com/vpc/](https://console.aws.amazon.com/vpc/).

   1. In the navigation pane, choose **Security Groups**. 

   1. Choose the security group that you want to use for your DB clusters. If necessary, add rules to link the security group to a security group for an EC2 instance.