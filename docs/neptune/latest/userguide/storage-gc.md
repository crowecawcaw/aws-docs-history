...
<a name="storage-gc"></a>
 ...
 Neptune supports dictionary garbage collection (GC) which can be enabled via the `neptune_lab_mode` [parameter](parameters.md) for property graph data. The feature requires a writer instance reboot to activate. This feature is available from engine release [1.4.3.0](https://docs.aws.amazon.com/releases/release-1.4.3.0.xml). 

**Compatibility with engine releases before 1.4.8.0**  
 Prior to engine release 1.4.8.0, you could enable dictionary GC only on clusters without `neptune_streams` enabled. Neptune automatically disabled the feature if `neptune_streams` was enabled. Neptune also disabled the feature if any unexpired `neptune_streams` data existed. Starting with engine release 1.4.8.0, this restriction no longer applies, and you can enable dictionary GC on clusters that have `neptune_streams` enabled. 
 ...
 GC can be run in two modes, `soft_delete` and `enabled`. When run in the `soft_delete` mode, unused dictionary entries are marked deleted (soft\_delete) but are not explicitly deleted. This mode could also be used to understand performance characteristics after the background operation is turned on. When the `enabled` mode is used, entries are explicitly deleted ('hard' delete). It is recommended to run GC in `soft_delete` mode for a period of time before switching to `enabled` mode. 
