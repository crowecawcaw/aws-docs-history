...
It may take up to 4 weeks to provision and prepare the device for your job before it is shipped. This timeline should be factored into your project plan to ensure a seamless transition. While AWS is preparing your device to ship, you can monitor the status of your job through the AWS Snow Family Management Console. For more information, see [Statuses of Snowball Edge jobs](jobstatuses.md).
...
For more information on choosing shipping speed and entering your shipping address when creating a job to order a Snowball Edge device, see [Choosing security, shipping, and notification preferences](create-job-common.md#security-shipping-notification). For more information about returning a Snowball Edge device to AWS, see [Returning the Snowball Edge device](return-device.md).
...
<a name="shipwithinregion"></a>

Before you create a job to order a Snowball Edge device, you should sign in to the console from the same AWS Region as your Amazon S3 data. AWS does not ship Snowball Edge between countries within the same AWS Region—for example, from Asia Pacific (India) to Asia Pacific (Australia).
