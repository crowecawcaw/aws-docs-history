...
The following are storage quotas and shared resource limitations for compute resources
on an AWS Snowball Edge device.

## Compute resource storage quotas on Snowball Edge

The storage available for compute resources is a separate resource from the
dedicated Amazon S3 storage on a Snowball Edge device. The quotas for storage are as
follows:
