...
| Canceled                              | `Cancelled`              | Your job has been canceled.                                                                                                                                                    | ## Statuses of Snowball Edge cluster jobs Each cluster has a _status_, which changes to denote the current general progress state of the cluster. Each individual node of the cluster has its own job status. This cluster status information doesn't reflect the health, the current processing state, or the storage used for the cluster or its nodes. |
