...
<a name="WF_RANK"></a>
 ...
 RANK differs from the [DENSE\_RANK window function](WF_DENSE_RANK.md) in one respect: For DENSE\_RANK, if two or more rows tie, there is no gap in the sequence of ranked values. For example, if two rows are ranked 1, the next rank is 2.
