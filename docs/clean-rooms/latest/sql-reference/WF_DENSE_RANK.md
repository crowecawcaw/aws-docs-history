...
The following example orders the table by the quantity sold (in descending order),
and assign both a dense rank and a regular rank to each row. The results are sorted
...
the quantity (in descending order) and assign a dense rank to each row. The results are
