...
Note the difference in rankings assigned to the same set of rows when the DENSE\_RANK and RANK functions are used side by side in the same query. For a description of the WINSALES table, see [Sample table for window function examples](Window_functions.md#Window_function_example).
