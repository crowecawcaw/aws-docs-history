...
Given an ordered set of rows, The LAST\_VALUE function returns the value of the
expression with respect to the last row in the frame.

For information about selecting the first row in the frame, see [FIRST\_VALUE window function](WF_first_value.md "WF_first_value.md") .
...
with the results ordered by capacity (high to low). The LAST\_VALUE function is used to
