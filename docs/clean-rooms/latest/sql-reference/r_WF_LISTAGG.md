...
LISTAGG is a compute-node only function. The function returns an error if the query
doesn't reference a user-defined table or AWS Clean Rooms system table.
...
uses the first value encountered. For more information, see [Significance of
...
(Optional) The string constant to separate the concatenated values. The
default is NULL.

AWS Clean Rooms supports any amount of leading or trailing whitespace around an
optional comma or colon as well as an empty string or any number of
spaces.
...
Deterministic only if ORDER BY provides unique ordering. The default is to
aggregate all rows and return a single value.
...
table, see [Sample table for window function
...
The following example uses DISTINCT to return a list of unique sales dates for buyer
B.
