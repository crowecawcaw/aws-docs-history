...
The NTILE window function divides ordered rows in the partition into the specified
number of ranked groups of as equal size as possible and returns the group that a given row
falls into.
...
The number of ranking groups and must result in a positive integer value
(greater than 0) for each partition. The _expr_ argument
...
The following example ranks into four ranking groups the price paid for Hamlet
tickets on August 26, 2008. The result set is 17 rows, divided almost evenly among the
rankings 1 through 4:
