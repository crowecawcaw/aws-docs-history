...
| --- | --- | --- | --- | 
| Control mechanism  | Control how data in the table can be used in a query*(For example, allow COUNT and SUM of column hashed\_email.)* | Control how data in the table can be used in a query*(For example, allow use of column hashed\_email only for joining.)* | Control what queries are allowed to run on the table*(For example, allow only queries defined in analysis templates "Custom query 1".)* | 
| Built-in privacy enhancing techniques  |  +  Blind match  <br />+  Aggregation required  <br />+  Min aggregation threshold >=  <br />+  2 Pre-defined query structure   |  +  Blind match <br />+  Overlap required <br />+  Pre-defined query structure <br />+  Allowed additional analyses   |  +  [Minimum aggregation thresholds](custom-min-agg-thresholds.md) <br />+  [Comparison controls](custom-comparison-controls.md) <br />+  [Disallowed output columns](disallowed-output-columns.md) <br />+  [Differential privacy](differential-privacy.md#dp-overview)   | 
