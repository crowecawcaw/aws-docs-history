...
3. On the **With active membership** tab, choose a
   collaboration.
4. On the **ML models** tab, under **Ready-to-use
   lookalike models**, choose **Associate lookalike
   model**.
5. On the **Associate configured lookalike model** page, for
   **Configured lookalike model association details**:
   ...
   The description helps differentiate between other associated
   configured audience models with similar names.

6. For **Configured lookalike model**, choose a configured
   lookalike model from the dropdown list.
7. Choose **Associate**.
   For the corresponding API action, see [CreateConfiguredAudienceModelAssociation](../apireference/API_CreateConfiguredAudienceModelAssociation.md "../apireference/API_CreateConfiguredAudienceModelAssociation.md").
