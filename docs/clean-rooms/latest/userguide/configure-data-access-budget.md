...
A collaborator can view, add, edit, and delete a _data access
budget_ to set a limit on the number of times a table can be used in queries,
PySpark jobs, and ML jobs. Use these budgets to manage data and costs.

Each time a table is queried, a PySpark job is run on the table, or an ML job is run using
an ML input channel derived from a table, the budget for that table is reduced by one. When the
budget reaches zero, you can't run queries, PySpark jobs, or ML jobs on that table.
