...
1. On the **Configured lookalike models** tab, choose **Configure lookalike model**.

1. On the **Configure lookalike model** page, for **Configured lookalike model details**, enter a **Name** and optional **Description**.

   1. Choose the **Lookalike model** that you want to configure from the dropdown list.
**Note**  
To verify that this is the correct lookalike model, turn on **Show lookalike model details** to view the details.  
To create a new lookalike model, choose **Create lookalike model**.
...
1. For **Lookalike segment destination location**, enter the Amazon S3 bucket where the lookalike segment is exported. This bucket must be located in the same region as your other resources.

1. For **Service access**, choose the **Existing service role name** that will be used to access this table.

1. For **Advanced bin size configuration**, specify the **Audience size type** as either an **Absolute** number or a **Percentage**.

1. If you want to enable **Tags** for the configured table resource, choose **Add new tag** and then enter the **Key** and **Value** pair. 

1. Choose **Configure lookalike model**. 

For the corresponding API action, see [CreateConfiguredAudienceModel](https://docs.aws.amazon.com/cleanrooms-ml/latest/APIReference/API_CreateConfiguredAudienceModel.html).