...
The following sections describe the console commands for converting a `secret.key` file to base64 and storing it as an environment variable. The `secret.key` file could have been generated from any of the commands listed in [Step 5: Create a shared secret key](create-SSK.md) and is only an example source. 
