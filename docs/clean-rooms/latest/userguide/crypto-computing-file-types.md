...
**Topics**
+ [CSV files](#csv-file-type)
+ [Parquet files](#parquet-file-type)
+ [Encrypting non-string values](#encrypt-non-string-values)
...
A positional encryption schema specifies output columns by position instead of by name. A mapped encryption schema maps source column names to target column names. For more information, including a detailed discussion and examples of both schema formats, see [Mapped and positional table schemas](create-schema.md#mapped-and-positional-schemas).
...
+ `Fixed_Len_Binary_Array` primitive type with the `Decimal(scale, precision)` logical annotation (`DECIMAL` data type)
