...
<a name="network-components-members"></a>

An AWS account initially creates a Hyperledger Fabric network on AMB Access, but the network is not owned by that AWS account or any other AWS account. The network is decentralized, so changes to the network are made by consensus.
...
## Peer Nodes
<a name="network-components-peer-nodes"></a>
...
<a name="network-components-connecting"></a>
...
Each network resource has a unique, addressable endpoint that AMB Access creates from these IDs. Other members of the network, Hyperledger Fabric chaincode, and other tools use these endpoints to identify and interact with resources on the network.
