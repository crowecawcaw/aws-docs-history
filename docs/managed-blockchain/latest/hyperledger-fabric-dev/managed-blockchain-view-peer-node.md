...
You can also view and monitor **Metrics** related to peer node performance. For more information, see [Use Hyperledger Fabric Peer Node Metrics on Amazon Managed Blockchain (AMB)](managed-blockchain-peer-node-metrics.md "managed-blockchain-peer-node-metrics.md").

You can check
the peer node status using the `get-node` command, as shown in the following
example:
...
need this endpoint and port when communicating with the node using your blockchain
framework client or addressing the node within an application.
