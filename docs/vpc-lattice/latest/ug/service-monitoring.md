...
4. Choose the **Monitoring** tab and then choose **Logs**.
   Check **Access logs** to see whether access logs are enabled.
5. To enable or disable access logs, choose **Edit access logs**,
   and then turn the **Access logs** toggle switch on or off.
6. When you enable access logs, you must select the type of delivery
   destination, and then create or choose the destination for the access logs.
   You can also change the delivery destination at any time. For example:
   - Select **CloudWatch Log group** and choose
     a CloudWatch Log group. To create a log group, choose **Create a
