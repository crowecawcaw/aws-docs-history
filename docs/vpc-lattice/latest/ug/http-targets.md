...
```
For information about the resource ARNs for VPC Lattice, see [Resource types defined by Amazon VPC Lattice](https://docs.aws.amazon.com/service-authorization/latest/reference/list_amazonvpclattice.html#amazonvpclattice-resources-for-iam-policies).

The caller identity headers can't be spoofed. VPC Lattice strips these headers from any incoming requests. These identity headers express a map that supports empty values using the following format. When parsing, you should not rely on the specific order of the KEYs in these headers, you should expect that new KEYs may be added any time and you should be prepared to handle empty values.
