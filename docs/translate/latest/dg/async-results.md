...
Once the job's status is `COMPLETED` or `COMPLETED_WITH_ERROR`, your
...
For instance, if you translated a document called `mySourceText.txt` into French,
the output document will be called `fr.mySourceText.txt`.

If the status of a batch translation job is `FAILED`, the [DescribeTextTranslationJob](../APIReference/API_DescribeTextTranslationJob.md "../APIReference/API_DescribeTextTranslationJob.md")
operation response includes a `Message` field that describes the reason why
