...
3. Follow the directions in the MySQL documentation to prepare the external MySQL
   database as a replica. For more information, see [Setting the Replica Configuration](https://dev.mysql.com/doc/refman/8.0/en/replication-howto-slavebaseconfig.html "https://dev.mysql.com/doc/refman/8.0/en/replication-howto-slavebaseconfig.html") in the MySQL
   documentation.
...
7.  Use the `mysql.rds_set_configuration` stored procedure to set the
    ...
    during the export. For more information, see [Accessing MySQL binary logs](USER_LogAccess.MySQL.md "USER_LogAccess.MySQL.md").
...
before 8.0.23, then use `SHOW SLAVE STATUS`. 2. Use the mysqldump utility to create a snapshot, which copies the data from
...
the directions in [Creating a Data Snapshot Using mysqldump](https://dev.mysql.com/doc/mysql-replication-excerpt/8.0/en/replication-howto-mysqldump.html "https://dev.mysql.com/doc/mysql-replication-excerpt/8.0/en/replication-howto-mysqldump.html") in the MySQL
documentation.
...
[Reloading SQL-Format Backups](https://dev.mysql.com/doc/refman/8.0/en/reloading-sql-format-dumps.html "https://dev.mysql.com/doc/refman/8.0/en/reloading-sql-format-dumps.html") in the MySQL documentation. You can run another utility
...
1. Use the MySQL `CHANGE MASTER` statement to configure the external
   ...
   MASTER TO Statement](https://dev.mysql.com/doc/refman/8.0/en/change-master-to.html "https://dev.mysql.com/doc/refman/8.0/en/change-master-to.html") in the MySQL documentation.

###### Note

Previous versions of MySQL used `SHOW SLAVE STATUS` instead of
`SHOW REPLICA STATUS`. If you are using a MySQL version
before 8.0.23, then use `SHOW SLAVE STATUS`. 2. Use the MySQL `START REPLICA` command to initiate replication from the
...
8.0.23, then use `START SLAVE`. 3. Run the MySQL `SHOW REPLICA STATUS\G` command on the external MySQL database to verify
that it is operating as a read replica. For more information about interpreting the
results, see [SHOW SLAVE | REPLICA STATUS Statement](https://dev.mysql.com/doc/refman/8.0/en/show-slave-status.html "https://dev.mysql.com/doc/refman/8.0/en/show-slave-status.html") in the MySQL documentation. 4. After replication on the external MySQL database has caught up with the source MySQL DB instance, use
...
8.0.23, then use `STOP SLAVE`. 5. On the Amazon RDS read replica, call the `mysql.rds_start_replication`
