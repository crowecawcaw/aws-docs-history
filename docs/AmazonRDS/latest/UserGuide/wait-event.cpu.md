...
- [Supported engine versions](#wait-event.cpu.context.supported "#wait-event.cpu.context.supported")
- [Context](#wait-event.cpu.context "#wait-event.cpu.context")
- [Likely causes of increased waits](#wait-event.cpu.causes "#wait-event.cpu.causes")
- [Actions](#wait-event.cpu.actions "#wait-event.cpu.actions")
...
- [How to tell when this wait occurs](#wait-event.cpu.when-it-occurs "#wait-event.cpu.when-it-occurs")
- [DBLoadCPU metric](#wait-event.cpu.context.dbloadcpu "#wait-event.cpu.context.dbloadcpu")
- [os.cpuUtilization metrics](#wait-event.cpu.context.osmetrics "#wait-event.cpu.context.osmetrics")
- [Likely cause of CPU scheduling](#wait-event.cpu.context.scheduling "#wait-event.cpu.context.scheduling")
...
- [Likely causes of sudden spikes](#wait-event.cpu.causes.spikes "#wait-event.cpu.causes.spikes")
- [Likely causes of long-term high frequency](#wait-event.cpu.causes.long-term "#wait-event.cpu.causes.long-term")
- [Corner cases](#wait-event.cpu.causes.corner-cases "#wait-event.cpu.causes.corner-cases")
...
- [Investigate whether the database is causing the CPU increase](#wait-event.cpu.actions.db-CPU "#wait-event.cpu.actions.db-CPU")
- [Determine whether the number of connections increased](#wait-event.cpu.actions.connections "#wait-event.cpu.actions.connections")
- [Respond to workload changes](#wait-event.cpu.actions.workload "#wait-event.cpu.actions.workload")
...
In this case, consider the following options:
    ...
    + Decrease the number of backend processes connected to your database. For example,
     implement a connection pooling solution such as RDS Proxy. To learn more, see [Amazon RDS Proxy](rds-proxy.md "rds-proxy.md").
