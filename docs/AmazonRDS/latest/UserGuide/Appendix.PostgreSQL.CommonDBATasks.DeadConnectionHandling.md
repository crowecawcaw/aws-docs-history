...
| --- |--- |--- |
| tcp\_keepalives\_idle | Specifies number of seconds of inactivity before sending keepalive message. | 300 | 
| tcp\_keepalives\_interval | Specifies number of seconds between retransmissions of unacknowledged keepalive messages. | 30 | 
| tcp\_keepalives\_count | Maximum lost keepalive messages before declaring connection dead | 2 | 
| tcp\_user\_timeout | Specifies how long (in Milliseconds) unacknowledged data can remain before forcibly closing the connection. | 0 | 
| client\_connection\_check\_interval | Sets the interval (in Milliseconds) for checking client connection status during long-running queries. This ensures quicker detection of closed connections. | 0 | 
