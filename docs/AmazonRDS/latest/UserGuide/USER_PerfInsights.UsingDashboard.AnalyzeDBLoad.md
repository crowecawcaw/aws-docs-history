...
![SQL text](images/sql-text-oracle.png)

To learn how to see more than the default 500 bytes of SQL text, see [Accessing more SQL text in the Performance Insights dashboard](USER_PerfInsights.UsingDashboard.md "USER_PerfInsights.UsingDashboard.md").
...
Performance Insights can show Oracle SQL text as **Unknown**. The text has this status in the
following situations:

- An Oracle database user other than `SYS` is active but not currently executing SQL. For example, when a parallel
  query completes, the query coordinator waits for helper processes to send their session statistics. For the duration of the wait,
  the query text shows **Unknown**.
- For an RDS for Oracle instance on Standard Edition 2, Oracle Resource Manager limits the number of parallel threads. The background
  process doing this work causes the query text to show as **Unknown**.
...
- Only one sample exists. For example, Performance Insights calculates rates of change for RDS PostgreSQL queries based on multiple samples from the
...
For a description of the SQL statistics for the Amazon RDS
...
[Average active sessions](USER_PerfInsights.Overview.md#USER_PerfInsights.Overview.ActiveSessions.AAS "USER_PerfInsights.Overview.md#USER_PerfInsights.Overview.ActiveSessions.AAS").
...
For more information about per-second or per-call statistics for the Amazon RDS engines,
