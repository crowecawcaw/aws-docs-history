...
**Warning**  
You can't restore exported snapshot data from S3 to a new DB instance or import snapshot data from S3 into an existing DB instance. However, you can process the data using Amazon Athena or Redshift Spectrum for analysis. Additionally, you can use AWS Glue to transform the data and then import it into Amazon RDS using tools like AWS DMS or custom scripts.

For more information about exporting DB snapshots to Amazon S3, see the following topics.

**Topics**
+ [Monitoring snapshot exports for Amazon RDS](USER_ExportSnapshot.Monitoring.md)
+ [Canceling a snapshot export task for Amazon RDS](USER_ExportSnapshot.Canceling.md)
+ [Failure messages for Amazon S3 export tasks for Amazon RDS](USER_ExportSnapshot.failure-msg.md)
+ [Troubleshooting RDS for PostgreSQL permissions errors](USER_ExportSnapshot.postgres-permissions.md)
+ [File naming conventions for exports to Amazon S3 for Amazon RDS](USER_ExportSnapshot.FileNames.md)
+ [Data conversion when exporting to an Amazon S3 bucket for Amazon RDS](USER_ExportSnapshot.data-types.md)
...
1. Create a symmetric encryption AWS KMS key for the server-side encryption. The KMS key is used by the snapshot export task to set up AWS KMS server-side encryption when writing the export data to S3.
   ...
   The KMS key policy must include both the `kms:CreateGrant` and `kms:DescribeKey` permissions. For more information on using KMS keys in Amazon RDS, see [AWS KMS key management](Overview.Encryption.Keys.md).
...
For information about other Amazon S3 access management tools, see [Access control in Amazon S3](https://docs.aws.amazon.com/AmazonS3/latest/userguide/access-management.html) in the *Amazon S3 User Guide*.
...
Feature availability and support varies across specific versions of each database engine and across AWS Regions. For more information on version and Region availability with exporting snapshots to S3, see [Supported Regions and DB engines for exporting snapshots to S3 in Amazon RDS](Concepts.RDS_Fea_Regions_DB-eng.Feature.ExportSnapshotToS3.md).
