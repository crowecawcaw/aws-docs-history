...
| `dump_directory` | `VARCHAR2` | `BDUMP` | No | The directory to write the report or export file to. If you specify a nondefault directory, the user that runs the `rdsadmin_diagnostic_util` procedures must have write permissions for the directory. | 
| `p_tag` | `VARCHAR2` | — | No | A tag to identify the report output file. You can specify up to 30 characters. Valid characters are `a-z`, `A-Z`, `0-9`, underscore (`_`), dash (`-`), and period (`.`). | 
...
The following example generates an ASH report that includes the data from September 18, 2019, at 6:07 PM through September 18, 2019, at 6:15 PM. The name of the output HTML report is `ashrpt_20190918180700_20190918181500.html`. The report is stored in the nondefault database directory named `AWR_RPT_DUMP`.
