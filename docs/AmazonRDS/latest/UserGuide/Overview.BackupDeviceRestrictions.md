...
- If you want to take a snapshot of your MyISAM tables before restoring, follow these steps:
  ...
  You can close all sessions by calling the [mysql.rds\_kill](Appendix.MySQL.CommonDBATasks.md "Appendix.MySQL.CommonDBATasks.md") command for each
  process that is returned from the `SHOW FULL PROCESSLIST` command. 2. Lock and flush each of your MyISAM tables. For example, the following commands lock and
