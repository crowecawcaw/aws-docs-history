...
| `p_remap_user_list`        | `CLOB`                              | NULL    | No       | A list of user schemas to be remapped during the metadata import. Use the format ``from_schema_name`:`to_schema_name``. For example, specify `hr:human_resources`. | ## Examples The example imports the tablespace metadata from the file `xttdump.dmp`, which is located in directory `DATA_PUMP_DIR`. ``BEGIN rdsadmin.rdsadmin_transport_util.import_xtts_metadata('`xttdump.dmp`','`DATA_PUMP_DIR`'); END; /`` |
