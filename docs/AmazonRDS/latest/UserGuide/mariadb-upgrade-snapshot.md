...
you create a DB snapshot, the snapshot is based on the engine version used by your DB instance. You can
...
To view the available engine versions for your RDS for MariaDB DB snapshot, use the following AWS CLI
example.
...
information, see [Upgrade options for DB snapshots with unsupported engine versions for RDS for MariaDB](mariadb-upgrade-snapshot.upgrade-options.md "mariadb-upgrade-snapshot.upgrade-options.md").

After restoring a DB snapshot upgraded to a new engine version, make sure to test that the
upgrade was successful. For more information about a major version upgrade, see [Upgrades of the MariaDB DB engine](USER_UpgradeDBInstance.MariaDB.md "USER_UpgradeDBInstance.MariaDB.md"). To learn how to restore a DB snapshot, see
...
- `--db-snapshot-identifier` – The identifier of the DB
  ...
  (ARN). For more information, see [Amazon Resource Names (ARNs) in Amazon RDS](USER_Tagging.ARN.md "USER_Tagging.ARN.md").
