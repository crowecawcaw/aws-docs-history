...
information, see [Upgrade options for DB snapshots with unsupported engine versions for RDS for MariaDB](mariadb-upgrade-snapshot.md "mariadb-upgrade-snapshot.md").

After restoring a DB snapshot upgraded to a new engine version, make sure to test that the
upgrade was successful. For more information about a major version upgrade, see [Upgrades of the MariaDB DB engine](USER_UpgradeDBInstance.md "USER_UpgradeDBInstance.md"). To learn how to restore a DB snapshot, see
