...
| `The RDS Custom for SQL Server Multi-AZ primary DB instance is busy or unresponsive.` | The primary DB instance is unresponsive. We recommend that you try the following steps: + Examine the event logs and CloudWatch logs for excessive CPU, memory, or swap space usage. For more information, see [Working with Amazon RDS event notification](USER_Events.md). <br />+ Create a rule that triggers on an Amazon RDS event. For more information, see [Creating a rule that triggers on an Amazon RDS event](rds-cloud-watch-events.md). <br />+ Evaluate your workload to determine whether you're using the appropriate DB instance class. For more information, see [DB instance classes](Concepts.DBInstanceClass.md).  | 
