...
If you repeatedly modify your DB instance maintenance window to avoid a pending mandatory upgrade, Amazon RDS can still apply the upgrade outside of your preferred maintenance window after the mandatory apply date has passed. This applies to all forced upgrades, including engine version deprecations and instance type deprecation campaigns. To avoid unplanned downtime, we recommend proactively upgrading your DB instances before the mandatory apply date rather than relying on maintenance window scheduling to delay the upgrade.
