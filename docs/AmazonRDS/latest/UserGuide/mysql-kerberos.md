...
For AWS Regions launched after Israel (Tel Aviv), starting with Canada West (Calgary), use the engine-specific service principal `directoryservice-{{engine}}.rds.{{region-code}}.amazonaws.com`, where {{engine}} is `mysql`. For example, for Canada West (Calgary) Region:
