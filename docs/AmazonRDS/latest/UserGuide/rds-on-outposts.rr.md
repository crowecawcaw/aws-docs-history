...
(CoIP). For more information, see [Customer-owned IP addresses for Amazon RDS on AWS Outposts](rds-on-outposts.coip.md "rds-on-outposts.coip.md").
...
- Read replicas require customer-owned IP (CoIP) pools. For more information, see [Customer-owned IP addresses for Amazon RDS on AWS Outposts](rds-on-outposts.coip.md "rds-on-outposts.coip.md").
...
- You can't create read replicas for DB instances encrypted with AWS KMS External Key Store (XKS).
  ...
  information on read replicas, see [Working with DB instance read replicas](USER_ReadRepl.md "USER_ReadRepl.md").
...
6. Specify your settings for **Outposts Connectivity**.
...
7. Choose your **DB instance class**. We recommend that you use the same or larger DB instance class and
...
DB instance. 9. (Optional) Under **Connectivity**, set values for
...
DB subnet group. 10. For **AWS KMS key**, choose the AWS KMS key identifier of the KMS key.

The read replica must be encrypted. 11. Choose other options as needed. 12. Choose **Create read replica**.
