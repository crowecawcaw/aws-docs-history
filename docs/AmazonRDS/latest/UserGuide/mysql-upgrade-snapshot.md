...
information, see [Upgrade options for DB snapshots with unsupported engine versions for RDS for MySQL](mysql-upgrade-snapshot.upgrade-options.md "mysql-upgrade-snapshot.upgrade-options.md").

After restoring a DB snapshot upgraded to a new engine version, make sure to test that the
upgrade was successful. For more information about a major version upgrade, see [Upgrades of the RDS for MySQL DB engine](USER_UpgradeDBInstance.MySQL.md "USER_UpgradeDBInstance.MySQL.md"). To learn how to restore a DB snapshot, see
