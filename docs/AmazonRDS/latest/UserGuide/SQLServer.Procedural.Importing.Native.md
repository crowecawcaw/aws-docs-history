...
Full restores with NORECOVERY aren't supported on Multi-AZ instances or instances with read replicas.

Full restores with both NORECOVERY and KEEP_CDC aren’t supported.

Differential restores aren't supported on any Multi-AZ deployment configuration.

All native restores aren't supported on instances that have read replicas, including cross-region read replicas.
