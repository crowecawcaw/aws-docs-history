...
- `LWLock:SubtransBuffer`: A process is waiting for I/O on a simple
  least-recently used (SLRU) buffer for a subtransaction. In RDS for PostgreSQL 12.22 and earlier, this wait event is called
  `subtrans`.
...
This wait event information is supported for all versions of RDS for
PostgreSQL.
