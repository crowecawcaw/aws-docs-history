...
1. In the Secrets Manager console, complete the following steps:
   ...
   1. Choose the secret that uses the naming format **do-not-delete-rds-custom-{{resource\_id}}-{{uuid}}** or **rds-custom\!oracle-do-not-delete-{{resource\_id}}-{{uuid}}**. The {{resource\_id}} is the resource ID that you copied in step 2.d.
