...
| `SERVER_AUDIT_EVENTS` | `CONNECT`, `QUERY`, `QUERY_DDL`, `QUERY_DML`, `QUERY_DML_NO_SELECT`, `QUERY_DCL` | `CONNECT`, `QUERY` | The types of activity to record in the log. Installing the MariaDB Audit Plugin is itself logged. +   `CONNECT`: Log successful and unsuccessful connections to the database, and disconnections from the database.  <br />+   `QUERY`: Log the text of all queries run against the database.  <br />+   `QUERY_DDL`: Similar to the `QUERY` event, but returns only data definition language (DDL) queries (`CREATE`, `ALTER`, and so on).  <br />+   `QUERY_DML`: Similar to the `QUERY` event, but returns only data manipulation language (DML) queries (`INSERT`, `UPDATE`, and so on, and also `SELECT`).  <br />+   `QUERY_DML_NO_SELECT`: Similar to the `QUERY_DML` event, but doesn't log `SELECT` queries.  <br />+   `QUERY_DCL`: Similar to the `QUERY` event, but returns only data control language (DCL) queries (`GRANT`, `REVOKE`, and so on).  <br />For MySQL, `TABLE` is not supported. | 
...
| connection\_type | The security state of the connection to the server. Possible values are:+  `Undefined` <br />+  `TCP/IP` <br />+  `Socket` <br />+  `Named pipe` <br />+  `SSL/TLS` <br />+  `Shared memory`   | 
