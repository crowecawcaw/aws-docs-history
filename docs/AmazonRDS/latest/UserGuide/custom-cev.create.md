...
1. In **Version details**, do the following:
   ...
   1. Enter a valid name in **Custom engine version name**. For example, you might enter the name **19.cdb\_cev1**.
...
+ `--engine-version` – Specify the engine version. The format is {{major-engine-version}}.{{customized\_string}}. The CEV name must contain 1–50 alphanumeric characters, underscores, dashes, or periods. The CEV name can't contain consecutive periods, as in `19..cdb_cev1`.
