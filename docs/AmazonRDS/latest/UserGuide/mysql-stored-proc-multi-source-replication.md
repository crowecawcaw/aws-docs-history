...
Before you call `mysql.rds_set_external_source_with_auto_position_for_channel`, make sure to call [mysql.rds\_set\_external\_source\_gtid\_purged](mysql-stored-proc-replicating.md#mysql_rds_set_external_source_gtid_purged) to set the `gtid_purged` system variable with a specified GTID range from an external source.
