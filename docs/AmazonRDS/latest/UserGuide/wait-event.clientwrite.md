...
- [Supported engine versions](#wait-event.clientwrite.context.supported "#wait-event.clientwrite.context.supported")
- [Context](#wait-event.clientwrite.context "#wait-event.clientwrite.context")
- [Likely causes of increased waits](#wait-event.clientwrite.causes "#wait-event.clientwrite.causes")
- [Actions](#wait-event.clientwrite.actions "#wait-event.clientwrite.actions")
...
A client process must read all of the data received from an RDS for PostgreSQL DB
...
Reduced network throughput between the RDS for PostgreSQL DB instance and the client
...
the client delays the reception of data from the RDS for PostgreSQL DB instance.

**Large volume of data sent to the client**

The RDS for PostgreSQL DB instance might be sending a large amount of
...
- [Place the clients in the same Availability Zone and VPC subnet as the cluster](#wait-event.clientwrite.actions.az-vpc-subnet "#wait-event.clientwrite.actions.az-vpc-subnet")
- [Use current generation instances](#wait-event.clientwrite.actions.db-instance-class "#wait-event.clientwrite.actions.db-instance-class")
- [Reduce the amount of data sent to the client](#wait-event.clientwrite.actions.reduce-data "#wait-event.clientwrite.actions.reduce-data")
- [Scale your client](#wait-event.clientwrite.actions.scale-client "#wait-event.clientwrite.actions.scale-client")
...
network round trips and increase network throughput. For more information, see [Jumbo frames (9001 MTU)](../../../AWSEC2/latest/UserGuide/network_mtu.md#jumbo_frame_instances "../../../AWSEC2/latest/UserGuide/network_mtu.md#jumbo_frame_instances") in
the _Amazon EC2 User Guide_.

For information about DB instance classes, see [DB instance classes](Concepts.DBInstanceClass.md "Concepts.DBInstanceClass.md"). To determine the DB instance class that is equivalent to an Amazon EC2
...
When possible, adjust your application to reduce the amount of data that the RDS for PostgreSQL DB instance sends to the client.
