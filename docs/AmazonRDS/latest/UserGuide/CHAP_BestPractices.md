...
+ [Keeping database engine versions up to date](#CHAP_BestPractices.Upgrades)
+ [AWS database drivers](#CHAP_BestPractices.Drivers)
...
+ [Amazon RDS new features video](#CHAP_BestPractices.Presentation)

**Note**  
For common recommendations for Amazon RDS, see [Recommendations from Amazon RDS](monitoring-recommendations.md).
...
+ If your database workload requires more I/O than you have provisioned, recovery after a failover or database failure will be slow. To increase the I/O capacity of a DB instance, do any or all of the following:
  ...
  + Convert from General Purpose SSD to Provisioned IOPS SSD storage, depending on how much of an increase you need. For information on available storage types, see [Amazon RDS storage types](CHAP_Storage.md#Concepts.Storage).
...
For more information, see [Connecting to DB instances with the AWS drivers](CHAP_CommonTasks.Connect.md#RDS.Connecting.Drivers).
...
1.  Use the pagination buttons to page through the additional metrics, or adjust the settings to see more metrics. 
...
For information about backing up your DB instance, see [Backing up, restoring, and exporting data](CHAP_CommonTasks.BackupRestore.md).
