...
- Reduce the overall number of connections and use a connection pooler such as [Amazon RDS Proxy](rds-proxy.md "rds-proxy.md") or other solutions such as PgBouncer. To learn more, see the [PgBouncer website](https://www.pgbouncer.org/ "https://www.pgbouncer.org/").

**Impact: Inode exhaustion**

Error: "No space left on device". If this is observed when there is plenty of storage free space, this is caused by running out of inodes. [Amazon RDS Enhanced Monitoring](USER_Monitoring.OS.md "USER_Monitoring.OS.md") provides visibility for inodes in use and the maximum number available for your host.
...
Unlogged tables are generally not included in logical replication, including [Blue/Green Deployments](blue-green-deployments.md "blue-green-deployments.md"), because logical replication relies on the WAL to capture and transfer changes.
...
If you plan to use [Amazon RDS Blue/Green Deployments](blue-green-deployments.md "blue-green-deployments.md") for any type of configuration change or upgrade, it is important to understand the impact of a high number of sequences on switchover. One of the last phases of a switchover will synchronize the current value of sequences, and if there are several thousand, this will increase the overall switchover time.
