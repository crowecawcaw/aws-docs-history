...
1. Run the procedure `rdsadmin.rdsadmin_transport_util.import_xtts_tablespaces`, specifying the tablespaces to import and the directory containing the backups.
   ...
   The following example imports the tablespaces {{TBS1}}, {{TBS2}}, and {{TBS3}} from the directory {{DATA\_PUMP\_DIR}}. The source platform is AIX-Based Systems (64-bit), which has the platform ID of `6`. You can find the platform IDs by querying `V$TRANSPORTABLE_PLATFORM`.
...
Export your tablespace metadata by running the `expdp` utility on your source host. The following example exports tablespaces {{TBS1}}, {{TBS2}}, and {{TBS3}} to dump file {{xttdump.dmp}} in directory {{DATA\_PUMP\_DIR}}.
...
If {{DATA\_PUMP\_DIR}} is a shared directory in Amazon EFS, skip to [Step 4: Import the tablespaces on your target DB instance](#oracle-migrating-tts.restore-full).
...
1. Run the procedure `rdsadmin.rdsadmin_transport_util.import_xtts_tablespaces`, specifying the tablespaces to import and the directory containing the backups.
   ...
   The following example imports the tablespaces {{TBS1}}, {{TBS2}}, and {{TBS3}} from the directory {{DATA\_PUMP\_DIR}}.
...
1. Remove old backups in {{DATA\_PUMP\_DIR}} as follows:
