...
![RDS Proxy monitoring dashboard.](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/images/Proxy-monitoring-dashboard.png)
...
You can't add or remove metrics from the default dashboard. If you want to create custom monitoring views for your proxies, you can do so directly in CloudWatch. For more information, see [Using CloudWatch dashboards](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch_Dashboards.html).

The monitoring dashboard is divided into several sections, with metrics grouped into widgets using predefined aggregations. For more information about the underlying RDS Proxy metrics, see [Monitoring RDS Proxy metrics with CloudWatch](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/rds-proxy.monitoring.html).

**Topics**
+ [Overview](#rds-proxy-monitoring-dashboard-overview)
+ [Latency](#rds-proxy-monitoring-dashboard-latency)
+ [Connection pool](#rds-proxy-monitoring-dashboard-connection-pool)
+ [Database connections utilization](#rds-proxy-monitoring-dashboard-db-connections-utilization-section)
...
1. For the `READWRITE` role containing one writer, the maximum size of the proxy connection pool will be slightly less than 250 (`50% * 500 - reserved_connections`), and the 70% guide line will correspond to less than 175 connections (`70% * 250 - reserved connections`).

1. For the `READ_ONLY` role containing two readers, the maximum size of the proxy connection pool will be slightly less than 500 (`2 * 50% * 500 - reserved_connections`), and the 70% guide line will correspond to less than 350 connections (`70% * 500 - reserved connections`).
...
To learn more about connection pinning, see [Avoiding pinning an RDS Proxy](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/rds-proxy-pinning.html).
