...
Database Mirroring (DBM) or Always On Availability Groups (AGs). Amazon RDS monitors and maintains the health
...
instance to use DBM or AGs. Amazon RDS handles the primary, the witness, and the secondary DB
instance for you. Because configuration is automatic, RDS selects DBM or Always On AGs based
...
When you create a new SQL Server DB instance using the AWS Management Console, you can add Multi-AZ with Database Mirroring (DBM) or Always On
AGs. You do so by choosing **Yes (Mirroring / Always On)** from **Multi-AZ deployment**. For
more information, see [Creating an Amazon RDS DB instance](USER_CreateDBInstance.md "USER_CreateDBInstance.md").

When you modify an existing SQL Server DB instance using the console, you can add Multi-AZ with DBM or AGs by choosing
**Yes (Mirroring / Always On)** from **Multi-AZ deployment** on the **Modify DB
...
Multi-AZ with DBM or AGs. You can do this by choosing **No (Mirroring / Always
On)** from **Multi-AZ deployment** on the **Modify
