...
When you upgrade your database engine version, you can't change the database architecture in the same operation. Therefore, to upgrade an Oracle Database 19c non-CDB to an Oracle Database 21c or Oracle Database 26ai CDB, you first need to convert the non-CDB to a CDB in one step, and then upgrade the 19c CDB to a 21c or 26ai CDB in a separate step.
