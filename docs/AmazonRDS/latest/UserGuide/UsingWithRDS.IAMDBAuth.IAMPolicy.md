...
To learn more about IAM policies, see [Identity and access management for Amazon RDS](UsingWithRDS.IAM.md "UsingWithRDS.IAM.md").
...
In this format, replace the following:
    ...
    If you are using Amazon Aurora, specify a `DbClusterResourceId` instead of a `DbiResourceId`.
     For more information, see [Creating and using an IAM policy for IAM database access](../AuroraUserGuide/UsingWithRDS.IAMDBAuth.IAMPolicy.md "../AuroraUserGuide/UsingWithRDS.IAMDBAuth.IAMPolicy.md") in the *Amazon Aurora User Guide*.
