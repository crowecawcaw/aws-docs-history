...
You use the `FreeableMemory` metric in Amazon CloudWatch to monitor the freeable memory. For more information, see [Monitoring tools for Amazon Aurora](MonitoringOverview.md).

If your DB instance consistently runs low on freeable memory or uses swap space, consider scaling up to a larger DB instance class. For more information, see [Amazon AuroraDB instance classes](Concepts.DBInstanceClass.md).
...
+ [Read replica replication fails to initialize metadata structure](#CHAP_Troubleshooting.MySQL.ReadReplicas.ReplicationErrorMetadata)
