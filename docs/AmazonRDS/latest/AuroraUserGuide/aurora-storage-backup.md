...
DB cluster snapshots are always full backups that capture the size of the cluster volume when you create them. Whether you take snapshots manually or through an [AWS Backups](https://docs.aws.amazon.com/aws-backup/latest/devguide/about-backup-plans.html) plan, Aurora treats them as manual snapshots. Aurora provides unlimited free storage for snapshots within the automated backup retention period. After a manual snapshot is outside this period, it incurs charges per GB-month. Automated system snapshots remain free unless you copy them. Because automated backups don’t cover snapshot copies, AWS always bills for them.

For general information about Aurora backups, see [Backups](Aurora.Managing.Backups.md#Aurora.Managing.Backups.Backup). For information about pricing for Aurora backup storage, see the [Amazon Aurora pricing](https://aws.amazon.com/rds/aurora/pricing) page.
...
The usage for an automated backup is calculated by looking at all of the incremental records that must be stored, to be able to restore to any point in time within the retention period of the backup. These changes include not just the number of write operations, but also the size and scope of data modifications. Each type of operation (INSERT, UPDATE, DELETE) creates change records that must be retained for point-in-time recovery. Therefore, while two databases might have the same number of write operations (IOPS), their backup storage requirements could differ significantly based on the volume of data being modified in each transaction.
