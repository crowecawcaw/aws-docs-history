...
- [Prerequisites](#limitless-shard-split.prereqs "#limitless-shard-split.prereqs")
- [Splitting a shard](#limitless-shard-split.proc "#limitless-shard-split.proc")
- [Tracking shard splits](#limitless-shard-split.track "#limitless-shard-split.track")
- [Finalizing shard splits](#limitless-shard-split.finalize "#limitless-shard-split.finalize")
- [Canceling a shard split](#limitless-shard-split.cancel "#limitless-shard-split.cancel")
...
| `rds_aurora.limitless_finalize_split_shard_mode` | This parameter determines how \*system-initiated<br>• shard splits are finalized. The value can be one of<br>the following:<br>• `user_initiated` – You decide when to finalize the shard split. This is the default<br>value.<br>• `immediate` – Shard splits are finalized immediately.<br>For more information, see [Finalizing shard splits](#limitless-shard-split.finalize "#limitless-shard-split.finalize").<br>NoteThis parameter applies only to system-initiated shard splits. |
...
- `IN_PROGRESS` – The shard-split job has been submitted and is in progress. You can have only one job in progress at
  a time.
- `PENDING` – The shard-split job is waiting for you to finalize it. For more information, see [Finalizing shard splits](#limitless-shard-split.finalize "#limitless-shard-split.finalize").
