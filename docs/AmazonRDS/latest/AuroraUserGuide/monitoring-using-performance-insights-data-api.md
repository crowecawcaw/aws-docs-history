...
+  For Data API queries that you submit to an Aurora cluster, the **Host** field in the PI dashboard is marked as **RDS Data API**. For Aurora PostgreSQL, the **application\_name** field has the value `rds-data-api`. Look for these labels when you analyze database load using **Top hosts** or **Top Applications** as a dimension. 
