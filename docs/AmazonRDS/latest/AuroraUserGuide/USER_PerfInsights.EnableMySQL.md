...
# Overview of the Performance Schema for Database Insights in Aurora MySQL
<a name="USER_PerfInsights.EnableMySQL"></a>

The Performance Schema is an optional feature for monitoring Aurora MySQL runtime performance at a low level of detail. The Performance Schema is designed to have minimal impact on database performance. Database Insights is a separate feature that you can use with or without the Performance Schema.
...
Database Insights and the Performance Schema are separate features, but they are connected. The behavior of Database Insights for Aurora MySQL depends on whether the Performance Schema is turned on, and if so, whether Database Insights manages the Performance Schema automatically. The following table describes the behavior.
...
When you create an Aurora MySQL DB instance with Database Insights turned on, the Performance Schema is also turned on. In this case, Database Insights automatically manages your Performance Schema parameters. This is the recommended configuration.
