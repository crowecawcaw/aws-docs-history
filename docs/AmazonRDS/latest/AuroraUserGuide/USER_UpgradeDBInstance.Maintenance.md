...
+ `serverless-platform-version-update` – Update the platform version of all the serverless DB instances in the DB cluster, using rolling upgrades. For more information about platform versions, see [Aurora serverless capacity](aurora-serverless-v2.how-it-works.md#aurora-serverless-v2.how-it-works.capacity).
...
| It was set previously to False on the DB cluster.<br />You set it to True on at least one DB instance, but not all instances. | False | True for one or more instances, but not all instances | No | 
...
Automatic minor version upgrades are communicated in advance through an Amazon RDS DB cluster event with a category of `maintenance` and ID of `RDS-EVENT-0156`. For more information, see [Amazon RDS event categories and event messages for Aurora](USER_Events.Messages.md).
