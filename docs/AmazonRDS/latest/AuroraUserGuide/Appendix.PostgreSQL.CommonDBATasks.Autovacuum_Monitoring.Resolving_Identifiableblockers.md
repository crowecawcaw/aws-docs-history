...
When the hot\_standby\_feedback setting is enabled, it prevents autovacuum on the writer instance from removing dead rows that might still be needed by queries running on the reader instance. This behavior is necessary because queries running on the reader instance (also applicable to reader instances in Aurora Global Database) require those rows to remain available on the writer instance preventing query conflicts and cancellations.
