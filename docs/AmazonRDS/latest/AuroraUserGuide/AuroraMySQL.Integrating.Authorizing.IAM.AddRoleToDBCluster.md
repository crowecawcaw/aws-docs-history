...
| `aws_default_logs_role`      | This parameter is no longer required for exporting log data from your DB cluster to<br>Amazon CloudWatch Logs. Aurora MySQL now uses a service-linked role for the required permissions. For more<br>information about service-linked roles, see [Using service-linked roles for Amazon Aurora](UsingWithRDS.IAM.ServiceLinkedRoles.md "UsingWithRDS.IAM.ServiceLinkedRoles.md").                                                        |
