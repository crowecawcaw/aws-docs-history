...
where `name` is your DB cluster. 5. Enter the following settings:
    ...
    * For **AWS KMS key**, choose a key from the list of AWS KMS keys.
    ...
    ###### Note
     ...
     in [Network prerequisites for Aurora MySQL database activity streams](DBActivityStreams.Prereqs.md "DBActivityStreams.Prereqs.md") to enable such
     access first.
...
database, see [Upgrading Amazon Aurora DB clusters](Aurora.VersionPolicy.Upgrading.md "Aurora.VersionPolicy.Upgrading.md").
