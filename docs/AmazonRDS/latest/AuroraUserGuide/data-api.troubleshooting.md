...
Multi-statements are not supported in the Data API for Aurora serverless and provisioned clusters. Attempting to execute multiple statements in a single API call results in this error.
...
Aurora serverless and provisioned clusters explicitly reject API calls that include the schema parameter.

To solve this issue, remove the schema parameter from all calls to the Data API when you use Aurora serverless or provisioned clusters.
