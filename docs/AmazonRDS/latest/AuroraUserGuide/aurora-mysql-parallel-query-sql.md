...
**Tip**  
 When you view these examples in HTML, you can use the **Copy** widget on each code listing to copy the SQL code to try yourself. Using the **Copy** widget avoids copying the extra characters around the `mysql>` prompt and `->` continuation lines. 
...
Instant DDL in Aurora MySQL version 3 replaces the fast DDL feature in Aurora MySQL version 2. For information about instant DDL, see [Instant DDL (Aurora MySQL version 3 and version 8.4)](AuroraMySQL.Managing.FastDDL.md#AuroraMySQL.mysql80-instant-ddl). 
