...
Aurora MySQL is performing a sort because it can't use an existing index to satisfy the `ORDER
 BY` or `GROUP BY` clause of a query. For more information, see [creating sort index](ams-states.sort-index.md "ams-states.sort-index.md").
...
necessary to satisfy the query. For more information, see [sending data](ams-states.sending-data.md "ams-states.sending-data.md").
