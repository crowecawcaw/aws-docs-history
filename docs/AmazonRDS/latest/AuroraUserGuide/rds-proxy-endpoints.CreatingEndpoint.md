...
1.  For **Virtual Private Cloud (VPC)**, choose the default to access the endpoint from the same EC2 instances or other resources that normally use to access the proxy or its associated database. To set up cross-VPC access for this proxy, choose a VPC other than the default. For more information about cross-VPC access, see [Accessing Aurora databases across VPCs](rds-proxy-endpoints.md#rds-proxy-cross-vpc). 
...
+  `--target-role { READ_WRITE | READ_ONLY }`. This parameter defaults to `READ_WRITE`. The `READ_ONLY` value affects only Aurora provisioned clusters that contain one or more reader DB instances. When the proxy is associated with an Aurora cluster that only contains a writer DB instance, you can't specify `READ_ONLY`. For more information about the intended use of read-only endpoints with Aurora clusters, see [Using reader endpoints with Aurora clusters](rds-proxy-endpoints.md#rds-proxy-endpoints-reader) . 
