...
This guide covers the four most common performance issues that affect Aurora PostgreSQL databases: table and index bloat, parallel query resource exhaustion,
...
- **Overall database growth is a workload change.** As
  ...
  when the query logic and indexes remain unchanged. Monitor `VolumeBytesUsed` to track storage growth
  trends.
...
- [Maintenance activities for PostgreSQL databases in Amazon RDS and Amazon
  Aurora](../../../prescriptive-guidance/latest/postgresql-maintenance-rds-aurora/introduction.md "../../../prescriptive-guidance/latest/postgresql-maintenance-rds-aurora/introduction.md") (AWS Prescriptive Guidance)
- [Optimizing PostgreSQL query performance](../../../prescriptive-guidance/latest/postgresql-query-tuning/introduction.md "../../../prescriptive-guidance/latest/postgresql-query-tuning/introduction.md") (AWS Prescriptive Guidance)
- [Managing query execution plans for Aurora PostgreSQL](AuroraPostgreSQL.Optimize.md "AuroraPostgreSQL.Optimize.md")
- [Monitoring Amazon Aurora metrics with Amazon CloudWatch](Aurora.AuroraMonitoring.Metrics.md "Aurora.AuroraMonitoring.Metrics.md") (monitor
  `VolumeBytesUsed` for storage growth trends)
...
1. **Check `pg_stat_activity`.** Look at
   ...
   information, see [Essential concepts for Aurora PostgreSQL tuning](AuroraPostgreSQL.Tuning.concepts.md "AuroraPostgreSQL.Tuning.concepts.md").
2. **Check for bloat.** Look for high
   `n_dead_tup` in `pg_stat_user_tables` and consider using
   `pgstattuple` for precise measurement. For more information, see [Diagnosing table and index bloat](AuroraPostgreSQL.diag-table-ind-bloat.md "AuroraPostgreSQL.diag-table-ind-bloat.md").
3. **Check `pg_stat_user_tables`.** Look for
   high `n_dead_tup` values and stale `last_autovacuum`
   timestamps. For more information, see [Working with PostgreSQL autovacuum on Aurora PostgreSQL](Appendix.PostgreSQL.CommonDBATasks.Autovacuum.md "Appendix.PostgreSQL.CommonDBATasks.Autovacuum.md").
4. **Review `EXPLAIN ANALYZE` on slow
   queries.** Look for parallel plans and sequential scans on large tables. For
   more information, see [Best practices for parallel queries in Aurora PostgreSQL](PostgreSQL.ParallelQueries.md "PostgreSQL.ParallelQueries.md").
5. **Check CloudWatch and Performance Insights metrics.** Review
   ...
   [Monitoring Amazon Aurora](MonitoringAurora.md "MonitoringAurora.md"). For common wait events and corrective actions, see [Aurora PostgreSQL wait events](AuroraPostgreSQL.Tuning.concepts.summary.md "AuroraPostgreSQL.Tuning.concepts.summary.md").
...
properly tuned for your workload. See [Autovacuum tuning](#PostgreSQL.InitialTroubleshooting.Autovacuum "#PostgreSQL.InitialTroubleshooting.Autovacuum") for tuning
recommendations.
...
[IPC:parallel wait events](apg-ipc-parallel.md "apg-ipc-parallel.md").
...
For detailed guidance on diagnosing and controlling parallel query behavior, see [Best practices for parallel queries in Aurora PostgreSQL](PostgreSQL.ParallelQueries.md "PostgreSQL.ParallelQueries.md").
...
their recommended corrective actions, see [Aurora PostgreSQL wait events](AuroraPostgreSQL.Tuning.concepts.summary.md "AuroraPostgreSQL.Tuning.concepts.summary.md").
...
configuring per-table overrides, see [Working with PostgreSQL autovacuum on Aurora PostgreSQL](Appendix.PostgreSQL.CommonDBATasks.Autovacuum.md "Appendix.PostgreSQL.CommonDBATasks.Autovacuum.md").

## Related information

- [Best practices for parallel queries in Aurora PostgreSQL](PostgreSQL.ParallelQueries.md "PostgreSQL.ParallelQueries.md")
- [Managing Aurora PostgreSQL connection churn with pooling](AuroraPostgreSQL.BestPractices.connection_pooling.md "AuroraPostgreSQL.BestPractices.connection_pooling.md")
- [Managing autovacuum with large indexes](Appendix.PostgreSQL.CommonDBATasks.Autovacuum.LargeIndexes.md "Appendix.PostgreSQL.CommonDBATasks.Autovacuum.LargeIndexes.md")
- [Diagnosing table and index bloat](AuroraPostgreSQL.diag-table-ind-bloat.md "AuroraPostgreSQL.diag-table-ind-bloat.md")
- [IPC:Parallel wait events reference](apg-ipc-parallel.md "apg-ipc-parallel.md")
- [Best practices with Amazon Aurora PostgreSQL](AuroraPostgreSQL.BestPractices.md "AuroraPostgreSQL.BestPractices.md")
