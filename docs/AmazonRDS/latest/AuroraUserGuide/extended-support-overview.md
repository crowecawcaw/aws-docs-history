...
and DB clusters, then Amazon Aurora will automatically enroll them in RDS Extended Support. You
can also prevent automatic enrollment by changing the `EngineLifecycleSupport`
parameter on an existing DB instance or DB cluster. Aurora
...
day that you complete an upgrade to a newer engine version that's still under Aurora standard
support.
