...
+ If you don't delete the DB cluster in the green environment (`--no-delete-target`), the cluster is retained, but it's no longer part of a blue/green deployment. For Aurora MySQL, replication continues between the environments. For Aurora PostgreSQL, the green environment is promoted to a standalone environment, so replication stops.
...
**Important**  
After you delete a blue/green deployment, RDS removes read-only protections from the previous production DB cluster. If the `read_only` parameter is disabled for the DB cluster, it starts to allow write operations again. 
