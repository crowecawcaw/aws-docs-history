...
- `:'aws_lambda_arn_1'` – This parameter identifies the composite structure
  created in [Step 4: Use Lambda helper functions with your Aurora PostgreSQL DB cluster (Optional)](PostgreSQL-Lambda.md#PostgreSQL-Lambda-specify-function "PostgreSQL-Lambda.md#PostgreSQL-Lambda-specify-function"), with the `aws_commons.create_lambda_function_arn` helper
