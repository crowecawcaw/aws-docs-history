...
**Tip**  
 If your database is sometimes idle but at other times has a substantial workload, you can use Aurora serverless as an alternative to T instances. With Aurora serverless, you define a capacity range and Aurora automatically scales your database up or down depending on the current workload. For usage details, see [Using Aurora serverless](aurora-serverless-v2.md). For the database engine versions that you can use with Aurora serverless, see [Requirements and limitations for Aurora serverless](aurora-serverless-v2.requirements.md). 
