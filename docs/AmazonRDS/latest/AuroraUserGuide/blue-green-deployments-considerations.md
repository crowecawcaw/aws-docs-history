...
| NEXTVAL operations on sequence objects aren't synchronized between the blue environment and the green environment. | During switchover, Aurora increments sequence values in the green environment to match those in the blue environment. While a high volume of sequences generally allows the switchover to proceed, an exceptionally large number—such as several hundred thousand—might cause the process to time out before completion. You can increase the switchover timeout to allow more time for synchronization. For more information, see [Switchover timeout](blue-green-deployments-switching.md#blue-green-deployments-switching-timeout). | 
