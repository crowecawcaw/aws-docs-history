...
**Approximate threshold:** [Millions](#PostgreSQL.HighObjectCount.Note "#PostgreSQL.HighObjectCount.Note")
...
**Approximate threshold:** [Tens of thousands](#PostgreSQL.HighObjectCount.Note "#PostgreSQL.HighObjectCount.Note")
...
Unlogged tables are generally not included in logical replication, including [Blue/Green Deployments](blue-green-deployments.md "blue-green-deployments.md"), because logical replication relies on the WAL to capture and transfer changes. Read more on how to configure Aurora PostgreSQL to [replicate unlogged tables](aurora-postgresql-unlogged-tables.md#aurora-postgresql-unlogged-tables-logicalrep "aurora-postgresql-unlogged-tables.md#aurora-postgresql-unlogged-tables-logicalrep").

**Impact: Reader Nodes**

You can access unlogged tables only from the writer node in the Aurora DB cluster. See [Working with unlogged tables in Aurora PostgreSQL](aurora-postgresql-unlogged-tables.md "aurora-postgresql-unlogged-tables.md") for full details.
...
- Monitor your temp file usage with [Performance Insights](USER_PerfInsights.md "USER_PerfInsights.md").
...
In PostgreSQL, logical replication does not currently replicate a sequence's current value to any subscriber. To learn more, see the [Restrictions page in PostgreSQL documentation](https://www.postgresql.org/docs/current/logical-replication-restrictions.html "https://www.postgresql.org/docs/current/logical-replication-restrictions.html").

**Impact: Extended switchover time**

If you plan to use [Blue/Green Deployments](blue-green-deployments.md "blue-green-deployments.md") for any type of configuration change or upgrade, it is important to understand the impact of a high number of sequences on switchover. One of the last phases of a switchover will synchronize the current value of sequences, and if there are several thousand, this will increase the overall switchover time.
