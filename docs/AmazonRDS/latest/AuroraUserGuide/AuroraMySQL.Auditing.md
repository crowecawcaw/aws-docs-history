...
If a certain user is reporting these internal `SELECT` queries in the audit logs, then you can exclude that user by setting the [server\_audit\_excl\_users](#AuroraMySQL.Auditing.Enable.server_audit_excl_users) DB cluster parameter. However, if that user is also used in other activities and can't be omitted, then there is no other option for excluding `SELECT` queries.
