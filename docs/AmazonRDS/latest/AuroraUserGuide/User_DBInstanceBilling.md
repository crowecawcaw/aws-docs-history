...
+ **Aurora serverless** – Aurora serverless provides on-demand capacity where the billing unit is Aurora capacity unit (ACU) hours instead of DB instance hours. Aurora serverless capacity increases and decreases, within a range that you specify, depending on the load on your database. You can configure a cluster where all the capacity is Aurora serverless. Or you can configure a combination of Aurora serverless and on-demand or reserved provisioned instances. For information about how Aurora serverless ACUs work, see [How Aurora serverless works](aurora-serverless-v2.how-it-works.md). 
...
+ [Reserved DB instances for Amazon Aurora](USER_WorkingWithReservedDBInstances.md)