...
1. On the details page, the **Target groups** section shows how the proxy is linked to a specific Aurora DB cluster. You can navigate to the default target group page for a deeper view of this association, including configuration settings defined during proxy creation. These settings include the maximum connection percentage, connection borrow timeout, engine family, and session pinning filters.
...
1.  To see the details of the Aurora DB cluster associated with the returned target group, run [describe-db-proxy-targets](https://docs.aws.amazon.com/cli/latest/reference/rds/describe-db-proxy-targets.html). 
