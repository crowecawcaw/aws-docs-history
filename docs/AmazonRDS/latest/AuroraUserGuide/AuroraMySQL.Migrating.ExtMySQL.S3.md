...
**Contents**
+ [Limitations and considerations](#AuroraMySQL.Migrating.ExtMySQL.S3.Limits)
+ [Before you begin](#AuroraMySQL.Migrating.ExtMySQL.S3.Prereqs)
  ...
  + [Required permissions](#AuroraMySQL.Migrating.ExtMySQL.S3.Prereqs.Permitting)
  + [Creating the IAM service role](#AuroraMySQL.Migrating.ExtMySQL.S3.Prereqs.CreateRole)
...
+ [Reducing the time for physical migration to Amazon Aurora MySQL](AuroraMySQL.Migrating.ExtMySQL.Prechecks.md)
  + [Unsupported table types](AuroraMySQL.Migrating.ExtMySQL.Prechecks.md#AuroraMySQL.Migrating.ExtMySQL.Prechecks.Tables)
  + [User accounts with unsupported privileges](AuroraMySQL.Migrating.ExtMySQL.Prechecks.md#AuroraMySQL.Migrating.ExtMySQL.Prechecks.Users)
  + [Dynamic privileges in Aurora MySQL version 3](AuroraMySQL.Migrating.ExtMySQL.Prechecks.md#AuroraMySQL.Migrating.ExtMySQL.Prechecks.Dynamic)
  + [Stored objects with 'rdsadmin'@'localhost' as the definer](AuroraMySQL.Migrating.ExtMySQL.Prechecks.md#AuroraMySQL.Migrating.ExtMySQL.Prechecks.Objects)
...
1. In the Amazon RDS console, choose the AWS Region in which to create your DB cluster. Choose the same AWS Region as the Amazon S3 bucket that contains your database backup. 
...
1. Run the [mysql.rds\_import\_binlog\_ssl\_material](mysql-stored-proc-replicating.md#mysql_rds_import_binlog_ssl_material) stored procedure to import the SSL information into the Aurora MySQL DB cluster.
...
**Note**  
After running the procedure, the secrets are stored in files. To erase the files later, you can run the [mysql.rds\_remove\_binlog\_ssl\_material](mysql-stored-proc-replicating.md#mysql_rds_remove_binlog_ssl_material) stored procedure.
...
1. Enable binary log replication by running the [mysql.rds\_reset\_external\_master (Aurora MySQL version 2)](mysql-stored-proc-replicating.md#mysql_rds_reset_external_master) or [mysql.rds\_reset\_external\_source (Aurora MySQL version 3)](mysql-stored-proc-replicating.md#mysql_rds_reset_external_source) stored procedure. This stored procedure has the following syntax.
   ...
   For information about the parameters, see [mysql.rds\_reset\_external\_master (Aurora MySQL version 2)](mysql-stored-proc-replicating.md#mysql_rds_reset_external_master) and [mysql.rds\_reset\_external\_source (Aurora MySQL version 3)](mysql-stored-proc-replicating.md#mysql_rds_reset_external_source).
...
1. Start binary log replication by running the [mysql.rds\_start\_replication](mysql-stored-proc-replicating.md#mysql_rds_start_replication) stored procedure.
...
1. Connect to the MySQL replication primary database and stop replication. To do so, run the [mysql.rds\_stop\_replication](mysql-stored-proc-replicating.md#mysql_rds_stop_replication) stored procedure.
