...
| TEST_TRAFFIC_SHIFT            | The blue and green service revisions are running. The blue service<br>revision handles 100% of the production traffic. The green service<br>revision is migrating from 0% to 100% of test traffic. | Yes (Lambda only)                  |
...
| PRE_PRODUCTION_TRAFFIC_SHIFT  | Occurs before the production traffic shift. For blue/green<br>deployments, this stage is invoked once.                                                                                             | Yes                                |
| PRODUCTION_TRAFFIC_SHIFT      | Production traffic is shifting to the green service revision. The<br>green service revision is migrating from 0% to 100% of production<br>traffic.                                                 | Yes (Lambda only)                  |
