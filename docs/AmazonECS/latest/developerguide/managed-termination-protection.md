...
You must turn on Amazon EC2 Auto Scaling _instance scale-in protection_ on the
Amazon EC2 Auto Scaling group to use the managed termination protection feature of cluster auto scaling.
...
the EC2 instances in the Amazon EC2 Auto Scaling group. Then, Amazon ECS places the tasks on the instances. When all
non-daemon tasks are stopped on an instance, Amazon ECS initiates the scale-in process and
turns off scale-in protection for the EC2 instance. The Amazon EC2 Auto Scaling group can then terminate the
instance.

Amazon EC2 Auto Scaling _instance scale-in protection_ controls which EC2 instances
can be terminated by Amazon EC2 Auto Scaling. Instances with the scale-in feature turned on can't be
terminated during the scale-in process. For more information about Amazon EC2 Auto Scaling instance
scale-in protection, see [Using
instance scale-in protection](../../../autoscaling/ec2/userguide/ec2-auto-scaling-instance-protection.md "../../../autoscaling/ec2/userguide/ec2-auto-scaling-instance-protection.md") in the _Amazon EC2 Auto Scaling User
Guide_.

You can set the `targetCapacity` percentage so that you have spare
capacity. This helps future tasks launch more quickly because the Amazon EC2 Auto Scaling group does not have to
launch more instances. Amazon ECS uses the target capacity value to manage the CloudWatch metric
that the service creates. Amazon ECS manages the CloudWatch metric. The Amazon EC2 Auto Scaling group is treated as a
...
- Placement constraints such as Availability Zones, without additional
  ...
  You must turn on Amazon EC2 Auto Scaling instance scale-in protection on the Amazon EC2 Auto Scaling group to use managed
...
actions, like detaching the instance, on the Amazon EC2 Auto Scaling group associated with the capacity
provider. Manual actions can break the scale-in operation of the capacity provider. If
you detach an instance from the Amazon EC2 Auto Scaling group, you need to also [deregister
the detached instance](deregister_container_instance.md "deregister_container_instance.md") from the Amazon ECS cluster.
