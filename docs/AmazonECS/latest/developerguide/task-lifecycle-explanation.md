...
![Diagram of the task lifecycle states. The states are PROVISIONING, PENDING, ACTIVATING, RUNNING, DEACTIVATING, STOPPING.](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/images/task-lifecycle.png)
...
For Linux containers, the container agent will send the stop signal defined in your container image to notify the application needs to finish and shut down using the `STOPSIGNAL` instruction. This is `SIGTERM` by default. Then it will send a `SIGKILL` after waiting the `StopTimeout` duration set in the task definition. 
...
If your task stopped because of an error, see [Viewing Amazon ECS stopped task errors](stopped-task-errors.md).
