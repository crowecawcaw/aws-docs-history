...
+ ECS Exec and SSM
  + Your task requires a task IAM role with the required permissions for SSM Session Manager. For tasks on Amazon EC2, if no task role is configured, the instance role of the underlying Amazon EC2 instance is used instead. For more information, see [ECS Exec permissions](task-iam-roles.md#ecs-exec-required-iam-permissions).
