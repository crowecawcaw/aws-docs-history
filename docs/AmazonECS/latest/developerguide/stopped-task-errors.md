...
If your task ran on a container instance on Amazon EC2 or external computers, you can also look at the logs of the container runtime and the Amazon ECS Agent. These logs are on the host Amazon EC2 instance or external computer. For more information, see [Viewing Amazon ECS container agent logs](logs.md).
...
1. Configure the filter to display stopped tasks. For **Filter desired status**, choose **Stopped**.
...
1. In the row for your stopped task, in the **Last Status** column, choose **Stopped**.
...
1. Describe the stopped task to retrieve the information. For more information, see [describe-tasks](https://docs.aws.amazon.com/cli/latest/reference/ecs/describe-tasks.html) in the* AWS Command Line Interface Reference*.
...
View your stopped tasks so you can get information about the cause. For more information, see [Amazon ECS stopped tasks error messages](stopped-task-error-codes.md).