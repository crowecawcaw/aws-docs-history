...
Create the host volume mount points on your container instance.  
     ...
     ```
Add the `--privileged` option to the **docker run** command that follows.
Append the `:Z` option to the `/log` and `/data` container volume mounts (for example, `--volume=/var/log/ecs/:/log:Z`) to the **docker run** command that follows.
