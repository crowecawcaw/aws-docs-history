...
scale-in events from either service scaling or deployments.
...
- You have a queue-processing asynchronous application such as a video transcoding
  ...
  is low.
- You have a gaming application that runs game servers as tasks that need to
...
agent endpoint, see [Amazon ECS task scale-in protection endpoint](managed-instance-task-scale-in-protection-endpoint.md "managed-instance-task-scale-in-protection-endpoint.md").
