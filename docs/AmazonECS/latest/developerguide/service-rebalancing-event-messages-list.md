# Amazon ECS Availability Zone

service rebalancing service event messages

The following are examples of service event messages you might see.

## service

(`service-name`) is not AZ balanced with
`number-tasks` tasks in `Availability
 Zone 1`, `number-tasks` in
`Availability Zone 2`, and
`number-tasks` in `Availability Zone
 3`. AZ Rebalancing in progress.
...
(`service-name`) is AZ balanced with
`number-tasks` tasks in `Availability
 Zone 1`, `number-tasks` tasks in
`Availability Zone 2`, and
`number-tasks` tasks in `Availability
 Zone 3`.
...
## `service-name` has started

`number-tasks` tasks in `Availability
 Zone` to AZ Rebalance:
`task-ids`.
...
## `service-name` has stopped

`number-tasks` running tasks in
`Availability Zone` due to AZ rebalancing:
`task-id`.
...
(`service-name`) was unable to AZ Rebalance because `task-set-name` was unable to scale in due to `reason`.
...
(`service-name`) stopped AZ Rebalancing.
