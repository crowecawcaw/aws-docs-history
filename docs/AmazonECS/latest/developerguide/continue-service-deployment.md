...
- `CONTINUE` - This option continues the deployment to the next
  lifecycle stage.
...
- You need the `hookId` from the
  `DescribeServiceDeployments` response.
...
2. On the **Clusters** page, choose the
   cluster.
3. In the **Services** section, choose the
   service.
4. Choose the **Deployments** tab.
5. Verify that the pause hook status shows **Awaiting
   action**.
6. Choose **Take Action**, then choose
   **Continue** or **Roll
   back**.

CLI

1. Get the `hookId` by calling
...
2. Call `continue-service-deployment` with the
   `hookId` and the action:
...
- Monitor the deployment progress using `DescribeServiceDeployments` or
  the Amazon ECS console.
- If additional pause hooks are configured at later lifecycle stages, the deployment
  pauses again at those stages.
