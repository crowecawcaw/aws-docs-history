...
1. In the left-hand navigation, there is a single page that was created with your app. Choose **\+ Add** twice to create two more pages. The navigation pane should show three total pages.
...
1. Configure the action by performing the following steps:
   ...
   1. For **Configuration**, enter the following text, replacing {{bucket\_name}} with the bucket you created in the prerequisites:
...
1. The output of this automation will be used to populate a table component with objects from your Amazon S3 bucket. However, by default, automations don't create outputs. Configure the automation to create an automation output by performing the following steps:
   ...
   1. On the right-hand **Properties** menu, in **Automation output**, choose **\+ Add output**.

   1. For **Output**, enter **{{results.ListObjects1.Contents}}**. This expression returns the contents of the action, and can now be used to populate a table component.
...
1. Add an automation parameter, used to pass data to an automation, by performing the following steps:
   ...
   1. On the right-hand **Properties** menu, in **Automation parameters**, choose **\+ Add**.
...
1. By default, automations don't create outputs. Configure the automation to create an automation output by performing the following steps:
   ...
   1. For **Output**, enter **{{results.GetObject1.Body.transformToWebStream()}}**. This expression returns the contents of the action.
**Note**  
...
1. Add a column to be filled with the name of the file.
   ...
   1. On the right-hand **Properties** menu, next to **Columns**, choose **\+ Add**.
   ...
   1. For **Value**, enter **{{currentRow.Key}}**.
...
1. Add a table action to delete the file in a row.
   ...
   1. On the right-hand **Properties** menu, next to **Actions**, choose **\+ Add**.

   1. In **Actions**, rename **Button** to **Delete**.

   1. Choose the arrow icon to the right of the **Delete** action that was just renamed.

   1. In **On click**, choose **\+ Add action** and choose **Invoke automation**.
   ...
   1. In the parameter text box, enter **{{currentRow.Key}}**.
...
1. Add the **PDF viewer** component.
   ...
   1. In the parameter text box, enter **{{ui.table1.selectedRow["Filename"]}}**.

   1. In the right-hand panel, there is also a **File name** field. The value of this field is used as the header for the PDF viewer component. Enter the same text as the previous step: **{{ui.table1.selectedRow["Filename"]}}**.
...
1. On the right-hand **Properties** menu, configure the component:
   ...
   1. For **File name**, enter **{{ui.s3Upload1.files[0]?.nameWithExtension}}**.
