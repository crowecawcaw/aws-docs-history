...
of Amplify and your other AWS solutions. AWS provides the following monitoring tools to
watch Amplify, report when something is wrong, and take automatic actions when
appropriate:

- _Amazon CloudWatch_ monitors
  in real time
  your AWS resources and the applications
  that you
  run on
  AWS.
  You can collect and track metrics, create customized dashboards, and set alarms that notify you
  or take actions when a certain metric reaches a threshold that you specify. For example, you can
  have CloudWatch track CPU usage or other metrics of your
  Amazon Elastic Compute Cloud
  (Amazon EC2)
  instances and automatically launch new instances when needed. For more information about using
  CloudWatch metrics and alarms with Amplify, see [Monitoring an Amplify application](access-logs.md "access-logs.md").
- _Amazon CloudWatch Logs_ enables you to monitor, store, and access your log files
  from Amazon EC2 instances,
  AWS CloudTrail,
  and other sources. CloudWatch Logs can monitor information in the log files and notify you when certain
...
- _AWS CloudTrail_ captures API calls and related events made by or on behalf of
  your AWS account and delivers the log files to an
...
- _Amazon EventBridge_ is a serverless event bus service that makes it easy to
  ...
  services, and routes that data to targets such as AWS Lambda. This enables you to monitor events
  that happen in services and build event-driven architectures. For more information, see the
