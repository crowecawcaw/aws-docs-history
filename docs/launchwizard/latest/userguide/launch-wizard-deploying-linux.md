...
- [Access AWS Launch Wizard](#accessing-launch-wizard-linux "#accessing-launch-wizard-linux")
- [Deploy AWS Launch Wizard on Ubuntu](#deploy-console-launch-wizard-linux "#deploy-console-launch-wizard-linux")
- [Post-deployment cluster tasks](#launch-wizard-linux-post-deployment "#launch-wizard-linux-post-deployment")
...
AWS Launch Wizard on the Ubuntu platform after you have launched it from the console. For SQL
Server deployments on Ubuntu, you must use an instance type built on the [Nitro
System](../../../AWSEC2/latest/UserGuide/instance-types.md#ec2-nitro-instances "../../../AWSEC2/latest/UserGuide/instance-types.md#ec2-nitro-instances"). EBS volumes are exposed as NVMe block devices on instances built
...
1. When you select **Choose application** from the AWS Launch Wizard
   ...
   **Create deployment**.
2. Under **Review Permissions**, Launch Wizard displays the AWS Identity and Access Management
   (IAM) role required for Launch Wizard to access other AWS services on your behalf.
   For more information about setting up IAM for Launch Wizard, see [AWS Identity and Access Management (IAM)](launch-wizard-getting-started.md#launch-wizard-iam "launch-wizard-getting-started.md#launch-wizard-iam"). Choose
   **Next** .
3. On the **Configure application settings** page, select the
   **Operating System** on which you want to install SQL
   Server — in this case, **Ubuntu**.
4. **Deployment model**. Choose **High availability deployment** to deploy your SQL
...
5. You are prompted to enter specifications for the new deployment. The following
...
General
    ...
    * **Simple Notification Service (SNS)
     ...
     where AWS Launch Wizard can send notifications and alerts. For more
     information, see the [*Amazon Simple Notification Service Developer Guide*](../../../sns/latest/dg/welcome.md "../../../sns/latest/dg/welcome.md").
...
VPC.
    ...
    * **Select Virtual Private Cloud
     ...
     outbound connectivity to the internet and other AWS
     services (S3, CFN, SSM, Logs). We recommend that you enable
     this connectivity with a NAT Gateway. For more information
     about NAT Gateways, see [NAT Gateways](../../../vpc/latest/userguide/vpc-nat-gateway.md "../../../vpc/latest/userguide/vpc-nat-gateway.md") in the Amazon VPC User Guide.
    	...
    	If the traffic of a subnet is routed to an
    	 ...
    	 subnet using the following steps.
    		...
    		- To configure your subnets to route internet
    		 ...
    		 Amazon VPC User Guide. Use IPv4 format
    	...
    	If a subnet doesn't have a route to an
    	 ...
    	 public subnets, see the steps in [Creating a NAT Gateway](../../../vpc/latest/userguide/vpc-nat-gateway.md#nat-gateway-creating "../../../vpc/latest/userguide/vpc-nat-gateway.md#nat-gateway-creating") to create a NAT
    	 ...
    	 your chosen private subnets.
    		...
    		- Follow the steps in [Creating a Subnet](../../../vpc/latest/userguide/working-with-vpcs.md#AddaSubnet "../../../vpc/latest/userguide/working-with-vpcs.md#AddaSubnet") in the Amazon VPC User Guide
    		 using the existing VPC you will use in AWS Launch Wizard.
    ...
    * **Create new Virtual Private Cloud
     (VPC)** option. Launch Wizard creates your VPC. You can
     optionally enter a VPC name tag.
...
###### SQL Server configuration
    ...
    * **User name and password**.
     ...
     See [Password Policy](https://docs.microsoft.com/en-us/sql/relational-databases/security/password-policy?view=sql-server-2017 "https://docs.microsoft.com/en-us/sql/relational-databases/security/password-policy?view=sql-server-2017") for more information.
...
services or resources. Pacemaker is maintained by the [ClusterLabs](https://www.clusterlabs.org/ "https://www.clusterlabs.org/")
community.
    ...
    * **Pacemaker cluster
     ...
     pacemaker username `hacluster`. This username is
...
Pacemaker.
    ...
    * **SQL Pacemaker user name and
     ...
     reenter the password to verify it. See [Password Policy](https://docs.microsoft.com/en-us/sql/relational-databases/security/password-policy?view=sql-server-2017 "https://docs.microsoft.com/en-us/sql/relational-databases/security/password-policy?view=sql-server-2017") for more information.
...
performance, and volume types, or to use AWS recommended
resources. If you choose to use AWS recommended resources, you
...
use AWS recommended resources. If no selections are made,
default values are assigned.
    ...
    * **Volume type**. Choose your
     ...
     types, see [Amazon EBS volume types](../../../AWSEC2/latest/UserGuide/ebs-volume-types.md "../../../AWSEC2/latest/UserGuide/ebs-volume-types.md")
...
corresponds to the ID of the AWS resource group created for a
...
AWS Launch Wizard provides an estimate for application charges incurred to
...
your actual costs may be lower. 8. When you are satisfied with your infrastructure selections, select
**Next**. If you don't want to complete the configuration,
select **Cancel**. When you select **Cancel**,
all of the selections on the specification page are lost and you are returned to
the landing page. To go to the previous screen, select
**Previous**. 9. On the **Review and deploy** page, review your configuration
...
**Deploy**, you agree to the terms of the **Note** at the bottom of the page. 10. Launch Wizard validates the inputs and notifies you if you must update a
specification. 11. When validation is complete, Launch Wizard deploys your AWS resources and configures
...
deployments. 12. When your deployment is ready, a notification informs you that your SQL Server
...
and then selecting **Manage** from the **Actions** dropdown list. 13. When the SQL Server Always On application is deployed, you can access your
Amazon EC2 instances through the EC2 console. You can also use [AWS SSM](../../../systems-manager/latest/userguide/what-is-systems-manager.md "../../../systems-manager/latest/userguide/what-is-systems-manager.md") to manage your SQL Server Always On application for
...
Before performing any of these tasks, verify the cluster status using `pcs resource
 status --all`. This command returns all cluster issues. All issues must be
...
allocated to their assigned cluster nodes. 6. Verify that the availability group listener is available by pinging the
floating IP address.
...
listener will follow. 5. Verify cluster status using the following command: `pcs resource
 --all`.

The resource that was moved should be located on the
`<NODE_NAME>`. 6. Clear temporary constraints using the following command: `pcs resource
 clear `<RESOURCE_NAME>``.
...
nodes. 5. Verify the shut down status using the following command: `pcs status
 --all`.
...
If a node is restarted from the operating system or the AWS Management Console, the Pacemaker
...
Availability Group resource must be returned to the primary node. 7. Remove all temporary constraints using the following commands: `pcs
 resource clear `<AG_RESOURCE>`` and `pcs
 resource clear `<AG_LISTENER>``. 8. Run the following command: `pcs resource move
 `<RESOURCE_NAME>`
`<PRI_NODE_NAME>` --force`.

This command moves the resources to
`<PRI_NO_NAME>` and starts the
resource. Any cluster constraints are applied. In this scenario, if the
Microsoft SQL Server resource agent is moved, then the availability group
listener follows. 9. Verify cluster status using the following command: `pcs resource
 --all`. The restarted node will be located on
`<PRI_NO_NAME>`.
