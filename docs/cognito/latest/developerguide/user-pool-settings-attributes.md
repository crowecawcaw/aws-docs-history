...
Aliases can also transfer between accounts when a user verifies an attribute with `VerifyUserAttribute`. If a user verifies an email address or phone number that is already a verified alias on another account, Amazon Cognito transfers the alias to their account and marks the attribute as unverified on the original account. This transfer doesn't raise an `AliasExistsException`.
