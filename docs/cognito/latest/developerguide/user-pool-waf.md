...
When you have an AWS WAF web ACL associated with a user pool, Amazon Cognito forwards selected non-confidential headers and contents of requests from your users to AWS WAF. AWS WAF inspects the contents of the request, compares it to the rules that you specified in your web ACL, and returns a response to Amazon Cognito. The request components that Amazon Cognito forwards to AWS WAF depend on the type of request. For managed login and the classic hosted UI, Amazon Cognito doesn't forward the request body. For more information, see [Things to know about AWS WAF web ACLs and Amazon Cognito](#user-pool-waf-things-to-know).
