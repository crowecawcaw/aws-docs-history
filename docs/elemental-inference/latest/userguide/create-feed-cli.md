...
1. Use `create-feed` to create a new feed. 
   ...
   (Optional) Specify an **access role** (`accessRoleArn`), the ARN of an IAM role that Elemental Inference assumes on your behalf to access the resources a feed needs. An access role is required when a smart crop output uses graphic composition, so that Elemental Inference can read your template images from Amazon S3.
