...
A service-linked role makes setting up Image Builder more efficient, because you don’t have to add
...
The service-linked role trusts the _imagebuilder.amazonaws.com_
service to assume the role.

You don't need to manually create this service-linked role. When you create your first
...
- Use one of the following API actions, or its corresponding AWS CLI command:
  - The **[CreateImage](../APIReference/API_CreateImage.md "../APIReference/API_CreateImage.md")**
    ...
    in the AWS CLI).
  - The **[ImportVmImage](../APIReference/API_ImportVmImage.md "../APIReference/API_ImportVmImage.md")**
    API action (**[import-vm-image](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/imagebuilder/import-vm-image.html "https://awscli.amazonaws.com/v2/documentation/api/latest/reference/imagebuilder/import-vm-image.html")**
    in the AWS CLI).
  - The **[StartImagePipelineExecution](../APIReference/API_StartImagePipelineExecution.md "../APIReference/API_StartImagePipelineExecution.md")**
    API action (**[start-image-pipeline-execution](https://awscli.amazonaws.com/v2/documentation/api/latest/reference/imagebuilder/start-image-pipeline-execution.html "https://awscli.amazonaws.com/v2/documentation/api/latest/reference/imagebuilder/start-image-pipeline-execution.html")**
    in the AWS CLI).

###### Important

If the service-linked role is deleted from your account, you can use the same process
...
see the [AWSServiceRoleForImageBuilder policy](security-iam-awsmanpol.md#sec-iam-manpol-AWSServiceRoleForImageBuilder "security-iam-awsmanpol.md#sec-iam-manpol-AWSServiceRoleForImageBuilder") page. To learn more
...
ensure that there are no Image Builder resources enabled that refer to it.

###### Note

If the Image Builder service is using the role when you try to delete the resources, the
...
1. Verify that no pipeline builds are running before you start. To cancel a running
   build, use the `cancel-image-creation` command from the AWS CLI.
...
2. Change all pipeline schedules to use a manual build process, or delete them
