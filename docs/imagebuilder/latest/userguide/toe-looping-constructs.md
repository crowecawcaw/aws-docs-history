...
types of looping constructs in AWSTOE:
...
- `forEach` constructs
  ...
  - `forEach` loop with delimited list – Iterates over a
    finite collection of strings joined by a delimiter.
...
`{{ <loop_name>.* }}` with `{{ loop.* }}` in the input
...
- `{{ <loop_name>.* }}` – AWSTOE resolves this expression using the following logic:
  ...
  - `<loop_name>` resolves to the named looping construct if it appears within the
    currently running step.

- `{{ loop.* }}` – AWSTOE resolves the expression using the looping construct
  defined in the currently running step.
...
- [for loop](#toe-loop-types-for "#toe-loop-types-for")
- [forEach loop with input list](#toe-loop-types-foreach "#toe-loop-types-foreach")
- [forEach loop with delimited list](#toe-loop-types-foreach-delimited "#toe-loop-types-foreach-delimited")

### `for` loop

The `for` loop iterates on a range of integers specified within a boundary outlined by the
start and end of the variables. The iterating values are in the set `[start, end]` and
includes boundary values.

AWSTOE verifies the `start`, `end`, and `updateBy` values to ensure that the
