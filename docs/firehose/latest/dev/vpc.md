...
1. In the list of policies, search for and add the two policies named `AWSLambdaVPCAccessExecutionRole` and `AmazonDataFirehoseReadOnlyAccess`.
**Important**  
...
1. Enter a name for the function, then set **Runtime** to `Python 3.9` or higher.
...
1. Choose **Test** again. This causes the function to run. After the execution result appears, expand **Details** and compare the log output to the function code. Successful results show a list of the Firehose streams in the Region, as well as the following output:
