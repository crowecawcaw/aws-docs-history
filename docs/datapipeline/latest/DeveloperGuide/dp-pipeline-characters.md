...
| #                 | Expression. Expressions are delimited by: "#{" and "}" and the contents of the braces are evaluated by AWS Data Pipeline. For more information, see [Expressions](dp-pipeline-expressions.md "dp-pipeline-expressions.md"). | #{format(myDateTime,'YYYY-MM-dd hh:mm:ss')} s3://amzn-s3-demo-bucket/#{id}.csv |
