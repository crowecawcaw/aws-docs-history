...
Sends an Amazon SNS notification message when an activity fails or finishes
successfully.

## Example

The following is an example of this object type. The values for
`node.input` and `node.output` come from the data node
or activity that references this object in its `onSuccess` field.
