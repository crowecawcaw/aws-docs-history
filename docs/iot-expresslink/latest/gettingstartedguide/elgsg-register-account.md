...
<a name="elgsg-register-account"></a>
...
1. Open the [AWS IoT console](https://console.aws.amazon.com/iot/). In the navigation pane choose **Manage** then choose **Things**. Choose **Create things**, select **Create single thing**, then choose **Next**.
...
1. In the AWS IoT console, on the **Configure device certificate** page, select **Use my certificate**, then select **CA is not registered with AWS IoT**. 

1. Under **Certificate**, choose **Choose file**. Select the file "`ThingName.cert.pem`" that you created in a previous step, then choose **Open**. 

1. Under **Certificate status**, select **Active**, then choose **Next**.

1. Under **Attach policies to certificate**, choose **Create policy**.

1. Enter a **Policy name** (for example, "IoTDevPolicy"), then under **Policy document** select **JSON**. 

1. Copy the the following into the console **Policy document**:
...
1. In the AWS IoT console, in the navigation pane, choose **Settings**. Under **Device data endpoint** select the **Endpoint** to make a copy of the endpoint for your account. 

1. In the terminal application, type this command using the endpoint you just copied: 
