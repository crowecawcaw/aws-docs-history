...
| 48 | BLECentral\# | BLE PAIR | Pairing event (see [Table 3 - Configuration dictionary non-persistent keys](elpg-configuration-dictionary.md#elpg-table3)) | 
| 49 | Auth Index | BLE AUTHORIZE | Authorizing event (see [13.3.6 BLE AUTH*[\#] [0/1]*   »Authorize access to a characteristic«](elpg-ble.md#elpg-ble-auth-command))<br />Auth Index encoding is defined below in *8.2.1.5*. | 
...
If the time parameter is greater than the maximum duration allowed for any ExpressLink command (see [4.6.2 Response timeout](elpg-commands.md#elpg-response-timeout)).  
