...
- **appearance** (string) – a 4 digit hex number that
  ...
  [Bluetooth
  Specification](https://www.bluetooth.com/specifications/assigned-numbers/ "https://www.bluetooth.com/specifications/assigned-numbers/")).
  Example 1
...
[BLE CONNECT command](#elpg-ble-connect-command "#elpg-ble-connect-command")). BLECentral# and
...
using the appropriate SET/GET commands (see [BLE SET](#elpg-ble-set-command "#elpg-ble-set-command")
and [BLE GET](#elpg-ble-get-command "#elpg-ble-get-command") commands).
...
![Figure 7 - BLE scanning for devices](images/ble-scanning-for-devices.png)
...
(see [Table 4 - ExpressLink event codes](elpg-event-handling.md#elpg-table4 "elpg-event-handling.md#elpg-table4")) will be produced.
Queued data can then be retrieved using the [BLE GET DISCOVER](#elpg-ble-get-discover-command "#elpg-ble-get-discover-command") command.
...
event on successful Discover complete or cancellation (see [Table 4 - ExpressLink event codes](elpg-event-handling.md#elpg-table4 "elpg-event-handling.md#elpg-table4")).
...
![Figure 8 - Connecting to a BLE device](images/connecting-to-ble-device.png)
...
specified then the Peripheral command [13.3.1 BLE CONNECT?   »Connection status query«](#elpg-ble-connectq-command "#elpg-ble-connectq-command") is invoked.
...
![Figure 9 - Reading a connected BLE peripheral](images/reading-a-connected-ble-peripheral.png)
...
![Figure 10 - Writing to a connected BLE device](images/writing-to-connected-ble-device.png)
...
The host can subscribe to receive notifications (see [Table 4 - ExpressLink event codes](elpg-event-handling.md#elpg-table4 "elpg-event-handling.md#elpg-table4")) when connected to a peripheral and the selected
...
![Figure 11 - Subscribing to receive peripheral notifications](images/subscribe-to-peripheral-notifications.png)
...
![Figure 12 - Reading from a local characteristic in BLE peripheral mode](images/reading-from-a-local-characteristic.jpg)
...
![Figure 13 - Writing to a connected BLE device](images/writing-to-a-local-characteristic.jpg)
...
![Figure 14 - Typical BLE Peripheral initialization and pairing flow](images/typical-peripheral-pairing.png)
...
![Smartphone initiates pairing with ExpressLink module](images/ble_pairing_step1.png) 2. The host queries the sub-state:
...
![Host queries pairing sub-state and receives NC_VALUE](images/ble_pairing_step2.png) 3. Fetch the six-digit number and display it:
...
![Retrieve and display six-digit pairing code](images/ble_pairing_step3.png) 4. The user taps "Yes" on the device to accept pairing:
...
![User accepts pairing by tapping Yes on device](images/ble_pairing_step4.png) 5. Pairing completes and you receive a final notification event:
...
![Pairing completion notification and success confirmation](images/ble_pairing_step5.png)
