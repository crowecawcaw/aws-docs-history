...
1. Create a JSON file that contains the encryption settings that you want the
   cluster to have. The following is an example.

###### Note

You can only update the client-broker encryption setting. You can't
update the in-cluster (broker-to-broker) encryption setting.
...
The output of this `update-security` operation looks like the
following JSON.
...
4. To see the status of the `update-security` operation, run the
   following command, replacing `ClusterOperationArn`
   with the ARN that you obtained in the output of the
   `update-security` command.
...
If `OperationState` has the value `PENDING` or
