...
For information about how to rebalance partitions after you remove brokers from a cluster, see [Reassign partitions](bestpractices.md#bestpractices-balance-cluster "bestpractices.md#bestpractices-balance-cluster").
...
- [Remove broker partitions](#msk-remove-broker-partitions "#msk-remove-broker-partitions")
- [Remove a broker with console](#msk-remove-broker-console "#msk-remove-broker-console")
- [Remove a broker with CLI](#msk-remove-broker-cli "#msk-remove-broker-cli")
- [Remove a broker with API](#msk-remove-broker-api "#msk-remove-broker-api")
...
1. Create a client machine as described in [Create a client machine](create-client-machine.md "create-client-machine.md").
...
In this example, we see four topic names, `__amazon_msk_canary`, `__consumer_offsets`, `__amazon_msk_connect_offsets_my-mskc-connector_12345678-09e7-c657f7e4ff32-2`, and `msk-brk-rmv`. 3. Create a json file called `topics.json` on the client machine and add all the user topic names as in the following code example. You don’t need to include the `__amazon_msk_canary` topic name as this is a service managed topic that will be automatically moved when necessary.
...
6. Run the following command to move partitions that you specified in the
...
4. Enter the number of brokers that you want the cluster to have per Availability Zone.
   The console summarizes the number of brokers across availability zones that will be removed. Make sure this what you want.
...
Run the following command, replacing `ClusterArn` with the Amazon Resource Name (ARN) that you obtained when you created your cluster. If you don't have the ARN for your cluster, you can find it by listing all clusters. For more information, [Listing Amazon MSK clusters](msk-list-clusters.md "msk-list-clusters.md").
...
The `Target-Number-of-Brokers` parameter represents the
total number of broker nodes that you want the cluster to have when this operation completes successfully. The value you specify for `Target-Number-of-Brokers` must be a whole number that is less than the current number of brokers in the cluster. It must also be a multiple of the number of Availability Zones.
...
To remove brokers in a cluster using the API, see [UpdateBrokerCount](../../1.0/apireference/clusters-clusterarn-nodes-count.md#clusters-clusterarn-nodes-count-url "../../1.0/apireference/clusters-clusterarn-nodes-count.md#clusters-clusterarn-nodes-count-url") in the _Amazon Managed Streaming for Apache Kafka API Reference_.
