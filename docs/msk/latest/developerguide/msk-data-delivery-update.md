...
Use the update field that matches the Channel's destination type — `--iceberg-destination-update` for an Iceberg destination or `--s3-destination-update` for an S3 destination.
