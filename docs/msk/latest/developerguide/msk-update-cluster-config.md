...
- [Broker availability during configuration updates](#update-config-cluster-availability "#update-config-cluster-availability")
- [Updating the configuration of a cluster using the AWS CLI](#update-config-cli "#update-config-cli")
- [Update the configuration of a Amazon MSK cluster using the API](#update-config-api "#update-config-api")
...
If you face any issues while upgrading your MSK cluster, see [How do I troubleshoot issues when I upgrade my Amazon MSK cluster?](https://repost.aws/knowledge-center/msk-upgrade-cluster-issues "https://repost.aws/knowledge-center/msk-upgrade-cluster-issues")

## Updating the configuration of a cluster using the AWS CLI

1. Copy the following JSON and save it to a file. Name the file
   `configuration-info.json`. Replace
   `ConfigurationArn` with the Amazon Resource Name
   (ARN) of the configuration that you want to use to update the cluster. The ARN
   string must be in quotes in the following JSON.

Replace `Configuration-Revision` with the revision of
the configuration that you want to use. Configuration revisions are integers
(whole numbers) that start at `1`. This integer mustn't be in quotes
...
2. Run the following command, replacing `ClusterArn`
   with the ARN that you obtained when you created your cluster. If you don't have
   the ARN for your cluster, you can find it by listing all clusters. For more information, see [List Amazon MSK clusters](msk-list-clusters.md "msk-list-clusters.md").

Replace `Path-to-Config-Info-File` with the path to
your configuration info file. If you named the file that you created in the
previous step `configuration-info.json` and saved it in the
current directory, then `Path-to-Config-Info-File` is
...
The output of this `update-cluster-configuration` command looks
...
3. To get the result of the `update-cluster-configuration` operation,
   run the following command, replacing
   `ClusterOperationArn` with the ARN that you
   obtained in the output of the `update-cluster-configuration`
   command.
...
`UPDATE_CLUSTER_CONFIGURATION`. If `OperationState`
has the value `UPDATE_IN_PROGRESS`, wait a while, then run the
`describe-cluster-operation` command again.

## Update the configuration of a Amazon MSK cluster using the API

To use the API to update the configuration of a Amazon MSK cluster, see [UpdateClusterConfiguration](../../1.0/apireference/clusters-clusterarn-configuration.md#UpdateClusterConfiguration "../../1.0/apireference/clusters-clusterarn-configuration.md#UpdateClusterConfiguration").
