...
## Trust policy (both destinations)
...
## Permission policy — S3 Tables (Iceberg) destination

The following single policy contains every statement the service role may need. Keep the statements that apply to your setup (see [When you need each statement](#msk-data-delivery-iam-when-needed "#msk-data-delivery-iam-when-needed")) and remove the rest.
...
## Permission policy — S3 bucket destination
...
- **Amazon CloudWatch Logs (optional):** add `logs:CreateLogStream` and `logs:PutLogEvents` on the log group (see [Logging for Channel](msk-data-delivery-logging.md "msk-data-delivery-logging.md")).
