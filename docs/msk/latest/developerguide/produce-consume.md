...
Amazon MSK](getting-started.md "getting-started.md"), you produce and consume data.
...
2. Enter any message that you want, and press **Enter**. Repeat
   ...
   **Enter**, that line is sent to your Apache Kafka cluster
   as a separate message.
3. Keep the connection to the client machine open, and then open a second, separate connection to that machine in a new window. Because this is a new session, set the `KAFKA_ROOT` and `BOOTSTRAP_SERVER` environment variables again. For information about how to set these environment variables, see [Creating a topic on the client machine](create-topic.md#create-topic-client-machine "create-topic.md#create-topic-client-machine").
...
You should start seeing the messages you entered earlier when you used the console producer command. 5. Enter more messages in the producer window, and watch them appear in the
consumer window.
**Next Step**
