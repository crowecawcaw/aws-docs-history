...
| Maximum TCP connections per broker ([IAM Access control](iam-access-control.md)) | 3000 | To increase this limit, you can adjust the listener.name.client\_iam.max.connections or the listener.name.client\_iam\_public.max.connections configuration property using the Kafka AlterConfig API or the kafka-configs.sh tool. It's important to note that increasing either property to a high value can result in unavailability. | 
