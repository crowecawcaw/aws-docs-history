...
- The source cluster must have multi-VPC private connectivity turned on for IAM access control. See [Cluster owner turns on multi-VPC](mvpc-cluster-owner-action-turn-on.md "mvpc-cluster-owner-action-turn-on.md").
- You must attach a resource-based permissions policy to the source cluster. See [Prepare the source cluster](msk-replicator-prepare-clusters.md#msk-replicator-prepare-source "msk-replicator-prepare-clusters.md#msk-replicator-prepare-source").
