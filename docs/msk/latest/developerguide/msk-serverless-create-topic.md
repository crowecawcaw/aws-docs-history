...
In this step, you use the previously created client machine to create a topic on the
serverless cluster.

###### Topics

- [Setting up your environment for creating topics](#msk-serverless-create-topic-prerequisites "#msk-serverless-create-topic-prerequisites")
- [Creating a topic and writing data to it](#msk-serverless-create-topic-procedure "#msk-serverless-create-topic-procedure")
...
1. In the following `export` command, replace
   `my-endpoint` with the bootstrap-server string you
   that you saved after you created the cluster. Then, go to the
   `kafka_2.12-2.8.1/bin` directory on the client machine
...
2. Run the following command to create a topic called
