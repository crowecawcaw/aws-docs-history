...
1. SASL/SCRAM, mTLS, or SASL/OAUTHBEARER authentication enabled on source cluster
...
1. Reconfigure producers to MSK Provisioned cluster with IAM authentication

1. Monitor `MessageLag` until it reaches 0

1. Stop consumers on self-managed cluster

1. Reconfigure consumers to MSK Provisioned cluster
