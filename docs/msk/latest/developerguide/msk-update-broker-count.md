...
If you want to expand an
MSK cluster, make sure to use this Amazon MSK operation. Don't try to add brokers to a cluster without using this operation.

For information about how to rebalance partitions after you add brokers to a cluster,
see [Reassign partitions](bestpractices.md#bestpractices-balance-cluster "bestpractices.md#bestpractices-balance-cluster").
...
4. Enter the number of brokers that you want the cluster to have per Availability
   Zone and then choose **Save changes**.
...
The `Target-Number-of-Brokers` parameter represents
the total number of broker nodes that you want the cluster to have when this
operation completes successfully. The value you specify for
`Target-Number-of-Brokers` must be a whole number
that is greater than the current number of brokers in the cluster. It must also
be a multiple of the number of Availability Zones.
...
2. To get the result of the `update-broker-count` operation,
   run the following command, replacing
   `ClusterOperationArn` with the ARN that you
   obtained in the output of the `update-broker-count`
   command.
...
In this output, `OperationType` is
`INCREASE_BROKER_COUNT`. If `OperationState` has the
value `UPDATE_IN_PROGRESS`, wait a while, then run the
`describe-cluster-operation` command again.

## Expand a Amazon MSK cluster using the API

To increase the number of brokers in a cluster using the API, see [UpdateBrokerCount](../../1.0/apireference/clusters-clusterarn-nodes-count.md#UpdateBrokerCount "../../1.0/apireference/clusters-clusterarn-nodes-count.md#UpdateBrokerCount").
