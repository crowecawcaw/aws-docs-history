...
+ Create an Apache ZooKeeper configuration file for use with the `kafka-configs.sh` and [`kafka-acls.sh`](https://kafka.apache.org/documentation/#security_authz_cli) tools, or with the ZooKeeper shell. With each tool, you use the `--zk-tls-config-file` parameter to specify your Apache ZooKeeper config.
