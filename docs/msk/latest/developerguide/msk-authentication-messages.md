...
2. Run the following command to start a console producer. The file named
   `client.properties` is the one you created in the previous
   procedure.
...
3. In a new command window on your client machine, run the following command to start
   a console consumer.
...
4. Type messages in the producer window and watch them appear in the consumer
   window.
