...
You can create a private connection between your VPC and Connector for SCEP by configuring
an interface VPC endpoint. Interface endpoints are powered by [AWS PrivateLink](../../../whitepapers/latest/aws-vpc-connectivity-options/aws-privatelink.md "../../../whitepapers/latest/aws-vpc-connectivity-options/aws-privatelink.md"), a technology for privately accessing Connector for SCEP API
operations. AWS PrivateLink routes all network traffic between your VPC and
Connector for SCEP through the Amazon network, avoiding exposure on the open internet.
...
The interface VPC endpoint connects your VPC directly to Connector for SCEP without an
internet gateway, NAT device, VPN connection, or Direct Connect connection. The instances
in your VPC don't need public IP addresses to communicate with the Connector for SCEP
API.

To use Connector for SCEP through your VPC, you must connect from an instance that is
...
Connector for SCEP does not require the use of AWS PrivateLink, but we recommend it as an
...
Before you set up interface VPC endpoints for Connector for SCEP, be aware of the
following considerations:

- Connector for SCEP might not support VPC endpoints in some Availability
...
- VPC endpoints do not support cross-Region requests. Ensure that you
  create your endpoint in the same Region where you create your connector.
...
You can create a VPC endpoint for the Connector for SCEP service using either the
...
[Creating an Interface Endpoint](../../../vpc/latest/userguide/vpce-interface.md#create-interface-endpoint "../../../vpc/latest/userguide/vpce-interface.md#create-interface-endpoint") procedure in the _Amazon VPC User Guide_. Connector for SCEP supports making
...
Connector for SCEP endpoint now resolves to your VPC endpoint. For a comprehensive
...
For more information, see [VPC endpoints (AWS PrivateLink)](../../../vpc/latest/userguide/vpc-endpoints.md "../../../vpc/latest/userguide/vpc-endpoints.md") in the _Amazon VPC User Guide_.

## Create a VPC endpoint policy for Connector for SCEP

You can create a policy for Amazon VPC endpoints for Connector for SCEP to specify the
following:
...
principals to the listed Connector for SCEP actions on the specified connector resource.
...
Connector for SCEP provides a separate VPC endpoint service for enrollment operations
such as `GetCACaps` and `PKIOperation`.

When creating the enrollment endpoint, specify
`com.amazonaws.`region`.pca-connector-scep.enroll`
as the service name.

When creating a connector, you can optionally specify a
`VpcEndpointId` to restrict the connector to only be accessible
through that specific VPC endpoint.
...
To reach your connector, you must use the endpoint URL included in the
connector details, not the VPC endpoint DNS name directly. However, you can
replace the DNS name portion of the connector endpoint URL with any valid
VPC endpoint DNS name, such as an AZ-specific DNS name.
...
You can attach a VPC endpoint policy to control access to enrollment operations.
When attached to an endpoint, the following policy grants access for all
principals to the `GetCACaps` and `PKIOperation` operations.
The resource in the stanza is a connector.

Connector for SCEP enrollment operations aren't authenticated with SigV4. Due to this, they aren't
associated with an IAM principal and instead are considered anonymous by VPC endpoint policies.
As such, your VPC endpoint policy must allow all principals for these actions.
