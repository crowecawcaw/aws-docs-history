...
| New policy names: <br>• `AWSPrivateCAFullAccess` <br>• `AWSPrivateCAReadOnly` <br>• `AWSPrivateCAPrivilegedUser` <br>• `AWSPrivateCAAuditor` <br>• `AWSPrivateCAUser` | Policy name prefixes were changed from `AWSCertificateManagerPrivateCA` to `AWSPrivateCA`. Functionality remains unchanged.                                                             | February 13, 2023 |
