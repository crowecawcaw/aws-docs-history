...
**Note**  
This documentation is for Amazon Nova Version 1. For information about using Web Grounding with Amazon Nova 2, visit [Web Grounding](https://docs.aws.amazon.com/nova/latest/nova2-userguide/web-grounding.html).
...
1. **Search Assessment** — The model analyzes the prompt and determines if search is needed to improve the response.

1. **Search Execution** — When required, the model automatically performs one or more searches for relevant information.

1. **Result Analysis** — The model decides whether additional searches are required, to expand its understanding or dive deeper on specific subtopics.

1. **Response Generation** — Nova automatically synthesizes information from search results to generate a final API response grounded in real-time information, complete with citations to its sources.
...
In your `toolConfig`, add a tool with the `systemTool` parameter as in the example above. This parameter allows you to specify *system-defined tools*. *System-defined tools* are tools that are provided and invoked by the model provider. For details on using the `systemTool` parameter, see [SystemTool](https://docs.aws.amazon.com/bedrock/latest/APIReference/API_runtime_SystemTool.html) in the Amazon Bedrock API reference.
...
1. **Add Specific Permissions (if needed)**: If you require more granular access control, add this policy to your role's IAM policy, replacing the red text with your AWS account ID:
