...
In this example, `max_epochs` and `optim.lr` are set explicitly via overrides. All other training parameters (batch size, warmup steps, model-parallel and so on) fall through to the Hub default recipe for the `nova-textgeneration-lite` model.
