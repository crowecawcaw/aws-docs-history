...
+ `embeddings` (Required) - For most requests, this array will contain a single embedding. For video requests where the "AUDIO\_VIDEO\_SEPARATE" embeddingMode mode was selected, this array will contain two embeddings - one embedding for the video content and one for the the audio content.
