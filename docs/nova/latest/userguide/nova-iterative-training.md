...
You can track metrics via MLflow by [setting up MLflow](nova-model-monitor.md#nova-customization-mlflow-setup) for your jobs.
...
+ Adapters from each stage are merged based on the merge weights you specify. For details on how adapters are merged, see [LoRA fine-tuning](nova-model-merge.md#nova-model-merge-lora) in the model merge documentation.
