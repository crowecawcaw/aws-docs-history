...
+ `strategy`: (Required) Defines the evaluation approach:
  + zs\_cot: Zero-shot Chain-of-Thought - An approach to prompt large language models that encourages step-by-step reasoning without requiring explicit examples.
...
+ `metric`: (Required) The evaluation metric to use.
  ...
  + exact\_match: (For `math` benchmark), returns the rate at which the input predicted strings exactly match their references.
  ...
  + `all`: Returns the following metrics:
    ...
    + For `llm_judge` and `mm_llm_judge`, bring your own dataset benchmark, return following metrics:
      ...
      + `winrate`: the probability that response\_B will be preferred over response\_A calculated using Bradley-Terry probability.
