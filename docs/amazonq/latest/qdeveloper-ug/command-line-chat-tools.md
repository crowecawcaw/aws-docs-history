...
![Command line interface showing tool permissions, with fs_read set to per-request and fs_write trusted.](images/q_cli_tools_trust_untrust.png)
...
Using `/tools trust-all` carries risks. For more information, see [Understanding security risks](command-line-chat-security.md#command-line-chat-security-risks "command-line-chat-security.md#command-line-chat-security-risks")
.

The following image shows the status of the CLI tools when they are all in their default trust
status.

![Command-line interface showing current tool permissions with various access levels.](images/q_cli_tools_trustall_all_trusted2.png)
...
When Amazon Q attempts to use a tool that doesn't have explicit permission, it will ask
...
![Command prompt showing a file read request and asking for permission to allow the action.](images/q_cli_tools_show_trust_used.png)
You can also specify trust permissions as part of starting a `q chat`
session.

![Command-line options for configuring trust and execution settings in a model.](images/q_cli_tools_specify_trust_start.png)
