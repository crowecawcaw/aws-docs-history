...
to Amazon Q Business Pro to continue using Q Apps
...
User subscriptions are created per Amazon Q Business application or Quick Suite account. Each admin can manage subscriptions for users for their specific
Amazon Q Business application or Quick Suite account.

For applications using IAM Identity Center, AWS will deduplicate subscriptions across all Amazon Q Business applications and Quick Suite accounts, and charge each user only once for
their highest subscription level. Note that deduplication will apply only if the Amazon Q Business applications and Quick Suite accounts share the same IAM Identity Center instance.
