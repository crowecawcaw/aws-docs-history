...
- Used the AD application ID to register a secret key for the application on
  the AD site. The secret key must contain the client ID and a client
  secret.
...
Choose the Application permissions type instead of Delegated
permissions while adding the API permissions.
