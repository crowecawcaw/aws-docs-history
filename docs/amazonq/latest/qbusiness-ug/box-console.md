...
4. Then, on the **Add data sources** page, from
   **Data sources**, add the **Box** data source to your Amazon Q application.
...
See [Authorization](connector-concepts.md#connector-authorization "connector-concepts.md#connector-authorization") for more details. 9. In **Authentication** – Choose to create an
**AWS
Secrets Manager secret** and then enter the following
information for your **AWS Secrets Manager secret**.

    1. **Secret name** – A name for your
     secret.
    2. **Client ID** – The client ID provided by
     Box.
    3. **Client Secret** – The client secret provided
     by Box.
    4. **Public Key ID** – Your
     Box public key ID.
    5. **Private Key** – The private key provided by
     Box.
    6. **Pass Phrase** – The pass phrase you use to
...
For more information, see [IAM role](box-connector.md#box-iam "box-connector.md#box-iam"). 12. **Configure VPC and security group –
_optional_** – Choose
...
13. In **Sync scope**, enter the following information:
    ...
    Box files are indexed by
    default. 2. For **Maximum file size** – Specify the file
    ...
    equal to 50MB. 3. **Additional configuration –
