...
4. Then, on the **Add data sources** page, from
   **Data sources**, add the **Microsoft Exchange** data source to your Amazon Q application.
...
See [Authorization](connector-concepts.md#connector-authorization "connector-concepts.md#connector-authorization") for more details. 9. **Authentication** – Choose between
**New** and **Existing**.
...
For more information, see [IAM role](exchange-connector.md#exchange-iam "exchange-connector.md#exchange-iam"). 11. In **Sync scope**, choose from the following options:

    * **UserIDs** – Choose to filter content by
     specific user email IDs.
    * **User email ID** – Upload a file with user
     email IDs to filter content. Format email IDs on separate lines in the file.
...
3. **Field mappings** – A list of data source document
