Amazon Q Business is no longer open to new customers. For capabilities similar to Q Business, explore Amazon Quick. [Learn more](qbusiness-availability-change.md "qbusiness-availability-change.md").

# Supported connectors

Amazon Q Business supports the following connectors:

- [Amazon Q Business custom data source connector](custom-connector.md "custom-connector.md")
- [Amazon Q Web Crawler](connector-webcrawler.md "connector-webcrawler.md")
- [Asana
  (_Preview_)](Asana-connector.md "Asana-connector.md")
- [Box](box-connector.md "box-connector.md")
- [Confluence (Cloud)](confluence-cloud-connector.md "confluence-cloud-connector.md")
- [Confluence (Server)](confluence-server-connector.md "confluence-server-connector.md")
- [Dropbox](dropbox-connector.md "dropbox-connector.md")
- [Amazon FSx Windows](fsx-windows-connector.md "fsx-windows-connector.md")
- [GitHub
  (Cloud)](github-cloud-connector.md "github-cloud-connector.md")
- [GitHub
  (Server)](github-server-connector.md "github-server-connector.md")
- [Gmail](gmail-connector.md "gmail-connector.md")
- [Google Calendar
  (_Preview_)](gcal-connector.md "gcal-connector.md")
- [Google
  Drive](google-connector.md "google-connector.md")
- [Jira](jira-connector.md "jira-connector.md")
- [Microsoft
  Exchange](exchange-connector.md "exchange-connector.md")
- [Microsoft
  OneDrive](onedrive-connector.md "onedrive-connector.md")
- [Microsoft SharePoint (Cloud)](sharepoint-cloud-connector.md "sharepoint-cloud-connector.md")
- [Microsoft SharePoint Server (Subscription Edition)](sharepoint-server-subscription-connector.md "sharepoint-server-subscription-connector.md")
- [Microsoft
  Teams](teams-connector.md "teams-connector.md")
- [Amazon S3](s3-connector.md "s3-connector.md")
- [Salesforce
  Online](salesforce-connector.md "salesforce-connector.md")
- [ServiceNow
  Online](servicenow-connector.md "servicenow-connector.md")
- [Slack](slack-connector.md "slack-connector.md")
- [Smartsheet](smartsheet-connector.md "smartsheet-connector.md")
- [Zendesk](zendesk-connector.md "zendesk-connector.md")
