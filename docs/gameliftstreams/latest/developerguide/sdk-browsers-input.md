...
| Client operating system | Browser                                                          | Input                                                                                                                     |
| ----------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
...
| Firefox                 | Keyboard, mouse, microphone, game controller (in Bluetooth mode) |
| Linux                   | Chrome, Edge, Firefox                                            | Keyboard, mouse, game controller                                                                                          |
...
- To support plug and play of game controllers on Proton, games running in Proton runtime environments will
  _always_ show a game controller connected, even if none are plugged in on the client. This could be an
  issue for games that prompt for controller input even when the controller is idle and unused. We recommend that games show
  input UI based on the last input method.

### Limitations

- Gamepad support on Ubuntu 22.04 LTS is available for applications that use the Simple DirectMedia Layer (SDL) libraries.
