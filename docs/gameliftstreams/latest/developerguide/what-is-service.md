...
+ [How to get started with Amazon GameLift Streams](#what-is-getstarted)
+ [Accessing Amazon GameLift Streams](#what-is-accessing)
...
+ Stream URLs to share a stream session, with no client integration required. A stream URL opens a stream session in a supported web browser, so end users can start streaming without your own web client and without AWS credentials of their own. For more information, see [Share stream sessions with stream URLs](stream-urls.md).

## How to get started with Amazon GameLift Streams
<a name="what-is-getstarted"></a>

If you're a first-time Amazon GameLift Streams user, we recommend that you begin with the following topics:
+ [Setting up Amazon GameLift Streams as a developer](setting-up.md) covers one-time setup tasks, including getting an AWS account with user access and setting up the software you need for development with Amazon GameLift Streams.
+ [Starting your first stream in Amazon GameLift Streams](streaming-process.md) guides you through the critical steps in the content streaming workflow. Starting with your content, such as a game build, you will provision Amazon GameLift Streams streaming cloud resources and initiate a streaming session. 
...
+ AWS Command Line Interface (AWS CLI) – Provides commands for a broad set of AWS services and is supported on Windows, Mac, and Linux. For more information on this tool, see the [AWS Command Line Interface page](https://aws.amazon.com/cli/).
