...
Passthrough is enabled or disabled at the output level: only in outputs that have an MPEG-2 TS container.
...
## Apple HLS
<a name="pass-through-or-removal-apple-hls"></a>
...
1. In the Profile or Event screen, go to the Output Groups section at the bottom of the screen and display the tab for Apple HLS Output Group. 
   ...
   1. In each output, open the PID Control section. You will note that the SCTE-35 field is automatically checked (because you set up for manifest decoration); it cannot be unchecked. 
