...
<a name="config-wrkr-srvr-cg-bkup"></a>
...
For steps to restore a database backup, see [Database Backups for AWS Elemental Server](config-wrkr-srvr-cg-bkup-chg.md)
...
1. In the **Cluster Tasks** section, the following fields configure the database backups:
   ...
   + **Path to store management database backups** indicates where AWS Elemental Server stores backups.
     ...
     The folder to receive backups must be the local disk or on a remote server that is mounted to the node. For assistance, see [Add Mount Points to AWS Elemental ServerNodes](config-wrkr-cf-cg-mount.md).
