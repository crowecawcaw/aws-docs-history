...
- For the REST API, users must include these additional HTTP headers in commands that they send:
  ...
  AWS Elemental Server REST API documentation.
...
For information about the different authentication options, see [User Authentication Reference](config-wrkr-srvr-cg-auth-ref.md "config-wrkr-srvr-cg-auth-ref.md") 8. At the prompts, create an admin API user name, email address, and password.
...
administrator API user, see [Authentication User Types](auth-ref-type-user.md "auth-ref-type-user.md") 9. For the prompt `Httpd must be restarted, which may interrupt REST
 commands. Restart now?`, type `Y`. 10. Create users through the node's web interface. For instructions, see [Add Users](config-wrkr-srvr-cg-users.md "config-wrkr-srvr-cg-users.md").
