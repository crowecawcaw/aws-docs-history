...
<a name="install-kvm-srvr-ig"></a>

This section is for IT administrators who perform first-time installation of AWS Elemental Server software on a KVM (kernel-based virtual machine).

**KVM Guest System Requirements**  
The resources available to your virtual machine (VM) determine the speed for encoding assets and the number of streams, bitrate, and possible types of encoding. Your VM guest should have, at minimum, the following resources allocated to it:
+ RAM: 16 GB
+ Disk space: 500 GB
+ CPU cores: 24
...
**Topics**
+ [Step A: Prepare Hardware and Download Files](install-kvm-srvr-ig-prep.md)
+ [Step B: Deploy the VM](install-kvm-srvr-ig-install-vm.md)
+ [Step C: Enable CPU Passthrough](install-kvm-srvr-ig-passthrough.md)
+ [Step D: Install the AWS Elemental Software](install-kvm-srvr-ig-install-sw.md)
+ [Step E: Set-up Licensing](install-kvm-srvr-ig-licensing.md)
+ [Step F: Complete Node Configuration](install-kvm-srvr-ig-complete.md)