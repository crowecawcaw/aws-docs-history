...
| `Does eth0 use DHCP to get its IP address?`                                        | Type `Yes` to use DHCP or type `No` to enter a static IP address.<br>If you plan to bond eth0 and eth1 (which you will set up in a later phase), we recommend that you enter a static IP address and set up eth0, eth1, and bond0 all on the same subnet. |
...
4. Start a web browser and start the AWS Elemental Server web interface by typing
   the following:
