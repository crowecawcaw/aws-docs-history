...
- Asset 1. One version of the data that is compatible with 2.18.0 or later.
...
- Asset 2. One version of the data that is compatible with 2.17.x and
  earlier. You might later decide to downgrade a node back to a version below
  2.18.0. When you restore the backup after you’ve installed RHEL 7 or CentOS
...
recommend that you follow it because the restore operation that you [later perform](migrate-server-218-install-restore.md "migrate-server-218-install-restore.md") might fail if the
