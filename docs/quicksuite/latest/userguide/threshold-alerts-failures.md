...
When an alert fails, Quick Suite sends you an email notification about the
...
- There is no data for the data tracked by the alert.
  When a failure occurs, Quick Suite sends you a notification and disables the
  ...
  deleted. Otherwise, Quick Suite attempts to check your data for threshold
  breaches again. After four failures, Quick Suite turns off the alert and
  notifies you that the alert is turned off. If the alert can be checked again,
  Quick Suite sends you a notification.
