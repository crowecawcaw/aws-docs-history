...
appropriate IP address range for the Quick Suite servers in that AWS Region.
...
Enabling connection from Quick Suite servers to your instance is just one of
...
###### To enable Amazon Quick Suite access to an Amazon EC2 instance
...
6. Create a new rule with the following values:
   ...
   - For **Source**, enter the CIDR address block for the
     AWS Region where you plan to use Amazon Quick Suite. For example, here is
     the CIDR address block for Europe (Ireland):
     `52.210.255.224/27`. For more information on the IP
     address ranges for Amazon Quick Suite in supported AWS Regions, see [AWS
     Regions, websites, IP address ranges, and endpoints](../../../quicksight/latest/user/regions.md "../../../quicksight/latest/user/regions.md").

   ###### Note

   If you have activated Amazon Quick Suite in multiple AWS Regions, you
   can create inbound rules for each Amazon Quick Suite endpoint CIDR. Doing
   this allows Amazon Quick Suite to have access to the Amazon RDS DB instance
   from any AWS Region defined in the inbound rules.

   An Amazon Quick Suite user or administrator who uses Amazon Quick Suite in
   multiple AWS Regions is treated as a single user. In other words,
   even if you are using Amazon Quick Suite in every AWS Region, both your
   Amazon Quick Suite account and your users are global.
