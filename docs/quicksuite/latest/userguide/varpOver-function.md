...
The `varpOver` function calculates the variance of the specified measure,
...
partitioned by `City` and `State`, based on a biased
population.
...
`Customer Region`, based on a biased population. The fields in the
