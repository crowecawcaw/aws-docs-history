...
The period to date computation evaluates values for a specified period to
date. For example, you can create a computation to find out how much you've
earned in year-to-date sales.
...
- `**periodGranularity**`
  – The period granularity for this computation
  (**MONTH**, **YEAR**, and so
  on).
...
2. For **Computation type**, choose **Period
   to date**, and then choose
   **Select**.
3. In the new insight that you created, add the time dimenstion and
