...
In the Enterprise edition of Amazon Quick Suite, you can restrict access to a dataset by
...
- If a field includes a comma (,) Amazon Quick Suite treats each word separated from another
...
can be shortened.
Amazon Quick Suite treats spaces as literal values. If you have a space in a field that you are
restricting, the dataset rule applies to those rows. Amazon Quick Suite treats both NULLs and blanks
...
If you are specifying groups, use only Amazon Quick Suite groups or Microsoft AD
groups.
...
Once datasets are flagged as rules datasets, Amazon Quick Suite will apply strict SPICE
...
with row-level security:
    ...
    * To prevent accidentally exposing sensitive information,
     Amazon Quick Suite skips empty RLS rules that grant access to everyone.
     An *empty RLS rule* occurs when all columns
     of a row have no value. Quick Suite RLS treats NULL, empty
...
outside of Amazon Quick Suite. For queries, the permissions are updated when the direct
