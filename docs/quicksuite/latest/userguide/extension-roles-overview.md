...
Beyond the broad user type categories, Amazon Quick Suite implements granular permission
...
Amazon Quick Suite administrators, or users granted admin privileges, establish the
...
Amazon Quick Suite authors to deploy extensions and end users to access them
safely.

###### Note

Creating and adjusting extension access requires IAM administrator
privileges. All administrators will see the extension links, but if not [signed in with IAM credentials](iam-credentials.md "iam-credentials.md"), you will need to sign in with
appropriate IAM permissions to manage extension access. This must be an IAM
user who is an Amazon Quick Suite administrator and not non-IAM admin users.
...
## Amazon Quick Suite author capabilities
...
## Amazon Quick Suite end user access
...
| Knowledge and actions        | • Access organizational knowledge bases and custom<br>agents through extension interfaces<br>• Perform external actions in third-party applications<br>using configured [action<br>connectors](action-connectors.md "action-connectors.md")<br>• Receive permissions-aware responses based on<br>individual access levels |
...
- **Cannot create extensions:** Your IAM user
  who is a Amazon Quick Suite administrator must first configure extension access
