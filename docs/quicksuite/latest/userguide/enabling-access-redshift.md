...
the data before the data is presented in a Amazon Quick Suite asset to the user. Amazon Quick Suite
...
identity propagation is supported when Amazon Quick Suite and Amazon Redshift use the same organization
...
- Anomaly detection
  For Amazon Quick Suite to connect to an Amazon Redshift instance, you must create a new security group
  for that instance. This security group contains an inbound rule that authorizes access
  from the appropriate IP address range for the Amazon Quick Suite servers in that AWS Region.
  To learn more about authorizing Amazon Quick Suite connections, see [Manually
...
Enabling connection from Amazon Quick Suite servers to your cluster is just one of several
prerequisites for creating a data set based on an AWS database data source. For more
information about what is required, see [Creating a dataset from a database](create-a-database-data-set.md "create-a-database-data-set.md").
...
Amazon Quick Suite assets that leverage a trusted identity propagation enabled data
...
automatically apply all data security rules to data consumers in Amazon Quick Suite. With
...
- Trusted identity propagation is only supported for Amazon Quick Suite
  accounts that are integrated with IAM Identity Center. For more information, see
  [Configure your Amazon Quick Suite account with
...
- An Amazon Redshift application that is integrated with IAM Identity Center. The Amazon Redshift cluster
  ...
  Amazon Quick Suite account that you want to use. The cluster must also be
  configured with the same organization instance in IAM Identity Center that your
  Amazon Quick Suite account is configured to. For more information about
...
propagation, configure Amazon Redshift OAuth scopes to your Amazon Quick Suite account.

To add a scope that allows Amazon Quick Suite to authorize identity propagation to
Amazon Redshift, specify the AWS account ID of the Amazon Quick Suite account and the service
...
Amazon Quick Suite to propagate user identities to. This information can be found in
the Amazon Redshift console. If you don't specify authorized targets for the Amazon Redshift
scope, Amazon Quick Suite authorizes users from any Amazon Redshift cluster that share the same
IAM Identity Center instance. The example below configures Amazon Quick Suite to connect to Amazon Redshift
...
The following example deletes OAuth scopes from a Amazon Quick Suite account.
...
1. Create a new dataset in Amazon Quick Suite. For more information about
...
14. Choose **Save rules** to save your new inbound
    rule.
15. In Amazon Quick Suite, navigate to the **Manage Amazon Quick Suite**
    menu.
...
20. In Amazon Quick Suite, choose **Datasets**, and then choose
    **New dataset**. Create a new dataset with the
    ...
    - The instance ID should auto populate with the VPC connection that
      you created in Amazon Quick Suite. If the instance ID doesn't auto
      populate, choose the VPC that you created from the dropdown
      list.
    - Enter the database credentials. If your Amazon Quick Suite account uses
...
Using Amazon Redshift Spectrum, you can connect Amazon Quick Suite to an external catalog with Amazon Redshift.
...
[Hive metastore](https://aws.amazon.com/blogs/big-data/migrate-external-table-definitions-from-a-hive-metastore-to-amazon-athena/ "https://aws.amazon.com/blogs/big-data/migrate-external-table-definitions-from-a-hive-metastore-to-amazon-athena/")), you can use Amazon Quick Suite to choose the external schema
...
To connect to Redshift Spectrum tables, you don't need to grant Amazon Quick Suite access to Amazon S3 or
Athena. Amazon Quick Suite needs access only to the Amazon Redshift cluster. For full details on
