...
query. Then, an Quick Suite administrator or a developer assigns the IAM Role to an
...
- Work with your Quick Suite administrator to learn if adding roles to data
...
- The ARN must correspond to an IAM role in the same AWS account as the
  Quick Suite instance that uses it.
- The IAM role must have a trust relationship allowing Quick Suite to assume
  the role.
- The identity that calls Quick Suite's APIs must have permission to pass
...
- The ARN string needs to match an existing IAM role in the AWS account and
  ...
  When Quick Suite connects to another service in AWS, it uses an IAM role. By
  default, this less granular version of the role is created by Quick Suite for each
...
After you verify your permissions, you can use the role in Quick Suite data
...
Amazon S3 data sources contain a manifest file that Quick Suite uses to find and
parse your data. You can upload a JSON manifest file through the Quick Suite
console, or you can provide a URL that points to a JSON file in an S3 bucket. If you
choose to provide a URL, Quick Suite must be granted permission to access the file
in Amazon S3. Use the Quick Suite administration console to control access to the
...
Amazon Quick Suite when you create a new Amazon S3 dataset or place the file into the Amazon S3 bucket
...
to Quick Suite, create or update an existing role in IAM that grants
