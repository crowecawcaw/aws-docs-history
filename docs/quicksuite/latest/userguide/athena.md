...
types of data by using AWS Lambda. Using a connection from Quick Suite to Athena,
...
The Amazon Resource Name (ARN) for the Amazon Quick Suite role has the format
`arn:aws:iam::<account
...
- For Amazon Athena, Amazon S3, and Athena Query Federation connections, Amazon Quick Suite uses
...
- If you assigned scope-down policies to your users, verify that the policies
  ...
  information about assigning IAM policies to your users in Amazon Quick Suite, see
...
###### To authorize Amazon Quick Suite to connect to Athena or Athena federated data
...
2. Open your profile menu at top right and choose **Manage
   QuickSight**. You must be a Amazon Quick Suite administrator to do this.
...
region of Amazon Quick Suite. 7. To confirm your changes, choose **Finish**.
...
1. From the Amazon Quick Suite start page, choose **Datasets**,
   **New dataset**.
...
an Athena query, an error displays directing you to contact a Amazon Quick Suite
administrator. This error means need to recheck your connection authorization
settings to find the discrepancy. 5. After you can connect successfully, you or your Amazon Quick Suite authors can
create data sources connections and share them with other Amazon Quick Suite authors.
The authors can then create multiple datasets from the connections, to use in
Amazon Quick Suite dashboards.

For troubleshooting information on Athena, see [Connectivity issues when using Athena with Amazon Quick Suite](../../../quicksight/latest/user/troubleshoot-athena.md "../../../quicksight/latest/user/troubleshoot-athena.md").
...
When administrators configure Quick Suite, Athena, Amazon S3 Access Grants, and
...
is accessed from Quick Suite by an IAM Identity Center user, Athena or Lake Formation can make
...
- Configure your Quick Suite account with IAM Identity Center. Trusted identity
  propagation is only supported for Quick Suite accounts that are
  integrated with IAM Identity Center. For more information, see [Configure your Amazon Quick Suite
...
in a Quick Suite account that uses IAM Identity Center.
...
- An Athena workgroup that is enabled with IAM Identity Center. The Athena workgroup
  ...
  Quick Suite account. For more information about configuring an
...
- Permissions to data must be managed with Lake Formation and Lake Formation must be
  configured with the same IAM Identity Center instance as Quick Suite and the
...
- The data lake administrator needs to grant permissions to IAM Identity Center users
  ...
  _AWS Lake Formation Developer Guide_.
- The Quick Suite administrator needs to authorize connections to
  ...
  Quick Suite role Amazon S3 bucket permissions or AWS KMS permissions. You
...
To use trusted identity propagation with Athena, your Quick Suite account
must have the required permissions to access your resources. To provide those
permissions, you must configure your Quick Suite account to use an IAM
role with the permissions.

If your Quick Suite account is already using a custom IAM role, you
...
your Quick Suite account to use the IAM role

After configuring the IAM role in the previous step, you must configure your
Quick Suite account to use it. For information about how to do that, see
[Using existing IAM roles in
Quick Suite](security-create-iam-role.md#security-create-iam-role-use "security-create-iam-role.md#security-create-iam-role-use").

### Update the

identity propogation config with the AWS CLI

To authorize Quick Suite to propagate end user identities to Athena
...
Athena dataset in Quick Suite

Now, create an Athena dataset in Quick Suite configured with the IAM Identity Center
...
identity propagation with Quick Suite and Athena.

- Quick Suite Athena data sources that use trusted identity
...
- When using Athena data sources that use trusted identity propagation,
  ...
  you elect to use Quick Suite’s scope down policy feature, scope
