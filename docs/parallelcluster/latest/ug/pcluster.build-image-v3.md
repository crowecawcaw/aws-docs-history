...
When `true`, the command performs validation without creating any resources. You can use this to validate the image
...
When `true`, automatically initiates an image stack rollback on failure. (Defaults to
`false`.)
...
`pcluster build-image` uses the default VPC. If the default VPC has been deleted, perhaps by using
AWS Control Tower or AWS Landing Zone, then the subnet ID must be specified in the image configuration file. For more
information, see [SubnetId](HeadNode-v3.md#yaml-HeadNode-Networking-SubnetId "HeadNode-v3.md#yaml-HeadNode-Networking-SubnetId").
