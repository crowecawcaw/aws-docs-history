...
The AWS ParallelCluster UI (PCUI) is a web-based user interface that mirrors the AWS ParallelCluster `pcluster` CLI, while providing a console-like experience. You install and access the PCUI in your AWS account. When you run it, the PCUI accesses an instance of the AWS ParallelCluster API hosted on Amazon API Gateway in your AWS account. For more information about the PCUI, see [AWS ParallelCluster UI](pcui-using-v3.md).

**Prerequisites:**
+ You must have an AWS account
+ You must have access to the AWS Management Console

For more information, see [Sign up for an AWS account](install-v3.md#sign-up-for-aws).

**Topics**
+ [Install the PCUI](#install-pcui-steps-v3)
+ [Stack parameters](#install-pcui-steps-v3-stack-params)
+ [Configure a custom domain](tutorials_08_custom_domain-v3.md)
+ [Amazon Cognito user pool options](install-pcui-cognito-v3.md)
+ [Identify the AWS ParallelCluster and PCUI version](install-pcui-version-v3.md)
+ [PCUI costs](install-pcui-costs-v3.md)
...
1. Deploy the PCUI by choosing an AWS Region quick-create link from the list here. This takes you to the CloudFormation **Create Stack Wizard** in the console.
...
1. Enter a valid email address for **Admin's Email** and a **ParallelCluster version**.
...
1. Keep the rest of the form blank or enter values for (optional) parameters to customize the PCUI build.

1. Note the stack name for use in later steps.

1. Navigate to **Capabilities**. Agree to the CloudFormation capabilities.

1. Choose **Create**. It takes about 15 minutes to complete the AWS ParallelCluster API and PCUI deployment.
...
1. In the CloudFormation console list of stacks, choose the link to the stack name that you noted in a previous step.

1. In **Stack details**, choose **Outputs** and select the link for the key named **{{Stackname}}URL** to open the PCUI (where **{{Stackname}}** is the name that you noted in a previous step).

1. Enter the temporary password. Follow the steps to create your own password and log in again.

1. You are now on the home page of the PCUI in the AWS Region that you selected.

1. To get started using the PCUI, see [Configure and create a cluster with the PCUI](configure-create-pcui-v3.md).
