...
If set to `True`, AWS ParallelCluster performs the request validation without creating any resources. You can use this to validate the image
configuration. The default is `False`.

**`rollback_on_failure`**

If set to `True`, AWS ParallelCluster automatically initiates an image stack rollback on failures. The default is
`False`.
