...
**Topics**
+ [Seeing `Node bootstrap error` in `clustermgtd.log`](compute-node-initialization-bootstrap-error-v3.md)
+ [I configured on demand capacity reservations (ODCRs) or zonal Reserved Instances](compute-node-initialization-odcr-v3.md)
+ [Seeing `An error occurred (VcpuLimitExceeded)` in `slurm_resume.log` when I fail to run a job, or in `clustermgtd.log`, when I fail to create a cluster](compute-node-initialization-vpc-limit-v3.md)
+ [Seeing `An error occurred (InsufficientInstanceCapacity)` in `slurm_resume.log` when I fail to run a job, or in `clustermgtd.log`, when I fail to create a cluster](compute-node-initialization-ice-failure-v3.md)
