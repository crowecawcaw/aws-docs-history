...
You can view your queue and check the status of the job with the `squeue` command. Because you didn't specify a specific queue, the default queue (`spot`) is used. For more information about `squeue`, see [squeue](https://slurm.schedmd.com/squeue.html) in the *Slurm documentation*.
