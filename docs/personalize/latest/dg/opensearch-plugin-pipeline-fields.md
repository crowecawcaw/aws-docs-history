...
+  **recipe (required)** – Specify the name of the Amazon Personalize recipe to use. To use the plugin, you can specify `aws-personalized-ranking-v2` or `aws-personalized-ranking`. 
...
+ **iam\_role\_arn (required for OpenSearch Service, optional for open source OpenSearch)** – For OpenSearch Service, provide the Amazon Resource Name (ARN) for the role that you created when [setting up permissions](opensearch-granting-access-managed.md) for OpenSearch Service to access your Amazon Personalize resources. If your OpenSearch Service and Amazon Personalize resources exist in different accounts, specify the role that grants `AssumeRole` permissions for OpenSearch Service. For more information, see [Configuring permissions when resources are in different accounts](configuring-multiple-accounts.md).
