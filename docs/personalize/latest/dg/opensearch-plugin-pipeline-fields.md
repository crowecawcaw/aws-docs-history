...
- **campaign\_arn (required)** – Specify the Amazon Resource Name (ARN) of the Amazon Personalize
  campaign to use to personalize results.
- **item\_id\_field (optional)** – If the `_id` field for an indexed
...
- **aws\_region (required)** – The AWS Region where you created your Amazon Personalize
  campaign.
- **ignore\_failure (optional)** – Specify whether the plugin ignores any
...
- **external\_account\_iam\_role\_arn** – If you use OpenSearch Service, and your Amazon Personalize and OpenSearch Service
