...
- ITEM_ID `string`
- TIMESTAMP (`long`)
- EVENT_TYPE (`string` and depending on [use case](domain-use-cases.md "domain-use-cases.md"), `Watch` and `Click` event types)
...
the default schema for Item interactions datasets for VIDEO_ON_DEMAND domains, see [Default Interactions schema (VIDEO_ON_DEMAND domain)](#VIDEO-ON-DEMAND-interactions-schema "#VIDEO-ON-DEMAND-interactions-schema").

Optionally add the
reserved keyword EVENT_VALUE if you have value data for events, such as the percentage of a video watched. Optionally add the reserved keyword IMPRESSION if you want to include explicit and implicit impressions data.
...
For more information on minimum requirements and maximum data limits for an Item interactions dataset for the VIDEO_ON_DEMAND domain, see [Service quotas](limits.md#limits-table "limits.md#limits-table").
