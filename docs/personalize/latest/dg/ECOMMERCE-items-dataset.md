...
Use reserved keywords CATEGORY\_L2 and CATEGORY\_L3 for items with multiple multi-level categories.
...
For items with multiple categories, separate each value with the vertical bar, '|'. For example, for a CATEGORY\_L1 field your data for an item might be
...
CATEGORY\_L1, CATEGORY\_L2, CATEGORY\_L3. This allows you filter recommendations based on sub-categories, even if an item belongs to multiple multi-level categories.
