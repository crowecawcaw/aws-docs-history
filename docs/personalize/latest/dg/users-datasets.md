...
**Topics**
+ [User data requirements](#user-data-requirements)
+ [Categorical metadata](#user-categorical-data)
+ [Non-categorical string data](#user-string-data)
+ [Users metadata example](#users-data-example)
...
+ Your data must have an USER\_ID column that stores the unique identifier for each user. Every user must have an user ID. It must be a `string` with a max length of 256 characters.
