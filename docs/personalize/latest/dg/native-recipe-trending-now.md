...
| `Trend discovery frequency`                | Specify how often Amazon Personalize evaluates your interactions data and identifies trending items. For<br>example, if you specify `30 minutes` for `Trend discovery frequency`, every 30<br>minutes Amazon Personalize identifies items with the greatest rate of increase in interactions over 30-minute<br>intervals.<br>Available frequencies include 30 minutes, 1 hour, 2 hours, 3 hours, and 1 day. Choose a frequency that<br>aligns with the distribution of your interactions data. Missing data over the interval you choose<br>can reduce recommendation accuracy. If you use the CreateSolution API operation and don't specify a value,<br>the default is every 2 hours.<br>NoteFor any parameter values that are greater than 2 hours,<br>Amazon Personalize automatically refreshes the trending item<br>recommendations every 2 hours to account for new interactions<br>and new items.<br>Default value: 2 hours<br>Possible values: 30 minutes, 1 hour, 2 hours, 3 hours, and 1 day.<br>Value type: String<br>HPO tunable: No |

## Creating a solution (SDK for Python (Boto3))

The following code shows how to create a solution with the Trending-Now recipe using the SDK for Python (Boto3).
Possible values for `trend_discovery_frequency` are `30 minutes`, `1 hour`,
`3 hours`, and `1 day`. For information about creating a solution with the console,
see [Creating a solution (console)](create-solution.md#configure-solution-console "create-solution.md#configure-solution-console").
