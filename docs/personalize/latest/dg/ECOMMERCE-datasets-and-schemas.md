...
+ [Item interactions dataset requirements (ECOMMERCE domain)](ECOMMERCE-interactions-dataset.md)
...
| --- | --- | --- | 
| Item interactions ([default schema](ECOMMERCE-interactions-dataset.md#ECOMMERCE-interactions-schema)) | USER\_ID (`string`)<br />ITEM\_ID (`string`)<br />TIMESTAMP (`long`)<br />EVENT\_TYPE (`string` and depending on [use case](domain-use-cases.md), `Purchase` and `View` event types) | EVENT\_VALUE (`float`, `null`)<br />IMPRESSION (`string`, `null`)<br />RECOMMENDATION\_ID (`string`, `null`)<br />EVENT\_ATTRIBUTION\_SOURCE (`string`, `null`) | 
| Users ([default schema](ECOMMERCE-users-dataset.md#ECOMMERCE-users-dataset-schema)) | USER\_ID (`string`)<br />1 metadata field (categorical `string` or numerical) |   | 
| Items ([default schema](ECOMMERCE-items-dataset.md#ECOMMERCE-items-dataset-schema)) | ITEM\_ID (`string`)<br />PRICE (`float`)<br />CATEGORY\_L1 (categorical `string`) | CATEGORY\_L2 (categorical `string`, `null`)<br />CATEGORY\_L3 (categorical `string`, `null`)<br />PRODUCT\_DESCRIPTION (textual `string`, `null`)<br />CREATION\_TIMESTAMP (`long`)<br />AGE\_GROUP (categorical `string`, `null`)<br />ADULT (categorical `string`, `null`)<br />GENDER (categorical `string`, `null`) | 