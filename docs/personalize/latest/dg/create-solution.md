...
1. On the **Training configuration** page, customize the solution to meet your business requirements. 
   ...
   + In **Event configuration**, if your Item interactions dataset has EVENT\_TYPE or both EVENT\_TYPE and EVENT\_VALUE columns, optionally use the **Event type** and **Event value threshold** fields to choose the item interactions data that Amazon Personalize uses when training the model. For more information, see [Choosing the item interaction data used for training](event-values-types.md). 
