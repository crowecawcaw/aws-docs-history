...
When Amazon DocumentDB creates a cluster in a VPC, it assigns a network interface to the cluster by using an IP address from your subnet group. Always use the Domain Name System (DNS) name to connect to your cluster because the underlying IP address changes during failover.
...
Dual-stack mode is supported on Amazon DocumentDB version 4.0, 5.0 and 8.0. If you are unable to access dual-stack mode on any of these versions, make sure that you are running the latest engine patch version on your cluster.
