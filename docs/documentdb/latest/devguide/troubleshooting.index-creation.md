...
Foreground index builds do not exhibit the same blocking behavior. Instead, foreground index builds take an exclusive lock on the collection until the index build is completed. To create indexes on an empty collection and avoid blocking long-running queries, use foreground index builds.
