...
**Topics**
+ [Managing document compression](#manage-compression)
+ [Monitoring document compression](#monitoring-compression)
...
Note that the compression threshold can only be set to a value between 128 to 8000 bytes. Additionally, the `enable` option needs to be set to “true” when specifying the compression threshold.
...
View compression statistics by running the `db.printCollectionStats()` or `db.collection.stats()` command from the mongo shell. The output shows you the original size and compressed size that you can compare to analyze the storage gains from document compression. In this example, statistics for a collection named “sample\_collection” are shown below. A scaling factor of 1024\*1024 is used below to output the `size` and `storageSize` values in MB.
