...
**Topics**
+ [Amazon DocumentDB elastic cluster sharding](#elastic-how-it-works.sharding)
+ [Elastic cluster migration](#elastic-how-it-works-migration)
+ [Elastic cluster scaling](#elastic-how-it-works-scaling)
+ [Elastic cluster reliability](#elastic-reliability)
+ [Elastic cluster storage and availability](#how-it-works-storage)
+ [Functional differences between Amazon DocumentDB 4.0 and elastic clusters](#elastic-functional-differences)
...
Amazon DocumentDB supports migrating MongoDB sharded data to elastic clusters. Offline, online, and hybrid migration methods are supported. For more information, see [Migrating and upgrading Amazon DocumentDB](docdb-migration.md).
...
+ Results from `top` and `collStats` are partitioned by shards. For sharded collections, data is distributed among multiple partitions and `collStats` reports aggregated `collScans` from the partitions.
