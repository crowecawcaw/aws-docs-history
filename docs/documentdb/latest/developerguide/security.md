...
As a managed service, Amazon DocumentDB is protected by AWS global network security. For
...
You use AWS published API calls to access Amazon DocumentDB through the network. Clients must
...
- Cipher suites with perfect forward secrecy (PFS) such as DHE (Ephemeral
  ...
  You can call these API operations from any network location.
  You can use Amazon DocumentDB policies to control access from specific Amazon Virtual Private Cloud (Amazon VPC) endpoints or specific VPCs.
  Effectively, this isolates network access to a given Amazon DocumentDB resource from only the specific VPC within the AWS network.

###### Note

Amazon DocumentDB does not support resource-based access policies.
