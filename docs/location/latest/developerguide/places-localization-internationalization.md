...
<a name="places-localization-internationalization"></a>
...
All name translations and alternative names for the requested address fields in all available languages. Specify which components to translate using the `AddressTranslations` request parameter. Valid values are `District`, `Locality`, `Region`, and `SubRegion`.
