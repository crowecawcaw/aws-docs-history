...
**Core**  
 This pricing bucket supports the travel modes `Car` and `Truck` without toll cost calculation, and `Pedestrian` and `Transit`. 
...
**Premium**  
 This pricing bucket supports toll cost calculation and the `Intermodal` travel mode. You will be charged at Premium price when you request toll cost calculation by setting the request parameters `LegAdditionalFeatures["Tolls"]` or `SpanAdditionalFeatures["TollSystems"]`, or when using the `Intermodal` travel mode. 
