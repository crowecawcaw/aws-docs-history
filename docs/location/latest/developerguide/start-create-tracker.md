...
By default, you will get EventBridge events each time a tracked device enters or exits a linked geofence. For more information, see [Link a tracker to a geofence collection](associate-consumer.md).
...
1. Under **Position filtering**, choose the option that best fits how you intend to use your tracker resource. If you do not set **Position filtering**, the default setting is `TimeBased`. For more information, see [Amazon Location Service trackers](trackers.md) in this guide, and `[PositionFiltering](https://docs.aws.amazon.com/location/latest/APIReference/API_WaypointTracking_CreateTracker.html#location-WaypointTracking_CreateTracker-request-PositionFiltering)` in the Amazon Location Service Trackers API Reference.

1. (Optional) Under **Tags**, enter a tag **Key** and **Value**. This adds a tag your new geofence collection. For more information, see [How to use tags](manage-resources.md#manage-resources_how-to).
...
Use the `[CreateTracker](https://docs.aws.amazon.com/location/latest/APIReference/API_WaypointTracking_CreateTracker.html)` operation from the Amazon Location Trackers APIs.
...
For information on using the Bounding Polygon Queries feature, see [List your device positions](list-device-positions.md)
