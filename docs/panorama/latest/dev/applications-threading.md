...
You can run your application logic on a processing thread and use other threads for other background processes.
For example, you can create a thread that [serves HTTP traffic](applications-ports.md "applications-ports.md") for
debugging, or a thread that monitors inference results and sends data to AWS.

To run multiple threads, you use the [threading
module](https://docs.python.org/3/library/threading.html "https://docs.python.org/3/library/threading.html") from the Python standard library to create a thread for each process. The following example shows
the main loop of the debug server sample application, which creates an application object and uses it to run
three
threads.
...
When all of the threads exit, the application restarts itself. The `run_cv` loop processes images
from camera streams. If it receives a signal to stop, it shuts down the debugger process, which runs an HTTP server
...
Threads communicate via the application's `self` object. To restart the application processing loop,
